#!/usr/bin/env bash
# Outbox-based remote XO backlog handoff and dropped-link recovery.
set -u
...
#   rm: cannot remove '/tmp/sq-remote-handoff.XXXXXX': Directory not empty
...
case "${SQUAD_FAKE_SSH_MODE:-normal}:$command_name" in
  unreachable:*) exit 255 ;;
  serialize:sq-backlog-receive.sh)
    if mkdir "$SQUAD_FAKE_SERIALIZE_ONCE" 2>/dev/null; then
      touch "$SQUAD_FAKE_SERIALIZE_ENTERED"
      while [ ! -f "$SQUAD_FAKE_SERIALIZE_RELEASE" ]; do sleep 0.02; done
    fi
    exec "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@"
    ;;
  after-put:sq-remote-file.sh)
    "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  after-receive:sq-backlog-receive.sh)
    "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@"
    exit 255
    ;;
  *) exec "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@" ;;
...
handoff_env() {
  ...
  SQUAD_FAKE_SERIALIZE_ONCE="$TMP_ROOT/serialize.once" \
  SQUAD_FAKE_SERIALIZE_ENTERED="$TMP_ROOT/serialize.entered" \
  SQUAD_FAKE_SERIALIZE_RELEASE="$TMP_ROOT/serialize.release" \
...
: > "$SSH_COUNT"
set +e
SQUAD_FAKE_SSH_MODE=after-receive handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios ios-a ios-b \
...
out=$(handoff_env "$ROOT/bin/sq-backlog-handoff.sh" --resume-pending)
...
: > "$SSH_COUNT"
set +e
SQUAD_FAKE_SSH_MODE=after-put handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios transfer-cut \
...
handoff_env "$ROOT/bin/sq-backlog-handoff.sh" --resume-pending >/dev/null \
...
SQUAD_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios serialized-a \
...
SQUAD_FAKE_SSH_MODE=serialize handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios serialized-b \
...
handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios stale-lock-item >/dev/null \
...
set +e
SQUAD_FAKE_SSH_MODE=unreachable handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios pending-offline \
...
SQUAD_HOME="$PARENT" /bin/bash -c '
  ...
  fm_lock_release "$3"
  fm_lock_release "$2"
' _ "$ROOT/bin/sq-stand-to-lib.sh" "$registry_lock" "$handoff_lock" \
  "$TMP_ROOT/route.entered" "$TMP_ROOT/route.release" "$PARENT/data/XOs.md" &
route_holder_pid=$!
route_wait=0
...
handoff_env "$ROOT/bin/sq-backlog-handoff.sh" ios route-race \
