...
func daemonStartTimeout() time.Duration {
	...
	return durationFromEnv("DRILL_TEST_DAEMON_START_TIMEOUT", 45*time.Second)
}
...
func daemonStopTimeout() time.Duration {
	...
	return durationFromEnv("DRILL_TEST_DAEMON_STOP_TIMEOUT", fallback)
}

func daemonStartPollInterval() time.Duration {
	return durationFromEnv("DRILL_TEST_DAEMON_START_POLL_INTERVAL", 100*time.Millisecond)
}
...
func Start(p *paths.Paths) error {
	...
	// alive under a different path spelling (symlinked or relative DRILL_HOME),
...
func startDetachedDaemon(p *paths.Paths) error {
	...
	cmd.Env = upsertEnv(os.Environ(), "DRILL_HOME", p.Root())
	cmd.Stdout = logFile
	cmd.Stderr = logFile
...
// logs, and reaping its bootstrap log-sink child. The DRILL_HOME singleton lock
...
func EnsureDaemon(p *paths.Paths) error {
	alive, err := daemonHealthCheck(p)
	if err != nil {
		return fmt.Errorf("%w (run 'drill daemon start' to recover)", err)
	}
	if alive {
		return nil
	}
