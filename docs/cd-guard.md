...
`bin/sq-cd-command-policy.mjs` is the single decision owner.
...
It is one member of a family of primary-session guards that share the same cross-harness hook machinery:
the sentry-arm, delegation, backend-raw, poll-loop, and turn-end guards, alongside this cd guard.
The sentry-arm and turn-end guards have dedicated reference pages in [`arm-pretool-check.md`](arm-pretool-check.md) and [`turnend-guard.md`](turnend-guard.md); the backend-raw and poll-loop policies are owned by their script headers (`bin/sq-backend-pretool-check.sh` and `bin/sq-poll-pretool-check.sh`).
...
Its threat model is agent mistakes, the same as the sentry-arm seatbelt: an accidental bare `cd projects/foo`, not a deliberately obfuscated bypass.

## Scope: plain Squad checkouts only

The guard fires only in a plain Squad checkout where git-dir equals git-common-dir.
It is a silent no-op (exit 0, no output) everywhere else, so it never interferes with an operator or recon that legitimately works inside its own project or Squad task worktree.

`bin/sq-cd-pretool-check.sh` owns its checkout detection; the turn-end guard's marker-aware scope is a separate contract (`docs/turnend-guard.md`).
A plain, non-worktree checkout has `git rev-parse --git-dir` equal to `git rev-parse --git-common-dir`.
An operator or recon task worktree - the shape `bin/sq-spawn.sh` always hands out - is a linked git worktree where the two differ, so the guard is inert there.
...
- Malformed or untokenizable syntax fails open (allow). Unlike the sentry-arm seatbelt, which fails closed on unclassifiable protected commands, the cd-guard prioritizes zero false blocks over catching a malformed bypass, because a blocked backlog write is a correctness hazard while a missed exotic `cd` is only the pre-existing status quo.
...
`bin/sq-cd-pretool-check.sh` supports all five harness-engine entry shapes used by the tracked adapters, with pi-signed sharing Pi's shape:
...
The quoting-decoder marker set is coupled to the classifier's decoder set in `bin/sq-arm-command-policy.mjs`: adding any new quote or expansion form the classifier decodes requires extending the prefilter marker set in the same change, or it stops being a strict superset.
...
`bin/sq-cd-command-policy.mjs` imports the shell tokenizer and command-position analysis (`Lexer`, `splitProgram`, `commandPosition`) from `bin/sq-arm-command-policy.mjs`, the sole owner of Squad's shell classification.
...
`bin/sq-arm-command-policy.mjs` runs its own CLI entry point only when invoked directly, never on import, so the two policies stay independent CLIs over one parser.
...
| Codex | `.codex/hooks.json` PreToolUse hook that anchors from `pwd -P`, verifies the hook-loaded Squad root, and forwards the payload | Blocks on exit 2 and displays stderr. |
| Grok | `.grok/hooks/sq-primary-cd-check.json` PreToolUse hook anchored on `${GROK_WORKSPACE_ROOT:-}` | Consumes the stdout `decision=deny` object. |
| OpenCode | `.opencode/plugins/sq-primary-cd-check.js` `tool.execute.before` | Throws, which surfaces as the failed tool result. |
| Pi | `.pi/extensions/sq-primary-turnend-guard.ts` `tool_call` handler | Returns `{block: true}`; piggybacks on the already-loaded primary extension so no extra `-e` flag is needed. |

Each harness runs the cd-guard alongside the sentry-arm seatbelt; the two are independent checks, and either deny blocks the command.
...
`tests/sq-cd-pretool-check.test.sh` owns the acceptance matrix.
...
The suite also proves the end-to-end cwd-leak regression (a Squad-owned backlog write leaking into a project clone, then denied at the exact command), the checkout scoping (fires in a git-cloned XO fixture, inert in an operator/recon linked worktree, inert outside a Squad checkout, inert outside a git repo), the fail-open transport behavior, the prefilter fast path, the policy CLI output contract, and the per-harness wiring.
...
Each harness ran against a scratch primary-shaped Squad checkout: a plain git repo with `AGENTS.md`, `bin/` holding the real `sq-cd-pretool-check.sh`, `sq-cd-command-policy.mjs`, and `sq-arm-command-policy.mjs` plus a no-op dummy `sq-arm-pretool-check.sh`, a `projects/foo/` stand-in clone, and the tracked harness hook config.
No live sentry, unit state, or the commander's real primary checkout was involved.
...
- **Claude Code 2.1.207** - blocked. Claude reported the top-level command "denied by the `PreToolUse` hook (`sq-cd-pretool-check.sh`)", the `BLOCKED` sentinel was absent, and the subshell form was permitted to run. A prior control `touch` proved the harness executed commands.
...
- **grok 0.2.93** - inconclusive live run: the Grok Build API returned `402 Payment Required: Grok Build usage balance exhausted`, so the model never issued the probe commands. The grok cd hook (`.grok/hooks/sq-primary-cd-check.json`) is structurally identical to the arm-seatbelt grok hook already live-validated on 2026-07-09 (`docs/arm-pretool-check.md`) - same `${GROK_WORKSPACE_ROOT:-}` anchoring and same PreToolUse deny consumption - and the grok-shaped stdin path (`.toolInput.command` in, `{"decision":"deny"}` out) is covered by `tests/sq-cd-pretool-check.test.sh`. Re-run once the Grok balance is restored to close the live gap.
