#!/usr/bin/env bash
# tests/sq-backend-zellij-smoke.test.sh - real zellij smoke test for the
# zellij session-provider adapter (bin/backends/zellij.sh), P3 of
# data/sq-backend-design-d7 (report.md "Zellij Backend"). Mirrors
# tests/sq-backend-herdr-smoke.test.sh's structure: every other suite fakes
# the CLI, this one talks to a REAL zellij server - but ALWAYS on a private,
# named, throwaway session (via SQUAD_ZELLIJ_SESSION, never the real "Squad"
# session name), so it never touches a commander's real zellij usage. Skips
...
# delete-all-sessions - the same unit-safety discipline PR #199 established
# for herdr.
set -u
...
SESSION="sq-backend-smoke-$$"
export SQUAD_ZELLIJ_SESSION="$SESSION"
...
LONG_CWD="$TMP_CWD/sq-zellij-wrap-$$/aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa/bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb/cccccccccccccccccccccccccccccccccccccccc/dddddddddddddddddddddddddddddddddddddddd"
...
if fm_backend_zellij_send_key "$TARGET" Escape "sq-not-$LABEL" >/dev/null 2>&1; then
...
fm_backend_zellij_send_literal "$TARGET" 'echo literal-then-key-commander' \
...
fm_backend_zellij_send_text_line "$TARGET" "echo commander-on-deck-line" \
...
# The load-bearing case: a NESTED SUBSHELL's own cd (exactly what `fob
...
# pwd-probe (docs/zellij-backend.md) is what sq-spawn.sh's worktree-discovery
...
[ "$p2" = "$TMP_CWD" ] || fail "real zellij: current_path did not track a nested subshell's own cd (the fob-get-shaped case), got '$p2'"
pass "real zellij: current_path tracks a NESTED SUBSHELL's own cd (the fob-get-shaped case a bare pane_cwd read cannot see)"
...
# dispatcher (bin/sq-backend.sh) falls through to "unknown" for any backend
...
pass "real zellij: fm_backend_busy_state reports unknown (D5 - sentry falls back to pane-regex, same as tmux)"
...
pass "real zellij: list_live discovers a live task tab by sq-<id> name"
