#!/usr/bin/env bash
# Behavior tests for sq-spawn.sh concrete dispatch profile flags.
#
# These tests drive sq-spawn through meta writing and launch construction with a
...
# command Squad would run without starting any real harness.
set -u
...
TMP_ROOT=$(fm_test_tmproot sq-spawn-dispatch-profile)
...
SH
  ...
  fm_fake_exit0 "$fakebin" fob pi-signed
  printf '%s\n' "$fakebin"
}
...
run_spawn() {
  ...
  shift 4
  : > "$launchlog"
  # CLAUDE_CONFIG_DIR is forwarded onto claude launches by sq-spawn, so pin it
...
test_no_profile_keeps_claude_profile_defaults() {
  ...
  expected="CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude --dangerously-skip-permissions \"\$('${ROOT}/bin/sq-operational-input.sh' encode launch-brief < '$HOME_DIR/data/$id/brief.md')\""
...
test_relative_home_overrides_launch_with_absolute_cross_process_paths() {
  ...
  assert_contains "$launch" "-e '$home_real/state/$id.pi-ext.ts'" \
    "relative SQUAD_STATE_OVERRIDE leaked into Pi's cross-process extension path"
  assert_contains "$launch" "< '$home_real/data/$id/brief.md'" \
    "relative SQUAD_DATA_OVERRIDE leaked into the cross-process brief path"
...
test_absolute_override_spelling_is_preserved_in_launch_paths() {
  ...
  assert_contains "$launch" "-e '$linked_home/state/$id.pi-ext.ts'" \
    "absolute SQUAD_STATE_OVERRIDE spelling changed in Pi's cross-process extension path"
  assert_contains "$launch" "< '$linked_home/data/$id/brief.md'" \
    "absolute SQUAD_DATA_OVERRIDE spelling changed in the cross-process brief path"
...
test_unresolvable_relative_overrides_fail_loudly() {
  ...
  assert_contains "$out" "SQUAD_STATE_OVERRIDE directory cannot be resolved: missing-state" \
    "spawn did not name the unresolvable SQUAD_STATE_OVERRIDE"
  ...
  assert_contains "$out" "SQUAD_DATA_OVERRIDE directory cannot be resolved: missing-data" \
    "spawn did not name the unresolvable SQUAD_DATA_OVERRIDE"
...
test_active_dispatch_profile_requires_explicit_harness_for_scout() {
  ...
  pass "active crew-dispatch profile requires an explicit harness for recon spawns"
}
...
test_grok_omits_invalid_max_reasoning_effort() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "grok --always-approve --model 'grok-4' \"\$('${ROOT}/bin/sq-operational-input.sh' encode launch-brief < " \
...
test_pi_signed_threads_shared_pi_profile_and_preserves_identity() {
  ...
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "SQUAD_PI_HARNESS=pi-signed pi-signed --model 'openai-codex/gpt-5.6-sol' --thinking 'max' -e" \
  ...
  assert_contains "$(cat "$HOME_DIR/state/$id.busy-state")" "state=busy source=sq-spawn" \
...
test_pi_signed_persistent_XO_uses_pi_extensions_and_identity() {
  ...
  id=profile-pi-signed-XO-z8d
  rec=$(make_spawn_case profile-pi-signed-XO codex "$id")
  read_case_record "$rec"
  printf '%s\n' pi-signed > "$HOME_DIR/config/xo-harness"
  sm="$CASE_DIR/XO-home"
  make_seeded_XO_home "$sm" "$id"
  sm=$(cd "$sm" && pwd -P)

  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$sm" --xo)
  status=$?
  expect_code 0 "$status" "pi-signed persistent XO spawn should succeed"
  assert_contains "$out" "spawned $id harness=pi-signed kind=xo" \
    "pi-signed XO spawn did not preserve its runtime identity"
  assert_meta_profile "$HOME_DIR/state/$id.meta" pi-signed default default
  launch=$(cat "$LAUNCH_LOG")
  assert_contains "$launch" "SQUAD_PI_HARNESS=pi-signed pi-signed -e '$sm/.pi/extensions/sq-primary-turnend-guard.ts' -e '$sm/.pi/extensions/sq-primary-pi-watch.ts'" \
    "pi-signed XO did not share Pi's primary extension launch shape"
...
test_claude_forwards_Squad_config_dir_when_set() {
  ...
  out=$(SQUAD_TEST_CLAUDE_CONFIG_DIR="/opt/test/claude-work" \
  ...
  assert_contains "$launch" "CLAUDE_CONFIG_DIR='/opt/test/claude-work' CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false claude" \
    "claude launch did not forward Squad's CLAUDE_CONFIG_DIR to the operator pane"
  pass "claude forwards Squad's CLAUDE_CONFIG_DIR so the operator uses the same credential store"
}
...
test_claude_omits_config_dir_prefix_when_unset() {
  ...
  # default path where sq-spawn adds no prefix.
  out=$(run_ship_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$PROJ_DIR")
  status=$?
  expect_code 0 "$status" "claude spawn without CLAUDE_CONFIG_DIR should succeed"
  launch=$(cat "$LAUNCH_LOG")
  assert_not_contains "$launch" "CLAUDE_CONFIG_DIR=" \
    "claude launch must not add a config-dir prefix when Squad has no CLAUDE_CONFIG_DIR set"
  pass "claude omits the config-dir prefix when Squad runs with the single-store default"
}
...
test_active_dispatch_profile_does_not_block_XO_launch() {
  local rec id sm out status
  id=profile-XO-z16
  rec=$(make_spawn_case profile-XO codex "$id")
  read_case_record "$rec"
  enable_dispatch_profile "$HOME_DIR"
  sm="$CASE_DIR/XO-home"
  make_seeded_XO_home "$sm" "$id"
  ...
  out=$(run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$id" "$sm" --xo)
  status=$?
  expect_code 0 "$status" "XO spawn should be exempt from the dispatch-profile explicit harness requirement"
  assert_contains "$out" "spawned $id harness=codex kind=xo" "XO launch did not use XO harness resolution"
  assert_grep "kind=xo" "$HOME_DIR/state/$id.meta" "XO meta missing kind=xo"
  assert_meta_profile "$HOME_DIR/state/$id.meta" codex default default
  pass "active crew-dispatch profile does not block XO launches"
}
...
echo "# all sq-spawn-dispatch-profile tests passed"
