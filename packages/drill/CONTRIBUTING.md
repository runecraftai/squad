...
**All pull requests to this repository must be raised through `drill`.**

This repo _is_ drill. Contributions should be done using the tool itself, which reduces the maintainer's burden of reviewing and merging contributions.
The `Require drill` GitHub Actions workflow runs on every PR and fails if the body is missing the deterministic signature that drill writes. PRs without it will not be reviewed or merged.
...
- The stable check name is `PR must be raised via drill`.
...
3. Initialize or refresh the gate with your fork as the push target: `drill init --fork-url git@github.com:<you>/drill.git`.
...
- Run `make skill` when you change the canonical agent skill content under `internal/skill`; `make lint` fails if any committed drill skill file has drifted.
