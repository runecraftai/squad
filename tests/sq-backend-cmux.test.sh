#!/usr/bin/env bash
# tests/sq-backend-cmux.test.sh - fake-cmux-CLI unit tests for the cmux
# session-provider adapter (bin/backends/cmux.sh), verified against the real
# cmux 0.64.17 binary (docs/cmux-backend.md). Mirrors
# tests/sq-backend-zellij.test.sh's/tests/sq-backend-herdr.test.sh's
...
# tests/sq-backend-cmux-smoke.test.sh, gated on the cmux binary actually
...
TMP_ROOT=$(fm_test_tmproot sq-backend-cmux-tests)

# make_cmux_fakebin: a `cmux` stub that logs every invocation (one line,
# unit-separated args, to $SQUAD_CMUX_LOG) and returns the canned response for
# that call read from $SQUAD_CMUX_RESPONSES/<n>.out, consumed IN ORDER (call 1
# reads 1.out, call 2 reads 2.out, ...), mirroring
# tests/sq-backend-zellij.test.sh's make_zellij_fakebin. A missing response
...
if [ "${1:-}" = version ]; then
  printf 'cmux %s (97) [abcdef1]\n' "${SQUAD_CMUX_FAKE_VERSION:-0.64.17}"
  exit 0
fi
if [ "${1:-}" = ping ]; then
  printf '%s\n' "${SQUAD_CMUX_FAKE_PING:-PONG}"
  exit "${SQUAD_CMUX_FAKE_PING_EXIT:-0}"
fi
...
cmux_expected_scoped_title() {  # <sq-task-label> [home] [root]
  local label=$1 home=${2:-$ROOT} root=${3:-$ROOT} rest
  case "$label" in
    sq-*) rest=${label#sq-} ;;
    *) rest=$label ;;
  esac
  printf 'sq-%s-%s' "$(cmux_expected_home_label "$home" "$root")" "$rest"
}
...
test_version_check_accepts_current_version() {
  ...
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_VERSION=0.64.17 \
...
test_version_check_accepts_newer_version() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_VERSION=0.70.0 \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_version_check' "$ROOT"
  status=$?
...
test_version_check_refuses_old_version() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_VERSION=0.50.0 \
...
test_version_check_refuses_missing_cmux() {
  ...
  # SQUAD_BACKEND_CMUX_BUNDLE_BIN must also be overridden to a nonexistent path:
  ...
  out=$( PATH="$dir/empty-fakebin:/usr/bin:/bin" SQUAD_BACKEND_CMUX_BUNDLE_BIN="$dir/no-such-cmux" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_version_check' "$ROOT" 2>&1 )
  status=$?
...
test_parse_target() {
  ( . "$ROOT/bin/backends/cmux.sh"
    ...
    [ "$SQUAD_BACKEND_CMUX_WORKSPACE" = "11111111-1111-1111-1111-111111111111" ] || { echo "workspace mismatch: $SQUAD_BACKEND_CMUX_WORKSPACE" >&2; exit 1; }
    [ "$SQUAD_BACKEND_CMUX_SURFACE" = "22222222-2222-2222-2222-222222222222" ] || { echo "surface mismatch: $SQUAD_BACKEND_CMUX_SURFACE" >&2; exit 1; }
...
test_normalize_key() {
  ...
  ) || fail "fm_backend_cmux_normalize_key did not map Squad's key vocabulary to cmux's verified names"
...
test_scoped_title_changes_with_root_path() {
  ...
  pass "fm_backend_cmux_scoped_title: includes the resolved SQUAD_ROOT hash in the home label"
}
...
test_dispatch_composer_state_routes_cmux() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/sq-backend.sh"; fm_backend_composer_state cmux "$1"' "$ROOT" "$target" )
...
test_ping_state_ok() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING=PONG \
...
test_ping_state_denied() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING_EXIT=1 \
    SQUAD_CMUX_FAKE_PING="Error: ERROR: Access denied - only processes started inside cmux can connect" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_ping_state' "$ROOT" )
...
test_ping_state_unauth() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING_EXIT=1 \
    SQUAD_CMUX_FAKE_PING="Error: ERROR: Authentication required - send auth <password> first" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_ping_state' "$ROOT" )
...
test_ping_state_invalid_password() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING_EXIT=1 \
    SQUAD_CMUX_FAKE_PING="Error: ERROR: Invalid password" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_ping_state' "$ROOT" )
...
test_ping_state_down() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING_EXIT=1 \
    SQUAD_CMUX_FAKE_PING="Error: Socket not found at /home/x/.local/state/cmux/cmux.sock" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_ping_state' "$ROOT" )
...
SH
  chmod +x "$fb/open"
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING=PONG \
...
test_ensure_running_fails_fast_on_denied_without_launching() {
  ...
  cat > "$fb/open" <<'SH'
#!/usr/bin/env bash
echo "LAUNCHED" >> "${SQUAD_CMUX_LAUNCH_MARKER:?}"
exit 0
SH
  chmod +x "$fb/open"
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" SQUAD_CMUX_FAKE_PING_EXIT=1 \
    SQUAD_CMUX_FAKE_PING="Error: ERROR: Access denied - only processes started inside cmux can connect" \
    SQUAD_CMUX_LAUNCH_MARKER="$dir/launched" \
...
test_create_task_refuses_duplicate_label() {
  ...
  title=$(cmux_expected_scoped_title sq-dup1)
  cmux_workspace_list_response "$dir" 1 "aaaaaaaa-0000-0000-0000-000000000000" "$title"
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_create_task sq-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
test_create_task_creates_and_parses_ids() {
  ...
  title=$(cmux_expected_scoped_title sq-newtask)
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_create_task sq-newtask /tmp/proj' "$ROOT" )
...
test_target_ready_fails_when_target_absent() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
...
test_target_ready_checks_expected_label() {
  ...
  title=$(cmux_expected_scoped_title sq-label)
  cmux_workspace_list_response "$dir" 1 "aaaaaaaa-0000-0000-0000-000000000000" "$title"
  # 2: list-panes --json --id-format uuids -> matching surface
  cmux_panes_response "$dir" 2 "bbbbbbbb-1111-1111-1111-111111111111"
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_target_ready "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" sq-label' "$ROOT"
...
test_send_key_recovers_stale_target_by_label() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_send_key "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" Enter sq-label' "$ROOT"
...
test_current_path_probes_with_marker() {
  ...
  # a foreground subshell (e.g. fob get) - so current_path actively
  ...
  assert_contains "$(cat "$dir/log")" "__SQUAD_CMUX_CWD_BEGIN__" "current_path did not send the cwd begin marker"
...
test_send_text_submit_detects_landed_send() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "hello commander" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty (submitted) once the composer row reads empty, got '$out'"
  assert_contains "$(cat "$dir/log")" $'\x1f''send'$'\x1f''--workspace'$'\x1f''aaaaaaaa-0000-0000-0000-000000000000'$'\x1f''--surface'$'\x1f''bbbbbbbb-1111-1111-1111-111111111111'$'\x1f''--'$'\x1f''hello commander' \
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_send_text_submit "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "hello commander" 2 0.01 0.01' "$ROOT" )
...
test_kill_adds_sibling_when_last_in_window() {
  ...
  assert_not_contains "$(cat "$dir/log")" $'\x1f''new-workspace'$'\x1f''--name' \
    "the throwaway sibling must stay an unnamed default workspace, never an sq- task title"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-workspace'$'\x1f''--workspace'$'\x1f''aaaaaaaa-0000-0000-0000-000000000000' \
...
test_kill_recovers_stale_target_by_label() {
  ...
  fb=$(make_cmux_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_CMUX_LOG="$dir/log" SQUAD_CMUX_RESPONSES="$dir/responses" \
    bash -c '. "$0/bin/backends/cmux.sh"; fm_backend_cmux_kill "aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111" "" sq-label' "$ROOT"
...
test_list_live_filters_by_title_prefix() {
  ...
  title=$(cmux_expected_scoped_title sq-task1)
  other_title=$(cmux_expected_scoped_title sq-task2 "$ROOT" "$other_root")
  ...
  [ "$out" = $'aaaaaaaa-0000-0000-0000-000000000000:bbbbbbbb-1111-1111-1111-111111111111\tsq-task1' ] \
    || fail "list_live should list only the in-home task workspace with its plain label and surface id, got '$out'"
  pass "fm_backend_cmux_list_live: lists only this home's scoped task workspaces using plain sq-<id> labels"
}

# --- sq-spawn.sh: --xo refuses backend=cmux --------------------------

test_XO_spawn_refuses_cmux_backend() {
  local dir state data config projects out status
  dir="$TMP_ROOT/XO-refuse"; state="$dir/state"; data="$dir/data"; config="$dir/config"; projects="$dir/projects"
  mkdir -p "$state" "$data" "$config" "$projects"
  out=$( SQUAD_STATE_OVERRIDE="$state" SQUAD_DATA_OVERRIDE="$data" SQUAD_CONFIG_OVERRIDE="$config" SQUAD_PROJECTS_OVERRIDE="$projects" \
    "$ROOT/bin/sq-spawn.sh" sm-cmux-test --xo --backend cmux 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "sq-spawn.sh should refuse a --xo spawn with --backend cmux"
  assert_contains "$out" "does not support --xo" "sq-spawn.sh did not report the cmux XO refusal"
  pass "sq-spawn.sh: refuses backend=cmux for --xo spawns (mirrors Orca's refusal; no XO launch design exists yet)"
}
