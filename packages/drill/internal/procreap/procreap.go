...
//
// Why a sweep is needed at all. Every subprocess drill spawns is already
...
// <DRILL_HOME>/worktrees/<repoID>/<runID> belongs to that run, no matter who its
...
//
// A process whose cwd is outside <DRILL_HOME>/worktrees can never match, which is
...
type Options struct {
	// WorktreesRoot is <DRILL_HOME>/worktrees. Required.
