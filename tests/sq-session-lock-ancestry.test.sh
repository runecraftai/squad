#!/usr/bin/env bash
# tests/sq-session-lock-ancestry.test.sh - session-lock harness identity
# (bin/sq-session-lock-lib.sh).
#
...
TMP_ROOT=$(fm_test_tmproot sq-session-lock-ancestry)
fm_git_identity fmtest fmtest@example.invalid

LIB="$ROOT/bin/sq-session-lock-lib.sh"
...
case "$pid:$field:${SQUAD_TEST_CLAUDE_SHAPE:-linux}" in
  ...
  700:ppid=:*) printf '%s\n' 1 ;;
  *:comm=:*) printf '%s\n' bash ;;
  *:args=:*) printf '%s\n' 'bash /repo/bin/sq-claude-stop-autoarm.sh' ;;
  *:ppid=:*) printf '%s\n' 700 ;;
esac
SH
  chmod +x "$fakebin/ps"
  ...
  for shape in linux macos; do
    got=$(SQUAD_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" 'fm_harness_ancestry_pid') \
    ...
    SQUAD_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" 'fm_harness_pid_alive 700' \
      || fail "$shape: a live version-named session was not recognized as a harness"
    SQUAD_TEST_CLAUDE_SHAPE="$shape" lib_eval "$fakebin" "fm_session_lock_owned_by_self '$dir/state'" \
...
case "$pid:$field:${SQUAD_TEST_PATH_SHAPE:-hookdir}" in
  ...
  810:ppid=:*) printf '%s\n' 1 ;;
  *:comm=:*) printf '%s\n' bash ;;
  *:args=:*) printf '%s\n' 'bash /repo/bin/sq-sentry-arm.sh' ;;
  *:ppid=:*) printf '%s\n' 810 ;;
esac
SH
  chmod +x "$fakebin/ps"
  ...
  for shape in hookdir piprefix; do
    if SQUAD_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" 'fm_harness_ancestry_pid'; then
      fail "$shape: an ordinary script path was treated as a harness process"
    fi
    if SQUAD_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" 'fm_harness_pid_alive 810'; then
      fail "$shape: an ordinary script path passed the harness-liveness predicate"
    fi
    if SQUAD_TEST_PATH_SHAPE="$shape" lib_eval "$fakebin" "fm_session_lock_owned_by_self '$dir/state'"; then
...
#!/usr/bin/env bash
if [ "${SQUAD_FIXTURE_ORPHAN_HERE:-0}" = 1 ]; then
  i=0
