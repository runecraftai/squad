...
This primary harness does not have a verified sentry wake adapter.
...
Use `bin/sq-sentry-arm.sh` only when the harness has a tracked background mechanism that survives the tool call and notifies the model on process exit.
Use a bounded foreground wait over `bin/sq-sentry.sh` when that wake mechanism is not verified.
Never use shell `&` for sentry supervision.
