...
func TestSplitWindowsTokensPreservesRootPath(t *testing.T) {
	command := `"C:\Program Files\drill.exe" daemon run --root "C:\Users\tester\AppData\Local\Temp\nm-e2e-1\nmhome"`
...
func TestReapAllRetainsEntryWhenProcessCheckIsInconclusive(t *testing.T) {
	...
	drillHome := filepath.Join(dir, "nm-e2e-retry", "nmhome")
	if err := inv.Register("retry", drillHome, "", 0, os.Getpid()); err != nil {
		t.Fatal(err)
	}
...
func TestIsAllowedTempRoot(t *testing.T) {
	...
	if home, err := os.UserHomeDir(); err == nil {
		cases = append(cases, struct {
			path string
			want bool
		}{filepath.Join(home, ".drill"), false})
	}
