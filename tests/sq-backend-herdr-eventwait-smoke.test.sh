#!/usr/bin/env bash
# tests/sq-backend-herdr-eventwait-smoke.test.sh - REAL-herdr smoke test for the
...
# that the sentry's handle_push_transition lands a stale record in a scratch
# state/.stand-to-queue. Skips cleanly when herdr, jq, or python3 is missing.
#
# Safety (2026-07-02 incident, tests/herdr-test-safety.sh): cleanup uses ONLY
# herdr_safe_stop_and_delete on a private sq-lab-* session, never a bare/ambient
# `herdr server stop`. Every lifecycle op goes through bin/sq-herdr-lab.sh, which
# refuses the default session and verifies the unit-state tripwire.
set -u
...
SESSION="sq-lab-eventwait-smoke-$$"
...
IDS=$(fm_backend_herdr_create_task "$CONTAINER" "sq-evwait1" /tmp "$SEEDED_TAB_ID") || fail "create_task failed"
...
# scratch Squad state so window_to_task and the stand-to queue resolve
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/sq-evwait.XXXXXX")
...
fm_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source sq-evwait-test --agent claude --state idle >/dev/null 2>&1 \
...
fm_herdr_lab_cli "$SESSION" pane report-agent "$PANE_ID" --source sq-evwait-test --agent claude --state blocked >/dev/null 2>&1 \
...
# --- the sentry's fast-path lands a stale record in the scratch stand-to queue ---
...
# shellcheck source=/dev/null
. "$ROOT/bin/sq-push-transition-lib.sh"
wake() { return 0; }
handle_push_transition herdr "$SESSION" "$REC"
[ -e "$STATE/.stand-to-queue" ] || fail "handle_push_transition did not create the stand-to queue"
grep -q 'stale' "$STATE/.stand-to-queue" || fail "the stand-to queue must carry a stale record: $(cat "$STATE/.stand-to-queue")"
grep -q "$TARGET" "$STATE/.stand-to-queue" || fail "the stale record must name the task window $TARGET"
grep -q 'herdr: agent blocked' "$STATE/.stand-to-queue" || fail "the stale payload must name the herdr-blocked cause"
pass "real herdr: the sentry fast-path enqueues a stale wake naming the task window from the live blocked transition"
