...
func mutatesPipelineControl(cmd *cobra.Command) bool {
	...
	switch path {
	case "drill", "drill init", "drill eject", "drill rerun",
		"drill axi run", "drill axi respond", "drill axi abort",
		"drill daemon start", "drill daemon stop", "drill daemon restart",
		"drill update":
		return true
	case "drill sync", "drill axi sync":
...
func emitGateContextRefusal(cmd *cobra.Command, result gatecontext.Result) error {
	errorFields := []toon.Field{
		...
		{Key: "message", Value: "refusing pipeline control from an active drill validation step"},
	}
	if result.RunID != "" {
	...
	allowed := []string{"drill axi status", "drill axi logs --step <phase>", "drill doctor"}
	if result.RunID != "" {
		allowed[0] = "drill axi status --run " + result.RunID
		allowed[1] = "drill axi logs --run " + result.RunID + " --step <phase>"
	}
	emitDoc(cmd,
