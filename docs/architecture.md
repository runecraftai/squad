...
When called with `SQUAD_HOME=<this-Squad-base>` or when `SQUAD_HOME` is already set to the active Squad base, metadata-routed `sq-send.sh` requests to a live `kind=xo` use the live-charter-compatible `from-squad` carrier owned by `bin/sq-operational-input.sh`, so the XO returns terse answers through status lines and detailed answers through docs plus status pointers instead of replying only in its own chat.
