...
TMP_ROOT=$(fm_test_tmproot sq-remote-job)
mkdir -p "$TMP_ROOT"
...
cp "$ROOT/bin/sq-remote-job-lib.sh" "$ROOT/bin/sq-remote-job-worker.sh" \
  "$ROOT/bin/sq-remote-delta-read.sh" "$REMOTE_ROOT/bin/"
...
exit "${SQUAD_PROBE_EXIT:-0}"
SH
cat > "$REMOTE_ROOT/bin/sq-timeout-job.sh" <<'SH'
#!/bin/bash
sleep 3
SH
cat > "$REMOTE_ROOT/bin/sq-delay-job.sh" <<'SH'
#!/bin/bash
sleep "$1"
printf 'ran\n' > "$2"
SH
cat > "$REMOTE_ROOT/bin/sq-touch-job.sh" <<'SH'
#!/bin/bash
printf 'ran\n' > "$1"
SH
cat > "$REMOTE_ROOT/bin/sq-shutdown-job.sh" <<'SH'
#!/bin/bash
trap '' HUP INT TERM
printf 'started\n' > "$1"
sleep 3
printf 'ran\n' > "$2"
SH
cat > "$REMOTE_ROOT/bin/sq-output-job.sh" <<'SH'
#!/bin/bash
set -e
...
DEFAULT_BOUNDS=$(
  unset SQUAD_REMOTE_JOB_QUEUE_TIMEOUT
  unset SQUAD_REMOTE_JOB_TIMEOUT
  # shellcheck disable=SC2030 # This source intentionally initializes subshell-only defaults.
  SQUAD_REMOTE_JOB_STATE_ROOT="$DEFAULT_STATE"
  export SQUAD_REMOTE_JOB_STATE_ROOT
  # shellcheck source=bin/sq-remote-job-lib.sh
  . "$ROOT/bin/sq-remote-job-lib.sh"
  fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" sq-probe-job.sh </dev/null >/dev/null
  printf '%s %s\n' \
    "$(cat "$DEFAULT_STATE/jobs/$SQUAD_REMOTE_JOB_ID/queue_deadline")" \
    "$(cat "$DEFAULT_STATE/jobs/$SQUAD_REMOTE_JOB_ID/timeout")"
)
...
export SQUAD_REMOTE_JOB_STATE_ROOT="$STATE_ROOT"
export SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux
# shellcheck disable=SC2031 # The sourced defaults above were confined to DEFAULT_BOUNDS.
export SQUAD_REMOTE_JOB_QUEUE_TIMEOUT=5
# shellcheck disable=SC2031 # The sourced defaults above were confined to DEFAULT_BOUNDS.
export SQUAD_REMOTE_JOB_TIMEOUT=5
# shellcheck source=bin/sq-remote-job-lib.sh
. "$ROOT/bin/sq-remote-job-lib.sh"
...
case ":$SQUAD_REMOTE_JOB_OPERATOR_PATH:" in
...
fm_remote_job_compose_operator_path "$ACCOUNT_HOME" >/dev/null
NVM_SELECTED=$(PATH="$SQUAD_REMOTE_JOB_OPERATOR_PATH" node)
...
HOME="$ACCOUNT_HOME" PATH="$RUNTIME_BIN:/usr/bin:/bin:/usr/sbin:/sbin" SQUAD_FAKE_PERL_LOG="$FAKE_PERL_LOG" \
  SQUAD_ROOT_OVERRIDE="$REMOTE_ROOT" SQUAD_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux SQUAD_REMOTE_JOB_TIMEOUT=5 \
  "$REMOTE_ROOT/bin/sq-remote-job-worker.sh" > "$TMP_ROOT/worker.out" 2> "$TMP_ROOT/worker.err" &
...
TOP_SECRET=must-not-cross fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-probe-job.sh 'two words' '$(not executed)' < "$TMP_ROOT/stdin" > /dev/null
...
SQUAD_REMOTE_JOB_TIMEOUT=10
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-delay-job.sh 4 "$ACTIVE_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
for _ in $(seq 1 100); do
...
fm_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$(cat "$STATE_ROOT/worker.pid")" = "$ACTIVE_WORKER_PID" ] \
  || fail "ensure replaced a healthy worker during an active job"
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 0 ] || fail "the active job did not complete after the readiness probe"
...
printf '\n' >> "$REMOTE_ROOT/bin/sq-remote-job-worker.sh"
...
fm_remote_job_stage "$ACCOUNT_HOME" "$RELOCATED_ROOT" "$REMOTE_HOME" sq-probe-job.sh < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 0 ] || fail "the relocated worker rejected its configured code root"
...
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 124 ] || fail "the worker did not terminate an over-time job"
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" sq-timeout-job.sh < /dev/null > /dev/null
FIRST_JOB_ID=$SQUAD_REMOTE_JOB_ID
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" sq-touch-job.sh "$QUEUED_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
printf '%s\n' "$(fm_remote_job_read_deadline "$FIRST_JOB_DIR")" > "$STATE_ROOT/jobs/$JOB_ID/queue_deadline"
fm_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 124 ] || fail "an expired queued job did not publish a timeout result"
...
SQUAD_REMOTE_JOB_QUEUE_TIMEOUT=5
SQUAD_REMOTE_JOB_TIMEOUT=3
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-delay-job.sh 1.8 "$FIRST_DELAYED_SIDE_EFFECT" < /dev/null > /dev/null
FIRST_JOB_ID=$SQUAD_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(fm_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-delay-job.sh 1.8 "$SECOND_DELAYED_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 0 ] || fail "queue time consumed the second job's execution timeout"
...
SQUAD_REMOTE_JOB_QUEUE_TIMEOUT=60
SQUAD_REMOTE_JOB_TIMEOUT=40
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 30 < /dev/null > /dev/null
POLL_JOB_ID=$SQUAD_REMOTE_JOB_ID
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-touch-job.sh "$PREEMPT_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
PREEMPT_ELAPSED=$(( $(date +%s) - PREEMPT_BEGAN ))
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 0 ] || fail "the short command behind a long poll did not complete"
...
fm_remote_job_wait "$ACCOUNT_HOME" "$POLL_JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 75 ] || fail "a preempted long poll did not publish its elapsed-window result"
[ ! -s "$SQUAD_REMOTE_JOB_STDOUT" ] || fail "a preempted long poll published partial stdout"
[ ! -s "$SQUAD_REMOTE_JOB_STDERR" ] || fail "a preempted long poll published partial stderr"
...
SQUAD_REMOTE_JOB_TIMEOUT=10
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 5 < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 0 ] || fail "the re-armed poll after preemption did not complete"
OUT=$(<"$SQUAD_REMOTE_JOB_STDOUT")
...
SQUAD_REMOTE_JOB_TIMEOUT=15
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 6 < /dev/null > /dev/null
FIRST_JOB_ID=$SQUAD_REMOTE_JOB_ID
FIRST_JOB_DIR="$STATE_ROOT/jobs/$FIRST_JOB_ID"
for _ in $(seq 1 100); do
  [ "$(fm_remote_job_read_state "$FIRST_JOB_DIR" 2>/dev/null || true)" = running ] && break
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-remote-delta-read.sh "$REPLY_LOG_REL" 0 "$EMPTY_SHA" 1 < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$FIRST_JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
POLL_PAIR_ELAPSED=$(( $(date +%s) - POLL_PAIR_BEGAN ))
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 75 ] || fail "the first sibling poll did not close its own window"
[ "$POLL_PAIR_ELAPSED" -ge 4 ] || fail "a queued sibling poll preempted a running poll"
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 75 ] || fail "the queued sibling poll did not run after the first window"
...
SQUAD_REMOTE_JOB_TIMEOUT=5
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-shutdown-job.sh "$STARTED" "$SHUTDOWN_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
for _ in $(seq 1 100); do
  [ -f "$STARTED" ] && break
  sleep 0.05
...
HOME="$ACCOUNT_HOME" SQUAD_ROOT_OVERRIDE="$REMOTE_ROOT" SQUAD_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux SQUAD_REMOTE_JOB_TIMEOUT=1 \
  "$REMOTE_ROOT/bin/sq-remote-job-worker.sh" >> "$TMP_ROOT/worker.out" 2>> "$TMP_ROOT/worker.err" &
for _ in $(seq 1 100); do
  [ -f "$STATE_ROOT/worker.ready" ] && break
  sleep 0.05
...
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 125 ] || fail "the interrupted job did not publish an unknown-completion result"
sleep 3
...
SQUAD_REMOTE_JOB_TIMEOUT=5
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-shutdown-job.sh "$CRASH_STARTED" "$CRASH_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
for _ in $(seq 1 100); do
...
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 125 ] || fail "worker crash recovery did not publish unknown completion"
sleep 3
...
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 124 ] || fail "the pre-execution deadline did not publish a timeout result"
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" sq-output-job.sh < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 23 ] || fail "bounded output changed the command exit status"
OUTPUT_BYTES=$(LC_ALL=C wc -c < "$SQUAD_REMOTE_JOB_STDOUT" | tr -d ' ')
[ "$OUTPUT_BYTES" -le "$SQUAD_REMOTE_JOB_MAX_BYTES" ] || fail "the worker retained output beyond its byte bound"
ERROR_BYTES=$(LC_ALL=C wc -c < "$SQUAD_REMOTE_JOB_STDERR" | tr -d ' ')
[ "$ERROR_BYTES" -le "$SQUAD_REMOTE_JOB_MAX_BYTES" ] || fail "the worker retained stderr beyond its byte bound"
...
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" sq-touch-job.sh "$SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
rm -f -- "$JOB_DIR/argv"
...
fm_remote_job_ensure_worker "$REMOTE_ROOT" "$ACCOUNT_HOME" || fail "$SQUAD_REMOTE_JOB_ERROR"
fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID" || fail "$SQUAD_REMOTE_JOB_ERROR"
[ "$SQUAD_REMOTE_JOB_EXIT" -eq 126 ] || fail "the worker accepted a symlinked argv record"
...
SQUAD_REMOTE_JOB_TIMEOUT=5
fm_remote_job_stage "$ACCOUNT_HOME" "$REMOTE_ROOT" "$REMOTE_HOME" \
  sq-shutdown-job.sh "$QUARANTINE_STARTED" "$QUARANTINE_SIDE_EFFECT" < /dev/null > /dev/null
JOB_ID=$SQUAD_REMOTE_JOB_ID
JOB_DIR="$STATE_ROOT/jobs/$JOB_ID"
for _ in $(seq 1 100); do
...
set +e
HOME="$ACCOUNT_HOME" SQUAD_ROOT_OVERRIDE="$REMOTE_ROOT" SQUAD_REMOTE_JOB_STATE_ROOT="$STATE_ROOT" \
  SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux "$REMOTE_ROOT/bin/sq-remote-job-worker.sh" \
...
set +e
HOME="$RECOVERY_HOME" SQUAD_ROOT_OVERRIDE="$REMOTE_ROOT" SQUAD_REMOTE_JOB_STATE_ROOT="$RECOVERY_STATE" \
  SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux "$REMOTE_ROOT/bin/sq-remote-job-worker.sh" \
