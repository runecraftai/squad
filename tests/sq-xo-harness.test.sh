#!/usr/bin/env bash
# Tests for the XO-vs-operator harness split, the optional model/effort
# tokens config/xo-harness carries alongside the harness, and the
# primary->XO inherited local-material propagation.
#
# Three capabilities are under test:
#   A) Harness split. config/xo-harness sets the harness the PRIMARY uses
#      to launch XO agents, independent of config/crew-harness (the
#      operator harness). sq-harness.sh XO resolves the fallback chain
#      config/xo-harness -> config/crew-harness -> own; an absent or
#      "default" xo-harness behaves exactly as the operator harness did before
#      this knob existed (full backward-compat). sq-spawn.sh resolves an XO
...
#      down into each XO home's config/, so the XO's OWN operators,
...
#      (re-pushed at XO spawn, on the bootstrap XO sweep, and by
#      config push).
#      config/xo-harness is deliberately NOT inherited (XOs do
#      not spawn XOs). After a successful push that changes allowlisted
#      config under an already-running home, a literal-content reread instruction
#      is written to the XO home and only its pointer is sent via the
#      routed XO path (exact destination bytes, no summaries); unchanged
#      config sends nothing unless a previous send failure is pending.

#   C) Model/effort pin. config/xo-harness may carry optional model and
#      effort tokens after the harness ("<harness> [<model>] [<effort>]"), read by
#      sq-harness.sh XO-model / XO-effort. A bare harness-only
#      line (today's format) yields empty model/effort - full backward-compat.
#      sq-spawn.sh populates MODEL/EFFORT from those tokens for a --xo
...
unset CLAUDECODE PI_CODING_AGENT SQUAD_PI_HARNESS GROK_AGENT

BASE_PATH=${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
fm_git_identity fmtest fmtest@example.com
TMP_ROOT=$(fm_test_tmproot sq-xo-harness)
export SQUAD_BACKEND=tmux

# ===========================================================================
# A) sq-harness.sh XO resolution + fallback (deterministic detect_own)
# ===========================================================================
# detect_own is pinned to claude via CLAUDECODE=1 so the "fall through to own"
# cases are reproducible. Each row sets crew-harness / xo-harness in a
# fresh config dir (a literal '-' means leave the file absent) and asserts BOTH
# the XO resolution AND that crew resolution is unchanged (backward-compat).
#   <label>^<crew-harness>^<xo-harness>^<expect-XO>^<expect-crew>
test_harness_resolution() {
  ...
  while IFS='^' read -r label crew sm exp_sm exp_crew; do
    ...
    [ "$sm" = "-" ] || printf '%s\n' "$sm" > "$cfg/xo-harness"
    got_sm=$(CLAUDECODE=1 SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh" XO)
    got_crew=$(CLAUDECODE=1 SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh" crew)
    [ "$got_sm" = "$exp_sm" ] || fail "$label: XO resolved '$got_sm', expected '$exp_sm'"
    [ "$got_crew" = "$exp_crew" ] || fail "$label: crew resolved '$got_crew', expected '$exp_crew'"
  done <<'ROWS'
both absent -> own (backward-compat)^-^-^claude^claude
crew set, XO absent -> crew (backward-compat)^codex^-^codex^codex
crew set, XO set -> XO wins, crew untouched^codex^grok^grok^codex
crew absent, XO set -> XO value, crew own^-^grok^grok^claude
signed Pi wrapper remains a distinct XO value^codex^pi-signed^pi-signed^codex
XO=default defers to crew^codex^default^codex^codex
crew=default resolves to own, XO follows^default^-^claude^claude
XO=default with crew absent -> own^-^default^claude^claude
ROWS
  pass "A1 sq-harness.sh XO resolves the fallback chain; crew mode unchanged"
}

# ===========================================================================
# C) sq-harness.sh XO-model / XO-effort token resolution
# ===========================================================================
# config/xo-harness holds "<harness> [<model>] [<effort>]" on one line.
...
test_XO_model_effort_tokens() {
  ...
  while IFS='^' read -r label line exp_harness exp_model exp_effort; do
    ...
    got_h=$(CLAUDECODE=1 SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh" XO)
    got_m=$(CLAUDECODE=1 SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh" XO-model)
    got_e=$(CLAUDECODE=1 SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh" XO-effort)
...
ROWS
  pass "C1 sq-harness.sh XO-model/XO-effort resolve the optional tokens; bare harness stays empty (backward-compat)"
}
...
case "$pid:$field:${SQUAD_TEST_SIGNED_SHAPE:-exact}" in
  100:comm=:*) printf '%s\n' '/test/Pi.app/bin/pi' ;;
  100:args=:*) printf '%s\n' 'Pi' ;;
  100:ppid=:*) printf '%s\n' 200 ;;
...
SH
  ...
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "unmarked shared signed-wrapper ancestry resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true SQUAD_PI_HARNESS=pi-signed "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi-signed ] || fail "selected signed wrapper resolved '$got', expected pi-signed"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true SQUAD_PI_HARNESS=pi "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "selected plain Pi resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true SQUAD_PI_HARNESS=pi-signed-helper "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "inexact signed selection marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT -u PI_CODING_AGENT PATH="$fakebin:$BASE_PATH" SQUAD_PI_HARNESS=pi-signed "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "signed selection marker without Pi's family marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true SQUAD_TEST_SIGNED_SHAPE=plain "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "plain Pi marker resolved '$got', expected pi"
  got=$(env -u CLAUDECODE -u GROK_AGENT PATH="$fakebin:$BASE_PATH" PI_CODING_AGENT=true SQUAD_TEST_SIGNED_SHAPE=helper "$ROOT/bin/sq-harness.sh")
  [ "$got" = pi ] || fail "unrelated pi-signed-helper ancestry resolved '$got', expected pi"

  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/sq-session-lock-lib.sh"; fm_harness_ancestry_pid' "$ROOT")
  [ "$got" = 100 ] || fail "session-lock ancestry selected '$got', expected the inner Pi engine pid 100"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/sq-session-lock-lib.sh"; kill() { return 0; }; fm_harness_pid_alive 200' "$ROOT" \
    || fail "session-lock liveness rejected exact pi-signed holder"
  if PATH="$fakebin:$BASE_PATH" SQUAD_TEST_SIGNED_SHAPE=helper bash -c \
    '. "$0/bin/sq-session-lock-lib.sh"; kill() { return 0; }; fm_harness_pid_alive 200' "$ROOT"; then
...
SH
  ...
  err="$dir/sq-harness.err"
  got=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" "$ROOT/bin/sq-harness.sh" 2>"$err")
  [ "$got" = codex ] || fail "dash-leading shell ancestry resolved '$got', expected codex"
  [ ! -s "$err" ] || fail "sq-harness wrote basename option noise for literal -zsh: $(cat "$err")"

  err="$dir/sq-session-lock-ancestry.err"
  got=$(PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/sq-session-lock-lib.sh"; fm_harness_ancestry_pid' "$ROOT" 2>"$err")
  ...
  err="$dir/sq-session-lock-alive.err"
  PATH="$fakebin:$BASE_PATH" bash -c \
    '. "$0/bin/sq-session-lock-lib.sh"; kill() { return 0; }; fm_harness_pid_alive 5252' \
    "$ROOT" 2>"$err"; status=$?
...
test_propagate_lib() {
  ...
  assert_contains "$(cat "$stderr")" "sq-config-inherit: error: failed to remove crew-harness" \
  ...
  # 5. xo-harness is never inherited; backend still is
  printf 'grok\n' > "$src/xo-harness"
  printf '{"default":{"harness":"codex"}}\n' > "$src/crew-dispatch.json"
  printf 'codex\n' > "$src/crew-harness"
  printf 'manual\n' > "$src/backlog-backend"
  ...
  [ -e "$d/home2/config/xo-harness" ] && fail "xo-harness was inherited (must not be)"
  ...
  SQUAD_INHERITABLE_CONFIG=crew-dispatch.json propagate_inheritable_config "$src" "$guard_repo/config" >"$stdout" 2>"$stderr" \
    || fail "guard skip should not make propagation fail"
  [ ! -s "$stdout" ] || fail "guard skip wrote to stdout"
  err_text=$(cat "$stderr")
  assert_contains "$err_text" "sq-config-inherit: warning: skipped crew-dispatch.json" \
...
# ===========================================================================
# B/A integration: an XO spawn resolves the XO harness and
# propagates the operator harness into the home's config.
# ===========================================================================
...
# A minimal seeded XO home (validate_Squad_home_for_spawn needs the
...
# Runs sq-spawn.sh in XO mode. SQUAD_ROOT is the real repo (so sq-harness.sh
...
spawn_XO() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    "$ROOT/bin/sq-spawn.sh" "${spawn_args[@]}" >/dev/null 2>&1 || true
}

meta_harness() { grep '^harness=' "$1" 2>/dev/null | tail -1 | cut -d= -f2-; }

# Split active: crew-harness=claude + xo-harness=codex. The XO
# AGENT launches on codex; its own operators inherit claude; xo-harness
...
test_spawn_split_and_inherit() {
  ...
  [ "$(meta_harness "$meta")" = codex ] \
    || fail "split: XO launched on '$(meta_harness "$meta")', expected codex"
  ...
  [ -e "$sm/config/xo-harness" ] \
    && fail "split: xo-harness leaked into the XO home"
  pass "B2 spawn: XO runs the XO harness; its home inherits declared config"
}

# Backward-compat: xo-harness absent -> the XO launches on the
...
test_spawn_backward_compat_crew_fallback() {
  ...
  meta="$w/home/state/sm.meta"
  [ "$(meta_harness "$meta")" = codex ] \
    || fail "compat: XO launched on '$(meta_harness "$meta")', expected the operator harness codex"
  [ "$(cat "$sm/config/crew-harness" 2>/dev/null)" = codex ] \
    || fail "compat: home crew-harness not inherited as codex"
  pass "B3 spawn: an absent xo-harness falls back to the operator harness (backward-compat)"
}

# Bare backward-compat: no config at all. The XO falls through to its own
...
test_spawn_bare_backward_compat() {
  ...
  [ "$(meta_harness "$meta")" = claude ] \
    || fail "bare: XO launched on '$(meta_harness "$meta")', expected own harness claude"
...
# An explicit per-spawn harness arg wins over config/xo-harness.
test_spawn_explicit_harness_wins() {
  ...
  spawn_XO "$w" sm "$sm" claude

  meta="$w/home/state/sm.meta"
  [ "$(meta_harness "$meta")" = claude ] \
    || fail "explicit: launched on '$(meta_harness "$meta")', expected explicit claude over config codex"
  pass "B5 spawn: an explicit per-spawn harness arg overrides config/xo-harness"
}

# The unverified-adapter guard holds on the resolved XO path: an unknown
# config/xo-harness aborts the spawn (no meta written) and names the source.
test_spawn_unverified_XO_harness_refused() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    SQUAD_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/sq-spawn.sh" sm "$sm" --xo >/dev/null 2>"$err" || rc=$?
  ...
  assert_contains "$(cat "$err")" "config/xo-harness" \
    "unverified: error names the xo-harness source"
  [ -e "$w/home/state/sm.meta" ] && fail "unverified: a meta was written despite the abort"
  pass "B6 spawn: an unverified resolved XO harness is refused (guard intact)"
}

# ===========================================================================
# C integration: config/xo-harness's optional model/effort tokens thread
# into the XO launch command and meta, durably and without a new file.
# ===========================================================================

meta_field() { grep "^$2=" "$1" 2>/dev/null | tail -1 | cut -d= -f2-; }

# A tmux stub that behaves like make_noop_tmux but also captures the literal
# `send-keys -l <cmd>` launch command into SQUAD_FAKE_LAUNCH_LOG, mirroring the
# capture technique in sq-spawn-dispatch-profile.test.sh so the constructed
# launch command (not just meta) can be asserted on. Also answers the
# `#{pane_current_path}` probe from SQUAD_FAKE_PANE_PATH so this same stub works
# for an operator/recon (non-XO) spawn's fob-worktree wait loop.
...
# spawn_XO_capture <world> <id> <home> <launchlog> [extra sq-spawn.sh args...]
# Same shape as spawn_XO but captures the launch command into <launchlog>
...
spawn_XO_capture() {
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    "$ROOT/bin/sq-spawn.sh" "$id" "$home" "$@" --xo
}
...
test_spawn_backend_precedence_over_inherited_config() {
  ...
  out=$(SQUAD_BACKEND=tmux spawn_XO_capture \
    "$w" sm "$sm" "$launchlog" 2>&1); status=$?
  expect_code 0 "$status" \
    "SQUAD_BACKEND=tmux should beat inherited config/backend=herdr"$'\n'"$out"
  ...
  assert_no_grep '^backend=' "$meta" \
    "SQUAD_BACKEND=tmux did not beat inherited config/backend=herdr"
  pass "B5b spawn: SQUAD_BACKEND wins over inherited config/backend"
}
...
test_spawn_explicit_backend_precedence_over_env_and_inherited_config() {
  ...
  out=$(SQUAD_BACKEND=zellij spawn_XO_capture \
    "$w" sm "$sm" "$launchlog" --backend tmux 2>&1); status=$?
  expect_code 0 "$status" \
    "explicit --backend tmux should beat SQUAD_BACKEND=zellij and inherited config/backend=herdr"$'\n'"$out"

  meta="$w/home/state/sm.meta"
  [ "$(cat "$sm/config/backend")" = herdr ] \
    || fail "explicit backend precedence fixture did not inherit config/backend=herdr"
  assert_no_grep '^backend=' "$meta" \
    "explicit --backend tmux did not beat SQUAD_BACKEND=zellij and inherited config/backend=herdr"
  pass "B5c spawn: explicit --backend wins over SQUAD_BACKEND and inherited config/backend"
}

# A bare "<harness>" xo-harness file (today's format) must launch with
...
test_spawn_bare_harness_no_model_effort_flag() {
  ...
  out=$(spawn_XO_capture "$w" sm "$sm" "$launchlog" 2>&1); status=$?
  expect_code 0 "$status" "bare-harness XO spawn should succeed"
  ...
  pass "C2 spawn: a bare harness-only xo-harness file launches with no model/effort flag (backward-compat)"
}

# "<harness> <model>" durably threads --model into the XO launch and
# records it in meta, with no --effort flag (no effort token supplied).
test_spawn_XO_harness_model_token() {
  ...
  spawn_XO_capture "$w" sm "$sm" "$launchlog" >/dev/null 2>&1
  ...
  pass "C3 spawn: config/xo-harness's model token threads --model into the launch and meta"
}

# "<harness> <model> <effort>" threads both flags into the launch and meta.
test_spawn_XO_harness_model_and_effort_tokens() {
  local w sm meta launchlog launch
  ...
  pass "C4 spawn: config/xo-harness's model+effort tokens thread into the launch and meta"
}

# Precedence: an explicit per-spawn --model overrides the file's model token.
test_spawn_explicit_model_overrides_XO_harness_token() {
  local w sm meta launchlog launch
  ...
  spawn_XO_capture "$w" sm "$sm" "$launchlog" --model sonnet >/dev/null 2>&1
  ...
  pass "C5 spawn: an explicit --model overrides config/xo-harness's model token; the file's effort token still applies"
}

# Precedence: an explicit per-spawn --effort overrides the file's effort token.
test_spawn_explicit_effort_overrides_XO_harness_token() {
  local w sm meta launchlog launch
  ...
  spawn_XO_capture "$w" sm "$sm" "$launchlog" --effort low >/dev/null 2>&1
  ...
  pass "C6 spawn: an explicit --effort overrides config/xo-harness's effort token; the file's model token still applies"
}

test_spawn_explicit_harness_does_not_inherit_XO_harness_tokens() {
  local w sm meta launchlog launch
  ...
  spawn_XO_capture "$w" sm "$sm" "$launchlog" --harness codex >/dev/null 2>&1
...
test_spawn_explicit_harness_uses_explicit_profile_axes() {
  ...
  spawn_XO_capture "$w" sm "$sm" "$launchlog" --harness codex --model gpt-5.5 --effort xhigh >/dev/null 2>&1
...
test_spawned_XO_uses_its_harness_supervision_model() {
  ...
  for harness in codex claude; do
    ...
    launchlog="$w/launch.log"
    mkdir -p "$w/home/config"
    printf '%s\n' "$harness" > "$w/home/config/xo-harness"
    make_seeded_home "$sm" sm
    spawn_XO_capture "$w" sm "$sm" "$launchlog" >/dev/null 2>&1
    fm_write_meta "$sm/state/task.meta" "window=Squad:sq-task" "kind=strike"
...
#!/usr/bin/env bash
SQUAD_ROOT_OVERRIDE="$sm" "$ROOT/bin/sq-guard.sh"
SH
    ...
    case "$harness" in
      codex)
        ...
        assert_contains "$out" "$expected" \
          "Codex XO inherited Claude auto-arm despite its persistent sentry model"
        ;;
      claude)
        [ -z "$out" ] \
          || fail "Claude XO with a fresh beacon should use auto-arm supervision, got: $out"
        ;;
    esac
  done
  pass "C9 spawn: XO launch pins supervision to its own harness"
}

# The harness fallback chain (xo-harness -> crew-harness -> own) still
# resolves correctly with no model/effort tokens anywhere in the chain, and a
# crew/recon (non-XO) launch is entirely unaffected by this feature: no
...
test_spawn_fallback_chain_and_crew_scout_unaffected() {
  ...
  [ "$(meta_field "$meta" harness)" = codex ] \
    || fail "fallback: XO harness did not fall back to crew-harness codex"
  ...
  # Crew/recon launch: same crew-harness config, no --xo. Must resolve
  # the operator harness and record no model/effort - this codepath must never read
  # config/xo-harness's tokens at all.
  ...
  pass "C9 spawn: the harness fallback chain still resolves with no tokens; crew/recon launches are unaffected by this feature"
}
...
# A PRIMARY Squad repo on main with one commit + a home dir, mirroring the
# real gitignore (config/crew-harness ignored, so a propagated value never dirties
# the XO worktree on a later sweep). Echoes the world dir.
new_world() {
...
record_live_sentry_fixture() {
  local home=$1 identity
  identity=$(SQUAD_STATE_OVERRIDE="$home/state" bash -c '. "$1"; fm_pid_identity "$2"' _ \
    "$ROOT/bin/sq-stand-to-lib.sh" "$$") || fail "could not identify the live sentry fixture"
  mkdir "$home/state/.sentry.lock"
  printf '%s\n' "$$" > "$home/state/.sentry.lock/pid"
  printf '%s\n' "$home" > "$home/state/.sentry.lock/sq-home"
  printf '%s\n' "$ROOT/bin/sq-sentry.sh" > "$home/state/.sentry.lock/sentry-path"
  printf '%s\n' "$identity" > "$home/state/.sentry.lock/pid-identity"
  touch "$home/state/.last-sentry-beat"
}

# A live XO home as a DETACHED worktree of the primary at <commit>, with
# its seed marker and a live kind=xo meta.
add_sm_worktree() {
...
case "$*" in
  ...
  *capture-pane*) printf '\n'; exit 0 ;;
  *'send-keys'*' -l '*)
    [ "${SQUAD_FAKE_TMUX_FAIL_LITERAL:-0}" = 1 ] && exit 1
    exit 0
    ;;
  *send-keys*)
    [ "${SQUAD_FAKE_TMUX_FAIL_LITERAL:-0}" = 1 ] && exit 1
...
reread_retry_stage_path() {
  ...
  retry_dir="$home/state/.sq-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.sq-inherited-config-reread.*; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
reread_retry_report_path() {
  local home=$1 id=$2 retry_dir path latest=
  retry_dir="$home/state/.sq-inherited-config-reread-retry/$id"
  for path in "$retry_dir"/.sq-inherited-config-reread.*.report; do
    [ -f "$path" ] && [ ! -L "$path" ] || continue
    latest="$path"
  done
  [ -n "$latest" ] || return 1
...
assert_no_reread_pending() {
  local home=$1 state path
  state="$home/state"
  for path in "$state"/.sq-inherited-config-reread.*.pending; do
...
# primary clears it - all while never inheriting xo-harness.
test_bootstrap_sweep_propagates_and_reconverges() {
  ...
  # Initial push: primary crew-harness=codex, xo-harness=grok (must NOT flow).
  printf '{"default":{"harness":"codex"}}\n' > "$w/home/config/crew-dispatch.json"
  printf 'codex\n' > "$w/home/config/crew-harness"
  printf 'manual\n' > "$w/home/config/backlog-backend"
  printf 'tmux\n' > "$w/home/config/backend"
  ...
  [ -e "$w/sm/config/xo-harness" ] \
    && fail "sweep: xo-harness was inherited (must not be)"
  ...
  pass "B7 bootstrap sweep pushes, re-converges, and mirrors absence; never inherits xo-harness"
}
...
test_bootstrap_sweep_defers_dispatch_on_stale_unignored_home() {
  ...
  assert_contains "$out" "XO_SYNC: XO sm: skipped: diverged from" \
...
# absence must converge an XO to the same unconfigured default rather
...
# inheritance, so it is pinned in tests/sq-backend-herdr.test.sh instead.
...
test_presentation_inheritance_default_on_and_opt_out() {
  ...
  [ "$verdict" = on ] || fail "primary default left the XO projection $verdict"
  ...
  assert_contains "$out" "herdr-presentation-spaces: pushed - mirrored primary absence" \
    "a local XO opt-out should reconverge on the primary default"
  verdict=$(sm_presentation_verdict "$w/sm/config")
  [ "$verdict" = on ] || fail "primary default did not reconverge a locally opted-out XO ($verdict)"
  ...
  verdict=$(sm_presentation_verdict "$w/sm/config")
  [ "$verdict" = off ] || fail "explicit primary opt-out left the XO projection $verdict"

  : > "$w/home/config/herdr-presentation-spaces"
  out=$(run_config_push "$w" 2>"$err"); status=$?
  expect_code 0 "$status" "presentation legacy opt-in push should succeed"
  verdict=$(sm_presentation_verdict "$w/sm/config")
  [ "$verdict" = on ] || fail "a legacy primary opt-in file left the XO projection $verdict"
...
test_bootstrap_sweep_surfaces_config_propagation_failure() {
  ...
  fail_line=$(printf '%s\n' "$out" | grep '^XO_SYNC: XO sm: skipped: inheritance failed' || true)
...
test_bootstrap_rereads_after_partial_propagation() {
  ...
  add_sm_worktree "$w" sm "$head"
  printf '{"default":{"harness":"codex"}}\n' > "$w/home/config/crew-dispatch.json"
  printf 'invalid shared header\n' > "$w/home/data/commander-shared.md"
  log="$w/boot-prop-partial.tmux.log"

  out=$(run_bootstrap "$w" "$log")
  assert_contains "$out" "XO_SYNC: XO sm: skipped: inheritance failed" \
...
test_config_push_propagates_reports_without_ff_or_nudge() {
  ...
  printf 'manual\n' > "$w/home/config/backlog-backend"
  printf 'tmux\n' > "$w/home/config/backend"
  record_live_sentry_fixture "$w/home"
  : > "$w/home/config/trace-context"
  ...
  assert_contains "$out" "config-push: $w/home -> live XO homes" \
    "config push lacked the header"
  assert_contains "$out" "XO sm ($sm_real):" \
    "config push did not discover the live XO through registry fallback"
  ...
  [ ! -e "$w/sm/config/trace-context" ] \
    || fail "live config push retroactively enabled trace context in a legacy XO home"
  ...
  assert_contains "$(cat "$log")" "[sq-from-squad]" \
    "config reread must use the marked routed XO path"
...
test_config_push_reports_skips_dirty_and_invalid_home() {
  ...
  assert_contains "$out" "XO dirty ($dirty_real):" \
  ...
  assert_contains "$out" "XO stale ($stale_real):" \
  ...
  assert_contains "$out" "XO bad ($bad_home): skipped - unsafe home: not a seeded XO home" \
    "config push did not report invalid XO home"
  err_text=$(cat "$err")
  assert_contains "$err_text" "sq-config-inherit: warning: skipped crew-dispatch.json" \
...
test_config_push_exits_nonzero_on_copy_error() {
  ...
  err_text=$(cat "$err")
  assert_contains "$err_text" "sq-config-inherit: error: failed to copy crew-harness" \
...
test_config_push_rereads_after_partial_propagation() {
  ...
  assert_contains "$out" "data/commander-shared.md: error" \
...
shared_commander_header_for_tests() {
  cat <<'EOF'
...
test_config_reread_per_home_changed_sets_and_exact_bytes() {
  ...
  # Path order follows SQUAD_INHERITABLE_CONFIG.
  awk '
  ...
  # No parsed/effective summary, no SHA, no commander-shared dump.
  ...
  assert_not_contains "$(cat "$instr_a")" "SHA" "must not emit SHA tokens"
  assert_not_contains "$(cat "$instr_a")" "commander-shared" "commander-shared path must not appear"
  assert_not_contains "$(cat "$instr_a")" "shared secret preference body" \
    "commander-shared content must never be inlined"
  assert_not_contains "$(cat "$instr_b")" "shared secret preference body" \
    "beta must not inline commander-shared either"
  ...
  # Routed send used the from-squad marker and carried only the pointer.
  pointer="CONFIG_REREAD: $(reread_instruction_path "$w/alpha")"
  assert_contains "$(cat "$log")" "[sq-from-squad]" "reread send must be marked"
...
test_config_reread_isolation_and_absent_and_send_failure() {
  ...
  fm_config_write_reread_instruction "$w/beta" "$report" "$w/beta/state/.sq-inherited-config-reread-absent" \
    || fail "ABSENT instruction write failed"
  assert_contains "$(cat "$w/beta/state/.sq-inherited-config-reread-absent")" \
    $'-----BEGIN config/crew-dispatch.json-----\nABSENT\n-----END config/crew-dispatch.json-----' \
    "helper ABSENT representation"
  assert_not_contains "$(cat "$w/beta/state/.sq-inherited-config-reread-absent")" "commander-shared" \
    "helper must ignore commander-shared even when report says pushed"
  assert_not_contains "$(cat "$w/beta/state/.sq-inherited-config-reread-absent")" "config/crew-harness" \
  ...
  assert_contains "$out" "CONFIG_REREAD: XO" "send failure diagnostic missing"
...
#!/usr/bin/env bash
case "\$*" in
  *"$alpha_state/.sq-inherited-config-reread."*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
...
test_config_reread_write_failure_retains_exact_retry_generation() {
  ...
  fakebin=$(make_fake_toolchain "$w")
  real_mv=$(command -v mv)
  mkdir -p "$w/home/state/.sq-inherited-config-reread-retry/sm"
  retry_dir=$(cd "$w/home/state/.sq-inherited-config-reread-retry/sm" && pwd -P)
  cat > "$fakebin/mv" <<SH
#!/usr/bin/env bash
target=
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.sq-inherited-config-reread.*)
    case "\$target" in
      *.exact) ;;
      *) exit 1 ;;
    esac
...
for arg in "\$@"; do target="\$arg"; done
case "\$target" in
  *"$retry_dir"/.sq-inherited-config-reread.*) exit 1 ;;
esac
exec "$real_mv" "\$@"
SH
  chmod +x "$fakebin/mv"
...
test_config_reread_full_retry_queue_drains_before_new_push() {
  ...
  printf 'old\n' > "$w/sm/config/crew-harness"
  printf 'new\n' > "$w/home/config/crew-harness"
  retry_dir="$w/home/state/.sq-inherited-config-reread-retry/sm"
  mkdir -p "$retry_dir"
  for n in $(seq -w 1 16); do
    path="$retry_dir/.sq-inherited-config-reread.20260721T000000.$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  fakebin=$(make_fake_toolchain "$w")
...
test_config_reread_cleanup_runs_after_mixed_delivery_failure() {
  ...
  fail_path="$state_real/.sq-inherited-config-reread.9999-fail"
  for n in $(seq -w 1 18); do
    path="$state_real/.sq-inherited-config-reread.00$n"
    printf 'generation-%s\n' "$n" > "$path"
    chmod 0600 "$path"
  done
  printf 'failed-generation\n' > "$fail_path"
  chmod 0600 "$fail_path"
  for path in "$state_real"/.sq-inherited-config-reread.*; do
...
test_config_reread_stops_after_failed_generation() {
  ...
  mkdir -p "$w/sm/state"
  state_real=$(cd "$w/sm/state" && pwd -P)
  old="$state_real/.sq-inherited-config-reread.0000-fail"
  new="$state_real/.sq-inherited-config-reread.0001-new"
...
test_config_reread_skips_when_unchanged_and_reads_after_push() {
  ...
  instr="$w/sm/state/.sq-inherited-config-reread-dest"
...
-----END config/crew-harness-----' \
  ...
  pending_instruction="$w/sm/state/.sq-inherited-config-reread.20260721T000000.01"
  ...
  for n in $(seq -w 2 18); do
    path="$w/sm/state/.sq-inherited-config-reread.20260721T000000.$n"
    printf '%s\n' generation > "$path"
    chmod 0600 "$path"
  done
  fm_config_reread_cleanup_sent "$w/sm"
  count=0
  for path in "$w/sm/state"/.sq-inherited-config-reread.*; do
    case "$path" in
      *.pending) continue ;;
    esac
    [ -f "$path" ] && [ ! -L "$path" ] || continue
...
test_config_reread_bootstrap_path_and_spawn_flexibility() {
  ...
  assert_contains "$(cat "$log")" "[sq-from-squad]" \
    "bootstrap config reread must use routed XO send"
  assert_contains "$(cat "$instr")" \
    $'-----BEGIN config/crew-harness-----\ncodex\n-----END config/crew-harness-----' \
    "bootstrap instruction must carry exact post-write harness bytes"

  # sq-spawn still permits a conscious explicit runtime outside the config
  ...
  printf '%s\n' $'crew-harness\tpushed\t' > "$report"
  stale="$sm/state/.sq-inherited-config-reread.spawn-stale"
  ...
  spawn_XO_capture "$w" sm-flex "$sm" "$launchlog" --harness pi >/dev/null 2>&1
...
test_bootstrap_respawns_before_config_reread() {
  ...
  printf '%s\n' $'crew-harness\tpushed\t' > "$report"
  stale="$w/sm/state/.sq-inherited-config-reread.stale-generation"
...
cat > "$w/main/bin/sq-spawn.sh" <<SH
#!/usr/bin/env bash
. '$w/main/bin/sq-config-inherit-lib.sh'
printf '%s' spawn >> '$log'
printf '%s' codex > '$w/sm/config/crew-harness'
printf '%s\n' 7500 > '$w/sm/config/startup-memory-budget'
SH
  chmod +x "$w/main/bin/sq-spawn.sh"
  fakebin=$(make_fake_toolchain "$w")
  cat > "$fakebin/tmux" <<SH
#!/usr/bin/env bash
case "\$*" in
...
SH
  ...
  assert_contains "$(cat "$log")" "spawn" \
    "bootstrap did not respawn the dead XO"
  assert_not_contains "$(cat "$log")" "send-keys" \
    "bootstrap nudged an XO before its respawn completed"
...
test_spawn_quarantines_pending_rereads_on_cleanup_failure() {
  ...
  quarantine_root="$sm/state/.sq-inherited-config-reread-quarantine"
...
#!/usr/bin/env bash
case "\$*" in
  *'.sq-inherited-config-reread.'*) exit 1 ;;
esac
exec "$real_rm" "\$@"
SH
  chmod +x "$fakebin/rm"
  ...
  out=$(PATH="$fakebin:$BASE_PATH" TMUX='' CLAUDECODE=1 \
    ...
    SQUAD_PROJECTS_OVERRIDE="$w/home/projects" SQUAD_CONFIG_OVERRIDE="$w/home/config" \
    SQUAD_SPAWN_NO_GUARD=1 SQUAD_FAKE_LAUNCH_LOG="$launchlog" \
    "$ROOT/bin/sq-spawn.sh" sm "$sm" --xo 2>&1); status=$?
  expect_code 0 "$status" "spawn should remain available after reread cleanup failure"
  assert_contains "$out" "CONFIG_REREAD: XO sm: quarantined pre-relaunch generations" \
...
echo "# all sq-xo-harness tests passed"
