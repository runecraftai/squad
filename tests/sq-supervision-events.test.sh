#!/usr/bin/env bash
# tests/sq-supervision-events.test.sh - unit tests for the sentry's native
# event-wait splice (event_wait_or_sleep in bin/sq-sentry.sh and
# handle_push_transition in bin/sq-push-transition-lib.sh). The sentry's source
...
# real herdr, sentry process, or blocking sleeps.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP=$(fm_test_tmproot sq-supervision-events)
STATE_DIR="$TMP/state"
mkdir -p "$STATE_DIR"

# Source the sentry with an isolated state/home. The guard returns before the
# lock/loop, so only the functions load.
export SQUAD_STATE_OVERRIDE="$STATE_DIR"
export SQUAD_ROOT_OVERRIDE="$ROOT"
...
reset_state
fm_write_meta "$STATE_DIR/tk1.meta" "window=default:wG:pQ" "backend=herdr" "kind=strike"
handle_push_transition herdr default "$(mkrec wG:pQ blocked)"
[ -e "$STATE_DIR/.stand-to-queue" ] || fail "handle_push_transition should enqueue a wake for a blocked crew"
grep -q 'stale' "$STATE_DIR/.stand-to-queue" || fail "the enqueued wake must be a stale record: $(cat "$STATE_DIR/.stand-to-queue")"
grep -q 'default:wG:pQ' "$STATE_DIR/.stand-to-queue" || fail "the stale record must name the operator's window"
grep -q 'herdr: agent blocked' "$STATE_DIR/.stand-to-queue" || fail "the stale payload must name the herdr-blocked cause"
...
reset_state
fm_write_meta "$STATE_DIR/tk2.meta" "window=default:wG:pQ" "backend=herdr" "kind=strike"
printf 'paused: waiting on the upstream release\n' > "$STATE_DIR/tk2.status"
handle_push_transition herdr default "$(mkrec wG:pQ blocked)"
if [ -e "$STATE_DIR/.stand-to-queue" ] && grep -q 'stale' "$STATE_DIR/.stand-to-queue"; then
  fail "a declared-pause crew must NOT be fast-escalated: $(cat "$STATE_DIR/.stand-to-queue")"
fi
...
reset_state
fm_write_meta "$STATE_DIR/tk3.meta" "window=default:wG:pQ" "backend=herdr" "kind=strike"
fm_write_meta "$STATE_DIR/sm1.meta" "window=default:wA:pS" "backend=herdr" "kind=xo"
# shellcheck disable=SC2329 # Runtime overrides called by the isolated sentry.
...
case "$PANES" in *"default:wA:pS"*) fail "a kind=xo window must be EXCLUDED from the event pane list, got '$PANES'" ;; *) : ;; esac
pass "event_wait_or_sleep: herdr windows go on the event pane list, but kind=xo endpoints are excluded"
...
fm_backend_wait_transition() {
  [ "${SQUAD_BACKEND_EVENTS_CAPABILITY_CONFIRMED:-0}" = 1 ] || fail "cached capability verdict was not passed to the wait"
  return 1
}
event_wait_or_sleep
event_wait_or_sleep
...
reset_state
fm_write_meta "$STATE_DIR/tk4.meta" "window=fmses:sq-tk4" "kind=strike"   # no backend= -> tmux
# shellcheck disable=SC2329 # Runtime override called by the isolated sentry.
...
reset_state
fm_write_meta "$STATE_DIR/tk5.meta" "window=default:wG:pQ" "backend=herdr" "kind=strike"
...
echo "# sq-supervision-events.test.sh: all assertions passed"
