...
TMP_ROOT=$(fm_test_tmproot sq-teardown-endpoint-safety)
...
#!/usr/bin/env bash
printf 'tmux' >> "${SQUAD_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${SQUAD_RUNTIME_LOG:?}"
printf '\n' >> "${SQUAD_RUNTIME_LOG:?}"
exit 0
SH
  cat > "$TMP_ROOT/$dir/fakebin/fob" <<'SH'
#!/usr/bin/env bash
printf 'fob' >> "${SQUAD_RUNTIME_LOG:?}"
printf ' <%s>' "$@" >> "${SQUAD_RUNTIME_LOG:?}"
printf '\n' >> "${SQUAD_RUNTIME_LOG:?}"
exit 0
SH
  chmod +x "$TMP_ROOT/$dir/fakebin/tmux" "$TMP_ROOT/$dir/fakebin/fob"
  printf '%s\n' "$TMP_ROOT/$dir"
}
...
test_invalid_endpoint_records_refuse_before_mutation() {
  ...
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:sq-other-task" "endpoint_task_id=other-task" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=recon"
  assert_refused_without_mutation "$dir" "$id" "task-mismatched endpoint"

  dir=$(make_case empty-binding)
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:sq-$id" "endpoint_task_id=" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=recon"
  assert_refused_without_mutation "$dir" "$id" "empty task binding"

  dir=$(make_case duplicate-binding)
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=isolated:sq-$id" "endpoint_task_id=$id" "endpoint_task_id=$id" \
    "worktree=$dir/worktree" "project=$dir/project" "kind=recon"
  assert_refused_without_mutation "$dir" "$id" "duplicate task binding"

  pass "sq-teardown: missing, empty, malformed, ambiguous, and task-mismatched endpoints refuse before every mutation or runtime call"
}
...
test_supported_backend_endpoint_records_validate() {
  ...
  id=tmux-task
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=Squad:sq-$id" "worktree=$dir/worktree" "project=$dir/project"
  fm_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint refused"
  [ "$SQUAD_BACKEND_VALIDATED_BACKEND:$SQUAD_BACKEND_VALIDATED_TARGET" = "tmux:Squad:sq-$id" ] || fail "tmux endpoint validation returned wrong identity"

  id=tmux-spaced-session
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=team work:sq-$id" "worktree=$dir/worktree" "project=$dir/project"
  fm_backend_validate_task_endpoint "$dir/home/state/$id.meta" "$id" || fail "valid tmux endpoint with a spaced session name refused"
  [ "$SQUAD_BACKEND_VALIDATED_TARGET" = "team work:sq-$id" ] || fail "tmux validation changed the spaced session identity"
  ...
  id=orca-task
  fm_write_meta "$dir/home/state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-7" \
  ...
  [ "$SQUAD_BACKEND_VALIDATED_TARGET" = term-7 ] || fail "Orca validation did not select its terminal"
...
test_tmux_empty_target_refuses_without_invocation() {
  ...
  set +e
  SQUAD_RUNTIME_LOG="$dir/runtime.log" PATH="$dir/fakebin:$PATH" \
    bash -c '. "$1/bin/sq-backend.sh"; fm_backend_source tmux; fm_backend_tmux_kill ""' _ "$ROOT" \
...
test_isolated_tmux_invalid_and_valid_cleanup() {
  local dir socket socket_id session='endpoint safety' target_id=target control=control target=sq-target
  local prefix_target=sq-prefix prefix_survivor=sq-prefix2 rc
...
[ "\${SQUAD_TEST_TMUX_SOCKET:-}" = '$socket_id' ] || exit 92
...
SH
  ...
  env -u TMUX -u TMUX_PANE SQUAD_TEST_TMUX_SOCKET="$socket_id" SQUAD_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/sq-backend.sh"; fm_backend_source tmux; fm_backend_tmux_kill ""' _ "$ROOT" \
  ...
  env -u TMUX -u TMUX_PANE SQUAD_TEST_TMUX_SOCKET="$socket_id" SQUAD_RUNTIME_LOG="$dir/runtime.log" \
    PATH="$dir/fakebin:$PATH" bash -c \
    '. "$1/bin/sq-backend.sh"; fm_backend_source tmux; fm_backend_tmux_kill "$2"' _ "$ROOT" "$session:$prefix_target"
  ...
  pass "sq-teardown: exact tmux cleanup preserves invalid and prefix-matched neighbors while removing only the recorded target"
}
