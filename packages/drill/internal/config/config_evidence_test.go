...
func TestTestEvidenceDefaults(t *testing.T) {
	...
	if got.Evidence.Dir != ".drill/evidence" {
		t.Errorf("default Dir = %q, want .drill/evidence", got.Evidence.Dir)
	}
...
func TestTestEvidenceMerge_GlobalEnable(t *testing.T) {
	...
	if cfg.Test.Evidence.Dir != ".drill/evidence" {
...
func TestTestEvidenceMerge_RepoOverridesGlobal(t *testing.T) {
	...
	disabled := false
	dir := "docs/evidence"
	globalBranch := "drill/global-evidence"
...
test:
  evidence:
    ...
    branch: team/ci/evidence
`
	if err := os.WriteFile(filepath.Join(dir, ".drill.yaml"), []byte(yaml), 0o644); err != nil {
		t.Fatalf("write: %v", err)
	}
