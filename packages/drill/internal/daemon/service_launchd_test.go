...
func TestStartInstallsLaunchAgentAndBootstrapsManagedDaemon(t *testing.T) {
	...
	for _, want := range []string{
		"<string>/opt/drill/bin/drill</string>",
...
func TestInstallLaunchAgentKeepsLegacyPlistOnScopedWriteFailure(t *testing.T) {
	...
	err := installLaunchAgent(p, "/opt/drill/bin/drill")
	if err == nil {
...
func TestInstallLaunchAgentDoesNotRemoveLegacyPlistForDifferentRoot(t *testing.T) {
	...
	legacyPlist := renderLaunchAgent("/opt/drill/bin/drill", paths.WithRoot(otherRoot), home)
	...
	if err := installLaunchAgent(p, "/opt/drill/bin/drill"); err != nil {
		t.Fatal(err)
	}
...
func TestRenderLaunchAgentForwardsProxyEnv(t *testing.T) {
	...
	plist := renderLaunchAgent("/opt/drill/bin/drill", paths.WithRoot(t.TempDir()), "/home/u")
	for _, want := range []string{
		"<key>HTTPS_PROXY</key>",
...
func TestRenderLaunchAgentForwardsEveryProxyEnvKey(t *testing.T) {
	...
	plist := renderLaunchAgentWithProxyEnv("/opt/drill/bin/drill", paths.WithRoot(t.TempDir()), "/home/u", proxyEnv)
	for _, key := range proxyEnvKeys {
