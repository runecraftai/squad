...
name: afk
description: >-
  Enter away-mode supervision when the commander invokes /afk, says they are going afk, `state/.afk` exists, an incoming message starts with `SQUAD_INJECT_MARK`, or any `state/.subsuper-*` marker is involved.
  It sets a durable away-mode flag so the sub-supervisor daemon can self-handle routine wakes and escalate commander-relevant events plus bounded declared-external-wait rechecks as batched digests during walk-away stretches, then exits automatically when any real unmarked message returns Squad to full per-wake responsiveness.
user-invocable: true
metadata:
  internal: true
---
...
tradeoff **consented** and **explicit**: the commander is stepping away, so the
sub-supervisor may triage routine wakes in bash instead of waking Squad's
LLM for each one. Escalations still reach the commander, but as one pre-read,
batched digest rather than per-wake injections.

## What it does

1. **Enter the lifecycle through `bin/sq-afk-launch.sh`.**
...
2. **Ensure the sub-supervisor daemon is running as a tracked background process.**
   ...
   - **Harness WITH a native in-pane tracked-background tool** (e.g. claude's
     ...
     `bin/sq-afk-launch.sh start-native`, then run
     `SQUAD_AFK_STATE_PREPARED=1 bin/sq-afk-start.sh` through that native tool.
     This is a deliberate no-separate-terminal exception because the harness-hosted job creates no terminal or layout mutation, and a shell launcher cannot invoke a harness-native background tool.
     The launcher still owns lifecycle state and records the no-terminal mode, while the daemon inherits and auto-discovers the commander pane.
     If the native launch fails, run `bin/sq-afk-launch.sh stop` to roll back the prepared lifecycle.
     Do not wrap it in `nohup ... &` (Codex/herdr can reap fire-and-forget shell children after a tool call returns).
   - **Harness WITHOUT one** (e.g. pi): run `bin/sq-afk-launch.sh start`. It is
     ...
     a detached tmux session), records its exact id, and passes the commander pane
     in as `SQUAD_SUPERVISOR_TARGET` so the daemon injects into the commander, not its
     own new pane. **Never manufacture a terminal by splitting the commander's
     active pane** (`herdr pane split`): a split co-tenants the tab and visibly
     shrinks the commander's pane (docs/herdr-backend.md "Away-mode supervisor
     support").
...
- A message **without** the current operational prefix or a legacy bare marker, and **not** starting with `/afk` -> the commander is back.
  Run `bin/sq-afk-return.sh` before acting on the message that brought the commander back.
  ...
  If it reports a Squad-actionable `blocked:` event, remediate it immediately through the normal lifecycle, or explicitly reclassify it with a durable reason and close its decision key with `resolved [key=...]`, then run `bin/sq-afk-return.sh check`.
  ...
  Do not answer a Sitrep request or perform any other ordinary commander work until the check exits successfully.
- A message **with** the current operational prefix (`SQUAD_OPERATIONAL_PREFIX`, U+2063 INVISIBLE SEPARATOR followed by `SQUAD_OP: `), or a legacy bare `SQUAD_INJECT_MARK` daemon escalation -> stay afk and process it.
- Re-invoking `/afk` while already away -> stay afk (refresh the flag); this
  does **not** trigger an exit.

Bias ambiguous cases toward exit: a present commander beats token savings, and
a false exit is self-correcting (the commander re-runs `/afk`).

## Orthogonal to approval authority

afk changes how aggressively Squad surfaces things, **not who approves what**.
"Away" never means "approves more" or "approves less."
A PR ready for merge or a needs-decision finding keeps the same configured authority and exceptions from `AGENTS.md` section 7, while anything requiring the commander still waits for the commander's explicit word.
The daemon only batches the notification.

## Operational prefix contract

The daemon constructs every current injection as the `away-supervisor` kind owned by `bin/sq-operational-input.sh`, beginning with `SQUAD_OPERATIONAL_PREFIX`: `SQUAD_INJECT_MARK` (U+2063 INVISIBLE SEPARATOR) followed by the stable `SQUAD_OP: ` label.
The bare `SQUAD_INJECT_MARK` form remains accepted for legacy daemon escalations during rollout.
U+2063 has no normal keyboard keystroke and survives terminal transport as UTF-8 text.
This is how Squad tells a daemon escalation apart from a real message in the same pane.
...
- **Composer-state guard** - `inject_msg` reads the full `empty`/`pending`/`unknown` verdict from `fm_backend_composer_state` and injects only when it is affirmatively `empty`.
  ...
  The shared `bin/sq-composer-lib.sh` owns the content decision after each backend captures and structurally identifies its own composer row.
...
In afk mode the composer guard is belt-and-suspenders (no human is typing), but it protects against the race window between the commander returning and their message landing, a dead shell, and the daemon's own previous injection sitting unsent.

**Max-defer escape (the daemon must never silently wedge).**
If anything stays buffered past `SQUAD_MAX_DEFER_SECS` (default 300), the daemon
...
`sq-send.sh` uses the same primitive and exits non-zero
when a steer's Enter is positively swallowed, so Squad learns an instruction
...
and the lenient-`pending`-fails-for-`sq-send` policy still holds for steer
...
The daemon wraps `sq-sentry.sh`, runs the sentry as a child, classifies each
wake reason in bash, and self-handles the routine majority without consuming a
Squad turn.
Commander-relevant events, plus a bounded recheck of a declared external wait that remains idle, escalate to Squad's context as one pre-read, single-line, batched digest.
The classification predicates (the commander-relevant verb set, declared-pause vocabulary, signal/stale tests, and unit-scan) live in the shared `bin/sq-classify-lib.sh`, the same library the always-on sentry uses for its own triage when afk is off, so the two modes apply one identical policy.
While `state/.afk` exists the daemon owns the sentry, so the sentry reverts to one-shot and lets the daemon do the triage - the two never run their triage at the same time.

Classify each wake this way:

- `signal` with a terminal commander verb (`done:`, `needs-decision:`, `blocked:`, or `failed:`) -> escalate.
  ...
  Other signals with no commander-relevant status -> self-handle.
- `signal` or `stale` for a declared `paused:` external wait -> self-handle and track the pause rather than a wedge.
  If it remains declared and idle past `SQUAD_PAUSE_RESURFACE_SECS` (default 3600s), housekeeping sends one awaiting-external recheck and resets the pause window.
- `check` -> always escalate. Check scripts print only when Squad should wake.
- `stale` with a terminal status or bare legacy commander-relevant line -> escalate.
  Nonterminal progress remains transient even when its prose contains a legacy free-text token or its seen-status marker already matches, so record a marker and self-handle.
  If the pane is still idle past `SQUAD_STALE_ESCALATE_SECS` (default 240s), housekeeping escalates it as a possible wedge.
  This bounds wedge-detection latency to the threshold plus a tick: a delay, never a loss.
  Healthy operators are autonomous and do not wait on Squad mid-task.
- `heartbeat` -> self-handle. The daemon runs its own cheap bash unit scan
  every `SQUAD_HEARTBEAT_SCAN_SECS` (default 300s) as the catch-all for a
  commander-relevant status line the per-wake classifier might miss.
- Unknown reason, or any uncertainty -> escalate fail-safe.

Escalations are buffered up to `SQUAD_ESCALATE_BATCH_SECS` (default 90s; 0 =
...
the operational prefix lets Squad distinguish it from a real commander message.
...
- The shared composer classifier receives a candidate row only after the active backend performs its own capture and structural row recognition.
  ...
  `SQUAD_COMPOSER_IDLE_RE` still overrides tmux empty-composer matching after shared ghost and border stripping, and `SQUAD_BUSY_REGEX` overrides the rendered delivery guards plus Grok's isolated task-state fallback.
- **Max-defer escape** - the daemon must never silently wedge. If anything stays
  buffered past `SQUAD_MAX_DEFER_SECS` (default 300s), the daemon attempts one
...
- **Marker strip** - `strip_injection_marker` removes the current operational
  ...
  text Squad sees is clean.
- **Portable singleton lock** - the daemon uses the repo's portable lock helper
  (`sq-stand-to-lib.sh`) instead of `flock`, which is absent on macOS.
...
Always enter through `bin/sq-afk-launch.sh`, which clears prior-session artifacts only for a fresh entry and preserves the current session's buffer on refresh.
Always exit through `bin/sq-afk-launch.sh stop`, which keeps `state/.afk` present through the daemon's shutdown flush and clears it last.
...
- Nothing is lost. The durable queue plus `sq-stand-to-drain.sh` recover any missed
...
`SQUAD_INJECT_SKIP` (default `heartbeat`) force-self-handles matching kinds,
