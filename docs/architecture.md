...
Actionable wakes include commander-relevant status signals, no-verb signals whose crew is neither provably working nor paused, authenticated check output such as PR merge polling or a Relay mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, finished-task stale panes absorbed when current state already proves the task done or failed, provably-working stale panes that persist past `SQUAD_STALE_ESCALATE_SECS`, declared external waits that remain paused past `SQUAD_PAUSE_RESURFACE_SECS`, and heartbeat backstop hits.
...
No-verb wakes, such as `working:` notes and bare turn-ended signals, are benign when `bin/sq-crew-state.sh` reports positive evidence that the operator is still working (an actively running drill step attributed to that crew's current code, or an exact busy verdict from the semantic busy-state contract) or when every referenced crew is in a declared external-wait pause.
...
The explicit resolution is written by the actor that answers, not the busy worker: `sq-send`'s `--resolve-key` appends the closing `resolved` line to this base's own copy of the ledger at answer time, which covers operators, local XOs, and remote XOs identically because a remote mate's escalations reach that local copy through the parent-replies ingest and only the answer message itself crosses the transport.
`sq-stand-to-drain.sh` also prints a bounded, unit-wide TEARDOWN PENDING section on every drain (including the empty-queue path) for any task whose latest status-log line verifies `done:`/`failed:` while its `state/<id>.meta` is still present, excluding `kind=xo` tasks whose retirement is owned by `xo-provisioning` rather than `bin/sq-teardown.sh`; it is a pure reminder that never invokes teardown itself, leaving `bin/sq-teardown.sh`'s landed-work safety check as the sole authority on whether the recorded work is actually safe to clean up.
...
A registered XO's validated base is the authority for sitrep current state because it owns the child metadata inventory, each child's current-state result, endpoint observations, backlog holds and dependencies, keyed unresolved decisions, and recent Done baseline.
The original cross-base projection instead treated the XO agent as an ordinary parent task, so an idle XO's `sq-crew-state` fallback selected the latest append-only parent status event even when structured state in the registered base contradicted it.
The parent-status contract also required explicit keyed resolution for decisions and blockers but not for a material `working` phase, so a start event could remain unsuperseded after the corresponding base backlog had moved the work to Done.
Generated XO charters reject generic receipt or start acknowledgements, key only supervisor-actionable material phase reports, and close an opened phase with a same-key later state or `resolved` event, while the structured base remains authoritative even if that closure is missing.
Cross-base reads validate the seeded identity and operational-directory boundaries, use per-base time and output bounds, and classify unavailable, malformed, or inconsistent structured state as unknown rather than reviving a parent event as current work.
When only an owned child's current classification is unavailable, the base classification stays unknown while independently trustworthy structured decisions, holds, queued and landed records, endpoint identities, counts, and provenance remain available; every other invalid path stays strict and exposes none of those child-derived surfaces.
...
Pi and OpenCode verify session-lock ownership and launch one singleton successor from their child-close handlers, with bounded exponential retry for failed restoration. In away mode the adapter classifies routine child closes (routine signal, stale, and heartbeat reasons) and suppresses the wake prompt, delivering only actionable or commander-relevant wakes to the primary.
Claude's `bin/sq-claude-stop-autoarm.sh` hook fires on every Stop and, when the base is eligible and still needs supervision, claims one base-scoped cycle, foregrounds the arm wrapper, and translates actionable closes into exit-2 rewakes.
...
Its `--restart` mode signals only the sentry recorded in the current base's `state/.sentry.lock`, so restarting one base cannot kill sibling XO sentrys.
...
The guard covers the main primary and genuinely marked XO bases, exempts child operator/recon worktrees, is loop-safe per harness, and is documented in [turnend-guard.md](turnend-guard.md).

A presence-gated sub-supervisor (`bin/sq-supervise-daemon.sh`) extends this for walk-away supervision: the `/afk` skill starts it through the tracked foreground helper `bin/sq-afk-start.sh`, after which the sentry reverts to daemon-managed one-shot mode and the daemon self-handles routine wakes in bash. The sentry's wake output carries a `routine:` envelope prefix for routine reasons (via `afk_wake_is_routine` in `sq-classify-lib.sh`) so the Pi extension suppresses the wake prompt while the daemon still triages the original reason.
...
Pane existence, busy checks, composer checks, capture, and backend-verified submit route through `bin/sq-backend.sh`: tmux keeps the same submit core used by the tmux send backend, while herdr uses native busy state, native agent-state submit confirmation on idle baselines, and its ANSI-aware structural composer classifier for pending-input guards and submit fallback.
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats an OpenCode-busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and `sq-send` do not re-send; other harnesses (e.g. Pi) keep the `pending` verdict because their busy indicators do not prove Enter was queued, and an idle pane also keeps `pending` as a genuine swallow. The same opencode busy-queue case is a known gap on the herdr adapter and is recorded in `docs/herdr-backend.md` rather than patched here.
...
[`configuration.md`](configuration.md#runtime-backend-configbackend--squad_backend) owns new-spawn backend selection precedence and authorization.
...
Its default-on presentation projection may place one clean new task in a disposable workspace without changing endpoint authority or lifecycle ownership; [Presentation spaces](herdr-backend.md#presentation-spaces) owns that conditional design, the Herdr version floor its unconfigured default is gated behind, and its narrow base-local restored-shell cleanup at locked session start.
...
Zellij's container shape is simpler than herdr's: one shared `Squad` session, one tab per task, with no per-base workspace split; visible tab titles are scoped by the active base label plus a short hash of the resolved `SQUAD_ROOT` path.
...
cmux's container shape is one workspace per task with one surface, no per-base container split; workspace titles are scoped by the active base label plus a short hash of the resolved `SQUAD_ROOT` path, and `--xo` spawns are refused, mirroring Orca.
...
The primary checkout is healthy on its default branch, and linked worktrees or XO bases are healthy at detached HEAD.
...
The helper's header owns the exact signal detection, relocated-base limitation, test-harness bypass, and relationship to drill's HEAD-continuity guard.
...
`data/XOs.md` records persistent XOs with natural-language scopes, project clone lists, and base paths.
A local route points directly at its base, while a remote route adds an SSH alias and remote Squad code root so the entire base and all of its child work stay on that host.
Remote placement pins the remote second-mate agent to Herdr while leaving the remote base's worker backend selection independent, and every non-doctor primary-to-remote `sq-on` command runs through the remote account's Squad-owned job worker rather than its SSH process or a Herdr pane.
...
`sq-home-seed.sh` provisions a local isolated base, clones the listed PR-based projects into it, initializes newly cloned `drill` projects, copies the charter to `data/charter.md`, and `sq-spawn.sh --xo` launches it through the same session-provider and status-file path as any direct report.
For a domain whose subject is the Squad repo itself, a deliberate `--no-projects` seed creates a project-less base whose operators take pooled worktrees of that repo instead of separate clones.
The signal cannot be mixed with project names or omitted accidentally, and a populated base cannot be converted in place; the full seed contract is in [configuration.md](configuration.md#xo-routes-dataxosmd).
...
When seeded with `-`, the base is a durable fob lease under the XO id, so it survives with no live process and is not recycled by later `fob get` or pruning.
Retirement or seed rollback returns the leased base; normal restart/recovery keeps it leased.
If returning the lease fails during teardown, Squad leaves the route and base intact instead of hiding a still-held lease.
Seeding is transactional: if validation, cloning, initialization, or registry update fails, generated briefs, new bases, new project clones, and registry edits are rolled back.
...
The same project may appear in multiple XO bases when their scopes differ, such as issue triage versus feature development.
XOs are idle by default: after startup recovery reconciles only work already in their own base, an empty queue waits silently for routed tasks, and they never self-initiate surveys or audits.
...
Idle XO panes are healthy; teardown is explicit and refuses while the XO base has in-flight work unless the commander has approved discard with `--force`.

XO bases converge conservatively to the primary's version and declared inherited local material at launch and during locked session start.
...
`config/crew-harness` remains the operator harness and is inherited into XO bases.
...
`bin/sq-project-mode.sh` remains the one registry parser for the mechanical consumers that have no task in hand: unit sync's `local-only` skip and base seeding's refusal and drill initialization.
...
A user enables it by putting `SQX_PAIRING_TOKEN` in the Squad base's gitignored `.env`; `SQX_RELAY_URL` is optional and defaults to `https://mySquad.io`.
...
The relay uses owner-only routing: a mention delivered to a base is from that base's owner, while parent-thread context may still include other public accounts.
...
Work routed to another base reports a *typed* terminal result through `bin/sq-public-followup-emit.sh`; Squad never recovers the source base, work id, outcome, or deliverables by parsing a free-form `done:` sentence, and the child never learns the thread.
...
Reconciliation rides the existing relay poll and the session-start digest instead of a new sentry, daemon, or timer, and both are gated on the same `.env` activation contract so a base that never opted into the relay executes none of it.
...
`learnings-curation` owns the memory curation procedure that `/debrief` invokes: it reads the current destination first, then consolidates, slims, and routes findings in place instead of appending by default.
...
Wake-time refreshes can target a single clone by project name, so the primary base also catches up when an XO reports a merge from its own base.
...
`/updatesquad` fast-forwards the running Squad repo and registered XO bases from `origin`, then re-reads updated instructions and nudges updated XOs without touching project clones.
For a remote route, the configured code root updates from its own origin on that host before the persistent base fast-forwards to the code-root commit.
The update is fast-forward only: dirty, diverged, offline, and off-default targets are reported and left untouched.
Local bases share the guarded fast-forward helper, while remote updates delegate the same safety decision to the configured host through the generic transport.
