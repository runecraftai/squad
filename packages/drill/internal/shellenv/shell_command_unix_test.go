...
func TestCombinedOutputShellCommand_WaitDelayBoundsEscapedPipeHolder(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		"DRILL_SHELLENV_PIPE_HELPER=leader",
		"DRILL_SHELLENV_PIPE_READY="+readyFile,
	)
	ConfigureShellCommand(cmd)
	cmd.WaitDelay = 100 * time.Millisecond
...
func TestShellOutputPipeHelper(t *testing.T) {
	switch os.Getenv("DRILL_SHELLENV_PIPE_HELPER") {
	case "leader":
		child := exec.Command(os.Args[0], "-test.run=^TestShellOutputPipeHelper$")
		child.Env = append(os.Environ(),
			"DRILL_SHELLENV_PIPE_HELPER=escaped",
			"DRILL_SHELLENV_PIPE_READY="+os.Getenv("DRILL_SHELLENV_PIPE_READY"),
		)
		...
		if !waitForHelperReady(os.Getenv("DRILL_SHELLENV_PIPE_READY"), 5*time.Second) {
			os.Exit(3)
		}
	...
	case "escaped":
		_, _ = syscall.Setsid()
		_ = os.WriteFile(os.Getenv("DRILL_SHELLENV_PIPE_READY"), []byte("ready"), 0o644)
