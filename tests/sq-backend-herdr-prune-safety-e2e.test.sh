#!/usr/bin/env bash
# tests/sq-backend-herdr-prune-safety-e2e.test.sh - isolated real-herdr
...
# HERDR_SESSION (never the commander's default): a startup-workspace-shaped
# layout - one tab labeled "1" in a pre-existing workspace labeled
# "Squad" - with a live long-running process in that pane, exactly as
# the commander's own live operator session looked at incident time. Then
...
# sq-<id> task tab), mirroring tests/sq-backend-herdr-smoke.test.sh's broader
...
SESSION="sq-lab-prune-safety-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/sq-herdr-prune-safety.XXXXXX")
cleanup_all() {
...
# Explicitly label the startup workspace "Squad" to create the collision
...
LIVE_CWD="$SCRATCH/Squad"
mkdir -p "$LIVE_CWD"

fm_backend_herdr_server_ensure "$SESSION" || fail "could not start the isolated session's server"

CREATE_OUT=$(fm_backend_herdr_cli "$SESSION" workspace create --cwd "$LIVE_CWD" --label Squad --no-focus) \
...
[ "$LIVE_LABEL" = Squad ] || fail "the startup workspace label should be 'Squad', got '$LIVE_LABEL' - repro setup is wrong"
pass "repro setup: a pre-existing workspace labeled 'Squad' collides with the primary home's own label"
...
TASK_IDS=$(fm_backend_herdr_create_task "$CONTAINER" sq-prunesafety-e2e "$LIVE_CWD" "$SEEDED_TAB_ID") \
...
# default tab pruned, leaving exactly one clean sq-<id> task tab -------------
...
HAPPY_TASK_IDS=$(fm_backend_herdr_create_task "$HAPPY_CONTAINER" sq-prunesafety-happy "$HAPPY_CWD" "$HAPPY_SEEDED") \
...
pass "happy path: a genuinely fresh workspace's seeded default tab is still pruned, leaving exactly one clean sq-<id> task tab"
