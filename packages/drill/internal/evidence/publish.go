...
const (
	...
	fetchRef = "refs/drill/evidence-fetch"

	scratchIndexName = "drill-evidence-index"
)
...
// assertEvidenceBranch refuses a branch that exists but is not a drill
...
func assertEvidenceBranch(ctx context.Context, repoDir, tip, branch string) error {
	...
	if err != nil || string(content) != MarkerContent {
		return fmt.Errorf("refusing to publish evidence: branch %q already exists and is not a drill evidence branch (invalid %s marker at its tip)", branch, MarkerPath)
	}
	return nil
}
...
// drill author when the repository configures none, so publishing never
...
func commitTree(ctx context.Context, repoDir, tree, parent, message string) (string, error) {
	name := configuredIdentity(ctx, repoDir, "user.name", "drill")
	email := configuredIdentity(ctx, repoDir, "user.email", "drill@localhost")
	env := []string{
