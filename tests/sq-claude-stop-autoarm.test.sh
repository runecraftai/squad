#!/usr/bin/env bash
# Behavior tests for the Claude Stop-owned sentry auto-arm
# (bin/sq-claude-stop-autoarm.sh, docs/sentry-continuity.md).
#
...
TMP_ROOT=$(fm_test_tmproot sq-claude-stop-autoarm)
...
# A genuine linked git worktree: the shape every operator/recon task worktree
...
# Arm fixture variants, installed per test as <dir>/bin/sq-sentry-arm.sh.
write_arm_fixture() {
  local dir=$1 kind=$2
  case "$kind" in
    actionable)
...
test_reclaims_stale_session_lock_before_arming() {
  ...
  pass "auto-arm: a demonstrably dead recorded session owner is reclaimed through sq-lock.sh before arming"
}
...
test_actionable_close_rewakes_with_reason() {
  ...
  assert_contains "$out" "Squad sentry wake" "rewake must carry the wake banner"
  assert_contains "$out" "stale: fixture-win actionable" "rewake must carry the arm's reason line"
  assert_contains "$out" "bin/sq-stand-to-drain.sh" "rewake must direct the drain-first protocol"
  assert_contains "$out" "do NOT run bin/sq-sentry-arm.sh" "rewake must forbid a duplicate model re-arm"
...
test_actionable_close_with_live_successor_rewakes_once() {
  ...
  sleep 60 &
  pid=$!
  identity=$(sentry_identity "$dir" "$pid") || fail "could not identify live successor for actionable close"
  ...
  [ "$(printf '%s\n' "$out" | grep -c '^Squad sentry wake')" -eq 1 ] \
...
test_failed_close_rewakes_with_failure_banner() {
  ...
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "a typed sentry failure must rewake as an alarm"
  ...
  assert_contains "$out" "sentry: FAILED" "failure rewake must carry the arm's typed failure"
  assert_not_contains "$out" "bin/sq-sentry-arm.sh" "failure rewake must not create a manual arm loop"
...
test_unverified_clean_close_exhausts_retries() {
  ...
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "a non-actionable close without a healthy sentry must fail closed"
...
test_benign_cycle_end_with_live_sentry_is_silent() {
  local dir out out2 status status2 pid identity
  ...
  sleep 60 &
  pid=$!
  identity=$(sentry_identity "$dir" "$pid") || fail "could not identify live sentry holder for benign close"
  record_sentry_lock "$dir" "$pid" "$identity"
  touch "$dir/state/.last-sentry-beat"
  ...
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  expect_code 0 "$status" "a failed-looking cycle with a live fresh sentry must be benign"
  expect_code 0 "$status2" "the next Stop-owned cycle must remain benign with the live sentry"
  ...
  pass "auto-arm: benign cycle end with a live sentry and fresh beacon stays silent across the next cycle"
}
...
test_positive_recovery_budget_contention_preserves_episode() {
  ...
  sleep 60 &
  pid=$!
  identity=$(sentry_identity "$dir" "$pid") || fail "could not identify live sentry holder for recovery contention"
  record_sentry_lock "$dir" "$pid" "$identity"
  touch "$dir/state/.last-sentry-beat"
  printf 'session=sess-autoarm\ncount=3\nepoch=9\n' > "$dir/state/.turnend-claude-blocks"
  : > "$dir/state/.claude-autoarm-failure-notified"
...
test_need_vanished_mid_cycle_closes_quietly() {
  ...
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 0 "$status" "an actionable close after the unit went idle must not rewake"
...
test_active_in_marked_XO_home() {
  local dir out status
  dir=$(make_XO_dir "$TMP_ROOT/XO")
  : > "$dir/state/task.meta"
  write_arm_fixture "$dir" actionable
  out=$(run_autoarm "$dir" 2>/dev/null); status=$?
  expect_code 2 "$status" "a marked XO home must get the same active auto-arm as the main primary"
  [ -e "$dir/state/arm-ran" ] || fail "hook did not arm in a marked XO home"
  [ "$(epoch_outcome "$dir")" = rewake ] || fail "XO epoch must record outcome=rewake"
  pass "auto-arm: active in a marked XO home"
}
