...
1. Does the Squad AGENT need this on every session or every turn to operate?
...
`AGENTS.md`'s token cost is paid by every session of every unit member, every time, whether or not that session ever hits the situation the new lines describe.
...
Briefs for tasks that touch Squad's own tracked material should tell the operator to load this skill.
`bin/sq-brief.sh`'s `REPO` argument is a caller-supplied string with no reliable signal that it names Squad's own repo, unlike a project registered in `data/projects.md`, so there is no clean point inside the scaffold to detect this case automatically.
Squad adds this skill's load instruction to Squad-repo briefs by hand instead.
`CONTRIBUTING.md`'s "Development" section carries the same instruction as a durable reminder.

## Compatibility and enforcement

Before changing shared tracked behavior, review every affected supported primary harness and runtime backend rather than checking only the adapters active in the current unit.
...
- A live guard in the `live-harness-optin` family (`bin/sq-test-run.sh`), env-gated and self-skipping, that exercises every INSTALLED harness for real and fails naming the harness and version.
...
Run `bin/sq-doc-audience-check.sh`; it enforces classification, README setup routing, local link targets, and owner pointers without keyword-linting legitimate evidence prose.
...
- `bin/*.sh`, `bin/backends/*.sh`, and `tmux/*.tmux` must pass `shellcheck`.
- Run `bin/sq-lint.sh` before treating a script change as done; it is the single owner of the lint definition (file set, config, and pinned shellcheck version) that CI and the drill pre-push gate both invoke, and it refuses to run under any other shellcheck version.
...
- When coining a term for Squad's own material, prefer a compact pre-trained concept over a multi-word description, and hunt for restatements a single word could retire.
