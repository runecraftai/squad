...
This page is the overview. For each step's exact behavior, defaults, skip rules, and fix-commit format, see [Pipeline Steps](/drill/reference/pipeline-steps/).
...
See [Auto-Fix Loop](/drill/concepts/auto-fix/) for how the fix cycle works, and [Using the TUI](/drill/guides/tui/) for what the approval UI looks like.
...
- Skip steps for one run with `drill --skip <steps>`, `git push -o drill.skip=<steps>`, `drill axi run --skip <steps>`, or from the TUI.

See [Configuration](/drill/guides/configuration/).
