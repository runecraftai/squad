...
When a pipeline step finds issues, `drill` can automatically ask the agent to fix them before pausing for your approval. This is controlled by the `auto_fix` configuration.
...
[`ci.rerun_transient`](/drill/reference/repo-config/#cirerun_transient) owns the budget, the exact classification, and every case that skips the rerun.
...
Per-step attempt limits come from the `auto_fix` config object; the [`auto_fix` field reference](/drill/reference/global-config/#auto_fix) owns the defaults, per-step meanings, and the legacy alias.
...
Agent output validated against a findings schema is completed at parse time: a finding that omits `action` gets the schema default `no-op`, and a review finding that omits `review_scope` gets `source` (the tolerant-parse contract for the `review`, `test`, and `lint` steps). On non-schema paths (IPC, user-supplied findings, legacy `requires_human_review` payloads), a missing `action` fails closed as `ask-user`. In both cases an unclassified finding is never eligible for automatic fixing.
...
The [global config reference](/drill/reference/global-config/#commitfix_message) owns the template syntax, default, validation rules, size limits, and supported placeholders; the [repo config reference](/drill/reference/repo-config/#commitfix_message) owns the repository override and trust behavior.
...
The CI step uses `drill: apply CI fixes`, and the Push step uses `drill: apply agent fixes` for remaining uncommitted changes.
...
The generated PR surfaces this recorded evidence in deterministic Risk Assessment, Testing, and Pipeline sections. The [pipeline steps reference](/drill/reference/pipeline-steps/#pr) owns the PR body composition and size-limit contract.
...
- `auto_fix` - also used when you press `f` in the TUI or use `drill axi respond --action fix` to run a follow-up fix
