#!/usr/bin/env bash
# tests/sq-xo-safety.test.sh - XO home safety invariants:
# the path-boundary matrices (seed/spawn/teardown), registry/charter/origin
# validation, fob lease handling, drill initialization of new
# clones, child-worktree protection, and backlog-handoff safety. The happy-path
# operator flow lives in sq-xo-lifecycle-e2e.test.sh; this file keeps the
...
TMP_ROOT=$(fm_test_tmproot sq-xo-safety)
export SQUAD_BACKEND=tmux
...
install_fake_process_event_sweep() {
  local home=$1 log=$2
  mkdir -p "$home/bin"
  cat > "$home/bin/sq-procevent.sh" <<'SH'
#!/usr/bin/env bash
set -eu
case "${1:-}" in
  sweep-home)
...
SH
  chmod +x "$home/bin/sq-procevent.sh"
  : > "$log"
}
...
test_seed_allows_overlapping_clones_and_drops_owner() {
  # A project may appear in several XOs' (non-exclusive) clone lists; the
  # registry never uses the legacy owns: field, and the removed `owner` subcommand
  # stays gone. The full happy seed - charter copied, clones+origins, drill
  # init, modes preserved - is asserted by sq-xo-lifecycle-e2e.
...
EOF
  ...
  assert_grep '- design - feature design for alpha beta' "$home/data/XOs.md" "design registry line missing"
  assert_grep 'projects: alpha, beta' "$home/data/XOs.md" "design project clone list missing"
...
test_home_seed_refuses_broken_registry_symlink() {
  ...
  [ -L "$home/data/XOs.md" ] || fail "home seeding replaced the broken registry symlink"
...
test_home_seed_uses_fob_acquired_home() {
  ...
  out=$(PATH="$fakebin:$PATH" SQUAD_BASE="$home" SQUAD_FAKE_FOB_HOME="$acquired" SQUAD_FAKE_TMUX_LOG="$log" \
    ...
    || fail "seed failed for a fob-acquired home"
  acquired_abs=$(cd "$acquired" && pwd -P)
  printf '%s\n' "$out" | grep -F "home=$acquired_abs" >/dev/null || fail "seed did not report acquired home"
  grep -F 'fob get --lease --lease-holder dash' "$log" >/dev/null || fail "seed did not durably lease a home under the XO id"
  [ -f "$lease" ] || fail "seed did not record a fob lease"
  [ "$(cat "$lease")" = dash ] || fail "seed did not set the lease holder to the XO id"
  [ -f "$acquired/.sq-xo-home" ] || fail "seed did not mark acquired home"
  [ "$(cat "$acquired/.sq-xo-home")" = dash ] || fail "seed wrote wrong acquired-home marker"
  [ -d "$acquired/projects/alpha/.git" ] || fail "seed did not clone project into acquired home"
  grep -F "home: $acquired_abs" "$home/data/XOs.md" >/dev/null || fail "registry did not record acquired home"
...
test_home_seed_returns_fob_acquired_home_on_assignment_failure() {
  ...
  grep -F "fob return --force $acquired_abs" "$log" >/dev/null \
    || fail "failed acquired seed did not return the home through fob"
  if [ -f "$home/data/XOs.md" ] && grep -F -- '- dash ' "$home/data/XOs.md" >/dev/null; then
    fail "failed acquired seed left a registry route"
  fi
  pass "home seeding returns rejected acquired homes through fob"
}
...
test_home_seed_warns_when_acquired_home_return_fails() {
  ...
  grep -F "warning: failed to return fob-acquired home $acquired_abs during seed rollback" "$err" >/dev/null \
    || fail "seed rollback did not warn when fob return failed"
  ...
  pass "home seed rollback warns when fob-acquired return fails"
}
...
test_home_seed_does_not_return_unsafe_acquired_home() {
  ...
  grep -F "fob return --force" "$log" >/dev/null \
    && fail "seed returned an unsafe acquired active home through fob"
  ...
  grep -F "fob return --force" "$log" >/dev/null \
    && fail "seed returned an unsafe acquired active descendant through fob"
...
EOF
  ...
  [ ! -e "$subhome" ] || fail "failed seed left the newly created XO home behind"
  [ ! -e "$subhome/.sq-xo-home" ] || fail "failed seed left a subhome marker"
  ...
  if [ -f "$home/data/XOs.md" ] && grep -F -- '- rollback ' "$home/data/XOs.md" >/dev/null; then
...
test_home_seed_no_projects_end_to_end() {
  # A domain whose subject is the Squad repo itself needs no project clones:
  ...
  assert_grep '- fdev - Squad self-development' "$home/data/XOs.md" "project-less registry line missing"
  assert_grep 'scope: Squad repo work' "$home/data/XOs.md" "project-less registry scope missing"
  assert_grep 'projects: ;' "$home/data/XOs.md" "project-less registry did not render an empty projects field"
  ...
  assert_grep 'kind=xo' "$meta" "project-less spawn meta lost kind=xo"
...
test_XO_spawn_resolves_punctuated_registry_projects() {
  ...
  mark_Squad_home "$sub"
  printf 'punctuated\n' > "$sub/.sq-xo-home"
  printf '# Charter\n\nHandled work.\n\necho '"'"'{state}: {note}'"'"' >> '"'"'<state-file>.status'"'"'\n' > "$sub/data/charter.md"
  sub_abs=$(cd "$sub" && pwd -P)
  ...
  [ "$projects" = 'alpha, beta' ] \
    || fail "XO spawn resolved the wrong projects field: '$projects'"
  pass "XO spawn resolves home validation and projects from punctuated registry fields"
}

test_XO_spawn_refuses_ambiguous_and_mismatched_registry_bindings() {
...
EOF
    ...
    if [ "$case_name" = metadata-mismatch ]; then
      ...
      [ ! -e "$home/state/domain.meta" ] || fail "XO spawn wrote metadata after $case_name refusal"
    fi
    [ ! -e "$home/state/.spawn-domain.lock" ] || fail "XO spawn left a lock after $case_name refusal"
    grep -F 'new-window' "$log" >/dev/null && fail "XO spawn created an endpoint before $case_name refusal"
  done
  pass "XO spawn refuses ambiguous, supplied-home, and metadata-home registry bindings"
}
...
test_home_seed_refuses_projectful_reused_charter_for_projectless_home() {
  ...
  mark_Squad_home "$reusable_sub"
  mark_Squad_home "$stale_sub"
  ...
  scaffold_XO_charter "$home" stale 'Squad self-development. None. This is a project-less domain.' alpha \
  ...
  grep -F 'sq-brief.sh stale --xo --no-projects' "$err" >/dev/null \
  ...
  assert_absent "$stale_sub/.sq-xo-home" "project-less charter refusal wrote a home marker"
  ...
  if grep -F -- '- stale ' "$home/data/XOs.md" >/dev/null; then
...
EOF
  ...
  assert_absent "$sub/.sq-xo-home" "project-less conversion refusal wrote a home marker"
  ...
  if [ -f "$home/data/XOs.md" ] && grep -F -- '- fdev ' "$home/data/XOs.md" >/dev/null; then
...
test_home_seed_refuses_projectless_home_with_uninspectable_projects() {
  ...
  assert_absent "$sub/.sq-xo-home" "project-less inspection refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_symlinked_projects() {
  ...
  assert_absent "$sub/.sq-xo-home" "project-less symlink refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_non_directory_projects() {
  ...
  assert_absent "$sub/.sq-xo-home" "project-less non-directory refusal wrote a home marker"
...
test_home_seed_refuses_projectless_home_with_uninspectable_registry() {
  ...
  assert_absent "$sub/.sq-xo-home" "project-less registry inspection refusal wrote a home marker"
...
test_home_seed_refuses_registry_delimiter_home() {
  ...
  [ ! -e "$subhome/.sq-xo-home" ] || fail "delimiter home seed wrote a marker"
  if [ -f "$home/data/XOs.md" ] && grep -F -- '- design ' "$home/data/XOs.md" >/dev/null; then
...
test_home_seed_refuses_active_home_and_root() {
  ...
  grep -F 'XO home cannot be inside the Squad repo' "$err" >/dev/null \
    || fail "seed did not explain SQUAD_ROOT descendant rejection"
  [ ! -e "$root_clone/tmp" ] || fail "seed created a directory inside SQUAD_ROOT before descendant rejection"
  ...
  grep -F 'XO home cannot be an ancestor of the Squad repo' "$err" >/dev/null \
    || fail "seed did not explain SQUAD_ROOT ancestor rejection"
  [ ! -f "$root_ancestor/.sq-xo-home" ] || fail "seed marked an ancestor of SQUAD_ROOT"
...
test_home_seed_refuses_home_marked_for_another_id() {
  ...
  [ "$(cat "$subhome/.sq-xo-home")" = "other" ] || fail "seed overwrote another XO marker"
...
test_home_seed_refuses_home_registered_to_another_id() {
  ...
  [ ! -e "$subhome/.sq-xo-home" ] || fail "seed wrote a marker before rejecting a registered home"
...
test_home_seed_refuses_reassigning_existing_id_to_different_home() {
  ...
  grep -F "XO id design is already registered to home $first_abs" "$err" >/dev/null \
    || fail "seed did not explain same-id different-home rejection"
  [ ! -e "$second" ] || fail "failed id reassignment created the new subhome"
  [ "$(cat "$first/.sq-xo-home")" = design ] || fail "failed id reassignment changed the original marker"
  grep -F "home: $first_abs" "$home/data/XOs.md" >/dev/null \
  ...
  grep -F "home: $second_abs" "$home/data/XOs.md" >/dev/null \
...
EOF
  ...
  [ ! -f "$parent/.sq-xo-home" ] || fail "seed marked a home containing a registered home"
...
test_home_seed_skips_initialized_existing_drill_projects() {
  ...
  log="$TMP_ROOT/existing-initialized-drill.log"
  mkdir -p "$home/projects" "$home/data" "$home/state"
  fm_git_init_commit "$home/projects/alpha"
  fm_git_init_commit "$home/projects/beta"
  ...
  git -C "$subhome/projects/alpha" remote add drill "$TMP_ROOT/drill-alpha.git"
  printf '%s\n' '- alpha - alpha project (added 2026-06-22)' '- beta - beta project (added 2026-06-22)' > "$home/data/projects.md"
  fakebin=$(make_recording_drill "$TMP_ROOT/existing-initialized-fake")
  : > "$log"
  ...
  grep -F 'failed to initialize drill for beta' "$err" >/dev/null \
    || fail "seed did not explain later drill initialization failure"
  grep -F "$subhome/projects/alpha" "$log" >/dev/null \
    && fail "seed ran drill against an initialized existing clone"
  [ ! -f "$subhome/projects/alpha/.drill-init" ] || fail "seed mutated initialized existing clone with drill init"
  [ ! -f "$subhome/projects/alpha/.drill-doctor" ] || fail "seed mutated initialized existing clone with drill doctor"
  [ ! -e "$subhome/projects/beta" ] || fail "failed seed left a newly cloned project after drill failure"
  pass "home seeding skips initialized existing drill clones"
}

test_home_seed_refuses_uninitialized_existing_drill_project() {
  local home subhome err fakebin log origin
  ...
  log="$TMP_ROOT/existing-uninitialized-drill.log"
  mkdir -p "$home/projects" "$home/data" "$home/state"
  fm_git_init_commit "$home/projects/alpha"
  ...
  fakebin=$(make_recording_drill "$TMP_ROOT/existing-uninitialized-fake")
  : > "$log"
  ...
  grep -F 'refusing to mutate preexisting clone' "$err" >/dev/null \
    || fail "seed did not explain uninitialized existing drill clone refusal"
  [ ! -s "$log" ] || fail "seed ran drill before refusing an uninitialized existing clone"
  [ ! -f "$subhome/projects/alpha/.drill-init" ] || fail "seed mutated uninitialized existing clone"
  pass "home seeding refuses uninitialized existing drill clones"
}
...
test_home_seed_refuses_project_destinations_outside_subhome() {
  ...
  [ ! -f "$subhome/.sq-xo-home" ] || fail "seed marked subhome after unsafe project destination rejection"
...
test_home_seed_refuses_operational_dirs_outside_subhome() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_XO_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink operational dir seed test"
  ...
  for opdir in data state config; do
    ...
    [ ! -f "$subhome/.sq-xo-home" ] || fail "seed marked subhome after unsafe $opdir directory rejection"
  done
...
test_home_seed_refuses_unsafe_leaf_files() {
  ...
  printf '%s\n' '- alpha [direct-PR] - alpha project (added 2026-06-22)' > "$home/data/projects.md"
  scaffold_XO_charter "$home" design 'design domain' alpha || fail "charter scaffold failed for symlink leaf seed test"

  for leaf in data/projects.md data/charter.md .sq-xo-home .sq-xo-parent; do
    ...
    [ ! -f "$subhome/.sq-xo-home" ] || [ "$leaf" = ".sq-xo-home" ] || fail "seed marked subhome after symlinked leaf refusal"
  done
  for leaf in data/projects.md data/charter.md .sq-xo-home .sq-xo-parent; do
...
test_home_seed_preserves_existing_parent_binding() {
  ...
  cmp -s "$before/.sq-xo-parent" "$child/.sq-xo-parent" \
...
test_XO_spawn_requires_seeded_matching_home() {
  ...
  cat > "$fakeroot/bin/sq-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$fakeroot/bin/sq-guard.sh"
  mkdir -p "$ancestor_active_home/data" "$ancestor_active_home/state" "$active_ancestor/data" "$root_ancestor/data" "$root_inside/bin"
  cat > "$root_inside/bin/sq-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root_inside/bin/sq-guard.sh"
  ...
  grep -F 'not a seeded XO home' "$err" >/dev/null || fail "spawn did not explain missing seed marker"
  ...
  grep -F 'marked for XO other, expected domain' "$err" >/dev/null || fail "spawn did not explain marker mismatch"
  ...
  grep -F 'XO home cannot be the Squad repo' "$err" >/dev/null || fail "spawn did not reject Squad repo root"
  ...
  grep -F 'XO home cannot be inside the active Squad home' "$err" >/dev/null || fail "spawn did not reject active home descendant"
  ...
  grep -F 'XO home cannot be an ancestor of the active Squad home' "$err" >/dev/null || fail "spawn did not reject active home ancestor"
  ...
  grep -F 'XO home cannot be inside the Squad repo' "$err" >/dev/null || fail "spawn did not reject repo root descendant"
  ...
  grep -F 'XO home cannot be an ancestor of the Squad repo' "$err" >/dev/null || fail "spawn did not reject repo ancestor"

  pass "XO spawn validates homes before launch"
}

test_XO_spawn_refuses_operational_dirs_outside_subhome() {
  ...
  pass "XO spawn refuses operational directories outside the subhome"
}

test_fm_send_refuses_bare_window_without_home_meta() {
  # The happy path (a bare sq-<id> resolves the window recorded in THIS home's
  ...
  grep -F "no metadata for sq-missing in $home/state" "$err" >/dev/null \
    || fail "sq-send did not explain missing home metadata"
  grep -F 'send-keys -t other-session:sq-missing' "$log" >/dev/null \
    && fail "sq-send fell back to a foreign same-name window"
  pass "sq-send refuses a bare Squad window with no metadata in this home"
}

test_XO_teardown_retires_empty_home() {
...
EOF
  ...
  grep -F "fob return --force $subhome_abs" "$log" >/dev/null || fail "teardown did not release the XO home lease via fob return"
  [ ! -e "$lease" ] || fail "teardown left the XO home lease held after retirement"
  [ ! -d "$subhome" ] || fail "teardown did not remove the retired XO home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/XOs.md" >/dev/null && fail "teardown did not remove XO registry route"
  pass "XO teardown retires empty homes and releases routing"
}

test_XO_teardown_refuses_ambiguous_and_mismatched_registry_bindings() {
...
EOF
    ...
    cp "$home/state/domain.meta" "$meta_before"
    cp "$home/data/XOs.md" "$registry_before"
    ...
    [ -d "$sub" ] || fail "XO teardown removed the home after $case_name refusal"
    cmp -s "$meta_before" "$home/state/domain.meta" || fail "XO teardown changed metadata after $case_name refusal"
    cmp -s "$registry_before" "$home/data/XOs.md" || fail "XO teardown changed registry after $case_name refusal"
    grep -F 'kill-window' "$log" >/dev/null && fail "XO teardown killed an endpoint before $case_name refusal"
  done
  pass "XO teardown refuses ambiguous and identity-mismatched registry bindings"
}

test_XO_teardown_sweeps_process_events_before_removal() {
  ...
  PATH="$fakebin:$PATH" SQUAD_BASE="$home" SQUAD_FAKE_TMUX_LOG="$log" \
    ...
    "$ROOT/bin/sq-teardown.sh" domain >/dev/null 2>/dev/null \
    || fail "normal XO teardown failed after process-event sweep"
  grep -Fx "$subhome_abs" "$sweep_log" >/dev/null || fail "normal XO teardown did not invoke the child home's sweep"
  [ ! -d "$subhome" ] || fail "normal XO teardown retained a successfully swept home"
  [ ! -e "$home/state/domain.meta" ] || fail "normal swept teardown retained parent evidence"
  pass "normal XO teardown sweeps process events before removal"
}

test_XO_teardown_refuses_process_events_without_sweep_script() {
  ...
  grep -F 'no sweep-capable bin/sq-procevent.sh' "$err" >/dev/null || fail "missing sweep capability refusal was not explained"
  ...
  grep -F -- '- domain ' "$home/data/XOs.md" >/dev/null || fail "missing sweep capability refusal removed the route"
  ...
  pass "XO teardown preserves state when process-event sweeping is unavailable"
}

test_XO_teardown_preserves_process_events_on_later_refusal() {
  ...
  printf 'adapter=sq-report\n' > "$subhome/state/procevent/source.source"
  install_fake_process_event_sweep "$subhome" "$sweep_log"
  printf 'SQX_PAIRING_TOKEN=test-token\n' > "$home/.env"
  printf 'work_home=XO:domain\nwork_id=domain\n' > "$home/state/public-followup/registry/obligation"
  fm_write_XO_meta "$home/state/domain.meta" "$subhome"
  printf '%s\n' '- domain - design domain (home: '"$subhome"'; scope: design domain; projects: alpha; added 2026-06-22)' > "$home/data/XOs.md"
...
SH
  ...
  [ -d "$subhome" ] || fail "later refusal removed the XO home"
...
test_XO_force_teardown_sweeps_nested_homes() {
  ...
  mark_Squad_home "$subhome"
  mark_Squad_home "$childhome"
  printf 'domain\n' > "$subhome/.sq-xo-home"
  printf 'nested\n' > "$childhome/.sq-xo-home"
  ...
  fm_write_XO_meta "$home/state/domain.meta" "$subhome"
  fm_write_XO_meta "$subhome/state/nested.meta" "$childhome"
  cat > "$home/data/XOs.md" <<EOF
...
EOF
  ...
  grep -Fx "$subhome_abs" "$sweep_log" >/dev/null || fail "force teardown did not sweep the parent XO home"
  grep -Fx "$childhome_abs" "$sweep_log" >/dev/null || fail "force teardown did not sweep the nested XO home"
  ...
  pass "force teardown sweeps nested XO homes before deletion"
}
...
EOF
  ...
  backup=$(find "$TMP_ROOT" -maxdepth 1 -type d -name '.sq-procevent-restore.*' \
...
EOF
  ...
  [ "$rc" -ne 0 ] || fail "teardown succeeded despite failed fob return"
  grep -F "fob return --force $subhome_abs" "$log" >/dev/null || fail "teardown did not try to return the leased home"
  grep -F 'fob return failed for XO home' "$err" >/dev/null || fail "teardown did not report failed leased home return"
...
EOF
  ...
  grep -F "fob return --force $subhome_abs" "$log" >/dev/null && fail "teardown tried to return a plain-clone home through fob"
  [ ! -d "$subhome" ] || fail "teardown did not remove the plain-clone XO home"
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta for plain-clone home"
  grep -F -- '- domain ' "$home/data/XOs.md" >/dev/null && fail "teardown did not remove plain-clone registry route"
  pass "XO teardown raw-removes plain-clone homes"
}
...
EOF
  ...
  [ ! -e "$home/state/domain.meta" ] || fail "teardown did not clear parent meta"
  grep -F -- '- domain ' "$home/data/XOs.md" >/dev/null && fail "force teardown did not remove XO registry route"
  grep -F 'kill-window -t =Squad:=sq-child' "$log" >/dev/null || fail "force teardown did not kill child window"
  grep -F 'kill-window -t =Squad:=sq-domain' "$log" >/dev/null || fail "force teardown did not kill parent window"
  pass "XO force teardown discards child work"
}
...
EOF
  ...
  pass "XO force teardown prevalidates child quarantine cleanup without following symlinks"
}
...
exit 0
SH
  chmod +x "$fakebin/fob" "$fakebin/lsof"
  ...
  [ "$rc" -ne 0 ] || fail "force teardown succeeded after child fob refused an unproven lock"
  ...
  pass "XO force teardown preserves child worktree after unproven lock refusal"
}

test_XO_force_teardown_allows_operational_dir_symlinks_inside_home() {
...
EOF
    ...
    grep -F 'kill-window -t =Squad:=sq-domain' "$log" >/dev/null || fail "force teardown did not kill parent window for inside $opdir symlink"
  done
  pass "force teardown allows operational directory symlinks inside the subhome"
}

test_XO_force_teardown_refuses_operational_dir_symlink_outside_home() {
...
EOF
  ...
  grep -F 'resolves outside the XO home' "$err" >/dev/null || fail "teardown did not identify unsafe state symlink"
...
test_XO_teardown_path_boundary_matrix() {
  # The teardown path-boundary matrix: an XO home is refused (and left
  # fully intact, with no window killed before validation) when it is unmarked,
  # an ancestor of the active Squad home, inside the active Squad home,
  # or inside the Squad repo. One row per hazard, one shared assertion block.
  local row base home subhome fmroot fakebin log err expect tid
  while IFS='|' read -r row expect; do
    [ -n "$row" ] || continue
    base="$TMP_ROOT/td-pb-$row"
    fmroot="$ROOT"   # real Squad repo unless a row overrides it
    tid=domain
    case "$row" in
      unmarked)
        home="$base/main"; subhome="$base/sub"
        mkdir -p "$home/state" "$home/data" "$subhome/state"
        # No .sq-xo-home marker on purpose.
        ;;
      ancestor)
        # The home being torn down is an ANCESTOR of the active Squad home.
      ...
      repo-descendant)
        ...
        cat > "$fmroot/bin/sq-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
        chmod +x "$fmroot/bin/sq-guard.sh"
        printf 'repo-domain\n' > "$subhome/.sq-xo-home"
        ;;
    esac
    fm_write_XO_meta "$home/state/$tid.meta" "$subhome"
    printf -- '- %s - design domain (home: %s; scope: design domain; projects: alpha; added 2026-06-22)\n' \
      "$tid" "$subhome" > "$home/data/XOs.md"
    ...
    grep -F -- "- $tid " "$home/data/XOs.md" >/dev/null || fail "teardown ($row) removed the registry route after refusal"
    grep -F 'kill-window' "$log" >/dev/null && fail "teardown ($row) killed a window before validation"
  done <<'ROWS'
unmarked|not a seeded XO home
ancestor|ancestor of the active Squad home
active-descendant|inside the active Squad home
repo-descendant|inside the Squad repo
ROWS
  pass "XO teardown path-boundary matrix refuses unmarked/ancestor/active-descendant/repo-descendant homes"
}

test_XO_teardown_refuses_registered_nested_home() {
...
EOF
  ...
  grep -F 'contains registered XO home' "$err" >/dev/null || fail "teardown did not explain registered nested-home refusal"
  pass "XO teardown refuses homes containing registered nested homes"
}

test_XO_teardown_refuses_child_registry_nested_home() {
  local home subhome nested fakebin err log
...
EOF
  ...
  grep -F 'contains registered XO home' "$err" >/dev/null || fail "teardown did not explain child-registry nested-home refusal"
  pass "XO teardown refuses nested homes from the child registry"
}

test_XO_force_teardown_prevalidates_before_child_cleanup() {
...
EOF
  ...
  grep -F 'not a seeded XO home' "$err" >/dev/null || fail "force teardown did not explain missing seed marker"
  pass "force teardown validates subhome before child cleanup"
}

test_XO_force_teardown_refuses_child_active_home_descendant() {
  local home subhome childproj childwt fakebin err log
...
EOF
  ...
  grep -F 'inside the active Squad home' "$err" >/dev/null || fail "force teardown did not explain active home descendant rejection"
  pass "force teardown refuses child worktrees inside the active home"
}

test_XO_force_teardown_refuses_child_repo_descendant() {
...
EOF
  ...
  grep -F 'inside the Squad repo' "$err" >/dev/null || fail "force teardown did not explain repo descendant rejection"
  pass "force teardown refuses child worktrees inside the Squad repo"
}

test_XO_force_teardown_refuses_unregistered_child_worktree() {
  local home subhome childproj childwt fakebin err log
...
test_XO_idle_pane_is_not_stale() {
...
EOF
  ...
  if ! wait_live "$pid" 25; then
    wait "$pid" || true
    grep -F "stale: $window" "$out" >/dev/null && fail "idle XO pane triggered stale wake"
    fail "sentry exited unexpectedly while supervising idle XO"
  fi
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  grep -F "stale: $window" "$out" >/dev/null && fail "idle XO pane triggered stale wake"
  pass "idle kind=xo pane is healthy and not stale"
}

test_XO_charter_brief_is_idle_by_default() {
  local home brief
  home="$TMP_ROOT/idle-charter-home"
  mkdir -p "$home/data" "$home/state"
  scaffold_XO_charter "$home" idle-sm 'feature work for alpha' alpha
  brief="$home/data/idle-sm/brief.md"
  [ -f "$brief" ] || fail "XO charter brief was not scaffolded"
  # Idle contract: waits for routed work, never self-initiates.
  grep -F 'go idle and wait silently for the main Squad' "$brief" >/dev/null \
    || fail "charter brief does not tell the XO to go idle and wait for routed work"
  grep -F 'Act only on tasks the main Squad routes to you' "$brief" >/dev/null \
  ...
  grep -F 'run normal Squad bootstrap and recovery' "$brief" >/dev/null \
  ...
  pass "XO charter brief is idle by default and does not self-initiate work"
}
...
test_backlog_handoff_refuses_done_items_and_non_XO_homes() {
  ...
  seed_XO_home_marker "$subhome" archive
  subhome_abs=$(cd "$subhome" && pwd -P)
  ...
  cmp -s "$before_sub" "$subhome/data/backlog.md" \
    || fail "Done-item refusal mutated the XO backlog"

  # A registered home that is not a seeded XO home (e.g. a project clone)
...
EOF
  ...
  pass "sq-backlog-handoff refuses Done items under whitespace section headings and unsafe homes"
}
