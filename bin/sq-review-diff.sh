...
# current after drill fix rounds push to the PR. A recorded pr_head= is
...
if ! git -C "$WT" rev-parse --verify --quiet "refs/heads/$BRANCH" >/dev/null; then
  ...
  [ -n "$BRANCH" ] || { echo "error: branch sq/$ID does not exist and worktree $WT is detached" >&2; exit 1; }
