#!/usr/bin/env bash
# Behavior tests for sq-unit-sync.sh drift handling.
#
# sq-unit-sync fast-forwards a clone that is cleanly on its default branch. This
...
# must be unchanged, and bootstrap must relay the new outcomes as UNIT_SYNC lines.
#
# It also pins the orphaned .git/packed-refs.lock recovery in the fetch step
# (fetch_with_packed_refs_lock_guard, backed by bin/sq-lock-lib.sh's shared
...
TMP_ROOT=$(fm_test_tmproot sq-unit-sync-tests)
HOME_N=0
...
# run_sync <home> [args...]: run unit-sync against an isolated home, stdout only.
run_sync() {
  local home=$1
  shift
...
# lsof shims mirror tests/sq-teardown.test.sh: no-holder (provably free), a live
...
# run_sync_guarded <home> <fakebin> <outfile> <errfile> [args...]: run unit-sync
# with the fakebin on PATH and stdout/stderr captured separately. Per-test knobs
# (SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_*, GIT_FETCH_COUNTER) are read from the caller's
...
test_whole_fleet_form() {
  ...
  home=$(new_home)
  behind=$(build_pair "$home" unit-behind)
  advance_origin "$home" unit-behind C1
  current=$(build_pair "$home" unit-current)

  # Whole-unit form: no project-dir argument.
  out=$(run_sync "$home")

  assert_contains "$out" "unit-behind: synced" "whole-unit form syncs a behind clone"
  assert_contains "$out" "unit-current: already current" "whole-unit form reports a current clone"
  : "$behind $current"
  pass "whole-unit form processes every clone under projects/"
}
...
test_bootstrap_relays_recovered_and_stuck() {
  ...
  pass "bootstrap relays recovered: and STUCK: unit-sync outcomes"
}
...
test_orphaned_stale_packed_refs_lock_recovers() {
  ...
  set +e
  SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_RETRIES=2 \
  SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
  SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_AGE_SECS=0 \
...
test_live_packed_refs_lock_is_never_removed() {
  ...
  assert_contains "$(cat "$out")" "locklive: skipped: fetch failed" "live lock: unit-sync did not skip"
...
test_transient_packed_refs_lock_self_clears() {
  ...
  SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_RETRIES=3 \
  SQUAD_UNIT_SYNC_PACKED_REFS_LOCK_RETRY_WAIT_SECS=0 \
...
test_non_signature_fetch_failure_is_not_retried() {
  ...
  assert_contains "$(cat "$out")" "locknonsig: skipped: fetch failed" "non-signature: unit-sync did not report the fetch failure"
