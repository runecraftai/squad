...
#
# bin/sq-cd-command-policy.mjs is the single owner of the block/allow decision;
# it reuses the shell classifier owned by bin/sq-arm-command-policy.mjs.
# bin/sq-cd-pretool-check.sh is the stable transport: it scopes the guard to the
...
# scoping (including the deliberate XO-home difference from the turn-end
...
TMP_ROOT=$(fm_test_tmproot sq-cd-pretool-check)

# A primary-shaped checkout: plain (non-worktree) git repo, AGENTS.md, bin/ with
# the transport plus both policy files (sq-cd-command-policy.mjs imports the
# shared classifier from sq-arm-command-policy.mjs). This is what the transport's
# scoping treats as the real primary Squad checkout.
install_cd_scripts() {
  local dir=$1
  mkdir -p "$dir/bin"
  cp "$ROOT/bin/sq-cd-pretool-check.sh" "$dir/bin/sq-cd-pretool-check.sh"
  cp "$ROOT/bin/sq-cd-command-policy.mjs" "$dir/bin/sq-cd-command-policy.mjs"
  cp "$ROOT/bin/sq-arm-command-policy.mjs" "$dir/bin/sq-arm-command-policy.mjs"
  chmod +x "$dir/bin/sq-cd-pretool-check.sh" "$dir/bin/sq-cd-command-policy.mjs"
}
...
# Same shape as primary plus the .sq-xo-home marker: an XO's own
...
# A genuine linked git worktree - the shape bin/sq-spawn.sh hands operator/recon
...
CHECK="$PRIMARY/bin/sq-cd-pretool-check.sh"
...
matrix_case B09 deny 'cd projects/foo && sq-tasks add x'
matrix_case B10 deny 'echo before; cd projects/foo'
matrix_case B11 deny 'true && cd projects/foo'
matrix_case B12 deny 'sq-tasks done x || cd projects/foo'
...
MATRIX_TMP=$(mktemp -d "${TMPDIR:-/tmp}/sq-cd-policy-matrix.XXXXXX")
...
test_fires_in_XO_home() {
  local dir out rc
  dir=$(make_XO_fixture "$TMP_ROOT/XO")
  out=$("$dir/bin/sq-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 2 "$rc" "cd-guard must fire in an XO's own primary session (unlike the turn-end guard)"
  assert_contains "$out" '[persistent-cd]' "XO-home block must carry the reason code"
  pass "cd-guard: fires in an XO home (its own primary session is a primary)"
}
...
test_inert_in_child_worktree() {
  ...
  out=$("$dir/bin/sq-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 0 "$rc" "cd-guard must be inert in an operator/recon linked worktree"
  [ -z "$out" ] || fail "cd-guard produced output in a child worktree: $out"
  pass "cd-guard: inert in an operator/recon task worktree (linked git worktree)"
}

test_inert_when_not_Squad_repo() {
  local dir out rc
  dir="$TMP_ROOT/not-Squad"
  git init -q "$dir"
  git -C "$dir" commit -q --allow-empty -m init
  install_cd_scripts "$dir"   # bin/ present but no AGENTS.md
  out=$("$dir/bin/sq-cd-pretool-check.sh" --claude --command 'cd projects/foo' 2>&1); rc=$?
  expect_code 0 "$rc" "cd-guard must be inert without AGENTS.md (not a Squad checkout)"
  [ -z "$out" ] || fail "cd-guard produced output outside a Squad checkout: $out"
  pass "cd-guard: inert in a non-Squad repo (no AGENTS.md)"
}
...
test_e2e_cwd_leak_regression() {
  ...
  # `cd projects/clone` makes the next Squad-owned backlog write land in the
...
EOF
  ...
  out=$(PATH="$fakebin" "$dir/bin/sq-cd-pretool-check.sh" --command 'git status' 2>&1); rc=$?
...
test_policy_cli_direct() {
  local policy
  policy="$ROOT/bin/sq-cd-command-policy.mjs"
  ...
  pass "cd-guard: sq-cd-command-policy.mjs CLI honors the deny/allow output contract"
}
...
test_scripts_are_shellcheck_clean() {
  ...
  shellcheck "$ROOT/bin/sq-cd-pretool-check.sh" >/dev/null 2>&1 \
    || fail "bin/sq-cd-pretool-check.sh is not shellcheck-clean"
  pass "bin/sq-cd-pretool-check.sh is shellcheck-clean"
}
