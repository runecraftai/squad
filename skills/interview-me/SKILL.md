...
When a decision is hard to reverse, surprising without context, or the result of a real tradeoff, record it in an ADR under `docs/adr/` or the project's existing decision-log location.
When a domain term was clarified, record it in a glossary file (`CONTEXT.md` or the project's existing glossary).
Inspect existing files before creating anything, and follow the project's existing format when present.
Keep one decision in one authoritative document, and keep glossary entries free of implementation details.
