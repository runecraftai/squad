#!/usr/bin/env bash
# Behavior tests for the semantic busy-state contract (bin/sq-busy-lib.sh and
# its only writer bin/sq-busy-event.sh).
#
# Covers the commander-approved redesign invariants: busy/idle/unknown/dead with
...
TMP_ROOT=$(fm_test_tmproot sq-busy-state)
...
test_arm_seeds_busy_spawn() {
  ...
  [ "$out" = "busy sq-spawn" ] || fail "seed should classify 'busy sq-spawn', got '$out'"
  pass "arm mints a gen sidecar and seeds busy sq-spawn at seq=1"
}
...
test_apply_current_gen_reset() {
  ...
  "$EV" apply "$state" t1 idle --current-gen --source sq-interrupt --event interrupt \
    || fail "apply --current-gen failed"
  out=$(fm_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "idle sq-interrupt" ] || fail "expected 'idle sq-interrupt', got '$out'"
  "$EV" apply "$state" t1 unknown --current-gen --source sq-recovery --event relaunch \
    || fail "apply unknown failed"
  out=$(fm_busy_classify tmux w1 pi t1 "$state")
  [ "$out" = "unknown sq-recovery" ] || fail "expected 'unknown sq-recovery', got '$out'"
  pass "Squad-owned interrupt and recovery events bind to the current gen"
}
...
test_retire_serializes_and_rejects_stale_gen() {
  ...
  out=$(fm_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy sq-spawn" ] || fail "stale retirement changed the new incarnation, got '$out'"
...
test_stale_gen_event_rejected() {
  ...
  out=$(fm_busy_classify tmux w1 claude t1 "$state")
  [ "$out" = "busy sq-spawn" ] || fail "stale event must not change the record, got '$out'"
...
test_dead_endpoint_overrides() {
  ...
  out=$(fm_busy_classify_live tmux w1 claude t1 "$state")
  [ "$out" = "busy sq-spawn" ] || fail "live endpoint must fall through to the record, got '$out'"
...
# The record parser runs inside sourcing callers (the sentry, the daemon, the
...
echo "all sq-busy-state tests passed"
