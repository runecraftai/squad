...
#   Set SQUAD_XO_SCOPE='<scope>' to write a routing scope distinct from the charter text.
#   --herdr-lab is mandatory when the task will issue Herdr lifecycle commands.
#   It adds the hard isolation contract backed by bin/sq-herdr-lab.sh.
...
#   drill  implement -> /drill pipeline -> PR -> configured merge authority
...
# "Delivery contract: mode=<mode>" line. bin/sq-spawn.sh reads that line and refuses
# to launch a strike task whose explicit --mode disagrees, so an adjusted brief and the
# recorded task metadata cannot drift apart.
# Ship briefs begin with a worktree-isolation assertion before the branch step.
# --mode is refused on recon and XO scaffolds: a recon's deliverable is a
# report rather than a merge, and a charter is not a delivery contract.
# There is no --yolo flag here. The worker never owns approval decisions, so yolo is
# a spawn-time and Squad-side input only (AGENTS.md section 7).
# Every scaffold's status protocol distinguishes the configured
# declared-external-wait verb (SQUAD_CLASSIFY_PAUSED_VERB, default "paused") from
# "blocked:": pause for a known external wait expected to clear on its own,
# blocked when Squad must act.
# Strike tasks include a project-memory section so durable project-intrinsic
# learnings can be committed to AGENTS.md through the project's delivery path;
# it carries the AGENTS.md authoring bar (widely useful knowledge only, pointers
# over copied detail) and has the operator add the sq-ensure-agents-md.sh
...
PAUSED_VERB=${SQUAD_CLASSIFY_PAUSED_VERB:-$SQUAD_CLASSIFY_PAUSED_VERB_DEFAULT}
...
for a in "$@"; do
  ...
  case "$a" in
    ...
    --yolo|--yolo=*) echo "error: --yolo is not a brief input; pass it to bin/sq-spawn.sh, which records the task's approval posture" >&2; exit 1 ;;
    *) POS+=("$a") ;;
  esac
done
...
if [ "$KIND" = strike ]; then
  [ "$MODE_SET" -eq 1 ] || {
    echo "error: strike briefs require --mode <drill|direct-PR|local-only>; resolve it at intake from the commander's instruction and the project's registered posture in data/projects.md" >&2
    exit 1
  }
  case "$MODE" in
...
idx=1
while [ "$idx" -lt "${#POS[@]}" ]; do
  XO_PROJECTS="${XO_PROJECTS}${XO_PROJECTS:+ }${POS[$idx]}"
  idx=$((idx + 1))
done
...
cat > "$BRIEF" <<EOF
You are a persistent second mate managed by the main Squad. Work on your own; do not wait for a human.
...
Act only on tasks the main Squad routes to you.
...
Report only true commander-relevant outcomes or a declared external wait by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
Use \`$PAUSED_VERB: {why}\` (distinct from \`blocked:\`) only when your domain is deliberately idling on a known external wait you expect to clear on its own; use \`blocked:\` when you are stuck and need Squad to act.
Use this only for material phase changes, a commander decision, a real blocker, a failure, or work ready for review.
This is also how you return the answer to a marked from-squad request above.
...
if [ "$XO_CHARTER" = "{TASK}" ]; then
  ...
  echo "scaffolded: $BRIEF (XO charter)"
fi
exit 0
fi
...
HERDR_LAB_HELPER=$(shell_quote "$SQUAD_ROOT/bin/sq-herdr-lab.sh")
...
'6. The helper records the live default session before provisioning and verifies the identical unit state after teardown.' \
'   A missing, stopped, or changed default session is a hard tripwire failure, never a cleanup warning to ignore.' \
'' \
'Never bypass the helper, even for a read-only lifecycle probe or cleanup after failure.' \
'The commander unit uses the running `default` session.')
else
...
3. Use sq-gh for GitHub operations and sq-browser for browser operations.
4. Report status by appending one line:
   \`echo "{state}: {one short line}" >> $STATUS_FILE\`
   States: working, needs-decision, blocked, $PAUSED_VERB, done, failed.
   Each append wakes Squad, so report sparingly: only phase changes a supervisor
   would act on and the needs-decision/blocked/paused/done/failed states. No step-by-step
   FYI progress lines; Squad reads your pane for that.
   Use \`$PAUSED_VERB: {why}\` - distinct from \`blocked:\` - ONLY when you are deliberately idling on a
   known external wait you expect to clear on its own (an upstream release, a rate-limit reset):
   Squad then leaves your idle pane alone and rechecks it on a long cadence instead of
   treating it as a possible wedge. Use \`blocked:\` when you are stuck and need help.
5. If you hit the same obstacle twice, append \`blocked: {why}\` and stop; Squad will help.
6. If a decision belongs to a human (product choices, destructive actions),
   append \`needs-decision: {summary of options}\` and stop. Squad will reply with the decision.
...
Before reporting done, read and follow \`$SQUAD_ROOT/.agents/skills/decision-hold-lifecycle/SKILL.md\` and pass its shared completion gate for the report and any visual review.
...
# Strike task: shape Setup / Rule 1 / Definition of done by this task's explicit
# delivery mode, validated above. The generated DOD opens with the fixed
# "Delivery contract: mode=<mode>" line that bin/sq-spawn.sh checks against its own
# explicit --mode before launching.
case "$MODE" in
  direct-PR)
    SETUP2=""
    RULE1='1. Never push to the default branch (push only your `sq/'"$ID"'` branch). Never merge a PR.'
...
EOF
  ...
  local-only)
    SETUP2=""
    RULE1="1. Never push to any remote and never open a PR. Work only on your \`sq/$ID\` branch; Squad handles the merge into local \`main\`."
    IFS= read -r -d '' DOD <<EOF || true
# Definition of done
Delivery contract: mode=local-only
This task ships **local-only**: no remote, no PR, no pipeline.
The task is complete only when committed on your branch \`sq/$ID\`. Do NOT push, do NOT open a PR, do NOT merge.
Keep your branch a clean fast-forward onto the current default branch - if \`main\` has advanced, rebase onto it so the eventual merge stays a fast-forward.
When it is implemented and committed, append \`done: ready in branch sq/$ID\` to the status file and stop.
The configured merge authority approves the ready branch, then Squad merges it into local \`main\` through the guarded fast-forward path.
EOF
    ;;
  *)  # drill
    SETUP2="
...
1. First action: create your branch: \`git checkout -b sq/$ID\`$SETUP2
...
6. If a decision belongs above the implementation worker (product choices, destructive actions, ask-user findings),
   append \`needs-decision: {summary of options}\` and stop. Squad will apply the configured authority and reply with the decision.
   A decision or blocker you opened stays open until a \`resolved\` line carrying its exact key lands; a later \`done:\` or \`working:\` line never closes it, even when the answer is what started that work.
   Squad's reply normally writes that closing line at answer time; when a blocker or wait clears WITHOUT a Squad reply, append \`resolved: {how it cleared}\` yourself (same \`[key=<slug>]\` if you opened it with one) as you resume.
7. Never stop, restart, or update the shared \`drill\` daemon - it is one instance serving
   every lane/base, so restarting it kills other lanes' in-flight pipeline runs. On ANY drill
...
# Project memory
If \`AGENTS.md\` or \`CLAUDE.md\` already exists, or if this task produced durable project-intrinsic knowledge, run \`$SQUAD_ROOT/bin/sq-ensure-agents-md.sh .\` in the worktree.
Record only project knowledge useful to almost every future session.
For anything the codebase already shows, prefer a pointer to the authoritative file, command, or doc over copying the detail.
If you touch a project \`AGENTS.md\` that lacks \`## Maintaining this file\`, add that short self-governance section from \`$SQUAD_ROOT/bin/sq-ensure-agents-md.sh\` in the same pass.
