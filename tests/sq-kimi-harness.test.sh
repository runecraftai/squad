#!/usr/bin/env bash
# Behavior tests for the verified Kimi Code CLI operator adapter.
set -u
...
KIMI_HOOK="$ROOT/bin/sq-kimi-turnend-hook.sh"
TMP_ROOT=$(fm_test_tmproot sq-kimi-harness)
KIMI_RUNTIME_TASK_TMP=
PYTHON_BIN=$(command -v python3) || fail "test needs python3"
PYTHON_BIN_DIR=$(dirname "$PYTHON_BIN")
JQ_BIN=$(command -v jq) || fail "test needs jq"
BASE_PATH=${SQUAD_TEST_BASE_PATH:-$PYTHON_BIN_DIR:/usr/bin:/bin:/usr/sbin:/sbin}
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$SQUAD_FAKE_TMUX_CALL_LOG"
state=$(cat "$SQUAD_FAKE_KIMI_STATE" 2>/dev/null || true)
fake_screen() {
  case "$state" in
    ready)
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ -n "$literal" ]; then
      case "$literal" in
        *' --auto')
          printf '%s\n' "$literal" >> "$SQUAD_FAKE_LAUNCH_LOG"
          printf 'launched\n' > "$SQUAD_FAKE_KIMI_STATE"
          ;;
        *)
          printf '%s\n' "$literal" >> "$SQUAD_FAKE_POINTER_LOG"
    ...
    case " $* " in
      *' Enter '*)
        case "$state" in
          launched)
            if [ "${SQUAD_FAKE_KIMI_READY:-yes}" = yes ]; then
              printf 'ready\n' > "$SQUAD_FAKE_KIMI_STATE"
            fi
            ;;
          pointer-typed)
            if [ "${SQUAD_FAKE_KIMI_DELIVERY:-yes}" = yes ]; then
              if [ "${SQUAD_FAKE_KIMI_SWALLOW_FIRST:-no}" = yes ] \
                 && [ ! -f "$SQUAD_FAKE_KIMI_SWALLOWED" ]; then
                : > "$SQUAD_FAKE_KIMI_SWALLOWED"
              else
                printf 'delivered\n' > "$SQUAD_FAKE_KIMI_STATE"
              fi
            else
...
run_spawn() {
  ...
  HOME="$home" SQUAD_ROOT_OVERRIDE='' SQUAD_HOME="$home" \
    ...
    SQUAD_FAKE_LAUNCH_LOG="$case_dir/launch.log" \
    SQUAD_FAKE_POINTER_LOG="$case_dir/pointer.log" \
    SQUAD_FAKE_KIMI_STATE="$case_dir/kimi.state" \
    SQUAD_FAKE_KIMI_SWALLOWED="$case_dir/kimi.swallowed" \
    SQUAD_FAKE_KIMI_SWALLOW_FIRST="${SQUAD_FAKE_KIMI_SWALLOW_FIRST:-no}" \
    SQUAD_FAKE_TMUX_CALL_LOG="$case_dir/tmux-calls.log" \
...
test_kimi_launch_then_send_is_verified() {
  ...
  out=$(SQUAD_FAKE_KIMI_SWALLOW_FIRST=yes run_spawn \
  ...
  assert_grep 'BEGIN SQUAD KIMI TURN-END HOOK' "$HOME_DIR/.kimi-code/config.toml" \
    "kimi spawn did not install its guarded global hook region"
  assert_grep 'token=' "$WT_DIR/.sq-kimi-turnend" "kimi spawn did not write its token pointer"
  assert_present "$HOME_DIR/state/$id.kimi-turnend-token" "kimi spawn did not record its token"
  pass "sq-spawn: kimi launches, delivers its brief, and registers a guarded turn-end token"
}
...
test_kimi_hook_install_is_surgical_idempotent_and_removable() {
  ...
  cat > "$config" <<'EOF'
# Commander's leading comment stays exactly here.
...
EOF
  ...
  count=$(grep -c '^# BEGIN SQUAD KIMI TURN-END HOOK' "$config")
  [ "$count" -eq 1 ] || fail "idempotent install left $count Squad regions"

  HOME="$home" "$KIMI_HOOK" remove || fail "Kimi hook removal failed"
  cp "$config" "$stripped"
  cmp -s "$original" "$stripped" \
    || fail "config with the Squad region excised was not byte-identical to the original"
  assert_absent "$home/.kimi-code/sq-turn-end.sh" "removal left the Squad hook script"
  assert_absent "$home/.kimi-code/sq-turn-end.d" "removal left the Squad registry"
...
test_kimi_hook_remove_preserves_owned_newline_boundary() {
  ...
  cmp -s "$expected" "$config" \
    || fail "removal did not preserve appended commander config on its own line"
  "$PYTHON_BIN" - "$config" <<'PY' || fail "config with appended commander TOML did not parse after removal"
import sys
import tomllib
...
test_kimi_hook_fails_closed_on_missing_malformed_or_partial_config() {
  ...
  assert_absent "$missing/.kimi-code/sq-turn-end.sh" "missing config refusal wrote the hook script"
  ...
  assert_absent "$malformed/.kimi-code/sq-turn-end.sh" "malformed config refusal wrote the hook script"

  printf '# BEGIN SQUAD KIMI TURN-END HOOK\n' > "$partial/.kimi-code/config.toml"
  cp "$partial/.kimi-code/config.toml" "$partial/before"
  rc=0
  out=$(HOME="$partial" "$KIMI_HOOK" install 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "partial Squad marker was accepted"
...
test_kimi_hook_install_refuses_without_jq() {
  ...
  assert_absent "$home/.kimi-code/sq-turn-end.sh" "missing-jq refusal wrote the hook script"
  assert_absent "$home/.kimi-code/sq-turn-end.d" "missing-jq refusal wrote the registry"
...
test_kimi_hook_is_silent_and_requires_registered_workspace_token() {
  ...
  hook="$HOME_DIR/.kimi-code/sq-turn-end.sh"
  target="$HOME_DIR/state/$id.turn-ended"
  token=$(sed -n 's/^token=//p' "$WT_DIR/.sq-kimi-turnend")
  assert_present "$HOME_DIR/.kimi-code/sq-turn-end.d/$token" "Kimi registry token is missing"
  ...
  pass "Kimi hook stays silent and inert without a Squad registry token"
}
...
test_kimi_spawn_refuses_unsafe_global_config_before_pane_creation() {
  ...
  pass "sq-spawn: unsafe Kimi global config refuses before pane creation"
}
...
test_kimi_teardown_removes_pointer_and_registry_token() {
  ...
  assert_absent "$WT_DIR/.sq-kimi-turnend" "Kimi token pointer survived teardown"
  assert_absent "$HOME_DIR/.kimi-code/sq-turn-end.d/$token" "Kimi registry token survived teardown"
  assert_absent "$HOME_DIR/state/$id.kimi-turnend-token" "Kimi token state survived teardown"
  pass "sq-teardown: Kimi task pointer and registry token are removed"
}
...
test_kimi_falls_back_to_expanded_home_binary() {
  ...
  pass "sq-spawn: Kimi fallback expands the active HOME"
}
...
test_kimi_missing_binary_refuses_before_pane_creation() {
  ...
  pass "sq-spawn: missing Kimi executable refuses before pane creation"
}
...
test_kimi_unconfirmed_delivery_fails_loudly() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(SQUAD_FAKE_KIMI_DELIVERY=no run_spawn \
  ...
  pass "sq-spawn: kimi treats a silent pointer drop as a failed spawn"
}
...
test_kimi_readiness_gate_precedes_pointer() {
  ...
  read_spawn_record "$rec"
  rc=0
  out=$(SQUAD_FAKE_KIMI_READY=no run_spawn \
    "$CASE_DIR" "$HOME_DIR" "$PROJ_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$id") || rc=$?
  ...
  pass "sq-spawn: kimi never sends the brief pointer before an observable ready signal"
}
...
SH
  ...
  out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
    PATH="$fakebin:$BASE_PATH" SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh")
  [ "$out" = kimi ] || fail "kimi ancestry detection returned '$out'"
  out=$(CLAUDECODE=1 PATH="$fakebin:$BASE_PATH" SQUAD_CONFIG_OVERRIDE="$cfg" "$ROOT/bin/sq-harness.sh")
  [ "$out" = claude ] || fail "verified env-marker precedence changed, got '$out'"
  pass "sq-harness: markerless kimi is detected by ancestry after env-marker precedence"
}
...
test_sentry_never_classifies_kimi_from_its_spinner() (
  ...
  if window_is_busy fake "$busy_capture"; then
    fail "sq-sentry classified a Kimi task busy from its spinner instead of unknown"
  fi
  ...
  printf 'window=fake\nharness=codex\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
    fail "sq-sentry applied Kimi's spinner to a recorded Codex task"
  fi
  printf 'window=fake\nharness=grok\n' > "$state/kimi-watch.meta"
  if window_is_busy fake "$busy_capture"; then
  ...
  pass "sq-sentry classifies Kimi as unknown rather than from its spinner, and Grok's fallback stays isolated"
)

test_kimi_bordered_prompt_needs_no_override() {
  local out
  # shellcheck source=/dev/null
  . "$ROOT/bin/sq-composer-lib.sh"
