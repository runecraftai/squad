...
// A trailing dependency edge, optionally carrying Squad's free-text reason
...
export function buildProse(task: Task): string {
  ...
  // A bare edge sits right after the title (Squad's `blocked-by: <id>
  ...
  // is emitted after the parenthetical tags - both to match Squad's real
...
function bulletPrefix(state: State, id: string): string {
  ...
  // match Squad's real backlog format; the section header carries the state.
  // A legacy `- **<id>**` line is still parsed, but normalizes to `- [ ]` so a
  // mutated file stays readable by Squad (never rewritten the other way).
  return `- [ ] ${id} - `;
}
