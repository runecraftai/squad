...
- The `sq-report new` starter templates are split between `src/templates.js` (kinds, token kits, per-kit class maps, and the `buildTemplate` renderer) and `src/components.js` (the text-fill catalog: one builder per catalog block, emitting markup only through `kit.cls()` lookups so authors fill prose, never classes).
  ...
  The offline kit must never receive a class its CSS does not define: every emitted class routes through the kit class maps into `BRAND_CSS` `.sq-*` rules, and the masthead keeps the tool name out of every `h1` - `Rendered with sq-report` appears once, in the colophon. Both invariants are enforced by `test/templates.test.js` (no-dead-classes check and h1/colophon placement); never reintroduce literal Tailwind utilities into the sq-report kit's markup.
