...
func TestPRStep_UpdatesExistingPR(t *testing.T) {
	...
	if !strings.Contains(ghLog, drillPRSignature) {
		t.Errorf("expected updated PR body to include drill signature, got:\n%s", ghLog)
	}
...
func TestPRStep_GitHubForkCreatesParentPRWithForkHead(t *testing.T) {
	...
	sctx := newTestContextWithDBRecords(t, ag, dir, baseSHA, headSHA, config.Commands{})
	sctx.Env = env
	sctx.Repo.UpstreamURL = "https://github.com/parent-owner/drill.git"
	sctx.Repo.ForkURL = "https://github.com/fork-owner/drill.git"
	...
	ghLog := string(logData)
	if !strings.Contains(ghLog, "pr list --head feature --base main --repo parent-owner/drill --state open --json number,url,headRefName,headRepositoryOwner") {
	...
	if !strings.Contains(ghLog, "pr create --head fork-owner:feature --base main --repo parent-owner/drill") {
		t.Fatalf("expected PR create to target parent repo with fork owner head, got:\n%s", ghLog)
	}
	if strings.Contains(ghLog, "--repo fork-owner/drill") {
...
func TestPRStep_BitbucketUsesProcessEnvWhenStepEnvIsNil(t *testing.T) {
	dir, baseSHA, headSHA := setupGitRepo(t)
	api := newFakeBitbucketPRAPI(t, 0, "")
	t.Setenv("DRILL_BITBUCKET_EMAIL", "test@example.com")
	t.Setenv("DRILL_BITBUCKET_API_TOKEN", "test-token")
	t.Setenv("DRILL_BITBUCKET_API_BASE_URL", api.server.URL)
...
func TestAssemblePRBody_RetainsAttestationWhenCoreExceedsAzureCap(t *testing.T) {
	...
	pipelineMD = strings.Replace(pipelineMD, drillPRSignature+"\n\n", drillPRSignature+"\n\n"+attestation+"\n\n", 1)
	...
	for _, want := range []string{"## Pipeline", drillPRSignature, attestation} {
		if !strings.Contains(got, want) {
...
func pipelineMarkdownForTest(rounds ...string) string {
	...
	b.WriteString("## Pipeline\n\nUpdates from [git push drill](https://github.com/runecraftai/squad/packages/drill)\n\n")
