...
func TestMarkdownFrontmatter(t *testing.T) {
	...
	if !strings.Contains(md, "drill axi run") {
...
func TestBodyDocumentsAxiGateGuidance(t *testing.T) {
	md := Markdown()
	for _, want := range []string{
		"inspect it with `drill axi status`",
		"drive it with `drill axi respond`",
...
// repo still carries a vendored skill copy from an older drill version.
