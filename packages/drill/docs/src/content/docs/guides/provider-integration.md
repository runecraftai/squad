...
Without any provider setup, `drill` still gives you the local gate:
...
|---|---|---|---|---|
| **PR** (create/update) | `gh` CLI, authenticated | `glab` CLI, authenticated | `DRILL_BITBUCKET_EMAIL` + `DRILL_BITBUCKET_API_TOKEN` | `az` CLI + `azure-devops` extension, authenticated |
...
| **[Cancelled-check rerun](/drill/reference/repo-config/#cirerun_transient)** | `gh` CLI | not supported | not supported | not supported |

## What changes when provider wiring is present

Once the host is wired up, `drill` can keep owning the branch after it
...
`drill doctor` also checks for `gh` availability.
For PR and workflow-run commands, drill passes the repository slug from the recorded upstream remote or PR URL to `gh`, so daemon-run commands do not depend on the daemon's current working directory.
...
drill init --fork-url git@github.com:your-user/repo.git
```
...
Re-running `drill init` later preserves the stored fork URL unless you pass a new `--fork-url`.
...
```sh
export DRILL_BITBUCKET_EMAIL=you@example.com
export DRILL_BITBUCKET_API_TOKEN=your-api-token

# Optional: override the API base URL
export DRILL_BITBUCKET_API_BASE_URL=https://api.bitbucket.org/2.0
```
...
- PR creation and update (`az repos pr create` / `update`); Azure DevOps caps
  ...
  See the [PR step reference](/drill/reference/pipeline-steps/#pr) for
...
Self-hosted GitHub Enterprise and self-hosted GitLab instances work through the same `gh` and `glab` CLIs. Authenticate the CLI against your instance (`gh auth login --hostname your-ghe.example.com`, `glab auth login --hostname gitlab.example.com`) and `drill` will route through the CLI as usual.
...
When the upstream hostname is not `github.com`, `drill` consults gh's configured hosts (`hosts.yml`, honoring `GH_CONFIG_DIR` then `XDG_CONFIG_HOME/gh`, then `~/.config/gh`) and treats the upstream as GitHub if its host appears there.
...
`drill` builds that automatically from the recorded upstream remote or PR URL, so daemon-run `gh` commands resolve the right repository regardless of the daemon's working directory.
...
When the hostname is not obviously GitLab, `drill` consults glab's configured hosts (`config.yml`, honoring `GLAB_CONFIG_DIR` then `XDG_CONFIG_HOME/glab-cli`, then `~/.config/glab-cli`) and treats the upstream as GitLab if its host appears there as a configured host or `api_host`.
...
- The **push** step still runs - `drill` pushes through git to the configured target like any other remote.
...
:::note
When the daemon runs through a managed service (launchd, systemd, Task Scheduler), it reloads environment from your login shell on macOS and Linux so `gh` auth and `DRILL_BITBUCKET_*` vars are picked up, and it augments `PATH` with common binary directories. If credentials or PATH-derived tools are missing, check `~/.drill/logs/daemon.log` for a login-shell environment resolution warning. On Windows it reuses the current process environment.
:::
