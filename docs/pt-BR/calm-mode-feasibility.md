...
Se o Pi entregar `agent_settled` ou `session_shutdown` com um contexto de UI invalidado por substituição ou reload de sessão, o Calm suprime apenas a falha identificada de limpeza por contexto obsoleto; erros de UI não relacionados continuam propagando, e a limpeza com contexto ativo permanece inalterada.
...
Ele também exercita contextos de UI obsoletos nos callbacks finais do ciclo de vida e verifica que falhas de UI não relacionadas continuam visíveis.
