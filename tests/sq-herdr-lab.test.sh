#!/usr/bin/env bash
# Behavior tests for bin/sq-herdr-lab.sh using a stateful fake Herdr client.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(fm_test_tmproot sq-herdr-lab)
...
case "$1 ${2:-}" in
  "session list")
    ...
    ;;
  "server --session")
    if [ "${SQUAD_FAKE_HERDR_SERVER_DELAY:-0}" != 0 ]; then
      "$SQUAD_FAKE_HERDR_REAL_SLEEP" "$SQUAD_FAKE_HERDR_SERVER_DELAY"
    fi
  ...
  "session delete")
    [ "$3" = "$session" ] || exit 92
    [ "${SQUAD_FAKE_HERDR_DELETE_FAIL:-}" != 1 ] || exit 93
...
. "$ROOT/bin/sq-herdr-lab.sh"

run_with_fake() {
  PATH="$FAKEBIN:$PATH" \
    SQUAD_FAKE_HERDR_STATE="$FAKE_STATE" \
    SQUAD_FAKE_HERDR_LOG="$FAKE_LOG" \
    SQUAD_FAKE_HERDR_REAL_SLEEP="$REAL_SLEEP" \
    SQUAD_FAKE_HERDR_SERVER_DELAY="${SQUAD_FAKE_HERDR_SERVER_DELAY:-0}" \
    SQUAD_FAKE_HERDR_FAST_POLL="${SQUAD_FAKE_HERDR_FAST_POLL:-}" \
    SQUAD_FAKE_HERDR_DELETE_FAIL="${SQUAD_FAKE_HERDR_DELETE_FAIL:-}" \
    SQUAD_HERDR_LAB_STATE_DIR="$TRIPWIRES" \
    "$@"
}
...
test_refuses_unsafe_names() {
  ...
  fm_herdr_lab_validate_name sq-lab-safe-123 || fail "valid lab session name was refused"
  generated=$(fm_herdr_lab_name sq-autodetect-smoke-concurrency-h3)
  ...
  pass "sq-herdr-lab: names fail closed and require the lab prefix"
}

test_provision_run_and_guarded_teardown() {
  local name='' line_count status=0 stop_line delete_line
  name="sq-lab-behavior-$$"
  : > "$FAKE_LOG"
  ...
  assert_present "$TRIPWIRES/$name.unit-state.json" "provision did not record the unit-state tripwire"
  ...
  assert_absent "$TRIPWIRES/$name.unit-state.json" "successful teardown left its tripwire behind"
  ...
  pass "sq-herdr-lab: provisioning, scoped calls, guarded teardown, and unit tripwire are deterministic"
}

test_missing_tripwire_blocks_destruction() {
  local name="sq-lab-no-tripwire-$$" status=0 before after
  ...
  pass "sq-herdr-lab: missing tripwire refuses teardown before any Herdr call"
}

test_changed_default_trips_after_teardown() {
  local name="sq-lab-tripwire-change-$$" status=0
  : > "$FAKE_LOG"
  ...
  run_with_fake fm_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "changed default unit state must fail teardown"
  assert_present "$TRIPWIRES/$name.unit-state.json" "failed tripwire should retain evidence"
  printf '%s\n' '/home/test/.config/herdr/herdr.sock' > "$FAKE_STATE/default-socket"
  rm -f "$TRIPWIRES/$name.unit-state.json"
  pass "sq-herdr-lab: changed default unit state is a hard failure"
}

test_stopped_owned_lab_can_reprovision() {
  local name="sq-lab-reprovision-$$"
  : > "$FAKE_LOG"
  ...
  assert_present "$TRIPWIRES/$name.unit-state.json" "stop removed the lab ownership tripwire"
  ...
  assert_present "$TRIPWIRES/$name.unit-state.json" "re-provision removed the lab ownership tripwire"
  run_with_fake fm_herdr_lab_teardown "$name" || fail "teardown after re-provision failed"
  pass "sq-herdr-lab: an owned stopped lab can re-provision safely"
}

test_failed_delete_retains_tripwire() {
  local name="sq-lab-delete-failure-$$" status=0
  : > "$FAKE_LOG"
  run_with_fake fm_herdr_lab_provision "$name" || fail "delete-failure fixture provision failed"
  SQUAD_FAKE_HERDR_DELETE_FAIL=1 run_with_fake fm_herdr_lab_teardown "$name" >/dev/null 2>&1 || status=$?
  ...
  assert_present "$TRIPWIRES/$name.unit-state.json" "failed delete removed the ownership tripwire"
  run_with_fake fm_herdr_lab_teardown "$name" || fail "retry after failed delete did not clean up the lab session"
  assert_absent "$TRIPWIRES/$name.unit-state.json" "successful retry left the ownership tripwire behind"
  pass "sq-herdr-lab: failed deletion retains ownership until absence is confirmed"
}

test_timed_out_provision_cancels_late_launch() {
  local name="sq-lab-late-launch-$$" status=0
  cat > "$FAKEBIN/sleep" <<'SH'
#!/usr/bin/env bash
if [ "${SQUAD_FAKE_HERDR_FAST_POLL:-}" = 1 ]; then
  exit 0
fi
exec "$SQUAD_FAKE_HERDR_REAL_SLEEP" "$@"
SH
  chmod +x "$FAKEBIN/sleep"
  : > "$FAKE_LOG"
  SQUAD_FAKE_HERDR_FAST_POLL=1 SQUAD_FAKE_HERDR_SERVER_DELAY=30 \
    run_with_fake fm_herdr_lab_provision "$name" >/dev/null 2>&1 || status=$?
  expect_code 1 "$status" "timed-out provision must fail"
  assert_present "$TRIPWIRES/$name.unit-state.json" \
  ...
  assert_absent "$TRIPWIRES/$name.unit-state.json" \
  ...
  pass "sq-herdr-lab: timed-out provisioning cancels the launch before teardown"
}
