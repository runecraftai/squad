...
import (
	...
	pipelinepkg "github.com/runecraftai/squad/packages/drill/internal/pipeline"
...
func (s *cancellationRaceStep) Execute(sctx *pipelinepkg.StepContext) (*pipelinepkg.StepOutcome, error) {
	...
	if _, err := gitpkg.Run(sctx.Ctx, sctx.WorkDir, "commit", "-m", "drill(review): fix"); err != nil {
		return nil, err
	}
...
func newRecoverFixture(t *testing.T, status types.RunStatus) *recoverFixture {
	...
	mustRun(t, pipeline, "commit", "-m", "drill(review): fix")
	mustWrite(t, filepath.Join(pipeline, "fix.txt"), "pipeline fix 2\n")
	mustRun(t, pipeline, "commit", "-am", "drill(lint): fix")
...
func (f *recoverFixture) anchorRef() string { return "refs/drill/recover/" + f.run.ID }
...
func TestActivePrePushRunStaysBlockedWithoutRecovery(t *testing.T) {
	...
	if state.NextAction == nil || state.NextAction.Code != "continue_active_run" || state.NextAction.Command != "drill axi status" {
...
func (f *recoverFixture) localAnchorRef() string {
	return "refs/drill/recover-local/" + f.run.ID
}
...
func TestRecoverRebasedPreservedHeadAdoptsWithoutEscalating(t *testing.T) {
	...
	mustRun(t, f.local, "fetch", "--no-tags", f.gate, "+refs/heads/feature/recover:refs/drill/test/preserved")
...
func TestRecoverRebasedPreservedHeadEscalatesWhenFixRoundsRewroteOperatorLines(t *testing.T) {
	...
	f := newRebasedRecoverFixtureWithPipelineWork(t, types.RunCancelled, func(t *testing.T, pipelineDir string) {
		...
		mustRun(t, pipelineDir, "commit", "-am", "drill(review): guard the second line")
	})
...
func TestRecoverSquashedEquivalentPreservedHeadAdopts(t *testing.T) {
	...
	mustRun(t, pipeline, "commit", "-m", "drill(rebase): squashed feature")
...
func TestRecoverSquashedPreservedHeadStillEscalatesForDroppedLocalWork(t *testing.T) {
	...
	mustRun(t, pipeline, "commit", "-m", "drill(review): squashed without the second line")
