...
const (
	...
	EnvInventory = "DRILL_E2E_DAEMON_INVENTORY"

	// EnvMaxConcurrent caps how many temporary E2E daemons may be live at once.
	EnvMaxConcurrent = "DRILL_E2E_DAEMON_MAX"
...
// DirFromEnv returns the inventory directory from DRILL_E2E_DAEMON_INVENTORY,
// or a stable per-user temp default when unset.
func DirFromEnv() string {
	if dir := os.Getenv(EnvInventory); dir != "" {
		return dir
	}
	return filepath.Join(os.TempDir(), "drill-e2e-daemon-inventory")
}
...
func (inv *Inventory) daemonsForRoot(drillHome string) ([]int, error) {
	if inv.findDaemons != nil {
		return inv.findDaemons(drillHome)
	}
	return FindDaemonsForRoot(drillHome)
}
...
// Register records or updates ownership for drillHome. pid may be 0 when the
// daemon has not started yet; SyncPID upgrades it later.
func (inv *Inventory) Register(id, drillHome, drillBin string, pid, ownerPID int) error {
	if inv == nil {
...
// UpdatePID sets the recorded pid for id (or drillHome match).
