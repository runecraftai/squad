...
# accounting command, primary-to-XO convergence, and exact reread bytes.
set -u
...
TMP_ROOT=$(fm_test_tmproot sq-startup-memory-budget)
BUDGET="$ROOT/bin/sq-startup-memory-budget.sh"
BOOTSTRAP="$ROOT/bin/sq-bootstrap.sh"
CONFIG_PUSH="$ROOT/bin/sq-config-push.sh"
...
SH
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
[ -z "${SQUAD_FAKE_TMUX_LOG:-}" ] || printf '%s\n' "$*" >> "$SQUAD_FAKE_TMUX_LOG"
case "$*" in
...
new_bootstrap_world() {
  ...
  printf '%s\n' '# Squad test root' > "$root/AGENTS.md"
...
test_primary_bootstrap_materializes_visible_default() {
  ...
  [ "$(<"$home/config/startup-memory-budget")" = 321 ] \
    || fail "bootstrap replaced a valid commander-selected budget"

  second="$TMP_ROOT/materialize/XO"
  mkdir -p "$second/config" "$second/data" "$second/state"
  printf '%s\n' sm > "$second/.sq-xo-home"
  run_bootstrap "$root" "$second" "$fakebin" >/dev/null
  [ ! -e "$second/config/startup-memory-budget" ] \
    || fail "XO bootstrap created an independent budget instead of awaiting inheritance"
  pass "primary bootstrap materializes only the visible default and preserves valid commander choices"
}
...
test_budget_accounting_reports_all_three_files_and_safe_failure() {
  ...
  assert_contains "$out" 'file=data/commander.md bytes=4 estimated_tokens=2 status=present' \
    "report did not account for commander memory"
  assert_contains "$out" 'file=data/commander-shared.md bytes=7 estimated_tokens=3 status=present' \
...
test_primary_budget_converges_with_exact_reread_and_safe_failures() {
  ...
  [ "$(<"$sm/config/startup-memory-budget")" = 321 ] \
    || fail "XO did not receive the primary budget bytes"
  ...
  [ ! -e "$sm/config/startup-memory-budget" ] \
    || fail "unsafe primary budget changed the converged XO copy"
...
echo '# all sq-startup-memory-budget tests passed'
