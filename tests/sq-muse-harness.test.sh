#!/usr/bin/env bash
# Behavior tests for the muse (Muse Code) operator adapter: harness detection,
# spawn launch shape and credential preflight, the XO refusal, the
...
HARNESS="$ROOT/bin/sq-harness.sh"
TMP_ROOT=$(fm_test_tmproot sq-muse-harness)
...
case "${1:-}" in
  show-environment)
    [ "${SQUAD_FAKE_WORKER_META_KEY:-}" = present ] || exit 1
  ...
  send-keys)
    ...
    for arg in "$@"; do
      if [ "$prev" = -l ]; then
        printf '%s\n' "$arg" >> "$SQUAD_FAKE_LAUNCH_LOG"
        if [ "${SQUAD_FAKE_EXECUTE_MUSE_LAUNCH:-}" = 1 ]; then
          case "$arg" in
            *"$SQUAD_FAKE_MUSE_EXECUTABLE"*) (cd "$SQUAD_FAKE_PANE_PATH" && bash -c "$arg") ;;
          esac
        fi
        break
      fi
...
#!/usr/bin/env bash
set -u
[ -n "${SQUAD_FAKE_HARNESS_RESULT:-}" ] || exit 0
...
run_muse_spawn() {  # <home> <proj> <wt> <fakebin> <id> [extra args...]
  ...
  SQUAD_ROOT_OVERRIDE='' SQUAD_HOME="$home" \
    ...
    SQUAD_FAKE_LAUNCH_LOG="$home/launch.log" \
    SQUAD_FAKE_MUSE_EXECUTABLE="$fakebin/muse" \
    SQUAD_FAKE_MUSE_VERSIONED="$fakebin/muse-bin-test-version" \
    SQUAD_FAKE_HARNESS_PROBE="$HARNESS" \
    SQUAD_FAKE_EXECUTE_MUSE_LAUNCH="${SQUAD_FAKE_EXECUTE_MUSE_LAUNCH:-}" \
    SQUAD_FAKE_HARNESS_RESULT="${SQUAD_FAKE_HARNESS_RESULT:-}" \
    SQUAD_FAKE_WORKER_META_KEY="${SQUAD_TEST_MUSE_WORKER_KEY-present}" \
    META_API_KEY="${SQUAD_TEST_MUSE_KEY-test-key}" \
    XDG_CONFIG_HOME="${SQUAD_TEST_MUSE_CONFIG_HOME-$home/xdgconfig}" \
    XDG_DATA_HOME="${SQUAD_TEST_MUSE_DATA_HOME-$home/xdgdata}" \
    PATH="$fakebin:$PATH" \
...
# sq-harness.sh from a child of it.
#
...
test_detects_versioned_process_ancestor() {
  ...
  for bin in muse-bin-0.1.0-R708.1 muse-bin-9.9.9-RZZZ.9 muse; do
    ...
    [ "$out" = muse ] || fail "sq-harness.sh under process '$bin' reported '$out', expected muse"
  done
...
test_detection_is_anchored() {
  ...
  for bin in musescore amuse notmuse-bin muse-binary muse-bind; do
    ...
    out=$(env -u CLAUDECODE -u PI_CODING_AGENT -u GROK_AGENT \
      "$dir/$bin" -c "r=\$(\"$HARNESS\"); printf '%s' \"\$r\"")
    [ "$out" != muse ] || fail "sq-harness.sh misdetected unrelated process '$bin' as muse"
  done
...
EOF
  ...
  out=$(CLAUDECODE=1 PI_CODING_AGENT=true GROK_AGENT=1 SQUAD_PI_HARNESS=pi-signed \
    SQUAD_FAKE_EXECUTE_MUSE_LAUNCH=1 SQUAD_FAKE_HARNESS_RESULT="$result" \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off)
  status=$?
...
$rec
EOF
  out=$(run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off)
  status=$?
  ...
  # --yolo is what makes an operator pane viable at all: without it muse holds
  ...
  # rules to Meta-hosted inference on every operator turn.
  ...
  # The commander accepted muse's self-update risk, so Squad must not pin it.
  assert_not_contains "$launch" 'MUSE_NO_AUTO_UPDATE' \
    "muse launch pinned auto-update, which the commander declined"
...
EOF
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
      --mode drill --yolo off --model muse-spark-1.2 --effort "$effort" >/dev/null \
...
$rec
EOF
  run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off >/dev/null \
...
EOF
  ...
  out=$(SQUAD_TEST_MUSE_KEY='' SQUAD_TEST_MUSE_WORKER_KEY='' run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
    --mode drill --yolo off)
  status=$?
...
$rec
EOF
  out=$(SQUAD_TEST_MUSE_KEY='caller-only-secret' SQUAD_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off)
  status=$?
  [ "$status" -ne 0 ] || fail "muse spawn accepted a caller-only META_API_KEY"
  assert_contains "$out" "set for sq-spawn but cannot be proven present" \
...
EOF
  ...
  SQUAD_TEST_MUSE_KEY='' SQUAD_TEST_MUSE_WORKER_KEY='' \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" \
    --mode drill --yolo off >/dev/null
  status=$?
...
EOF
  ...
  out=$(cd "$caller" && SQUAD_TEST_MUSE_KEY='' SQUAD_TEST_MUSE_WORKER_KEY='' \
    SQUAD_TEST_MUSE_CONFIG_HOME=cfg SQUAD_TEST_MUSE_DATA_HOME=data \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off)
  status=$?
...
# dialect rejects the model-reawakening handlers a Squad primary needs, so a
# XO on muse could never arm a supervision cycle.
test_spawn_refuses_XO() {
  local case_dir home fakebin id out status
  case_dir="$TMP_ROOT/XO"
  home="$case_dir/home"
  fakebin=$(make_spawn_fakebin "$case_dir/fake")
  id="muse-XO-x1"
  ...
  out=$(cd "$case_dir" && SQUAD_ROOT_OVERRIDE='' SQUAD_HOME="$home" \
    ...
    PATH="$fakebin:$PATH" \
    "$SPAWN" "$id" muse --xo 2>&1)
  status=$?
  [ "$status" -ne 0 ] || fail "muse was accepted as an XO harness"
  assert_contains "$out" "operator/recon adapter only" "muse XO refusal did not explain the boundary"
  pass "muse is refused as an XO harness"
}
...
EOF
  ...
  prior=$(printf '%s\n' "$prior" | sed 's://*:/:g')
  SQUAD_TEST_MUSE_DATA_HOME="$case_dir/xdgdata" \
    run_muse_spawn "$home" "$proj" "$wt" "$fakebin" "$id" --mode drill --yolo off >/dev/null \
...
case "${1:-}" in
  ...
  has-session) exit 0 ;;
  list-panes|list-windows) printf 'sq-send:0\n'; exit 0 ;;
  send-keys)
    shift
    printf '%s\n' "$*" >> "$SQUAD_FAKE_KEY_LOG"
    [ "${SQUAD_FAKE_KEY_FAIL:-}" = "$*" ] && exit 1
    exit 0
    ;;
esac
exit 0
SH
  ...
  fm_write_meta "$home/state/$id.meta" \
    "window=sq-send:0" "endpoint_task_id=$id" "worktree=$case_dir" \
    "project=$case_dir" "harness=$harness" "kind=strike" "mode=drill" "yolo=off"
...
EOF
  keylog="$case_dir/keys.log"
  : > "$keylog"
  out=$(SQUAD_FAKE_KEY_FAIL='-t sq-send:0 C-u' run_send_key "$home" "$fakebin" "$id" Escape "$keylog")
  status=$?
...
# report a working operator as finished.
