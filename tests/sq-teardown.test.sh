#!/usr/bin/env bash
# Tests for bin/sq-teardown.sh's landed-work safety and stale-lock recovery.
#
# The check refuses to tear down a worktree whose work has not LANDED, because
# fob return hard-resets the worktree. "Landed" means reachable from a remote
# OR - for a normal strike task whose commits are not so reachable - its PR is merged
...
TMP_ROOT=$(fm_test_tmproot sq-teardown-tests)
...
#   $CASE/state/        - Squad state dir (with a fresh sentry beacon)
#   $CASE/fakebin/      - mocks for fob, tmux (PATH-prepended by caller)
#   $CASE/origin.git/   - bare upstream repo (so the project clone has origin)
#   $CASE/project/      - clone of origin; acts as the Squad project dir
...
make_case() {
  ...
  cat > "$fakebin/fob" <<'SH'
#!/usr/bin/env bash
# `fob return --force <wt>`: succeed silently.
exit 0
SH
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
...
run_teardown() {
  ...
  SQUAD_CONFIG_OVERRIDE="$case_dir/config" \
  PATH="$case_dir/fakebin:${SQUAD_TEARDOWN_TEST_PATH:-$PATH}" \
    "$TEARDOWN" task-x1 "$@"
}
...
test_pr_check_does_not_refresh_stale_pr_head() {
  ...
  pass "sq-pr-check does not refresh PR head after HEAD moves"
}
...
test_pr_check_records_remote_head_when_local_lags() {
  ...
  pass "sq-pr-check records the remote PR head when the local worktree lags"
}
...
test_transient_index_lock_clears_after_first_attempt_and_retry_succeeds() {
  ...
  [ "$(cat "$attempt_file")" = 2 ] \
    || fail "transient-index-lock: expected exactly 2 fob return attempts, got $(cat "$attempt_file")"
...
test_empty_retry_wait_uses_default_without_aborting() {
  ...
  [ "$(cat "$attempt_file")" = 2 ] \
    || fail "empty-retry-wait: expected exactly 2 fob return attempts, got $(cat "$attempt_file")"
...
case "\${1:-} \${2:-}" in
  ...
  "status --json")
    ...
    ;;
  "session list")
    if [ "\${SQUAD_FAKE_HERDR_SESSION_LIST_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
    else
      printf '%s\n' '{"sessions":[{"name":"default","running":true,"socket_path":"$case_dir/herdr.sock"}]}'
    fi
    ;;
  "pane close")
    : > "\${SQUAD_FAKE_HERDR_CLOSED:?}"
    ;;
  "pane get")
    if [ "\${SQUAD_FAKE_HERDR_PANE_GET_GARBAGE:-0}" = 1 ]; then
      printf '%s\n' 'not-json'
      exit 0
    fi
    if [ -e "\${SQUAD_FAKE_HERDR_CLOSED:?}" ]; then
...
test_herdr_flat_teardown_refuses_orphaning_records_then_retry_completes() {
  ...
  # Record every fob invocation: the contended-lock refusal must fire
  # BEFORE the isolated copy is returned, so phase 1 may not invoke it at all.
  thlog="$case_dir/fob.log"; : > "$thlog"
  cat > "$case_dir/fakebin/fob" <<SH
#!/usr/bin/env bash
printf '%s\n' "\$*" >> "$thlog"
exit 0
SH
  ...
  lock=$(SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" PATH="$case_dir/fakebin:$PATH" \
  ...
  rc=0
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" \
  ...
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
...
test_herdr_flat_teardown_refuses_records_on_unparseable_presence() {
  ...
  : > "$case_dir/state/task-x1.status"
  rc=0
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_FAKE_HERDR_PANE_GET_GARBAGE=1 \
    SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
...
SH
  ...
  case "$mode" in
    missing-adapter|missing-parser|missing-explicit-close-helper)
      ...
      teardown_bin="$case_dir/test-root/bin/sq-teardown.sh"
      ;;
  esac
  rc=0
  SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_STATE_OVERRIDE="$case_dir/state" SQUAD_CONFIG_OVERRIDE="$case_dir/config" \
    SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" \
    SQUAD_FAKE_HERDR_SESSION_LIST_GARBAGE="$([ "$mode" = unresolvable-lock ] && printf 1 || printf 0)" \
    PATH="$case_dir/fakebin:$PATH" \
...
configure_XO_with_herdr_child() {  # <case-dir>
  local case_dir=$1 home="$1/XO-home"
  mkdir -p "$home/state" "$home/data" "$home/config" "$home/projects"
  printf '%s\n' task-x1 > "$home/.sq-xo-home"
...
case "\${1:-} \${2:-}" in
  ...
  "pane get")
    if [ -e "\${SQUAD_FAKE_HERDR_CLOSED:?}" ]; then
      if [ "\${SQUAD_FAKE_HERDR_PRESENCE_UNKNOWN:-0}" = 1 ]; then
        printf '%s\n' 'not-json'
      else
        printf '%s\n' '{"error":{"code":"pane_not_found"}}' >&2
        exit 1
    ...
    ;;
  "pane close") : > "\${SQUAD_FAKE_HERDR_CLOSED:?}" ;;
esac
SH
  chmod +x "$case_dir/fakebin/herdr"
}
...
SH
  ...
  rc=0
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" \
    SQUAD_FAKE_HERDR_SESSION_LIST_GARBAGE=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  ...
  [ -d "$home" ] || fail "herdr-child-preflight: refusal removed the XO home"
  ...
  pass "forced XO teardown preflights every Herdr child before cleanup mutation"
}
...
test_forced_XO_herdr_child_retains_records_when_close_unconfirmed() {
  ...
  rc=0
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  ...
  [ -d "$home" ] || fail "herdr-child-unconfirmed-close: failed child cleanup removed the XO home"
  ...
  pass "forced XO teardown retains Herdr child identity until exact pane disappearance"
}

configure_nested_XO_with_herdr_grandchild() {  # <case-dir>
  local case_dir=$1 home="$1/XO-home" nested_home="$1/XO-home/nested-home"
  mkdir -p "$home/state" "$home/data" "$home/config" "$home/projects"
  mkdir -p "$nested_home/state" "$nested_home/data" "$nested_home/config" "$nested_home/projects"
  printf '%s\n' task-x1 > "$home/.sq-xo-home"
  printf '%s\n' nested-sm > "$nested_home/.sq-xo-home"
  printf '%s\n' "home=$home" >> "$case_dir/state/task-x1.meta"
  fm_write_meta "$home/state/nested-sm.meta" \
    "window=Squad:sq-nested-sm" \
...
test_forced_teardown_retains_nested_XO_home_when_grandchild_close_unconfirmed() {
  ...
  [ -d "$nested_home" ] \
    || fail "herdr-grandchild-unconfirmed-close: the recursive failure still removed the nested XO home"
  ...
  [ -e "$home/state/nested-sm.meta" ] \
    || fail "herdr-grandchild-unconfirmed-close: the recursive failure erased the nested XO's own record"
  [ -e "$case_dir/state/task-x1.meta" ] \
    || fail "herdr-grandchild-unconfirmed-close: the recursive failure erased the top-level XO's record"
  pass "forced teardown retains a nested XO home and its grandchild's Herdr identity when the grandchild close is unconfirmed"
}
...
case "${1:-} ${2:-}" in
  ...
  "session list")
    ...
    ;;
  "pane close")
    if [ "${SQUAD_FAKE_HERDR_CLOSE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${SQUAD_FAKE_HERDR_CLOSED:?}"
    ;;
  ...
  "tab get")
    ...
    ;;
  "tab focus")
    if [ "${SQUAD_FAKE_HERDR_RESTORE_FAIL:-0}" = 1 ]; then
      exit 1
    fi
    : > "${SQUAD_FAKE_HERDR_RESTORED:?}"
...
test_herdr_projection_teardown_retires_journal_only_after_confirmed_close() {
  ...
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_FAKE_HERDR_RESTORED="$restored" \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
test_herdr_projection_teardown_retains_journal_when_close_unconfirmed() {
  ...
  local rc=0
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_FAKE_HERDR_RESTORED="$restored" SQUAD_FAKE_HERDR_PRESENCE_UNKNOWN=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
  [ "$rc" -ne 0 ] \
...
test_herdr_projection_teardown_surfaces_restore_failure_without_blocking_cleanup() {
  ...
  SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_CLOSED="$closed" SQUAD_FAKE_HERDR_RESTORED="$restored" \
    SQUAD_FAKE_HERDR_RESTORE_FAIL=1 \
    run_teardown "$case_dir" --force > "$case_dir/stdout" 2> "$case_dir/stderr" \
...
EOF
  ...
  assert_absent "$case_dir/fob.log" \
...
EOF
  ...
  rc=0
  SQUAD_TEARDOWN_TEST_PATH="$path_without_lsof" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
printf 'return\n' >> "$case_dir/fob.log"
EOF
  chmod +x "$case_dir/fakebin/lsof" "$case_dir/fakebin/fob"
  ...
  assert_absent "$case_dir/fob.log" "lsof-error-refusal: teardown returned the worktree"
...
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${SQUAD_FAKE_REUSED_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  count=0
  [ ! -f "$SQUAD_FAKE_PS_COUNT" ] || count=$(cat "$SQUAD_FAKE_PS_COUNT")
  count=$((count + 1))
  printf '%s\n' "$count" > "$SQUAD_FAKE_PS_COUNT"
...
SH
  ...
  rc=0
  SQUAD_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  SQUAD_FAKE_REUSED_PID="$pid" SQUAD_FAKE_PS_COUNT="$case_dir/ps-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
test_exec_changed_process_is_still_reaped() {
  ...
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${SQUAD_FAKE_EXEC_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
  out=$("$REAL_PS_FOR_TEST" "$@") || exit $?
  [ -e "$SQUAD_FAKE_EXEC_MARKER" ] || : > "$SQUAD_FAKE_EXEC_MARKER"
  printf '%s\n' "$out"
  exit 0
fi
exec "$REAL_PS_FOR_TEST" "$@"
...
#!/usr/bin/env bash
count=0
[ ! -f "$SQUAD_FAKE_LSOF_COUNT" ] || count=$(cat "$SQUAD_FAKE_LSOF_COUNT")
count=$((count + 1))
printf '%s\n' "$count" > "$SQUAD_FAKE_LSOF_COUNT"
if [ "$count" -eq 2 ]; then
  i=0
  while [ "$i" -lt 100 ]; do
    [ ! -e "$SQUAD_FAKE_EXEC_DONE" ] || break
    sleep 0.01
    i=$((i + 1))
  done
fi
...
SH
  ...
  rc=0
  SQUAD_PROC_ROOT_OVERRIDE="$case_dir/no-proc" \
  SQUAD_FAKE_EXEC_PID="$pid" SQUAD_FAKE_EXEC_MARKER="$marker" \
  SQUAD_FAKE_EXEC_DONE="$done_flag" SQUAD_FAKE_LSOF_COUNT="$case_dir/lsof-count" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${SQUAD_FAKE_PERSISTENT_PID:-}" ] \
   && [ "${3:-}" = -o ] && [ "${4:-}" = lstart= ]; then
...
SH
  ...
  rc=0
  SQUAD_PROC_ROOT_OVERRIDE="$case_dir/no-proc" SQUAD_FAKE_PERSISTENT_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?
...
EOF
  cat > "$case_dir/fakebin/ps" <<'SH'
#!/usr/bin/env bash
if [ "${1:-}" = -p ] && [ "${2:-}" = "${SQUAD_FAKE_EXITED_PID:-}" ]; then
  exit 1
fi
exec "$REAL_PS_FOR_TEST" "$@"
SH
...
printf 'returned\n' > "$case_dir/fob.log"
EOF
  chmod +x "$case_dir/fakebin/lsof" "$case_dir/fakebin/ps" "$case_dir/fakebin/fob"

  rc=0
  SQUAD_PROC_ROOT_OVERRIDE="$case_dir/no-proc" SQUAD_FAKE_EXITED_PID="$fake_pid" \
    run_teardown "$case_dir" > "$case_dir/stdout" 2> "$case_dir/stderr" || rc=$?

  expect_code 0 "$rc" "identity-exit-convergence: teardown should succeed"
  assert_present "$case_dir/fob.log" \
...
test_run_abort_precedes_process_reap_precedes_worktree_removal() {
  ...
  # A fob fake that snapshots, at the exact moment the destructive
