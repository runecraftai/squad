...
func TestPreReceiveHookScript(t *testing.T) {
	script := preReceiveHookScript("/opt/Drill/drill")
	for _, want := range []string{
		"#!/bin/sh",
		"DRILL_BIN='/opt/Drill/drill'",
...
func TestPostReceiveHookScript(t *testing.T) {
	script := postReceiveHookScript("/opt/Drill/drill")
	...
	if !strings.Contains(script, "DRILL_BIN='/opt/Drill/drill'") {
		t.Fatal("hook should embed the drill executable path")
	}
	...
	if !strings.Contains(script, "\"$DRILL_BIN\" daemon notify-push") {
		t.Fatal("hook should execute the embedded binary path")
	}
	if !strings.Contains(script, "command -v drill") {
...
func TestShellSingleQuote(t *testing.T) {
	tests := []struct {
		...
		{"plain", "/usr/bin/drill", "'/usr/bin/drill'"},
		{"spaces", "/opt/Drill/bin", "'/opt/Drill/bin'"},
...
func TestPostReceiveHookScriptWithQuotedPath(t *testing.T) {
	script := postReceiveHookScript("/opt/it's here/drill")
	if !strings.Contains(script, "DRILL_BIN='/opt/it'\"'\"'s here/drill'") {
...
func TestPostReceiveHookScriptDoesNotEvaluatePushOptions(t *testing.T) {
	...
	fakeBin := filepath.Join(base, "fake-drill")
...
func TestPostReceiveHook_ResolvesAbsoluteGateDir(t *testing.T) {
	...
	// Fake drill binary: record the notify-push argv so we can inspect
...
func TestPostReceiveHook_SurfacesNotifyFailures(t *testing.T) {
	...
	// Fake drill binary that always fails notify-push with a
