...
# bin/sq-spawn.sh installs under the contract owned by bin/sq-busy-lib.sh.
#
# These tests run the REAL sq-spawn against a fake tmux pane and an isolated
...
# bin/sq-busy-event.sh writer, and the real classifier are exercised together
...
TMP_ROOT=$(fm_test_tmproot sq-busy-adapter-wiring)
...
SH
  ...
  fm_fake_exit0 "$fakebin" fob pi opencode claude codex
  printf '%s\n' "$fakebin"
}
...
test_pi_extension_semantic_lifecycle() {
  ...
  out=$(classify pi "$id" "$state")
  [ "$out" = "busy sq-spawn" ] || fail "turn_end must stay a notification, not a state edge, got '$out'"
...
test_pi_extension_stale_incarnation_rejected() {
  ...
  "$ROOT/bin/sq-busy-event.sh" arm "$state" "$id" >/dev/null
...
test_opencode_plugin_semantic_lifecycle() {
  ...
  state="$HOME_DIR/state"
  plugin="$WT_DIR/.opencode/plugins/sq-busy-state.js"
...
test_claude_hooks_stale_incarnation_harmless() {
  ...
  out=$(classify claude "$id" "$state")
  [ "$out" = "busy sq-spawn" ] || fail "a stale-gen hook event must not change state, got '$out'"
...
echo "all sq-busy-adapter-wiring tests passed"
