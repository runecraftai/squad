...
- Runner: `bin/fm-test-run.sh` (upstream) with `PATH=/tmp/sq-tools:$PATH`; throwaway bases per-test (runner unsets FM_HOME internally per script).
...
### OQ-03 resolution — org real (session 3) ✅
- Repo público criado em `github.com/runecraftai/squad` (2026-08-10; login da org `runecraftai`, não `RuneCraft` — o usuário é admin da primeira). Push inicial via HTTPS (SSH sem chave registrada; token ganhou scope `workflow` via `gh auth refresh`).
- Sweep repo-wide do placeholder: `squad-org/squad` → `runecraftai/squad` em todos os arquivos tracked (README, CONTRIBUTING, docs, bin/, CI, packages — incluindo módulos Go `github.com/runecraftai/squad/packages/{fob,no-mistakes}` + imports internos; go build/test verdes). Context.md OQ-01/OQ-03 atualizados; textos históricos no STATE.md mantidos como registro da época.
