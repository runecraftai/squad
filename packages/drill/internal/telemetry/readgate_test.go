...
func TestReadSurfaceGate_ConcurrentProcessesEmitOnce(t *testing.T) {
	const helperEnv = "DRILL_READ_GATE_HELPER"
	if os.Getenv(helperEnv) == "1" {
		gate := NewReadSurfaceGate(os.Getenv("DRILL_READ_GATE_PATH"), 10*time.Minute, func() time.Time {
	...
	for range callers {
		go func() {
			...
			cmd.Env = append(os.Environ(), helperEnv+"=1", "DRILL_READ_GATE_PATH="+path)
