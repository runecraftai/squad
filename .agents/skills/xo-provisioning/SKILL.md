...
description: >-
  ...
  Use when creating, seeding, validating, launching, recovering, handing backlog to, pushing inherited local material into, or retiring an XO home, or when editing data/XOs.md.
  Covers local leases, whole-home remote routes, transactional seeding, record intake for an existing or inherited domain, project clone restrictions, XO harness pins, inherited local-material push, idle charter, handoff helper, and teardown safety.
...
A remote second-mate agent always runs on the Herdr backend and every seed, launch, and liveness relaunch first gates its host on `bin/sq-remote-doctor.sh` readiness, so an unready host refuses with that doctor's own gap text rather than half-creating a route; the workers that second mate supervises keep the home's ordinary backend selection.
This release places whole XO homes remotely and never individual workers.
...
Pass `--no-projects` instead of a project list to scaffold a project-less charter for a domain whose subject is the Squad repo itself, whose home is a Squad worktree and whose operators take pooled worktrees of the same repo.
...
`bin/sq-home-seed.sh` copies the charter into the XO home as `data/charter.md`.
...
Before a local launch, `sq-spawn.sh --xo` locally fast-forwards the home to the primary Squad checkout's current default-branch commit when it is safe; dirty, diverged, or in-flight homes launch unchanged with a warning.
The locked session-start deferred network stage runs the same bootstrap sweep for every live local XO home, discovered from `state/<id>.meta` records with `kind=xo` (`data/XOs.md` only backfills `home=` for older records).
...
`/updatesquad` first updates the remote code root from its own origin, then runs that guarded home sync.
...
Inherited `config/backend` becomes that XO home's local runtime-backend default for future spawns only; it never retargets, rewrites, migrates, stops, or restarts an already-live worker endpoint.
A present primary value always converges byte-exact into validated XO homes, and primary absence removes the destination so those homes keep runtime auto-detection.
Explicit per-spawn `--backend` and `SQUAD_BACKEND` remain stronger than every home's local `config/backend`, including an inherited default.
`config/xo-harness` is not inherited because it is only the primary's knob for launching XO agents.
`data/commander-shared.md` is main-authoritative in the primary home and read-only in XO homes.
...
Before replacing divergent XO bytes, the helper hash-compares source and destination, quarantines the XO-local version to a collision-safe private dated sibling file, and emits a `XO_SYNC:` diagnostic naming the home and quarantine artifact.
Never copy any XO `data/commander-shared.md` back into the primary.
Keep each home's `data/commander.md` domain-local.
After first propagation to an existing home, trim that home's local `data/commander.md` by hand to domain-specific content plus pointers to `data/commander-shared.md`; do not automate or silently delete private content.
...
A remote home receives the same allowlisted bytes through `sq-remote-inherit.sh` and gets one marked re-read instruction after a changed transfer.
...
For a local route, the helper resolves and validates the XO home from `data/XOs.md`, then delegates the item move to `tasks-axi mv` (the single owner of the backlog format), which moves each named item - and a whole connected set, blocker plus dependents, atomically - from the main `data/backlog.md` into the XO home's `data/backlog.md`.
For a remote route, the same helper first moves the dependency-closed set atomically from the main backlog into `data/handoff/<id>.outbox.md`, then transfers that backlog-format outbox through `sq-on.sh` and lets the remote home's `sq-backlog-receive.sh` move every not-already-present key under the destination lock.
...
It refuses any destination that is not a genuine seeded Squad home with safe operational directories and a matching `.sq-xo-home` marker, so a move can never land in a project.
...
If meta is missing but `data/XOs.md` still registers the XO, respawn from the registry entry and its persistent home.
...
Respawn re-resolves the XO harness from current config, uses the same guarded pre-launch sync, and re-propagates inherited local material, so recovered XOs converge inherited config items and shared commander preferences whenever their home validates; tracked-file sync remains guarded separately.
If the XO is already running and only inherited local material changed, prefer `bin/sq-config-push.sh` over respawning.

Do not reconstruct an XO's whole tree from the main home.
The main Squad reconciles only direct reports.
Each XO is a Squad in its own home, so it runs recovery on startup and reconciles its own operators.
...
The safety check is the XO's own home.
...
When safe, teardown kills the direct endpoint, removes the `data/XOs.md` route, clears the main home metadata, and removes the retired XO home.
Removing a leased home releases its durable fob lease via `fob return`, so the pool slot is freed for reuse rather than left leased forever.
A plain-clone home with no pool slot is simply removed.
If `fob return` fails for a leased home, teardown stops with state intact rather than raw-removing the directory and hiding a held lease.
...
It kills child windows, discards child work and state inside the XO home, removes the route, releases the lease, and removes the retired XO home.
