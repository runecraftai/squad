...
config/claude-accounts  optional registry mapping short labels to absolute CLAUDE_CONFIG_DIR paths for harness=claude account selection; LOCAL, gitignored; inherited by XO bases; see docs/configuration.md "Claude account selection"
...
state/               volatile runtime signals; gitignored
  <id>.status        appended by operators: "<state>: <note>" wake-event lines, not current-state truth; the last nonblank line is terminal completion evidence for bin/sq-exec-state.sh when its verb is done: or failed:
  ...
  <id>.meta          written by sq-spawn: window=, endpoint_task_id=, worktree=, project=, harness=, model=, effort=, kind=, mode=, yolo=, tasktmp=; an optional traceparent= only when trace context is enabled (docs/configuration.md "Trace context propagation"); a found WORKFLOW.md records workflow= and base64-encoded workflow_config= (docs/workflow-spec.md); kind=xo also records home= and projects=, plus remote_host=/remote_root=/remote_backend=/remote_herdr_session=/remote_target= for a remote route; an MCP-originated task records mcp_request= through bin/sq-mcp-link.sh; a non-default runtime backend records further backend-specific fields (docs/configuration.md "Runtime backend"; bin/sq-backend.sh, section 8); sq-pr-check, including through sq-pr-merge, records one canonical pr= and the forge's pr_head= when available (GitHub pull requests and GitLab merge requests; docs/gitlab-merge-watch.md); sq-x-link appends x_request=, x_request_ts=, x_followups=, and optional x_platform=/x_reply_max_chars= for a Relay-originated task (section 14)
  <id>.exec          per-attempt execution state sidecar: exec_state=, exec_attempt=, exec_retry_count=, exec_started_at=, exec_last_activity=, exec_previous_state=, exec_error=, exec_next_retry_at=, exec_max_retries=, exec_workflow_version=, exec_workspace=, exec_backend=, exec_harness=; missing for legacy tasks (unclaimed); managed by bin/sq-exec-state.sh; state machine: unclaimed -> claimed -> running -> retry_queued -> released; terminal done:/failed: in .status blocks claim and transition to running, and triggers release during recovery
...
3. **Wake queue** - when locked, drains the durable stand-to queue and prints the raw records prominently as this turn's first work queue; a bounded, clearly labeled historical status-event annotation may follow a valid `signal` record but never replaces it or current-state reconciliation, and a lapsed sentry chain still surfaces here via the same guard alarm.
   ...
   The same drain also prints a bounded `TEARDOWN PENDING` section, including on the empty-queue path, naming any task that reported `done:`/`failed:` but still has a live `state/<id>.meta` (`kind=xo` excluded); run `bin/sq-teardown.sh <id>` once its work is confirmed landed, never automatically.
...
After spawning, confirm the worker is processing the brief, handle any trust dialog through `harness-adapters`, and record strike or recon work as under way. When dispatching an MCP launch-brief, pass its leading request id to `bin/sq-mcp-link.sh <task-id> <request-id>` after the task meta exists so the requester receives the durable task id; ordinary dispatches omit the request id.
...
When merging several green PRs for one repository, order the batch by ascending PR number; if one still conflicts, rebase the oldest and revalidate it before continuing.
After an autonomous merge, give the commander a one-line full-URL or local-main outcome.

**Requested PR/diff review:** for an explicitly requested audit or knowledge-only review, use the read-only Drill surface documented in [`docs/pr-review.md`](docs/pr-review.md); it does not replace delivery through the full Drill pipeline.
...
For PR or diff work, `interrogate` uses the read-only Drill review surface in [`docs/pr-review.md`](docs/pr-review.md); separate reviews happen only when requested or when the deliverable is a knowledge-only review.
...
- `skill-forge` - load when creating, validating, or optimizing a new agent skill; vendored from `runecraftai/skills` (`packages/skills/skills/skill-forge/`), refresh by copying that upstream directory.
