...
func startColdDetachedFixture(t *testing.T, gateCount int, delayedGit bool) time.Duration {
	...
	if delayedGit {
		...
		shim := "#!/bin/sh\n" +
			...
			"exec \"$DRILL_TEST_REAL_GIT\" \"$@\"\n"
		if err := os.WriteFile(gitShim, []byte(shim), 0o755); err != nil {
			t.Fatal(err)
		}
		t.Setenv("DRILL_TEST_REAL_GIT", realGit)
	...
	t.Setenv("DRILL_DAEMON_HELPER_PROCESS", "daemon")
	t.Setenv("DRILL_TEST_DAEMON_START_TIMEOUT", "")
	t.Setenv("DRILL_TEST_DAEMON_START_POLL_INTERVAL", "20ms")
	...
	if evidenceDir := os.Getenv("DRILL_TEST_STARTUP_EVIDENCE_DIR"); evidenceDir != "" {
