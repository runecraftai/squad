#!/usr/bin/env bash
# sq-unit-view.sh - human renderer over sq-unit-snapshot.sh.
#
# This command intentionally does not parse unit state itself.
# It shells out to sq-unit-snapshot.sh --json and renders that stable
...
usage() {
  cat <<'EOF'
usage: sq-unit-view.sh [--json]

Render a human unit view from sq-unit-snapshot.sh.
...
case "${1:-}" in
  -h|--help) usage; exit 0 ;;
  --json) "$SCRIPT_DIR/sq-unit-snapshot.sh" --json; exit $? ;;
  "") ;;
  *) usage >&2; exit 2 ;;
esac

command -v jq >/dev/null 2>&1 || { echo "sq-unit-view: jq not found" >&2; exit 1; }

SNAPSHOT=$("$SCRIPT_DIR/sq-unit-snapshot.sh" --json) || exit $?
