...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/e2edaemon"
...
// drill binary against a fake agent. Each test gets its own Harness;
...
type SetupOpts struct {
	...
	// committed to the trusted default-branch .drill.yaml (never the
...
// NewHarness builds the drill + fakeagent binaries (once per test
// process), creates a temp git repo with origin, writes the drill
...
func NewHarness(t *testing.T, opts SetupOpts) *Harness {
	...
	drillBin, fakeBin := buildBinaries(t)
	...
	// test, so subprocesses spawned by drill inherit these. The
	...
	t.Setenv("DRILL_TEST_START_DAEMON", "1")
	...
	t.Setenv("DRILL_NO_UPDATE_CHECK", "1")
...
// writeGlobalConfig writes a drill global config that pins the
// agent name and binary path. The path override forces drill'
...
func (h *Harness) initGitRepos() {
	...
	// .drill.yaml (never global, never the pushed branch). The harness
	...
	repoConfig := filepath.Join(h.WorkDir, ".drill.yaml")
	...
	mustGit(h.WorkDir, "add", "README.md", ".drill.yaml")
...
// Run invokes the drill binary in the working repo and returns
...
// RunInDir invokes the drill binary in dir and returns stdout+stderr.
func (h *Harness) RunInDir(dir string, args ...string) (string, error) {
	h.t.Helper()
	return h.RunInDirWithEnv(dir, nil, args...)
}

// RunInDirWithEnv invokes the drill binary in dir with env overrides.
...
// PushToGate pushes the current branch through the drill remote,
...
// buildBinaries compiles the drill binary and the fakeagent binary
// once per `go test` invocation. Both are placed in a per-process build
// dir; subsequent harnesses reuse them.
func buildBinaries(t *testing.T) (drillBin, fakeBin string) {
	t.Helper()
	buildOnce.Do(func() {
		...
		nm := filepath.Join(dir, executableName("drill"))
...
// the go.mod that declares the drill module. Robust to test
