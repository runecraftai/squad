...
func FindDaemonsForRoot(drillHome string) ([]int, error) {
	...
	for {
		...
		if strings.EqualFold(exe, "drill.exe") || strings.EqualFold(exe, "drill") {
			...
			} else if commandMatchesDaemonRoot(command, drillHome) {
