...
- **agent 原生** —— `/drill` 既能让编码 agent 完成一个任务再过网关，也能直接为已提交的工作过网关：它跑完流水线、让流水线应用安全的修复，剩下的升级给你。
...
如果是 GitHub fork 贡献，让 `origin` 指向父仓库，并用 `drill init --fork-url <your-fork-url>` 初始化。
...
- **`git push drill`** —— 显式的 Git 路径。把已提交的分支推给网关 remote，而不是 `origin`。
- **`drill`** —— TUI。改完之后运行它（无需先提交），向导会带你建分支、提交、推过网关，然后挂到这次运行上。`drill -y` 会把这一切自动做完。
- **`/drill`** —— agent skill。用 `/drill <task>` 让编码 agent 完成一个任务再过网关，或用裸 `/drill` 为已提交的工作过网关。它跑完流水线、让流水线应用安全的修复，并在任何需要人来拍板的地方停下来问你。
...
make skill   # 重新生成已提交的 drill skill 文件
make fmt     # 运行 gofmt -w .
