...
// EnsureGitignore adds fobDir to the .gitignore of the enclosing git
// repo, if fobDir is inside a git repo. It is a no-op if the directory
// is not inside a repo or if the entry already exists.
func EnsureGitignore(fobDir string) error {
	// Walk up from fobDir to find an existing ancestor for the git check,
	...
	rel, err := filepath.Rel(repoRoot, fobDir)
	if err != nil {
		return nil
	}
