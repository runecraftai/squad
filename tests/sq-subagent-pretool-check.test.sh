...
CHECK="$ROOT/bin/sq-subagent-pretool-check.sh"
TMP_ROOT=$(fm_test_tmproot sq-subagent-pretool-tests)
...
BRIEF_ONLY_ROUTE='first classify the work under the AGENTS.md intake contract, then use bin/sq-brief.sh followed by bin/sq-spawn.sh for dispatched work'
SCOUT_ROUTE='first classify the work under the AGENTS.md intake contract: work already classified as a recon goes to bin/sq-recon.sh "<question>" [project], while authorized ship work and its bounded research go to bin/sq-brief.sh then bin/sq-spawn.sh'
...
test_guard_never_classifies_mcp_tools() {
  ...
  # has nothing to do with unit dispatch.
  local tool
...
test_deny_message_defers_to_intake_classification() {
  local actual
  printf '#!/usr/bin/env bash\n' > "$PRIMARY/bin/sq-recon.sh"
  run_tool Agent && fail "recon-present case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$SCOUT_ROUTE"*) ;;
    *) fail "deny must reserve bin/sq-recon.sh for classified recon work: $actual" ;;
  esac
  case "$actual" in
    *'investigation or diagnosis goes to bin/sq-recon.sh'*) fail "deny must not classify all investigation or diagnosis as recon work: $actual" ;;
  esac
  rm -f "$PRIMARY/bin/sq-recon.sh"
  run_tool Agent && fail "recon-absent case must still deny"
  actual=$(jq -r '.systemMessage' "$ERR")
  case "$actual" in
    *"$BRIEF_ONLY_ROUTE"*) ;;
    *) fail "deny must degrade to brief-then-spawn when sq-recon.sh is absent: $actual" ;;
  esac
  pass "deny defers to intake classification and degrades gracefully without sq-recon.sh"
}

test_escape_hatch_allows_deliberate_use() {
  local rc value
  expect_allow "escape hatch set" Agent SQUAD_ALLOW_SUBAGENT=1
  expect_deny "escape hatch unset" Agent
  for value in '' 0 yes true 11; do
    rc=0
    run_tool Agent "SQUAD_ALLOW_SUBAGENT=$value" || rc=$?
    [ "$rc" -eq 2 ] || fail "SQUAD_ALLOW_SUBAGENT='$value' must not release the guard, got exit $rc"
  done
  pass "the single documented escape hatch releases the guard only on the exact opt-in value"
}

test_task_worktree_and_non_Squad_repo_are_inert() {
