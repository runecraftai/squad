#!/usr/bin/env bash
# tests/sq-remote-XO-trace-context.test.sh - trace-context regressions for
# the REMOTE second mate route, over the deterministic generic SSH boundary.
#
# The local spawn path's coverage lives in tests/sq-trace-context-spawn.test.sh.
...
TMP_ROOT=$(fm_test_tmproot sq-remote-trace-context)
mkdir -p "$TMP_ROOT"
...
# literal's SQUAD_TRACE_CONTEXT prefix are both observable exactly as the pane
...
# would inspect the RUNNER's own account; tests/sq-remote-doctor.test.sh owns
# the doctor's behavior against controlled account fixtures.
if printf '%s' "$4" | base64 --decode 2>/dev/null | tr '\0' '\n' | head -1 | grep -q '^sq-remote-doctor.sh$'; then
...
# does, hermetically against an ambient SQUAD_TRACE_CONTEXT.
...
remote_launch_snapshot() {
  grep -o 'SQUAD_TRACE_CONTEXT=[a-z]*' "$HERDR_LOG" | tail -1 | cut -d= -f2
}
...
SQUAD_XO_CHARTER='Own iOS delivery on the build Mac.' \
  ...
  remote_env "$ROOT/bin/sq-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$REMOTE_HOME" --no-projects >/dev/null \
  || fail "remote seed did not provision the traced route"

# --- disabled: the remote route must stay byte-identically untraced ----------
freeze_parent_session
: > "$HERDR_LOG"
remote_env "$ROOT/bin/sq-spawn.sh" ios --xo >/dev/null 2>&1 \
  || fail "default-off remote XO spawn failed"
...
[ "$(remote_launch_snapshot)" = off ] \
  || fail "default-off remote spawn must deliver SQUAD_TRACE_CONTEXT=off (got '$(remote_launch_snapshot)')"
...
: > "$HERDR_LOG"
remote_env "$ROOT/bin/sq-spawn.sh" ios --xo >/dev/null 2>&1 \
  || fail "enabled remote XO spawn failed"
...
[ "$(remote_launch_snapshot)" = on ] \
  || fail "an enabled remote spawn must deliver SQUAD_TRACE_CONTEXT=on (got '$(remote_launch_snapshot)')"
...
LAUNCH_LINE=$(grep -n 'SQUAD_TRACE_CONTEXT=' "$HERDR_LOG" | tail -1 | cut -d: -f1)
...
: > "$HERDR_LOG"
remote_env "$ROOT/bin/sq-spawn.sh" ios --xo >/dev/null 2>&1 \
  || fail "enabled remote XO relaunch failed"
...
SQUAD_XO_CHARTER='Own the second build Mac.' \
  SQUAD_XO_SCOPE='second remote domain' \
  TRACEPARENT="$AMBIENT" \
  remote_env "$ROOT/bin/sq-remote-home-seed.sh" ios2 remote-mac "$REMOTE_ROOT" "$SECOND_HOME" --no-projects >/dev/null \
  || fail "second remote seed failed"
reset_remote_herdr_fixture "$HERDR_STATE"
: > "$HERDR_LOG"
TRACEPARENT="$AMBIENT" remote_env "$ROOT/bin/sq-spawn.sh" ios2 --xo >/dev/null 2>&1 \
  || fail "second remote XO spawn failed"
...
assert_absent "$PROTOCOL_HOME/config/xo-harness" "a non-inheritable item was published remotely"
...
# that argument is refused unless it is an XO launch carrying a strictly
...
try_flag 'not a valid W3C traceparent' \
  ...
  --xo --traceparent '00-00000000000000000000000000000000-bbbbbbbbbbbbbbbb-01'
try_flag 'requires a non-empty value' \
  "an empty carrier must be refused rather than silently ignored" \
  --xo --traceparent=
pass "delivery: a parent-supplied carrier is accepted only for an XO launch and only as a strict W3C value"
