...
| `sq-brief.sh`            | Estruturar briefs ship (`--mode` explícito), recon, charter de XO e Herdr-lab; `--playbook` seleciona um playbook de execução   |
...
| `sq-backend-pretool-check.sh` | Guard PreToolUse de CLI bruto do provedor de sessão na base primária |
| `sq-backend-command-policy.mjs` | Dono semântico da política de CLI bruto do provedor de sessão |
| `sq-poll-pretool-check.sh` | Guard PreToolUse de loop de polling de `state/` na base primária |
| `sq-poll-command-policy.mjs` | Dono semântico da política de loop de polling estreito de `state/` |
...
| `sq-playbook-validate.sh` | Validar evidência estrutural de um playbook de execução materializado (bug-fix@1) |
