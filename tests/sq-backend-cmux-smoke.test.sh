#!/usr/bin/env bash
# tests/sq-backend-cmux-smoke.test.sh - real cmux smoke test for the cmux
# session-provider adapter (bin/backends/cmux.sh), verified against the real
# cmux 0.64.17 binary (docs/cmux-backend.md). Mirrors
# tests/sq-backend-zellij-smoke.test.sh's/sq-backend-herdr-smoke.test.sh's
...
# Orca). So this test creates ONLY `sq-test-`-prefixed task labels, touches and
...
WS2=""
cleanup_all() {
  [ -z "$WS1" ] || cmux_safe_close_workspace "$WS1" "sq-test-smoke1"
  [ -z "$WS2" ] || cmux_safe_close_workspace "$WS2" "sq-test-smoke2"
}
trap cleanup_all EXIT

# --- create_task + duplicate refusal -----------------------------------------

LABEL="sq-test-smoke1"
...
if fm_backend_cmux_send_key "$TARGET" Escape "sq-test-not-$LABEL" >/dev/null 2>&1; then
...
fm_backend_cmux_send_literal "$TARGET" 'echo literal-then-key-commander' \
...
fm_backend_cmux_send_text_line "$TARGET" "echo commander-on-deck-line" \
...
# sq-spawn.sh's worktree-discovery poll actually depends on, so this must be
...
case "$p2" in
  */private/tmp|*/tmp) : ;;
  *) fail "real cmux: current_path did not track a nested subshell's own cd (the fob-get-shaped case), got '$p2'" ;;
esac
pass "real cmux: current_path tracks a NESTED SUBSHELL's own cd (the fob-get-shaped case a bare cwd read cannot see)"
...
pass "real cmux: fm_backend_busy_state reports unknown (sentry falls back to pane-regex, same as tmux/zellij/orca)"
...
pass "real cmux: list_live discovers a live task workspace by sq-<id> title"
