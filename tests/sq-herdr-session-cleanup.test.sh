#!/usr/bin/env bash
# Focused safety tests for bin/sq-herdr-session-cleanup.sh.
...
TMP_ROOT=$(fm_test_tmproot sq-herdr-session-cleanup)
...
export SQUAD_HERDR_SESSION_CLEANUP_SOURCE_ONLY=1
# shellcheck source=/dev/null
. "$ROOT/bin/sq-herdr-session-cleanup.sh"
unset SQUAD_HERDR_SESSION_CLEANUP_SOURCE_ONLY
...
argv_pid=$(
  ...
  SQUAD_HERDR_PS_BIN="$FAKE_PS" fm_backend_herdr_pane_idle_shell_pid test w2:p1
...
if (
  ...
  SQUAD_HERDR_PS_BIN="$FAKE_PS" SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
...
fixture_workspaces() {
  ...
  printf '[{"workspace_id":"w1","label":"Squad","focused":%s,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},' \
...
fixture_tabs() {
  ...
  while [ "$i" -le "$count" ]; do
    [ "$i" -eq 1 ] || printf ','
    printf '{"tab_id":"%s:t%s","workspace_id":"%s","focused":false,"label":"sq-task"}' "$WS" "$i" "$WS"
    i=$((i + 1))
  done
  printf ']'
}
...
fm_backend_herdr_cli() {
  ...
  if [ -e "$FIXTURE_DIR/closed" ]; then
    case "$first $second" in
      ...
      "workspace list") printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1}]}}'; return 0 ;;
    esac
  fi
  if [ -e "$FIXTURE_DIR/race" ] && [ -e "$FIXTURE_DIR/snapshotted" ] && [ "$first $second" = "workspace list" ]; then
    printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad","focused":true,"active_tab_id":"w1:t1","tab_count":1,"pane_count":1},{"workspace_id":"w2","label":"renamed","focused":false,"active_tab_id":"w2:t1","tab_count":1,"pane_count":1}]}}'
    return 0
  fi
  title=$(cat "$FIXTURE_DIR/title")
  tabs=$(cat "$FIXTURE_DIR/tabs")
...
write_v1() { # <id> [token]
  ...
  } > "$SQUAD_STATE_OVERRIDE/$id.herdr-presentation"
}
...
reset_fixture() {
  rm -rf "$FIXTURE_DIR" "$TMP_ROOT"/*.lock "${SQUAD_STATE_OVERRIDE:?}/"*
  mkdir -p "$FIXTURE_DIR"
  : > "$LOCK_LOG"; : > "$CLOSE_LOG"
...
assert_preserved() { # <case>
  ...
  [ -f "$SQUAD_STATE_OVERRIDE/$ID.herdr-presentation" ] && had_journal=1
  fm_herdr_session_cleanup >/dev/null 2>&1
  if [ "$had_journal" -eq 1 ]; then
    [ -f "$SQUAD_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "$name retired the journal"
  fi
...
reset_fixture
fm_herdr_session_cleanup >/dev/null 2>&1
[ ! -e "$SQUAD_STATE_OVERRIDE/$ID.herdr-presentation" ] || fail "positive cleanup kept the journal"
[ "$(wc -l < "$CLOSE_LOG" | tr -d ' ')" = 1 ] || fail "positive cleanup did not close exactly once"
[ "$(sed -n '1p' "$LOCK_LOG")" = "$SQUAD_STATE_OVERRIDE/.spawn-$ID.lock" ] || fail "task lock was not acquired first"
...
reset_fixture; printf 'version=1\ntask_id=%s\nprojection_id=short\n' "$ID" > "$SQUAD_STATE_OVERRIDE/$ID.herdr-presentation"; assert_preserved "malformed journal"
...
reset_fixture; : > "$SQUAD_STATE_OVERRIDE/$ID.meta"; assert_preserved "current task metadata"
...
cat > "$INTEGRATION_ROOT/bin/sq-lock.sh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' 'lock acquired'
SH
...
printf 'all sq-herdr-session-cleanup tests passed\n'
