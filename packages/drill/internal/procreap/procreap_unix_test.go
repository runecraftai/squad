...
// tests above drive through DRILL_PROCREAP_HELPER. It exits before the testing
// framework can report anything.
func TestProcReapHelper(t *testing.T) {
	switch os.Getenv("DRILL_PROCREAP_HELPER") {
	case "leader":
		child := exec.Command(os.Args[0], "-test.run=^TestProcReapHelper$")
		child.Env = append(os.Environ(),
			"DRILL_PROCREAP_HELPER=escaped",
			"DRILL_PROCREAP_READY="+os.Getenv("DRILL_PROCREAP_READY"),
		)
		child.Dir = os.Getenv("DRILL_PROCREAP_WORKDIR")
		if err := child.Start(); err != nil {
			os.Exit(2)
		}
		if !waitForFile(os.Getenv("DRILL_PROCREAP_READY"), 10*time.Second) {
			os.Exit(3)
		}
	...
	case "escaped":
		...
		_ = os.WriteFile(os.Getenv("DRILL_PROCREAP_READY"), []byte("ready"), 0o644)
...
func TestParseLsofCWDKeepsDeletedDirectories(t *testing.T) {
	out := "p123\nn/Users/x/.drill/worktrees/repo/run (deleted)\np124\nn/tmp\np125\n"
	cwds := parseLsofCWD(out)
	if got, want := cwds[123], "/Users/x/.drill/worktrees/repo/run"; got != want {
