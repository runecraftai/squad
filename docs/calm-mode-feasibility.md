...
This document owns the version-scoped feasibility evidence, Pi transcript taxonomy, and supported-API boundaries for Squad calm mode.
...
`tests/sq-calm-pi-extension.test.sh` records the installed Pi version as evidence without gating on it and covers both newer synthetic versions and an unavailable adapter seam.
...
Pi loads project-local extensions before global or CLI-configured extensions, so Squad's tracked Calm extension previously won those collisions even when its persisted preference was off.
...
`tests/sq-calm-pi-extension.test.sh` covers the resulting split contract: no load-time claims while Calm is off, synchronous claims while it is already on, collision-checked first activation with a warning, preservation of a contested tool's execution, and the non-retroactive bound for rows rendered before first activation.
...
The commander-visible regression reproduced three consecutive times in a persisted Pi session under `~/.pi/agent/sessions/`.
...
The initiating trigger was `pi.sendUserMessage(..., { deliverAs: "followUp" })` from the sentry or turn-end adapter after a commander-facing response.
...
The visible symptom was a second assistant row repeating the prior commander answer.
...
A native deterministic Pi TUI reproduction on landed PR 927 produced `COMMANDER_VISIBLE_ANSWER` twice with Calm loaded and explicitly on, and produced the same duplicate with Calm loaded and explicitly off.
The same exact typed notification with Calm absent produced one commander answer followed by `MONITOR_NOTIFICATION_HANDLED`.
...
Legacy `Squad-synthetic-input-presentation` entries remained renderable so existing sessions preserved their stored presentation and zero-height hidden-row behavior.
...
The injected user message began with exact U+2063 plus `SQUAD_OP:` and carried the sentry status path from the durable commander screenshot followed by the blank line and stable drain instruction.
...
The initiating trigger was the exact sentry-generated user message.
...
The smallest counterfactual returned only from the transcript owner's ordinary-user branch for that exact sentry input.
...
The adapter probes for that exact method and, per the [compatibility contract](calm.md#pi-compatibility), degrades independently with a diagnostic rather than gating on a version number.
It delegates current recognition to `bin/sq-operational-input.sh`, adds only the evidence-backed bare-U+2063 `Supervisor escalate (` presentation compatibility shape, mounts a `UserMessageComponent` subclass that preserves Pi's stock row plus leading spacer while Calm is off, and returns zero rendered lines while Calm is on.
It never intercepts the input event, rewrites the message, changes its role, filters model context, or changes session data.
Messages containing an image are left on Pi's ordinary path even when their text equals an operational envelope because Squad's authoritative producers are text-only.

A native exact-sentry run and its process-restart replay kept the neighboring assistant text at the two-row visible-only spacing while retaining one exact user entry and one processing response.
...
The current exact marker and the narrow bare-U+2063 `Supervisor escalate (` compatibility shape hid under Calm, while quoted markers, ASCII `SQUAD_OP:` without U+2063, ordinary text before the current marker, unrelated text after U+2063, and image-bearing input remained visible.
...
`.pi/extensions/lib/sq-calm-visibility.ts` owns only the allowlist-style transcript presentation policy.
...
It asserts one persisted and rendered commander answer, exact user-role operational envelopes in order, no replacement custom messages, one processing result, zero operational transcript rows, and the two-row neighboring-assistant geometry for live, adjacent, and restart paths.
...
`tests/sq-pi-primary-live-e2e.test.sh` also proves the working ship replaces the built-in `Working...` row while Calm is active on the credentialed provider path, and that it clears when the run settles, before continuing its ordinary sentry lifecycle.
`tests/sq-pi-primary-types.test.sh` performs strict no-emit TypeScript checking against the installed Pi declarations, currently package version 0.81.1.
...
The credentialed live regression remains opt-in and was not required because this change does not alter sentry delivery or provider integration.
