...
TMP_ROOT=$(fm_test_tmproot sq-calm-pi-extension)
EXT="$ROOT/.pi/extensions/sq-calm.ts"
ASSISTANT_LAYOUT="$ROOT/.pi/extensions/lib/sq-calm-assistant-layout.ts"
OPERATIONAL_USER_LAYOUT="$ROOT/.pi/extensions/lib/sq-calm-operational-user-layout.ts"
VISIBILITY="$ROOT/.pi/extensions/lib/sq-calm-visibility.ts"
WORKING_SHIP="$ROOT/.pi/extensions/lib/sq-calm-working-ship.ts"
WATCH_EXT="$ROOT/.pi/extensions/sq-primary-pi-watch.ts"
OPERATIONAL_INPUT="$ROOT/bin/sq-operational-input.sh"
PI_OPERATIONAL_INPUT="$ROOT/.pi/extensions/lib/sq-operational-input.ts"
PI_PACKAGE_DIR=${SQUAD_PI_PACKAGE_DIR:-"$(npm root -g 2>/dev/null)/@earendil-works/pi-coding-agent"}
TMUX_SOCKET="sq-calm-$$"
TMUX_SESSION="sq-calm-e2e"
...
# .pi/extensions/sq-calm.ts) instead of relying on version inference, so a version
...
test_home_resolution() {
  ...
  mkdir -p \
    ...
    "$fixture/override" \
    "$fixture/launch-cwd"
  cp "$EXT" "$fixture/project/.pi/extensions/sq-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/project/.pi/extensions/lib/sq-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/project/.pi/extensions/lib/sq-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/project/.pi/extensions/lib/sq-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/project/.pi/extensions/lib/sq-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/project/.pi/extensions/lib/sq-operational-input.ts"
...
if (readFileSync(`${process.env.EXTENSION_HOME}/config/calm`, "utf8") !== "on\n") {
  throw new Error("Calm did not derive the Squad home from its extension path");
}
...
const assistant = await import("./.pi/extensions/lib/sq-calm-assistant-layout.ts");
const operational = await import("./.pi/extensions/lib/sq-calm-operational-user-layout.ts");
...
// commander-accepted, documented bound on the gate-at-load fix (see sq-calm.ts's file
...
test_rendering_and_session_lifecycle() {
  ...
  cp "$EXT" "$fixture/sq-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$fixture/lib/sq-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$fixture/lib/sq-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$fixture/lib/sq-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/sq-calm-working-ship.ts"
  cp "$ROOT/.pi/extensions/lib/sq-operational-input.ts" "$fixture/lib/sq-operational-input.ts"
  cp "$WATCH_EXT" "$fixture/sq-primary-pi-watch.ts"
...
#!/usr/bin/env bash
printf '%s\n' "${1-}" >>"$SQUAD_OPERATIONAL_INPUT_CALLS"
exec "$SQUAD_OPERATIONAL_INPUT_OWNER" "$@"
SH
...
// sq-calm.ts derives its own identity the same way (fileURLToPath(import.meta.url)),
...
extension.default(pi);
const visibility = await import(`${pathToFileURL(`${process.cwd()}/lib/sq-calm-visibility.ts`).href}?policy=${Date.now()}`);
const operationalInput = await import(`${pathToFileURL(`${process.cwd()}/lib/sq-operational-input.ts`).href}?input=${Date.now()}`);

// Registration is gated on config/calm at load (see sq-calm.ts's file header); this
...
if (
  ...
  "Toggle Squad's supported conversation-only transcript presentation."
) {
...
const sentryBody =
  "SQUAD WATCHER WAKE: signal: /tmp/probe.status\n\n" +
  "Run bin/sq-stand-to-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const sentryMessage = operationalInput.encodeSquadOperationalInput("sentry", sentryBody);
...
const callsBeforePlainReplay = readFileSync(process.env.SQUAD_OPERATIONAL_INPUT_CALLS, "utf8");
...
if (readFileSync(process.env.SQUAD_OPERATIONAL_INPUT_CALLS, "utf8") !== callsBeforePlainReplay) {
  throw new Error("ordinary replay rows invoked operational subprocess classification");
}
InteractiveMode.prototype.addMessageToChat.call(
  operationalMode,
  { role: "user", content: [{ type: "text", text: sentryMessage }] },
...
const stockOperationalComponent = new UserMessageComponent(sentryMessage, undefined, 1);
...
if (operationalHistory.length !== 1 || operationalHistory[0] !== sentryMessage) {
...
const watchArgs = {};
const watchResult = {
  content: [{ type: "text", text: "sentry: started Pi extension arm child 1" }],
  details: { ok: true, message: "sentry: started Pi extension arm child 1" },
  isError: false,
};
...
if (JSON.stringify(watchActual.render(100)) !== JSON.stringify(watchBaseline.render(100))) {
  throw new Error("Squad sentry tool changed stock rendering while Calm was off");
}
...
const presentationRenderer = entryRenderers.get("Squad-synthetic-input-presentation");
if (!presentationRenderer) throw new Error("legacy synthetic presentation renderer was not registered");
const presentationEntry = {
  customType: "Squad-synthetic-input-presentation",
  data: { content: sentryMessage, kind: "sentry" },
};
...
if (presentationComponent.hasContent() || presentationComponent.render(100).length !== 0) {
  throw new Error("Calm left a synthetic Squad presentation row or spacer visible");
}
...
const operationalNearMisses = [
  {
    content: `Commander quote: ${sentryMessage}`,
    visible: "Commander quote:",
  },
  {
    content: "SQUAD_OP: v1 sentry: ASCII_ONLY_COMMANDER_MESSAGE",
    visible: "ASCII_ONLY_COMMANDER_MESSAGE",
  },
  {
    content: `Ordinary commander text before ${sentryMessage}`,
    visible: "Ordinary commander text before",
  },
  {
    content: "\u2063ordinary commander text after an unrelated separator",
    visible: "ordinary commander text after an unrelated separator",
  },
  {
    content: "\u2063SQUAD_OP: legacy untyped commander message",
    visible: "legacy untyped commander message",
  },
  {
    content: "Run `bin/sq-session-start.sh` now, exactly once, before executing any other instructions.",
    visible: "before executing any other instructions",
  },
  {
    content:
      "SQUAD WATCHER WAKE: commander-authored legacy-shaped message\n\n" +
      "Run bin/sq-stand-to-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.",
    visible: "commander-authored legacy-shaped message",
  },
  {
    content:
      "TURN WOULD END BLIND - supervision is off. The sentry cycle is missing, failed, or unhealthy. " +
      "Follow the harness recovery instruction below before ending the turn.\n\n" +
      "commander-authored legacy-shaped message",
    visible: "commander-authored legacy-shaped message",
  },
  {
    content: [
      { type: "text", text: sentryMessage },
      { type: "image", data: "ignored-by-text-renderer", mimeType: "image/png" },
    ],
    visible: "SQUAD WATCHER WAKE",
  },
];
...
for (const reason of ["startup", "new", "resume", "fork", "reload"]) {
  ...
  if (workingVisible !== true || hiddenThinkingLabel !== "" || statuses.get("Squad-calm") !== undefined) {
...
test_operational_followup_turn_e2e() {
  local project home config sessions version label case_name calm_state expected_notifications session_file pane i commander_line handled_line geometry_gap exact_session
  ...
  cp "$EXT" "$project/.pi/extensions/sq-calm.ts"
  cp "$ASSISTANT_LAYOUT" "$project/.pi/extensions/lib/sq-calm-assistant-layout.ts"
  cp "$OPERATIONAL_USER_LAYOUT" "$project/.pi/extensions/lib/sq-calm-operational-user-layout.ts"
  cp "$VISIBILITY" "$project/.pi/extensions/lib/sq-calm-visibility.ts"
  cp "$WORKING_SHIP" "$project/.pi/extensions/lib/sq-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$project/.pi/extensions/lib/sq-operational-input.ts"
...
import { encodeSquadOperationalInput } from "./.pi/extensions/lib/sq-operational-input.ts";

let phase: "idle" | "commander" | "monitor" = "idle";
let label = "";
let adjacent = false;
let latestInputRole: "user" | "custom" | undefined;

const EXACT_WATCHER_INPUT =
  "\u2063SQUAD_OP: v1 sentry: SQUAD WATCHER WAKE: signal: /home/fixture/github/runecraftai/squad/state/oss-triage-t4.status\n\n" +
  "Run bin/sq-stand-to-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";

function monitorInput(suffix: "ONE" | "TWO"): string {
  if (label === "exact_sentry" && suffix === "ONE") return EXACT_WATCHER_INPUT;
  if (label === "legacy_away" && suffix === "ONE") {
    return "\u2063Supervisor escalate (LEGACY_AWAY_E2E)";
  }
  return encodeSquadOperationalInput("sentry", `MONITOR_${label}_${suffix}`);
}
...
export default function (pi: ExtensionAPI): void {
  pi.on("message_start", (event) => {
    ...
    if (event.message.role !== "assistant" || phase !== "commander") return;
    phase = "monitor";
  ...
  pi.registerCommand("followup-e2e", {
    description: "Run one commander prompt followed by typed monitoring input.",
    handler: async (args, ctx) => {
      ...
      phase = "commander";
      pi.sendUserMessage(`COMMANDER_PROMPT_${label}`);
    },
  });
}
TS
  ...
  run_followup_case() {
    ...
    if [ "$calm_state" = absent ]; then
      ...
      rm -f "$home/config/calm"
      extensions='-e ./.pi/extensions/sq-calm.ts -e ./followup-e2e.ts'
    else
    ...
    i=0
    while [ "$i" -lt 240 ]; do
      session_file=$(find "$sessions" -type f -name '*.jsonl' -exec grep -l "COMMANDER_PROMPT_$label" {} + 2>/dev/null | head -1 || true)
    ...
    pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
    [ "$(printf '%s\n' "$pane" | grep -Fc "COMMANDER_ANSWER_$label" || true)" -eq 1 ] \
      || fail "Pi follow-up $label case rendered a duplicate commander answer"
    assert_contains "$pane" "COMMANDER_PROMPT_$label" "Pi follow-up $label case hid the genuine commander prompt"
    ...
    if [ "$calm_state" = on ]; then
      ...
      if [ "$label" = exact_sentry ]; then
        assert_not_contains "$pane" "SQUAD WATCHER WAKE: signal: /home/fixture/github/runecraftai/squad/state/oss-triage-t4.status" \
          "Pi exact sentry case rendered the Calm-hidden authoritative payload"
        assert_not_contains "$pane" "Run bin/sq-stand-to-drain.sh first and handle the queued wake." \
          "Pi exact sentry case rendered the Calm-hidden drain instruction"
      ...
      commander_line=$(printf '%s\n' "$pane" | grep -Fn "COMMANDER_ANSWER_$label" | tail -1 | cut -d: -f1)
      handled_line=$(printf '%s\n' "$pane" | grep -Fn "MONITOR_HANDLED_${label}_ONE" | tail -1 | cut -d: -f1)
      geometry_gap=$((handled_line - commander_line))
      [ "$geometry_gap" -eq 2 ] \
...
if (text(userEntries[0].message.content) !== commanderPrompt) throw new Error("genuine commander prompt changed");
...
if (assistantText.filter((value) => value === commanderAnswer).length !== 1) {
  throw new Error(`commander answer count changed: ${JSON.stringify(assistantText)}`);
}
...
if (assistantEntries.length !== 2) throw new Error(`expected one commander and one processing turn, found ${assistantEntries.length}`);
...
JS
  ...
  replay_exact_case() {
    ...
    while [ "$i" -lt 120 ]; do
      pane=$(tmux -L "$TMUX_SOCKET" capture-pane -p -t "$TMUX_SESSION" -S - 2>/dev/null || true)
      printf '%s\n' "$pane" | grep -Fq 'MONITOR_HANDLED_exact_sentry_ONE' && break
      sleep 0.05
      i=$((i + 1))
    done
    assert_contains "$pane" "COMMANDER_PROMPT_exact_sentry" "Pi restart lost the genuine commander prompt"
    assert_contains "$pane" "MONITOR_HANDLED_exact_sentry_ONE" "Pi restart lost the operational processing response"
    assert_not_contains "$pane" "SQUAD WATCHER WAKE: signal: /home/fixture/github/runecraftai/squad/state/oss-triage-t4.status" \
      "Pi restart replayed the Calm-hidden exact sentry row"
    commander_line=$(printf '%s\n' "$pane" | grep -Fn 'COMMANDER_ANSWER_exact_sentry' | tail -1 | cut -d: -f1)
    handled_line=$(printf '%s\n' "$pane" | grep -Fn 'MONITOR_HANDLED_exact_sentry_ONE' | tail -1 | cut -d: -f1)
    geometry_gap=$((handled_line - commander_line))
    [ "$geometry_gap" -eq 2 ] \
      || fail "Pi restart replay consumed $geometry_gap rows between neighboring assistant text"
    node - "$exact_session" <<'JS' || fail "Pi restart replay changed exact sentry persistence"
const fs = require("node:fs");
const entries = fs.readFileSync(process.argv[2], "utf8").trim().split("\n").map(JSON.parse);
const text = (content) => typeof content === "string"
  ? content
  : (content ?? []).filter((item) => item.type === "text").map((item) => item.text).join("");
const exact = "\u2063SQUAD_OP: v1 sentry: SQUAD WATCHER WAKE: signal: /home/fixture/github/runecraftai/squad/state/oss-triage-t4.status\n\nRun bin/sq-stand-to-drain.sh first and handle the queued wake. Watcher continuity is extension-owned.";
const users = entries.filter((entry) => entry.type === "message" && entry.message.role === "user" && text(entry.message.content) === exact);
const responses = entries.filter((entry) => entry.type === "message" && entry.message.role === "assistant" && text(entry.message.content) === "MONITOR_HANDLED_exact_sentry_ONE");
...
JS
  ...
  run_followup_case exact-sentry on exact_sentry 1
...
test_hidden_block_geometry_e2e() {
  ...
  mkdir -p \
    "$project/.agents/skills/reporting" \
  ...
  cat >"$project/.agents/skills/reporting/SKILL.md" <<'MD'
---
name: reporting
...
TS
  ...
  assert_geometry_gap() {
    ...
    skill_line=$(grep -n -m1 '\[skill\] reporting' "$file" | cut -d: -f1)
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  sleep 0.1
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l '/skill:reporting'
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" Enter
  wait_for_geometry_text "$snapshot" "visible row two" \
    || fail "Pi Calm hidden-block geometry E2E did not complete the /skill:reporting turn"
  i=0
  while [ "$i" -lt 120 ]; do
  ...
  assert_contains "$(cat "$snapshot")" "[skill] reporting" "Calm hid the collapsed skill header"
  ...
  assert_geometry_gap "$snapshot" "completed native Calm /skill:reporting turn"
  ...
  sleep 0.2
  tmux -L "$TMUX_SOCKET" kill-session -t "$TMUX_SESSION" 2>/dev/null || true
  pass "Pi Calm native /skill:reporting geometry keeps every collapsed thinking and tool block at zero height while preserving expansion, history, restart, and Calm-off rendering"
}
...
test_working_ship_geometry_and_lifecycle() {
  ...
  cp "$VISIBILITY" "$fixture/lib/sq-calm-visibility.ts"
  cp "$WORKING_SHIP" "$fixture/lib/sq-calm-working-ship.ts"
  cp "$PI_OPERATIONAL_INPUT" "$fixture/lib/sq-operational-input.ts"
  ln -s "$PI_PACKAGE_DIR" "$fixture/node_modules/@earendil-works/pi-coding-agent"
  ln -s "$PI_PACKAGE_DIR/node_modules/@earendil-works/pi-tui" "$fixture/node_modules/@earendil-works/pi-tui"
  ln -s "$PI_PACKAGE_DIR/node_modules/typebox" "$fixture/node_modules/typebox"
  printf '%s\n' '{"type":"module"}' >"$fixture/package.json"
...
const ship = await import(
  `${pathToFileURL(`${process.cwd()}/lib/sq-calm-working-ship.ts`).href}?ship=${Date.now()}`
);
const {
...
test_interactive_terminal_e2e() {
  ...
  cp "$VISIBILITY" "$project/.pi/extensions/lib/sq-calm-visibility.ts"
  cp "$WORKING_SHIP" "$project/.pi/extensions/lib/sq-calm-working-ship.ts"
  cp "$ROOT/.pi/extensions/lib/sq-operational-input.ts" "$project/.pi/extensions/lib/sq-operational-input.ts"
  cp "$WATCH_EXT" "$project/.pi/extensions/sq-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/sq-primary-turnend-guard.ts" "$project/.pi/extensions/sq-primary-turnend-guard.ts"
  cp \
    "$ROOT/bin/sq-sessionstart-run.sh" \
    "$ROOT/bin/sq-sessionstart-nudge.sh" \
    "$ROOT/bin/sq-primary-scope-lib.sh" \
    "$ROOT/bin/sq-gate-refuse-lib.sh" \
  ...
  # unit home into a rendering test.
  cat >"$project/bin/sq-session-start.sh" <<'SH'
#!/usr/bin/env bash
printf 'CALM_E2E_SESSION_START_DIGEST\n'
exit 0
SH
  chmod +x "$project/bin/"*.sh
  cat >"$project/.pi/extensions/sq-calm-e2e-inject.ts" <<'TS'
import {
  type AssistantMessage,
  createAssistantMessageEventStream,
} from "@earendil-works/pi-ai";
  ...
  pi.registerCommand("calm-inject-e2e", {
    ...
    handler: async (args, ctx) => {
      const fixtures = new Map([
        ["sentry", "CURRENT_WATCHER_E2E /tmp/active-probe.status"],
        ["turn-end-guard", "CURRENT_TURN_END_E2E"],
        ["away-supervisor", "CURRENT_AWAY_E2E"],
        ["from-squad", "corr=0123456789abcdef CURRENT_FROM_SQUAD_E2E"],
        ["launch-brief", "CURRENT_LAUNCH_BRIEF_E2E"],
      ] as const);
      const kind = args.trim() as Parameters<typeof encodeSquadOperationalInput>[0];
      ...
      await pi.sendUserMessage(encodeSquadOperationalInput(kind, body), {
        deliverAs: "followUp",
      });
    },
  });
...
{"type":"message","id":"a0000012","parentId":"a0000011","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Commander quote: \u2063SQUAD_OP: v1 sentry: QUOTED_CURRENT_NEAR_MISS"}],"timestamp":12}}
{"type":"message","id":"a0000013","parentId":"a0000012","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"SQUAD_OP: v1 sentry: ASCII_ONLY_NEAR_MISS"}],"timestamp":13}}
{"type":"message","id":"a0000014","parentId":"a0000013","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"Ordinary commander text before \u2063SQUAD_OP: v1 sentry: EMBEDDED_CURRENT_NEAR_MISS"}],"timestamp":14}}
{"type":"message","id":"a0000015","parentId":"a0000014","timestamp":"$now","message":{"role":"user","content":[{"type":"text","text":"\u2063ordinary commander text after unrelated separator"}],"timestamp":15}}
...
JSON
  ...
  assert_not_contains "$(cat "$default_snapshot")" 'Run `bin/sq-session-start.sh` now' \
  ...
  while [ "$active_screen_wait" -lt 120 ]; do
    ...
    # first-ever activation are a separate, documented exception (see sq-calm.ts's
    ...
    if ! grep -Fq "Thinking..." "$hidden_snapshot" &&
      ...
      grep -Fq "SQUAD WATCHER WAKE: can you explain this phrase?" "$hidden_snapshot" &&
  ...
  # session. This is the commander-accepted, documented bound on the collision fix (see
  # sq-calm.ts's file header and docs/calm.md): the alternative was letting Calm
  ...
  assert_contains "$(cat "$hidden_snapshot")" "SQUAD WATCHER WAKE: can you explain this phrase?" "/calm hid a genuine near-miss user prompt"
  for near_miss in \
    QUOTED_CURRENT_NEAR_MISS \
    ASCII_ONLY_NEAR_MISS \
    EMBEDDED_CURRENT_NEAR_MISS \
    "ordinary commander text after unrelated separator"
  do
  ...
  for fixture in \
    "sentry|CURRENT_WATCHER_E2E" \
    "turn-end-guard|CURRENT_TURN_END_E2E" \
    "away-supervisor|CURRENT_AWAY_E2E" \
    "from-squad|CURRENT_FROM_SQUAD_E2E" \
    "launch-brief|CURRENT_LAUNCH_BRIEF_E2E"
  do
    kind=${fixture%%|*}
    needle=${fixture#*|}
...
for (const [needle, kind] of expected) {
  ...
  const exactEnvelope = kind === "from-squad"
    ? text?.startsWith("[sq-from-squad]\u2063corr=0123456789abcdef ")
    : text?.startsWith(`\u2063SQUAD_OP: v1 ${kind}: `);
...
JS
  ...
  # shellcheck disable=SC2016 # Backticks are literal prompt markup.
  assert_not_contains "$(cat "$active_hidden_snapshot")" 'Run `bin/sq-session-start.sh` now' \
    "Calm showed the native session-start operational input"
  for hidden in \
    CURRENT_WATCHER_E2E \
    CURRENT_TURN_END_E2E \
    CURRENT_AWAY_E2E \
    CURRENT_FROM_SQUAD_E2E \
...
if (!serialized.includes("Squad-synthetic-input") || !serialized.includes("/tmp/probe.status")) process.exit(1);
const synthetic = entries.find((entry) => entry.type === "custom_message" && entry.customType === "Squad-synthetic-input");
...
if (messages.includes("[Squad-synthetic-input]")) process.exit(1);
for (const current of ["CURRENT_WATCHER_E2E", "CURRENT_TURN_END_E2E", "CURRENT_AWAY_E2E", "CURRENT_FROM_SQUAD_E2E", "CURRENT_LAUNCH_BRIEF_E2E"]) {
  if (!messages.includes(current)) process.exit(1);
}
if (!tree.includes("Squad-synthetic-input") || !tree.includes("/tmp/probe.status")) process.exit(1);
JS
  ...
  assert_not_contains "$(cat "$boat_frame_one")" "SQUAD WATCHER WAKE: signal: /tmp/probe.status" "the real provider wait restored a hidden operational row"
  ...
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" -l "/quit"
  tmux -L "$TMUX_SOCKET" send-keys -t "$TMUX_SESSION" M-s
  pass "Pi calm native E2E replaces the stock working row with a moving, resize-clamped working ship that freezes and resumes across two working periods in one Pi session, clears on abort, keeps commander turns visible, hides exact operational user rows without changing persistence, restores stock rendering Calm-off, survives restart, and preserves export plus Ctrl+O behavior"
}
