#!/usr/bin/env bash
# tests/sq-stand-to-drain-open-decisions.test.sh - behavior tests for the OPEN
# DECISIONS section bin/sq-stand-to-drain.sh prints on every drain (including the
# empty-queue fast path). The section is pure wiring around
# sq-classify-lib.sh's status_open_decisions fold (the ONE authoritative
...
TMP_ROOT=$(fm_test_tmproot sq-stand-to-drain-open-decisions-tests)
...
test_buried_decision_still_surfaces() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a buried decision"
  ...
  grep -F "close one by answering it: bin/sq-send.sh <task> --resolve-key <key>" "$out" >/dev/null \
...
test_explicit_resolution_closes_it() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after an explicit resolution"
...
test_reserved_key_namespace_is_owned_by_its_library() {
  ...
  state="$dir/state"
  out="$dir/drain.out"
  # `pending-reply-<id>` names a decision bin/sq-pending-reply-lib.sh raises and
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on reserved-key lines"
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after the owner closed its decision"
  if grep -F 'OPEN DECISIONS' "$out" >/dev/null; then
...
test_later_unrelated_terminal_line_does_not_close_it() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed after an unrelated terminal line"
...
test_no_open_decisions_prints_nothing() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with no open decisions"
  ...
  pass "no open decisions across the unit prints nothing"
}

test_open_decision_surfaces_even_with_an_unrelated_queued_wake() {
  local dir state out
  dir=$(make_case unit-wide)
  state="$dir/state"
  out="$dir/drain.out"
  # task6 has a buried, still-open decision but generates NO new queue record
  # this turn; task7 is what actually wakes the drain. The unit-wide scan
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with a mixed unit"
  ...
  pass "the open-decision section is unit-wide, not scoped to this drain's own queued records"
}
...
test_buried_decision_surfaces_on_the_empty_queue_fast_path() {
  ...
  pass "a buried open decision surfaces even when the stand-to queue itself is empty"
}
...
test_status_symlink_is_not_followed() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed with a symlinked status file"
  ...
  if grep -F 'do not expose this' "$out" >/dev/null; then
    fail "the unit scan followed a status symlink outside the state directory"
  fi
  pass "the unit-wide decision scan does not follow status symlinks"
}

# The per-item cut now comes from bin/sq-line-cap-lib.sh, shared with the
...
test_over_long_decision_note_is_capped_with_a_marker() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on an over-long decision note"
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" || fail "drain failed on a short decision note"
