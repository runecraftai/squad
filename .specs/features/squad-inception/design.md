...
5. Mapped-sense vocabulary patterns → 0 hits (tracked, **excluding `.specs/`**). Concrete pattern list (not bare words): `\bcaptain\b`, `\bcrewmate\b`, `\bfleet\b`, `\bsecondmate\b`, `ci:(^|[^[:alpha:]])treehouse([^[:alpha:]]|$)` (case-insensitive and `_`-adjacency-aware since the Nivel-2 slice-6 closure, decisions 2026-08-11, so `TREEHOUSE_*`/`SQUAD_TREEHOUSE_*` identifiers can no longer hide behind case or word-boundary gaps; only the two documented legacy-alias env names `SQUAD_TREEHOUSE_RETURN_LOCK_RETRIES` and `SQUAD_TREEHOUSE_RETURN_LOCK_RETRY_WAIT_SECS` from the docs/configuration.md env table stay allowlisted), `\bahoy\b`, `\bbearings\b`, `\bstow\b`, `\bwake-queue\b`, `\bship task\b`, `\bscout task\b`, `\bscout worktree\b`, `\bthe watch\b`, `watch\.sh`, `watcher-continuity`. Natural-English `watch`/`ship`/`scout` outside these patterns are **allowed** (e.g., "watch for errors", "ship the change"); the allowlist is enforced by this pattern list, never by bare-word greps. The exact live list is owned by `tests/sq-rebrand-guard.test.sh` (guard 5), which reports the FULL remaining hit list in one pass.
