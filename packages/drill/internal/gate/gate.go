...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/scm/github"
)
...
// Init sets up a drill gate for the git repo at workDir.
...
// Git supports config --worktree, adds the drill remote, and records
...
// the gate identified by the leftover drill remote is reattached at the
		...
		if listErr == nil && !hasOrigin {
			return nil, false, fmt.Errorf(
				"no 'origin' remote in %s\n\n"+
					"drill pushes your branch and opens a pull request, so it needs a remote to push to.\n"+
					"Add one, then re-run:\n\n"+
...
// after init: it carries a drill remote pointing at a gate in our repos
...
// Eject removes the drill gate from the repo at workDir.
