...
# shellcheck source=/dev/null
. "$ROOT/bin/sq-check-lib.sh"

PR_CHECK="$ROOT/bin/sq-pr-check.sh"
PR_MERGE="$ROOT/bin/sq-pr-merge.sh"
MIGRATE="$ROOT/bin/sq-pr-check-migrate.sh"
POLL="$ROOT/bin/sq-pr-poll.sh"
WATCH="$ROOT/bin/sq-sentry.sh"
TEARDOWN="$ROOT/bin/sq-teardown.sh"
REGISTER="$ROOT/bin/sq-check-register.sh"
TMP_ROOT=$(fm_test_tmproot sq-pr-check-security)
...
make_case() {
  ...
  cat > "$fake_root/bin/sq-guard.sh" <<'SH'
#!/usr/bin/env bash
printf 'guard\n' >> "$SQUAD_TEST_GUARD_LOG"
SH
  chmod +x "$fake_root/bin/sq-guard.sh"
  cat > "$fakebin/gh" <<'SH'
#!/usr/bin/env bash
printf '%s\n' "$*" >> "$SQUAD_TEST_GH_LOG"
case " $* " in
...
write_sentry_lock() {
  ...
  rm -rf "$state/.sentry.lock"
  mkdir "$state/.sentry.lock"
  identity=$(SQUAD_STATE_OVERRIDE="$state" bash -c '. "$1"; fm_pid_identity "$2"' _ "$ROOT/bin/sq-stand-to-lib.sh" "$pid")
  [ -n "$identity" ] || fail "could not capture fake older-sentry identity"
  printf '%s\n' "$pid" > "$state/.sentry.lock/pid"
  printf '%s\n' "$home" > "$state/.sentry.lock/sq-home"
...
assert_valid_migration_marker() {
  ...
  grep -qxF sq-pr-check-migration-v1 "$marker" || fail "migration marker bytes were not exact"
...
assert_valid_scan_marker() {
  ...
  grep -qxF sq-pr-check-migration-scan-v1 "$marker" || fail "migration scan marker bytes were not exact"
...
test_parser_matrix() {
  ...
  while IFS='|' read -r url owner repo number; do
    ...
    [ "$SQUAD_PR_URL" = "$url" ] || fail "parser changed canonical URL"
    [ "$SQUAD_PR_OWNER" = "$owner" ] || fail "parser returned wrong owner"
    [ "$SQUAD_PR_REPO" = "$repo" ] || fail "parser returned wrong repository"
    [ "$SQUAD_PR_NUMBER" = "$number" ] || fail "parser returned wrong PR number"
  done <<'EOF'
...
EOF
  while IFS='|' read -r url host path number; do
    ...
    [ "$SQUAD_PR_PROVIDER" = gitlab ] || fail "parser did not tag a merge request URL as gitlab"
    [ "$SQUAD_PR_URL" = "$url" ] || fail "parser changed a canonical merge request URL"
    [ "$SQUAD_PR_HOST" = "$host" ] || fail "parser returned wrong GitLab host"
    [ "$SQUAD_PR_PATH" = "$path" ] || fail "parser returned wrong GitLab project path"
    [ "$SQUAD_PR_NUMBER" = "$number" ] || fail "parser returned wrong merge request number"
    [ -z "$SQUAD_PR_OWNER" ] && [ -z "$SQUAD_PR_REPO" ] \
...
EOF
  ...
  [ "$SQUAD_PR_PROVIDER" = github ] || fail "parser did not tag a pull request URL as github"
  [ "$SQUAD_PR_HOST" = github.com ] || fail "parser returned wrong GitHub host"
  [ "$SQUAD_PR_PATH" = a/b ] || fail "parser returned wrong GitHub project path"
...
test_valid_recording_and_merge_derivation() {
  ...
  SQUAD_TEST_GH_HEAD=$expected run_check_entry "$dir" task-a https://github.com/my-org/repo_name.with-dots/pull/37 \
...
test_rejected_metacharacter_bytes_are_inert() {
  ...
  for family in "${families[@]}"; do
    ...
    fm_pr_poll_prepare "$dir/home/state" safe-check github https://github.com/o/r/pull/99 github.com o/r 99 "$POLL" \
      || fail "could not prepare bounded sentry poll"
    fm_pr_poll_publish_prepared || fail "could not publish bounded sentry poll"

    set +e
    SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
    rc=$?
    set -e
    [ "$rc" -eq 0 ] || fail "bounded sentry did not complete through the authenticated poll"
    rm -f "$dir/home/state/.last-check"
  done

  SQUAD_TEST_GH_STATE=OPEN run_check_entry "$dir" task-a https://github.com/o/r/pull/1 >/dev/null 2>/dev/null \
  ...
  pass "rejected metacharacter bytes remain inert at generation and sentry time"
}
...
run_poll() {
  local dir=$1
  SQUAD_TEST_GH_LOG="$dir/gh.log" SQUAD_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$dir/fakebin:$BASE_PATH" \
...
test_static_poll_contract() {
  ...
  for state in OPEN CLOSED EMPTY MALFORMED; do
    ...
    out=$(SQUAD_TEST_GH_STATE="$value" run_poll "$dir")
    [ -z "$out" ] || fail "static poll emitted for non-merged state"
  done
  out=$(SQUAD_TEST_GH_STATE=MERGED run_poll "$dir")
  [ "$out" = merged ] || fail "static poll did not emit exactly one merged line"
  out=$(SQUAD_TEST_GH_FAIL=1 run_poll "$dir")
  ...
  make_poll_fixture "$dir"
  set +e
  out=$(SQUAD_STATE_OVERRIDE="$dir/home/state" SQUAD_CHECK_TIMEOUT=1 SQUAD_TEST_GH_LOG="$dir/gh.log" \
    SQUAD_TEST_GH_SLEEP=3 PATH="$dir/fakebin:$BASE_PATH" \
    bash -c '. "$1"; run_check "$2"' bash "$WATCH" "$dir/home/state/task-a.check.sh")
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "sentry run_check timeout wrapper failed"
  ...
  fm_pr_poll_prepare "$dir/home/state" task-a github https://github.com/o/r/pull/1 github.com o/r 1 "$POLL" \
    || fail "could not prepare authenticated sentry poll"
  fm_pr_poll_publish_prepared || fail "could not publish authenticated sentry poll"
  rm -f "$dir/home/state/.last-check"
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "sentry did not surface merged poll"
  [ "$(grep -c '^check: .*: merged$' "$dir/watch.out")" -eq 1 ] || fail "sentry did not convert merged output into exactly one wake"
  pass "static poll is silent except for one merged line and remains sentry-bounded"
}
...
SH
  ...
  ! find "$dir/home/state" -name '.sq-pr-poll-*' -print | grep . >/dev/null \
...
test_concurrent_sentry_sees_only_complete_publication() {
...
SH
    ...
    direct_pid=$!
    i=0
    while [ "$i" -lt 100 ] && ! find "$dir/home/state" -name '.sq-pr-poll-check.*' -print | grep . >/dev/null; do
      sleep 0.01
      i=$((i + 1))
    done
    ...
    [ "$rc" -eq 0 ] || fail "concurrent sentry did not complete"
    grep -q '^check: .*: merged$' "$dir/watch.out" || fail "concurrent sentry never saw complete poll"
    [ ! -s "$dir/watch.err" ] || fail "concurrent sentry observed a partial artifact error"
  ...
  pass "concurrent sentrys observe only complete private poll publications"
}

test_migration_excludes_older_sentry_before_scan() {
...
SH
  ...
  [ ! -e "$sentinel" ] || fail "older sentry ran a legacy check during migration startup"
  ...
  set -e
  wait "$older_pid" 2>/dev/null || true
  [ "$rc" -eq 0 ] || fail "no-check older-sentry migration failed"
  ! kill -0 "$older_pid" 2>/dev/null || fail "no-check migration left the older sentry running"
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  pass "migration pauses older sentrys and acquires exclusion before its first scan or marker"
}
...
test_migration_initializes_fresh_state() {
  ...
  assert_valid_migration_marker "$state/.pr-check-migration-v1"
  pass "migration creates and validates private state before sentry exclusion"
}
...
#!/usr/bin/env bash
last=${!#}
"${SQUAD_TEST_REAL_MV:?}" "$@" || exit $?
[ "$last" = "${SQUAD_TEST_FINAL_PATH:?}" ] || exit 0
case "${SQUAD_TEST_FINAL_ACTION:?}" in
  type)
    rm -f -- "$last"
    ln -s "${SQUAD_TEST_FAULT_LINK_TARGET:?}" "$last"
    ;;
  mode) "${SQUAD_TEST_REAL_CHMOD:?}" 0644 "$last" ;;
  content) printf 'faulted final bytes\n' > "$last" ;;
  device) : > "${SQUAD_TEST_FAULT_GATE:?}" ;;
  *) exit 2 ;;
esac
SH
...
#!/usr/bin/env bash
last=${!#}
if [ "$last" = "${SQUAD_TEST_FINAL_PATH:-}" ] && [ -e "${SQUAD_TEST_FAULT_GATE:-/nonexistent}" ]; then
  case " $* " in
    *" %d "*) printf '%s\n' 999999; exit 0 ;;
  esac
fi
exec "${SQUAD_TEST_REAL_STAT:?}" "$@"
SH
...
test_postrename_poll_validation_revokes_and_retries() {
  ...
  for artifact in data registration check; do
    for action in type mode device content; do
      ...
      if SQUAD_TEST_FINAL_PATH="$destination" SQUAD_TEST_FINAL_ACTION="$action" \
...
for arg in "$@"; do
  case "$arg" in
    *"${SQUAD_TEST_MV_MATCH:?}"*) matched=1 ;;
  esac
done
if [ "$matched" -eq 1 ]; then
  case "${SQUAD_TEST_MV_ACTION:?}" in
    fail) exit 1 ;;
    signal)
      kill -TERM "$PPID"
      sleep 0.1
...
last=${!#}
case "$last" in
  ${SQUAD_TEST_CHMOD_MATCH:?}) exit 0 ;;
esac
exec "${SQUAD_TEST_REAL_CHMOD:?}" "$@"
SH
...
"${SQUAD_TEST_REAL_MV:?}" "$@" || exit $?
case "$last" in
  */.pr-check-quarantine/task-a.check.*) : > "${SQUAD_TEST_FAULT_GATE:?}" ;;
esac
SH
  cat > "$dir/fakebin/stat" <<'SH'
#!/usr/bin/env bash
...
case "$last" in
  */.pr-check-quarantine/task-a.check.*)
    if [ -e "${SQUAD_TEST_FAULT_GATE:?}" ]; then
      case " $* " in
        *" %d "*) printf '%s\n' 999999; exit 0 ;;
      esac
    fi
...
case "$last" in
  */.pr-check-quarantine/task-a.check.*)
    "${SQUAD_TEST_REAL_CP:?}" "$source" "$last"
    exit $?
    ;;
esac
exec "${SQUAD_TEST_REAL_MV:?}" "$@"
  ...
  for case_name in copied-pair copied-registration metadata-mismatch task-mismatch forged-registration partial-publication; do
    ...
    case "$case_name" in
      ...
      forged-registration)
        write_manual_poll_pair "$state"
        printf '%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n%s\n' \
          sq-pr-poll-registration-v2 task-a github https://github.com/o/r/pull/10 github.com o/r 10 \
...
test_complete_single_link_validation() {
  ...
  ! fm_custom_check_snapshot_prepare "$state" custom \
    || fail "sentry snapshot accepted a hard-linked custom check source"
  ...
  ! fm_custom_check_snapshot_prepare "$state" custom \
    || fail "sentry snapshot accepted a hard-linked custom check trust record"
  fm_custom_check_snapshot_cleanup
  ...
  ! fm_custom_check_snapshot_prepare "$state" custom \
    || fail "sentry snapshot accepted a non-private custom check source"
  fm_custom_check_snapshot_cleanup
...
SH
  ...
  printf 'legacy quarantined bytes\n' \
    > "$state/.pr-check-quarantine/_noncanonical.check.abc123"
  printf 'sq-pr-check-migration-scan-v1\n' > "$state/.pr-check-migration-scan-v1"
...
test_nonexecuting_migration() {
  ...
  x_after=$(state_snapshot "$state" | grep 'x-sentry.check.sh')
...
test_historical_x_shim_transition_matrix() {
  ...
  for marker_kind in unmarked completed safe-scan; do
    ...
    cat > "$dir/root/bin/sq-x-poll.sh" <<SH
#!/usr/bin/env bash
touch '$executed'
SH
    chmod 0700 "$dir/root/bin/sq-x-poll.sh"
  ...
  dir=$(make_case historical-x-transition-sentry)
  state="$dir/home/state"
  shim="$state/x-sentry.check.sh"
  executed="$dir/x-poll-executed"
  cat > "$dir/root/bin/sq-x-poll.sh" <<SH
...
SH
  ...
  set +e
  SQUAD_TEST_CHECK_INTERVAL=999999 SQUAD_TEST_WATCH_ROOT="$dir/root" \
    run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "standalone sentry did not complete the historical X transition"
  fmx_poll_shim_valid "$shim" "$dir/home" "$dir/root" \
    || fail "standalone sentry did not publish the current X identity"
  [ "$(file_mode "$shim")" = 700 ] || fail "standalone sentry X shim mode was not 0700"
  [ ! -e "$executed" ] || fail "standalone sentry executed the historical X shim"
...
test_direct_registration_refreshes_v1_x_shim() {
  ...
  for marker_kind in unmarked completed safe-scan; do
    ...
    state="$dir/home/state"
    shim="$state/x-sentry.check.sh"
    fm_write_meta "$state/task-a.meta" 'window=sq-task-a'
    write_v1_x_shim "$shim" "$dir/home" "$dir/root"
    chmod 0755 "$shim"
    case "$marker_kind" in
      completed)
...
test_bootstrap_isolates_incomplete_poll_migration() {
  ...
  fm_write_meta "$state/XO-a.meta" \
    'window=Squad:sq-xo-a' \
    'kind=xo' \
    'harness=codex' \
    'backend=tmux'
...
#!/usr/bin/env bash
case " $* " in
  *' list-windows '*) printf 'sq-xo-a\n' ;;
  *' display-message '*) printf 'node\n' ;;
esac
SH
  cat > "$dir/root/bin/sq-unit-sync.sh" <<'SH'
#!/usr/bin/env bash
: > "${SQUAD_TEST_FLEET_MARKER:?}"
printf 'alpha: recovered: continued after isolated migration failure\n'
SH
  cat > "$dir/root/bin/sq-x-poll.sh" <<'SH'
#!/usr/bin/env bash
: > "${SQUAD_TEST_X_POLL_MARKER:?}"
SH
  ...
  assert_grep 'XO_SYNC: XO XO-a: skipped:' "$dir/bootstrap.out" \
    "incomplete poll migration suppressed XO sync"
  assert_grep 'XO_LIVENESS: XO XO-a: skipped: existing endpoint has ambiguous agent process' "$dir/bootstrap.out" \
    "incomplete poll migration suppressed persistent supervisor recovery"
  assert_grep 'SQX: X mode on - relay poll armed' "$dir/bootstrap.out" \
    "incomplete poll migration suppressed X mention setup"
  fmx_poll_shim_valid "$state/x-sentry.check.sh" "$dir/home" "$dir/root" \
    || fail "incomplete poll migration did not arm a private authenticated X relay shim"
  [ -e "$fleet_marker" ] || fail "incomplete poll migration suppressed unit refresh"
  assert_grep 'UNIT_SYNC: alpha: recovered: continued after isolated migration failure' "$dir/bootstrap.out" \
    "continued unit refresh was not operator-visible"
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "sentry remained blocked after unsafe legacy check exclusion: $(cat "$dir/watch.err")"
  [ -e "$x_poll_marker" ] || fail "sentry did not continue X mention polling after isolated migration failure"
  assert_no_grep 'replacement-ran' "$dir/watch.out" \
    "sentry executed an unauthenticated check created after scan completion"
  assert_grep "check: $state/z-healthy.check.sh: merged" "$dir/watch.out" \
    "sentry did not continue the healthy authenticated poll"
  [ ! -e "$state/task-a.check.sh" ] && [ ! -L "$state/task-a.check.sh" ] \
    || fail "sentry continuation rearmed the unsafe legacy check"
  ...
  assert_grep "check: $state/b-custom.check.sh: custom-ready" "$dir/watch-custom.out" \
    "registered custom check output did not wake the sentry"
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "sentry failed while rejecting a replaced custom check: $(cat "$dir/watch-custom-replaced.err")"
  assert_no_grep 'custom-replacement-ran' "$dir/watch-custom-replaced.out" \
    "sentry executed a custom check after its registered bytes changed"
  ...
  printf '%s\n' '#!/usr/bin/env bash' "printf '%s\\n' forged-x-ran" > "$state/x-sentry.check.sh"
  chmod 0700 "$state/x-sentry.check.sh"
  rm -f "$state/.last-check" "$x_poll_marker"
  set +e
  SQUAD_HOME="$dir/home" SQUAD_ROOT_OVERRIDE="$dir/root" SQUAD_TEST_X_POLL_MARKER="$x_poll_marker" \
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "sentry failed while rejecting a replaced X shim: $(cat "$dir/watch-replaced.err")"
  assert_no_grep 'forged-x-ran' "$dir/watch-replaced.out" \
    "sentry executed a filename-only X shim replacement"
  [ ! -e "$x_poll_marker" ] || fail "sentry trusted the replaced X shim identity"
  [ ! -e "$state/b-custom.check.sh" ] && [ ! -L "$state/b-custom.check.sh" ] \
    || fail "locked X-shim scan left the replaced custom check runnable"
  [ ! -e "$state/x-sentry.check.sh" ] && [ ! -L "$state/x-sentry.check.sh" ] \
...
test_custom_snapshot_cleanup_on_signal() {
  ...
  printf '%s\n' '#!/usr/bin/env bash' 'trap "" TERM' \
    'printf "%s\n" "$$" > "$SQUAD_TEST_CUSTOM_CHILD_PID"' 'while :; do sleep 1; done' \
    > "$state/custom.check.sh"
  chmod 0700 "$state/custom.check.sh"
...
SH
  ...
  [ -s "$child_pid_file" ] || fail "sentry did not start the custom check child"
  find "$state" -maxdepth 1 -name '.sq-custom-check.*' -print | grep . >/dev/null \
    || fail "sentry did not create the custom check snapshot"
  child_pid=$(cat "$child_pid_file")
  kill -TERM "$pid" 2>/dev/null || fail "could not signal sentry during custom check"
  i=0
  ...
  if kill -0 "$pid" 2>/dev/null; then
    ...
    fail "signaled sentry did not exit promptly"
  fi
  rc=0
  wait "$pid" || rc=$?
  [ "$rc" -ne 0 ] || fail "signaled sentry exited successfully"
  ! kill -0 "$child_pid" 2>/dev/null || fail "signaled sentry left the custom check child running"
  ! find "$state" -maxdepth 1 -name '.sq-custom-check.*' -print | grep . >/dev/null \
    || fail "signaled sentry left a private custom check snapshot"
  ! find "$state" -maxdepth 1 -name '.sq-check-output.*' -print | grep . >/dev/null \
    || fail "signaled sentry left a private check output file"
  [ ! -e "$state/.sentry.lock/pid" ] || fail "signaled sentry left its singleton lock"
  pass "sentry signals promptly stop custom checks and clean private state"
}

test_returned_custom_check_descendants_are_drained() {
  local backend dir state fakebin ready direct_done child_pid_file sentinel sentry_pid child_pid i rc alive force_fallback
...
#!/usr/bin/env bash
perl -e '$SIG{TERM}="IGNORE"; open my $ready, ">", $ENV{SQUAD_TEST_DESCENDANT_READY} or die $!; print {$ready} "ready\n"; close $ready; select undef, undef, undef, 4; open my $sentinel, ">", $ENV{SQUAD_TEST_DESCENDANT_SENTINEL} or die $!; print {$sentinel} "late\n"; close $sentinel; select undef, undef, undef, 1' &
printf '%s\n' "$!" > "$SQUAD_TEST_DESCENDANT_PID"
while [ ! -s "$SQUAD_TEST_DESCENDANT_READY" ]; do sleep 0.01; done
...
SH
    ...
    SQUAD_HOME="$dir/home" SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_POLL=0.1 SQUAD_CHECK_INTERVAL=999999 \
      ...
      SQUAD_TEST_DIRECT_DONE="$direct_done" PATH="$fakebin:$BASE_PATH" "$WATCH" \
      > "$dir/watch.out" 2> "$dir/watch.err" &
    sentry_pid=$!
    i=0
    while [ "$i" -lt 200 ]; do
      ...
      kill -0 "$sentry_pid" 2>/dev/null || break
      sleep 0.02
      i=$((i + 1))
    done
    [ -s "$ready" ] && [ -s "$child_pid_file" ] && [ -e "$direct_done" ] \
      && [ -e "$state/.last-check" ] \
      || fail "$backend sentry did not complete the direct custom check"
    child_pid=$(cat "$child_pid_file")
    kill -TERM "$sentry_pid" 2>/dev/null || fail "could not stop $backend sentry"
    i=0
    while kill -0 "$sentry_pid" 2>/dev/null && [ "$i" -lt 150 ]; do
      sleep 0.02
      i=$((i + 1))
    done
    if kill -0 "$sentry_pid" 2>/dev/null; then
      kill -KILL "$sentry_pid" 2>/dev/null || true
      wait "$sentry_pid" 2>/dev/null || true
      kill -KILL "$child_pid" 2>/dev/null || true
      fail "$backend sentry did not stop after the direct check returned"
    fi
    rc=0
    wait "$sentry_pid" || rc=$?
    [ "$rc" -ne 0 ] || fail "$backend signaled sentry exited successfully"
    alive=0
    kill -0 "$child_pid" 2>/dev/null && alive=1
    [ "$alive" -eq 0 ] || kill -KILL "$child_pid" 2>/dev/null || true
    wait "$child_pid" 2>/dev/null || true
    [ "$alive" -eq 0 ] || fail "$backend sentry left a returned check descendant alive"
    [ ! -e "$sentinel" ] || fail "$backend returned check descendant reached its sentinel"
    ! find "$state" -maxdepth 1 -name '.sq-custom-check.*' -print | grep . >/dev/null \
      || fail "$backend sentry left a private custom check snapshot"
    ! find "$state" -maxdepth 1 -name '.sq-check-output.*' -print | grep . >/dev/null \
      || fail "$backend sentry left a private check output file"
    [ ! -e "$state/.sentry.lock/pid" ] || fail "$backend sentry left its singleton lock"
  done
...
7" ] || fail "published GitLab sidecar bytes were not exact"
  ...
  # Only an exact merged state wakes Squad. Every other reading, including
  ...
  for value in opened closed locked '' not-a-state MERGED merged-but-not; do
    out=$(SQUAD_TEST_GLAB_STATE="$value" run_poll "$dir")
    [ -z "$out" ] || fail "GitLab poll emitted for a non-merged state"
  done
  out=$(SQUAD_TEST_GLAB_STATE=merged run_poll "$dir")
  [ "$out" = merged ] || fail "GitLab poll did not emit exactly one merged line"
  out=$(SQUAD_TEST_GLAB_FAIL=1 run_poll "$dir")
  ...
  # repository the sentry does not have.
...
EOF
  ...
  out=$(SQUAD_TEST_GLAB_STATE=merged SQUAD_TEST_GH_LOG="$dir/gh.log" SQUAD_TEST_GLAB_LOG="$dir/glab.log" \
    PATH="$noglab" \
    bash "$state/task-a.check.sh")
...
test_merged_poll_retires_once() {
  ...
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-1.out" 2> "$dir/watch-1.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "merged retirement sentry failed: $(cat "$dir/watch-1.err")"
  ...
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch-2.out" 2> "$dir/watch-2.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "second sentry cycle failed: $(cat "$dir/watch-2.err")"
  ...
  [ "$(grep -c $'\tcheck\t.*task-a.check.sh\t' "$state/.stand-to-queue" 2>/dev/null || true)" -eq 1 ] \
    || fail "merged poll did not queue exactly one terminal notification"
  pass "validated merged polls notify once and retire before the next sentry cycle"
}

test_persistent_XO_retirement_is_poll_only() {
  local dir state meta_before status_before registry_before endpoint_before rc
  dir=$(make_case merged-retirement-XO)
  state="$dir/home/state"
  fm_write_meta "$state/domain.meta" \
    'window=session:sq-domain' \
    "worktree=$dir/XO-home" \
    "project=$dir/project" \
    'kind=xo' \
    'mode=xo' \
    'backend=tmux' \
  ...
  printf -- '- domain | scope: test | home: %s\n' "$dir/XO-home" > "$dir/home/data/XOs.md"
  ...
  registry_before=$(shasum -a 256 "$dir/home/data/XOs.md")
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "persistent XO merged sentry failed: $(cat "$dir/watch.err")"
  assert_poll_absent "$state" domain
  [ "$(shasum -a 256 "$state/domain.meta")" = "$meta_before" ] || fail "retirement changed XO metadata"
  [ "$(shasum -a 256 "$state/domain.status")" = "$status_before" ] || fail "retirement changed XO status"
  [ "$(shasum -a 256 "$dir/home/data/XOs.md")" = "$registry_before" ] || fail "retirement changed XO registry"
  [ "$(shasum -a 256 "$dir/endpoint-sentinel")" = "$endpoint_before" ] || fail "retirement changed XO endpoint evidence"
  [ -d "$dir/XO-home" ] || fail "retirement removed the persistent XO home"
  pass "merged poll retirement preserves every persistent XO lifecycle artifact"
}
...
test_retirement_crash_recovery() {
  ...
  SQUAD_STATE_OVERRIDE="$state" bash -c '. "$1"; fm_wake_append check "$2" "$3"' _ \
    "$ROOT/bin/sq-stand-to-lib.sh" "$state/task-a.check.sh" "check: $state/task-a.check.sh: merged" \
    || fail "could not seed post-queue crash"
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "post-queue retry sentry failed: $(cat "$dir/watch.err")"
  assert_poll_absent "$state" task-a
  ...
  add_stop_custom_check "$dir"
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/restart.out" 2> "$dir/restart.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "receipt recovery sentry failed: $(cat "$dir/restart.err")"
  assert_poll_absent "$state" task-a
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-sq-pr-poll.sh"
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "template-update recovery sentry failed: $(cat "$dir/restart.err")"
  ...
  [ "$(grep -c $'\tcheck\t.*task-a.check.sh\t' "$state/.stand-to-queue")" -eq 1 ] \
...
test_external_merge_transition_retires_only_terminal_poll() {
  ...
  for label in open-green open-red closed-unmerged forge-error malformed; do
    ...
    case "$label" in
      open-green|open-red)
        SQUAD_TEST_GH_STATE=OPEN run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      closed-unmerged)
        SQUAD_TEST_GH_STATE=CLOSED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      forge-error)
        SQUAD_TEST_GH_FAIL=1 run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
      malformed)
        SQUAD_TEST_GH_STATE=NOT_A_FORGE_STATE run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/$label.out" 2> "$dir/$label.err"
        ;;
    esac
    rc=$?
    set -e
    [ "$rc" -eq 0 ] || fail "$label sentry cycle failed: $(cat "$dir/$label.err")"
  ...
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/merged.out" 2> "$dir/merged.err"
  rc=$?
  set -e
...
test_retirement_refuses_replacement_and_nonterminal_results() {
  ...
  state="$dir/home/state"
  historical_poll="$dir/historical-sq-pr-poll.sh"
  current_poll="$dir/current-sq-pr-poll.sh"
  cp "$POLL" "$historical_poll"
  ...
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "merged custom sentry failed: $(cat "$dir/custom.err")"
...
test_retirement_queue_failure_and_receipt_tampering() {
  ...
  mkdir "$state/.stand-to-queue"
  before=$(poll_artifact_snapshot "$state" task-a)
  set +e
  SQUAD_TEST_GH_STATE=MERGED run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  [ "$rc" -ne 0 ] || fail "sentry retired despite queue publication failure"
...
test_gitlab_merged_poll_retires() {
  ...
  set +e
  SQUAD_TEST_GLAB_STATE=merged run_sentry_bounded "$dir/home" "$dir/fakebin" > "$dir/watch.out" 2> "$dir/watch.err"
  rc=$?
  set -e
  [ "$rc" -eq 0 ] || fail "GitLab merged retirement sentry failed: $(cat "$dir/watch.err")"
