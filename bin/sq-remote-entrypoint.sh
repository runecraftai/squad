#!/usr/bin/env bash
# Fixed remote entrypoint for bin/sq-on.sh.
#
# Install this tracked file as sq-remote-entrypoint.sh on the remote account's
# non-interactive SSH PATH. It accepts protocol metadata plus a base64-encoded
# NUL argv stream, validates one genuine tracked executable in <root>/bin/sq-*.sh,
# then stages it for the Squad-owned remote job worker. It never accepts a
# shell command string.
#
# The readiness-owning sq-remote-doctor.sh runs in this plain SSH bootstrap so
...
# disconnect remains unknown completion to sq-on.sh, which preserves OpenSSH's
# exit 255 behavior. The shared library header owns job fields, bounds, PATH,
# LaunchAgent contract, and worker environment.
set -eu

PROTOCOL=1
DOCTOR_SHA256=6feb8840f72b50b3a38a44b71d8ac4987c1537974b576e7f930de0bceeef2453
...
TMP=$(mktemp -d "${TMPDIR:-/tmp}/sq-remote-entrypoint.XXXXXX") || die "cannot create protocol staging directory" 70
trap 'rm -rf -- "$TMP"' EXIT
...
[ -f "$ROOT/AGENTS.md" ] && [ ! -L "$ROOT/AGENTS.md" ] || die "remote root is not a Squad checkout"
...
COMMAND=${ARGV[0]}
case "$COMMAND" in sq-*.sh) ;; *) die "command is outside the sq-*.sh namespace: $COMMAND" ;; esac
...
if [ -n "$GIT_BIN" ]; then
...
elif [ "$COMMAND" = sq-remote-doctor.sh ]; then
  ...
  die "required tool git does not resolve on the remote operator PATH; install git there or put a wrapper for it in ~/.local/bin using the recipe in docs/remote-XOs.md"
fi
if [ "$COMMAND" = sq-remote-doctor.sh ]; then
  ...
  if [ -n "${SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE:-}" ]; then
    DOCTOR_ENV+=("SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=$SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE")
  fi
  if [ -n "${SQUAD_REMOTE_JOB_STATE_ROOT:-}" ]; then
    DOCTOR_ENV+=("SQUAD_REMOTE_JOB_STATE_ROOT=$SQUAD_REMOTE_JOB_STATE_ROOT")
  fi
  trap - EXIT
  rm -rf -- "$TMP"
...
if ! fm_remote_job_ensure_worker "$ROOT" "$ACCOUNT_HOME"; then
  die "${SQUAD_REMOTE_JOB_ERROR:-remote job worker is unavailable; run sq-on.sh <route> sq-remote-doctor.sh --fix}"
fi
if ! JOB_ID=$(fm_remote_job_stage "$ACCOUNT_HOME" "$ROOT" "$HOME_PATH" "$COMMAND" "${ARGV[@]:1}"); then
  die "${SQUAD_REMOTE_JOB_ERROR:-cannot stage remote job}" 70
fi
if ! fm_remote_job_wait "$ACCOUNT_HOME" "$JOB_ID"; then
  die "${SQUAD_REMOTE_JOB_ERROR:-remote job did not complete}" 70
fi
cat "$SQUAD_REMOTE_JOB_STDOUT"
cat "$SQUAD_REMOTE_JOB_STDERR" >&2
RESULT=$SQUAD_REMOTE_JOB_EXIT
