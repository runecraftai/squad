...
It provides task workspaces and surfaces while FOB continues to provide git worktrees.
...
cmux is macOS-only, GUI-first, and unsuitable for a headless or SSH-only Squad session.
...
cmux defaults to a control mode that rejects external shells, while Squad always controls it from an external process.
Open Settings > Automation and choose a viable Socket Control Mode before the first cmux-backed spawn.

| Setting | Value | Squad support | Security boundary |
| --- | --- | --- | --- |
...
For Password mode, store the password as the first line of local gitignored `config/cmux-socket-password` or provide `CMUX_SOCKET_PASSWORD` in Squad's environment.
...
Select cmux with local `config/backend` containing `cmux`, `SQUAD_BACKEND=cmux` for one launch, or an explicit request to Squad.
It can also be runtime auto-detected when Squad itself runs inside cmux.
...
Routine supervision uses `bin/sq-peek.sh <id>` and `SQUAD_HOME=<home> bin/sq-send.sh <id> '<text>'` without bringing the cmux window forward.
...
When the task is last in its window, Squad creates one unfocused unnamed sibling workspace in that same window, closes the task workspace, and leaves the window with cmux's fresh default workspace.
The sibling never carries an `sq-` title and is ignored by recovery.
...
Squad does not attempt to close the macOS window because cmux's socket cannot close a window holding a live terminal.

Real tests share the commander's running app rather than creating an isolated cmux session.
`tests/cmux-test-safety.sh` permits cleanup only for an exact currently listed `sq-test-` workspace and never enumerates and closes unrelated workspaces or relaunches the app.
