#!/usr/bin/env bash
# tests/sq-startup-network.test.sh - behavior tests for bin/sq-startup-network.sh,
...
#   - mutating sweeps are refused when the unit lock no longer names the session
...
TMP_ROOT=$(fm_test_tmproot sq-startup-network-tests)
...
# tests/sq-bootstrap.test.sh, so pinning it here would duplicate that owner and
...
new_world() {
  ...
  rm -f "$root/bin/sq-bootstrap.sh"
  cat > "$root/bin/sq-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
# Scriptable stand-in: records how it was invoked, then behaves as the test asks.
set -u
printf 'network=%s detect_only=%s\n' \
  "${SQUAD_BOOTSTRAP_NETWORK:-all}" "${SQUAD_BOOTSTRAP_DETECT_ONLY:-0}" \
  >> "${SQUAD_FAKE_BOOTSTRAP_LOG:?}"
# The real sweeps record their elapsed times through sq-timing-lib.sh, which
# reaches them as an exported SQUAD_TIMING_LOG. Recording the same way here proves
...
# is owned by tests/sq-bootstrap.test.sh.
if [ -n "${SQUAD_TIMING_LOG:-}" ]; then
  . "$(dirname "$0")/sq-timing-lib.sh"
  fm_timing_record phase "${SQUAD_FAKE_TIMING_PHASE:-gh-auth}" \
    "$(( $(fm_timing_now_ms) - 1500 ))" "${SQUAD_FAKE_TIMING_DETAIL:-}"
fi
[ -z "${SQUAD_FAKE_BOOTSTRAP_SLEEP:-}" ] || sleep "$SQUAD_FAKE_BOOTSTRAP_SLEEP"
[ -z "${SQUAD_FAKE_BOOTSTRAP_OUT:-}" ] || printf '%s\n' "$SQUAD_FAKE_BOOTSTRAP_OUT"
exit "${SQUAD_FAKE_BOOTSTRAP_RC:-0}"
SH
  chmod +x "$root/bin/sq-bootstrap.sh"
  cat > "$root/bin/ps" <<'SH'
#!/usr/bin/env bash
pid=
previous=
...
if [ "$pid" = "${SQUAD_FAKE_HARNESS_PID:-}" ]; then
  case "$*" in
...
EOF
  ...
  pass "sq-startup-network: wait fails when no deferred stage publishes before its deadline"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=10 \
  ...
  pass "sq-startup-network: start returns immediately and never holds the caller's stdout open"
}
...
EOF
  ...
  sleep 30 &
  claimant=$!
  SQUAD_SESSION_START_TIMEOUT=15 SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_OUT='acknowledged result' \
  ...
  [ ! -s "$home/state/.stand-to-queue" ] \
    || fail "a result harvest acknowledged also queued a wake: $(cat "$home/state/.stand-to-queue")"
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" run --locked 0
  assert_grep 'check	startup-network' "$home/state/.stand-to-queue" \
    "an unclaimed result never reached the stand-to queue"

  : > "$home/state/.stand-to-queue"
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" \
  ...
  pass "sq-startup-network: exactly one of the digest and the wake reports each result"
}
...
EOF
  sleep 10 &
  claimant=$!
  SQUAD_SESSION_START_TIMEOUT=4 SQUAD_FAKE_BOOTSTRAP_LOG="$log" \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  pass "sq-startup-network: a claimant crash after publication still surfaces the result"
}
...
EOF
  ...
  SQUAD_SESSION_START_TIMEOUT=4 SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_OUT='unpublishable result' \
    run_stage "$home" "$root" start --locked 0 --harvest-pid "$claimant"
  ...
  assert_grep 'check	startup-network' "$home/state/.stand-to-queue" \
    "the report-publication failure did not reach the stand-to queue"

  kill "$claimant" 2>/dev/null || true
  wait "$claimant" 2>/dev/null || true
  chmod 700 "$home/state/.startup-network.report"
  pass "sq-startup-network: a report-publication failure is failed, diagnosed, and still wakes"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" run --locked 1 --lock-pid 111111
  ...
  assert_contains "$report" "NETWORK_CHECKS: the unit lock was no longer held" \
  ...
  : > "$log"
  printf '%s\n' $$ > "$home/state/.lock"
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "sq-startup-network: manual callers cannot forge mutation authority"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=20 SQUAD_STARTUP_NETWORK_TIMEOUT=2 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  run_stage "$home" "$root" harvest --pid $$ >/dev/null
  SQUAD_STARTUP_NETWORK_TIMEOUT=2 run_stage "$home" "$root" wait 10 >/dev/null \
  ...
  assert_contains "$report" "sq-startup-network.sh run --locked 1" \
  ...
  pass "sq-startup-network: an aggregate bound turns a wedged sweep into an actionable line"
}
...
EOF
  ...
  assert_contains "$report" "dead-XO relaunch" \
...
phases=probe,sweeps
EOF
  assert_contains "$(SQUAD_STARTUP_NETWORK_TIMEOUT=10 run_stage "$home" "$root" report)" \
  ...
  pass "sq-startup-network: an abandoned run reports as needing a rerun, never as in progress forever"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=6 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  await_worker_record "$home"
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=6 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "sq-startup-network: a second start never launches a competing worker"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=5 \
  ...
  pass "sq-startup-network: start atomically reserves the generation harvest observes"
}
...
EOF
  ...
  SQUAD_FAKE_HARNESS_PID_OVERRIDE="$next_owner" SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=1 \
    run_stage "$home" "$root" start --locked 1 --harvest-pid $$
  ...
  pass "sq-startup-network: a new lock owner gets a distinct worker generation"
}
...
EOF
  ...
  assert_contains "$out" "lock acquired: harness pid $new_owner" \
    "the unit lock did not record the harness owner reported by acquisition"
  [ "$new_owner" != "$$" ] || fail "the prior harness still owned the lock after takeover"
  pass "sq-startup-network: unit-lock takeover cannot overlap a mutating sweep"
}
...
test_records_share_one_origin_so_offsets_form_a_timeline() {
  ...
  (
    ...
    . "$ROOT/bin/sq-timing-lib.sh"
    unset SQUAD_TIMING_EPOCH_MS
    SQUAD_TIMING_LOG=$log
    export SQUAD_TIMING_LOG
  ...
  pass "sq-startup-network: timing records share one origin so their offsets form a timeline"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_OUT='sweep finding' \
    SQUAD_FAKE_TIMING_PHASE=unit-sync SQUAD_FAKE_TIMING_DETAIL=dotfiles-private \
  ...
  assert_grep 'unit-sync' "$home/state/.startup-network.timings" \
  ...
  assert_contains "$report_out" "unit-sync dotfiles-private" \
  ...
  assert_not_contains "$harvest_out" "slowest:" \
    "the timings leaked into the session-start digest section"
  pass "sq-startup-network: timings are durable and printed only on demand"
}
...
EOF
  ...
  SQUAD_STARTUP_NETWORK_TIMEOUT=1 SQUAD_SESSION_START_TIMEOUT=2 \
    SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_BOOTSTRAP_SLEEP=20 \
    SQUAD_FAKE_TIMING_PHASE=XO-liveness SQUAD_FAKE_TIMING_DETAIL='mate-a@host-one' \
    run_stage "$home" "$root" run --locked 1
  ...
  assert_contains "$report_out" "XO-liveness mate-a@host-one" \
    "a timed-out run discarded the partial timings its sweeps had already recorded"
  pass "sq-startup-network: a timed-out run still publishes the partial timings it recorded"
}
...
EOF
  ...
  SQUAD_FAKE_BOOTSTRAP_LOG="$log" SQUAD_FAKE_TIMING_PHASE=XO-sync \
    SQUAD_FAKE_TIMING_DETAIL="ssh -i /key host	v1	forged	0	9999
GITHUB_TOKEN=ghp_supersecretvalue" \
  ...
  assert_grep 'XO-sync' "$home/state/.startup-network.timings" \
  ...
  pass "sq-startup-network: the timing artifact cannot carry a command line or forge records"
}
...
test_a_bounded_run_still_publishes_the_timings_it_managed_to_record
test_the_timing_artifact_cannot_carry_a_command_line_or_forge_records
echo "# sq-startup-network.test.sh: all assertions passed"
