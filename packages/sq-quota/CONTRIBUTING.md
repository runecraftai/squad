...
- Do not change `release-please-config.json`'s `bootstrap-sha` for ordinary changes.
  It pins release history to commit `71715714efeea44e804152126f25c199a8ffe7fa` (the per-package release wiring in this repo), so generated changelogs never include the pre-release scaffolding commits.
