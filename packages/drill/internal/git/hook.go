...
const gateConfigStampFile = "drill-gate-config"
const preservedPreReceiveHook = "pre-receive.drill-user"
...
func preReceiveHookScript(command string) string {
	return `#!/bin/sh
# drill pre-receive hook
# Authorize the pushing process before any managed gate ref changes.
DRILL_BIN=` + shellSingleQuote(command) + `
if [ ! -f "$DRILL_BIN" ]; then
  DRILL_BIN="$(command -v drill 2>/dev/null || echo drill)"
fi
...
out=$(DRILL_HOOK_HELPER=1 "$DRILL_BIN" daemon admit-push --gate "$GATE_DIR" 2>&1)
status=$?
if [ $status -ne 0 ]; then
  printf 'drill: gate push refused before ref mutation:\n%s\n' "$out" >&2
  exit $status
fi
...
func isManagedPreReceiveHook(content []byte) bool {
	text := string(content)
	return strings.Contains(text, "# drill pre-receive hook") && strings.Contains(text, "daemon admit-push")
}
...
while read oldrev newrev refname; do
	  ...
	  out=$(DRILL_HOOK_HELPER=1 "$DRILL_BIN" daemon notify-push "$@" 2>&1)
  status=$?
  if [ $status -ne 0 ]; then
    notify_failed=1
    {
    ...
    {
      printf 'drill: notify-push failed for %s (exit %d):\n' "$refname" "$status"
      printf '%s\n' "$out"
...
func isManagedPostReceiveHook(content []byte) bool {
	text := string(content)
	return strings.Contains(text, "# drill post-receive hook") && strings.Contains(text, "daemon notify-push")
}
...
// RefreshManagedPostReceiveHook updates an existing drill-owned hook.
