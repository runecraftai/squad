#!/usr/bin/env bash
# Full remote XO lifecycle over the deterministic generic SSH boundary.
set -u
...
TMP_ROOT=$(fm_test_tmproot sq-remote-XO-e2e)
mkdir -p "$TMP_ROOT"
...
case "\${1:-}" in
  ...
  list-windows)
    [ -f "\$state" ] || exit 0
    name=\$(cut -d'|' -f1 "\$state")
    case "\$*" in *'#{session_name}:#{window_name}'*) printf 'Squad:%s\n' "\$name" ;; *) printf '%s\n' "\$name" ;; esac
    exit 0
    ;;
  new-window)
    name=; cwd=
...
case "${SQUAD_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  inherit-partial:sq-remote-inherit.sh:config/crew-harness) exit 255 ;;
  inherit-block:sq-remote-inherit.sh:data/commander-shared.md)
    cat > "$SQUAD_FAKE_INHERIT_PAYLOAD"
    touch "$SQUAD_FAKE_INHERIT_ENTERED"
    while [ ! -f "$SQUAD_FAKE_INHERIT_RELEASE" ]; do sleep 0.02; done
    "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@" < "$SQUAD_FAKE_INHERIT_PAYLOAD"
    exit $?
    ;;
esac
# The readiness gate is answered here rather than by the real doctor, which
# would inspect and repair the RUNNER's own account. tests/sq-remote-doctor.test.sh
...
if [ "$command_name" = sq-remote-doctor.sh ]; then
  printf '%s %s\n' "${SQUAD_FAKE_SSH_MODE:-normal}" "${_command_action:--}" >> "$SQUAD_FAKE_DOCTOR_LOG"
  case "${SQUAD_FAKE_SSH_MODE:-normal}" in
    ...
    doctor-fix-unknown)
      ...
      printf 'check launchagent=fixable: no Squad herdr launch agent\n'
...
case "${SQUAD_FAKE_SSH_MODE:-normal}:$command_name:$command_rel" in
  ...
  launch-default-session-route:sq-remote-XO-control.sh:*)
    ...
    exit 0
    ;;
  provision-block-fail:sq-remote-home-provision.sh:*)
    touch "$SQUAD_FAKE_SEED_ENTERED"
...
case "${SQUAD_FAKE_SSH_MODE:-normal}" in
  unreachable) exit 255 ;;
  ambiguous) "$SQUAD_FAKE_REMOTE_ENTRYPOINT" "$@"; exit 255 ;;
...
remote_env() {
  ...
  SQUAD_FAKE_SSH_MODE="${SQUAD_FAKE_SSH_MODE:-normal}" \
  SQUAD_FAKE_REMOTE_CWD="$TMP_ROOT" \
  SQUAD_FAKE_SEED_ENTERED="$TMP_ROOT/seed.entered" \
  SQUAD_FAKE_SEED_RELEASE="$TMP_ROOT/seed.release" \
  SQUAD_FAKE_DOCTOR_LOG="$DOCTOR_LOG" \
  SQUAD_FAKE_DOCTOR_REPAIRED="$TMP_ROOT/doctor.repaired" \
  SQUAD_FAKE_INHERIT_ENTERED="$TMP_ROOT/inherit.entered" \
  SQUAD_FAKE_INHERIT_RELEASE="$TMP_ROOT/inherit.release" \
  SQUAD_FAKE_INHERIT_PAYLOAD="$TMP_ROOT/inherit.payload" \
  SQUAD_FAKE_LAUNCH_ENTERED="$TMP_ROOT/launch.entered" \
  SQUAD_FAKE_LAUNCH_RELEASE="$TMP_ROOT/launch.release" \
  SQUAD_SEND_SETTLE=0 SQUAD_SEND_SLEEP=0 SQUAD_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
[ "$(cat "$TMP_ROOT/concurrent-home/.sq-xo-home")" = ios ] \
...
if [ "${SQUAD_TEST_PROVISION_ONLY:-0}" = 1 ]; then
  echo "ALL TESTS PASSED"
  exit 0
fi

mkdir -p "$TMP_ROOT/seed-parent/data" "$TMP_ROOT/seed-parent/state"
SQUAD_XO_CHARTER='Failing seed charter.' SQUAD_XO_SCOPE='failed seed' \
  SQUAD_FAKE_SSH_MODE=provision-block-fail seed_env "$ROOT/bin/sq-remote-home-seed.sh" \
...
SQUAD_XO_CHARTER='Successful seed charter.' SQUAD_XO_SCOPE='successful seed' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-keep remote-mac "$REMOTE_ROOT" \
...
assert_no_grep '- seed-fail ' "$TMP_ROOT/seed-parent/data/XOs.md" "failed seed route survived rollback"
assert_grep '- seed-keep ' "$TMP_ROOT/seed-parent/data/XOs.md" "failed seed rollback removed a competing successful route"
assert_present "$TMP_ROOT/seed-keep-home/.sq-xo-home" "serialized seed lost its published remote home"
pass "remote seed rollback preserves serialized competing routes"

: > "$DOCTOR_LOG"
if SQUAD_XO_CHARTER='Unknown readiness charter.' SQUAD_XO_SCOPE='unknown readiness' \
  SQUAD_FAKE_SSH_MODE=doctor-fix-unknown seed_env "$ROOT/bin/sq-remote-home-seed.sh" \
...
assert_grep '- seed-unknown ' "$TMP_ROOT/seed-parent/data/XOs.md" \
...
: > "$DOCTOR_LOG"
if SQUAD_XO_CHARTER='Unready host charter.' SQUAD_XO_SCOPE='unready host' \
  SQUAD_FAKE_SSH_MODE=doctor-human seed_env "$ROOT/bin/sq-remote-home-seed.sh" \
...
assert_no_grep '- seed-toolless ' "$TMP_ROOT/seed-parent/data/XOs.md" \
...
out=$(SQUAD_XO_CHARTER='Repairable host charter.' SQUAD_XO_SCOPE='repairable host' \
  SQUAD_FAKE_SSH_MODE=doctor-fixable seed_env "$ROOT/bin/sq-remote-home-seed.sh" \
...
assert_present "$TMP_ROOT/seed-repair-home/.sq-xo-home" "the repaired host was never provisioned"
assert_grep '- seed-repair ' "$TMP_ROOT/seed-parent/data/XOs.md" "the repaired route was not registered"
...
# Seeding must not need a copy of the project in this home: Squad names the
...
if SQUAD_XO_CHARTER='Unsupplied origin charter.' SQUAD_XO_SCOPE='unsupplied origin' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-noorigin remote-mac "$REMOTE_ROOT" \
...
if SQUAD_XO_CHARTER='Unsafe origin charter.' SQUAD_XO_SCOPE='unsafe origin' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-unsafe remote-mac "$REMOTE_ROOT" \
...
if SQUAD_XO_CHARTER='Local-only charter.' SQUAD_XO_SCOPE='local only' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-localonly remote-mac "$REMOTE_ROOT" \
...
if SQUAD_XO_CHARTER='Unregistered charter.' SQUAD_XO_SCOPE='unregistered' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-unregistered remote-mac "$REMOTE_ROOT" \
...
out=$(SQUAD_XO_CHARTER='Own beta delivery on the build Mac.' \
  SQUAD_XO_SCOPE='beta delivery and validation' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-noclone remote-mac "$REMOTE_ROOT" \
...
assert_grep '- seed-noclone ' "$TMP_ROOT/seed-parent/data/XOs.md" \
...
# Squad is a shared template, so seeding must carry a project origin from any
...
forge_project() { # <project> <origin-url>
  ...
  git -C "$TMP_ROOT/forge-src-$project" \
    -c user.name='Squad Tests' -c user.email='tests@example.invalid' commit -qm origin
...
out=$(SQUAD_XO_CHARTER='Own delivery for projects hosted anywhere.' \
  SQUAD_XO_SCOPE='multi-forge delivery' \
  seed_env "$ROOT/bin/sq-remote-home-seed.sh" seed-forge remote-mac "$REMOTE_ROOT" \
  "$FORGE_HOME" \
...
assert_grep '- seed-forge ' "$TMP_ROOT/seed-parent/data/XOs.md" \
...
# Provision and register the remote route from the commander-facing primary.
out=$(SQUAD_XO_CHARTER='Own iOS delivery on the build Mac.' \
...
assert_grep 'host: remote-mac; root:' "$PARENT/data/XOs.md" "registry did not record the remote host dimension"
assert_present "$REMOTE_HOME/.sq-xo-home" "remote provisioning did not publish the identity marker"
...
if SQUAD_XO_CHARTER='Own iOS delivery on the build Mac.' \
  SQUAD_XO_SCOPE='iOS implementation and Xcode validation' \
  remote_env "$ROOT/bin/sq-remote-home-seed.sh" ios remote-mac "$REMOTE_ROOT" "$TMP_ROOT/other-home" alpha \
  >/dev/null 2>&1; then
  fail "remote seed allowed an existing id to move to another home"
fi
assert_grep "home: $REMOTE_HOME" "$PARENT/data/XOs.md" "refused remote reassignment changed the durable route"
...
# Add one local route to prove mixed units remain parseable and projected.
mkdir -p "$LOCAL_HOME/data" "$LOCAL_HOME/state" "$LOCAL_HOME/config" "$LOCAL_HOME/projects" "$LOCAL_HOME/bin"
printf 'local\n' > "$LOCAL_HOME/.sq-xo-home"
printf 'fixture\n' > "$LOCAL_HOME/AGENTS.md"
printf '## In flight\n\n## Queued\n\n## Done\n' > "$LOCAL_HOME/data/backlog.md"
cat >> "$PARENT/data/XOs.md" <<EOF
- local - Local delivery (home: $LOCAL_HOME; scope: local work; projects: alpha; added 2026-08-02)
EOF
remote_env "$ROOT/bin/sq-home-seed.sh" validate >/dev/null || fail "mixed local and remote registry validation failed"
...
if SQUAD_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/sq-spawn.sh" ios --xo \
...
out=$(remote_env "$ROOT/bin/sq-spawn.sh" ios --xo)
...
assert_grep 'remote_herdr_session=sq-remote' "$PARENT/state/ios.meta" "parent metadata omitted the pinned remote Herdr session"
assert_grep 'remote_target=sq-remote:' "$PARENT/state/ios.meta" "parent metadata did not record an sq-remote endpoint"
assert_grep 'herdr_session=sq-remote' "$REMOTE_HOME/state/parent-route/ios.meta" "remote metadata did not record the pinned Herdr session"
assert_grep '--session sq-remote' "$HERDR_LOG" "remote launch did not target the sq-remote session"
...
cp "$PARENT/data/XOs.md" "$TMP_ROOT/registry-before-nonherdr.md"
set +e
SQUAD_FAKE_SSH_MODE=launch-nonherdr-route remote_env "$ROOT/bin/sq-spawn.sh" ios --xo \
...
cmp -s "$TMP_ROOT/registry-before-nonherdr.md" "$PARENT/data/XOs.md" \
  || fail "parent removed or changed the registry route after a non-herdr route refusal"

set +e
SQUAD_FAKE_SSH_MODE=launch-default-session-route remote_env "$ROOT/bin/sq-spawn.sh" ios --xo \
...
assert_grep "remote launch returned Herdr session 'default', expected 'sq-remote'" "$TMP_ROOT/spawn-default-session-route.out" \
...
cat > "$PARENT/data/commander-shared.md" <<'EOF'
# Shared commander preferences
This file is main-authoritative and maintained by the main Squad.
It is read-only in XO homes and must not be edited there.
Changes return through a marked status document pointer.
stale spawn preference
EOF
SQUAD_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/sq-spawn.sh" ios --xo \
  > "$TMP_ROOT/spawn-concurrent.out" 2>&1 &
spawn_concurrent=$!
spawn_inherit_wait=0
# Earlier inherited files traverse the worker before commander-shared.md, so give
...
EOF
remote_env "$ROOT/bin/sq-config-push.sh" > "$TMP_ROOT/spawn-concurrent-push.out" 2>&1 &
spawn_config_push=$!
sleep 0.2
...
[ "$(tail -1 "$REMOTE_HOME/data/commander-shared.md")" = 'current post-spawn preference' ] \
...
set +e
SQUAD_FAKE_SSH_MODE=ambiguous remote_env "$ROOT/bin/sq-send.sh" sq-ios \
...
remote_env "$ROOT/bin/sq-procevent.sh" start "$SID" >/dev/null \
  || fail "remote reply source did not capture the correlated answer"
RESULT="$PARENT/state/procevent-inbox/$SID.1.result"
remote_env "$ROOT/bin/sq-procevent-remote-reply.sh" handle ios 1 "$RESULT" >/dev/null \
...
rm -f "$PARENT/state/.stand-to-queue"

printf '{"revision":2}\n' > "$PARENT/config/crew-dispatch.json"
printf 'grok\n' > "$PARENT/config/crew-harness"
set +e
SQUAD_FAKE_SSH_MODE=inherit-partial remote_env "$ROOT/bin/sq-config-push.sh" \
...
NUDGE_MARKER="$PARENT/state/.XO-nudge-pending/ios.pending"
...
remote_env "$ROOT/bin/sq-bootstrap.sh" > "$TMP_ROOT/config-partial-retry.out" \
...
remote_env "$ROOT/bin/sq-procevent-remote-reply.sh" handle ios 2 "$PARTIAL_CONFIG_RESULT" >/dev/null \
...
EOF
SQUAD_FAKE_SSH_MODE=inherit-block remote_env "$ROOT/bin/sq-config-push.sh" \
...
EOF
remote_env "$ROOT/bin/sq-bootstrap.sh" > "$TMP_ROOT/config-concurrent-second.out" 2>&1 &
config_second=$!
sleep 0.2
...
[ "$(tail -1 "$REMOTE_HOME/data/commander-shared.md")" = 'current concurrent preference' ] \
...
if remote_env "$ROOT/bin/sq-config-push.sh" > "$TMP_ROOT/config-push-fail.out" 2>&1; then
...
remote_env "$ROOT/bin/sq-config-push.sh" > "$TMP_ROOT/config-push-retry.out" \
...
remote_env "$ROOT/bin/sq-procevent-remote-reply.sh" handle ios 3 "$CONFIG_RESULT" >/dev/null \
...
resolve_ios_pending() {
  ...
  for pending_record in "$PARENT/state/pending-replies"/*; do
    ...
    remote_env "$ROOT/bin/sq-procevent-remote-reply.sh" handle ios "$pending_seq" "$pending_result" >/dev/null \
      || fail "concurrent inheritance acknowledgment was not ingested"
  done
}
resolve_ios_pending

# Structured unit state comes from each home's own snapshot. The remote host is
# explicit, and the local route remains alongside it.
SNAPSHOT=$(remote_env "$ROOT/bin/sq-unit-snapshot.sh" --json)
if ! printf '%s' "$SNAPSHOT" | jq -e '.XO_current.records | any(.id == "ios" and .remote == true and .host == "remote-mac" and .provenance.selected == "structured-home")' >/dev/null; then
  printf 'XO projection:\n%s\n' "$(printf '%s' "$SNAPSHOT" | jq '.XO_current')" >&2
  fail "unit snapshot did not select the remote structured-home projection"
fi
...
printf '%s' "$SNAPSHOT" | jq -e '.XO_current.records | any(.id == "local" and .remote == false)' >/dev/null \
  || fail "unit snapshot lost the existing local XO route"
pass "unit snapshot projects mixed local and remote structured state"
rm -f "$PARENT/state/.stand-to-queue"
...
REMOTE_SEED="$TMP_ROOT/Squad-seed"
...
assert_not_contains "$BOOT_REPAIRED" 'XO_LIVENESS: XO ios:' \
...
cp "$PARENT/data/XOs.md" "$TMP_ROOT/registry-before-liveness-legacy.md"
cat > "$remote_route_meta" <<EOF
window=Squad:sq-ios
worktree=$REMOTE_HOME
project=$REMOTE_ROOT
...
BOOT_LEGACY=$(remote_env "$ROOT/bin/sq-bootstrap.sh")
assert_contains "$BOOT_LEGACY" "XO_LIVENESS: XO ios: skipped: remote endpoint state is unverified on remote-mac" \
...
cmp -s "$TMP_ROOT/registry-before-liveness-legacy.md" "$PARENT/data/XOs.md" \
...
rm -rf -- "$PARENT/state/.sentry.lock"
rm -f -- "$PARENT/state/.last-sentry-beat"
BOOT_UNAVAILABLE=$(SQUAD_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/sq-bootstrap.sh")
assert_contains "$BOOT_UNAVAILABLE" 'XO_LIVENESS: XO ios: skipped: remote host unavailable or endpoint state unknown' \
  "bootstrap did not preserve an unreachable remote endpoint as unknown"
UNAVAILABLE=$(SQUAD_FAKE_SSH_MODE=unreachable remote_env "$ROOT/bin/sq-unit-snapshot.sh" --json)
printf '%s' "$UNAVAILABLE" | jq -e '.XO_current.records | any(.id == "ios" and .current.state == "unknown")' >/dev/null \
...
assert_grep '- ios ' "$PARENT/data/XOs.md" "unreachable readiness removed the registry route"
...
# A sibling remote XO workspace shares sq-remote and must survive every
# refusal and the eventual successful retirement of ios.
# This fixture overrides SQUAD_ROOT for transport, so teardown's root-owned guard
# sees the fixture root rather than the source script path used by sq-send.
publish_healthy_sentry_identity "$PARENT/state" "$PARENT" "$REMOTE_ROOT/bin/sq-sentry.sh"
resolve_ios_pending
...
if remote_env "$ROOT/bin/sq-teardown.sh" ios >/dev/null 2>&1; then
...
assert_grep '- ios ' "$PARENT/data/XOs.md" "refused remote retirement removed the route"
rm -f "$PARENT/state/procevent"
mkdir "$PARENT/state/procevent"
remote_env "$ROOT/bin/sq-bootstrap.sh" >/dev/null \
...
SQUAD_HOME="$PARENT" /bin/bash -c '
  ...
  fm_lock_release "$2"
' _ "$ROOT/bin/sq-stand-to-lib.sh" "$handoff_lock" "$TMP_ROOT/handoff.entered" \
...
SQUAD_FAKE_SSH_MODE=launch-block remote_env "$ROOT/bin/sq-spawn.sh" ios --xo \
...
remote_env "$ROOT/bin/sq-teardown.sh" ios > "$TMP_ROOT/teardown-serialized.out" 2>&1 &
teardown_pid=$!
sleep 0.2
...
assert_no_grep 'session stop' "$HERDR_LOG" "remote retirement stopped the shared sq-remote session"
assert_no_grep 'server stop' "$HERDR_LOG" "remote retirement stopped the shared sq-remote server"
