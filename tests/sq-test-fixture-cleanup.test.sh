...
test_fixture_root_gone_after_normal_exit() {
  ...
  child_out=$(bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(fm_test_tmproot sq-test-cleanup-exit)
    printf "%s\n" "$d"
...
test_fixture_root_gone_after_sigterm() {
  ...
  harness=$(fm_test_tmproot sq-test-cleanup-sigterm-harness)
  dirfile="$harness/child-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(fm_test_tmproot sq-test-cleanup-term)
    printf "%s\n" "$d" > "'"$dirfile"'"
...
test_fixture_registration_failure_rolls_back_root() {
  ...
  harness=$(fm_test_tmproot sq-test-cleanup-registration-harness)
  failure_tmp="$harness/tmp"
  registry_dir="$harness/registry-dir"
  mkdir -p "$failure_tmp" "$registry_dir"

  if output=$(TMPDIR="$failure_tmp" SQUAD_TEST_CLEANUP_REGISTRY="$registry_dir" \
    fm_test_tmproot sq-test-cleanup-registration-failure 2>/dev/null); then
  ...
  for leaked_root in "$failure_tmp"/sq-test-cleanup-registration-failure.*; do
...
test_orphan_sweep_respects_fixture_ownership() {
  ...
  harness=$(fm_test_tmproot sq-test-cleanup-orphan-harness)
  dirfile="$harness/active-dir"
  bash -c '
    # shellcheck source=tests/lib.sh
    . "'"$LIB"'"
    d=$(fm_test_tmproot sq-test-cleanup-active)
    printf "%s\n" "$d" > "'"$dirfile"'"
    while :; do sleep 0.1; done
  ' &
  pid=$!
  ...
  touch -t 202001010000 "$active_dir/.sq-test-fixture"

  stale_dir=$(mktemp -d "${TMPDIR:-/tmp}/sq-test-cleanup-stale.XXXXXX")
  printf '%s\n%s\n' "$$" reused-process-identity > "$stale_dir/.sq-test-fixture"
  touch -t 202001010000 "$stale_dir/.sq-test-fixture"
  fresh_dir=$(mktemp -d "${TMPDIR:-/tmp}/sq-test-cleanup-fresh.XXXXXX")
  : > "$fresh_dir/.sq-test-fixture"
