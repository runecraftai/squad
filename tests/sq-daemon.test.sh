#!/usr/bin/env bash
# tests/sq-daemon.test.sh - supervise-daemon classifiers, the commander-relevant
...
# sq-send swallow reporting, composer-pending ANSI parsing). The operator-visible
# inject flow lives in sq-afk-inject-e2e and sq-stand-to-daemon-lifecycle-e2e.
set -u

# shellcheck source=tests/stand-to-helpers.sh
. "$(dirname "${BASH_SOURCE[0]}")/stand-to-helpers.sh"

DAEMON="$ROOT/bin/sq-supervise-daemon.sh"
AFK_START="$ROOT/bin/sq-afk-start.sh"
...
if [ -z "${SQUAD_TEST_DAEMON_SOURCED:-}" ]; then
  export SQUAD_TEST_DAEMON_SOURCED=1
  # shellcheck source=bin/sq-supervise-daemon.sh
  . "$DAEMON"
fi

TMP_ROOT=$(fm_test_tmproot sq-daemon-tests)
SQUAD_DAEMON_PRIMARY_HARNESS=claude
export SQUAD_DAEMON_PRIMARY_HARNESS
...
test_afk_start_refuses_when_flag_cannot_be_written() {
  ...
  out=$(SQUAD_STATE_OVERRIDE="$state" SQUAD_SUPERVISOR_BACKEND=unsupported "$AFK_START" 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "sq-afk-start.sh should fail when state/.afk cannot be written"
  assert_not_contains "$out" "starting supervise daemon" "sq-afk-start.sh continued into daemon startup after .afk write failure"
  assert_absent "$state/.supervise-daemon.log" "sq-afk-start.sh started the daemon after .afk write failure"
  pass "sq-afk-start.sh fails before daemon startup when the afk flag cannot be written"
}
...
test_afk_start_ignores_stale_pidfile_without_lock() {
  ...
  [ "$status" -ne 0 ] || fail "sq-afk-start.sh should attempt daemon startup instead of trusting a pidfile-only live pid"
  assert_contains "$out" "starting supervise daemon" "sq-afk-start.sh did not attempt daemon startup"
  assert_contains "$out" "does not support supervisor backend 'unsupported'" "daemon startup did not reach backend validation"
  assert_not_contains "$out" "daemon already running" "sq-afk-start.sh trusted a stale pidfile-only live pid"
  pass "sq-afk-start.sh ignores stale pidfile-only live pids"
}
...
test_afk_start_reclaims_stale_daemon_lock_reused_pid() {
  ...
  [ "$status" -ne 0 ] || fail "sq-afk-start.sh should attempt daemon startup after rejecting a reused-pid lock"
  assert_contains "$out" "starting supervise daemon" "sq-afk-start.sh did not attempt daemon startup after rejecting the stale lock"
  ...
  assert_not_contains "$out" "daemon already running" "sq-afk-start.sh trusted a stale daemon lock with a reused pid"
  assert_not_contains "$out" "another sq-supervise-daemon is already running" "daemon singleton lock still trusted the reused pid"
  pass "sq-afk-start.sh reclaims stale daemon locks whose live pid identity no longer matches"
}

test_daemon_state_root_uses_fm_home() {
  local dir home override out
  dir=$(make_supercase daemon-sq-home)
  home="$dir/Squad-home"
...
test_classify_routine_signal_self() {
  ...
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_signal "$state/foo-x1.status" "$state")
...
test_classify_terminal_signal_escalates() {
  ...
  pass "commander-relevant status verbs escalate"
}
...
test_stale_transient_self_records_marker() {
  ...
  stale_marker_record "sess:sq-qux-w4" "$state"
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_stale "sess:sq-qux-w4" "$state")
  case "$out" in self\|*) ;; *) fail "transient stale did not self-handle: $out" ;; esac
  key=$(printf '%s' "$(window_to_task "sess:sq-qux-w4")" | tr ':/.' '___')
...
test_stale_diagnostic_wedge_survives_busy_housekeeping() {
  ...
  for case_name in working prior-terminal paused; do
    ...
    (
      ...
      LOG="$dir/daemon.log" SQUAD_STATE_OVERRIDE="$state" handle_wake "$reason" "$state"
      PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_WINDOW="$win" SQUAD_FAKE_TMUX_CAPTURE="$pane" \
...
test_stale_paused_classifies_pause() {
  ...
  status_is_commander_relevant "$pause_reason" && fail "pause reason phrases made the status commander-relevant"
  printf '%s\n' "$pause_reason" > "$state/held-w9.status"
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_stale "sess:sq-held-w9" "$state")
  case "$out" in pause\|*) ;; *) fail "declared pause did not classify as pause: $out" ;; esac
  pass "paused reasons with commander phrases remain pause-classified"
}
...
test_handle_wake_paused_records_pause_marker() {
  ...
  SQUAD_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
  [ -e "$state/.subsuper-paused-$key" ] || fail "pause marker not recorded by handle_wake"
  [ ! -e "$state/.subsuper-stale-$key" ] || fail "wedge marker not cleared when the operator declared a pause"
...
test_handle_wake_paused_signal_records_pause_marker() {
  ...
  state="$dir/state"
  win="sess:sq-held-w10-signal"
  printf 'window=%s\nkind=strike\n' "$win" > "$state/held-w10-signal.meta"
  printf 'paused: awaiting the vendor rate-limit reset\n' > "$state/held-w10-signal.status"
  key=$(printf '%s' "held-w10-signal" | tr ':/.' '___')
  date +%s > "$state/.subsuper-stale-$key"
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-signal.status" "$state"
...
test_handle_wake_terminal_signal_clears_pause_tracking() {
  local dir state key sentry_key win
  dir=$(make_supercase handle-terminal-signal)
  state="$dir/state"
  win="sess:sq-held-w10-terminal"
  ...
  sentry_key=$(printf '%s' "$win" | tr '.:/' '___')
  date +%s > "$state/.subsuper-paused-$key"
  date +%s > "$state/.subsuper-stale-$key"
  : > "$state/.paused-$sentry_key"
  : > "$state/.stale-$sentry_key"
  : > "$state/.wedge-escalations-$sentry_key"
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $state/held-w10-terminal.status" "$state"
  ...
  [ ! -e "$state/.paused-$sentry_key" ] || fail "terminal signal retained sentry pause tracking"
  [ ! -e "$state/.stale-$sentry_key" ] || fail "terminal signal retained sentry stale tracking"
  [ ! -e "$state/.wedge-escalations-$sentry_key" ] || fail "terminal signal retained sentry wedge tracking"
  SQUAD_STATE_OVERRIDE="$state" handle_wake "stale: $win" "$state"
...
test_housekeeping_migrates_sentry_pause_marker() {
  local dir state key win
  dir=$(make_supercase migrate-sentry-pause)
  state="$dir/state"
  win="sess:sq-held-w10-migrate"
  printf 'window=%s\nkind=strike\n' "$win" > "$state/held-w10-migrate.meta"
  printf 'paused: awaiting the upstream release\n' > "$state/held-w10-migrate.status"
  key=$(printf '%s' "$win" | tr '.:/' '___')
  : > "$state/.paused-$key"
  SQUAD_STATE_OVERRIDE="$state" housekeeping "$state"
  key=$(printf '%s' "held-w10-migrate" | tr '.:/' '___')
  [ -e "$state/.subsuper-paused-$key" ] || fail "sentry pause marker was not migrated into daemon tracking"
  [ ! -e "$state/.subsuper-stale-$key" ] || fail "sentry pause migration left a wedge marker behind"
  pass "housekeeping migrates a normal-sentry's declared pause into daemon tracking"
}

test_housekeeping_migrates_sentry_unpaused_marker_to_clear() {
  local dir state key sentry_key win
  dir=$(make_supercase migrate-sentry-unpaused)
  state="$dir/state"
  win="sess:sq-held-w10-migrate-unpaused"
  ...
  [ ! -e "$state/.paused-$sentry_key" ] || fail "stale sentry pause marker was not cleared after resume"
  [ ! -e "$state/.subsuper-paused-$key" ] || fail "unpaused sentry handoff created a daemon pause marker"
  [ ! -e "$state/.subsuper-stale-$key" ] || fail "unpaused sentry handoff retained daemon stale tracking"
  [ ! -e "$state/.stale-$sentry_key" ] || fail "unpaused sentry handoff retained sentry stale tracking"
...
test_housekeeping_seeds_pause_marker_from_status() {
  ...
  pass "housekeeping seeds pause tracking from status without a sentry marker"
}
...
test_housekeeping_paused_resurfaces_and_resets() {
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_WINDOW="$win" SQUAD_FAKE_TMUX_CAPTURE="$pane" \
    SQUAD_STATE_OVERRIDE="$state" SQUAD_PAUSE_RESURFACE_SECS=240 housekeeping "$state"
...
# A pause whose pane became busy again (the operator resumed) drops its marker without
# escalating, exactly like a resumed wedge.
test_housekeeping_paused_resumed_cleared() {
  local dir state fakebin win pane key
  dir=$(make_supercase paused-resumed)
  state="$dir/state"; fakebin="$dir/fakebin"
  win="sess:sq-held-w12"; pane="$dir/pane.txt"
  printf 'paused: holding for the upstream tool release\n' > "$state/held-w12.status"
  printf 'Working...\n' > "$pane"
  fm_write_meta "$state/held-w12.meta" "window=$win" "worktree=$dir/wt" "kind=strike" "harness=pi"
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$state" held-w12)
  "$ROOT/bin/sq-busy-event.sh" apply "$state" held-w12 busy --gen "$gen" \
...
# A pane still idle but whose status is no longer a pause (the operator changed state
...
test_housekeeping_paused_unpaused_cleared() {
  ...
  [ ! -s "$state/.subsuper-escalations" ] || fail "an operator that left its pause was re-surfaced as a pause"
  pass "housekeeping clears a paused marker once the operator is no longer declaring the pause"
}
...
test_housekeeping_stale_marker_transitions_to_pause() {
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_WINDOW="$win" SQUAD_FAKE_TMUX_CAPTURE="$pane" \
    SQUAD_STATE_OVERRIDE="$state" SQUAD_STALE_ESCALATE_SECS=240 housekeeping "$state"
...
test_housekeeping_pause_marker_transitions_to_clear() {
  ...
  date +%s > "$state/.subsuper-paused-$key"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_WINDOW="$win" SQUAD_FAKE_TMUX_CAPTURE="$pane" \
    SQUAD_STATE_OVERRIDE="$state" SQUAD_PAUSE_RESURFACE_SECS=999999 housekeeping "$state"
  [ ! -e "$state/.subsuper-paused-$key" ] || fail "pause marker remained after the operator resumed"
  ...
  pass "housekeeping clears tracking when an operator leaves pause"
}
...
test_housekeeping_resumed_stale_cleared() {
  ...
  # record (bin/sq-busy-lib.sh), not through the pane's rendered footer.
  fm_write_meta "$state/res-w6.meta" "window=$win" "worktree=$dir/wt" "kind=strike" "harness=pi"
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$state" res-w6)
  "$ROOT/bin/sq-busy-event.sh" apply "$state" res-w6 busy --gen "$gen" \
    --source pi-ext --event agent-start
...
test_housekeeping_herdr_idle_busy_record_clears_stale() {
  ...
  gen=$("$ROOT/bin/sq-busy-event.sh" arm "$state" herdr-footer)
  "$ROOT/bin/sq-busy-event.sh" apply "$state" herdr-footer busy --gen "$gen" \
...
test_housekeeping_orca_persistent_stale_resolves_terminal() {
  ...
  state="$dir/state"
  fm_write_meta "$state/orca-w8.meta" "window=sq-orca-w8" "terminal=term-orca-w8" "backend=orca"
...
test_escalate_batches_into_one_digest() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_PANE_ALIVE=1 SQUAD_FAKE_TMUX_SENT="$sent" \
    SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
    || fail "escalate_flush failed"
  grep -F 'SQUAD_OP: v1 away-supervisor: ' "$sent" >/dev/null \
...
test_escalate_batch_age_uses_first_append() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_PANE_ALIVE=1 SQUAD_FAKE_TMUX_SENT="$sent" \
    SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_ESCALATE_BATCH_SECS=90 SQUAD_HOUSEKEEPING_TICK=0 \
    housekeeping "$state"
  grep -F 'event A: done: PR 1 | event B: done: PR 2' "$sent" >/dev/null \
...
test_handle_wake_routes_self_and_escalate() {
  ...
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-routine.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "routine signal was escalated by handle_wake"
  printf 'done: PR 1\n' > "$state/h-done.status"
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $state/h-done.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "commander signal was not buffered by handle_wake"
...
test_inject_skip_forces_self() {
  ...
  state="$dir/state"
  printf 'done: PR 1\n' > "$state/s1.status"
  SQUAD_STATE_OVERRIDE="$state" SQUAD_INJECT_SKIP="signal" handle_wake "signal: $state/s1.status" "$state"
  [ -s "$state/.subsuper-escalations" ] && fail "INJECT_SKIP=signal did not force self-handle"
  pass "INJECT_SKIP forces self-handle, bypassing commander-relevant classification"
}

test_is_wake_reason_distinguishes_status_stdout() {
  # Real wake reasons are recognized; sentry status lines (singleton collision)
  ...
  pass "is_wake_reason distinguishes sentry wake reasons from singleton-status stdout"
}
...
test_signal_escalate_marks_seen_no_catchall_refire() {
  ...
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $state/sig-t8.status" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "commander signal was not escalated"
  key=$(printf '%s' "sig-t8" | tr ':/.' '___')
  [ "$(cat "$state/.subsuper-seen-status-$key" 2>/dev/null || true)" = "done: PR https://x/y/pull/8" ] \
    || fail "commander signal escalate did not write the seen-status marker"
  : > "$state/.subsuper-escalations"
  rm -f "$state/.subsuper-last-scan"
  SQUAD_STATE_OVERRIDE="$state" housekeeping "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "catch-all scan re-fired an already-escalated signal"
  pass "commander signal escalate marks seen so the catch-all scan does not re-fire"
}
...
test_afk_absent_daemon_does_not_inject() {
  ...
  if PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_PANE_ALIVE=1 SQUAD_FAKE_TMUX_SENT="$sent" \
    SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_ESCALATE_BATCH_SECS=0 escalate_flush "$state"; then
...
test_marker_detection() {
  local marker_hex
  marker_hex=$(printf '%s' "$SQUAD_INJECT_MARK" | od -An -tx1 | tr -d ' \n')
  [ "$marker_hex" = e281a3 ] \
    || fail "SQUAD_INJECT_MARK must use terminal-safe U+2063 bytes, got $marker_hex"
  [ "$SQUAD_OPERATIONAL_PREFIX" = "${SQUAD_INJECT_MARK}SQUAD_OP: " ] \
    || fail "away-mode operational prefix drifted from the shared commander-boundary marker"
  # message_is_injection: marker present -> injection; absent -> real message
  message_is_injection "${SQUAD_OPERATIONAL_PREFIX}Supervisor escalate: done" \
    || fail "operationally-prefixed message not detected as injection"
  message_is_injection "${SQUAD_INJECT_MARK}Supervisor escalate: done" \
...
test_afk_turn_exemption() {
  ...
  # a non-/afk skill invocation DOES exit (the commander is actively working)
  should_exit_afk "$state" "/drill" \
...
test_should_exit_afk_when_afk_inactive() {
  ...
  should_exit_afk "$state" "${SQUAD_INJECT_MARK}test" \
...
test_strip_injection_marker() {
  ...
  stripped=$(strip_injection_marker "${SQUAD_OPERATIONAL_PREFIX}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
    || fail "landed untyped operational prefix not stripped: '$stripped'"
  stripped=$(strip_injection_marker "${SQUAD_INJECT_MARK}Supervisor escalate: done")
  [ "$stripped" = "Supervisor escalate: done" ] \
  ...
  # Only marker → empty.
  stripped=$(strip_injection_marker "$SQUAD_INJECT_MARK")
...
test_pane_input_pending_detects_partial_input() {
  ...
  printf 'line one\nline two\nhuman draft text\n' > "$capture"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=2 \
...
test_pane_input_pending_requires_proven_empty_prompt() {
  ...
  for prompt in '❯' '›'; do
    printf 'output\noutput\n%s \n' "$prompt" > "$capture"
    if PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=2 \
...
# The safety fix at the tmux classifier (task sq-composer-shellglyph-safety): a
...
test_tmux_composer_state_bare_shell_is_unknown() {
  ...
  for g in '$' '%' '#' '>'; do
    printf 'output\noutput\n%s \n' "$g" > "$capture"
    out=$(PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=2 \
...
test_tmux_composer_state_bordered_and_agent_rows_are_empty() {
  ...
  fakebin="$dir/fakebin"; capture="$dir/pane.txt"
  printf '╭────────────────────────╮\n│ >                      │\n╰────────────────────────╯\n' > "$capture"
  out=$(PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=1 \
    fm_tmux_composer_state "fakepane")
  [ "$out" = empty ] || fail "a bordered '│ > │' composer should read empty, got '$out'"
  printf '%s\n' "❯ " > "$capture"
  out=$(PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=0 \
    fm_tmux_composer_state "fakepane")
...
test_pane_input_pending_honors_idle_override_after_border_strip() {
  ...
  capture="$dir/pane.txt"
  printf '╭────────────────╮\n│ custom idle>   │\n╰────────────────╯\n' > "$capture"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=1 \
    SQUAD_COMPOSER_IDLE_RE='^custom idle>$' pane_input_pending "fakepane" \
    && fail "SQUAD_COMPOSER_IDLE_RE was not applied after border stripping"
  pass "pane_input_pending honors SQUAD_COMPOSER_IDLE_RE after border stripping"
}
...
test_classify_signal_dedup_against_scan() {
  ...
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_signal "$state/dup-s9.status" "$state")
...
test_classify_stale_dedup_against_signal() {
  ...
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_stale "sess:sq-dup-s10" "$state")
...
test_afk_nonterminal_working_merged_keeps_wedge_aging() {
  ...
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_stale "$win" "$state")
  case "$out" in
    self\|*transient*) ;;
...
test_afk_genuine_done_still_terminal_stale() {
  ...
  state="$dir/state"
  printf 'done: PR https://example.com/pull/76 checks green; stage 1 of 4 ready for Squad merge\n' \
    > "$state/stage1-w2.status"
  out=$(SQUAD_STATE_OVERRIDE="$state" classify_stale "sess:sq-stage1-w2" "$state")
...
test_pane_input_pending_bordered_idle_not_pending() {
  ...
  for line in '>' '❯' ''; do
    ...
    if PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=1 \
      pane_input_pending "fakepane"; then
...
test_pane_input_pending_bordered_with_text_is_pending() {
  ...
  # still read as pending so the daemon defers (and the commander-return race is
  # still protected).
  local dir state fakebin capture
...
test_submit_ack_confirms_on_bordered_empty_composer() {
  ...
  verdict=$(PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$dir/composer" SQUAD_FAKE_SENT="$sent" \
...
test_submit_ack_reports_pending_on_persistent_swallow() {
  ...
  # reported as "pending" — the daemon keeps the buffer, sq-send exits non-zero —
...
test_max_defer_empty_swallow_types_once_and_alarms() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$dir/composer" SQUAD_FAKE_SENT="$sent" \
    SQUAD_FAKE_SWALLOW="$dir/.swallow" SQUAD_FAKE_PERSIST_SWALLOW=1 SQUAD_INJECT_CONFIRM_SLEEP=0.05 \
    SQUAD_ESCALATE_BATCH_SECS=99999 SQUAD_MAX_DEFER_SECS=60 housekeeping "$state"
...
test_max_defer_flushes_empty_idle_pane() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$dir/composer" SQUAD_FAKE_SENT="$sent" \
    SQUAD_ESCALATE_BATCH_SECS=99999 SQUAD_MAX_DEFER_SECS=60 SQUAD_INJECT_CONFIRM_SLEEP=0.05 \
    housekeeping "$state"
...
test_normal_flush_clears_stale_wedge_marker() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$dir/composer" SQUAD_FAKE_SENT="$sent" \
    SQUAD_INJECT_CONFIRM_SLEEP=0.05 escalate_flush "$state" \
...
test_below_max_defer_does_nothing() {
  ...
  afk_enter "$state"
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_PANE_ALIVE=1 SQUAD_FAKE_TMUX_SENT="$sent" \
    SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_FAKE_TMUX_CURSOR_Y=0 \
    SQUAD_ESCALATE_BATCH_SECS=99999 SQUAD_MAX_DEFER_SECS=300 housekeeping "$state"
...
# ACTIVE alert channel must reach the commander even when the wedged pane and its
...
# the SQUAD_WEDGE_ALARM_EXEC seam, which tests/stand-to-helpers.sh forces to a recorder
# ($SQUAD_WEDGE_ALARM_LOG logs "<channel>\t<summary>"); the daemon also defaults
...
#!/usr/bin/env bash
printf '%s\n' "${SQUAD_FAKE_UNAME:-Darwin}"
SH
  # Fakes keep command discovery deterministic on any CI host.
  cat > "$fakebin/osascript" <<'SH'
#!/usr/bin/env bash
printf '%s\n' osascript >> "${SQUAD_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
  cat > "$fakebin/herdr" <<'SH'
#!/usr/bin/env bash
printf '%s\n' herdr >> "${SQUAD_WEDGE_ALARM_REAL_LOG:-/dev/null}"
exit 0
SH
...
test_wedge_alarm_library_mode_defaults_to_discard() {
  ...
  # SQUAD_WEDGE_ALARM_EXEC to "discard", so a sourced context (every test) cannot
  # fire a real notification even if it forgets to stub. Checked in a clean
  # subshell that first unsets this harness's recorder.
  local out
  # shellcheck disable=SC2016  # $1/$SQUAD_WEDGE_ALARM_EXEC must expand in the child, not here
  out=$(env -u SQUAD_WEDGE_ALARM_EXEC bash -c '. "$1"; printf "%s" "${SQUAD_WEDGE_ALARM_EXEC:-UNSET}"' _ "$DAEMON")
  [ "$out" = discard ] \
    || fail "sourcing the daemon did not default the notifier seam to discard (got: $out)"
  pass "library mode: sourcing the daemon defaults SQUAD_WEDGE_ALARM_EXEC to discard (no test can fire a real notification)"
}
...
test_wake_helpers_replace_inherited_notifier_override() {
  ...
  cat > "$unsafe" <<'SH'
#!/usr/bin/env bash
printf '%s\n' invoked >> "${SQUAD_WEDGE_ALARM_UNSAFE_LOG:?}"
SH
  chmod +x "$unsafe"
  SQUAD_WEDGE_ALARM_EXEC="$unsafe" SQUAD_WEDGE_ALARM_UNSAFE_LOG="$unsafe_log" \
    SQUAD_WEDGE_ALARM_LOG="$alert_log" SQUAD_WEDGE_ALARM_CHANNEL=osascript \
    bash -c '. "$1"; . "$2"; wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"' \
      _ "$ROOT/tests/stand-to-helpers.sh" "$DAEMON"
...
test_wedge_alarm_discard_seam_fires_nothing() {
  ...
  PATH="$dir/fakebin:$PATH" SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_WEDGE_ALARM_EXEC=discard \
    SQUAD_WEDGE_ALARM_CHANNEL=$'osascript\nherdr\n'"$channel" \
...
test_wedge_alarm_direct_notifiers_honor_discard_seam() {
  ...
  PATH="$dir/fakebin:$PATH" SQUAD_WEDGE_ALARM_REAL_LOG="$real_log" SQUAD_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_osascript "away-mode WEDGED 900s"
  PATH="$dir/fakebin:$PATH" SQUAD_WEDGE_ALARM_REAL_LOG="$real_log" SQUAD_WEDGE_ALARM_EXEC=discard \
    wedge_alarm_via_herdr "away-mode WEDGED 900s"
  SQUAD_WEDGE_ALARM_EXEC=discard wedge_alarm_via_command "$command" "away-mode WEDGED 900s"
...
test_wedge_alarm_osascript_channel_selected() {
  ...
  SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_WEDGE_ALARM_CHANNEL=osascript \
...
test_wedge_alarm_herdr_channel_selected() {
  ...
  SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_WEDGE_ALARM_CHANNEL=herdr \
...
test_wedge_alarm_command_channel_receives_summary() {
  ...
  SQUAD_WEDGE_ALARM_EXEC='' SQUAD_WEDGE_ALARM_CHANNEL="$chan" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  ...
  pass "command channel runs the commander command with the summary on \$1 and on stdin"
}
...
test_wedge_alarm_command_failure_hides_configured_command() {
  ...
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_EXEC='' SQUAD_WEDGE_ALARM_CHANNEL="command:exit 73 # $secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_unknown_channel_hides_configured_directive() {
  ...
  secret="https://alerts.example.invalid/hook?token=private-wedge-token"
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_CHANNEL="webhook:$secret" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_off_disables_active_alert_regardless_of_position() {
  ...
  for directives in $'osascript\noff' $'off\nosascript'; do
    : > "$log"
    SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_WEDGE_ALARM_CHANNEL="$directives" \
      wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_darwin_selects_osascript() {
  ...
  PATH="$dir/fakebin:$PATH" SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_FAKE_UNAME=Darwin SQUAD_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_auto_non_darwin_has_no_os_channel() {
  ...
  PATH="$dir/fakebin:$PATH" SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_FAKE_UNAME=Linux SQUAD_WEDGE_ALARM_CHANNEL=auto \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
test_wedge_alarm_config_file_multi_channel() {
  ...
  SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_CONFIG_OVERRIDE="$cfgdir" \
...
test_wedge_alarm_failing_channel_degrades_gracefully() {
  ...
  SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_WEDGE_ALARM_FAIL=osascript \
    SQUAD_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  rc=$?
...
test_wedge_alarm_hung_channel_times_out_and_falls_through() {
  ...
  start=$SECONDS
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_EXEC='' SQUAD_WEDGE_ALARM_TIMEOUT_SECS=1 \
    SQUAD_WEDGE_ALARM_CHANNEL=$'command:sleep 30\n'"$channel" \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
...
SH
  chmod +x "$blocker"
  start=$SECONDS
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_EXEC="$blocker" SQUAD_WEDGE_ALARM_TIMEOUT_SECS=1 \
    SQUAD_WEDGE_ALARM_CHANNEL=$'osascript\nherdr' \
    wedge_alarm_notify "away-mode WEDGED 900s" "/s/.marker"
  elapsed=$((SECONDS - start))
...
test_inject_wedge_alarm_fires_active_alert_on_non_tmux_backend() {
  ...
  SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_STATE_OVERRIDE="$state" \
    SQUAD_WEDGE_ALARM_CHANNEL=osascript SQUAD_SUPERVISOR_BACKEND=herdr \
...
test_inject_wedge_alarm_throttles_when_marker_cannot_be_written() {
  ...
  chmod u-w "$state"
  WEDGE_ALARM_LAST_EPOCH=0
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_MAX_DEFER_SECS=600 \
    SQUAD_WEDGE_ALARM_CHANNEL=osascript SQUAD_SUPERVISOR_BACKEND=herdr \
    inject_wedge_alarm "$state" 30600
  LOG="$daemon_log" SQUAD_WEDGE_ALARM_LOG="$log" SQUAD_MAX_DEFER_SECS=600 \
    SQUAD_WEDGE_ALARM_CHANNEL=osascript SQUAD_SUPERVISOR_BACKEND=herdr \
...
test_fm_send_exits_nonzero_on_confirmed_swallow() {
  # sq-send.sh must exit NON-ZERO when a steer's Enter is positively swallowed
  # (text left in the composer), so Squad learns the instruction did not land
...
test_fm_send_exits_nonzero_on_initial_send_failure() {
  ...
  grep -F 'text not sent' "$err" >/dev/null || fail "sq-send did not explain initial send failure: $(cat "$err")"
  pass "sq-send exits non-zero when initial text send fails"
}
...
test_fm_send_exits_nonzero_on_unproven_submit() {
  ...
  grep -F 'verdict=pending-unproven' "$err" >/dev/null \
    || fail "sq-send did not preserve the unproven-submit verdict: $(cat "$err")"
  pass "sq-send exits non-zero unless delivery is proven empty"
}

# --- herdr backend-awareness (sq-turnend-guard-h6-adjacent transport fix) ----
...
test_discover_supervisor_backend_precedence() {
  local out
  out=$(SQUAD_SUPERVISOR_BACKEND=herdr TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
  [ "$out" = herdr ] || fail "explicit SQUAD_SUPERVISOR_BACKEND override was not honored: $out"

  out=$(SQUAD_SUPERVISOR_BACKEND='' TMUX_PANE='%9' HERDR_ENV=1 HERDR_PANE_ID=w1:p1 discover_supervisor_backend)
...
test_discover_supervisor_target_herdr() {
  local out
  out=$(SQUAD_SUPERVISOR_TARGET=explicit:target TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = "explicit:target" ] || fail "explicit SQUAD_SUPERVISOR_TARGET override was not honored: $out"

  out=$(SQUAD_SUPERVISOR_TARGET='' TMUX_PANE='%3' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 discover_supervisor_target)
  [ "$out" = '%3' ] || fail "TMUX_PANE should win over herdr markers: $out"

  out=$(SQUAD_SUPERVISOR_TARGET='' TMUX_PANE='' HERDR_ENV=1 HERDR_PANE_ID=w1:p9 HERDR_SESSION='' discover_supervisor_target)
  ...
  pass "discover_supervisor_target: override > TMUX_PANE > herdr '<session>:<pane-id>' composition > Squad:0 fallback"
}
...
test_pane_is_busy_herdr_native_busy_state() {
  ...
  (
    ...
    SQUAD_STATE_OVERRIDE="$dir/state" SQUAD_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr \
...
test_primary_busy_guard_is_harness_scoped() {
  (
    ...
    if SQUAD_DAEMON_PRIMARY_HARNESS=claude pane_is_busy "default:w1:p2" herdr; then
      fail "OpenCode's rendered signature must not classify a Claude primary busy"
    fi
    SQUAD_DAEMON_PRIMARY_HARNESS=opencode pane_is_busy "default:w1:p2" herdr \
...
test_pane_is_busy_defaults_to_tmux_when_backend_omitted() {
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_CAPTURE="$capture" SQUAD_STATE_OVERRIDE="$dir/state" SQUAD_DAEMON_PRIMARY_HARNESS=grok pane_is_busy "fakepane" \
...
test_inject_msg_herdr_busy_guard_defers() {
  ...
  (
    ...
    if SQUAD_SUPERVISOR_BACKEND=herdr SQUAD_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_pane_gone_defers() {
  ...
  (
    ...
    if SQUAD_SUPERVISOR_BACKEND=herdr SQUAD_SUPERVISOR_TARGET="default:w1:gone" inject_msg "hello" "$state"; then
...
test_inject_msg_herdr_submits_through_backend_dispatch() {
  ...
  (
    ...
    SQUAD_SUPERVISOR_BACKEND=herdr SQUAD_SUPERVISOR_TARGET="default:w1:p2" inject_msg "hello" "$state" \
...
# Safety-critical (task sq-composer-shellglyph-safety): the away-mode injector
