...
description: Agent-only reference for Squad harness operations. Use before spawning or recovering an operator or XO, handling a trust dialog, sending a harness-specific skill invocation, interrupting or exiting an agent, resuming an exited agent, or verifying a new harness adapter. Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, and muse.
...
XOs have their own harness knob, so an XO can run on a different adapter than operators.
`config/xo-harness` is the harness the primary uses to launch XO agents, resolved through the fallback chain `config/xo-harness` -> `config/crew-harness` -> Squad's own.
An absent or `default` `config/xo-harness` therefore behaves exactly as the operator harness did before this knob existed (XOs launched on the operator harness); setting it splits the two.
The [`xo-provisioning` skill](../xo-provisioning/SKILL.md) owns the complete inherited-local-material allowlist and propagation contract.
This skill owns only the harness-relevant consequence: an XO's own operators use the primary's inherited dispatch profiles and static harness value, while `config/xo-harness` is the primary's own setting and is never inherited - XOs do not spawn XOs.
Inheritance copies the literal `config/crew-harness` file, so for an XO's own operators to run on the primary's operator harness the commander must set `config/crew-harness` to a concrete adapter name, such as `codex`.
If `config/crew-harness` is unset or `default`, there is no concrete value to inherit, so the XO's own operators fall back to the XO's own/detected harness rather than the primary's effective operator harness.
Inheritance also copies the literal `config/crew-dispatch.json` file, so XOs apply the same best-fit profile rules for their own operators.

Each adapter splits into mechanics and knowledge.
The per-task mechanics, including launch command, autonomy flag, and any enabled operator turn-end hook, live in `bin/sq-spawn.sh`.
The primary-session "no turn ends blind" guard contract and harness hook installation paths live in `docs/turnend-guard.md`.
The primary-session sentry wake protocols are rendered from `docs/supervision-protocols/` by `bin/sq-supervision-instructions.sh`.
...
Each adapter's `Busy state` row names only which semantic source that harness uses; `bin/sq-busy-lib.sh` owns the contract itself, including verdicts, source attribution, and the verification gates that keep an unverified harness at unknown.

Never dispatch an operator or XO on an unverified adapter.
If `config/crew-harness` or `config/xo-harness` names an unverified adapter, tell the commander under `AGENTS.md` section 9 that the requested worker runtime is not verified yet, use Squad's own verified runtime for current work, and ask only whether to verify the requested runtime before future use.
Do not pause current work for that future-verification choice, and never launch an unverified adapter.
If the commander asks for a new harness, propose verifying it first: spawn a trivial supervised task using `sq-spawn`'s raw-launch-command escape hatch, confirm every fact empirically, then record the mechanics in `sq-spawn`, its semantic busy source and trust gate in `bin/sq-busy-lib.sh`, any needed `SQUAD_COMPOSER_IDLE_RE` empty-composer override plus any novel bare agent prompt glyph in `bin/sq-composer-lib.sh`'s shared composer classifier (the one unit-wide owner of the empty/dead-shell/pending decision, so a new harness's own idle composer is not misread as a dead shell), the tmux agent-process liveness classification in `bin/backends/tmux.sh` when the harness can launch an XO, and the verified knowledge here.

## Detection

`bin/sq-harness.sh` prints Squad's own harness, using verified env markers first and then process ancestry.
Within the Pi family, only the exact launch-boundary marker `SQUAD_PI_HARNESS=pi-signed` alongside `PI_CODING_AGENT=true` selects the signed identity; unmarked shared launcher ancestry remains `pi`.
`bin/sq-harness.sh crew` resolves the effective operator harness from `config/crew-harness` (absent or `default` -> own).
`bin/sq-harness.sh XO` resolves the XO-launch harness through the chain `config/xo-harness` -> `config/crew-harness` -> own, so an unset `config/xo-harness` matches the operator harness.
`bin/sq-spawn.sh` uses `crew` mode for an operator/recon launch and `XO` mode for a `--xo` launch, re-resolving on every spawn so the split is durable across respawns; an explicit per-spawn harness arg overrides either.
On `unknown`, ask the commander instead of guessing.
A commander override always beats detection.
When verifying a new adapter, record its env marker and command name in `bin/sq-harness.sh`.
...
`claude` and `codex` block directly through Stop hooks that preserve exit status 2 and stderr from `bin/sq-turnend-guard.sh`.
...
muse is OPERATOR/RECON ONLY and has no primary integration at all: its plugin engine (its only hook surface) is disabled in the default build, and its Claude-compatible hook dialect names `asyncRewake` and model reawakening as explicitly unsupported, which is exactly what a Squad primary's turn-end supervision needs.
...
The primary integrations for `claude`, `codex`, `opencode`, `pi`, `pi-signed`, and `grok` also have wired PreToolUse-equivalent hooks that deny a sentry-arm anti-pattern (shell `&`, truncating pipe, bundling, broad `pkill -f sq-sentry`) before it runs.
...
When changing any sentry-arm PreToolUse hook, validate the real harness behavior in a scratch project before trusting it, then update that doc.
...
## Primary sentry supervision

At session start, `bin/sq-session-start.sh` prints exactly one sentry supervision block for the detected primary harness.
Do not substitute another harness's wait shape when resuming supervision.
Claude's Stop `asyncRewake` hook (`bin/sq-claude-stop-autoarm.sh`) owns tokenless re-arm around `bin/sq-sentry-arm.sh`, and Grok uses tracked background-notify cycles around `bin/sq-sentry-arm.sh`.
Codex uses bounded foreground checkpoints through `bin/sq-sentry-checkpoint.sh` because Codex cannot reason while a foreground tool call is running.
OpenCode uses `.opencode/plugins/sq-primary-sentry-arm.js`, which coordinates with the turn-end guard plugin and wakes the TUI with `client.session.promptAsync`.
Pi and pi-signed use the tracked `.pi/extensions/sq-primary-turnend-guard.ts` plus the tracked `.pi/extensions/sq-primary-pi-watch.ts`, both project-local extensions the Pi engine auto-discovers once trusted.
When changing any primary sentry adapter, update `docs/supervision-protocols/`, `docs/turnend-guard.md` if a shared idle or turn-end hook changed, and the relevant concise fact below.
...
Use the fallback only when neither the commander nor applicable standing configuration specifies effort.
...
Never select `max` from this fallback; use it only when the commander has explicitly expressed that per-task or standing preference.
...
| grok | `--model <model>` | `--reasoning-effort <low\|medium\|high>` | Verified on grok 0.2.99 (2026-07-13). `--effort` is an alias, but Squad's profile axis is reasoning effort. As of 0.2.99 the ceiling is `high`; both `xhigh` and `max` are rejected with `use one of: high, medium, low`, so Squad omits them. |
| pi / pi-signed | `--model <model>` | `--thinking <low\|medium\|high\|xhigh\|max>` | Verified 2026-07-27 on Pi and pi-signed 0.82.0. Both expose the same accepted thinking levels and completed the same model-qualified max-thinking smoke. |
| opencode | `--model <provider/model>` | none for Squad's interactive launch | Verified on opencode 1.17.6. `opencode run` has `--variant`, but Squad launches the interactive `opencode --prompt` path, which has no verified effort flag. |
| kimi | `--model <model>` | none | Verified 2026-07-25 on Kimi Code CLI 0.29.1. |
| muse | `--model <model>` | `--reasoning-effort <low\|medium\|high\|xhigh>`, and `ultra` only for an explicit `max` | Verified 2026-08-05 on Muse Code 0.1.0-R708.1. The flag accepts `none\|minimal\|low\|medium\|high\|xhigh\|ultra` and defaults to `high`. `ultra` is muse's max-class level, so it is reachable only through an explicit commander `max`, never from the generic fallback; `none` and `minimal` sit below the shared vocabulary and stay unreachable. |
...
## drill skill invocation

Send the validation skill using the target harness's skill invocation form.
Natural language is acceptable if uncertain.

- claude: `/<skill>`, for example `/drill`.
- codex: `$<skill>`, for example `$drill`; `/<skill>` is claude-only and codex rejects it as "Unrecognized command".
...
- grok: `/<skill>`, for example `/drill` (same form as claude). Verified end to end: grok discovers the user-level `drill` skill, `/drill` invokes it, and grok drives a real `drill axi run`. Like codex's `$`/`/` popups, typing `/<skill>` opens grok's slash-autocomplete, so a too-fast Enter selects the popup entry instead of sending, and for an argument-taking command (like `/drill`'s optional task-first argument) that first Enter only expands the popup selection into an argument-hint placeholder rather than submitting - a genuine second Enter is required (see the grok section below for the 2026-07-03 incident and fix). `fm_tmux_submit_core`'s retried Enter (used by `sq-send` on the tmux backend) handles this through the structural composer reader; the herdr backend needed a dedicated fix (`fm_backend_herdr_composer_state`, docs/herdr-backend.md) because its prior delta-based verification false-positived on that same popup-close content change.
- kimi: `/<skill>`, for example `/drill`.
...
| Fact | Value |
|---|---|
| Busy state | Owned lifecycle hooks: `UserPromptSubmit` opens a turn, `Stop`, `StopFailure`, and `SessionEnd` close it. Claude fires no hook for a manual interrupt, so a Squad-initiated interrupt must record the clear itself. |
| Exit command | `/exit` |
| Interrupt | single Escape |
| Skill invocation | `/<skill>` (e.g. `/drill`) |
...
Squad launches every claude operator and XO with `CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false`, scoped to Squad-launched agents through `bin/sq-spawn.sh`, so it never touches the commander's global config.
...
As defense in depth for any pane that flag cannot reach, including the commander's own Squad composer that away-mode reads, the shared `fm_composer_strip_ghost` extractor in `bin/sq-composer-lib.sh` removes dim/faint SGR 2 ghost runs before pending-input classification on both ANSI-capable readers (tmux and herdr).
...
`sq-peek` and every other human or LLM-facing capture path stays plain `tmux capture-pane` with no escape codes.
...
This is separate from the per-task operator turn-end hook above (that one just `touch`es a marker file in a task's own `.claude/settings.local.json`).
The Squad PRIMARY's own `.claude/settings.json` registers two Stop hooks: `bin/sq-turnend-guard.sh --claude` and the Stop-owned auto-arm `bin/sq-claude-stop-autoarm.sh` (`asyncRewake: true`, `timeout: 28800`), and exiting the guard with status 2 plus stderr reliably forces the model to continue.
...
| Fact | Value |
|---|---|
| Busy state | Unknown until a semantic source is live-verified: the app-server turn lifecycle is unreachable for a pane worker, and project lifecycle hooks did not fire for a Squad-launched worker. |
| Exit command | `/quit` (slash popup needs about 1 second between text and Enter; `sq-send` handles it) |
| Interrupt | single Escape |
| Skill invocation | `$<skill>` (e.g. `$drill`); `/<skill>` is claude-only and codex rejects it as "Unrecognized command" |
...
`sq-send` handles it the same way it handles `/` - it gives the popup a longer settle (1.2s) between typing and the first Enter, with the target backend's submit retry as the safety net - but the `$` settle is scoped to `harness=codex`, read from the target metadata for exact task ids or legacy `sq-<id>` labels.
...
This is why the validation trigger (`$drill`) to a codex crew now lands on the first Enter instead of biting the popup.
...
The Squad PRIMARY's own `.codex/hooks.json` registers a Stop hook that pipes Codex's Stop payload to `bin/sq-turnend-guard.sh`.
...
The tracked hook anchors to `pwd -P`, verifies that root is Squad-shaped and hook-bearing, and then invokes `bin/sq-turnend-guard.sh` with the original payload.
Codex's primary sentry protocol is `bin/sq-sentry-checkpoint.sh --seconds "${SQUAD_CODEX_WATCH_CHECKPOINT:-180}"`, not `bin/sq-sentry-arm.sh`.
...
| Fact | Value |
|---|---|
| Busy state | The Squad-owned plugin's semantic `session.status`: `busy` and `retry` are active, `idle` is inactive, latched to the worker's own session. |
| Exit command | `/exit` |
...
`--prompt` does not auto-submit alongside `--continue`, so send the next instruction via `sq-send` once the TUI is up.
...
Without a fix, every `sq-send` to a busy opencode pane exits non-zero on a
false "Enter swallowed", and every daemon escalation that lands while the
primary is mid-turn is treated as wedged.
The shared `fm_tmux_submit_enter_core` (`bin/sq-tmux-lib.sh`) now falls back
...
Regression coverage: `tests/sq-tmux-submit-busy.test.sh` covers the four
...
The Squad PRIMARY's own `.opencode/plugins/sq-primary-turnend-guard.js` listens for `session.idle`.
Throwing from `session.idle` does not block `opencode run`, so the primary adapter treats the event as passive and uses `client.session.promptAsync` to force one follow-up turn when `bin/sq-turnend-guard.sh` returns 2.
The companion `.opencode/plugins/sq-primary-sentry-arm.js` owns normal TUI sentry wake supervision and coordinates with the guard plugin before the guard tries a blind-turn follow-up.
...
Pi has no permission system, so operators are always autonomous.
`pi-signed` is the signed wrapper identity verified on version 0.82.0 and exposes the same CLI and TUI behavior as Pi.
Squad launches the selected executable name from `PATH`, records `pi-signed` without normalization, and refuses rather than falling back to `pi` when that wrapper is unavailable.
...
The installed plain `pi` command also execs that signed launcher, so `SQUAD_PI_HARNESS=pi-signed` is the authoritative selection marker and shared unmarked ancestry remains `pi`.
Squad sets `SQUAD_PI_HARNESS` explicitly for both worker launch identities, and a signed primary uses the README launch command to establish the same boundary.
Keep the brief as one positional argument.
Multiple positional args become separate queued messages; `sq-spawn`'s template already does this correctly.
...
`sq-spawn` keeps the turn-end extension in `state/`, outside the worktree, because project-local extension files make the trust gate strictly worse and pollute the project.
The extension must listen for pi's `turn_end` event, not `agent_end`, so the sentry wakes after each completed turn instead of only when the whole agent run exits.
...
| Skill invocation | `/<skill>` (e.g. `/drill`), same as claude. Opens a slash-autocomplete popup, so a too-fast Enter selects the popup entry instead of sending. For an argument-taking command that first Enter does not submit at all - it expands the selection into an argument-hint placeholder in the composer (e.g. `/compact` -> `/compact compaction instructions`, live-verified), leaving real text still sitting there unsubmitted; a genuine second Enter is required. `sq-send`'s retried Enter lands it on BOTH backends, but only because each backend's own submit-verification correctly recognizes that placeholder-filled text as still-pending - see the incident below. |
...
**Incident (2026-07-03, herdr backend only, grok 0.2.82):** two grok/herdr operators were sent `/drill` via `sq-send`; both left it fully typed but unsubmitted in the composer for minutes (footer still `Enter:send`), and `sq-send` exited 0 with no error.
...
The Herdr adapter (`fm_backend_herdr_composer_state`, `bin/backends/herdr.sh`) classifies the composer's own row structurally instead of diffing raw content; see `docs/herdr-backend.md` "Composer and injection safety" for the current boundary and `tests/sq-backend-herdr.test.sh` for regression coverage.
...
`sq-spawn` launches inside the fob worktree (a git repo root), so the picker never appears and grok treats the worktree as a trusted project automatically - no post-launch keystroke is needed.
...
The shared ANSI-aware owner `fm_composer_strip_ghost` (`bin/sq-composer-lib.sh`) now drops a dark/muted truecolor foreground (perceived luminance below `SQUAD_COMPOSER_GHOST_LUMA_MAX`, default 128) as well as dim/faint, so the placeholder is stripped and the row reads empty on both ANSI-capable backends (tmux and herdr route through the same owner).
...
This assumes a dark terminal theme, the unit reality; the SGR-2 signal stays theme-independent.
Regression coverage: `tests/sq-composer-ghost.test.sh` (`test_strip_ghost_drops_dark_truecolor_ghost`, `test_dark_truecolor_ghost_only_composer_is_not_pending`) and `tests/sq-backend-herdr.test.sh` (`test_composer_state_grok_dark_truecolor_placeholder_is_empty`, `test_composer_state_grok_bright_truecolor_real_text_is_pending`).
...
Turn-end hook: grok fires a `Stop` hook at every turn boundary, giving Squad a precise per-turn wake instead of only stale-pane detection.
grok loads PROJECT hooks (`<worktree>/.grok/hooks/`, `<worktree>/.claude/settings.local.json`) only after the folder is granted hook-trust in `~/.grok/trusted_folders.toml`, which is not automatic and which Squad will not establish by editing grok's own managed trust store.
GLOBAL hooks in `~/.grok/hooks/` are always trusted and load on first launch.
So `sq-spawn` installs ONE Squad-owned global hook, `~/.grok/hooks/sq-turn-end.json`, plus the companion `~/.grok/hooks/sq-turn-end.sh`, guarded as a no-op for every non-Squad grok session.
Its `Stop` command fires only when the current workspace holds a `.sq-grok-turnend` token pointer that matches the Squad-owned hook registry under `~/.grok/hooks/sq-turn-end.d/`.
`sq-spawn` writes that per-task pointer (`<worktree>/.sq-grok-turnend`, gitignored via git info/exclude like the other harnesses' worktree hook files) and a matching registry entry naming this task's `state/<id>.turn-ended`.
The hook reads `$GROK_WORKSPACE_ROOT`, which is always set for hooks and equals the worktree.
This keeps the hook outside the worktree, needs no trust grant, and writes only Squad-owned files.
`sq-teardown` removes the worktree pointer before returning a pooled worktree.
XO spawns skip the pointer (idle panes are healthy, no stale-pane detection for them).

**Primary-session guard fact (verified 2026-07-28, Grok 0.2.112 and 0.2.73).**
The Squad PRIMARY's own `.grok/hooks/sq-primary-turnend-guard.json` invokes `bin/sq-turnend-guard-grok.sh`.
...
Project-local Grok hooks require folder trust, verified with launch-time `--trust`; if the primary Squad checkout is not trusted for Grok hooks, this primary guard fails open and `sq-guard.sh` remains the next-command alarm.
Grok's primary sentry protocol remains background-notify around `bin/sq-sentry-arm.sh`; native Stop continuation does not provide Pi-like extension ownership.
...
| Skill invocation | `/<skill>`, for example `/drill`; Squad skills are discovered. |
...
`sq-spawn.sh` launches Kimi bare, waits for the composer box or `Welcome to Kimi Code!`, sends only `Read the brief at <absolute-path> and follow it exactly.`, and requires a cleared composer plus either the echoed `✨` submission or nonzero context before accepting delivery.
...
[`docs/turnend-guard.md`](../../../docs/turnend-guard.md) owns Kimi's verified global hook surface and commander-approved crew wake integration.
`sq-spawn.sh` installs one marker-delimited Squad entry in `$HOME/.kimi-code/config.toml`, one silent always-zero hook script, and one private token registry under `$HOME/.kimi-code/sq-turn-end.d/`.
Each Kimi crew worktree receives a gitignored `.sq-kimi-turnend` token pointer, and the global hook touches that task's `state/<id>.turn-ended` only when the Stop payload's `cwd`, pointer, and registry entry all agree.
...
Muse Code is a OPERATOR and RECON adapter only.
`bin/sq-spawn.sh` refuses `--xo` on muse, and muse has no supervision protocol under `docs/supervision-protocols/`, so a Squad primary detected as muse falls back to the `unknown` protocol.
...
| Busy state | Its own durable session event log, folded on demand by `bin/sq-busy-lib.sh`. There is no hook or plugin writer, so nothing is armed and no busy record is ever seeded. |
...
| Interrupt | Single Escape. It closes the run with `terminal: cancelled` AND restores the interrupted prompt into the composer as real bright text, so `sq-send.sh` follows Escape with `C-u` to clear it. |
...
| Trust dialog | `Do you trust this workspace?` with `1 Trust and continue` preselected, accepted by Enter. `--yolo` suppresses it entirely, which is what Squad relies on because every task gets a fresh worktree path. |
...
| Composer | Bordered box whose prompt glyph is `⟩` (U+27E9) in truecolor `38;2;90;160;255`, luminance ~149.9 - the narrowest margin over the 128 ghost threshold in the unit. Typed text is `38;2;204;211;219` (~209.8). No idle placeholder or ghost text was observed. |
...
`bin/sq-spawn.sh` accepts `META_API_KEY` only when it can prove the backend worker already has it, because a command-scoped caller variable does not cross a long-lived backend daemon and the secret must never enter launch argv.
The supported unit path is the stored credential, and `sq-spawn` resolves the non-secret `XDG_CONFIG_HOME` and `XDG_DATA_HOME` roots to absolute paths before preflight and forwarding to keep authentication and session-log binding aligned with the worker.
`bin/sq-spawn.sh` refuses the launch when neither worker-reachable path is present, because an unauthenticated pane does NOT exit: it sits on `Sign in at this page: https://auth.meta.com/oauth/device/?code=XXXX-XXXX` / `Waiting for approval…` indefinitely, which supervision would read as a wedged worker rather than a missing credential.
Escalate that refusal to the commander as a needed credential.
...
The control that reaches a pane worker is `MUSE_EXPERIMENTAL_FOREIGN_PERSONAL_CONTEXT_KILL=on`, which `sq-spawn` sets on every muse launch.
It was verified to drop the foreign `rules_file` context block while KEEPING a project's own `AGENTS.md` rules, which the operator contract depends on.

### Session event log and the busy fold

Sessions persist to `${XDG_DATA_HOME:-$HOME/.local/share}/muse/sessions/YYYY/MM/DD/<session-uuid>/session.jsonl`, and `sq-spawn` writes `state/<id>.muse-session` pinning that root, the task worktree, its binding incarnation, and every pre-existing matching main log so the classifier binds a pane to its one new log.
...
Never use `--no-session-log` for an operator: it disables the only busy source muse has.
...
The recorded sessions root is the resolved `XDG_DATA_HOME` that `sq-spawn` also forwards to the worker launch, so the binding and pane remain aligned across a long-lived backend daemon.
...
Squad deliberately does NOT exclude any muse path from `sq-teardown.sh`'s uncommitted-work check.
Squad writes `.claude/settings.local.json` itself, which is why that path is excluded for claude; it does not write muse's, so a nested muse worktree or leftover scratch is the agent's own work product and MUST be able to refuse teardown.
A teardown refusal naming muse scratch is therefore correct behavior: inspect it rather than forcing past it.

### Maturity caveats

muse is a day-0 `0.1.0` beta whose launcher polls a release channel hourly and can replace the running binary underneath the unit, changing the process name with it.
The commander accepted that risk, so Squad does NOT set `MUSE_NO_AUTO_UPDATE=1`; a unit that later wants stability can set it in the launch environment without any adapter change.
