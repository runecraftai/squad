...
- **Floors reset to 0.1.0:** `GH_AXI_MIN=0.1.0`, `LAVISH_AXI_MIN=0.1.0` (sq-bootstrap.sh), `SQUAD_QUOTA_AXI_MIN=0.1.0` (sq-quota-lib.sh), `SQUAD_TASKS_AXI_MIN=0.1.0` (sq-tasks-lib.sh). The planned bump to vendored versions (OQ-M6-03 (a)) and the "durable 0.2.4" SQUAD_TASKS_AXI_MIN claim are superseded by the PR #16 reset; the sq-tasks `vendor.json` note "SQUAD_TASKS_AXI_MIN stays 0.2.4" is stale and belongs to the rebrand item or a follow-up sweep.
- **no-mistakes renamed to drill (PR #8, 2026-08-10):** `packages/no-mistakes` → `packages/drill`, `.no-mistakes.yaml` → `.drill.yaml`, `bin/sq-nm-run-lib.sh` → `bin/sq-drill-run-lib.sh`, `no-mistakes-required.yml` → `drill-required.yml`; `COMMON_TOOLS="node git gh drill sq-gh sq-browser sq-report sq-quota sq-tasks"`; `DRILL_MIN=1.31.2`.
