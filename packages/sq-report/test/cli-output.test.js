...
test("top-level help renders static home output without dynamic sessions", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-help-test-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "--help"],
      {
...
test("home directory collapse tolerates Windows mixed separators", () => {
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin/.local/bin/sq-report", "C:\\Users\\runneradmin"),
    "~/.local/bin/sq-report",
  );
  assert.equal(
    collapseHomeDirectory("C:\\Users\\runneradmin\\.local\\bin\\sq-report", "C:\\Users\\runneradmin"),
...
test("export command writes a portable HTML file next to the artifact", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/sq-report-export-test-`);
...
test("share command publishes the artifact to ht-ml.app and returns the public url", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/sq-report-share-test-`);
  const artifact = `${dir}/report.html`;
  ...
  try {
    ...
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "share", "--password", "pw", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
...
test("share command treats a whitespace-only password as public", async () => {
  ...
  try {
    const child = spawn(
      process.execPath,
      [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "share", "--password", "   ", artifact],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        env: {
          ...process.env,
...
test("spawned poll with piped stderr banners once and leaves re-run guidance when killed", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-poll-wait-test-`);
...
test("setup hooks creates a Copilot CLI hook that injects additional context", () => {
  ...
  assert.match(updated.hooks.sessionStart[1].bash, /sq-report/);
...
test("Copilot CLI ambient context script wraps lavish output as hook JSON", async () => {
  const tempDir = await mkdtemp(`${os.tmpdir()}/sq-report-copilot-hook-`);
  try {
    ...
    assert.match(output.additionalContext, /## AXI ambient context: sq-report/);
...
test("setup hooks installs agent session hooks explicitly", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "setup", "hooks"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
    ...
    assert.ok(existsSync(`${homeDir}/.copilot/hooks/sq-report.json`));

    const copilotHook = JSON.parse(await readFile(`${homeDir}/.copilot/hooks/sq-report.json`, "utf8"));
...
test("setup hooks exits with an error when hook installation fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-fail-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-fail-home-`);
  try {
...
function runSetupPlugin(homeDir, stateDir, pathDir) {
  return spawnSync(
    process.execPath,
    [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "setup", "plugin"],
    {
      cwd: fileURLToPath(new URL("..", import.meta.url)),
      encoding: "utf8",
...
test("setup plugin registers the installed package in the clients that are present", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-path-`);
  try {
...
test("setup plugin registers VS Code without disturbing existing settings", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-path-`);
...
test("setup plugin creates VS Code settings for a fresh installation", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-fresh-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-fresh-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-vs-fresh-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
  try {
    await mkdir(path.dirname(settingsFile), { recursive: true });
...
test("setup plugin leaves unparseable VS Code settings alone", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-jsonc-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-jsonc-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-jsonc-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
...
test("setup plugin repairs Copilot registration without trusting list text", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-path-`);
...
test("setup plugin preserves Copilot registration when replacement fails", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-failure-state-`);
...
test("setup plugin does not install when Copilot records are invalid", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-invalid-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-invalid-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-copilot-invalid-path-`);
  const installLog = path.join(homeDir, "copilot-install-log");
  try {
...
test("setup plugin isolates a client it cannot register from the ones it can", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-iso-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-iso-home-`);
  const pathDir = await mkdtemp(`${os.tmpdir()}/sq-report-plugin-iso-path-`);
  const settingsFile = resolveVsCodeSettingsFile({}, homeDir);
  try {
...
test("setup rejects an unknown action and names both supported ones", async () => {
  const stateDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-unknown-state-`);
  const homeDir = await mkdtemp(`${os.tmpdir()}/sq-report-setup-unknown-home-`);
  try {
    const result = spawnSync(
      process.execPath,
      [fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)), "setup", "everything"],
      {
        cwd: fileURLToPath(new URL("..", import.meta.url)),
        encoding: "utf8",
        env: setupHooksEnv(homeDir, stateDir),
...
test("server entry resolves to a node-executable script that actually invokes run()", () => {
  // Running from source, the entry must be `bin/sq-report.js` (the only file in the
  ...
  // From source: bin/sq-report.js is present and preferred.
  assert.equal(entry, fileURLToPath(new URL("../bin/sq-report.js", import.meta.url)));
});
...
test("local built CLI opens force a server restart while source and installed runs do not", () => {
  ...
  assert.equal(shouldForceRestartForLocalBuild(`${root}/bin/sq-report.js`, true), false);
  assert.equal(shouldForceRestartForLocalBuild("/usr/local/lib/node_modules/sq-report/dist/cli.mjs", false), false);
});
...
test("shouldRestartServer restarts same-version Lavish servers when forced", () => {
  assert.equal(shouldRestartServer("0.1.4", { ok: true, app: "sq-report", version: "0.1.4" }, true), true);
...
test("shouldKillProcessOnPort only kills Lavish servers with a mismatched version", () => {
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "sq-report", version: "0.1.3" }), true);
  assert.equal(shouldKillProcessOnPort("0.1.4", { ok: true, app: "sq-report", version: "0.1.4" }), false);
});
...
test("stop command shuts down the running server on the configured port", async () => {
  const dir = await mkdtemp(`${os.tmpdir()}/sq-report-stop-test-`);
