...
TMP_ROOT=$(fm_test_tmproot sq-guard-stale-banner)
...
make_guard_case() {
  ...
  fm_write_meta "$home/state/task.meta" "window=Squad:sq-task" "kind=strike"
  printf '%s\n' "$dir"
}
...
record_live_sentry() {
  ...
  home=$(case_home "$dir")
  identity=$(SQUAD_STATE_OVERRIDE="$home/state" bash -c '. "$1"; fm_pid_identity "$2"' _ "$ROOT/bin/sq-stand-to-lib.sh" "$pid") || return 1
  mkdir -p "$home/state/.sentry.lock"
  printf '%s\n' "$pid" > "$home/state/.sentry.lock/pid"
...
# These cases exercise the persistent-sentry model (a live pid is the real
...
# The Claude Stop auto-arm model: the sentry runs only between turns, so a fresh
# beacon with no live sentry process is the healthy mid-turn state.
...
test_first_stale_call_prints_full_banner() {
  ...
  assert_contains "$out" "Trust the emitted supervision protocol" \
    "full banner must keep the actionable sentry-repair instruction"
  assert_contains "$out" "WILL still run" \
    "full banner must keep the guarded-operation continuation line"
  pass "sq-guard stale banner: first stale call prints the full actionable banner"
}
...
test_repeated_same_episode_prints_reminder_only() {
  ...
  marker="$(case_home "$dir")/state/.guard-sentry-stale-banner"
  ...
  pass "sq-guard stale banner: repeated same-episode calls print a concise reminder only"
}

test_fresh_beacon_without_live_sentry_stays_alarm() {
  local dir out
  dir=$(make_guard_case fresh-no-live)
  touch "$(case_home "$dir")/state/.last-sentry-beat"
  out=$(run_guard_case "$dir")
  [ "$(count_text "$out" "WATCHER DOWN - SUPERVISION IS OFF")" -eq 1 ] \
    || fail "a fresh leftover beacon without a live sentry must still alarm: $out"
  pass "sq-guard stale banner: a fresh beacon without a live sentry remains unhealthy"
}

test_x_mode_without_live_sentry_stays_alarm() {
  local dir home out
  dir=$(make_guard_case x-mode-no-live)
  home=$(case_home "$dir")
  rm -f "$home/state/task.meta"
  : > "$home/state/x-sentry.check.sh"
  out=$(run_guard_case "$dir")
  assert_contains "$out" "X-mode relay polling needs supervision" "X-mode-only need must remain guarded"
  pass "sq-guard stale banner: X-mode polling without a live sentry remains unhealthy"
}
...
test_healthy_recovery_rearms_next_stale_episode() {
  ...
  sleep 60 &
  pid=$!
  record_live_sentry "$dir" "$pid" || fail "could not record the live sentry for recovery"
  touch "$home/state/.last-sentry-beat"
  healthy=$(run_guard_case "$dir")
  kill "$pid" 2>/dev/null || true
  wait "$pid" 2>/dev/null || true
  [ -z "$healthy" ] || fail "guard should be silent after sentry recovery, got: $healthy"
  assert_absent "$home/state/.guard-sentry-stale-banner" \
    "healthy recovery must clear the stale-banner marker"

  rm -f "$home/state/.last-sentry-beat"
  rm -rf "$home/state/.sentry.lock"
  out2=$(run_guard_case "$dir")
  [ "$(count_text "$out2" "WATCHER DOWN - SUPERVISION IS OFF")" -eq 1 ] \
    || fail "second stale episode did not re-print the full banner: $out2"
  pass "sq-guard stale banner: healthy recovery rearms the next stale episode"
}
...
test_concurrent_same_episode_prints_one_full_banner() {
  ...
  pass "sq-guard stale banner: concurrent same-episode calls claim exactly one full banner"
}
...
test_queued_wake_warning_stays_independent() {
  ...
  pass "sq-guard stale banner: queued-wake warning remains independent"
}
...
test_read_only_before_writable_does_not_consume_full_banner() {
  ...
  home=$(case_home "$dir")
  marker="$home/state/.guard-sentry-stale-banner"
  lock="$home/state/.guard-sentry-stale-banner.lock"
  ...
  pass "sq-guard stale banner: read-only before writable does not consume full banner"
}
...
test_read_only_during_episode_observes_without_mutating_marker() {
  ...
  pass "sq-guard stale banner: read-only during episode observes without mutating marker"
}
...
test_healthy_read_only_does_not_clear_marker() {
  ...
  sleep 60 &
  pid=$!
  record_live_sentry "$dir" "$pid" || fail "could not record the live sentry for read-only recovery"
  touch "$home/state/.last-sentry-beat"
  ...
  pass "sq-guard stale banner: healthy read-only does not clear marker"
}
...
test_read_only_never_mutates_stale_banner_state_files() {
  ...
  before=$(find "$home/state" -maxdepth 1 -mindepth 1 -name '.guard-sentry-stale-banner*' -print | sort)
  run_guard_case_read_only "$dir" >/dev/null
  after=$(find "$home/state" -maxdepth 1 -mindepth 1 -name '.guard-sentry-stale-banner*' -print | sort)
  ...
  pass "sq-guard stale banner: read-only never mutates stale-banner state files"
}

test_autoarm_fresh_beacon_without_sentry_is_healthy() {
  local dir out
  dir=$(make_guard_case autoarm-fresh)
  # A fresh beacon and NO live sentry: the healthy mid-turn state under the
  # Claude Stop auto-arm model, where the sentry only runs between turns.
  touch "$(case_home "$dir")/state/.last-sentry-beat"
  out=$(run_guard_case_autoarm "$dir")
  [ -z "$out" ] \
    || fail "auto-arm model with a fresh beacon and no live sentry must stay silent, got: $out"
  pass "sq-guard stale banner: auto-arm fresh beacon without a live sentry is healthy"
}
...
test_autoarm_stale_beacon_alarms_with_correct_reason() {
  ...
  assert_contains "$out" "no sentry has a fresh beacon" \
    "auto-arm stale-beacon banner must name the stale-beacon reason"
  pass "sq-guard stale banner: auto-arm stale beacon alarms with the true reason"
}
...
test_autoarm_stale_episode_is_stable() {
  ...
  pass "sq-guard stale banner: auto-arm stale episode stays one episode across calls"
}

test_persistent_no_sentry_banner_names_missing_process() {
  local dir out
  dir=$(make_guard_case persistent-no-sentry-reason)
  # A fresh beacon with no live sentry under the persistent model: the real
  # failing condition is the missing process, not a stale beacon.
  touch "$(case_home "$dir")/state/.last-sentry-beat"
  out=$(run_guard_case "$dir")
  assert_contains "$out" "no live sentry process holds this home lock" \
    "persistent no-sentry banner must name the missing sentry process"
  assert_not_contains "$out" "no sentry has a fresh beacon" \
    "persistent no-sentry banner must not blame the fresh beacon"
  pass "sq-guard stale banner: persistent no-sentry banner names the true reason"
}

test_persistent_no_sentry_episode_survives_beacon_touch() {
  local dir home out1 out2
  dir=$(make_guard_case persistent-no-sentry-episode)
  home=$(case_home "$dir")
  touch "$home/state/.last-sentry-beat"
  out1=$(run_guard_case "$dir")
  [ "$(count_text "$out1" "WATCHER DOWN - SUPERVISION IS OFF")" -eq 1 ] \
    || fail "first persistent no-sentry call did not print the full banner: $out1"
  # The beacon mtime advancing with NO live sentry must not split the continuous
  ...
  out2=$(run_guard_case "$dir")
  [ "$(count_text "$out2" "WATCHER DOWN - SUPERVISION IS OFF")" -eq 0 ] \
    || fail "advancing the beacon mtime with no live sentry re-printed the banner: $out2"
  assert_contains "$out2" "full banner already printed this episode" \
    "same no-sentry episode did not print the concise reminder after a beacon touch"
  pass "sq-guard stale banner: a no-sentry episode survives a beacon mtime change"
}
