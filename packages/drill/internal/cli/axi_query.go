...
func startRunHelp() string {
	return `Run drill axi run --intent "the user's goal" --yes to validate the current branch`
}
...
func runAxiLogs(cmd *cobra.Command, step, runID string, full bool) (string, error) {
	...
	if !full && len(lines) > logTailLines {
		...
		fields = append(fields,
			...
			toon.Field{Key: "help", Value: []string{fmt.Sprintf("Run `drill axi logs --step %s --full` to see the entire log", step)}},
		)
		emitDoc(cmd, fields...)
		return fingerprint, nil
	}
