#!/usr/bin/env bash
# tests/sq-composer-lib.test.sh - the shared composer-content classifier
# (bin/sq-composer-lib.sh), the ONE unit-wide owner every backend adapter
# delegates its empty|pending|unknown verdict to.
#
# The load-bearing contract, task sq-composer-shellglyph-safety:
...
test_stripped_unbordered_content_uses_plain_content() {
  ...
  # threshold in the unit, so a raised threshold really can strip it to empty
