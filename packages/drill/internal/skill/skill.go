// Package skill holds the canonical content of the drill agent skill.
//
...
// Markdown() to the public skills/drill/SKILL.md (verified fresh in
...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/gateguidance"
...
// installed directory so the agent exposes it as the /drill command.
const Name = "drill"
...
const Description = "Validate your code changes through the drill pipeline - automated code review, tests, lint, docs, push, PR, and CI - before they reach the configured push target. Use when the user asks to run drill, gate or ship or validate their changes, push safely, asks you to do a task and then validate it, or invokes /drill."
...
When the user invokes ` + "`/drill`" + `, report the outcome at the end. If the user
...
` + "`drill axi run --help`" + ` to see the available flags.

## Two ways to invoke

` + "`/drill`" + ` works in two modes, depending on whether the user hands you a
task along with the command:

- **Validate-only** - bare ` + "`/drill`" + ` (optionally with flag-style requests
  like "skip the lint step"). The user's code changes are already committed;
  validate them and report the outcome.
- **Task-first** - ` + "`/drill <task>`" + `, e.g.
  ` + "`/drill add a --json flag to the status command`" + `. First carry out the
...
- The repository must already be initialized with ` + "`drill init`" + `.
- The daemon must have a runnable configured pipeline agent: a supported native
  ...
  before its first step; ` + "`drill doctor`" + ` reports the configuration problem.
...
repository is not initialized, run ` + "`drill init`" + ` first; if the ` + "`drill`" + `
command itself is missing or misbehaving, ` + "`drill doctor`" + ` reports what is
wrong.
Before starting, run ` + "`drill axi`" + ` (home view).
If it shows an active run on your current branch, inspect it with ` + "`drill axi status`" + `.
If it is parked at a gate, drive it with ` + "`drill axi respond`" + `.
Reattach an in-flight run by re-running ` + "`drill axi run`" + ` when it still matches your current ` + "`HEAD`" + ` - either as the submitted head or as the current pipeline head.
Only ` + "`drill axi abort`" + ` it when you mean to discard that run before starting over; aborting is a between-runs action, never a way to take over or bypass a gate while a run is still going (see [Validate and decide](#validate-and-decide)).
If it shows an active run on another branch, leave that run alone and start validation for your current branch with ` + "`drill axi run --intent \"...\"`" + `.
...
directly - drill uses it verbatim instead of inferring it from local agent
...
1. Start the run. It blocks until the first decision point or the end:
   ` + "```sh" + `
   drill axi run --intent "<what the user set out to accomplish>"
   ` + "```" + `
   ...
   disturbing the run, use ` + "`drill axi status`" + ` from a separate call.
...
2. If the output contains a ` + "`gate:`" + ` object, the pipeline is waiting on you.
   ...
   **Review auto-fix is disabled by default** (` + "`auto_fix.review: 0`" + `; a repo
   ...
   drill axi respond --action fix --findings <id1,id2> --instructions "<optional guidance>"
...
3. Repeat step 2 until the output has an ` + "`outcome:`" + ` instead of a ` + "`gate:`" + `. The
   ...
   - ` + "`checks-passed`" + ` - the change is validated and CI is green (or the
     ...
     check list without that declaration is not ready. drill keeps
   ...
   - ` + "`failed`" + ` or ` + "`cancelled`" + ` - they did not; read the output and address it.
     ...
     pipeline again - ` + "`drill axi run --intent \"...\"`" + ` starts a fresh run,
     or ` + "`drill rerun`" + ` re-runs the pipeline for the current branch. This
...
Only when its ` + "`next_action.code`" + ` is ` + "`sync`" + `, run ` + "`drill axi sync`" + ` first.
...
When ` + "`next_action.code`" + ` is ` + "`recover_custody`" + `, a terminal run left unpublished pipeline commits preserved in the local gate: run ` + "`drill axi sync --recover`" + ` to return custody and take the preserved head, or ` + "`drill rerun`" + ` to resume validating it instead.
Recovery takes that head by fast-forward, or by adopting a diverged preserved head proven to carry every local change - the ordinary result of the pipeline rebasing your commits onto a newer base - after anchoring your pre-recovery head under ` + "`refs/drill/recover-local/<run>`" + `.
...
A dirty worktree, or divergence that cannot be proven contained, makes the recovery refuse with explicit choices; ` + "`--keep-local`" + ` keeps your current head while the preserved commits stay anchored under ` + "`refs/drill/recover/<run>`" + `.
...
After synchronization, commit the follow-up on top and re-run ` + "`drill axi run --intent \"...\"`" + ` with the original user intent.
...
with ` + "`drill rerun`" + `, which cancels the stale monitor and re-runs the full
pipeline including a deterministic rebase step. Do **not** reach for
` + "`drill axi run`" + ` to refresh a still-active PR: after ` + "`checks-passed`" + ` it
...
` + "```sh" + `
drill axi               # home view: current branch, active runs, next steps
drill axi status        # full detail plus cached branch_sync when relevant
drill axi sync --check  # freshly verify an offered synchronization plan
drill axi sync          # apply only an offered guarded synchronization
drill axi sync --recover  # return custody after a terminal run left unpublished pipeline commits
drill axi logs --step <name> --full   # full log output of one step
drill axi abort         # cancel the current-branch active run
drill axi abort --run <id>   # cancel a specific run by id (works outside its worktree)
` + "```" + `
...
findings[2]{id,severity,file,line,action,description}:
  ...
  r2,error,cmd/drill/main.go,,ask-user,New --force flag bypasses the confirm prompt
help[6]:
  Run ` + "`drill axi respond --action approve`" + ` to accept this step and continue
  Run ` + "`drill axi respond --action fix --findings <ids>`" + ` to have the pipeline fix the selected findings (do not edit files yourself)
