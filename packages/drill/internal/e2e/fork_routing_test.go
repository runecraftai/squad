...
func TestForkRouting(t *testing.T) {
	...
	parentURL := "https://github.com/parent-owner/drill.git"
	forkURL := "https://github.com/fork-owner/drill.git"
	...
	t.Setenv("FAKEAGENT_GH_PARENT", "parent-owner/drill")
	...
	if run.PRURL == nil || !strings.HasPrefix(*run.PRURL, "https://github.com/parent-owner/drill/pull/") {
	...
	for _, inv := range invocations {
		...
		if len(inv.Args) >= 2 && inv.Args[0] == "pr" && inv.Args[1] == "create" {
			if inv.Repo == "fork-owner/drill" {
				t.Fatalf("created silent self-PR against fork: %+v", inv)
			}
			if inv.Repo == "parent-owner/drill" && inv.Head == "fork-owner:"+branch && inv.Base == "main" {
