#!/usr/bin/env bash
# Behavior tests for sq-spawn.sh batch dispatch (`id=repo` pairs).
#
# These exercise argument routing only: each spawn attempt fails fast at the
# missing-brief check, which is reached before any tmux/fob side effect, so
# the tests create no windows or worktrees. SQUAD_SPAWN_NO_GUARD=1 keeps them off the
# live sentry guard / state. Parser and path-scoping cases are table-driven; the
...
TMP_ROOT=$(fm_test_tmproot sq-spawn-batch)
...
# A projects/ path is resolved through the Squad home, never the caller cwd,
...
# A recon batch has no delivery contract to share, so the flags are refused rather
