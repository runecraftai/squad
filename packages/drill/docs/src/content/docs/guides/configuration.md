...
Configuration is optional. Without any config files, `drill` defaults to
...
- whether drill should infer intent from recent local agent transcripts

Config is split across two files:

| File                         | Scope                         | Full field reference                                          |
| ---------------------------- | ----------------------------- | ------------------------------------------------------------- |
| `~/.drill/config.yaml` | Global defaults for all repos | [Global Config Reference](/drill/reference/global-config/) |
| `<repo>/.drill.yaml`   | Per-repo overrides            | [Repo Config Reference](/drill/reference/repo-config/)     |

Set `DRILL_HOME` to relocate the global config directory (the global file becomes `$DRILL_HOME/config.yaml`).
Bitbucket Cloud credentials come from environment variables rather than config files.
For Azure DevOps, authenticate the `az` CLI with either `az devops login` or `AZURE_DEVOPS_EXT_PAT` for non-interactive daemon auth; see [Environment Variables](/drill/reference/environment/).
...
- drill reloads global config while setting up each run, so edits made before starting a run apply to it. For repeatable profiles (for example fast versus deep Codex settings), use separately initialized `DRILL_HOME` roots; `DRILL_HOME` moves all drill state, not just config.
...
Either way, available user intent can trigger an evidence-oriented agent follow-up after a successful test baseline. Evidence stays in a temporary local directory unless a supported provider publishes it to an orphan evidence branch through `test.evidence.store_in_repo`; the [Global Config Reference](/drill/reference/global-config/#testevidence) owns provider support and fail-closed behavior.
The [Repo Config Reference](/drill/reference/repo-config/) owns the exact per-command semantics (including that `commands.test` is targeted, not CI-parity), command process lifetime, and the `ignore_patterns` match rules.
...
Run `drill doctor` to check the global runner, and see [Choosing an Agent](/drill/guides/agents/) for how agent selection and fallback lists behave.
