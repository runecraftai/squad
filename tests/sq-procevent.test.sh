...
TMP_ROOT=$(fm_test_tmproot sq-procevent-tests)
export SQUAD_PROCEVENT_CLAIM_ROOT="$TMP_ROOT/claims"
...
wake_payloads() { awk -F '\t' '{print $5}' "$1/state/.stand-to-queue" 2>/dev/null; }
...
sup=$(PATH="${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/sq-supervision-lib.sh"; fm_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$IDLE/state")
...
sup=$(PATH="${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/sq-supervision-lib.sh"; fm_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$H1/state")
...
mode=$(PATH="${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
...
direct_leader=$(sed -n '2p' "$SQUAD_PROCEVENT_CLAIM_ROOT/direct-src.claim")
...
wait_for "$SQUAD_PROCEVENT_CLAIM_ROOT/shared-src.claim" || fail "shared-group start never claimed its source"
...
RACE_RECONCILE_PID=$!
sleep 0.3
assert_absent "$HRACE/state/.stand-to-queue" "publication bypassed the source serialization boundary"
...
assert_absent "$HRACE/state/.stand-to-queue" "an acknowledged result was appended after handling completed"
...
private_mode=$(PATH="${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}" bash -c \
  '. "$1/bin/sq-pr-lib.sh"; fm_pr_file_mode "$2"' _ "$ROOT" "$HPRIVATE/state/procevent-inbox/private-src.1.handled")
...
cat > "$ADAPTER_ROOT/bin/sq-procevent-endnow.sh" <<'SH'
#!/usr/bin/env bash
...
exit 2
SH
cat > "$ADAPTER_ROOT/bin/sq-procevent-openended.sh" <<'SH'
#!/usr/bin/env bash
# Fixture adapter with no terminal knowledge at all: nothing ever ends it.
exit 2
SH
cat > "$ADAPTER_ROOT/bin/sq-procevent-applying.sh" <<'SH'
#!/usr/bin/env bash
case "${1-}" in
  autohandle)
...
SH
chmod +x "$ADAPTER_ROOT/bin/sq-procevent-endnow.sh" "$ADAPTER_ROOT/bin/sq-procevent-openended.sh" \
  "$ADAPTER_ROOT/bin/sq-procevent-applying.sh"
...
mkdir "$HPUBLISH/state/.stand-to-queue"
...
rmdir "$HPUBLISH/state/.stand-to-queue"
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/ends-src.claim" "terminal retirement releases this runner's own claim"
...
replace_old_pid=$!
wait_for "$SQUAD_PROCEVENT_CLAIM_ROOT/replace-src.claim" || fail "the old registration was never claimed"
...
assert_present "$SQUAD_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/retire-fail-src.claim" \
...
# reading the stand-to queue at turn end without yet acting on this specific line.
mv "$HW/state/.stand-to-queue" "$HW/state/.stand-to-queue.drained-unhandled"
[ -z "$(wake_payloads "$HW")" ] || fail "the stand-to queue was not actually drained"

# Simulate a replacement Squad session: reconcile runs cold, as it would on
...
mv "$HW/state/.stand-to-queue" "$HW/state/.stand-to-queue.post-handle"
out=$(pe "$HW" reconcile)
...
runner_pid=$(sed -n '2p' "$SQUAD_PROCEVENT_CLAIM_ROOT/shared-src.claim" 2>/dev/null)
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/shared-src.claim" "retire releases the claim"
...
CLAIM="$SQUAD_PROCEVENT_CLAIM_ROOT/stale-src.claim"
mkdir -p "$SQUAD_PROCEVENT_CLAIM_ROOT"
...
printf '%s\n%s\ncross-home-token\ncross-home-identity\n%s\n' \
  "$HC_OLD" "999999" "$HC_OLD_STATE/procevent" > "$SQUAD_PROCEVENT_CLAIM_ROOT/cross-home-src.claim"
...
orphan_leader=$(sed -n '2p' "$SQUAD_PROCEVENT_CLAIM_ROOT/orphan-src.claim")
...
case "$orphan_out" in
  *"started=1"*)
    [ -e "$SQUAD_PROCEVENT_CLAIM_ROOT/orphan-src.claim" ] \
...
awk 'NR == 3 { print "replacement-token"; next } { print }' \
  "$SQUAD_PROCEVENT_CLAIM_ROOT/torn-src.claim" > "$TMP_ROOT/torn-next.claim"
...
mv "$TMP_ROOT/torn-next.claim" "$SQUAD_PROCEVENT_CLAIM_ROOT/torn-src.claim"
: > "$TORN_RELEASE"
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/retire-start-src.claim" "retirement cannot leave a late claim"
...
printf '%s\n%s\nreused-token\nnot-the-live-process-identity\n' \
  "$HI" "$innocent_pid" > "$SQUAD_PROCEVENT_CLAIM_ROOT/reused-src.claim"
chmod 0600 "$SQUAD_PROCEVENT_CLAIM_ROOT/reused-src.claim"
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/reused-src.claim" "retirement releases the exact reused-pid claim"
...
identity_status=0
identity_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" SQUAD_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-proc" \
...
assert_present "$SQUAD_PROCEVENT_CLAIM_ROOT/identity-src.claim" "uncertain retirement preserves claim generation"
...
sweep_pid_one=$(sed -n '2p' "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-one.claim")
sweep_pid_two=$(sed -n '2p' "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-two.claim")
...
assert_present "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep preflight does not release claims"
...
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-one.claim" "home sweep releases the first claim"
assert_absent "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-two.claim" "home sweep releases a claim with no registration"
...
assert_present "$SQUAD_PROCEVENT_CLAIM_ROOT/foreign-src.claim" "home sweep preserves a foreign-home claim"
[ "$(sed -n '1p' "$SQUAD_PROCEVENT_CLAIM_ROOT/foreign-src.claim")" = "$HN" ] || fail "home sweep changed foreign claim ownership"
...
sweep_status=0
sweep_out=$(PATH="$IDENTITY_FAKEBIN:$PATH" SQUAD_PROC_ROOT_OVERRIDE="$TMP_ROOT/no-sweep-proc" \
...
assert_present "$SQUAD_PROCEVENT_CLAIM_ROOT/sweep-uncertain.claim" "uncertain home sweep preserves the claim"
...
sup=$(bash -c '. "$1/bin/sq-supervision-lib.sh"; fm_supervision_needed "$2" && echo yes || echo no' _ "$ROOT" "$HV/state")
...
runner_help=$("$ROOT/bin/sq-procevent.sh" --help 2>&1 || true)
