...
func newCLISyncFixture(t *testing.T) cliSyncFixture {
	t.Helper()
	drillHome := filepath.Join(t.TempDir(), "nm-home")
...
func TestSyncHelpAndReferenceExposeGuardedModes(t *testing.T) {
	...
	for _, want := range []string{"## drill sync", "## drill axi sync", "drill axi sync --check"} {
...
func TestAxiSyncCheckAndApplyReturnFullStructuredState(t *testing.T) {
	...
	for _, want := range []string{"branch_sync:", "state: behind", "safety: safe_fast_forward", "freshness: live", f.old, f.pushed, "refs/heads/feature/sync", "command: drill axi sync"} {
...
func TestAxiSyncEquivalentDivergedCheckAndApply(t *testing.T) {
	...
	for _, want := range []string{"state: diverged", "safety: safe_equivalent_advance", "relation: diverged", "command: drill axi sync"} {
		if !strings.Contains(out, want) {
			t.Errorf("check missing %q:\n%s", want, out)
		}
	}
	...
	if got := cliGit(t, f.local, "rev-parse", "refs/drill/sync-anchor/"+f.runID); got != f.old {
...
func TestHumanSyncRequiresConfirmationOutsideTTY(t *testing.T) {
	...
	if !strings.Contains(out, "Re-run with `drill sync --yes`") {
		t.Fatalf("output:\n%s", out)
	}
	if got := cliGit(t, f.local, "rev-parse", "HEAD"); got != f.old {
		t.Fatal("HEAD changed")
...
// only in the repo's local gate at <DRILL_HOME>/repos/<id>.git, while the
...
func newCLIRecoverFixture(t *testing.T) cliRecoverFixture {
	...
	cliGit(t, pipeline, "add", "fix.txt")
	cliGit(t, pipeline, "commit", "-m", "drill(review): fix")
...
func TestAxiSyncCheckSurfacesRecoveryForTerminalPrePushRun(t *testing.T) {
	...
	for _, want := range []string{
		...
		"command: drill axi sync --recover",
		"drill rerun",
	} {
		if !strings.Contains(out, want) {
...
func TestAxiSyncRecoverReturnsCustodyEndToEnd(t *testing.T) {
	...
	for _, want := range []string{"recovered: true", "state: custody_returned", "changed: true", "relation: equal", "drill axi run --intent"} {
		if !strings.Contains(out, want) {
...
func TestAxiSyncRecoverDivergedRefusesThenKeepLocalSucceeds(t *testing.T) {
	...
	for _, want := range []string{"safety: blocked_recover_diverged", "refs/drill/recover/", "--keep-local"} {
		if !strings.Contains(out, want) {
...
func TestHumanSyncRecoverRequiresConfirmationOutsideTTY(t *testing.T) {
	...
	if !strings.Contains(out, "Re-run with `drill sync --recover --yes`") {
		t.Fatalf("output:\n%s", out)
	}
	if got := cliGit(t, f.local, "rev-parse", "HEAD"); got != f.submitted {
		t.Fatal("HEAD changed")
