#!/usr/bin/env bash
# tests/sq-backend-orca.test.sh - fake-Orca-CLI unit tests for the Orca
...
TMP_ROOT=$(fm_test_tmproot sq-backend-orca-tests)
...
if [ "${1:-}" = status ] && [ "${SQUAD_ORCA_STATUS_RESPONSE:-ready}" != sequence ]; then
...
neutral_fm_root() {  # <dir> -> echoes a minimal root with a quiet guard
  local root="$1/root"
  mkdir -p "$root/bin"
  cat > "$root/bin/sq-guard.sh" <<'SH'
#!/usr/bin/env bash
exit 0
SH
  chmod +x "$root/bin/sq-guard.sh"
  printf '%s\n' "$root"
}
...
test_send_text_submit_verifies_empty_composer_after_enter() {
  ...
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_send_text_submit term-123 "hello commander" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty on successful Orca send, got '$out'"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-123'$'\x1f''--text'$'\x1f''hello commander'$'\x1f''--json' \
...
test_send_text_submit_keeps_current_tail_when_limited() {
  ...
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/1.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["noise","│ > hello commander │"],"limited":true,"oldestCursor":"cursor-old"},"limited":true,"oldestCursor":"cursor-old"}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["╭──╮","│ > │","╰──╯"],"latestCursor":"cursor-new"}}}\n' > "$RESP/4.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/5.out"
...
test_send_text_submit_retries_when_composer_stays_pending() {
  ...
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/1.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/2.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > hello commander │"]}}}\n' > "$RESP/3.out"
  printf '{"ok":true,"result":{"send":{"handle":"term-123","accepted":true}}}\n' > "$RESP/4.out"
  printf '{"ok":true,"result":{"terminal":{"tail":["│ > │"]}}}\n' > "$RESP/5.out"
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_send_text_submit term-123 "hello commander" 3 0.01 0.01' "$ROOT" )
...
# Dead-shell injection safety (task sq-composer-shellglyph-safety): a pane whose
...
test_composer_state_bare_shell_prompt_is_unknown() {
  ...
  printf '{"ok":true,"result":{"terminal":{"tail":["some earlier output","kunchen@mac Squad $ "]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_composer_state term-123' "$ROOT" )
...
test_json_get_ignores_undocumented_terminal_id_shapes() {
  ...
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_worktree_create /repo/path sq-task' "$ROOT" )
  wt_id=${out%%$'\t'*}
  wt_path=${out#*$'\t'}
  term=${wt_path#*$'\t'}
  wt_path=${wt_path%%$'\t'*}
...
test_worktree_and_terminal_helpers_parse_json() {
  ...
  term=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_terminal_create wt-123 sq-task' "$ROOT" )
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''worktree'$'\x1f''create'$'\x1f''--repo'$'\x1f''id:repo-123'$'\x1f''--name'$'\x1f''sq-task'$'\x1f''--no-parent'$'\x1f''--setup'$'\x1f''skip'$'\x1f''--json' \
    "worktree helper did not create an independent no-hook worktree"
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-123'$'\x1f''--title'$'\x1f''sq-task'$'\x1f''--json' \
...
test_worktree_create_removes_worktree_when_path_missing() {
  ...
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/backends/orca.sh"; fm_backend_orca_worktree_create /repo/path sq-task' "$ROOT" 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "worktree helper should fail when Orca omits the worktree path"
  assert_contains "$out" "orca worktree create did not return a path for sq-task" \
...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  assert_grep "window=sq-$id" "$state/$id.meta" "preserved pathless metadata missing stable window alias"
  ...
  pass "sq-spawn.sh --backend orca: preserves metadata when pathless cleanup fails"
}
...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  assert_contains "$(cat "$log")" $'orca\x1f''terminal'$'\x1f''send'$'\x1f''--terminal'$'\x1f''term-spawn'$'\x1f''--text'$'\x1f''export GOTMPDIR=/tmp/sq-orcaspawnz1/gotmp'$'\x1f''--enter'$'\x1f''--json' \
  ...
  rm -rf "/tmp/sq-$id"
  pass "sq-spawn.sh --backend orca: reuses implicit terminal, records metadata, launches harness"
}

test_spawn_refuses_orca_XO_before_home_mutation() {
  local home subhome data state config id out status
  id="orcasmz1"
  home="$TMP_ROOT/XO-refusal-home"
  subhome="$TMP_ROOT/XO-refusal-subhome"
  data="$home/data"
  state="$home/state"
  config="$home/config"
  mkdir -p "$data" "$state" "$config" "$subhome/bin" "$subhome/data" "$subhome/state" "$subhome/projects"
  printf '%s\n' "$id" > "$subhome/.sq-xo-home"
  printf 'Squad\n' > "$subhome/AGENTS.md"
  printf 'claude\n' > "$config/crew-harness"
  touch "$state/.last-sentry-beat"
  set +e
  ...
  status=$?
  set +e
  [ "$status" -ne 0 ] || fail "backend=orca --xo should be refused"
  assert_contains "$out" "backend=orca does not support --xo spawns yet" \
    "orca XO refusal should happen at backend selection"
  assert_absent "$subhome/config/crew-harness" \
    "orca XO refusal should not propagate inherited local material into the XO home"
  pass "sq-spawn.sh --backend orca --xo: refuses before XO-home mutation"
}
...
test_spawn_refuses_orca_when_runtime_not_ready() {
  ...
  pass "sq-spawn.sh --backend orca: refuses before mutation when Orca runtime is not ready"
}
...
test_spawn_refuses_orca_nonisolated_worktree() {
  ...
  pass "sq-spawn.sh --backend orca: refuses non-isolated worktrees and closes implicit terminals"
}
...
test_spawn_removes_orca_worktree_when_terminal_create_fails() {
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''create'$'\x1f''--worktree'$'\x1f''id:wt-terminal-fail'$'\x1f''--title'$'\x1f'"sq-$id"$'\x1f''--json' \
  ...
  pass "sq-spawn.sh --backend orca: removes worktree when terminal creation fails"
}
...
test_spawn_preserves_orca_metadata_when_abort_cleanup_fails() {
  ...
  assert_grep "window=sq-$id" "$state/$id.meta" "preserved metadata missing stable window alias"
...
test_spawn_releases_orca_resources_when_metadata_write_fails() {
  ...
  assert_contains "$out" "Permission denied" "spawn should fail at metadata publication"
...
test_peek_send_and_crew_state_route_through_orca_meta() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-io" "worktree=$wt" "project=$wt" "harness=claude" "kind=recon" "backend=orca"
  touch "$state/.last-sentry-beat"
  orca_case io-path
  neutral=$(neutral_fm_root "$CASE_DIR/neutral")
  printf '{"ok":true,"result":{"terminal":{"tail":["ready"]}}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    SQUAD_ROOT_OVERRIDE="$neutral" SQUAD_STATE_OVERRIDE="$state" SQUAD_SEND_SETTLE=0 \
    "$ROOT/bin/sq-peek.sh" "sq-$id" 10 )
  [ "$out" = ready ] || fail "sq-peek should read through Orca metadata, got '$out'"
  ...
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_STATE_OVERRIDE="$state" "$ROOT/bin/sq-crew-state.sh" "$id" )
  assert_contains "$out" "state: unknown" "crew-state should fall back cleanly for an idle Orca recon"
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f'"sq-$id" \
  ...
  pass "sq-peek/sq-send/sq-crew-state route through backend=orca metadata"
}
...
test_peek_and_crew_state_fail_closed_on_orca_error_json() {
  ...
  fm_write_meta "$state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-stale" "worktree=$wt" "project=$wt" "harness=claude" "kind=recon" "backend=orca"
  touch "$state/.last-sentry-beat"
  orca_case read-error-json
  neutral=$(neutral_fm_root "$CASE_DIR/neutral")
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/1.out"
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    SQUAD_ROOT_OVERRIDE="$neutral" SQUAD_STATE_OVERRIDE="$state" "$ROOT/bin/sq-peek.sh" "sq-$id" 10 2>&1 )
  status=$?
  [ "$status" -ne 0 ] || fail "sq-peek should fail when Orca reports a stale terminal"
  assert_contains "$out" "terminal handle stale" "sq-peek should surface the Orca read error message"
  printf '{"ok":false,"error":{"code":"terminal_handle_stale","message":"terminal handle stale"}}\n' > "$RESP/2.out"
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_STATE_OVERRIDE="$state" "$ROOT/bin/sq-crew-state.sh" "$id" )
  ...
  assert_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''read'$'\x1f''--terminal'$'\x1f''term-stale' \
    "sq-peek/sq-crew-state did not read the recorded Orca terminal"
  pass "sq-peek/sq-crew-state: Orca read error JSON fails closed"
}
...
test_target_exists_rejects_orca_error_json() {
  ...
  set +e
  PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/sq-backend.sh"; fm_backend_target_exists orca term-stale sq-task' "$ROOT"
  status=$?
  set -e
...
test_scout_teardown_removes_orca_worktree_via_helper() {
  ...
  pass "sq-teardown.sh backend=orca: recon report gate then helper-backed worktree removal"
}
...
test_scout_teardown_refuses_orca_id_path_mismatch() {
  ...
  pass "sq-teardown.sh backend=orca: recon teardown refuses id/path mismatches"
}
...
test_teardown_removes_orca_worktree_when_path_missing() {
  ...
  pass "sq-teardown.sh backend=orca: releases terminal/worktree when path is absent"
}
...
test_teardown_preserves_metadata_when_orca_remove_error_json() {
  ...
  pass "sq-teardown.sh backend=orca: preserves metadata on remove ok:false JSON"
}
...
test_scout_teardown_refuses_orca_missing_report_when_path_missing() {
  ...
  pass "sq-teardown.sh backend=orca: recon report gate precedes pathless helper cleanup"
}
...
test_ship_teardown_refuses_orca_missing_worktree_path() {
  ...
  pass "sq-teardown.sh backend=orca: ship teardown fails closed when worktree path is missing"
}
...
test_ship_teardown_removes_orca_worktree_when_id_path_matches() {
  ...
  touch "$state/.last-sentry-beat"
  fm_write_meta "$state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-ship-match" "worktree=$wt" "project=$proj" \
...
test_ship_teardown_refuses_orca_unresolvable_worktree_id() {
  ...
  touch "$state/.last-sentry-beat"
  fm_write_meta "$state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-ship-unresolved" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=strike" "mode=local-only" "yolo=off" \
  ...
  pass "sq-teardown.sh backend=orca: ship teardown fails closed when id resolution fails"
}
...
test_ship_teardown_refuses_orca_id_path_mismatch() {
  ...
  touch "$state/.last-sentry-beat"
  fm_write_meta "$state/$id.meta" \
    "window=sq-$id" "endpoint_task_id=$id" "terminal=term-ship-mismatch" "worktree=$wt" "project=$proj" \
    "harness=claude" "kind=strike" "mode=local-only" "yolo=off" \
  ...
  pass "sq-teardown.sh backend=orca: ship teardown refuses id/path mismatches"
}
...
test_teardown_refuses_orca_missing_worktree_id() {
  ...
  pass "sq-teardown.sh backend=orca: refuses missing worktree ids before cleanup"
}
...
test_teardown_refuses_orca_worktree_without_terminal_handle() {
  ...
  pass "sq-teardown.sh backend=orca: refuses incomplete worktree-only endpoint metadata before runtime dispatch"
}

test_XO_force_teardown_removes_orca_child_via_orca() {
  local home subhome childproj childwt child_id neutral out rc
  home="$TMP_ROOT/orca-child-parent"
  subhome="$TMP_ROOT/orca-child-XO"
  ...
  assert_not_contains "$(cat "$LOG")" $'orca\x1f''terminal'$'\x1f''close'$'\x1f''--terminal'$'\x1f'"sq-$child_id" \
  ...
  pass "sq-teardown.sh --force: removes Orca XO children through Orca"
}

test_XO_force_teardown_refuses_orca_child_id_path_mismatch() {
  local home subhome childproj childwt other_wt child_id neutral out rc
  home="$TMP_ROOT/orca-child-mismatch-parent"
  subhome="$TMP_ROOT/orca-child-mismatch-XO"
  childproj="$subhome/projects/alpha"
  ...
  assert_present "$home/state/domain.meta" "refused forced XO teardown should preserve parent metadata"
  pass "sq-teardown.sh --force: refuses Orca child id/path mismatches"
}

test_XO_force_teardown_refuses_partial_orca_child() {
  local home subhome childproj childwt child_id neutral out rc
  home="$TMP_ROOT/orca-partial-child-parent"
  subhome="$TMP_ROOT/orca-partial-child-XO"
  childproj="$subhome/projects/alpha"
  ...
  pass "sq-teardown.sh --force: refuses partial Orca XO children before runtime dispatch"
}
...
test_dispatcher_sources_orca_and_routes_primitives() {
  ...
  out=$( PATH="$FB:$PATH" SQUAD_ORCA_LOG="$LOG" SQUAD_ORCA_RESPONSES="$RESP" \
    bash -c '. "$0/bin/sq-backend.sh"; fm_backend_validate orca; fm_backend_capture orca term-123 9' "$ROOT" )
  [ "$out" = "via dispatch" ] || fail "dispatcher should route capture to the Orca adapter, got '$out'"
  pass "sq-backend dispatcher: accepts orca and routes capture through bin/backends/orca.sh"
}
