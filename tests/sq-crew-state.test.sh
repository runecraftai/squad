#!/usr/bin/env bash
# Behavior tests for bin/sq-crew-state.sh - the deterministic crew-current-state
# helper.
#
...
#   (k) operator_is_provably_working end-to-end over the REAL helper (not a canned
#       fake sq-crew-state.sh verdict): cross-branch attribution via the runs
#       list -> absorbed; genuinely no run anywhere + idle pane -> surfaced.
#       This is the direct regression pair for the 2026-07-02 herdr incident,
#       proving the sentry's own absorb-only-when-provably-working predicate
...
TMP_ROOT=$(fm_test_tmproot sq-crew-state)
...
make_repo_on_branch() {  # <dir> <branch>
  ...
  # Real worktree HEAD for run head-binding (fixtures read SQUAD_FAKE_RUN_HEAD).
...
case "${1:-}" in
  ...
  pane)
    case "${2:-}" in
      read)
        [ "${SQUAD_FAKE_HERDR_MISSING:-0}" = 1 ] && exit 1
        if [ "${SQUAD_FAKE_HERDR_BUSY:-0}" = 1 ]; then printf 'work in progress\nesc to interrupt\n'
        else printf 'all quiet\n> \n'; fi
        exit 0 ;;
    esac ;;
  agent)
    case "${2:-}" in
      get)
        [ -n "${SQUAD_FAKE_HERDR_AGENT_STATUS:-}" ] || exit 1
...
# Run the helper for one case dir. SQUAD_FAKE_* env (run output, busy flag) are read
...
run:
  ...
  branch: $1
  status: running
  head: "${SQUAD_FAKE_RUN_HEAD:-abc1234}"
  pr: ""
  findings: none
...
# Regression for the PR #252 incident: the operator's own status log never got a
...
# merge/close. sq-crew-state must surface this as done, not "validating
...
test_no_run_busy_pane() {
  ...
  # semantic lifecycle record (bin/sq-busy-lib.sh), not from rendered text.
  SQUAD_FAKE_AXI_STATUS=""
  SQUAD_FAKE_RUNS_LIST=""
  SQUAD_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$d/state" feat-h)
  "$ROOT/bin/sq-busy-event.sh" apply "$d/state" feat-h busy --gen "$gen" \
...
test_no_run_grok_uses_isolated_fallback() {
  ...
  SQUAD_FAKE_RUNS_LIST=""
  SQUAD_FAKE_BUSY=1
  SQUAD_FAKE_BUSY_TEXT='Ctrl+c:cancel'
  export SQUAD_FAKE_BUSY_TEXT
...
test_no_run_herdr_unknown_uses_backend_capture() {
  ...
  SQUAD_FAKE_HERDR_BUSY=1
  SQUAD_FAKE_HERDR_AGENT_STATUS=working
...
test_no_run_herdr_idle_agent_status_outranked_by_record() {
  ...
  SQUAD_FAKE_RUNS_LIST=""
  SQUAD_FAKE_TMUX_MISSING=1
  SQUAD_FAKE_HERDR_AGENT_STATUS=idle
  SQUAD_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$d/state" feat-herdr-idle)
  "$ROOT/bin/sq-busy-event.sh" apply "$d/state" feat-herdr-idle busy --gen "$gen" \
    --source claude-hook --event user-prompt-submit
...
test_no_run_herdr_idle_agent_status_and_idle_record_stays_idle() {
  ...
  SQUAD_FAKE_HERDR_AGENT_STATUS=idle
  SQUAD_FAKE_HERDR_BUSY=0
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$d/state" feat-herdr-stopped)
  "$ROOT/bin/sq-busy-event.sh" apply "$d/state" feat-herdr-stopped idle --gen "$gen" \
    --source claude-hook --event stop
...
# supervisor reading the operator sees a distinct pause (and its reason) rather than a
# wedge-suspect idle. This is the reader half the sentry/daemon build on.
...
test_no_run_idle_pane_custom_paused_verb() {
  ...
  local out; out=$(SQUAD_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
  ...
  out=$(SQUAD_CLASSIFY_PAUSED_VERB=awaiting run_crew_state "$d" feat-custom-pause)
...
# detail: a healthy idle XO that just closed a keyed decision falls through
# to the idle default (unknown/none), not `unknown` with the resolution note as its
# `doing`. Regression for the sitrep render bug where such an XO showed
# state=unknown with resolution prose. The one-owner keyed fold in sq-classify-lib.sh
# is untouched; this only stops the deriver from reading a non-state event as state.
test_no_run_idle_XO_resolved_event_not_state() {
  reset_fakes
  local d; d=$(new_case resolved-idle)
  mkdir -p "$d/wt"
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/mate.meta" "window=fm:sq-mate" "worktree=$d/wt" "kind=xo" "home=$d/wt"
  printf 'needs-decision [key=race]: pick subscribe order\n' > "$d/state/mate.status"
  printf 'resolved [key=race]: went with subscribe-before-write\n' >> "$d/state/mate.status"
  SQUAD_FAKE_AXI_STATUS=""
  SQUAD_FAKE_BUSY=0
  local out; out=$(run_crew_state "$d" mate)
  assert_contains "$out" "state: unknown" "resolved-then-idle XO is not a spurious run-state"
...
SH
  ...
  fm_write_meta "$d/state/feat-timeout.meta" "window=fm:sq-feat-timeout" "worktree=$d/wt" "kind=strike" \
    "harness=claude"
  SQUAD_FAKE_BUSY=1
  local gen; gen=$("$ROOT/bin/sq-busy-event.sh" arm "$d/state" feat-timeout)
...
test_torn_down_worktree() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/gone-k.meta" "window=fm:sq-gone-k" "worktree=$d/no-such-worktree" "kind=strike"
  local out rc
...
# (k) operator_is_provably_working end-to-end over the REAL sq-crew-state.sh (not a
# canned fake verdict, unlike tests/sq-sentry-triage.test.sh's classifier
...
# another branch must still be absorbed by the sentry via the runs-list
# fallback (working), while an operator with genuinely no run anywhere and an idle
...
EOF
  ...
  pass "operator_is_provably_working absorbs a validating crew found only via the runs-list fallback"
}
...
EOF
  ...
  SQUAD_FAKE_BUSY=0
  PATH="$d/fakebin:$PATH" SQUAD_STATE_OVERRIDE="$d/state" operator_is_provably_working feat-stopped \
    && fail "a stopped crew with no run anywhere and an idle pane was treated as provably working"
  pass "operator_is_provably_working still surfaces a genuinely stopped crew (safety property preserved)"
}
...
test_historical_same_branch_rewritten_head_not_current() {
  ...
  make_fakebin "$d" >/dev/null
  fm_write_meta "$d/state/wishlist.meta" "window=fm:sq-wishlist" "worktree=$d/wt" "kind=strike" "harness=claude"
...
echo "all sq-crew-state tests passed"
