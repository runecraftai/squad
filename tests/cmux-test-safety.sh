...
# just "the app", the SAME real running instance the commander uses day to day.
...
#
# Fails CLOSED: any ambiguity (an empty id, a plain label without the sq-test-
...
# the commander's own live work.
set -u

# cmux_refuse_if_unsafe: 0 (SAFE to proceed) only if <workspace_id> is
# non-empty, <want_label> carries the sq-test- prefix, and the workspace is
...
cmux_refuse_if_unsafe() {  # <workspace_id> <want_label>
  ...
  case "$want_label" in
    sq-test-*) : ;;
    *) echo "cmux safety guard: refusing - label '$want_label' does not carry the sq-test- prefix" >&2; return 1 ;;
  esac
...
# cleanup leaves the isolated, throwaway workspace for that sq-test- label open
