#!/usr/bin/env bash
# tests/sq-remote-doctor.test.sh - the remote second-mate readiness gate.
#
# Drives the real bin/sq-remote-doctor.sh against a controlled account fixture:
...
TMP_ROOT=$(fm_test_tmproot sq-remote-doctor)
LABEL=dev.Squad.herdr.sq-remote
INTERACTIVE_LABEL=dev.Squad.herdr
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
JOB_LABEL=dev.Squad.remote-job
CASE_N=0
DOCTOR_WORKER_PID=
...
#!/usr/bin/env bash
set -u
printf '%s\n' "$*" >> "$SQUAD_FAKE_LAUNCHCTL_LOG"
domain=${2:-}
label=${domain##*/}
loaded="$SQUAD_FAKE_STATE/loaded-$label"
case "${1:-}" in
  print)
    case "$domain" in
      */dev.Squad.herdr.sq-remote)
        [ -f "$loaded" ] || exit 113
        cat "$loaded"
        ;;
      */dev.Squad.herdr)
        [ -f "$SQUAD_FAKE_STATE/interactive-loaded" ] || exit 113
        printf 'interactive default job\n'
        ;;
      */*/*) [ -f "$loaded" ] || exit 113; cat "$loaded" ;;
      *) [ -f "$SQUAD_FAKE_STATE/gui-session" ] || exit 113 ;;
    esac
    exit 0
    ;;
  bootout)
    [ ! -f "$SQUAD_FAKE_STATE/bootout-fail" ] || { printf 'Boot-out failed: operation not permitted\n' >&2; exit 6; }
    case "$domain" in
      */dev.Squad.herdr.sq-remote) rm -f "$loaded" ;;
      */dev.Squad.herdr) rm -f "$SQUAD_FAKE_STATE/interactive-loaded" ;;
      *) rm -f "$loaded" ;;
    esac
    exit 0
    ;;
  bootstrap)
    ...
    [ -f "$SQUAD_FAKE_STATE/gui-session" ] || { printf 'Bootstrap failed: 5: Input/output error\n' >&2; exit 5; }
    ...
    [ ! -f "$loaded" ] || { printf 'Bootstrap failed: service already loaded\n' >&2; exit 5; }
    case "$label" in
      dev.Squad.remote-job)
        cat > "$loaded" <<EOF
path = $SQUAD_FAKE_JOB_PLIST
program = $SQUAD_FAKE_JOB_WORKER
properties = keepalive | runatload | inferred program
EOF
        ;;
      *)
        cat > "$loaded" <<EOF
path = $SQUAD_FAKE_PLIST
program = $SQUAD_FAKE_HERDR_BIN
arguments = {
	$SQUAD_FAKE_HERDR_BIN
	server
	--session
	sq-remote
}
stdout path = $SQUAD_FAKE_LAUNCH_AGENT_LOG
stderr path = $SQUAD_FAKE_LAUNCH_AGENT_LOG
properties = keepalive | runatload | inferred program
EOF
        [ -f "$SQUAD_FAKE_STATE/bootstrap-does-not-start" ] || printf 'true\n' > "$SQUAD_FAKE_HERDR_RUNNING"
        ;;
    esac
    exit 0
    ;;
  kickstart)
    [ ! -f "$SQUAD_FAKE_STATE/kickstart-fail" ] || { printf 'Kickstart failed: service unavailable\n' >&2; exit 6; }
    case "$label" in
      dev.Squad.remote-job) : ;;
      *)
        if [ -f "$SQUAD_FAKE_STATE/kickstart-delay" ]; then
          cp "$SQUAD_FAKE_STATE/kickstart-delay" "$SQUAD_FAKE_STATE/herdr-delay"
        else
          printf 'true\n' > "$SQUAD_FAKE_HERDR_RUNNING"
        fi
        ;;
...
#!/usr/bin/env bash
set -u
running=$(cat "$SQUAD_FAKE_HERDR_RUNNING" 2>/dev/null || printf 'false')
case "${1:-} ${2:-}" in
  "status --json")
    if [ -f "$SQUAD_FAKE_STATE/herdr-delay" ]; then
      delay=$(cat "$SQUAD_FAKE_STATE/herdr-delay")
      if [ "$delay" -gt 0 ]; then
        printf '%s\n' "$((delay - 1))" > "$SQUAD_FAKE_STATE/herdr-delay"
        running=false
      else
        rm -f "$SQUAD_FAKE_STATE/herdr-delay"
        printf 'true\n' > "$SQUAD_FAKE_HERDR_RUNNING"
        running=true
      fi
    fi
...
exit 0
SH
  chmod +x "$CASE_BIN/uname" "$CASE_BIN/launchctl" "$CASE_BIN/tasks-axi" "$CASE_BIN/fob" "$CASE_BIN/claude"
...
doctor() {
  ...
  DOCTOR_OUT=$(
    ...
    SQUAD_FAKE_HERDR_RUNNING="$CASE_HERDR_RUNNING" \
    SQUAD_FAKE_HERDR_BIN="$CASE_BIN/herdr" \
    SQUAD_FAKE_PLIST="$CASE_PLIST" \
    SQUAD_FAKE_JOB_PLIST="$CASE_JOB_PLIST" \
    SQUAD_FAKE_JOB_WORKER="$ROOT/bin/sq-remote-job-worker.sh" \
    SQUAD_FAKE_LAUNCH_AGENT_LOG="$CASE_HOME/Library/Logs/$LABEL.log" \
    SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE="${CASE_PLATFORM_OVERRIDE-}" \
    SQUAD_REMOTE_JOB_ACTIVE="${CASE_REMOTE_JOB_ACTIVE-1}" \
    "$ROOT/bin/sq-remote-doctor.sh" "$@" 2>&1
  )
  DOCTOR_RC=$?
  set -e
}
...
assert_grep "<string>$LABEL</string>" "$CASE_PLIST" "the written plist does not carry the Squad label"
assert_grep '<string>server</string>' "$CASE_PLIST" "the written plist does not run a herdr server"
assert_grep '<string>sq-remote</string>' "$CASE_PLIST" "the written plist does not pin the remote-XO session"
assert_no_grep '<string>default</string>' "$CASE_PLIST" "the written plist pins the interactive default session"
assert_grep "<string>$JOB_LABEL</string>" "$CASE_JOB_PLIST" "the worker plist does not carry the Squad label"
assert_grep '<string>Aqua</string>' "$CASE_JOB_PLIST" "the worker plist is not Aqua-scoped"
assert_grep "$ROOT/bin/sq-remote-job-worker.sh" "$CASE_JOB_PLIST" "the worker plist does not use the configured code root"
...
cmp -s "$CASE_STATE/interactive-before.plist" "$CASE_INTERACTIVE_PLIST" \
  || fail "the sq-remote repair rewrote the interactive default launch agent"
assert_present "$CASE_STATE/interactive-loaded" "the sq-remote repair unloaded the interactive default launch agent"
assert_no_grep "gui/$(id -u)/$INTERACTIVE_LABEL$" "$CASE_LAUNCHCTL_LOG" \
  "the sq-remote repair inspected or controlled the interactive default launch agent"
assert_no_dangerous_calls "the repair reached for auto-login, FileVault, or the keychain"
pass "--fix installs the dedicated sq-remote launch agent without touching default"
...
assert_grep '# Squad remote tool wrapper v1' "$CASE_HOME/.local/bin/tasks-axi" \
  "the generated wrapper is not marked Squad-owned"
...
mv "$CASE_BIN/fob" "$MANAGER_BIN/fob"
mkdir -p "$CASE_HOME/.local/bin"
printf 'operator wrapper\n' > "$CASE_HOME/.local/bin/fob"
doctor --fix
expect_code 1 "$DOCTOR_RC" "--fix overwrote an operator-owned reserved wrapper"
assert_contains "$DOCTOR_OUT" 'fix required-fob=failed:' \
  "the non-Squad wrapper refusal was not reported"
[ "$(cat "$CASE_HOME/.local/bin/fob")" = 'operator wrapper' ] \
...
mkdir -p "$CASE_HOME/.local/bin"
for tool in herdr tasks-axi fob claude; do
  ln -s "$CASE_BIN/$tool" "$CASE_HOME/.local/bin/$tool"
done
HOME="$CASE_HOME" SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  "$ROOT/bin/sq-remote-job-worker.sh" > "$CASE_STATE/worker.out" 2> "$CASE_STATE/worker.err" &
DOCTOR_WORKER_PID=$!
for _ in $(seq 1 100); do
  [ -f "$CASE_HOME/.Squad/remote-job/worker.ready" ] && break
  sleep 0.05
done
assert_present "$CASE_HOME/.Squad/remote-job/worker.ready" "the stale-identity fixture worker did not start"
printf 'stale-worker-identity\n' > "$CASE_HOME/.Squad/remote-job/worker.identity"
doctor
expect_code 1 "$DOCTOR_RC" "doctor accepted a live worker with stale code identity"
assert_contains "$DOCTOR_OUT" 'check remote-job-worker=fixable: the running remote job worker does not match the current Squad code' \
...
DOCTOR_WORKER_PID=$(cat "$CASE_HOME/.Squad/remote-job/worker.pid")
...
printf '#!/usr/bin/env bash\n' > "$REMOTE_ROOT/bin/sq-remote-entrypoint.sh"
export SQUAD_ROOT_OVERRIDE="$REMOTE_ROOT"
doctor
...
[ "$(readlink "$CASE_HOME/.local/bin/sq-remote-entrypoint.sh")" = "$REMOTE_ROOT/bin/sq-remote-entrypoint.sh" ] \
  || fail "the entrypoint symlink does not point at this code root"
printf 'not a symlink\n' > "$CASE_HOME/.local/bin/other"
rm -f "$CASE_HOME/.local/bin/sq-remote-entrypoint.sh"
printf 'operator wrapper\n' > "$CASE_HOME/.local/bin/sq-remote-entrypoint.sh"
doctor --fix
assert_contains "$DOCTOR_OUT" 'check entrypoint-link=human:' "an operator-owned entrypoint file was not left to the operator"
[ "$(cat "$CASE_HOME/.local/bin/sq-remote-entrypoint.sh")" = 'operator wrapper' ] \
