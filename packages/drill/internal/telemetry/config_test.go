...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/buildinfo"
)
...
func TestDefaultUsesDotEnvInDevBuildWhenEnvMissing(t *testing.T) {
	...
	content := "DRILL_UMAMI_HOST=https://dotenv.example\nDRILL_UMAMI_WEBSITE_ID=website-from-dotenv\n"
...
func TestDefaultDisablesTelemetryWhenEnvIsOff(t *testing.T) {
	...
	if _, ok := Default().(*Client); ok {
		t.Fatal("Default() should disable telemetry when DRILL_TELEMETRY=off")
	}
}
...
func TestDefaultIgnoresDotEnvOutsideRepo(t *testing.T) {
	...
	parentDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(parentDir, ".env"), []byte("DRILL_UMAMI_WEBSITE_ID=outside-repo\n"), 0o644); err != nil {
...
func TestParseDotEnvStripsInlineCommentsFromUnquotedValues(t *testing.T) {
	values := parseDotEnv([]byte("DRILL_UMAMI_WEBSITE_ID=abc123 # dev\n"))
...
func TestParseDotEnvPreservesHashesInQuotedValues(t *testing.T) {
	values := parseDotEnv([]byte("DRILL_UMAMI_WEBSITE_ID=\"abc # dev\"\n"))
