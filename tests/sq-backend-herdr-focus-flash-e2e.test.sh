...
# lab teardown verifies that the default unit session is byte-identical.
set -u
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/sq-herdr-focus-flash-e2e.XXXXXX")
FAKEBIN="$TMP_ROOT/fakebin"
mkdir -p "$FAKEBIN"

HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name sq-herdr-focus-flash-regression-r1)
...
: > "$B_OPERATION_ACTIVE"
B_OUT=$(PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" SQUAD_FLASH_CALL_LOG="$CALL_LOG" bash -c '
  . "$1/bin/backends/herdr.sh"
  fm_backend_herdr_cli() {
    local session=$1
    shift
    printf "%s\n" "$*" >> "$SQUAD_FLASH_CALL_LOG"
...
# direnv, or anything an operator backgrounded) fails the lone-idle-shell proof
...
C_PROOF_POLLS=3
C_OUT=$(PATH="$FAKEBIN:$HERDR_ORIGINAL_PATH" SQUAD_FLASH_CALL_LOG="$C_CALL_LOG" \
  SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS="$C_PROOF_POLLS" bash -c '
  . "$1/bin/backends/herdr.sh"
  fm_backend_herdr_cli() {
    local session=$1
    shift
...
if [ "$STEAL_LIVE" = 1 ]; then
  ...
  # the operation rather than parking the commander somewhere else.
  [ "$C_WRONG" -ge 1 ] \
