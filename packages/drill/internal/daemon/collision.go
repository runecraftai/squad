...
// daemonProcessInfo describes a running `drill daemon run --root <root>`
...
// daemonListDaemonProcesses enumerates drill managed-daemon processes. It
...
// path spelling (e.g. a symlinked DRILL_HOME).
...
// looks like `drill daemon run --root <root>`, extracting the pid and the
...
// looksLikeDaemonRunCommand reports whether a command line is a drill
...
// reconcileCollidingDaemons detects drill daemons already running for the
// same logical root as p but started under a different path spelling (e.g. a
// symlinked or relative DRILL_HOME). The socket-keyed health check in Start cannot
