#!/usr/bin/env bash
# sq-send strict target resolution and key delivery reporting.
#
...
TMP_ROOT=$(fm_test_tmproot sq-send-strict)
...
case "${1:-}" in
  send-keys)
    ...
    printf 'send-keys target=%s literal=%s arg=%s\n' "$target" "$literal" "${1:-}" >> "$SQUAD_TMUX_LOG"
    # SQUAD_FAKE_TMUX_SEND_KEY_FAIL names one key whose delivery fails, so the
    # --key exit contract can be driven both ways from the same stub.
    if [ "$literal" = 0 ] && [ -n "${SQUAD_FAKE_TMUX_SEND_KEY_FAIL:-}" ] \
      && [ "${1:-}" = "$SQUAD_FAKE_TMUX_SEND_KEY_FAIL" ]; then
      exit 1
    fi
    exit 0 ;;
  display-message)
    ...
    if [ -n "${SQUAD_FAKE_TMUX_DEAD_TARGET:-}" ] && [ "$target" = "$SQUAD_FAKE_TMUX_DEAD_TARGET" ]; then
      exit 1
    fi
    [ "$cursor" = 1 ] && { printf '1\n'; exit 0; }
    printf '%%1\n'
  ...
  capture-pane)
    ...
    exit 0 ;;
  list-windows)
    printf 'foreign:%s\n' "${SQUAD_FAKE_TMUX_WINDOW:-sq-lost}"
    exit 0 ;;
esac
exit 0
SH
...
test_exact_lane_id_send_still_works() {
  ...
  got=$(cat "$log")
  assert_contains "$got" "target=sess:sq-mpf-lane-m8 literal=1 arg=lost dispatch" "exact id should type literal text to the meta target"
  assert_contains "$got" "target=sess:sq-mpf-lane-m8 literal=0 arg=Enter" "exact id should submit with Enter"
  pass "sq-send strict: exact task/lane ids resolve through home metadata"
}
...
test_unresolvable_target_does_not_tmux_fallback() {
  ...
  pass "sq-send strict: unresolvable selectors do not fall back to tmux"
}
...
test_prefixless_herdr_pane_id_fails() {
  ...
  pass "sq-send strict: prefixless herdr pane ids are rejected before tmux fallback"
}
...
test_unmatched_single_colon_target_must_exist() {
  ...
  pass "sq-send strict: unmatched single-colon explicit targets must verify live before sending"
}

test_fm_prefixed_herdr_session_is_an_explicit_target() {
  local dir fb home err log herdr_log rc
  dir="$TMP_ROOT/sq-remote-explicit"; mkdir -p "$dir"
  ...
  assert_grep 'pane send-keys w1:p2 enter --session sq-remote' "$herdr_log" "sq-prefixed Herdr target was not sent its key in its session"
  assert_no_grep '--session default' "$herdr_log" "sq-prefixed Herdr target fell back to the default session"
  pass "sq-send strict: sq-prefixed Herdr sessions remain explicit backend targets"
}
...
test_healthy_fm_id_send_still_works() {
  ...
  got=$(cat "$log")
  assert_contains "$got" "target=sess:sq-lane-ok literal=1 arg=hello commander" "healthy send should type literal text to the meta target"
  assert_contains "$got" "target=sess:sq-lane-ok literal=0 arg=Enter" "healthy send should submit with Enter"
  assert_contains "$(cat "$err")" "requested message WILL still be sent" "sq-send guard banner should keep send-specific continuation wording"
  pass "sq-send strict: healthy sq-<id> sends still type once and submit"
}

# A --key send is how Squad interrupts a worker, so its exit status is the
...
test_key_send_exit_status_follows_delivery() {
  ...
  assert_contains "$(cat "$log")" "target=sess:sq-lane-key literal=0 arg=Escape" "the undelivered case should still have attempted the send"
