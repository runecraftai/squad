...
func init() {
	...
	if os.Getenv("DRILL_HOOK_HELPER") == "1" {
	...
	if os.Getenv("DRILL_DAEMON_HELPER_PROCESS") == "bootstrap-sink" {
		root, ok := explicitDaemonLogSinkRootFromArgs(os.Args[1:])
		if !ok {
			os.Exit(1)
		}
		_ = os.Setenv("DRILL_HOME", root)
	...
	if root, ok := explicitDaemonRunRootFromArgs(os.Args[1:]); ok && root != "" {
		_ = os.Setenv("DRILL_HOME", root)
	} else if os.Getenv("DRILL_DAEMON") != "1" {
		return
	}
...
func TestMain(m *testing.M) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "create test DRILL_HOME: %v\n", err)
		os.Exit(1)
	}
	...
	_ = os.Setenv("DRILL_TELEMETRY", "off")
	_ = os.Setenv("DRILL_NO_UPDATE_CHECK", "1")
...
// sets DRILL_HOME to an isolated temp dir. Returns the repo path and a cleanup
// function that restores the original working directory and DRILL_HOME.
func setupTestRepo(t *testing.T) string {
	t.Helper()

	// Keep DRILL_HOME under a short temp root so the daemon socket path fits.
	repoDir := t.TempDir()
...
func cleanupWorktree(t *testing.T, repoDir, wtDir string) {
	...
	t.Cleanup(func() {
		_ = os.Chdir(repoDir)
		p := paths.WithRoot(os.Getenv("DRILL_HOME"))
		_ = daemon.Stop(p)
		if runtime.GOOS == "windows" {
			time.Sleep(500 * time.Millisecond)
		}
