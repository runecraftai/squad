...
// MatchesDaemonRoot reports whether pid's argv is a drill daemon run
// for exactly the given DRILL_HOME root (bounded ownership check).
func MatchesDaemonRoot(pid int, drillHome string) bool {
	cmd, err := processCommandLine(pid)
	if err != nil || cmd == "" {
		return false
	}
	return commandMatchesDaemonRoot(cmd, drillHome)
}

func commandMatchesDaemonRoot(command, drillHome string) bool {
	...
	return samePath(root, drillHome)
}
