...
/**
 * Dependency edge types. Squad uses only `blocked-by` today; `parent` and
...
export interface Dep {
  ...
  id: string;
  /**
   * Free-text rationale carried by Squad's `blocked-by: <id> - <reason>`
...
export interface Task {
  ...
  links: TaskLink[];
  /** Typed dependency edges (Squad uses blocked-by today). */
  deps: Dep[];
  /** Structured dispatch hold. Active holds keep queued work out of ready. */
  hold?: Hold;
  /** 0-4, optional (borrowed from beads; Squad orders by list position). */
  priority?: number;
  /** Maps to `(since ...)`. */
  created?: string;
  updated?: string;
