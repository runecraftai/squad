...
export async function serve({
  ...
  const logEvent = verbose ? (line) => writeLog(`[sq-report] ${line}`) : null;
  let publicPort = port;
    ...
    try {
      ...
      res.type("html").send(
        createChromeHtml(session, {
          ...
          faviconTag,
          title: title ? `${title} · sq-report` : "sq-report",
    ...
    try {
      ...
      if (!beforeRead.valid) {
        res
          ...
          .type("html")
          .send(
            "<!doctype html><title>Artifact load expired</title><p>This artifact load is no longer current. Reload sq-report to continue.</p>",
          );
        return;
      }
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>sq-report Whiteboard</title>
