...
// /drill skill) drives; humans use the bare `drill` TUI instead.
func newAxiCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "axi",
		Short: "Agent interface: drive drill from an autonomous agent",
		Long: "Agent eXperience Interface for drill. Prints token-efficient TOON\n" +
			"to stdout and is driven entirely by flags (no interactive prompts).\n" +
			"Running `drill axi` with no subcommand shows the current state.",
		Args:          cobra.NoArgs,
		SilenceErrors: true,
		SilenceUsage:  true,
...
func runAxiHome(cmd *cobra.Command) (string, error) {
	...
	case currentActive == nil:
		help = append(help, `Run `+"`"+`drill axi run --intent "<what the user set out to accomplish>"`+"`"+` to validate your changes`)
		if otherActive != nil {
	...
	case gated:
		help = append(help, "Run `drill axi respond --action approve` to clear the current gate")
	default:
		help = append(help, "Run `drill axi status` to inspect the active run")
	}
	if hasBranchSync {
		help = append(help, branchSyncAgentGuidance)
	}
	help = append(help, preserveGateFixCommitsGuidance)
	help = append(help, "The calling agent drives AXI gates but does not replace the configured pipeline agent; run `drill doctor` if no native agent or ACP runner is available")
	help = append(help, "How to drive the pipeline: `drill axi run --help`, or the `/drill` skill (loaded when you invoke `/drill`)")
...
func repoInitHelp(err error) []string {
	if err != nil && strings.Contains(err.Error(), "not initialized") {
		return []string{"Run `drill init` to set up the gate in this repository"}
	}
	return nil
}
