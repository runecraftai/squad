...
export const SETUP_HELP = `usage: sq-tasks setup hooks
...
export async function setupCommand(args: string[]): Promise<string> {
  ...
  if (failures.length > 0) {
    return renderOutput([
      ...
      renderHelp(["Fix the listed files and rerun `sq-tasks setup hooks`"]),
    ]);
  }
  ...
  return renderOutput([
    "hooks:\n  status: installed\n  integrations: Claude Code, Codex, OpenCode",
    renderHelp([
      "Restart your agent session to receive sq-tasks ambient context",
    ]),
  ]);
}
