#!/usr/bin/env bash
# tests/sq-backend-herdr.test.sh - fake-herdr-CLI unit tests for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/sq-backend-design-d7 (herdr-addendum.md). Mirrors tests/sq-backend.test.sh's
...
# The real-binary smoke test lives in tests/sq-backend-herdr-smoke.test.sh,
...
TMP_ROOT=$(fm_test_tmproot sq-backend-herdr-tests)
export SQUAD_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0

# make_herdr_fakebin: a `herdr` stub that logs every invocation (one line,
# unit-separated args, to $SQUAD_HERDR_LOG) and returns the canned response for
# that call read from $SQUAD_HERDR_RESPONSES/<n>.out, consumed IN ORDER (call 1
...
if [ "${1:-}" = status ] && [ "${2:-}" = --json ] && [ "${SQUAD_HERDR_SCRIPT_STATUS:-0}" != 1 ]; then
...
# repeatedly and the "one persistent Squad workspace, no orphans"
...
# ($SQUAD_FAKE_HERDR_STATE) mutated with real jq. Modeled behaviors, all
...
# a pane with no registered agent). Every call is logged to $SQUAD_HERDR_LOG in
...
# herdr_case <name> -> sets up SQUAD_HERDR_LOG/SQUAD_HERDR_RESPONSES/fb for one test,
...
test_version_check_accepts_current_protocol() {
  ...
  PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
...
# --- workspace_label: per-Squad-HOME resolution (P3, herdr-sm-spaces-k4) -
...
test_workspace_label_XO_home_uses_marker_id() {
  local home
...
test_workspace_label_different_XOs_get_different_labels() {
...
test_cli_helper_sets_env_and_appends_trailing_session_flag() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
...
# every process it manages a pane for, so a Squad or XO agent's own
# tool calls carry the identity of the workspace the commander is watching it in.
...
test_launcher_identity_absent_without_a_herdr_pane() {
  ...
  pass "fm_backend_herdr_launcher_identity: a Squad not running inside herdr has no launcher workspace to inherit"
}
...
test_launcher_identity_absent_when_herdr_env_alone_is_set() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  ( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" HERDR_ENV=1 \
    \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_launcher_identity fmtest' "$ROOT" )
  status=$?
...
test_launcher_identity_resolves_the_exact_pane_tab_and_workspace() {
  ...
  printf '{"sessions":[{"name":"fmtest","running":true,"socket_path":"/tmp/sq-herdr-unit/fmtest.sock"}]}\n' > "$resp/1.out"
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad"},{"workspace_id":"w7","label":"Squad"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/sq-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_launcher_identity fmtest || exit 1
      printf "%s|%s|%s" "$SQUAD_BACKEND_HERDR_LAUNCHER_PANE_ID" "$SQUAD_BACKEND_HERDR_LAUNCHER_TAB_ID" "$SQUAD_BACKEND_HERDR_LAUNCHER_WORKSPACE_ID"' "$ROOT" )
  [ "$out" = 'w7:p3|w7:t3|w7' ] \
...
test_launcher_identity_refuses_a_pane_from_another_server_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/sq-herdr-unit/other.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
  status=$?
...
test_launcher_identity_refuses_a_workspace_missing_from_the_session() {
  ...
  printf '{"result":{"pane":{"pane_id":"w7:p3","tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/2.out"
  printf '{"result":{"tab":{"tab_id":"w7:t3","workspace_id":"w7"}}}\n' > "$resp/3.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad"}]}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest HERDR_SOCKET_PATH=/tmp/sq-herdr-unit/fmtest.sock \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_launcher_identity fmtest' "$ROOT" 2>&1 )
...
test_workspace_ensure_refuses_an_ambiguous_label_with_no_launcher() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad"},{"workspace_id":"w7","label":"Squad"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
  ...
  assert_contains "$out" "labeled 'Squad'" "the ambiguity refusal did not name the duplicated label"
...
test_workspace_ensure_other_home_ignores_the_launcher_identity() {
  ...
  # --xo launch stands up a different home's workspace by design.
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    HERDR_ENV=1 HERDR_PANE_ID=w7:p3 HERDR_SESSION=fmtest \
  ...
  pass "fm_backend_herdr_workspace_ensure: a --xo container resolves that home's own workspace, not the launcher's"
}
...
test_container_ensure_refuses_an_ambiguous_home_label() {
  ...
  assert_contains "$out" "labeled 'Squad'" "container_ensure buried the specific ambiguity it refused"
...
test_container_ensure_starts_server_and_workspace() {
  ...
  # 5: workspace list -> empty (no "Squad" workspace yet)
  ...
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"Squad"},"tab":{"tab_id":"w1:t9"},"root_pane":{"pane_id":"w1:p9"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
  ...
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''Squad' \
    "container_ensure did not create the Squad workspace with the given cwd"
  pass "fm_backend_herdr_container_ensure: version-gates, starts the server, ensures the Squad workspace, echoes session:workspace_id + the seeded default tab id"
}
...
test_container_ensure_reuses_existing_workspace() {
  ...
  printf '{"client":{"version":"0.7.1","protocol":14}}\n' > "$resp/1.out"
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[{"workspace_id":"w9","label":"Squad"}]}}\n' > "$resp/3.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w9\t' ] || fail "container_ensure should reuse the existing Squad workspace id with an EMPTY seeded-tab field (an ADOPTED workspace is never a prune candidate), got '$out'"
  assert_not_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create' "container_ensure should not create a workspace that already exists"
  pass "fm_backend_herdr_container_ensure: reuses an existing Squad workspace without recreating it, and reports no seeded default tab (adopted, not created)"
}

test_create_task_refuses_duplicate_label() {
  local dir log resp fb out status
  dir="$TMP_ROOT/dup-task"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-dup1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
# panes) across a server restart, including a reboot. A restored sq-<id> task
# tab comes back a HUSK - a dead pane, or a plain agent-less shell sitting in
# the saved cwd - never the operator that used to be there. Before this fix,
# create_task refused ANY same-labeled tab unconditionally, so every unit
...
test_create_task_refuses_when_any_duplicate_label_is_live() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-mixed1","workspace_id":"w1"},{"tab_id":"w1:t3","label":"sq-mixed1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-mixed1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
test_create_task_closes_and_replaces_dead_pane_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-husk1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"sq-husk1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-husk1 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''sq-husk1' \
...
test_create_task_closes_and_replaces_no_agent_husk() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-husk2","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  ...
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"sq-husk2","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-husk2 /tmp/proj' "$ROOT" ) \
...
EOF
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''sq-husk2' \
    "create_task did not create the replacement tab"
...
test_create_task_closes_all_duplicate_husks_after_replacement() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-husk-many","workspace_id":"w1"},{"tab_id":"w1:t3","label":"sq-husk-many","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"},{"pane_id":"w1:p3","tab_id":"w1:t3"}]}}\n' > "$resp/5.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t4","label":"sq-husk-many","workspace_id":"w1"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-husk-many /tmp/proj' "$ROOT" ) \
...
test_create_task_refuses_when_preexisting_husk_tab_remains() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  printf '{"error":{"code":"agent_not_found","message":"agent target w1:p2 not found"}}\n' > "$resp/4.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/5.out"
  printf '1\n' > "$resp/6.exit"
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-stale-husk","workspace_id":"w1"},{"tab_id":"w1:t3","label":"sq-stale-husk","workspace_id":"w1"}]}}\n' > "$resp/7.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-stale-husk /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
test_create_task_refuses_when_agent_state_ambiguous() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-ambig1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p2"}}}\n' > "$resp/3.out"
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-ambig1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
test_create_task_husk_replacement_creates_before_closing() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w1:t2","label":"sq-order1","workspace_id":"w1"}]}}\n' > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p2","tab_id":"w1:t2"}]}}\n' > "$resp/2.out"
  printf '{"error":{"code":"pane_not_found","message":"pane w1:p2 not found"}}\n' > "$resp/3.out"
  printf '{"result":{"tab":{"tab_id":"w1:t3"},"root_pane":{"pane_id":"w1:p3"}}}\n' > "$resp/4.out"
  printf '{"result":{"tabs":[{"tab_id":"w1:t3","label":"sq-order1","workspace_id":"w1"}]}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-order1 /tmp/proj' "$ROOT" ) \
...
test_create_task_creates_and_parses_ids() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task fmtest:w1 sq-newtask /tmp/proj' "$ROOT" )
  [ "$out" = "w1:t2 w1:p2" ] || fail "create_task should echo '<tab_id> <pane_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''sq-newtask' \
...
test_container_ensure_creates_with_no_focus_flag() {
  ...
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[]}}\n' > "$resp/3.out"
  printf '{"result":{"workspace":{"workspace_id":"w1","label":"Squad"},"tab":{"tab_id":"w1:t1"},"root_pane":{"pane_id":"w1:p1"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_container_ensure /tmp' "$ROOT" )
  [ "$out" = $'fmtest:w1\tw1:t1' ] || fail "container_ensure should still echo '<session>:<workspace_id>\\t<seeded_default_tab_id>', got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''workspace'$'\x1f''create'$'\x1f''--cwd'$'\x1f''/tmp'$'\x1f''--label'$'\x1f''Squad'$'\x1f''--no-focus' \
    "container_ensure's workspace create did not pass --no-focus (focus-safety: never steal the commander's attention on spawn)"
  pass "fm_backend_herdr_container_ensure: workspace create passes --no-focus"
}

test_container_ensure_uses_XO_home_label() {
  local dir log resp fb out home
  dir="$TMP_ROOT/container-XO-label"; mkdir -p "$dir/responses"; log="$dir/log"; resp="$dir/responses"; : > "$log"
  home="$TMP_ROOT/container-XO-home"; mkdir -p "$home"; printf 'sshhip-h7\n' > "$home/.sq-xo-home"
  printf '{"client":{"version":"0.7.1","protocol":14}}\n' > "$resp/1.out"
  printf '{"server":{"running":true}}\n' > "$resp/2.out"
  printf '{"result":{"workspaces":[]}}\n' > "$resp/3.out"
...
test_create_task_creates_with_no_focus_flag() {
  ...
  assert_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''create'$'\x1f''--workspace'$'\x1f''w1'$'\x1f''--cwd'$'\x1f''/tmp/proj'$'\x1f''--label'$'\x1f''sq-newtask'$'\x1f''--no-focus' \
...
# fm_backend_herdr_presentation_enabled is the one gate bin/sq-spawn.sh consults
# before projecting an operator or recon, so these cases pin the default-on
...
test_projection_journal_v2_binds_and_advances_exact_endpoint() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    token=$(fm_backend_herdr_projection_journal_create "$1" sq-hibit-r1) || exit 1
    journal="$1/sq-hibit-r1.herdr-presentation"
    home=$(fm_backend_herdr_projection_home_identity "$2") || exit 1
    label=$(fm_backend_herdr_projection_workspace_label sq-hibit-r1 "$token")
    fm_backend_herdr_projection_journal_bind \
      "$journal" sq-hibit-r1 "$home" lab-session w2 w2:t2 w2:p2 w1 Squad "$label" sq-sq-hibit-r1 || exit 1
    fm_backend_herdr_projection_journal_snapshot "$journal" sq-hibit-r1 || exit 1
    printf "%s|%s|%s|%s|%s|%s|%s\n" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_VERSION" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_HOME" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_WORKSPACE_ID" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_TAB_ID" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_PANE_ID" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_PARENT_WORKSPACE_ID" \
      "$SQUAD_BACKEND_HERDR_JOURNAL_WORKSPACE_LABEL"
    fm_backend_herdr_projection_journal_replace_endpoint \
      "$journal" sq-hibit-r1 w2:t2 w2:p2 w2:t3 w2:p3 || exit 1
    fm_backend_herdr_projection_journal_snapshot "$journal" sq-hibit-r1 || exit 1
    printf "%s|%s\n" "$SQUAD_BACKEND_HERDR_JOURNAL_TAB_ID" "$SQUAD_BACKEND_HERDR_JOURNAL_PANE_ID"
  ' "$ROOT" "$state" "$home") || fail "version 2 projection journal binding failed"
  token=$(sed -n 's/^projection_id=//p' "$state/sq-hibit-r1.herdr-presentation")
  ...
  [ "$(wc -l < "$state/sq-hibit-r1.herdr-presentation" | tr -d '[:space:]')" = 12 ] \
    || fail "version 2 projection journal must have exactly 12 fields"
  printf 'pane_id=duplicate\n' >> "$state/sq-hibit-r1.herdr-presentation"
  if bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_journal_snapshot "$1" sq-hibit-r1' \
    "$ROOT" "$state/sq-hibit-r1.herdr-presentation"; then
...
test_projection_create_uses_exact_response_ids_and_leaves_one_task_pane() {
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"sq-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  ...
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"sq-task-p2","workspace_id":"w9"}]}}\n' > "$resp/10.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_projection_focus_snapshot() { printf "commander-ws\tcommander-tab"; }
      ...
      fm_backend_herdr_projection_create_task /tmp/proj "$label" sq-task-p2 || exit 1
      printf "%s %s %s %s %s\n" \
        "$SQUAD_BACKEND_HERDR_PROJECTION_WORKSPACE_ID" \
        "$SQUAD_BACKEND_HERDR_PROJECTION_SEEDED_TAB_ID" \
        "$SQUAD_BACKEND_HERDR_PROJECTION_SEEDED_PANE_ID" \
        "$SQUAD_BACKEND_HERDR_PROJECTION_TAB_ID" \
        "$SQUAD_BACKEND_HERDR_PROJECTION_PANE_ID"
  ...
  assert_contains "$(cat "$log")" $'tab\x1fcreate\x1f--workspace\x1fw9\x1f--cwd\x1f/tmp/proj\x1f--label\x1fsq-task-p2\x1f--no-focus' \
...
test_projection_create_never_closes_a_concurrent_same_label_tab() {
  ...
  printf '{"result":{"workspace":{"workspace_id":"w9"},"tab":{"tab_id":"w9:t1"},"root_pane":{"pane_id":"w9:p1"}}}\n' > "$resp/1.out"
  printf '{"result":{"tab":{"tab_id":"w9:t2"},"root_pane":{"pane_id":"w9:p2"}}}\n' > "$resp/2.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"sq-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"sq-task-p2","workspace_id":"w9"}]}}\n' > "$resp/3.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p1","tab_id":"w9:t1"},{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/4.out"
  printf '{"error":{"code":"agent_not_found"}}\n' > "$resp/5.out"
  printf '{"result":{"pane":{"pane_id":"w9:p1","tab_id":"w9:t1","workspace_id":"w9"}}}\n' > "$resp/6.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9"},{"tab_id":"w9:t2","label":"sq-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"sq-task-p2","workspace_id":"w9"}]}}\n' > "$resp/7.out"
  printf '{"error":{"code":"pane_not_found"}}\n' > "$resp/9.out"
  printf '{"result":{"tabs":[{"tab_id":"w9:t2","label":"sq-task-p2","workspace_id":"w9"},{"tab_id":"w9:t3","label":"sq-task-p2","workspace_id":"w9"}]}}\n' > "$resp/10.out"
  printf '{"result":{"panes":[{"pane_id":"w9:p2","tab_id":"w9:t2"},{"pane_id":"w9:p3","tab_id":"w9:t3"}]}}\n' > "$resp/11.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_focus_snapshot() { printf "commander-ws\tcommander-tab"; }; fm_backend_herdr_projection_focus_restore() { return 0; }; fm_backend_herdr_projection_create_task /tmp/proj label sq-task-p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_refuses_active_tab() {
  ...
  assert_contains "$out" "target is the commander's active tab" \
  ...
  pass "herdr presentation focus: cleanup refuses rather than close the commander's active tab"
}
...
test_projection_close_rechecks_required_agent_state_at_boundary() {
  ...
  out=$(ROOT="$ROOT" LOG="$log" bash -c '
    ...
    rc=$?
    set -e
    printf "%s:%s" "$rc" "$SQUAD_BACKEND_HERDR_PROJECTION_CLOSE_AGENT_STATE"
  ')
  status=${out%%:*}
...
# the pane-death close fixtures. The mover appends to $SQUAD_FAKE_MOVER_LOG and
# exits 9 unless $SQUAD_FAKE_MOVER_RESPONSE names a readable response file.
...
calls=$(wc -l < "$SQUAD_FAKE_MOVER_LOG" | tr -d ' ')
if [ "$calls" -ge 2 ] && [ -f "${SQUAD_FAKE_MOVER_RESPONSE_2:-}" ]; then
  cat "$SQUAD_FAKE_MOVER_RESPONSE_2"
  exit 0
fi
if [ -f "$SQUAD_FAKE_MOVER_RESPONSE" ]; then
  cat "$SQUAD_FAKE_MOVER_RESPONSE"
  exit 0
fi
exit 9
SH
...
test_projection_close_emptying_after_focus_uses_pane_death_without_move() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/no-response" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
...
test_projection_close_emptying_before_focus_repositions_then_uses_pane_death() {
  ...
  make_death_lab "$dir" "$bgpid"
  printf '%s\n' '{"id":"sq-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false},{"workspace_id":"w1","focused":false}]}}' > "$dir/mover-response"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/mover-response" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
...
test_projection_close_busy_pane_falls_back_to_plain_close() {
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/no-response" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2' "$ROOT" 2>&1)
  status=$?
...
test_projection_close_transient_prompt_helper_settles_then_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/no-response" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 SQUAD_BACKEND_HERDR_IDLE_SHELL_PROOF_POLLS=3 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_close_pane_focus_preserving fmtest w2:p2' "$ROOT" 2>&1)
  status=$?
  kill "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
assert_projection_close_failed_removal_rolls_back_the_reposition() {
  ...
  make_death_lab "$dir" "$bgpid"
  printf '%s\n' '{"id":"sq-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false},{"workspace_id":"w1","focused":false}]}}' > "$dir/mover-response"
  printf '%s\n' '{"id":"sq-workspace-move","result":{"type":"workspace_list","workspaces":[{"workspace_id":"w1","focused":false},{"workspace_id":"w2","focused":true},{"workspace_id":"w3","focused":false}]}}' > "$dir/mover-response-2"
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/mover-response" \
    SQUAD_FAKE_MOVER_RESPONSE_2="$dir/mover-response-2" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_close_pane_focus_preserving fmtest w1:p1' "$ROOT" 2>&1)
  status=$?
  kill -KILL "$bgpid" 2>/dev/null || true; wait "$bgpid" 2>/dev/null || true
...
test_kill_emptying_non_focused_uses_pane_death() {
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_HERDR_PS_BIN="$dir/ps" SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$dir/mover" \
    SQUAD_FAKE_MOVER_LOG="$dir/mover.log" SQUAD_FAKE_MOVER_RESPONSE="$dir/no-response" \
    SQUAD_BACKEND_HERDR_DEATH_CLOSE_POLLS=2 SQUAD_FAKE_LOCK_LOG="$lock_log" \
    SQUAD_FAKE_LOCK_HELD="$lock_held" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_target_ready() { fm_backend_herdr_parse_target "$1"; }
      fm_backend_herdr_presentation_session_lock_path() { printf "%s" "$SQUAD_FAKE_LOCK_HELD.lock"; }
      fm_lock_try_acquire() {
        printf "acquire\n" >> "$SQUAD_FAKE_LOCK_LOG"
        : > "$SQUAD_FAKE_LOCK_HELD"
      }
      fm_lock_release() {
        [ -e "$SQUAD_FAKE_LOCK_HELD" ] || return 1
        rm -f "$SQUAD_FAKE_LOCK_HELD"
        printf "release\n" >> "$SQUAD_FAKE_LOCK_LOG"
      }
      eval "$(declare -f fm_backend_herdr_cli | sed "1s/fm_backend_herdr_cli/fm_backend_herdr_cli_locked/")"
      fm_backend_herdr_cli() {
        [ -e "$SQUAD_FAKE_LOCK_HELD" ] || return 97
...
test_kill_focused_workspace_stays_plain_close() {
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    ...
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_presentation_session_lock_path() { printf "/tmp/sq-herdr-test-lock"; }
  ...
  for mode in unresolved contended; do
    ...
    out=$(ROOT="$ROOT" MODE="$mode" CLI_LOG="$dir/cli.log" ATTEMPTS="$dir/attempts" bash -c '
      ...
      fm_backend_herdr_presentation_session_lock_path() {
        ...
        printf "/tmp/sq-herdr-contended-test-lock"
      }
      fm_lock_try_acquire() {
        printf "x\n" >> "$ATTEMPTS"
        return 1
...
test_endpoint_confirmed_gone_gates_on_structured_presence() {
  ...
  out=$(bash -c '
    . "$0/bin/backends/herdr.sh"
    fm_backend_herdr_cli() { printf "%s\n" "$SQUAD_FAKE_PRESENCE_RESPONSE"; return "${SQUAD_FAKE_PRESENCE_STATUS:-0}"; }
    check() {  # <label> <response> <status> <mode> <expected-rc>
      SQUAD_FAKE_PRESENCE_RESPONSE=$2 SQUAD_FAKE_PRESENCE_STATUS=$3
      rc=0
...
test_projection_seeded_prune_refuses_active_tab() {
  ...
  log="$dir/log"; resp="$dir/responses"; : > "$log"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w9:t1","label":"1","workspace_id":"w9","focused":true},{"tab_id":"w9:t2","label":"sq-task","workspace_id":"w9","focused":false}]}}' > "$resp/1.out"
  ...
  assert_not_contains "$(cat "$log")" $'pane\x1fclose' \
    "projected seeded prune closed the commander's active tab"
...
test_projection_label_builder_uses_corner_and_strips_owner_prefixes() {
  ...
  XO=$(bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_workspace_label XO-child-demo '"$token" "$ROOT")
  [ "$XO" = "└ XO-child-demo · p:$token" ] \
    || fail "XO child label was wrong: $XO"
  ...
  pass "herdr presentation labels: └ concise-task · p:<full-token> for primary and XO children"
}
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    ...
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t2"; }; fm_backend_herdr_projection_focus_restore() { return 0; }; fm_backend_herdr_projection_order_best_effort fmtest w5 Squad' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_XO_parent_block() {
  local dir log resp fb mover mover_log out status
...
SH
  ...
  [ "$(cat "$mover_log")" = "$(cd /tmp && pwd -P)/fmtest.sock"$'\t'"w6"$'\t'"4" ] \
    || fail "XO child was not inserted after its parent block: $(cat "$mover_log")"
  assert_not_contains "$(cat "$log")" $'workspace\x1frename' "XO ordering renamed a legacy child"
  pass "herdr presentation ordering: XO children append under their owning parent block"
}
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_focus_snapshot() { printf "w4\tw4:t1"; }; fm_backend_herdr_projection_focus_restore() { return 0; }; fm_backend_herdr_projection_order_best_effort fmtest w6 Squad' "$ROOT" 2>&1)
  status=$?
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_LOG="$mover_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_focus_snapshot() { printf "w2\tw2:t1"; }; fm_backend_herdr_projection_focus_restore() { return 0; }; fm_backend_herdr_projection_order_best_effort fmtest w3 Squad' "$ROOT" 2>&1)
  status=$?
...
SH
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_focus_snapshot() { printf "w1\tw1:t1"; }; fm_backend_herdr_projection_focus_restore() { return 0; }; fm_backend_herdr_projection_order_best_effort fmtest w3 Squad' "$ROOT" 2>&1)
  status=$?
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_order_best_effort fmtest w4 Squad' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_anchors_the_parent_by_exact_id() {
  ...
  layout='{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad","focused":false},{"workspace_id":"w7","label":"Squad","focused":false},{"workspace_id":"wH","label":"human-notes","focused":false},{"workspace_id":"w8","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe","focused":false}]}}'
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_order_best_effort fmtest w8 Squad' "$ROOT" 2>&1)
  status=$?
  [ "$status" -eq 0 ] || fail "ambiguous projection ordering must not fail the spawn"
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_order_best_effort fmtest w8 Squad w7' "$ROOT" 2>&1)
  status=$?
...
test_projection_order_missing_parent_is_read_only() {
  ...
  log="$dir/log"; resp="$dir/responses"; mover="$dir/mover"; : > "$log"
  printf '%s\n' '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad"},{"workspace_id":"w2","label":"└ new · p:ZyXwVuTsRqPoNmLkJiHgFe"}]}}' > "$resp/1.out"
  cat > "$mover" <<'SH'
#!/usr/bin/env bash
echo called > "$SQUAD_FAKE_MOVER_CALLED"
exit 0
...
test_presentation_session_lock_path_is_shared_across_homes() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path_a=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
  ...
  path_b=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_presentation_session_lock_path fmtest' "$ROOT") \
  ...
  case "$path_a" in
    /tmp/Squad-herdr-presentation/order-*.lock) ;;
  ...
  path_other=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
  ...
  if [ -L /tmp ] || [ "$(cd /tmp && pwd -P)" != /tmp ]; then
    : > /tmp/sq-herdr-lock-canon-$$.sock
    printf '%s\n' '{"sessions":[{"name":"canon","running":true,"socket_path":"/tmp/sq-herdr-lock-canon-'"$$"'.sock"}]}' > "$resp/4.out"
    printf '%s\n' "{\"sessions\":[{\"name\":\"canon\",\"running\":true,\"socket_path\":\"$(cd /tmp && pwd -P)/sq-herdr-lock-canon-$$.sock\"}]}" > "$resp/5.out"
    path_tmp=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    ...
    path_private=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
      bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_presentation_session_lock_path canon' "$ROOT") \
      || fail "lock path with canonical socket failed"
    rm -f /tmp/sq-herdr-lock-canon-$$.sock
...
test_presentation_session_lock_path_rejects_malformed_socket() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  path=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
...
SH
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_HERDR_SCRIPT_STATUS=1 \
    SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$mover" SQUAD_FAKE_MOVER_CALLED="$dir/called" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_projection_order_best_effort fmtest w2 Squad' "$ROOT" 2>&1)
  status=$?
...
test_projection_reclaim_refusal_matrix_is_non_mutating() {
  ...
  token=$(bash -c '
    ...
    fm_backend_herdr_projection_journal_bind \
      ...
      w2 w2:t2 w2:p2 w1 Squad "$label" sq-refusal-r1 || exit 1
    printf "%s" "$token"
    ...
    bash -c '
      ...
      run_case() {
        ...
        fm_backend_herdr_projection_reclaim_task \
          fmtest "$journal" refusal-r1 "$home" w2 w2:t2 w2:p2 Squad sq-refusal-r1 /tmp/project \
          >/dev/null 2>&1
        rc=$?
        set -e
        printf "%s:%s\n" "$mode" "$rc"
...
test_projection_reclaim_replaces_only_exact_husk_and_advances_binding() {
  ...
  token=$(bash -c '
    ...
    label=$(fm_backend_herdr_projection_workspace_label sq-hibit-r1 "$token")
    fm_backend_herdr_projection_journal_bind \
      "$1/sq-hibit-r1.herdr-presentation" sq-hibit-r1 "$2" fmtest \
      w2 w2:t2 w2:p2 w1 Squad "$label" sq-sq-hibit-r1 || exit 1
    printf "%s" "$token"
  ' "$ROOT" "$state" "$home_real") || fail "could not create exact reclaim journal fixture"
  journal="$state/sq-hibit-r1.herdr-presentation"
  label="└ hibit-r1 · p:$token"
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w0\",\"label\":\"Squad\",\"focused\":false,\"active_tab_id\":\"w0:t1\"},{\"workspace_id\":\"w1\",\"label\":\"Squad\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/1.out"
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t2","label":"sq-sq-hibit-r1"}]}}' > "$resp/2.out"
  ...
  printf '%s\n' "{\"result\":{\"workspaces\":[{\"workspace_id\":\"w1\",\"label\":\"Squad\",\"focused\":true,\"active_tab_id\":\"w1:t1\"},{\"workspace_id\":\"w2\",\"label\":\"$label\",\"focused\":false,\"active_tab_id\":\"w2:t2\"}]}}" > "$resp/6.out"
  ...
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t2","label":"sq-sq-hibit-r1"},{"tab_id":"w2:t3","label":"sq-sq-hibit-r1"}]}}' > "$resp/20.out"
  : > "$resp/21.out"
  ...
  printf '%s\n' '{"result":{"tabs":[{"tab_id":"w2:t3","label":"sq-sq-hibit-r1"}]}}' > "$resp/27.out"
  ...
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '
      . "$0/bin/backends/herdr.sh"
      fm_backend_herdr_projection_reclaim_task \
        fmtest "$1" sq-hibit-r1 "$2" w2 w2:t2 w2:p2 Squad sq-sq-hibit-r1 /tmp/project || exit 1
      printf "%s %s" "$SQUAD_BACKEND_HERDR_PROJECTION_TAB_ID" "$SQUAD_BACKEND_HERDR_PROJECTION_PANE_ID"
...
test_projection_recovery_is_read_only_and_refuses_live_duplicate_risk() {
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad/task-p3 · p:%s"},{"workspace_id":"w2","label":"copy/task-p3 · p:%s"}]}}\n' "$token" "$token" > "$resp/1.out"
  ...
  printf '{"result":{"workspaces":[{"workspace_id":"w1","label":"Squad/task-p3 · p:%s"}]}}\n' "$token" > "$resp/1.out"
  printf '{"result":{"panes":[{"pane_id":"w1:p1","tab_id":"w1:t1"}]}}\n' > "$resp/2.out"
  printf '{"result":{"pane":{"pane_id":"w1:p1"}}}\n' > "$resp/3.out"
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/4.out"
  out=$(PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
...
test_list_live_scoped_to_this_homes_workspace_only() {
  ...
  assert_not_contains "$(cat "$log")" $'\x1f''tab'$'\x1f''list'$'\x1f''--workspace'$'\x1f''w1' \
    "list_live must never query the primary's (or a sibling XO's) workspace"
...
test_parse_target() {
  ( . "$ROOT/bin/backends/herdr.sh"
    ...
    [ "$SQUAD_BACKEND_HERDR_SESSION" = default ] || { echo "session mismatch: $SQUAD_BACKEND_HERDR_SESSION" >&2; exit 1; }
    [ "$SQUAD_BACKEND_HERDR_PANE" = "w1:p2" ] || { echo "pane mismatch: $SQUAD_BACKEND_HERDR_PANE" >&2; exit 1; }
...
test_normalize_key() {
  ...
  ) || fail "fm_backend_herdr_normalize_key did not map Squad's key vocabulary to herdr's verified names"
...
test_current_path_reads_cwd() {
  ...
  # running process (e.g. a fob get subshell) and is what must be read.
...
# bin/sq-supervise-daemon.sh's fm_backend_herdr_send_text_submit to never
...
test_wait_for_working_samples_budget_endpoint_without_final_sleep() {
  ...
  printf '{"result":{"agent":{"agent_status":"idle"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_SLEEP_LOG="$sleep_log" \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$SQUAD_SLEEP_LOG"; }; fm_backend_herdr_wait_for_working default w1:p2 0.5 6' "$ROOT" )
...
test_send_text_submit_applies_herdr_minimum_confirm_budget() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/9.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_SLEEP_LOG="$sleep_log" SQUAD_BACKEND_HERDR_SUBMIT_POLLS=6 SQUAD_BACKEND_HERDR_SUBMIT_MIN_SLEEP=0.6 \
    bash -c '. "$0/bin/backends/herdr.sh"; sleep() { printf "sleep:%s\n" "$1" >> "$SQUAD_SLEEP_LOG"; }; fm_backend_herdr_send_text_submit default:w1:p2 "hello commander" 1 0.4 0' "$ROOT" )
...
test_wait_for_working_treats_blocked_as_submit_active() {
  ...
  pass "fm_backend_herdr_wait_for_working: treats blocked as submit-active for confirmation without changing sentry busy-state semantics"
}
...
# SQUAD_BACKEND_HERDR_SUBMIT_POLLS=1 pins most tests
...
test_send_text_submit_detects_landed_send() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/4.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_send_text_submit default:w1:p2 "hello commander" 3 0.01 0.01' "$ROOT" )
  [ "$out" = empty ] || fail "send_text_submit should report empty (submitted) once agent_status reports working, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''send-text'$'\x1f''w1:p2'$'\x1f''hello commander' "send_text_submit did not type the literal text first"
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_BACKEND_HERDR_SUBMIT_POLLS=1 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_send_text_submit default:w1:p2 "hello commander" 2 0.01 0.01' "$ROOT" )
...
test_send_text_submit_preexisting_working_does_not_false_confirm_swallowed_enter() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/2.out"
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/3.out"
  printf '  \xe2\x9d\xaf hello commander\n' > "$resp/4.out"
  printf '  \xe2\x9d\xaf hello commander\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_send_text_submit default:w1:p2 "hello commander" 2 0.01 0.01' "$ROOT" )
...
# (bin/sq-supervise-daemon.sh's pane_input_pending): a real Codex idle
...
# (bin/sq-supervise-daemon.sh's pane_input_pending, dispatched via
...
test_composer_state_guard_still_refuses_real_pending_text_after_submit_confirmation_change() {
  ...
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    bash -c '. "$0/bin/sq-backend.sh"; fm_backend_composer_state herdr default:w1:p2' "$ROOT" )
...
test_send_text_submit_slow_transition_within_one_enter_needs_no_extra_enter() {
  ...
  printf '{"result":{"agent":{"agent_status":"working"}}}\n' > "$resp/6.out"
  fb=$(make_herdr_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" SQUAD_BACKEND_HERDR_SUBMIT_POLLS=3 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_send_text_submit default:w1:p2 "hello commander" 3 0.03 0.01' "$ROOT" )
...
# --- sq-backend.sh dispatch wiring -------------------------------------------

test_dispatch_routes_herdr_backend() {
  fm_backend_validate herdr 2>/dev/null || fail "fm_backend_validate should accept herdr (P2 adds it to SQUAD_BACKEND_KNOWN)"
...
test_dispatch_busy_state_unknown_for_tmux() {
  ...
  [ "$(fm_backend_busy_state tmux 'sess:win')" = unknown ] \
    || fail "fm_backend_busy_state should report unknown for tmux (no native agent-state primitive; sentry falls back to regex)"
...
test_dispatch_composer_state_routes_by_backend() {
  ...
  # classifier the away-mode daemon dispatches through - bin/sq-supervise-daemon.sh's
  ...
  (
    # shellcheck source=bin/sq-backend.sh
    . "$ROOT/bin/sq-backend.sh"
    _SQUAD_BACKEND_TMUX_SOURCED=1
    _SQUAD_BACKEND_HERDR_SOURCED=1
    _SQUAD_BACKEND_ORCA_SOURCED=1
    _SQUAD_BACKEND_ZELLIJ_SOURCED=1
...
SH
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ROOT_OVERRIDE="$neutral" SQUAD_STATE_OVERRIDE="$state" \
    SQUAD_HERDR_LOG="$log" SQUAD_HERDR_RESPONSES="$resp" \
    "$ROOT/bin/sq-peek.sh" default:w1:p2 5 2>/dev/null )
  [ "$out" = "captured herdr pane" ] || fail "sq-peek did not capture through herdr for an explicit metadata-matched target, got '$out'"
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''read'$'\x1f''w1:p2' \
  ...
  assert_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''send-keys'$'\x1f''w1:p2'$'\x1f''escape' \
    "sq-send did not route the explicit stale target through herdr send-key"

  pass "sq-peek/sq-send: explicit stale targets matching metadata use the recorded backend"
}
...
test_workspace_ensure_prunes_default_tab() {
  ...
  raw=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
  ...
  ids=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "sq-prunetest" "$seeded" ) \
...
EOF
  ...
  jq -r --arg w "$wsid" '[.tabs[]|select(.workspace_id==$w)][0].label' "$state" | grep -qx 'sq-prunetest' \
...
test_repeated_cycles_reuse_one_workspace_no_orphans() {
  ...
  for i in 1 2 3; do
    ...
    ids=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "sq-cycle$i" "$seeded" ) \
      || fail "cycle $i: create_task failed"
    read -r _ pane <<EOF
$ids
EOF
    [ -n "$pane" ] || fail "cycle $i: create_task returned no pane id"
    PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
      bash -c '
        . "$0/bin/backends/herdr.sh"
        fm_backend_herdr_presentation_session_lock_path() { printf "/tmp/sq-herdr-cycle-test-lock"; }
        fm_lock_try_acquire() { return 0; }
        fm_lock_release() { return 0; }
  ...
  # exactly one Squad workspace survives three spawn/teardown cycles
  wscount=$(jq -r '[.workspaces[]|select(.label=="Squad")]|length' "$state")
  [ "$wscount" = 1 ] || fail "expected exactly 1 Squad workspace after 3 cycles, got $wscount: $(jq -c '.workspaces' "$state")"
  ...
  # zero tabs remain: every sq- task tab torn down AND the default tab pruned
  ...
  pass "herdr repeated spawn/teardown: one persistent Squad workspace reused, zero orphans, default tab pruned, create ran once"
}
...
test_adopted_workspace_never_prunes_default_tab() {
  ...
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"Squad"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{}}' > "$state"
  raw=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  ...
  ids=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "sq-adopttest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
...
test_label_collision_startup_workspace_leaves_live_tab_alone() {
  # The exact live-fire incident shape (2026-07-02): a commander launches herdr
  # directly inside a directory named "Squad", so herdr auto-derives that
  # workspace's DISPLAYED label from the cwd basename - "Squad" - byte-
  # identical to the primary Squad home's own derived label, with no
  # --label ever passed and no Squad involvement at all. That workspace's
  # single auto-created tab (label "1") holds the commander's own live agent.
  # The very next operator spawn must adopt-and-leave-alone, never prune.
  local dir log state fb raw container seeded ids pane
  dir="$TMP_ROOT/label-collision"; mkdir -p "$dir"; log="$dir/log"; state="$dir/state.json"; : > "$log"
  fb=$(make_herdr_statefake "$dir")
  # Mimic a bare `herdr workspace create --cwd <dir-named-Squad>` (no
  ...
  # alone, from Squad's own freshly-seeded default tab. Its pane hosts a
  # live agent (agent_status=working), exactly like the commander's own pane.
  jq -n '{next:2,workspaces:[{workspace_id:"w1",label:"Squad"}],tabs:[{tab_id:"w1:t1",label:"1",workspace_id:"w1",pane_id:"w1:p1"}],agent_status:{"w1:p1":"working"}}' > "$state"
  raw=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_container_ensure /proj' "$ROOT" ) \
    || fail "container_ensure failed against the stateful fake"
  container=${raw%%$'\t'*}
  seeded=${raw#*$'\t'}
  [ "$container" = "fmtest:w1" ] || fail "container_ensure should adopt the commander's coincidentally-labeled workspace, got '$container'"
  ...
  ids=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "sq-collisiontest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
  ...
  jq -e '.tabs[] | select(.tab_id == "w1:t1")' "$state" >/dev/null \
    || fail "REGRESSION: the commander's live tab was closed - this is the exact 2026-07-02 self-kill incident"
  assert_not_contains "$(cat "$log")" $'\x1f''pane'$'\x1f''close'$'\x1f''w1:p1' \
    "REGRESSION: create_task closed the commander's live pane in the label-collision scenario"
  pass "fm_backend_herdr_create_task: the label-collision startup-workspace scenario (2026-07-02 incident) leaves the commander's live tab untouched"
}
...
test_prune_refuses_a_working_agent_pane_defense_in_depth() {
  ...
  ids=$( PATH="$fb:$PATH" SQUAD_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" HERDR_SESSION=fmtest \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_create_task "$1" "$2" /proj "$3"' "$ROOT" "$container" "sq-busytest" "$seeded" ) \
    || fail "create_task failed against the stateful fake"
  read -r _ pane <<EOF
$ids
EOF
...
# makes - `session list --json` (echoes one session, name SQUAD_FAKE_SESSION_NAME,
# socket SQUAD_FAKE_SOCKET), `status --json`, and `agent get <pane>` (per-pane
# status read from $SQUAD_FAKE_AGENT_DIR/<key>.status, else agent_not_found).
...
case "$cmd $sub" in
  ...
  "session list")
    printf '{"sessions":[{"name":"%s","running":true,"default":false,"socket_path":"%s"}]}\n' \
      "${SQUAD_FAKE_SESSION_NAME:-default}" "${SQUAD_FAKE_SOCKET:-/tmp/sq-fake.sock}" ;;
  "agent get")
    if [ -n "${SQUAD_FAKE_READER_READY_FILE:-}" ] && [ ! -e "$SQUAD_FAKE_READER_READY_FILE" ]; then
      exit 9
    fi
    pane=${3:-}
    key=$(printf '%s' "$pane" | tr ':/.' '___')
    f="${SQUAD_FAKE_AGENT_DIR:-/tmp}/$key.status"
    if [ -f "$f" ]; then
      printf '{"result":{"agent":{"agent_status":"%s"}}}\n' "$(cat "$f")"
    else
      printf '{"error":{"code":"agent_not_found"}}\n' >&2
...
# the socket, streams the TAB-separated lines in $SQUAD_FAKE_READER_LINES to stdout
# (one projected event per line: pane_id\tworkspace_id\tagent_status\tagent),
# then exits $SQUAD_FAKE_READER_EXIT (default 0). A non-zero exit with no lines
...
if [ -n "${SQUAD_FAKE_READER_READY_FILE:-}" ]; then
  : > "$SQUAD_FAKE_READER_READY_FILE"
fi
printf '%s\n' "${SQUAD_FAKE_READER_ACK:-@subscribed}"
if [ -n "${SQUAD_FAKE_READER_LINES:-}" ] && [ -f "$SQUAD_FAKE_READER_LINES" ]; then
  cat "$SQUAD_FAKE_READER_LINES"
fi
exit "${SQUAD_FAKE_READER_EXIT:-0}"
SH
  chmod +x "$path"
  printf '%s\n' "$path"
}
...
test_normalize_event_leaves_from_empty() {
  ...
  [ "$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_pane_id "$1"' "$ROOT" "$rec")" = "wG:pQ" ] \
    || fail "normalize_event pane_id wrong: $rec"
  [ "$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_from_status "$1"' "$ROOT" "$rec")" = "" ] \
    || fail "normalize_event should leave from_status empty (herdr carries no previous status): $rec"
  [ "$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_to_status "$1"' "$ROOT" "$rec")" = "blocked" ] \
...
test_escalation_marker_keys_like_sentry() {
  local m
  ...
  [ "$m" = "/st/.herdr-escalated-default_wG_pQ" ] \
    || fail "escalation marker key must match the sentry's tr ':/.' '___' scheme, got '$m'"
  pass "fm_backend_herdr_escalation_marker keys the dedupe marker exactly like the sentry's .stale-<key>"
}
...
test_apply_transition_blocked_requires_commit_to_dedupe() {
  ...
  rec=$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_record wG:pQ wG "" blocked claude' "$ROOT")
...
test_apply_transition_working_clears_marker() {
  ...
  marker="$state/.herdr-escalated-default_wG_pQ"
  blocked=$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_record wG:pQ wG "" blocked claude' "$ROOT")
  working=$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_record wG:pQ wG "" working claude' "$ROOT")
...
test_apply_transition_defer_and_fallback_are_noops() {
  ...
  for s in idle "done" unknown ""; do
    local rec
    rec=$(bash -c '. "$0/bin/sq-transition-lib.sh"; fm_transition_record wG:pQ wG "" "$1" claude' "$ROOT" "$s")
...
test_wait_transition_no_panes_returns_2() {
  local rc
  bash -c '. "$0/bin/backends/herdr.sh"; SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 fm_backend_herdr_wait_transition default 1 /tmp/st' "$ROOT"; rc=$?
...
test_wait_transition_not_capable_returns_2() {
  ...
  rc=$(PATH="$fb:$PATH" SQUAD_BACKEND_HERDR_EVENTS_FORCE=0 \
...
test_wait_transition_reconcile_blocked_returns_record() {
  ...
  out=$(PATH="$fb:$PATH" TMPDIR="$temp" SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 SQUAD_FAKE_SESSION_NAME=sess SQUAD_FAKE_SOCKET="$dir/x.sock" SQUAD_FAKE_AGENT_DIR="$agent" \
    SQUAD_BACKEND_HERDR_EVENT_READER="$reader" SQUAD_FAKE_READER_LINES="$lines" \
...
test_wait_transition_subscribes_before_reconcile() {
  ...
  rc=$(PATH="$fb:$PATH" SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 SQUAD_FAKE_SESSION_NAME=sess SQUAD_FAKE_SOCKET="$dir/x.sock" SQUAD_FAKE_AGENT_DIR="$agent" \
    SQUAD_BACKEND_HERDR_EVENT_READER="$reader" SQUAD_FAKE_READER_LINES="$lines" SQUAD_FAKE_READER_READY_FILE="$ready" \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
...
test_wait_transition_stream_blocked_returns_record() {
  ...
  marker="$state/.herdr-escalated-sess_wG_pQ"
  out=$(PATH="$fb:$PATH" SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 SQUAD_FAKE_SESSION_NAME=sess SQUAD_FAKE_SOCKET="$dir/x.sock" SQUAD_FAKE_AGENT_DIR="$agent" \
    SQUAD_BACKEND_HERDR_EVENT_READER="$reader" SQUAD_FAKE_READER_LINES="$lines" \
...
test_wait_transition_reader_failure_returns_2() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  rc=$(PATH="$fb:$PATH" TMPDIR="$temp" SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 SQUAD_FAKE_SESSION_NAME=sess SQUAD_FAKE_SOCKET="$dir/x.sock" SQUAD_FAKE_AGENT_DIR="$agent" \
    SQUAD_BACKEND_HERDR_EVENT_READER="$reader" SQUAD_FAKE_READER_LINES="$lines" SQUAD_FAKE_READER_EXIT=2 \
    bash -c '. "$0/bin/backends/herdr.sh"; fm_backend_herdr_wait_transition sess 1 "$1" sess:wG:pQ; echo $?' "$ROOT" "$state" | tail -1)
...
test_wait_transition_bad_ack_returns_2_and_cleans_up() {
  ...
  set_fake_agent "$agent" "wG:pQ" idle
  reader=$(make_fake_reader "$dir"); lines="$dir/lines"; : > "$lines"
  result=$(PATH="$fb:$PATH" TMPDIR="$temp" SQUAD_BACKEND_HERDR_EVENTS_FORCE=1 SQUAD_FAKE_SESSION_NAME=sess SQUAD_FAKE_SOCKET="$dir/x.sock" SQUAD_FAKE_AGENT_DIR="$agent" \
    SQUAD_BACKEND_HERDR_EVENT_READER="$reader" SQUAD_FAKE_READER_LINES="$lines" SQUAD_FAKE_READER_ACK=invalid \
