...
TMP_ROOT=$(fm_test_tmproot sq-supervision-instructions)
RENDER="$ROOT/bin/sq-supervision-instructions.sh"
...
test_selected_harness_block_only() {
  ...
  assert_contains "$out" "bin/sq-sentry-checkpoint.sh" "codex checkpoint helper missing"
...
test_cross_harness_ordinary_continuation_and_repair_matrix() {
  ...
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "plugin already owns sentry continuity" "opencode ordinary-wake line does not leave continuity to the plugin"
  assert_not_contains "$ordinary" "bin/sq-sentry-arm.sh" "opencode ordinary-wake line incorrectly calls the recovery probe"
  ...
  assert_contains "$ordinary" "bin/sq-claude-stop-autoarm.sh" "claude ordinary-wake line lost the auto-arm script name"
  assert_contains "$ordinary" "do not arm another cycle" "claude ordinary-wake line does not forbid a model re-arm"
  assert_not_contains "$ordinary" "bin/sq-sentry-arm.sh" "claude ordinary-wake line incorrectly calls the manual arm"
  out=$("$RENDER" --harness claude --repair-line)
  assert_contains "$out" "sentry supervision needs Stop-owned automatic recovery" "claude recovery line lost its neutral automatic-recovery guidance"
  assert_not_contains "$out" "is broken" "claude recovery line claimed failure before verification"
  assert_not_contains "$out" "bin/sq-sentry-arm.sh" "claude recovery line must not create a repeatable manual arm loop"
  ...
  assert_contains "$ordinary" "bin/sq-sentry-arm.sh" "grok ordinary-wake line lost the background arm command"
  ...
  assert_contains "$out" "bin/sq-sentry-arm.sh" "grok recovery line lost the arm command"

  out=$("$RENDER" --harness codex)
  ordinary=$(printf '%s\n' "$out" | grep -F -- '- Ordinary wake:')
  assert_contains "$ordinary" "next foreground" "codex ordinary-wake line lost its foreground checkpoint"
  assert_contains "$ordinary" "bin/sq-sentry-checkpoint.sh" "codex ordinary-wake line lost the checkpoint command"
  assert_not_contains "$ordinary" "bin/sq-sentry-arm.sh" "codex ordinary-wake line incorrectly uses a background arm"
  ...
  assert_contains "$out" "bin/sq-sentry-checkpoint.sh" "codex recovery line lost the checkpoint command"
...
test_grok_is_background_notify() {
  ...
  assert_contains "$out" "bin/sq-sentry-arm.sh" "grok snippet missing sentry arm"
  assert_not_contains "$out" "__SQUAD_X_MODE_ENV" "renderer leaked an x-mode path placeholder"
...
test_pi_snippet_uses_effective_extension_path() {
  ...
  assert_not_contains "$out" "__SQUAD_PI_EXT__" "renderer leaked the Pi extension path placeholder"
  assert_not_contains "$out" "__SQUAD_PI_TURNEND_EXT__" "renderer leaked the Pi turn-end extension path placeholder"
  assert_not_contains "$out" "state/sq-primary-pi-watch.ts" "pi snippet kept the old generated state-relative extension path"
