#!/usr/bin/env bash
# End-to-end remote reply relay through sq-on and the process-event runner.
set -u
...
TMP_ROOT=$(fm_test_tmproot sq-remote-reply)
mkdir -p "$TMP_ROOT"
...
remote_env() {
  ...
  SQUAD_FAKE_REMOTE_ENTRYPOINT="$ROOT/bin/sq-remote-entrypoint.sh" \
  SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  SQUAD_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" \
  SQUAD_REMOTE_REPLY_WAIT_SECONDS=10 \
  "$@"
}
...
remote_env "$ROOT/bin/sq-procevent.sh" start "$SID" > "$TMP_ROOT/start-one.out" 2>&1 &
RUNNER=$!
...
assert_grep "procevent remote-reply $SID 1" "$PARENT/state/.stand-to-queue" "runner did not publish the normalized remote-reply event"
assert_no_grep 'build verified' "$PARENT/state/.stand-to-queue" "reply payload leaked into the event queue"
...
reconcile_out=$(remote_env "$ROOT/bin/sq-procevent.sh" reconcile)
...
assert_grep 'data/remote-XOs/ios/data/reply/report.md' "$PARENT/state/ios.status" "remote document pointer was not rewritten locally"
cmp -s "$REMOTE/data/reply/report.md" "$PARENT/data/remote-XOs/ios/data/reply/report.md" \
...
assert_grep "$(printf 'caf\xc3\xa9 end')" "$PARENT/state/ios.status" \
  "normalization mangled a UTF-8 note a local XO could have written"
...
retry_destination="$PARENT/data/remote-XOs/ios/data/reply/retry.md"
retry_decoy="$TMP_ROOT/retry-decoy.md"
printf 'local decoy\n' > "$retry_decoy"
ln -s "$retry_decoy" "$retry_destination"
remote_env "$ROOT/bin/sq-procevent.sh" start "$SID" >/dev/null 2>&1 \
...
assert_grep 'data/remote-XOs/ios/data/reply/retry.md' "$PARENT/state/ios.status" \
...
SQUAD_PENDING_REPLY_SEND_HOOK=true \
...
unset SQUAD_PENDING_REPLY_GRACE_SECS
...
remote_env "$ROOT/bin/sq-procevent.sh" start "$SID" > "$TMP_ROOT/start-two.out" 2>&1 &
RUNNER=$!
