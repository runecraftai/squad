...
func TestClaudeAgent_LargePromptUsesExactStdinForColdAndResumedRuns(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			t.Setenv("DRILL_CLAUDE_STDIN_HELPER", "read")
			t.Setenv("DRILL_CLAUDE_STDIN_OBSERVATION", observationPath)
...
func TestClaudeAgent_EarlyExitWithoutReadingLargeStdinDoesNotLeakGoroutines(t *testing.T) {
	t.Setenv("DRILL_CLAUDE_STDIN_HELPER", "exit-early")
	a := newClaudeStdinHelperAgent(t)
...
func TestClaudeAgent_CancellationWithBlockedStdinAndInheritedPipesIsBounded(t *testing.T) {
	for _, mode := range []string{"block", "spawn-grandchild"} {
		t.Run(mode, func(t *testing.T) {
			t.Setenv("DRILL_CLAUDE_STDIN_HELPER", mode)
			ready := filepath.Join(t.TempDir(), "ready")
			t.Setenv("DRILL_CLAUDE_STDIN_READY", ready)
			t.Setenv("DRILL_CLAUDE_STDIN_PID", filepath.Join(t.TempDir(), "grandchild.pid"))
			a := newClaudeStdinHelperAgent(t)
			ctx, cancel := context.WithCancel(context.Background())
			defer cancel()
...
func TestClaudeStdinHelper(t *testing.T) {
	mode := os.Getenv("DRILL_CLAUDE_STDIN_HELPER")
	if mode == "" {
		return
	}
	...
	case "exit-early":
		os.Exit(0)
	case "block":
		_ = os.WriteFile(os.Getenv("DRILL_CLAUDE_STDIN_READY"), []byte("ready"), 0o644)
		for {
			time.Sleep(time.Second)
		}
	case "spawn-grandchild":
		child := exec.Command(os.Args[0], "-test.run=^TestClaudeStdinHelper$")
		child.Env = append(os.Environ(), "DRILL_CLAUDE_STDIN_HELPER=grandchild")
		child.Stdout = os.Stdout
		child.Stderr = os.Stderr
		if err := child.Start(); err != nil {
			os.Exit(2)
		}
		_ = os.WriteFile(os.Getenv("DRILL_CLAUDE_STDIN_PID"), []byte(strconv.Itoa(child.Process.Pid)), 0o644)
		_ = os.WriteFile(os.Getenv("DRILL_CLAUDE_STDIN_READY"), []byte("ready"), 0o644)
	...
	case "read":
		...
		if err := os.WriteFile(os.Getenv("DRILL_CLAUDE_STDIN_OBSERVATION"), data, 0o644); err != nil {
			os.Exit(4)
		}
		emitClaudeHelperResult()
		return
