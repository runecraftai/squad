...
// The permanent signing identity. The executable identifier MUST NEVER change
// once the first signed release ships; the Team ID is the placeholder
// `SQ00000000` that is replaced with the real Apple Developer Team ID at
// publication. Together they are the invariant part of the Developer ID
// designated requirement that lets macOS permissions survive
// `drill update`.
const (
	signingIdentifier = "com.squad.drill"
	signingTeamID     = "SQ00000000"
...
// and finalize still publishing a normal release.
func TestReleaseWorkflowPreservesArtifactContract(t *testing.T) {
	...
	for _, arch := range signingDarwinArches {
		...
		run := job.allRun()
		if !strings.Contains(run, "drill-") || !strings.Contains(run, "${GOOS}-${GOARCH}.tar.gz") {
			t.Errorf("darwin job %q must preserve the drill-<tag>-<goos>-<goarch>.tar.gz name", job.name)
		}
	}
	...
	checksums := wf.jobByRunContains("sha256sum", "checksums.txt")
	if checksums == nil || !strings.Contains(checksums.allRun(), "sha256sum drill-*") {
		t.Error("checksums job must still compute `sha256sum drill-*`")
	}

	finalize := wf.jobByRunContains("gh release edit")
	if finalize == nil || !wfContainsAll(finalize.allRun(), "--draft=false") {
		t.Error("finalize job must still run `gh release edit --draft=false`")
	}
	if strings.Contains(finalize.allRun(), "--prerelease=true") {
		t.Error("finalize job must publish a normal release, not a prerelease")
	}
}
