...
// Paths provides access to all drill filesystem locations.
// The root defaults to ~/.drill but can be overridden via DRILL_HOME
// or by using WithRoot (for testing).
type Paths struct {
	root string
}

// New returns Paths rooted at DRILL_HOME or ~/.drill.
func New() (*Paths, error) {
	if env := os.Getenv("DRILL_HOME"); env != "" {
		return &Paths{root: env}, nil
	}
	if testing.Testing() && os.Getenv("DRILL_ALLOW_DEFAULT_ROOT_IN_TESTS") != "1" {
		return nil, fmt.Errorf("DRILL_HOME must be set under go test to avoid touching the real drill daemon root")
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return nil, err
	}
	return &Paths{root: filepath.Join(home, ".drill")}, nil
}
...
// per DRILL_HOME (see the singleton lock in internal/daemon). Distinct from
