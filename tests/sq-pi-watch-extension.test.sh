#!/usr/bin/env bash
# Tests for the tracked Pi primary sentry extension and Pi XO wiring.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

TMP_ROOT=$(fm_test_tmproot sq-pi-watch-extension)
EXT="$ROOT/.pi/extensions/sq-primary-pi-watch.ts"
...
install_pi_watch_extension_fixture() {
  ...
  cp "$EXT" "$repo/.pi/extensions/sq-primary-pi-watch.ts"
  cp "$ROOT/.pi/extensions/lib/sq-calm-visibility.ts" "$repo/.pi/extensions/lib/sq-calm-visibility.ts"
  ...
  chmod +x "$repo/bin/sq-operational-input.sh"
...
test_pi_extension_reports_external_healthy_sentry() {
  local repo home plugin out status
  repo="$TMP_ROOT/pi-external-healthy-root"
  home="$TMP_ROOT/pi-external-healthy-home"
  mkdir -p "$repo/bin" "$home/state" "$home/config"
  install_pi_watch_extension_fixture "$repo"
  plugin="$repo/.pi/extensions/sq-primary-pi-watch.ts"
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'sentry: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
...
const pi = {
  ...
  registerCommand(name, options) {
    if (name === "sq-sentry-arm-pi") handler = options.handler;
  },
  registerTool() {},
...
if (!prompt.startsWith("\u2063SQUAD_OP: v1 sentry: ")) {
  console.error(`untyped operational follow-up: ${prompt}`);
  process.exit(1);
}
if (!prompt.includes("SQUAD WATCHER WAKE")) {
  console.error(`missing follow-up prompt: ${prompt}`);
  process.exit(1);
}
if (!prompt.includes("external healthy sentry")) {
  console.error(prompt);
  process.exit(1);
}
if (!prompt.includes("sentry: healthy pid=1")) {
  console.error(prompt);
  process.exit(1);
}
EOF
  ...
  status=$?
  expect_code 0 "$status" "Pi extension must surface an external healthy sentry as an owned-wake failure"
  [ -z "$out" ] || fail "Pi external-healthy test printed output: $out"
  pass "Pi extension reports external healthy sentry output"
}
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" node --input-type=module 2>&1 <<'EOF'
import { writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (tool.label !== "Arm Squad sentry") throw new Error(`unexpected label: ${tool.label}`);
...
if (!tool.description.includes("first required Pi sentry cycle")) throw new Error(`tool description omitted the first-cycle condition: ${tool.description}`);
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_STOP_FILE="$stop" node --input-type=module 2>&1 <<'EOF'
...
if (/^sentry: healthy\b/.test(redundant.content[0]?.text)) {
...
for (let i = 0; i < 100 && !existsSync(process.env.SQUAD_ARM_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (!existsSync(process.env.SQUAD_ARM_LOG)) throw new Error("initial arm child did not start");
await new Promise((resolve) => setTimeout(resolve, 100));
const rows = readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n");
if (rows.length !== 1) throw new Error(`redundant call spawned ${rows.length} arm children`);
writeFileSync(process.env.SQUAD_STOP_FILE, "stop\n");
EOF
)
  status=$?
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_WATCH_REARM_RETRY_BASE_MS=10000 SQUAD_WATCH_REARM_RETRY_MAX_MS=10000 node --input-type=module 2>&1 <<'EOF'
...
test_pi_actionable_close_starts_single_successor_before_delivery() {
  ...
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'arm=%s predecessor=%s\n' "$$" "${SQUAD_WATCH_PREDECESSOR_ARM_PID:-none}" >> "${SQUAD_ARM_LOG:?}"
count=$(wc -l < "$SQUAD_ARM_LOG" | tr -d '[:space:]')
printf 'sentry: started pid=%s (beacon fresh)\n' "$$"
if [ "$count" -eq 1 ]; then
...
const pi = {
  ...
  sendUserMessage: async () => {
    rowsAtDelivery = existsSync(process.env.SQUAD_ARM_LOG)
      ? readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n").length
      : 0;
...
await new Promise((resolve) => setTimeout(resolve, 100));
const stableRows = readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n");
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_PI_ARM_READY_TIMEOUT_MS=1000 SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
const pi = {
  ...
  sendUserMessage: async (message) => {
    prompt += message;
    rowsAtPrompt = existsSync(process.env.SQUAD_ARM_LOG)
      ? readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n").length
      : 0;
  },
};
...
if (!prompt.includes("could not restore sentry continuity after 2 retries")) throw new Error(`missing typed restoration failure: ${prompt}`);
await new Promise((resolve) => setTimeout(resolve, 100));
const stableRows = readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n");
...
writeFileSync(process.env.SQUAD_RELEASE_FILE, "release\n");
...
if [ "$count" -eq 2 ]; then
  trap 'printf "retired\\n" > "${SQUAD_UNRETIRED_RETIRE_FILE:?}"' TERM INT
  printf 'ready\n' > "${SQUAD_UNRETIRED_READY_FILE:?}"
  while [ ! -e "$SQUAD_RELEASE_FILE" ]; do sleep 0.02; done
  [ "$SQUAD_LATE_KIND" = actionable ] && printf 'signal: late wake\n'
  exit 0
fi
printf 'sentry: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
writeFileSync(process.env.SQUAD_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  if (rows().length >= 3 && (process.env.SQUAD_LATE_KIND !== "actionable" || prompts.some((message) => message.includes("late wake")))) break;
  await new Promise((resolve) => setTimeout(resolve, 10));
}
if (rows().length !== 3) throw new Error(`late close did not restore one successor: ${rows().join(" | ")}`);
if (process.env.SQUAD_LATE_KIND === "actionable") {
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_STOP_FILE="$stop" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" SQUAD_RELEASE_FILE="$release" node --input-type=module 2>&1 <<'EOF'
...
const lock = `${process.env.SQUAD_BASE}/state/.lock`;
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_ARM_LOG="$log" node --input-type=module 2>&1 <<'EOF'
...
const assertMissingLock = (result, label) => {
  ...
  if (!result.details.message.includes("bin/sq-session-start.sh") || !result.details.message.includes("re-arm")) {
    throw new Error(`${label} missing reclaim and re-arm guidance: ${result.details.message}`);
  }
  if (result.details.message.includes("held by another Squad session")) {
...
try {
  ...
  if (liveOther.details.message !== "sentry: read-only - session lock is held by another Squad session") {
...
if (existsSync(process.env.SQUAD_ARM_LOG)) throw new Error("sentry arm ran without lock ownership");
writeFileSync(lock, `${process.pid}\n`);
...
if (!existsSync(process.env.SQUAD_ARM_LOG)) throw new Error("owned lock did not run the sentry arm");
EOF
)
  status=$?
  expect_code 0 "$status" "Pi sentry arm must distinguish owned, live-other, and missing or dead session locks"
  [ -z "$out" ] || fail "Pi lock-ownership arm test printed output: $out"
  pass "Pi sentry arm distinguishes all session lock ownership states"
}
...
test_pi_session_transition_generation_owner() {
  ...
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'sentry: started pid=%s\n' "$$"
printf '%s\n' "$$" > "${SQUAD_CHILD_PID_FILE:?}"
printf 'arm pid=%s\n' "$$" >> "${SQUAD_ARM_LOG:?}"
trap 'exit 0' TERM INT
while :; do sleep 0.2; done
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_CHILD_PID_FILE="$child_pid_file" SQUAD_ARM_LOG="$arm_log" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
function liveArmPids() {
  if (!existsSync(process.env.SQUAD_ARM_LOG)) return [];
  return readFileSync(process.env.SQUAD_ARM_LOG, "utf8")
    .trim()
    .split(/\n/)
    .filter(Boolean)
    .map((line) => {
...
await waitFor(() => existsSync(process.env.SQUAD_CHILD_PID_FILE), "startup child");
const startupChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
...
async function replaceSession(previous, reason) {
  const previousChild = existsSync(process.env.SQUAD_CHILD_PID_FILE)
    ? readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim()
    : "";
...
const sameInstanceChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
...
const activeChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
...
const finalChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
...
if (quitArm.details?.ok !== false || quitArm.details.message !== "sentry: not armed - Pi session is shutting down") {
...
test_pi_process_exit_cleanup_stops_arm_child() {
  ...
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
trap 'printf "%s\n" "$$" >> "$SQUAD_CLEANUP_LOG"; exit 0' TERM
printf '%s\n' "$$" > "$SQUAD_CHILD_PID_FILE"
while :; do sleep 1; done
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" SQUAD_BASE="$home" SQUAD_ROOT_OVERRIDE="$repo" SQUAD_CLEANUP_LOG="$cleanup_log" SQUAD_CHILD_PID_FILE="$pid_file" node --input-type=module 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
for (let i = 0; i < 250 && !existsSync(process.env.SQUAD_CHILD_PID_FILE); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.SQUAD_CHILD_PID_FILE)) throw new Error("arm child did not start");
const firstChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
await handlers.get("session_shutdown")?.({ type: "session_shutdown" }, {});
await handlers.get("session_start")?.({ type: "session_start" }, {});
await tool.execute("tool-call-replacement", {}, undefined, undefined, {});
for (let i = 0; i < 250; i += 1) {
  const currentChild = readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim();
  if (currentChild !== firstChild) break;
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (readFileSync(process.env.SQUAD_CHILD_PID_FILE, "utf8").trim() === firstChild) {
...
EOF
  ...
  status=$?
  expect_code 0 "$status" "Pi process exit must run the sentry cleanup fallback"
...
test_opencode_primary_watch_plugin_uses_effective_state_home() {
  ...
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'home=%s root=%s\n' "${SQUAD_BASE:-}" "${SQUAD_ROOT_OVERRIDE:-}" >> "${SQUAD_ARM_LOG:?}"
printf 'sentry: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" node 2>&1 <<'EOF'
...
test_opencode_primary_watch_plugin_sources_effective_config() {
  ...
  git init -q "$repo"
  : > "$repo/AGENTS.md"
  printf 'export SQUAD_POLL=7\n' > "$home/config/x-mode.env"
  cat > "$repo/bin/sq-sentry-arm.sh" <<'SH'
#!/usr/bin/env bash
printf 'poll=%s\n' "${SQUAD_POLL:-missing}" >> "${SQUAD_ARM_LOG:?}"
printf 'sentry: healthy pid=1 (beacon 0s)\n'
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" node 2>&1 <<'EOF'
...
writeFileSync(`${process.env.SQUAD_BASE}/state/.lock`, "999999\n");
...
EOF
  ...
  status=$?
  expect_code 0 "$status" "OpenCode watch plugin must arm only when this session owns the unit lock"
  [ -z "$out" ] || fail "OpenCode session-lock test printed output: $out"
  pass "OpenCode sentry plugin requires session lock ownership"
}
...
writeFileSync(`${process.env.SQUAD_BASE}/state/.lock`, `${process.pid}\n`);
const status = await globalThis.__SquadOpenCodeWatchArm.ensureArmed("session-test", client);
...
EOF
  ...
  pass "OpenCode sentry coordinator respects primary scope"
}
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_STOP_FILE="$stop" node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
EOF
  ...
  pass "OpenCode sentry plugin starts one successor before wake prompt delivery settles"
}
...
if [ "$count" -eq 2 ]; then
  ...
  trap 'printf "retired\\n" > "${SQUAD_PRE_READY_RETIRED_FILE:?}"; exit 0' TERM INT
  while [ ! -e "$SQUAD_PRE_READY_RELEASE_FILE" ]; do sleep 0.02; done
  exit 0
fi
printf 'sentry: started pid=%s (beacon fresh)\n' "$$"
trap 'exit 0' TERM INT
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_PRE_READY_RELEASE_FILE="$release" SQUAD_PRE_READY_RETIRED_FILE="$retired" SQUAD_STOP_FILE="$stop" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (existsSync(process.env.SQUAD_PRE_READY_RETIRED_FILE)) throw new Error("pre-ready actionable successor was retired before its close");
writeFileSync(process.env.SQUAD_PRE_READY_RELEASE_FILE, "release\n");
for (let i = 0; i < 500; i += 1) {
  const successorRows = readFileSync(process.env.SQUAD_ARM_LOG, "utf8").trim().split("\n");
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_OPENCODE_ARM_READY_TIMEOUT_MS=1000 SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_RELEASE_FILE="$release" SQUAD_OPENCODE_ARM_READY_TIMEOUT_MS=1000 SQUAD_WATCH_ARM_RETIRE_TIMEOUT_MS=20 SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_STOP_FILE="$stop" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_WATCH_REARM_RETRY_BASE_MS=5 SQUAD_WATCH_REARM_RETRY_MAX_MS=10 SQUAD_WATCH_REARM_RETRY_LIMIT=2 node 2>&1 <<'EOF'
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh"
  out=$(PLUGIN="$plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_RELEASE_FILE="$release" node 2>&1 <<'EOF'
import { spawn } from "node:child_process";
import { existsSync, readFileSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
#!/usr/bin/env bash
printf 'arm\n' >> "${SQUAD_ARM_LOG:?}"
printf 'sentry: started pid=1 (beacon fresh)\n'
SH
...
exit 2
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh" "$repo/bin/sq-turnend-guard.sh"
  out=$(ARM_PLUGIN="$arm_plugin" GUARD_PLUGIN="$guard_plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_GUARD_LOG="$guard_log" node 2>&1 <<'EOF'
import { existsSync, writeFileSync } from "node:fs";
import { pathToFileURL } from "node:url";
...
if (existsSync(process.env.SQUAD_GUARD_LOG)) {
  console.error("turn-end guard ran before the sentry arm could establish supervision");
  process.exit(1);
}
if (promptBody) {
...
EOF
  ...
  pass "OpenCode sentry plugin coordinates with the turn-end guard"
}
...
#!/usr/bin/env bash
printf 'guard\n' >> "${SQUAD_GUARD_LOG:?}"
printf 'guard ran after external healthy sentry\n' >&2
exit 2
SH
  chmod +x "$repo/bin/sq-sentry-arm.sh" "$repo/bin/sq-turnend-guard.sh"
  out=$(ARM_PLUGIN="$arm_plugin" GUARD_PLUGIN="$guard_plugin" WORKTREE="$repo" SQUAD_BASE="$home" SQUAD_ARM_LOG="$log" SQUAD_GUARD_LOG="$guard_log" node 2>&1 <<'EOF'
...
writeFileSync(`${process.env.SQUAD_BASE}/state/.lock`, `${process.pid}\n`);
await guardHooks.event({ event: { type: "session.idle", properties: { sessionID: "session-test" } } });
for (let i = 0; i < 250 && !existsSync(process.env.SQUAD_GUARD_LOG); i += 1) {
  await new Promise((resolve) => setTimeout(resolve, 20));
}
if (!existsSync(process.env.SQUAD_ARM_LOG)) {
  console.error("watch arm did not run");
...
if (!readFileSync(process.env.SQUAD_ARM_LOG, "utf8").includes("args=--restart")) {
  console.error("watch arm was not asked to restart into an owned child");
  process.exit(1);
}
if (!existsSync(process.env.SQUAD_GUARD_LOG)) {
  console.error("turn-end guard was suppressed by an external healthy sentry");
  process.exit(1);
}
