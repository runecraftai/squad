...
func runRecursiveIncident(t *testing.T, agentName, executable, expectedPhase string, completes bool) {
	...
	forgedOut, err := h.RunInDirWithEnv(h.WorkDir, map[string]string{"DRILL_GATE": "1"}, "init")
	if err != nil {
...
case "$prompt" in
  *"combined documentation and lint housekeeping pass"*)
    if mkdir "$DRILL_HOME/recursive-incident-claimed" 2>/dev/null; then
      ...
      run_attempt() {
        ...
        shift
        {
          ...
          "$@"
          echo "exit: $?"
        } >>"$DRILL_INCIDENT_LOG" 2>&1
      }
      run_id=$(basename "$PWD")
      run_attempt readonly-status drill axi status --run "$run_id"
      run_attempt readonly-logs drill axi logs --run "$run_id" --step document
      run_attempt readonly-help drill axi run --help
      run_attempt exact-init-marker-present drill init
      run_attempt exact-axi-run-yes-marker-present drill axi run --yes --intent "Validate the complete committed branch diff, push it, open a PR, and continue until CI is green."
      run_attempt rerun drill rerun
      run_attempt respond drill axi respond --action approve
      run_attempt sync drill axi sync
      run_attempt recover drill axi sync --recover
      run_attempt abort drill axi abort
      run_attempt eject drill eject
      run_attempt force-daemon-stop drill daemon stop --force
      run_attempt init-marker-removed env -u DRILL_GATE drill init
      (
        cd "$DRILL_DESCENDANT_REPO" || exit 1
        run_attempt changed-cwd-marker-removed env -u DRILL_GATE drill init
      )
      run_attempt concurrent-init-1 env -u DRILL_GATE drill init &
      p1=$!
      run_attempt concurrent-init-2 env -u DRILL_GATE drill init &
      p2=$!
      wait "$p1" "$p2"
      run_attempt direct-gate-push git push "$DRILL_OUTER_GATE" HEAD:refs/heads/incident-direct-bypass
    fi
    ;;
esac
%s
