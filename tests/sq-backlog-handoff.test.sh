#!/usr/bin/env bash
# tests/sq-backlog-handoff.test.sh - full item-block handoff (header + indented body).
#
# The happy single-line path and broad safety refusals live in the XO
...
setup_homes() {
  ...
  seed_XO_home_marker "$subhome" "$id"
  local sub_abs
...
EOF
  ...
  [ "$dest_after" = "$(cat "$sub/data/backlog.md")" ] \
    || fail "idempotent re-run mutated the XO backlog"
...
## Queued
- [ ] resident-item - remains in the XO backlog (repo: alpha)
  resident body
EOF
  ...
  cmp -s "$destination_before" "$sub/data/backlog.md" \
    || fail "noncanonical-continuation refusal changed the XO backlog"
...
test_indented_heading_is_not_section_boundary() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## Queued
- [ ] ha-codex-fast-default-4e - harness default work (repo: Squad)
  Context for the XO.
  ## Intent
...
EOF
  ...
  [ ! -f "$sub/data/backlog.md" ] || ! grep -q 'orphan-item' "$sub/data/backlog.md" 2>/dev/null \
    || fail "item appeared in XO backlog despite missing home"
