...
TMP_ROOT=$(fm_test_tmproot sq-remote-job-orphan-reap)
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
REAPER="$ROOT/bin/sq-remote-job-reap-orphans.sh"
...
# --- a real worker fixture, launched exactly the way sq-on's Linux start does -

# build_remote_root <dir>: a minimal but genuine Squad code root carrying
# the real worker and job library.
build_remote_root() {
  local root=$1
  mkdir -p "$root/bin"
  cp "$ROOT/bin/sq-remote-job-lib.sh" "$ROOT/bin/sq-remote-job-worker.sh" "$root/bin/"
  chmod +x "$root/bin"/*.sh
  printf 'fixture\n' > "$root/AGENTS.md"
...
start_worker() {
  ...
  pid=$(
    ...
    export SQUAD_REMOTE_JOB_ORPHAN_GRACE_SECONDS=1
...
cat > "$CASE2/remote-root/bin/sq-remote-job-worker.sh" <<'SH'
#!/bin/bash
...
SH
chmod +x "$CASE2/remote-root/bin/sq-remote-job-worker.sh"
printf 'fixture\n' > "$CASE2/remote-root/AGENTS.md"

set -m
"$CASE2/remote-root/bin/sq-remote-job-worker.sh" >/dev/null 2>&1 &
STALE=$!
set +m
track "$STALE"
