...
test(
  ...
  async () => {
    ...
    function openArtifact(file) {
      const output = run(process.execPath, ["bin/sq-report.js", file, "--no-open"], lavishEnv);
    ...
    try {
      ...
      assert.match(
        run(process.execPath, ["bin/sq-report.js", "poll", revalidationFile, "--timeout-ms", "600"], lavishEnv),
        /status:\s*waiting/,
      );
