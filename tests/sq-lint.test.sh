...
# ShellCheck retired in 0.11.0. sq-lint.sh now pins one exact version and both
...
LINT="$ROOT/bin/sq-lint.sh"
INSTALLER="$ROOT/bin/sq-install-shellcheck.sh"
...
# invoking sq-lint.sh, so those tests can steer git state without depending on
# this worktree's actual branch, remotes, or history:
#   SQUAD_TEST_GIT_INSIDE_WORKTREE  1 (default) or 0
#   SQUAD_TEST_GIT_BRANCH           branch name for `rev-parse --abbrev-ref HEAD`
#   SQUAD_TEST_GIT_HAS_ORIGIN_MAIN  1 (default) or 0
#   SQUAD_TEST_GIT_HAS_MAIN         1 (default) or 0
#   SQUAD_TEST_GIT_MERGE_BASE_OK    1 (default) or 0
#   SQUAD_TEST_GIT_MERGE_BASE       merge-base value to print when OK
#   SQUAD_TEST_GIT_DIFF_FILE        path to a file of NUL-separated changed paths
fm_lint_stub_git() {
  local fakebin=$1
  cat > "$fakebin/git" <<'SH'
#!/usr/bin/env bash
case "$*" in
  "rev-parse --is-inside-work-tree")
    [ "${SQUAD_TEST_GIT_INSIDE_WORKTREE:-1}" = 1 ] || exit 1
    printf 'true\n'
    exit 0
    ;;
  "rev-parse --abbrev-ref HEAD")
    printf '%s\n' "${SQUAD_TEST_GIT_BRANCH:-feature}"
    exit 0
    ;;
  "rev-parse --verify -q origin/main")
    [ "${SQUAD_TEST_GIT_HAS_ORIGIN_MAIN:-1}" = 1 ] && exit 0 || exit 1
    ;;
  "rev-parse --verify -q main")
    [ "${SQUAD_TEST_GIT_HAS_MAIN:-1}" = 1 ] && exit 0 || exit 1
    ;;
  "merge-base "*)
    if [ "${SQUAD_TEST_GIT_MERGE_BASE_OK:-1}" = 1 ]; then
      printf '%s\n' "${SQUAD_TEST_GIT_MERGE_BASE:-fakebase123}"
      exit 0
    fi
    exit 1
    ;;
  "diff --name-only --diff-filter=ACMR -z "*)
    if [ -n "${SQUAD_TEST_GIT_DIFF_FILE:-}" ] && [ -f "$SQUAD_TEST_GIT_DIFF_FILE" ]; then
      cat "$SQUAD_TEST_GIT_DIFF_FILE"
    fi
    exit 0
    ;;
  *)
...
# in the shape `git diff --name-only -z` produces, for SQUAD_TEST_GIT_DIFF_FILE.
...
# them, so changed-file mode tests can assert exactly which files sq-lint.sh
...
test_changed_mode_lints_only_the_changed_file() {
  ...
  tmp=$(fm_test_tmproot sq-lint-changed)
  fakebin=$(fm_fakebin "$tmp")
  fm_lint_stub_git "$fakebin"
  log="$tmp/shellcheck.log"
  fm_lint_stub_shellcheck "$fakebin" "$log"
  diff_file="$tmp/diff.nul"
  target="bin/sq-install-shellcheck.sh"
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' SQUAD_LINT_JOBS=1 \
    SQUAD_TEST_GIT_BRANCH=feature \
    SQUAD_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" 2>&1) \
    || fail "changed-mode lint run failed"$'\n'"$out"
  [ "$(cat "$log")" = "$target" ] \
    || fail "changed-mode lint did not run ShellCheck on exactly the changed file"$'\n'"logged: $(cat "$log")"
  pass "sq-lint.sh changed mode lints only the changed canonical file"
}
...
test_main_branch_forces_full_lint() {
  ...
  tmp=$(fm_test_tmproot sq-lint-main-full)
  fakebin=$(fm_fakebin "$tmp")
  fm_lint_stub_git "$fakebin"
...
test_explicit_path_bypasses_changed_logic() {
  ...
  tmp=$(fm_test_tmproot sq-lint-explicit-override)
  fakebin=$(fm_fakebin "$tmp")
  fm_lint_stub_git "$fakebin"
  log="$tmp/shellcheck.log"
  fm_lint_stub_shellcheck "$fakebin" "$log"
...
test_zero_changed_files_exits_clean() {
  ...
  out=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' SQUAD_TEST_GIT_BRANCH=feature \
    SQUAD_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" 2>&1) || rc=$?
  ...
  pass "sq-lint.sh exits 0 with a note when the local branch has no changed lint targets"
}

test_list_files_respects_changed_mode() {
  local tmp fakebin diff_file listed
  tmp=$(fm_test_tmproot sq-lint-list-changed)
  fakebin=$(fm_fakebin "$tmp")
  fm_lint_stub_git "$fakebin"
  diff_file="$tmp/diff.nul"
  ...
  fm_lint_write_diff_file "$diff_file" \
    "tests/sq-lint.test.sh" "docs/README.md" "bin/definitely-not-real-file.sh"
  ...
  listed=$(PATH="$fakebin:$PATH" GITHUB_ACTIONS='' CI='' SQUAD_TEST_GIT_BRANCH=feature \
    SQUAD_TEST_GIT_DIFF_FILE="$diff_file" "$LINT" --list-files)
  [ "$listed" = "tests/sq-lint.test.sh" ] \
    || fail "--list-files did not report the would-be changed set in changed mode"$'\n'"got: $listed"
  pass "sq-lint.sh --list-files reports the would-be changed set in changed mode"
}

test_pins_an_explicit_version() {
  [ -n "$REQUIRED" ] || fail "sq-lint.sh --required-version printed nothing"
  # The commander-agreed pin: adopt ShellCheck 0.11.0's rule set consistently,
  # which is also what drops the upstream-retired, false-positive-prone SC2015.
  assert_contains "$REQUIRED" "0.11.0" "sq-lint.sh must pin ShellCheck 0.11.0"
  pass "sq-lint.sh pins an explicit ShellCheck version ($REQUIRED)"
}

test_installer_retries_transient_download_failure() {
  local tmp fakebin destination out
  tmp=$(fm_test_tmproot sq-shellcheck-download)
  fakebin=$(fm_fakebin "$tmp")
  destination="$tmp/bin"
...
test_rejects_wrong_shellcheck_version() {
  ...
  tmp=$(fm_test_tmproot sq-lint-ver)
  fakebin=$(fm_fakebin "$tmp")
  cat > "$fakebin/shellcheck" <<'SH'
#!/usr/bin/env bash
if [ "$1" = "--version" ]; then
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "sq-lint.sh accepted a shellcheck version other than the pin"$'\n'"$out"
  assert_contains "$out" "$REQUIRED" "sq-lint.sh did not name the required version on mismatch"
  assert_contains "$out" "0.9.9" "sq-lint.sh did not report the resolved (wrong) version"
  pass "sq-lint.sh refuses to lint under a non-pinned ShellCheck version"
}
...
test_catches_a_real_lint_defect() {
  ...
  tmp=$(fm_test_tmproot sq-lint-bad)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  rc=0
  out=$("$LINT" "$bad" 2>&1) || rc=$?
  [ "$rc" -ne 0 ] || fail "sq-lint.sh passed a known-bad fixture"$'\n'"$out"
  assert_contains "$out" "SC1007" "sq-lint.sh did not report the expected ShellCheck finding"
  pass "sq-lint.sh catches a real lint defect the old no-op gate passed"
}
...
test_ignores_ambient_shellcheck_opts() {
  ...
  local tmp bad out rc
  tmp=$(fm_test_tmproot sq-lint-opts)
  mkdir -p "$tmp"
  bad="$tmp/bad.sh"
  cat > "$bad" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -ne 0 ] || fail "sq-lint.sh allowed ambient SHELLCHECK_OPTS to hide a finding"$'\n'"$out"
  assert_contains "$out" "SC1007" "sq-lint.sh did not neutralize ambient SHELLCHECK_OPTS"
  pass "sq-lint.sh ignores ambient ShellCheck options"
}
...
test_clean_fixture_passes() {
  ...
  local tmp good rc
  tmp=$(fm_test_tmproot sq-lint-good)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  cat > "$good" <<'SH'
#!/usr/bin/env bash
...
SH
  ...
  [ "$rc" -eq 0 ] || fail "sq-lint.sh flagged a clean fixture (exit $rc)"
  pass "sq-lint.sh passes a clean fixture"
}
...
test_jobs_are_deterministic_and_complete() {
  ...
  tmp=$(fm_test_tmproot sq-lint-jobs)
  mkdir -p "$tmp"
  good="$tmp/good.sh"
  bad_a="$tmp/bad-a.sh"
  bad_b="$tmp/bad-b.sh"
...
SH
  ...
  rc_clean_1=0
  out_clean_1=$(SQUAD_LINT_JOBS=1 "$LINT" "$good" 2>&1) || rc_clean_1=$?
  rc_clean_2=0
  out_clean_2=$(SQUAD_LINT_JOBS=2 "$LINT" "$good" 2>&1) || rc_clean_2=$?
  ...
  rc_fail_1=0
  out_fail_1=$(SQUAD_LINT_JOBS=1 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_1=$?
  rc_fail_2=0
  out_fail_2=$(SQUAD_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2=$?
  rc_fail_2b=0
  out_fail_2b=$(SQUAD_LINT_JOBS=2 "$LINT" "$bad_a" "$bad_b" 2>&1) || rc_fail_2b=$?
  ...
  rc_bad_jobs=0
  SQUAD_LINT_JOBS=3 "$LINT" "$good" >/dev/null 2>&1 || rc_bad_jobs=$?
  [ "$rc_bad_jobs" -eq 2 ] || fail "the lint owner must reject unbounded worker counts"

  telemetry_out=$(SQUAD_LINT_JOBS=2 SQUAD_LINT_TELEMETRY="$telemetry" "$LINT" "$good" 2>&1) \
    || fail "telemetry-enabled clean lint failed"
  [ "$telemetry_out" = "$out_clean_2" ] || fail "quiet telemetry changed routine lint output"
  assert_grep $'format\tsq-lint-telemetry-v1' "$telemetry" "telemetry format marker is missing"
  ...
  mkdir -p "$cleanup_tmp"
  cleanup_out=$(TMPDIR="$cleanup_tmp" SQUAD_LINT_JOBS=2 "$LINT" "$good" 2>&1) \
    || fail "cleanup fixture lint failed"
  [ "$cleanup_out" = "$out_clean_2" ] || fail "cleanup fixture changed routine diagnostics"
  [ -z "$(find "$cleanup_tmp" -mindepth 1 -maxdepth 1 -name 'sq-lint.*' -print -quit)" ] \
...
test_worker_trees_stop_on_signal() {
  ...
  tmp=$(fm_test_tmproot sq-lint-signal)
  mkdir -p "$tmp"
  fakebin=$(fm_fakebin "$tmp")
  fixture="$tmp/good.sh"
  cat > "$fixture" <<'SH'
...
SH
  ...
  for jobs in 1 2; do
    for telemetry in off on; do
      ...
      PATH="$fakebin:$PATH" TMPDIR="$lint_tmp" SQUAD_LINT_JOBS="$jobs" \
        SQUAD_LINT_TELEMETRY="$telemetry_file" SQUAD_TEST_SHELLCHECK_PID="$pid_file" \
      ...
      [ -z "$(find "$lint_tmp" -mindepth 1 -maxdepth 1 -name 'sq-lint.*' -print -quit)" ] \
...
test_seeded_module_boundary_parity() {
  ...
  tmp=$(mktemp -d "$ROOT/.sq-lint-parity.XXXXXX")
  if [ "${#SQUAD_TEST_CLEANUP_DIRS[@]}" -eq 0 ]; then
    trap fm_test_cleanup EXIT
  fi
  SQUAD_TEST_CLEANUP_DIRS+=("$tmp")
  rel=${tmp#"$ROOT/"}
...
SH
  ...
  rc=0
  out=$(SQUAD_LINT_JOBS=2 "$LINT" "$dispatcher" "$adapter" "$owner" "$test_root" 2>&1) || rc=$?
