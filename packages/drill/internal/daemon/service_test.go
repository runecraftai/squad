...
func TestStart_ReinstallsManagedServiceWhenPlistChanged(t *testing.T) {
	...
	serviceExecutablePath = func() (string, error) { return "/opt/drill/bin/drill", nil }
...
func TestStart_DoesNotReinstallWhenPlistUnchanged(t *testing.T) {
	...
	current := renderLaunchAgent("/opt/drill/bin/drill", p, home)
...
func TestStartDoesNotRestartLaunchAgentForExecutableOnlyChange(t *testing.T) {
	...
	serviceUserHomeDir = func() (string, error) { return home, nil }
	serviceCurrentUser = func() (*user.User, error) { return &user.User{Uid: "501"}, nil }
	serviceExecutablePath = func() (string, error) { return "/private/var/folders/go-build/drill", nil }

	plistPath := filepath.Join(home, "Library", "LaunchAgents", launchdServiceLabel(p)+".plist")
	if err := os.MkdirAll(filepath.Dir(plistPath), 0o755); err != nil {
		t.Fatal(err)
	}
	installed := renderLaunchAgent("/opt/drill/bin/drill", p, home)
...
func TestStartPreservesInstalledExecutableWhenRefreshingLaunchAgent(t *testing.T) {
	...
	stale := renderLaunchAgentWithoutEnvironment("/opt/drill/bin/drill", p)
	...
	plist := string(data)
	if !strings.Contains(plist, "<string>/opt/drill/bin/drill</string>") {
		t.Fatalf("expected refreshed plist to preserve installed executable, got:\n%s", plist)
	}
	if strings.Contains(plist, "/private/var/folders/go-build/drill") {
...
func TestStartRestartsSystemdUnitWhenDefinitionChanged(t *testing.T) {
	...
	runtimeGOOS = "linux"
	serviceUserHomeDir = func() (string, error) { return home, nil }
	serviceExecutablePath = func() (string, error) { return "/usr/local/bin/drill", nil }

	unitPath := filepath.Join(home, ".config", "systemd", "user", systemdServiceName(p))
	if err := os.MkdirAll(filepath.Dir(unitPath), 0o755); err != nil {
		t.Fatal(err)
	}
	if err := os.WriteFile(unitPath, []byte("[Service]\nExecStart=/old/drill daemon run\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestStartFallsBackToDetachedDaemonWhenManagedStartFails(t *testing.T) {
	...
	cleanup := stubServiceRuntime(t)
	defer cleanup()
	t.Setenv("DRILL_DAEMON_HELPER_PROCESS", "1")
	runtimeGOOS = "linux"
	serviceUserHomeDir = func() (string, error) { return home, nil }
	serviceExecutablePath = func() (string, error) { return "/usr/local/bin/drill", nil }
...
func TestStartDetachedDaemonUsesProvidedRootViaNMHome(t *testing.T) {
	...
	t.Setenv("DRILL_DAEMON_HELPER_PROCESS", "1")
	t.Setenv("DRILL_CAPTURE_DRILL_HOME_FILE", capturePath)
	t.Setenv("DRILL_HOME", "")
	...
	if err != nil {
		t.Fatalf("read captured DRILL_HOME: %v", err)
	}
	if got := string(data); got != p.Root() {
		t.Fatalf("child DRILL_HOME = %q, want %q", got, p.Root())
	}
}
...
func TestStartDetachedDaemonCleansUpChildWhenStartTimeProbeFails(t *testing.T) {
	...
	t.Setenv("DRILL_DAEMON_HELPER_PROCESS", "block")
...
func TestStartStopsManagedServiceBeforeDetachedFallbackAfterTimeout(t *testing.T) {
	...
	defer cleanup()
	t.Setenv("DRILL_DAEMON_HELPER_PROCESS", "1")
	t.Setenv("DRILL_TEST_DAEMON_START_TIMEOUT", "20ms")
	t.Setenv("DRILL_TEST_DAEMON_START_POLL_INTERVAL", "1ms")
	runtimeGOOS = "linux"
	serviceUserHomeDir = func() (string, error) { return home, nil }
	serviceExecutablePath = func() (string, error) { return "/usr/local/bin/drill", nil }
...
func TestManagedServiceInstalledRequiresMatchingRoot(t *testing.T) {
	...
	if err := os.WriteFile(plistPath, []byte(renderLaunchAgent("/opt/drill/bin/drill", paths.WithRoot(otherRoot), home)), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestStartWithUnstubbedPathsDoesNotInvokeRealServiceCommands(t *testing.T) {
	...
	// already exits immediately when DRILL_DAEMON_HELPER_PROCESS=1, so the
	...
	// binary; with DRILL_DAEMON_HELPER_PROCESS=1 the helper exits and the
...
func TestStopDoesNotTouchManagedDaemonOwnedByDifferentNMHome(t *testing.T) {
	...
	// Simulate a live managed daemon owned by a DIFFERENT DRILL_HOME - i.e.
	// the user's real ~/.drill - by seeding the artifact that an
	...
	// other DRILL_HOME. Stop(p) for a test p.Root() must touch neither.
	...
	if err := Stop(p); err != nil {
		t.Fatalf("Stop(p) should be a no-op when no managed daemon is owned by this DRILL_HOME: %v", err)
	}
	for _, cmd := range called {
		// Destructive subcommands that would tear down another DRILL_HOME's daemon.
		for _, forbidden := range []string{"bootout", "/End", "/Delete", "--user stop", "--user disable"} {
			if strings.Contains(cmd, forbidden) {
				t.Fatalf("Stop(p) must not touch managed daemon owned by a different DRILL_HOME, got destructive command: %q", cmd)
			}
		}
	}
}
