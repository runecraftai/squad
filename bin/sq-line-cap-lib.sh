...
# Usage: . bin/sq-line-cap-lib.sh; fm_cap_line "<line>" [<max>]
#
# ONE OWNER for the bounded-line shape both digests use. The wake digest's
# OPEN DECISIONS section (bin/sq-stand-to-drain.sh) and the session-start digest's
# per-task status tails (bin/sq-session-start.sh) render the same kind of
...
#
# Callers keep their own composite policy: sq-stand-to-drain.sh still owns the
# OPEN DECISIONS global byte cap and its "N more omitted" disclosure, and
# sq-session-start.sh still owns how many tail lines it prints per task. This
...
SQUAD_LINE_CAP_DEFAULT=220
SQUAD_LINE_CAP_SUFFIX=' [truncated]'

# fm_cap_line_var <line> [<max>]: put <line> in SQUAD_LINE_CAP_LINE, cut to <max>
# characters with SQUAD_LINE_CAP_SUFFIX in place of the tail when it is longer. A
...
fm_cap_line_var() {
  local line=$1 max=${2:-$SQUAD_LINE_CAP_DEFAULT} keep
  if [ "${#line}" -le "$max" ]; then
    SQUAD_LINE_CAP_LINE=$line
    return 0
  fi
  keep=$((max - ${#SQUAD_LINE_CAP_SUFFIX}))
  [ "$keep" -ge 0 ] || keep=0
  SQUAD_LINE_CAP_LINE="${line:0:$keep}$SQUAD_LINE_CAP_SUFFIX"
}
