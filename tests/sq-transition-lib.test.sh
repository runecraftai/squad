#!/usr/bin/env bash
# tests/sq-transition-lib.test.sh - unit tests for the shared, backend-neutral
# normalized-transition shape and the single-owner status->action policy table
# (bin/sq-transition-lib.sh). Pure functions, no backend required.
set -u
...
echo "# sq-transition-lib.test.sh: all assertions passed"
