#!/usr/bin/env bash
# tests/sq-tmux-agent-liveness.test.sh - portable regression for the tmux
...
# live per-harness counterpart is tests/sq-harness-liveness-drift-live-e2e.test.sh.
#
...
SOCKET="sq-liveness-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/sq-liveness.XXXXXX")
SESSION=liveness
...
# A muse operator pane misclassified here reads as a dead endpoint, so a healthy
