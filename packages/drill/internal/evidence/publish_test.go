...
func baseRequest(remote, work, source string) Request {
	return Request{
		...
		SourceDir: source,
		Message:   "drill: evidence for fm/add-login",
	}
}
...
func TestPublish_LandsEvidenceOnOrphanBranchAndLeavesCodeBranchesUntouched(t *testing.T) {
	...
	for _, want := range []string{
		MarkerPath,
		".drill/evidence/fm/add-login/checkout.png",
		".drill/evidence/fm/add-login/logs/cli-run.txt",
	} {
		if !strings.Contains(tree, want) {
			t.Errorf("evidence branch is missing %q, has:\n%s", want, tree)
		}
	}
	if got := runGit(t, remote, "cat-file", "-p", tip+":.drill/evidence/fm/add-login/logs/cli-run.txt"); got != "it works" {
...
func TestPublish_AppendsWithoutRewritingEarlierEvidence(t *testing.T) {
	...
	if got := runGit(t, remote, "cat-file", "-p", one.CommitSHA+":.drill/evidence/fm/add-login/round-1.txt"); got != "first" {
		t.Errorf("earlier evidence commit no longer resolves: %q", got)
	}
	if got := runGit(t, remote, "cat-file", "-p", two.CommitSHA+":.drill/evidence/fm/add-login/round-2.txt"); got != "second" {
...
func TestPublish_RefusesExistingBranchThatIsNotAnEvidenceBranch(t *testing.T) {
	...
	runGit(t, work, "push", "origin", "main:refs/heads/drill/evidence")
...
func TestPublish_RefusesExistingBranchWithWrongMarkerContent(t *testing.T) {
	remote, work := newRepoWithRemote(t)
	source := writeEvidence(t, t.TempDir(), map[string]string{"proof.txt": "ok\n"})
	if err := os.WriteFile(filepath.Join(work, MarkerPath), []byte("not a drill evidence branch\n"), 0o644); err != nil {
...
func TestPublish_ReportsPublishedFilesRelativeToTheSourceDirectory(t *testing.T) {
	...
	if result.Dir != ".drill/evidence/fm/add-login" {
