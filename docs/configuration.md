...
On success it forwards `CLAUDE_CONFIG_DIR=<the account's dir>` onto the claude launch, overriding the pre-existing ambient-forwarding behavior in "Harness support" above, and records `account=<label>` in `state/<id>.meta`.
