...
`config/` sob a base efetiva contém escolhas de operação locais e gitignored, enquanto a raiz de código rastreada também contém o arquivo de política compartilhada `config/skill-verification.json`; `projects/` contém as cópias locais de projetos que Squad lê mas altera apenas através das exceções restritas, protegidas e concretamente aprovadas pelo comandante em `AGENTS.md`.
