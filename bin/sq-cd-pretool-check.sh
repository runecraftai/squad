...
#
# Usage:
#   <PreToolUse JSON on stdin> | bin/sq-cd-pretool-check.sh
#   bin/sq-cd-pretool-check.sh --command '<cmd>'
#
...
#
# Exit/output contract (identical shape to bin/sq-arm-pretool-check.sh):
...
usage() {
  cat <<'EOF'
Usage: sq-cd-pretool-check.sh [--command <cmd>] [--claude]

With no --command, reads a PreToolUse-style JSON payload on stdin (Grok
toolInput.command, or Claude/Codex tool_input.command).
Fires only in the real primary Squad checkout; it is a silent no-op in a
operator/recon task worktree or any non-Squad repo.
...
# decoder set in bin/sq-arm-command-policy.mjs: adding any new quote/expansion
...
[ -f "$SQUAD_ROOT/AGENTS.md" ] || exit 0
[ -d "$SQUAD_ROOT/bin" ] || exit 0
command -v git >/dev/null 2>&1 || exit 0
GIT_DIR=$(git -C "$SQUAD_ROOT" rev-parse --git-dir 2>/dev/null) || exit 0
GIT_COMMON_DIR=$(git -C "$SQUAD_ROOT" rev-parse --git-common-dir 2>/dev/null) || exit 0
[ "$GIT_DIR" = "$GIT_COMMON_DIR" ] || exit 0

POLICY="$SQUAD_ROOT/bin/sq-cd-command-policy.mjs"
