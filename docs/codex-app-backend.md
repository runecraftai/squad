...
Codex App is not a selectable Squad runtime backend.
Codex Desktop host tools can create and supervise visible threads and those threads can write Squad status files when given an authorized path, but Squad has no supported shell-callable bridge to those host tools.
...
5. Let the thread append Squad's normal lifecycle lines to `state/<id>.status`.

The status return channel is mandatory.
A visible thread that cannot report into Squad's normal lifecycle is not a complete backend.
...
A one-process probe could create and archive a thread record, but no supported bridge was found that lets Squad create, continue, read, and archive the same visible Desktop-owned endpoint over its full lifetime.
...
Once available, Squad should add a real `bin/backends/codex-app.sh`, persist `backend=codex-app` and `codex_app_thread_id=`, and route spawn, send, peek, watch, and cleanup through the shared dispatcher.

## Rollout

Ship and recon tasks come first.
XO support remains out of scope until create, send, read, status return, and archive are proven through the normal backend dispatcher.
