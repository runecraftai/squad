...
# (bin/sq-claude-stop-autoarm.sh + bin/sq-turnend-guard.sh --claude).
# Proves, against the real installed Claude Code and the real tracked hook
# registration: a fresh session with in-flight work, no sentry, and a stale
# session lock can run sq-session-start.sh first; session start reclaims the
...
if [ "${SQUAD_CLAUDE_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_CLAUDE_LIVE_E2E=1 to run the Claude Stop auto-arm regression"
  exit 0
fi
...
SH
chmod +x "$PROJECT/bin/sq-sentry-arm.sh" "$PROJECT/bin/sq-stand-to-drain.sh"

PROMPT='Run exactly `bin/sq-session-start.sh` with Bash as your first tool call. After reading its complete digest, reply with exactly CYCLE0 and stop. Whenever a Stop hook feedback message wakes you, run exactly `bin/sq-stand-to-drain.sh` once with Bash, then reply with exactly ACK and stop. Never run bin/sq-sentry-arm.sh or any other arm command, and never use any other tool.'
...
[ "$(sed -n '1p' "$HOME_DIR/state/tool-calls.log" 2>/dev/null)" = 'bin/sq-session-start.sh' ] \
...
if [ -f "$HOME_DIR/state/tool-calls.log" ]; then
  ! grep -q 'sq-sentry-arm.sh' "$HOME_DIR/state/tool-calls.log" \
