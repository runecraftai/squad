#!/usr/bin/env bash
# Real Pi/Herdr regression for exact-id XO marker delivery.
#
...
# is routed through bin/sq-herdr-lab.sh. The PATH shim strips only the adapter's
...
if [ "${SQUAD_SEND_MARKER_HERDR_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_SEND_MARKER_HERDR_E2E=1 to run the real Pi/Herdr XO-marker regression"
  exit 0
fi
...
SESSION=$("$LAB_HELPER" name sq-send-XO-marker-v7)
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/sq-send-marker-herdr-e2e.XXXXXX")
SENDER_HOME="$TMP_ROOT/sender-home"
SECOND_HOME="$TMP_ROOT/XO-home"
CAPTURE="$TMP_ROOT/pi-before-agent.jsonl"
FAKEBIN="$TMP_ROOT/fakebin"
ORIGINAL_PATH=$PATH
REAL_PI=$(command -v pi)
ID='marker-pi-sm'
REQUEST='SQUAD_MARKER_HERDR_E2E exact-id request'
DIRECT='SQUAD_MARKER_HERDR_DIRECT commander input'
...
printf '%s\n' "$ID" > "$SECOND_HOME/.sq-xo-home"
cat > "$SECOND_HOME/data/charter.md" <<'EOF'
# Isolated marker capture XO

You are a task-local XO used only for the marker transport regression.
...
# XO launch and its own extension arguments unchanged.
CAPTURE_JSON=$(printf '%s' "$CAPTURE" | jq -Rs .)
CAPTURE_EXTENSION="$TMP_ROOT/sq-send-marker-capture.ts"
...
[ -f "$META" ] || fail "real XO spawn did not write exact-id metadata"
[ "$(fm_meta_get "$META" kind)" = XO ] || fail "real XO metadata did not record kind=xo"
TARGET=$(fm_backend_target_of_meta "$META")
PANE=${TARGET#*:}
case "$TARGET" in
  "$SESSION":w*:p*) : ;;
  *) fail "real XO metadata recorded an unexpected Herdr target: $TARGET" ;;
esac
...
wait_for_prompt 'Isolated marker capture XO' \
...
# therefore remains conversational commander input.
...
[ "$GOT" = "$DIRECT" ] || fail "direct commander input was changed or marked"$'\n'"--- bytes ---"$'\n'"$(printf '%s' "$GOT" | od -An -tx1)"
if fm_message_from_Squad "$GOT"; then
  fail "direct commander input was classified as from-squad"
fi
printf 'evidence: direct-input received-hex=%s\n' "$(printf '%s' "$GOT" | od -An -tx1 | tr -d ' \n')"
pass "real Pi/Herdr: direct commander terminal input stays unmarked"
