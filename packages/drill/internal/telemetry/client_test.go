...
func TestClientTrackSendsUmamiEventPayload(t *testing.T) {
	...
	case got := <-reqCh:
		...
		if got.Payload.URL != "app://drill/command" {
			t.Fatalf("url = %q, want %q", got.Payload.URL, "app://drill/command")
		}
		if got.Payload.Title != "drill CLI" {
			t.Fatalf("title = %q, want %q", got.Payload.Title, "drill CLI")
		}
