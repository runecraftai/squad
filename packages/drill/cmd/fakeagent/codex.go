...
func runCodex(args []string, scenario *Scenario) int {
	...
	// to only fields declared in the schema. Otherwise drill'
	...
	// Replay recorded codex output if a fixture is available. drill
...
// real. drill parses JSON out of the agent_message text, so for
...
// mirroring that behaviour keeps the fake consistent with drill'
...
func extractCodexPrompt(args []string) string {
	...
	if positionals[0] == "resume" {
		...
		return "" // resume without id+prompt is not a shape drill emits
	}
