...
func TestDrillRequiredWorkflowChecksPipelineSignature(t *testing.T) {
	t.Parallel()

	workflow, err := os.ReadFile(filepath.Join("..", "..", "..", ".github", "workflows", "drill-required.yml"))
	if err != nil {
		t.Fatalf("read required workflow: %v", err)
	}
	if !strings.Contains(string(workflow), "marker='"+drillPRSignature+"'") {
		t.Fatalf("required workflow does not check the generated PR signature %q", drillPRSignature)
	}
}
...
func TestBuildPipelineSummary_AllClean(t *testing.T) {
	...
	if !strings.Contains(md, "[git push drill](https://github.com/runecraftai/squad/packages/drill)") {
...
func TestBuildPipelineSummary_EmitsStructuredStepAttestation(t *testing.T) {
	...
	const prefix = "<!-- drill-pipeline-attestation:v1 "
	...
	if signatureAt, attestationAt := strings.Index(got, drillPRSignature), strings.Index(got, prefix); signatureAt < 0 || attestationAt < signatureAt {
...
func TestBuildTestingSummaryForPR_RendersLocalTempVisualArtifactPath(t *testing.T) {
	t.Parallel()
	repoRoot := t.TempDir()
	localPath := filepath.Join(os.TempDir(), "drill-evidence", "run-123", "checkout.png")
