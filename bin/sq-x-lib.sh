...
# another .env-style file, but bootstrap activation still checks $SQUAD_BASE/.env.
...
#   fmx_load_config            - resolve SQX_TOKEN, SQX_RELAY, SQX_DRY, SQX_MAX,
#                                and SQX_THREAD_MAX (env wins over .env)
...
# Callers must have SQUAD_BASE set before calling fmx_load_config.
...
fmx_poll_shim_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by sq-bootstrap.sh - X mode connector poll shim.' \
    '# The sentry validates these bytes, then dispatches the trusted poll script.' \
    "export SQUAD_BASE=$(printf '%q' "$home")" \
    "exec $(printf '%q' "$root/bin/sq-x-poll.sh")"
}
...
fmx_poll_shim_v1_content() {
  ...
  printf '%s\n' \
    '#!/usr/bin/env bash' \
    '# Auto-generated by sq-bootstrap.sh - X mode connector poll shim.' \
    '# The sentry runs this each check cycle; output becomes a check: wake.' \
    "export SQUAD_BASE=$(printf '%q' "$home")" \
    "exec $(printf '%q' "$root/bin/sq-x-poll.sh")"
}
...
fmx_private_artifact_publish_stdin() {
  ...
  dest="$dir/$base"
  tmp=$(umask 077; mktemp "$dir/.${base}.sq-x.XXXXXX" 2>/dev/null) || return 1
  if ! cat > "$tmp" \
...
fmx_private_artifact_publish_stdin_once() {
  ...
  dest="$dir/$base"
  tmp=$(umask 077; mktemp "$dir/.${base}.sq-x.XXXXXX" 2>/dev/null) || return 2
  if ! cat > "$tmp" \
    || ! chmod "$mode" "$tmp" 2>/dev/null \
    || ! fmx_single_link_file_mode_valid "$tmp" "$mode" "$device"; then
    rm -f -- "$tmp"
...
# Resolve the X-mode settings into SQX_TOKEN, SQX_RELAY, SQX_DRY, SQX_MAX,
# SQX_DISCORD_MAX, and SQX_THREAD_MAX. An explicit environment variable always
# wins over the .env file; the relay URL defaults to the production host so a
# normal user configures only the token. SQX_RELAY has any trailing slash trimmed
# so callers can append "/connector/..." cleanly.
# SQX_DRY is set to "1" when SQX_DRY_RUN is a truthy value (anything other than
# unset/empty/0/false/no/off), and "" otherwise: preview mode, where the client
# composes a reply but records it instead of posting (see sq-x-reply.sh).
fmx_load_config() {
  local env_file="${SQX_ENV_FILE:-$SQUAD_BASE/.env}" dry
  if [ -n "${SQX_PAIRING_TOKEN+x}" ]; then
    SQX_TOKEN=${SQX_PAIRING_TOKEN-}
  else
    SQX_TOKEN=$(fmx_env_get SQX_PAIRING_TOKEN "$env_file")
  fi
  if [ -n "${SQX_RELAY_URL+x}" ]; then
    SQX_RELAY=${SQX_RELAY_URL-}
  else
    SQX_RELAY=$(fmx_env_get SQX_RELAY_URL "$env_file")
  fi
  [ -n "$SQX_RELAY" ] || SQX_RELAY="https://mySquad.io"
  SQX_RELAY=${SQX_RELAY%/}
  if [ -n "${SQX_DRY_RUN+x}" ]; then
    dry=${SQX_DRY_RUN-}
  else
    dry=$(fmx_env_get SQX_DRY_RUN "$env_file")
  fi
  # shellcheck disable=SC2034 # SQX_DRY is read by callers (sq-x-reply.sh) after sourcing.
  case "$(printf '%s' "$dry" | tr '[:upper:]' '[:lower:]')" in
    ''|0|false|no|off) SQX_DRY="" ;;
    *) SQX_DRY=1 ;;
  esac
  ...
  if [ -n "${SQX_X_REPLY_MAX_CHARS+x}" ]; then maxraw=${SQX_X_REPLY_MAX_CHARS-}; else maxraw=$(fmx_env_get SQX_X_REPLY_MAX_CHARS "$env_file"); fi
  case "$maxraw" in ''|*[!0-9]*) maxraw=280 ;; esac
  [ "$maxraw" -ge 50 ] 2>/dev/null || maxraw=50
  # shellcheck disable=SC2034 # SQX_MAX is read by callers (sq-x-reply.sh) after sourcing.
  SQX_MAX=$maxraw
  if [ -n "${SQX_DISCORD_REPLY_MAX_CHARS+x}" ]; then discordraw=${SQX_DISCORD_REPLY_MAX_CHARS-}; else discordraw=$(fmx_env_get SQX_DISCORD_REPLY_MAX_CHARS "$env_file"); fi
  ...
  # shellcheck disable=SC2034 # SQX_DISCORD_MAX is read by callers (sq-x-reply.sh) after sourcing.
  SQX_DISCORD_MAX=$discordraw
  if [ -n "${SQX_X_THREAD_MAX+x}" ]; then threadraw=${SQX_X_THREAD_MAX-}; else threadraw=$(fmx_env_get SQX_X_THREAD_MAX "$env_file"); fi
  case "$threadraw" in ''|*[!0-9]*) threadraw=25 ;; esac
  [ "$threadraw" -ge 1 ] 2>/dev/null || threadraw=25
  # shellcheck disable=SC2034 # SQX_THREAD_MAX is read by callers (sq-x-reply.sh) after sourcing.
  SQX_THREAD_MAX=$threadraw
}
...
# have populated SQX_TOKEN and SQX_RELAY first.
fmx_request_relay_context() {
  local rid=$1 payload_file body_file code rc ctx empty='{"platform":"","reply_max_chars":""}'
  [ -n "${SQX_TOKEN:-}" ] || { printf '%s\n' "$empty"; return 1; }
  command -v curl >/dev/null 2>&1 || { printf '%s\n' "$empty"; return 1; }
  command -v jq >/dev/null 2>&1 || { printf '%s\n' "$empty"; return 1; }
  payload_file=$(mktemp "${TMPDIR:-/tmp}/sq-x-reqctx.XXXXXX") || { printf '%s\n' "$empty"; return 1; }
  body_file=$(mktemp "${TMPDIR:-/tmp}/sq-x-reqctx-body.XXXXXX") || { rm -f "$payload_file"; printf '%s\n' "$empty"; return 1; }
...
# through one persistent XO: linking request B onto a task overwrites
...
# budget for one outbound message. X keeps the existing SQX_X_REPLY_MAX_CHARS
...
fmx_auth_header_file() {
  ...
  file=$(umask 077; mktemp "${TMPDIR:-/tmp}/sq-x-auth.XXXXXX") || return 1
  chmod 600 "$file" 2>/dev/null || { rm -f "$file"; return 1; }
  printf 'Authorization: Bearer %s\n' "$SQX_TOKEN" > "$file" || { rm -f "$file"; return 1; }
  printf '%s\n' "$file"
}
...
# The compact preview object is printed for SQX_DRY_RUN outbox records.
fmx_image_payload_file() {
  local path=$1 client=${2:-sq-x-reply} payload_file=${3:-} media_type bytes
...
fmx_post_json() (
  ...
  code=$(curl -m 10 -s -o "$body_file" -w '%{http_code}' \
    ...
    "$SQX_RELAY/connector/$endpoint" 2>/dev/null)
  rc=$?
...
# sq-x-followup.sh posts against that link (within the window, up to the cap),
# then either records the incremented count or clears the link. These helpers
# own the read/write/clear so sq-x-link.sh and sq-x-followup.sh never hand-edit
