...
if [ "${SQUAD_MUSE_SIGNALS_LIVE:-0}" != 1 ]; then
  echo "skip: set SQUAD_MUSE_SIGNALS_LIVE=1 to run the real Muse signal drift guard"
  exit 0
fi

[ -x "$MUSE_BIN" ] || fail "SQUAD_MUSE_SIGNALS_LIVE=1 but no real muse executable is installed on PATH"
[ -x "$REAL_TMUX" ] || fail "SQUAD_MUSE_SIGNALS_LIVE=1 but tmux is not installed"
command -v node >/dev/null 2>&1 || fail "node is required to inspect Muse's serialized session protocol"

LAB=$(mktemp -d "${TMPDIR:-/tmp}/sq-muse-signals.XXXXXX") || fail "could not create the isolated Muse lab"
trap cleanup EXIT
...
"$REAL_TMUX" -L "$SOCKET" new-window -d -t "$SESSION:" -n muse -c "$WORKSPACE" -- \
  ...
  "$MUSE_BIN" --provider echo --yolo "Squad Muse signal drift guard" \
