#!/usr/bin/env bash
# tests/sq-backend-herdr-smoke.test.sh - real herdr smoke test for the herdr
# session-provider adapter (bin/backends/herdr.sh), P2 of
# data/sq-backend-design-d7 (herdr-addendum.md), extended for the P3
# workspace-per-home pass (AGENTS.md task herdr-sm-spaces-k4). Mirrors
# tests/sq-backend-tmux-smoke.test.sh's structure: every other suite fakes the
...
# commander's real herdr usage. Skips cleanly when herdr (or jq) is not
...
# that command killed the commander's live default herdr server twice in
...
pass "real herdr: container_ensure starts the isolated session's server, creates the Squad workspace ($CONTAINER), and reports its seeded default tab id ($SEEDED_TAB_ID)"
...
pass "real herdr: container_ensure is idempotent (reuses/adopts the existing Squad workspace, reports no seeded default tab on adoption)"
...
# exactly one clean sq-<id> task tab.
...
pass "real herdr: create_task prunes the freshly-created workspace's seeded default tab, leaving exactly one clean sq-<id> task tab"
...
# server restart, and a restored sq-<id> task tab comes back a HUSK: a dead
...
LIVE_DUP_LABEL="sq-smoke-livedup"
...
herdr pane report-agent "$LIVE_DUP_PANE_ID" --source sq-smoke-test --agent sq-smoke-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
...
# --- workspace-per-home: an XO-shaped home gets its OWN space --------
...
SM_SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/sq-herdr-smoke-sm.XXXXXX")
SM_HOME="$SM_SCRATCH/XO-home"
mkdir -p "$SM_HOME"
...
case "$SM_CONTAINER" in
  "$SESSION":w*) : ;;
  *) fail "XO container_ensure returned an unexpected shape: $SM_CONTAINER" ;;
esac
[ "$SM_CONTAINER" != "$CONTAINER" ] || fail "an XO-shaped home must get a DIFFERENT workspace than the primary's, got the same: $SM_CONTAINER"
[ -n "$SM_SEEDED_TAB_ID" ] || fail "the XO-shaped home's container_ensure must CREATE its own workspace and report its seeded default tab id"
pass "real herdr: an XO-shaped home (.sq-xo-home) gets its OWN herdr workspace, distinct from the primary's, in the SAME session"
...
if [ -z "$SM_TAB_ID" ] || [ -z "$SM_PANE_ID" ]; then
  fail "XO create_task did not return tab/pane ids"
fi
pass "real herdr: a task spawned into the XO-shaped home lands as a tab inside the XO's OWN workspace"
...
case "$SM_LIVE" in
  *"$LABEL"*) fail "the XO-shaped home's list_live must not see the primary's task ($LABEL)"$'\n'"$SM_LIVE" ;;
esac
...
[ "$POST_SM_PANE" = "$SM_PANE_ID" ] || fail "the XO task's pane id did not survive the restart: before=$SM_PANE_ID after=$POST_SM_PANE"
...
fm_backend_herdr_send_literal "$TARGET" 'echo literal-then-key-commander' \
...
if [ "${SQUAD_HERDR_SMOKE_REAL_CLAUDE:-0}" = 1 ] && command -v claude >/dev/null 2>&1; then
...
elif [ "${SQUAD_HERDR_SMOKE_REAL_CLAUDE:-0}" != 1 ]; then
  echo "note: SQUAD_HERDR_SMOKE_REAL_CLAUDE=1 not set; skipping the real-agent busy_state check" >&2
else
...
# Real Squad spawns always re-run container_ensure immediately before
# create_task (bin/sq-spawn.sh), never reusing a container reference from an
...
pass "real herdr: list_live discovers a live task tab by sq-<id> label"
