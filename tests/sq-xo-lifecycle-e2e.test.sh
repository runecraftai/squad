#!/usr/bin/env bash
# tests/sq-xo-lifecycle-e2e.test.sh - the happy-path XO operator
...
# lease-specific paths live in sq-xo-safety.test.sh.
#
...
#   - a drill project is initialized (init + doctor) in the NEW subhome clone
#     and the parent project clone is never mutated (no write through a project)
#   - spawn meta records kind=xo, home=, and the project list; launch runs
#     in the subhome with the persistent charter and cleared operational overrides
#   - a bare `sq-<id>` send targets the window recorded in THIS home's meta
...
TMP_ROOT=$(fm_test_tmproot sq-xo-lifecycle)
...
EOF
  ...
  # One combined fakebin: tmux + fob (spawn/send/teardown) and drill
  # (gamma initialization during seed).
  FAKEBIN=$(make_fake_tmux "$TMP_ROOT/fake")
  make_fake_drill "$TMP_ROOT/fake" >/dev/null
  ...
  SQUAD_XO_SCOPE='customer onboarding from brief' \
    scaffold_XO_charter "$HOME_DIR" design 'customer onboarding charter' alpha beta gamma \
    || fail "filled XO charter scaffold failed"
}
...
phase_seed() {
  ...
  assert_present "$SUB/.sq-xo-home" "seed did not mark the subhome"
  ...
  # drill init runs in the NEW clone, never the parent project.
  assert_present "$SUB/projects/gamma/.drill-init" "drill project was not initialized in the subhome"
  assert_present "$SUB/projects/gamma/.drill-doctor" "drill project was not doctored in the subhome"
  assert_absent "$HOME_DIR/projects/gamma/.drill-init" "seed wrote drill state through the parent project"

  # Registry line: scope from the filled brief, project list, no legacy owns field.
  assert_grep '- design - customer onboarding charter' "$HOME_DIR/data/XOs.md" "registry summary not from the charter"
  assert_grep 'scope: customer onboarding from brief' "$HOME_DIR/data/XOs.md" "registry scope not from the filled brief"
  assert_grep 'projects: alpha, beta, gamma' "$HOME_DIR/data/XOs.md" "registry did not record the project list"
...
phase_spawn() {
  ...
  assert_grep 'kind=xo' "$meta" "spawn meta did not record kind=xo"
  ...
  assert_no_grep 'notify=' "$LOG" "XO codex launch included the parent turn-end notify hook"
  assert_no_grep 'turn-ended' "$LOG" "XO codex launch referenced a parent turn-ended signal"
  assert_no_grep 'fob get' "$LOG" "XO spawn ran a project fob get"
...
phase_send() {
  ...
  # design is a kind=xo target, so the request is prefixed with the
  # from-squad marker (bin/sq-marker-lib.sh): the send targets the meta window
  # AND carries the marker label, and the original payload still follows it.
  assert_grep 'send-keys -t Squad:sq-design -l [sq-from-squad]' "$LOG" "send did not use the window recorded in this home's meta, or did not mark the XO request"
  assert_grep 'route this work' "$LOG" "the original request text did not survive the marker"
  assert_no_grep 'send-keys -t other-session:sq-design' "$LOG" "send targeted a foreign same-named window"
  pass "send: a bare sq-<id> XO routes to the meta window with the from-squad marker"
}

phase_handoff() {
  # The move is delegated to `sq-tasks mv`; skip cleanly when it is absent (the
  # downstream recovery and teardown phases do not depend on this phase).
  if ! command -v tasks-axi >/dev/null 2>&1; then
    echo "skip: sq-tasks not found (backlog handoff delegates to it)"
    return 0
  fi
...
phase_recovery() {
  ...
  assert_grep 'window=Squad:sq-design' "$meta" "respawn did not reconstruct the direct-report window"
...
phase_teardown() {
  ...
  printf '%s\n' "$teardown_out" | grep -F 'Backlog:' >/dev/null \
    && fail "XO teardown emitted a main-backlog completion reminder"
  assert_absent "$SUB" "teardown did not remove the retired XO home"
  assert_absent "$HOME_DIR/state/design.meta" "teardown did not clear the parent meta"
  assert_no_grep '- design ' "$HOME_DIR/data/XOs.md" "teardown did not remove the registry route"
