...
// a short stable suffix derived from p.Root() so two drill installs
// with different DRILL_HOMEs cannot collide in the global launchctl/systemctl/
// schtasks namespace. See serviceInstanceSuffix for the full rationale.
const (
	launchdServiceLabelBase = "com.squad.drill.daemon"
	systemdServiceNameBase  = "drill-daemon"
	windowsTaskNameBase     = "drill-daemon"
)
...
const (
	legacyLaunchdServiceLabel = "com.squad.drill.daemon"
	legacySystemdServiceName  = "drill-daemon.service"
	legacyWindowsTaskName     = "drill-daemon"
)
...
//
// It returns true when DRILL_TEST_START_DAEMON=1 is set (the production escape
...
func defaultServiceManagerBypassed() bool {
	if os.Getenv("DRILL_TEST_START_DAEMON") == "1" {
		return true
	}
...
//
// Without scoping, the launchd label com.squad.drill.daemon (and
// its systemd/Windows equivalents) is a shared slot. Any drill process
// on the machine can `launchctl bootout gui/<uid>/com.squad.drill.daemon`
...
// daemon's DRILL_HOME. managedServiceInstalled(p) stats a non-existent scoped
// plist, returns false, and Stop never reaches serviceCommandRunner.
//
// A secondary benefit: multiple concurrent DRILL_HOMEs (e.g. a dev vs prod
// drill install) each get their own managed daemon and can coexist.
