...
func TestInitIsIdempotent(t *testing.T) {
	...
	skillPath := filepath.Join(h.HomeDir, ".claude", "skills", "drill", "SKILL.md")
	...
	// The drill remote must still be wired after the refresh.
	if out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "drill"); err != nil {
		t.Fatalf("drill remote missing after re-init: %v\n%s", err, out)
	}
}

// TestInitLegacyNotice proves init in a repo that still carries a vendored
// skill copy from an older drill version points it out without touching
...
func TestInitLegacyNotice(t *testing.T) {
	...
	legacy := filepath.Join(h.WorkDir, ".agents", "skills", "drill", "SKILL.md")
	...
	if !strings.Contains(out, filepath.Join(".agents", "skills", "drill", "SKILL.md")) {
...
//
//	init: add remote: remote "drill" already exists with url "..."
//
// The test name is deliberately short: it becomes part of t.TempDir(), which
// hosts DRILL_HOME, and the daemon's Unix socket path under it must stay within
...
func TestInitRepoRename(t *testing.T) {
	...
	gateURLOut, err := h.runGit(ctx, h.WorkDir, "remote", "get-url", "drill")
	if err != nil {
	...
	if out, err := h.runGit(ctx, renamed, "remote", "get-url", "drill"); err != nil {
		t.Fatalf("drill remote missing after rename re-init: %v\n%s", err, out)
...
// `drill init` from inside a Git submodule checkout must register the
...
func TestInitRollsBackWhenDaemonStartFails(t *testing.T) {
	...
	ctx := context.Background()
	if out, err := h.runGit(ctx, h.WorkDir, "remote", "get-url", "drill"); err == nil {
