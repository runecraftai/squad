...
// Address the server binds to (SQ_REPORT_HOST). Defaults to loopback. A wildcard value
// (0.0.0.0 or ::) binds every interface.
export function bindHost(env = process.env) {
  return env.SQ_REPORT_HOST?.trim() || LOOPBACK_HOST;
}
...
// Hostname written into the session URLs the server generates (SQ_REPORT_LINK_HOST).
// Defaults to the host the CLI dials.
export function linkHost(env = process.env) {
  return env.SQ_REPORT_LINK_HOST?.trim() || clientHost(env);
}

// Extra Host header values the server's DNS-rebinding guard accepts beyond the
// loopback names and the resolved bind/link host, set via SQ_REPORT_ALLOWED_HOSTS
...
export function extraAllowedHosts(env = process.env) {
  return (env.SQ_REPORT_ALLOWED_HOSTS || "").split(/\s+/).filter(Boolean);
}
...
export function stateDir() {
  return process.env.SQ_REPORT_STATE_DIR || path.join(os.homedir(), ".sq-report");
}
...
export function defaultPort() {
  return Number(process.env.SQ_REPORT_PORT || 4387);
}
