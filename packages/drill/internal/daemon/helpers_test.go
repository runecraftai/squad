...
func TestMain(m *testing.M) {
	switch os.Getenv("DRILL_DAEMON_HELPER_PROCESS") {
	case "1":
		if capturePath := os.Getenv("DRILL_CAPTURE_DRILL_HOME_FILE"); capturePath != "" {
			_ = os.WriteFile(capturePath, []byte(os.Getenv("DRILL_HOME")), 0o644)
		}
	...
	// The post-receive hook embeds os.Executable() as DRILL_BIN. In tests that
...
func setupTestGitRepo(t *testing.T, p *paths.Paths, d *db.DB, repoID string) (*db.Repo, string) {
	...
	if err := os.WriteFile(filepath.Join(workDir, ".drill.yaml"), []byte("auto_fix:\n  lint: 0\n  test: 0\n  review: 0\n"), 0o644); err != nil {
		t.Fatal(err)
	}
