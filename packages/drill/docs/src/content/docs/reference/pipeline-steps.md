...
This is the per-step reference. For the overview and rationale, see [Pipeline](/drill/concepts/pipeline/). For the fix loop, see [Auto-Fix Loop](/drill/concepts/auto-fix/).
...
The steering still allows requested test evidence under the managed temporary `drill-evidence` directory, plus incidental temp or cache writes from normal development tools.
Configured shell commands and one-shot agent subprocesses are scoped to their step: when the invocation exits, fails, or is cancelled, drill terminates remaining child processes it spawned so background workers do not outlive the run.
When configured Test or Lint command output exceeds 64 KiB, the complete output remains in the authoritative step log while findings, IPC responses, and repair prompts receive a valid-UTF-8 head-and-tail projection capped at 64 KiB. The truncation marker reports the exact original and omitted byte counts and points to `drill axi logs --step <step> --full` for the complete output.
Commits created by the shared Review, Test, Document, and Lint fix path use the configurable [`commit.fix_message`](/drill/reference/global-config/#commitfix_message) template.
...
- With GitHub fork routing, the pushed-branch target is the fork branch fetched into `refs/remotes/drill-push/<branch>`
...
- Includes user intent when the run has supplied intent or transcript matching found a relevant local agent session; the detailed provenance semantics are documented in [Intent extraction](/drill/guides/agents/#intent-extraction)
...
- With the default `session_reuse: true`, Claude, Codex, opencode, and pi reuse one durable fixer session across review-fix turns; a resume failure retries the same fix turn in a fresh fixer session, and unsupported agents run cold
- When `topology: specialized` is configured, emits operational progress (batch ID, per-lens status, consolidator state) visible in the TUI sidebar and `axi status`; never exposes agent prompts or outputs
...
At entry to every remaining step in the fixed pipeline order - Test, Document, Lint, Push, PR, and CI - drill compares the live worktree `HEAD` with the pipeline-recorded head. An equal head or a pipeline-descendant commit continues. A backward reset, divergent sibling, or unverifiable relationship fails the run before that step performs work, including for steps that would not create a commit.
...
[`commands.test`](/drill/reference/repo-config/#commandstest) owns the configuration contract for any explicit baseline command.
...
- Commits any uncommitted agent changes with message `drill: apply agent fixes`
...
- Bitbucket Cloud credentials are missing (`DRILL_BITBUCKET_EMAIL` or `DRILL_BITBUCKET_API_TOKEN`)
...
Immediately after the existing `Updates from [git push drill](https://github.com/runecraftai/squad/packages/drill)` signature, drill writes one stable HTML comment:

```html
<!-- drill-pipeline-attestation:v1 {"head_sha":"0123456789abcdef0123456789abcdef01234567","steps":[{"step":"review","status":"completed"}]} -->
```

The `v1` payload is compact JSON with these required fields:

- `head_sha`: the exact git commit SHA recorded for the run when drill writes the PR body
...
Items are ordered by the fixed pipeline order and represent the exact database snapshot when drill creates or updates the PR body. The attestation includes `pr` and `ci` records even though their human-readable details are not shown in `## Pipeline`; at the normal PR write point those records are commonly `running` and `pending`. The `head_sha` binds that snapshot to the commit it describes, so consumers can detect when a later push has made the comment stale. It is not refreshed after the PR step unless drill writes the body again.

The comment is intentionally data only. It does not declare any step required, passed for a policy, compliant, or mergeable. Consumers can parse the versioned JSON without scraping prose and apply their own policy. The comment stays with the Pipeline header when drill truncates older human-readable update details to fit a PR-body limit.
...
- Bitbucket Cloud requires `DRILL_BITBUCKET_EMAIL` and `DRILL_BITBUCKET_API_TOKEN`.
...
- The [`ci_timeout` reference](/drill/reference/global-config/#ci_timeout) owns idle re-arming, unlimited monitoring, and fail-closed reconciliation while that gate is parked
- On GitHub, GitLab, and Azure DevOps, polls provider mergeability alongside CI checks while the PR remains open
- While the PR stays open, the TUI and terminal title show `Checks passed` once CI readiness is established and known mergeability is clear, and `drill axi` returns `outcome: checks-passed` with successful-output reporting instructions so agents can summarize the run, ask the user to review and merge, and list any pipeline fixes instead of waiting
- An empty forge check list is never treated as green unless the trusted default-branch config declares [`no_ci: true`](/drill/reference/repo-config/#no_ci). That declaration is positive durable evidence the repository intentionally has no CI; absence means CI is expected and delayed registration stays not-ready. If checks still appear on a declared no-CI repo, their actual states are honored
...
- Once every check has finished, classifies each terminally failed check by the provider's own reported outcome before anything escalates; [`ci.rerun_transient`](/drill/reference/repo-config/#cirerun_transient) owns which outcomes count as the provider reporting itself
