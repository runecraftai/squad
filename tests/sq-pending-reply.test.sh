#!/usr/bin/env bash
# Parent-owned XO pending-reply guards (bin/sq-pending-reply-lib.sh).
#
...
#   9. Direct unmarked commander input creates no expectation
#  10. sq-send XO path embeds corr and creates durable pending records
#  11. Backend busy/idle observation works through the shared busy abstraction
#      used by Pi/Claude XO backends (no conversation scrape)
set -u
...
REPORT="$ROOT/bin/sq-xo-report.sh"
TMP_ROOT=$(fm_test_tmproot sq-pending-reply)

export SQUAD_PENDING_REPLY_GRACE_SECS=0
export SQUAD_SEND_SETTLE=0
...
run_send() {
  ...
  env PATH="$fb:$PATH" \
    ...
    SQUAD_PENDING_REPLY_GRACE_SECS=0 \
    "$SEND" "$@" 2>/dev/null
}
...
test_normal_correlated_reply_resolves_once() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=1000
...
test_completed_turn_no_report_triggers_one_recovery() {
  ...
  : > "$hook_log"
  export SQUAD_PENDING_REPLY_NOW=2000
  export SQUAD_PENDING_REPLY_SEND_HOOK='printf "%s\t%s\n" >>"'"$hook_log"'"'
  # The hook above is wrong for eval form - use a function.
  # Invoked indirectly through SQUAD_PENDING_REPLY_SEND_HOOK.
  # shellcheck disable=SC2329
  recovery_hook() {
    printf '%s\t%s\n' "$1" "$2" >> "$hook_log"
  }
  export -f recovery_hook
  export SQUAD_PENDING_REPLY_SEND_HOOK='recovery_hook'
...
test_recovery_attempt_is_never_reinjected() {
  ...
  : > "$hook_log"
  export SQUAD_PENDING_REPLY_NOW=2500
  recovery_fail_hook() {
    printf 'attempted\n' >> "$hook_log"
    return 1
  }
  export -f recovery_fail_hook
  export SQUAD_PENDING_REPLY_SEND_HOOK=recovery_fail_hook
  ...
  unset SQUAD_PENDING_REPLY_SEND_HOOK
...
test_recovery_reply_resolves_original() {
  ...
  export -f recovery_hook
  export SQUAD_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export SQUAD_PENDING_REPLY_NOW=3000
...
test_second_missed_turn_escalates_once_and_stays_durable() {
  ...
  export -f recovery_hook
  export SQUAD_PENDING_REPLY_SEND_HOOK='recovery_hook'
  export SQUAD_PENDING_REPLY_NOW=4000
  # Do not export STATE into the test process: sq-send resolves
  # SQUAD_STATE_OVERRIDE/STATE from the environment and a leak breaks later cases.
  ...
  [ ! -s "$state/.stand-to-queue" ] || fail "direct escalation must not enqueue a duplicate check wake"
...
test_escalation_publication_failure_retries() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4500
...
test_legacy_escalation_closes_default_decision() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4725
...
test_legacy_escalation_does_not_close_taken_default_decision() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4750
...
test_foreign_blocker_is_not_selected_as_escalation() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4775
  export SQUAD_PENDING_REPLY_SEND_HOOK=true
...
test_concurrent_resolution_closes_escalation_once() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4800
...
test_concurrent_escalation_yields_to_late_reply() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=4900
...
test_transport_success_is_not_reply_success() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=5000
...
test_undelivered_records_are_scan_immutable() {
  (
    ...
    export SQUAD_PENDING_REPLY_NOW=5500
    ...
    fm_write_XO_meta "$state/hibit.meta" "$sm_home" "sess:sq-hibit"
    before=$(cat "$rec")
    if fm_pending_reply_try_resolve "$state" "$corr"; then
    ...
    fm_backend_busy_state() { fail "undelivered sentry tick must not probe the backend"; }
    fm_backend_capture() { fail "undelivered sentry tick must not capture the backend"; }
    fm_pending_reply_tick "$state" || fail "undelivered sentry tick should succeed"
    after=$(cat "$rec")
...
test_delivery_confirmation_fallback_reconciles() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export SQUAD_PENDING_REPLY_NOW=5750
    ...
    fm_pending_reply_tick_one "$state" "$corr" unknown \
      || fail "sentry should reconcile the delivery marker"
    [ "$(fm_pending_reply_get "$rec" delivered_epoch)" = 5750 ] \
      || fail "sentry should restore the confirmed delivery epoch"
    ...
    fm_pending_reply_tick_one "$state" "$prepared_corr" unknown \
      || fail "sentry should preserve interrupted delivery state"
    ...
    export SQUAD_PENDING_REPLY_NOW=5760
    ...
    fm_pending_reply_tick "$state" || fail "sentry should accept a late delivery report"
    ...
    export SQUAD_PENDING_REPLY_NOW=5800
    ...
    fm_pending_reply_tick_one "$state" "$reported_corr" unknown \
      || fail "resolved attempted delivery should remain inert in sentry"
...
test_unrelated_and_stale_corr_cannot_resolve() {
  ...
  export SQUAD_PENDING_REPLY_NOW=6000
...
test_restart_preserves_expectation_and_parent_destination() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=7000
...
test_wrong_home_detected_not_acknowledged() {
  ...
  mkdir -p "$sm_home/state"
  export SQUAD_PENDING_REPLY_NOW=8000
  corr=$(fm_pending_reply_create "$home" "$state" "hibit" "report to parent")
  fm_pending_reply_mark_delivered "$state" "$corr"
  # Historical incident shape: report written under the XO home.
...
test_unmarked_commander_input_creates_no_expectation() {
  ...
  [ "$(cat "$log")" = "commander says hello" ] \
    || fail "operator send should stay unmarked"$'\n'"$(cat "$log" | od -An -c)"
  ...
  pass "direct unmarked commander input creates no expectation"
}
...
test_fm_send_marked_XO_creates_pending_and_embeds_corr() {
  ...
  fm_write_XO_meta "$home/state/hibit.meta" "$home/sm" "sess:sq-hibit"
  run_send "$fb" "$home" "$log" "hibit" "audit the build"; rc=$?
  expect_code 0 "$rc" "XO send should succeed"
  got=$(cat "$log")
  case "$got" in
    "$SQUAD_FROMFIRST_MARK"corr=*) : ;;
    *) fail "XO send must embed marker+corr"$'\n'"$(printf '%s' "$got" | od -An -c)" ;;
  esac
  ...
  [ "$(fm_pending_reply_get "$rec" task_id)" = hibit ] \
    || fail "task_id must match XO id"
  pass "sq-send marked XO path creates pending and embeds corr"
}
...
test_document_pointer_resolves() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=9000
...
test_helper_report_resolves() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=9100
...
test_busy_idle_observation_via_backend_abstraction() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=9200
  corr=$(fm_pending_reply_create "$home" "$state" "hibit" "backend turn")
  fm_pending_reply_mark_delivered "$state" "$corr"
  # Simulates Pi/Claude XO busy_state from fm_backend_busy_state without
...
test_unknown_backend_state_uses_capture_fallback() {
  ...
  for backend in tmux zellij; do
    (
      ...
      sm_home="$home/sm"
      mkdir -p "$sm_home/state"
      export SQUAD_PENDING_REPLY_GRACE_SECS=10
      # These fixture overrides are intentionally scoped to the isolated subshell.
      # shellcheck disable=SC2030,SC2031
      export SQUAD_PENDING_REPLY_NOW=10000
      corr=$(fm_pending_reply_create "$home" "$state" "hibit" "$backend fallback")
      fm_pending_reply_mark_delivered "$state" "$corr"
      fm_write_XO_meta "$state/hibit.meta" "$sm_home" "session:sq-hibit" alpha pi
      [ "$backend" = tmux ] || printf 'backend=%s\n' "$backend" >> "$state/hibit.meta"
      fm_backend_busy_state() { printf 'unknown'; }
      fm_backend_capture() { printf '%s' "$SQUAD_PENDING_TEST_CAPTURE"; }
      # Invoked indirectly through SQUAD_PENDING_REPLY_SEND_HOOK.
      # shellcheck disable=SC2329
      recovery_hook() { :; }
      ...
      # shellcheck disable=SC2030,SC2031
      export SQUAD_PENDING_REPLY_SEND_HOOK=recovery_hook
      export SQUAD_PENDING_TEST_CAPTURE='idle footer'
      ...
      # shellcheck disable=SC2030,SC2031
      export SQUAD_PENDING_REPLY_NOW=10010
      fm_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = recovery_sent ] \
        || fail "$backend fallback idle should trigger recovery after grace"
      export SQUAD_PENDING_REPLY_NOW=10011
      export SQUAD_PENDING_TEST_CAPTURE='Working...'
      fm_pending_reply_tick "$state"
      export SQUAD_PENDING_REPLY_NOW=10012
      export SQUAD_PENDING_TEST_CAPTURE='idle footer'
      fm_pending_reply_tick "$state"
      [ "$(phase_of "$state" "$corr")" = escalated ] \
...
test_kimi_capture_fallback_uses_recorded_harness() (
  ...
  # This fixture clock is intentionally scoped to the isolated subshell.
  # shellcheck disable=SC2030,SC2031
  export SQUAD_PENDING_REPLY_NOW=10020
  corr=$(fm_pending_reply_create "$home" "$state" hibit "kimi fallback")
  fm_pending_reply_mark_delivered "$state" "$corr"
  fm_write_XO_meta "$state/hibit.meta" "$sm_home" "session:sq-hibit" alpha kimi
  fm_backend_busy_state() { printf 'unknown'; }
  fm_backend_capture() { printf '%s' "$SQUAD_PENDING_KIMI_CAPTURE"; }
  export SQUAD_PENDING_KIMI_CAPTURE=' 🌑 · Tip: ask Kimi to schedule tasks, e.g. "remind me at 5pm"'

  [ "$(fm_pending_reply_backend_observation tmux session:sq-hibit sq-hibit codex)" = fallback-idle ] \
    || fail "Kimi spinner leaked into another harness"
  export SQUAD_PENDING_KIMI_CAPTURE='Ctrl+c:cancel'
  [ "$(fm_pending_reply_backend_observation tmux session:sq-hibit sq-hibit kimi)" = fallback-idle ] \
  ...
  [ "$(phase_of "$state" "$corr")" = awaiting_report ] \
    || fail "working Kimi XO entered recovery"
...
test_tick_skips_terminal_and_reuses_target_observation() {
  (
    ...
    # This fixture clock is intentionally scoped to the isolated subshell.
    # shellcheck disable=SC2030,SC2031
    export SQUAD_PENDING_REPLY_NOW=10100
    ...
    fm_write_XO_meta "$state/hibit.meta" "$home/hibit" "sess:sq-hibit"
    fm_write_XO_meta "$state/resolved.meta" "$home/resolved" "sess:sq-resolved"
    fm_write_XO_meta "$state/escalated.meta" "$home/escalated" "sess:sq-escalated"
...
test_correlations_reuse_only_for_matching_open_task() {
  ...
  state="$home/state"
  fm_write_XO_meta "$state/domain.meta" "$home/domain" "sess:sq-domain"
  fm_write_XO_meta "$state/other.meta" "$home/other" "sess:sq-other"
  run_send "$fb" "$home" "$log" domain "first request" || fail "first marked send failed"
  got=$(cat "$log")
  corr1=$(fm_pending_reply_extract_corr "$got")
  export SQUAD_PENDING_REPLY_EXISTING_CORR=$corr1
  run_send "$fb" "$home" "$log" other "forwarded request" || fail "cross-task send failed"
  unset SQUAD_PENDING_REPLY_EXISTING_CORR
  ...
  run_send "$fb" "$home" "$log" domain "${SQUAD_FROMFIRST_MARK}corr=${corr1} follow-up" \
...
test_tick_end_to_end_missed_then_escalate() {
  ...
  export SQUAD_PENDING_REPLY_SEND_HOOK='recovery_hook'
  # shellcheck disable=SC2031
  export SQUAD_PENDING_REPLY_NOW=9300
  ...
  export SQUAD_PENDING_REPLY_NOW=999999
  fm_pending_reply_tick_one "$state" "$corr" idle "$sm_home"
...
test_failed_send_discards_undelivered_expectation() {
  ...
  state="$home/state"
  export SQUAD_PENDING_REPLY_NOW=9400
