...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/sq-herdr-session-cleanup-e2e.XXXXXX")
...
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name sq-herdr-session-start-stale-projection-cleanup-r1)
...
ANCHOR=$(lab workspace create --cwd "$ROOT" --label commander-anchor --focus) || fail 'could not create focus anchor'
...
pass 'real named lab cleanup is idempotent and leaves the default unit session to the teardown tripwire'
