...
const (
	...
	drillPRSignature                       = "Updates from [git push drill](https://github.com/runecraftai/squad/packages/drill)"
	pipelineAttestationCommentPrefix       = "<!-- drill-pipeline-attestation:v1 "
...
func BuildPipelineSummary(steps []*db.StepResult, rounds map[string][]*db.StepRound, headSHA string) (string, string) {
	...
	b.WriteString(drillPRSignature)
	b.WriteString("\n\n")
...
// when drill writes the PR body. Its compact JSON is deliberately data
