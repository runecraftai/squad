...
func TestTestStep_FixMode(t *testing.T) {
	...
	if got := lastCommitMessage(t, dir); got != "drill(test): fix test failures" {
...
func TestTestStep_UserIntentRunsConfiguredCommandThenEvidenceAgent(t *testing.T) {
	...
	for _, want := range []string{
		...
		filepath.Join(os.TempDir(), "drill-evidence", sctx.Run.ID),
	...
	if _, err := os.Stat(filepath.Join(os.TempDir(), "drill-evidence", sctx.Run.ID)); err != nil {
...
func TestTestStep_EvidenceDirectoryIsAlwaysOutsideTheWorktree(t *testing.T) {
	...
	prompt := ag.calls[0].Prompt
	wantDir := filepath.Join(os.TempDir(), "drill-evidence", sctx.Run.ID)
	...
	if _, err := os.Stat(filepath.Join(dir, ".drill")); err == nil {
...
func TestTestStep_PublishedEvidenceGuidanceNamesTheEvidenceBranch(t *testing.T) {
	...
	sctx := newTestContextWithDBRecords(t, ag, dir, baseSHA, headSHA, config.Commands{})
	sctx.UserIntent = "Show users a success screen after checkout"
	sctx.Config.Test.Evidence = config.Evidence{StoreInRepo: true, Dir: ".drill/evidence", Branch: "team/ci/evidence"}
