...
REVIEW_DIFF="$ROOT/bin/sq-review-diff.sh"
TMP_ROOT=$(fm_test_tmproot sq-review-diff-tests)
...
test_pr_meta_uses_pr_head_not_stale_local() {
  ...
  pass "sq-review-diff falls back to recorded pr_head when pull head cannot be fetched"
}
...
test_stale_recorded_pr_head_loses_to_fetched_pull_head() {
  ...
  pass "sq-review-diff prefers freshly fetched PR head over a stale recorded pr_head="
}
...
test_pr_meta_fetches_pull_head_without_recorded_sha() {
  ...
  pass "sq-review-diff fetches refs/pull/<n>/head when pr_head= is absent"
}
...
test_no_pr_meta_uses_local_branch() {
  ...
  pass "sq-review-diff without pr= keeps the worktree-branch diff"
}
...
test_unreachable_pr_head_falls_back_with_warning() {
  ...
  pass "sq-review-diff falls back to local branch with a warning when PR head is unreachable"
}
