...
The arm, cd, and delegation guards have dedicated reference pages in [`arm-pretool-check.md`](arm-pretool-check.md), [`cd-guard.md`](cd-guard.md), and [`subagent-guard.md`](subagent-guard.md), while the backend-raw and poll-loop policies are owned by their script headers (`bin/sq-backend-pretool-check.sh` and `bin/sq-poll-pretool-check.sh`).
