...
test("createSkillMarkdown uses non-interactive npx commands", () => {
  ...
  assert.match(md, /`npx -y @runecraft\/report <html-file>`/);
  assert.match(md, /If sq-report output shows a follow-up command starting with `sq-report`/);
  assert.match(md, /run it as `npx -y @runecraft\/report/);
...
test("createSkillMarkdown documents installed-copy fallback for restricted sandboxes", () => {
  ...
  assert.match(md, /`node "\$\(npm root\)\/@runecraft\/report\/dist\/cli\.mjs" <html-file>`/);
  assert.match(md, /`node "\$\(npm root -g\)\/@runecraft\/report\/dist\/cli\.mjs" <html-file>`/);
