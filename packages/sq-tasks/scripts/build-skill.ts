...
const target = new URL("../skills/sq-tasks/SKILL.md", import.meta.url);
...
if (check) {
  ...
  if (actual === null || normalizeLineEndings(actual) !== expected) {
    console.error(
      "skills/sq-tasks/SKILL.md is out of date. Run `pnpm run build:skill` and commit the result.",
    );
    process.exit(1);
  }
  console.log("skills/sq-tasks/SKILL.md is up to date.");
} else {
  await mkdir(new URL("../skills/sq-tasks/", import.meta.url), {
    recursive: true,
  });
