...
// recorded DRILL_HOME (so the shared service is never a target).
...
// user DRILL_HOME or other non-temp paths even if inventory is corrupted.
func isAllowedTempRoot(drillHome string) bool {
	drillHome = filepath.Clean(drillHome)
	if drillHome == "" || drillHome == "." || drillHome == string(filepath.Separator) {
		return false
	}
	// Reject the default shared root spellings.
	if home, err := os.UserHomeDir(); err == nil {
		shared := filepath.Join(home, ".drill")
		if samePath(drillHome, shared) {
			return false
		}
		if resolved, err := filepath.EvalSymlinks(shared); err == nil && samePath(drillHome, resolved) {
			return false
		}
	}
	...
	lower := strings.ToLower(drillHome)
...
func Acquire(drillHome, drillBin string, slotTimeout time.Duration) (*Ownership, error) {
	inv, err := Open()
	if err != nil {
		return nil, err
	}
