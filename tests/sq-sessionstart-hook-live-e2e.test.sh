...
#
# Three facts in this area come from the vendor, not from Squad, so a stub
...
#       (bin/sq-startup-network.sh), so a harness that reaps the hook's process
...
# tests/sq-sessionstart-nudge.test.sh pins that routing portably with real
# processes and no harness. This guard covers only what CI cannot see.
#
# It swaps a RECORDER in for bin/sq-sessionstart-run.sh inside a throwaway lab
# checkout, so nothing here touches a real home, lock, or unit. The recorder
...
if [ "${SQUAD_SESSIONSTART_HOOK_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_SESSIONSTART_HOOK_LIVE_E2E=1 to run the live session-open hook regression"
  exit 0
fi
...
LAB="${TMPDIR:-/tmp}/sq-sessionstart-hook-live-e2e.$$"
SOCKET="sq-ss-hook-$$"
CHECKED=0
ABSENT=
...
# --- lab ---------------------------------------------------------------------
#
# A Squad-shaped checkout carrying the harness's own TRACKED registration,
...
make_lab() {  # <harness> -> echoes lab dir
  ...
  for stub in sq-turnend-guard.sh sq-claude-stop-autoarm.sh sq-arm-pretool-check.sh \
    sq-cd-pretool-check.sh sq-subagent-pretool-check.sh; do
  ...
  # tests/sq-bootstrap.test.sh already owns.
  ln -sf "$ROOT/bin/sq-startup-network.sh" "$lab/bin/sq-startup-network.sh"
  ln -sf "$ROOT/bin/sq-timeout-lib.sh" "$lab/bin/sq-timeout-lib.sh"
  ln -sf "$ROOT/bin/sq-stand-to-lib.sh" "$lab/bin/sq-stand-to-lib.sh"
  ln -sf "$ROOT/bin/sq-session-lock-lib.sh" "$lab/bin/sq-session-lock-lib.sh"
  cat > "$lab/bin/sq-bootstrap.sh" <<'SH'
#!/usr/bin/env bash
...
set -u
sleep 6
printf 'detached worker survived the hook\n' > "${SQUAD_LIVE_DETACH_MARKER:?}"
exit 0
SH
  chmod +x "$lab/bin/sq-bootstrap.sh"

  cat > "$lab/bin/sq-sessionstart-run.sh" <<'SH'
#!/usr/bin/env bash
...
printf '%s\n' "$source" >> "$record"
# Exactly what bin/sq-session-start.sh does after taking the lock.
if [ -n "${SQUAD_LIVE_DETACH_MARKER:-}" ]; then
  "$(dirname "$0")/sq-startup-network.sh" start --locked 0 --harvest-pid $$ >/dev/null 2>&1 || true
fi
printf 'FMHOOKTOKEN-%s-%s-%s\n' "$source" "$(grep -c . "$record" | tr -d ' ')" "${SQUAD_LIVE_NONCE:?}"
exit 0
SH
  chmod +x "$lab/bin/sq-sessionstart-run.sh"
  ...
  case "$harness" in
    ...
    pi)
      ...
      cp "$ROOT/.pi/extensions/sq-primary-turnend-guard.ts" "$lab/.pi/extensions/"
      cp "$ROOT/.pi/extensions/lib/sq-operational-input.ts" "$lab/.pi/extensions/lib/"
      cp "$ROOT/bin/sq-operational-input.sh" "$lab/bin/"
  ...
  case "$harness" in
    ...
    pi)
      probe_process_opens pi "$version" "$lab" startup \
        pi -p -e "$lab/.pi/extensions/sq-primary-turnend-guard.ts" --no-context-files --no-tools --no-session \
        -- pi -p -c -e "$lab/.pi/extensions/sq-primary-turnend-guard.ts" --no-context-files --no-tools
      probe_context_reset pi "$version" "$lab" /new \
        pi -e "$lab/.pi/extensions/sq-primary-turnend-guard.ts" --no-context-files
      ;;
  esac
  CHECKED=$((CHECKED + 1))
done
...
echo "# sq-sessionstart-hook-live-e2e.test.sh: all live assertions passed"
