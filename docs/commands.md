...
Three PreToolUse seatbelts mechanically enforce the sharpest-edged of these rules regardless of what the agent remembers: `bin/sq-cd-pretool-check.sh` blocks a persistent top-level `cd`/`pushd`/`popd` in the primary checkout ([`docs/cd-guard.md`](cd-guard.md)), `bin/sq-arm-pretool-check.sh` blocks a malformed or bypassed sentry-arm/checkpoint command ([`docs/arm-pretool-check.md`](arm-pretool-check.md)), and `bin/sq-subagent-pretool-check.sh` blocks primary-session delegation outside the unit ([`docs/subagent-guard.md`](subagent-guard.md)).
...
- [docs/cd-guard.md](cd-guard.md), [docs/arm-pretool-check.md](arm-pretool-check.md), [docs/subagent-guard.md](subagent-guard.md) - the three PreToolUse seatbelts that mechanically enforce a subset of these rules.
