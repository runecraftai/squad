#!/usr/bin/env bash
# tests/sq-backend-tmux-smoke.test.sh - real tmux smoke test for the tmux
...
# list, kill)" from data/sq-backend-design-d7/report.md. Every other suite in
...
SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sq-backend-smoke.XXXXXX")
...
# A second create for the SAME window name must refuse (mirrors sq-spawn.sh's
...
fm_backend_tmux_send_text_line "$TARGET" "printf 'commander-on-deck-%s\\n' line" \
  || fail "fm_backend_tmux_send_text_line failed"
wait_for_capture_text "$TARGET" "commander-on-deck-line" \
...
# --- send_literal + send_key(Enter), the two-step form sq-spawn.sh uses for the
...
wait_for_capture_text "$TARGET" "literal-then-key-commander" \
...
# far enough to still see the earliest line - the same -S -N bounding sq-peek.sh
# and sq-sentry.sh rely on for a bounded, cheap pane read.
