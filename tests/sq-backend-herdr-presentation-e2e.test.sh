...
# owning-parent ordering across primary and XO homes.
# The test drives the real spawn and teardown scripts, a real FOB pool,
# and the guarded named-session lab helper.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
HERDR_LAB_HELPER=${HERDR_LAB_HELPER:-$ROOT/bin/sq-herdr-lab.sh}
...
command -v fob >/dev/null 2>&1 || { echo "skip: fob not found"; exit 0; }
...
exit "$status"
SH
chmod +x "$FAKEBIN/herdr" "$FAKEBIN/fob"
chmod +x "$FAKEBIN/herdr-workspace-mover"
export PATH="$FAKEBIN:$PATH"
export SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$FAKEBIN/herdr-workspace-mover"
...
HERDR_LAB_SESSION=$(PATH="$HERDR_ORIGINAL_PATH" \
  "$HERDR_LAB_HELPER" name sq-herdr-presentation-projection)
...
# defect is demonstrated by tests/sq-backend-herdr-focus-flash-e2e.test.sh);
...
printf 'Hi Bit-style projection restart fixture.\necho done >> %s.status\n' "$HOME_DIR/data/sq-hibit-resume-r1" > "$HOME_DIR/data/sq-hibit-resume-r1/brief.md"
printf 'Wheelhouse-style projection restart fixture.\necho done >> %s.status\n' "$HOME_DIR/data/wheelhouse-healing-r1" > "$HOME_DIR/data/wheelhouse-healing-r1/brief.md"
printf 'Projection active seeded fixture.\necho done >> %s.status\n' "$HOME_DIR/data/active-seeded" > "$HOME_DIR/data/active-seeded/brief.md"
printf 'Projection abort fixture A.\necho done >> %s.status\n' "$HOME_DIR/data/abort-a" > "$HOME_DIR/data/abort-a/brief.md"
printf 'Projection abort fixture B.\necho done >> %s.status\n' "$HOME_DIR/data/abort-b" > "$HOME_DIR/data/abort-b/brief.md"
printf 'Projection lock contention fixture.\necho done >> %s.status\n' "$HOME_DIR/data/lock-contended" > "$HOME_DIR/data/lock-contended/brief.md"
printf 'Projection default-on fixture.\necho done >> %s.status\n' "$HOME_DIR/data/default-on" > "$HOME_DIR/data/default-on/brief.md"
make_project "$PROJECT_DIR"

# Keep one ordinary primary task live so the durable Squad workspace is
...
SQUAD_WSID=$(grep '^herdr_workspace_id=' "$ANCHOR_META" | cut -d= -f2-)
[ -n "$SQUAD_WSID" ] || fail "anchor metadata did not record the Squad workspace"
...
if [ "$FLOOR_VERDICT" = 0 ]; then
  ...
  [ -n "$DEFAULT_ON_WSID" ] && [ "$DEFAULT_ON_WSID" != "$SQUAD_WSID" ] \
    || fail "an unconfigured home reused the flat Squad workspace instead of projecting"
  ...
  [ "$DEFAULT_ON_WSID" = "$SQUAD_WSID" ] \
    || fail "an unconfigured home did not land in the flat Squad workspace on below-floor herdr $FLOOR_VERSION (got '${DEFAULT_ON_WSID:-<empty>}')"
...
SECOND_ONE_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label xo-alpha --no-focus) \
  || fail "could not create the first XO presentation fixture"
SECOND_TWO_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label xo-bravo --focus) \
  || fail "could not create the focused XO presentation fixture"
...
[ -n "$SECOND_ONE_WSID" ] && [ -n "$SECOND_TWO_WSID" ] && [ -n "$SECOND_TWO_TAB" ] && [ -n "$SECOND_TWO_PANE" ] \
  || fail "XO presentation fixtures returned incomplete IDs"
...
SECOND_TWO_INFO=$(lab workspace get "$SECOND_TWO_WSID") || fail "focused XO disappeared during projected create"
[ "$(printf '%s' "$SECOND_TWO_INFO" | jq -r '.result.workspace.focused')" = true ] \
  || fail "projected create or workspace.move stole focus from the commander's current space"
...
grep -F "target is the commander's active tab" "$TMP_ROOT/active-seeded.err" >/dev/null 2>&1 \
...
lab tab focus "$SECOND_TWO_TAB" >/dev/null || fail "could not restore the captured commander tab after the active seeded-tab fixture"
assert_focus_is "$COMMANDER_FOCUS" "active seeded-tab fixture restoration"
...
assert_focus_is "$COMMANDER_FOCUS" "active seeded-tab fixture cleanup"
assert_cleanup_focus_preserved "$ACTIVE_SEEDED_CLEANUP_FOCUS_START" "$ACTIVE_SEEDED_PANE" "$COMMANDER_FOCUS"
...
[ "$LOCK_CONTENTION_WSID" = "$SQUAD_WSID" ] \
  || fail "bounded lock contention did not use the ordinary flat Squad workspace"
...
assert_focus_is "$COMMANDER_FOCUS" "bounded presentation lock flat fallback"
...
assert_focus_is "$COMMANDER_FOCUS" "bounded presentation lock flat fallback teardown"
...
[ "$OFF_WT" = "$ON_WT" ] || fail "FOB did not reuse the same fixture worktree, so byte comparison is inconclusive"
...
assert_focus_is "$COMMANDER_FOCUS" "concurrent projected spawns"
...
EXPECTED_LABELS=$(printf 'Squad\n%s\n%s\nxo-alpha\nxo-bravo' "$PROJECTED_LABEL" "$CREATED_LABELS")
ACTUAL_LABELS=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[].label')
[ "$ACTUAL_LABELS" = "$EXPECTED_LABELS" ] || fail "workspace order was not Squad, stable primary block, XOs: $ACTUAL_LABELS"
PRIMARY_IDS=$(printf '%s' "$ORDER_LIST" | jq -r '
  .result.workspaces[]
  | select((.label | startswith("└ ")) or (.label | startswith("Squad/")))
  | .workspace_id
')
...
SECOND_ORDER_AFTER=$(printf '%s' "$ORDER_LIST" | jq -r '.result.workspaces[] | select(.label | startswith("xo-")) | .workspace_id')
[ "$SECOND_ORDER_AFTER" = "$SECOND_ORDER_BEFORE" ] \
  || fail "primary workspace ordering changed XO relative order"
...
SQUAD_BACKEND_HERDR_WORKSPACE_MOVER="$FAIL_MOVER" \
...
assert_focus_is "$COMMANDER_FOCUS" "failed presentation ordering"
...
assert_focus_is "$COMMANDER_FOCUS" "concurrent post-create abort cleanup"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_A_PANE" "$COMMANDER_FOCUS"
assert_cleanup_focus_preserved "$ABORT_FOCUS_START" "$ABORT_B_PANE" "$COMMANDER_FOCUS"
...
pass "real Herdr lab: FOB commands and metadata shape are byte-identical except for Herdr container IDs"
...
lab pane get "$SECOND_TWO_PANE" >/dev/null 2>&1 \
  || fail "projected teardown affected the focused XO workspace"
...
assert_focus_is "$COMMANDER_FOCUS" "concurrent projected teardowns"
...
assert_focus_is "$COMMANDER_FOCUS" "failed-order projection teardown"
...
for ROUND in 1 2 3; do
  ...
  printf 'Projection focus wave %s fixture A.\necho done >> %s.status\n' "$ROUND" "$HOME_DIR/data/focus-$ROUND-a" > "$HOME_DIR/data/focus-$ROUND-a/brief.md"
  printf 'Projection focus wave %s fixture B.\necho done >> %s.status\n' "$ROUND" "$HOME_DIR/data/focus-$ROUND-b" > "$HOME_DIR/data/focus-$ROUND-b/brief.md"
  ...
  assert_focus_is "$COMMANDER_FOCUS" "focus wave $ROUND concurrent spawns"
  ...
  WAVE_EXPECTED=$(printf 'Squad\n%s\nxo-alpha\nxo-bravo' "$WAVE_LABELS")
  WAVE_ACTUAL=$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "Squad" or (.label | startswith("└ ")) or (.label | startswith("xo-"))) | .label')
  [ "$WAVE_ACTUAL" = "$WAVE_EXPECTED" ] \
    || fail "focus wave $ROUND lost stable contiguous ordering: $WAVE_ACTUAL"
  WAVE_SECOND_ORDER=$(lab workspace list | jq -r '.result.workspaces[] | select(.label | startswith("xo-")) | .workspace_id')
  [ "$WAVE_SECOND_ORDER" = "$SECOND_ORDER_BEFORE" ] \
    || fail "focus wave $ROUND changed XO relative order"
  ...
  assert_focus_is "$COMMANDER_FOCUS" "focus wave $ROUND concurrent teardowns"
  WAVE_REMAINING=$(lab workspace list | jq -r '.result.workspaces[].label')
  [ "$WAVE_REMAINING" = $'Squad\nxo-alpha\nxo-bravo' ] \
...
SECOND_HOME_B="$TMP_ROOT/home-xo-bravo"
...
printf 'alpha\n' > "$SECOND_HOME_A/.sq-xo-home"
printf 'bravo\n' > "$SECOND_HOME_B/.sq-xo-home"
touch "$SECOND_HOME_A/state/.last-sentry-beat" "$SECOND_HOME_B/state/.last-sentry-beat"
# Ensure the XO homes look like gitignored Squad homes so inheritance
...
git -C "$SECOND_HOME_A" -c user.name='Squad Tests' -c user.email='tests@example.invalid' commit -qm init
git -C "$SECOND_HOME_B" -c user.name='Squad Tests' -c user.email='tests@example.invalid' commit -qm init
mkdir -p "$SECOND_HOME_A/bin"
printf '# Squad XO fixture\n' > "$SECOND_HOME_A/AGENTS.md"
printf 'XO alpha charter.\necho done >> %s.status\n' "$SECOND_HOME_A/data" > "$SECOND_HOME_A/data/charter.md"

# Primary setting only; real inheritance must push it into both XO homes.
...
[ ! -e "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "XO A unexpectedly had a local presentation setting before inheritance"
[ ! -e "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "XO B unexpectedly had a local presentation setting before inheritance"
SECOND_SPAWN_LOG_START=$(log_line_count)
spawn_XO_task alpha "$SECOND_HOME_A" > "$TMP_ROOT/alpha.out" 2> "$TMP_ROOT/alpha.err" \
  || fail "XO alpha spawn failed: $(cat "$TMP_ROOT/alpha.err")"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "XO spawn did not inherit the presentation setting"
[ ! -e "$HOME_DIR/state/alpha.herdr-presentation" ] \
  || fail "XO spawn published a presentation journal"
SECOND_META="$HOME_DIR/state/alpha.meta"
[ "$(grep '^kind=' "$SECOND_META" | cut -d= -f2-)" = xo ] \
  || fail "XO spawn did not record kind=xo"
...
[ "$SECOND_LABEL" = xo-alpha ] \
  || fail "XO spawn did not use its flat parent workspace: $SECOND_LABEL"
[ -z "$(projection_labels_from_log "$SECOND_SPAWN_LOG_START")" ] \
  || fail "XO spawn created a corner projection workspace"
if sed -n "$((SECOND_SPAWN_LOG_START + 1)),\$p" "$HERDR_CALL_LOG" \
  | grep -E $'^(workspace\tmove|session\tlist)' >/dev/null 2>&1; then
  fail "XO spawn attempted presentation ordering"
fi
# shellcheck source=/dev/null
. "$ROOT/bin/sq-config-inherit-lib.sh"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_A/config" \
  || fail "inheritance into XO A failed"
propagate_inheritable_config "$HOME_DIR/config" "$SECOND_HOME_B/config" \
  || fail "inheritance into XO B failed"
[ -f "$SECOND_HOME_A/config/herdr-presentation-spaces" ] \
  || fail "primary presentation setting did not reach XO A"
[ -f "$SECOND_HOME_B/config/herdr-presentation-spaces" ] \
  || fail "primary presentation setting did not reach XO B"
pass "real Herdr lab: the primary presentation setting inherits into real XO homes"

# Keep the pre-existing xo-alpha/bravo workspaces as owning parents and commander focus.
assert_focus_is "$COMMANDER_FOCUS" "multi-home commander focus"
...
printf 'Primary multi-home fixture 1.\necho done >> %s.status\n' "$HOME_DIR/data/p1" > "$HOME_DIR/data/p1/brief.md"
printf 'Primary multi-home fixture 2.\necho done >> %s.status\n' "$HOME_DIR/data/p2" > "$HOME_DIR/data/p2/brief.md"
printf 'XO A fixture 1.\necho done >> %s.status\n' "$SECOND_HOME_A/data/a1" > "$SECOND_HOME_A/data/a1/brief.md"
printf 'XO A fixture 2.\necho done >> %s.status\n' "$SECOND_HOME_A/data/a2" > "$SECOND_HOME_A/data/a2/brief.md"
printf 'XO B fixture 1.\necho done >> %s.status\n' "$SECOND_HOME_B/data/b1" > "$SECOND_HOME_B/data/b1/brief.md"
printf 'XO B fixture 2.\necho done >> %s.status\n' "$SECOND_HOME_B/data/b2" > "$SECOND_HOME_B/data/b2/brief.md"
...
assert_focus_is "$COMMANDER_FOCUS" "multi-home sequential spawns"
...
case "$A1_LABEL" in $'└ a1 · p:'*) ;; *) fail "XO A a1 label wrong: $A1_LABEL" ;; esac
case "$A2_LABEL" in $'└ a2 · p:'*) ;; *) fail "XO A a2 label wrong: $A2_LABEL" ;; esac
case "$B1_LABEL" in $'└ b1 · p:'*) ;; *) fail "XO B b1 label wrong: $B1_LABEL" ;; esac
case "$B2_LABEL" in $'└ b2 · p:'*) ;; *) fail "XO B b2 label wrong: $B2_LABEL" ;; esac
...
MULTI_EXPECTED=$(printf '%s\n' \
  Squad "$P1_LABEL" "$P2_LABEL" \
  xo-alpha "$A1_LABEL" "$A2_LABEL" \
  xo-bravo "$B1_LABEL" "$B2_LABEL")
[ "$MULTI_LABELS" = "$MULTI_EXPECTED" ] \
  || fail "multi-home topology was not owning-parent grouped: $MULTI_LABELS"
pass "real Herdr lab: primary and two XO homes each own a top-level contiguous child block"
...
printf 'Cross-home concurrent primary.\necho done >> %s.status\n' "$HOME_DIR/data/pcw" > "$HOME_DIR/data/pcw/brief.md"
printf 'Cross-home concurrent A.\necho done >> %s.status\n' "$SECOND_HOME_A/data/acw" > "$SECOND_HOME_A/data/acw/brief.md"
printf 'Cross-home concurrent B.\necho done >> %s.status\n' "$SECOND_HOME_B/data/bcw" > "$SECOND_HOME_B/data/bcw/brief.md"
...
assert_focus_is "$COMMANDER_FOCUS" "cross-home concurrent wave"
...
printf '%s' "$CROSS_LIST" | jq -e '
  ([.result.workspaces[].label] | index("Squad")) as $fm
  | ([.result.workspaces[].label] | index("xo-alpha")) as $a
  | ([.result.workspaces[].label] | index("xo-bravo")) as $b
...
case "$PCW_LABEL" in $'└ pcw · p:'*|Squad) ;; *) fail "cross-home primary label wrong: $PCW_LABEL" ;; esac
case "$ACW_LABEL" in $'└ acw · p:'*|xo-alpha) ;; *) fail "cross-home A label wrong: $ACW_LABEL" ;; esac
case "$BCW_LABEL" in $'└ bcw · p:'*|xo-bravo) ;; *) fail "cross-home B label wrong: $BCW_LABEL" ;; esac
...
printf 'Flat fallback under session lock contention.\necho done >> %s.status\n' "$SECOND_HOME_A/data/aflat" > "$SECOND_HOME_A/data/aflat/brief.md"
...
[ "$AFLAT_LABEL" = xo-alpha ] \
  || fail "cross-home lock contention did not use the ordinary XO home workspace: $AFLAT_LABEL"
[ ! -e "$SECOND_HOME_A/state/aflat.herdr-presentation" ] \
  || fail "cross-home lock contention published a projection journal"
assert_focus_is "$COMMANDER_FOCUS" "cross-home lock contention flat fallback"
...
pass "real Herdr lab: session lock contention from an XO home falls back flat with no journal"

# Same-identity recovery replaces only one exact agent-free husk in its
# original projected workspace.
# Exercise both the leading sq- identity style seen in Hi Bit work and the
# project-name identity style used by Wheelhouse work.
for RESTART_ID in sq-hibit-resume-r1 wheelhouse-healing-r1; do
  ...
  EXPECTED_CONCISE=${RESTART_ID#sq-}
  ...
  if [ "$RESTART_ID" = sq-hibit-resume-r1 ]; then
...
# An XO child binds and reclaims only inside its own home and parent.
CROSS_RESTART_ID=wheel-child-resume
mkdir -p "$SECOND_HOME_A/data/$CROSS_RESTART_ID"
printf 'Cross-home restart fixture.\necho done >> %s.status\n' "$SECOND_HOME_A/data/$CROSS_RESTART_ID" > "$SECOND_HOME_A/data/$CROSS_RESTART_ID/brief.md"
...
[ "$CROSS_BOUND_HOME" = "$(cd "$SECOND_HOME_A" && pwd -P)" ] \
  || fail "cross-home restart journal did not bind the XO's exact home"
...
[ "$CROSS_NEW_WSID" = "$CROSS_OLD_WSID" ] && [ "$CROSS_NEW_PANE" != "$CROSS_OLD_PANE" ] \
  || fail "cross-home reclaim did not replace one pane inside the same XO child workspace"
...
printf 'Concurrent primary recovery fixture.\necho done >> %s.status\n' "$HOME_DIR/data/$PRIMARY_WAVE_ID" > "$HOME_DIR/data/$PRIMARY_WAVE_ID/brief.md"
printf 'Concurrent XO recovery fixture.\necho done >> %s.status\n' "$SECOND_HOME_B/data/$BRAVO_WAVE_ID" > "$SECOND_HOME_B/data/$BRAVO_WAVE_ID/brief.md"
...
spawn_task "$BRAVO_WAVE_ID" "$SECOND_HOME_B" "$PROJECT_DIR" > "$TMP_ROOT/bravo-wave-first.out" 2> "$TMP_ROOT/bravo-wave-first.err" \
  || fail "XO recovery-wave fixture failed: $(cat "$TMP_ROOT/bravo-wave-first.err")"
...
wait "$BRAVO_WAVE_PID" || fail "concurrent XO recovery failed: $(cat "$TMP_ROOT/bravo-wave-resume.err")"
...
LEGACY_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "Squad/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" --no-focus) \
...
FLAT_TAB_OUT=$(lab tab create --workspace "$(lab workspace list | jq -r '.result.workspaces[] | select(.label == "xo-alpha") | .workspace_id' | head -1)" --cwd "$PROJECT_DIR" --label sq-flat-legacy-tab --no-focus) \
  || fail "could not seed a flat XO child tab"
FLAT_TAB_ID=$(printf '%s' "$FLAT_TAB_OUT" | jq -r '.result.tab.tab_id // empty')
mkdir -p "$HOME_DIR/data/post-legacy"
printf 'Post-legacy primary child.\necho done >> %s.status\n' "$HOME_DIR/data/post-legacy" > "$HOME_DIR/data/post-legacy/brief.md"
...
[ "$(lab workspace get "$LEGACY_WSID" | jq -r '.result.workspace.label')" = "Squad/legacy-seed · p:AbCdEfGhIjKlMnOpQrStUv" ] \
  || fail "correction renamed or moved the seeded legacy projection"
lab tab get "$FLAT_TAB_ID" >/dev/null 2>&1 \
  || fail "correction removed the seeded flat XO child tab"
pass "real Herdr lab: legacy projection labels and flat XO tabs are left unmigrated"
...
assert_focus_is "$COMMANDER_FOCUS" "multi-home teardown"
pass "real Herdr lab: multi-home exact-pane teardowns restore commander focus without workspace close authority"
...
RENAMED_OUT=$(lab workspace create --cwd "$PROJECT_DIR" --label "Squad/renamed1 · p:$RENAMED_TOKEN" --no-focus)
...
DUP1=$(lab workspace create --cwd "$PROJECT_DIR" --label "Squad/duplicate1 · p:$DUP_TOKEN" --no-focus)
...
lab pane report-agent "$DUP1_PANE" --source sq-projection-e2e --agent test-agent --state idle >/dev/null \
