#!/usr/bin/env bash
# Behavioral coverage for the internal /debrief cascade enumerator: per-home budget
# accounting that never sums a unit total, one stanza per registered home,
...
CASCADE="$ROOT/bin/sq-debrief-cascade.sh"
TMP_ROOT=$(fm_test_tmproot sq-debrief-cascade)
mkdir -p "$TMP_ROOT"
TMP_ROOT=$(cd "$TMP_ROOT" && pwd -P)
FAKEBIN=$(fm_fakebin "$TMP_ROOT/fakebin")
BASE_PATH=${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}

# A fake ssh standing in for the whole remote transport. It decodes sq-on.sh's
...
case "$*" in
  ...
  *list-panes*) printf '%s\n' "${SQUAD_FAKE_TMUX_PANE:-}" ;;
  *display-message*'#{pane_current_command}'*) printf '%s\n' "${SQUAD_FAKE_TMUX_COMMAND:-claude}" ;;
...
# stanza <output> <id>: the block of key=value lines for one XO.
stanza() {
  printf '%s\n' "$1" | awk -v id="XO=$2" '
    $0 == id { inside = 1 }
    inside && $0 == "" { inside = 0 }
...
test_budget_is_enforced_per_home_and_never_summed() {
  ...
  printf '%s\n' 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' > "$a/data/commander.md"
...
test_every_registered_home_is_enumerated_exactly_once() {
  ...
  fm_write_XO_meta "$primary/state/enum-a.meta" "$a"
  fm_write_XO_meta "$primary/state/enum-a-stale.meta" "$a"
  ...
  assert_contains "$out" 'duplicate XO home assignment' \
...
test_transport_routes_by_placement_and_liveness() {
  ...
  set +e
  out=$(run_cascade "$primary" \
    SQUAD_FAKE_TMUX_WINDOW='sq-live-local' \
    SQUAD_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    SQUAD_FAKE_REMOTE_AGENT_STATE=alive)
  set -e
  ...
  out=$(run_cascade "$primary" \
    SQUAD_FAKE_TMUX_WINDOW='sq-live-local' \
    SQUAD_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt" \
    SQUAD_FAKE_REMOTE_AGENT_STATE=dead)
  set -e
  s=$(stanza "$out" remote-live)
...
test_receipt_facts_are_complete_and_show_before_and_after() {
  ...
  printf '%s\n' 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa' > "$home/data/commander.md"
  printf '%s\n' 'bbbbbb' > "$home/data/learnings.md"
  local_record receipt-home "$home" > "$primary/data/XOs.md"
...
test_a_slow_remote_is_bounded_and_the_rest_still_report() {
  ...
  fm_write_XO_meta "$primary/state/bounded-remote.meta" "$remote" 'sq-remote:sq-bounded-remote'
  printf 'role=XO\n' > "$TMP_ROOT/remote-budget.txt"

  started=$(date +%s)
  set +e
  out=$(run_cascade "$primary" \
    SQUAD_STOW_CASCADE_TIMEOUT=2 \
    SQUAD_FAKE_SSH_MODE=hang \
    SQUAD_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt")
  rc=$?
  set -e
  ...
  set +e
  out=$(run_cascade "$primary" \
    SQUAD_FAKE_SSH_MODE=unreachable \
    SQUAD_FAKE_REMOTE_BUDGET="$TMP_ROOT/remote-budget.txt")
  rc=$?
  set -e
...
test_no_cascade_without_XOs_or_from_a_XO_home() {
  local primary XO out rc
  primary=$(new_primary quiet)
  set +e
  out=$(run_cascade "$primary")
  rc=$?
  set -e
  expect_code 0 "$rc" "a home with no registered XOs should be a clean no-op"
  assert_contains "$out" 'XOs=0' "an empty cascade did not say so"
  assert_not_contains "$out" 'XO=' "an empty cascade invented a home"

  XO=$(new_home quiet-XO)
  local_record quiet-XO "$XO" > "$XO/data/XOs.md"
  set +e
  out=$(run_cascade "$XO")
  rc=$?
  set -e
