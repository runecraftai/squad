...
func TestUpdaterRunReplacesExecutable(t *testing.T) {
	...
	execPath := filepath.Join(t.TempDir(), "drill")
	...
	if !strings.Contains(stdout.String(), "updated drill") {
...
func TestUpdaterRunFailsWhenDaemonUsesDifferentExecutable(t *testing.T) {
	...
	execDir := t.TempDir()
	execPath := filepath.Join(execDir, "drill")
	if err := os.WriteFile(execPath, []byte("old-binary"), 0o755); err != nil {
		t.Fatal(err)
	}
	otherExecPath := filepath.Join(execDir, "other-drill")
...
func TestUpdaterMaybeNotifyAndCheck(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "A new version of drill is available: v1.2.2 -> v1.2.3") {
		t.Fatalf("stderr = %q", stderr.String())
	}
	if !spawned {
