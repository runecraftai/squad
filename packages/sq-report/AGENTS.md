...
pnpm run build:skill    # Regenerate skills/sq-report/SKILL.md from shared CLI guidance
...
Run the opt-in real-browser suites: `SQ_REPORT_BROWSER_E2E=1 node --test test/layout-audit-browser.test.js test/layout-warning-inbox.browser.test.js` (requires `sq-browser`).
...
The committed `skills/sq-report/SKILL.md` is generated by `pnpm run build:skill`; `pnpm run check` fails if it drifts from the shared no-args home output.
...
- Runtime guidance strings (`src/cli.js`, `src/design-reference.js`, `src/playbooks.js`) own everything agents are told while using sq-report; the generated skill mirrors them via `src/skill.js`.
...
sq-report is a CLI + local HTTP server that opens agent-generated HTML artifacts in a browser, lets the user annotate elements and selected text ranges or edit Mermaid diagrams as whiteboards, and ships that feedback back to the agent over a long-polling API.

### Process model

The CLI (`bin/sq-report.js` -> `src/cli.js`) spawns `sq-report server` as a **detached** background process (`src/cli.js:startServer`) and waits for `/health`, which returns `{ ok, app, version }`.
...
A Host-header allowlist middleware (`buildAllowedHostnames`/`isAllowedRequestHost`) rejects any request whose `Host` is missing or not one this server answers to - the DNS-rebinding defense, since `isSameOriginRequest` alone does not stop rebinding (a rebound page sends its hostile domain in _both_ `Origin` and `Host`, so they still match). The allowlist is loopback names + the resolved bind/link host + explicit `SQ_REPORT_ALLOWED_HOSTS` extras (`src/paths.js:extraAllowedHosts`), minus wildcard binds; a lone `*` (`allowsAllHosts`) disables the guard for operators fronting it with their own auth. When a reverse proxy sits in front, `X-Forwarded-Host`'s outermost value is validated against the same allowlist (an AND check, so a spoofed forwarded host only narrows access, never bypasses `Host`). README's Allowed hosts bullet owns the user-facing contract. So a specific-interface bind stays rebinding-protected while its own hostname works, rather than the guard switching off outside loopback.
README's Network binding bullet owns the user-facing env vars and the non-loopback security warning.
Server self-shutdown keys off live connections (browser SSE and agent polls), not session status, so the next `sq-report <file>` re-spawns a fresh server and adopts the session from `state.json` when it is still resumable; README's Server cleanup bullet owns the user-facing idle-timeout rules.
State lives at `~/.sq-report/state.json` (`SQ_REPORT_STATE_DIR`), shared across all projects and keyed by a sha256 prefix of the canonicalized file path - the canonical HTML path _is_ the identity, so the CLI never needs opaque session IDs (`src/session-store.js:sessionKey`).

### Request flow

1. `sq-report <file.html>` (`openCommand`) -> POST `/api/sessions` -> `SessionStore.upsertSession` -> the CLI opens the returned session URL in the browser.
   `--no-gate` sends `noGate`, returns a one-open URL with `?no-gate=1`, and leaves the stored canonical session URL unchanged.
   Sessions record who ended them: `ended_by: "user"` for browser-initiated ends (including the composer's send-and-end, which posts final prompts and the `endSession` flag to `/api/:key/prompts` atomically) and `ended_by: "agent"` for `sq-report end <file>`.
...
5. The injected SDK runs the render-time layout audit and posts each pass - findings plus its own `complete` flag and viewport width - to the chrome, which POSTs it to `/api/:key/layout-diagnostics`.
   Detection is **passive**: that route updates the warning inbox and emits an SSE `layout-warnings` event, and it deliberately never emits `feedback`, so a diagnostic pass can never make `sq-report poll` return or wake an agent. The gate reveals on the first completed pass whatever it found (detection and user-facing behavior: README's Layout issues inbox, Layout issue lifecycle, and Open-time layout gate bullets).
...
7. `sq-report poll <file.html>` (`pollCommand`) hits `GET /api/poll`.
...
The opt-in attribute/meta (user-facing contract: README's Live reload bullet) switches the watcher to the artifact's directory, excluding `.git`, `node_modules`, `dist`, `build`, `.sq-report`.
...
Run `sq-report server --verbose` (or set `SQ_REPORT_DEBUG=1`) to log session and watcher events to stderr when diagnosing wedges; detached server stdout/stderr is also appended to `server.log` in `SQ_REPORT_STATE_DIR` for startup and crash diagnostics.
...
The chrome's **Export standalone HTML** overflow-menu item `GET`s `/api/:key/export`; the CLI exposes the same transform as `sq-report export`, server-independently.
sq-report itself sets **no** `Content-Security-Policy` on any response (the sandboxed iframe relies on the `sandbox` attribute, not CSP), but author-set CSP meta tags are preserved and reported as export notices because they may still block exported inline assets.

### Hosted sharing (ht-ml.app)

`src/html-app.js` (`publishToHtmlApp`) publishes the local-inlined HTML to ht-ml.app with `POST {SQ_REPORT_HTML_APP_API_URL or https://api.ht-ml.app}/v1/sites` as `{ html_content, password? }`.
Creating a site needs no account or API key; the response's secret `update_key` is returned once and is the only credential, and the optional bearer token (`SQ_REPORT_HTML_APP_TOKEN` / `--token`) is sent when set but never required.
...
The `home()` callback returns the rich object shown when the user runs `sq-report` with no arguments - the same TOON-serialized output that lands in the agent's optional `SessionStart` hook after `sq-report setup hooks`; top-level `--help` returns the same static guidance without dynamic sessions.
...
The generated skill intentionally omits a `version` frontmatter field because release-please updates `package.json` without regenerating `skills/sq-report/SKILL.md`.
`skills/sq-report/SKILL.md` is the only public Agent Skill shipped through npm.
...
The published package root **is** the Agent Plugin: `plugin.json` sits beside the `skills/` directory the standard discovers, so an installed copy is a conformant plugin with nothing downloaded and no marketplace involved (README's Agent Plugin bullet owns the user-facing contract). Keeping that true is the invariant - both `plugin.json` and `skills/sq-report` must stay in `package.json` `files`, and `resolvePluginRoot` resolves `../` from `src/plugin.js`, which is the package root when bundled and the repo root from source.
...
- The `sq-report new` starter templates are split between `src/templates.js` (kinds, token kits, per-kit class maps, and the `buildTemplate` renderer) and `src/components.js` (the text-fill catalog: one builder per catalog block, emitting markup only through `kit.cls()` lookups so authors fill prose, never classes).
  ...
  The offline kit must never receive a class its CSS does not define: every emitted class routes through the kit class maps into `BRAND_CSS` `.sq-*` rules, and every non-slides template uses its kind-specific title as the visible `h1`, with `Mission Briefing` as the masthead brand, while `Rendered with sq-report` appears once, in the colophon. Both invariants are enforced by `test/templates.test.js` (no-dead-classes check and h1/colophon placement); never reintroduce literal Tailwind utilities into the sq-report kit's markup.
...
- The SDK injected into artifacts lives in `src/artifact-sdk.js` and is wrapped by `createSdkJs`.
  ...
  The `sq-report design` Mermaid snippet chooses dark or light rendering from the effective artifact page background and re-renders when a page-theme or OS appearance change alters that appearance, so do not hardcode one Mermaid theme.
...
- Tests use `SQ_REPORT_STATE_DIR` and ephemeral ports to stay isolated. When adding tests that spin up the server, do the same.
