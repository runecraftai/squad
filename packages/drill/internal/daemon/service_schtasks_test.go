...
func TestStartInstallsWindowsTaskAndStartsManagedDaemon(t *testing.T) {
	...
	defer cleanup()
	runtimeGOOS = "windows"
	exe := `C:\Program Files\drill\drill.exe`
...
func TestInstallWindowsTaskDoesNotRemoveLegacyTaskForDifferentRoot(t *testing.T) {
	...
	if err := installWindowsTask(p, `C:\Program Files\drill\drill.exe`); err != nil {
		t.Fatal(err)
	}
	if len(commands) != 2 {
...
func TestInstallWindowsTaskKeepsLegacyTaskOnCreateFailure(t *testing.T) {
	...
	err := installWindowsTask(p, `C:\Program Files\drill\drill.exe`)
	if err == nil {
