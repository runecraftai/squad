...
if [ "${SQUAD_GROK_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_GROK_LIVE_E2E=1 to run the interactive Grok continuity regression"
  exit 0
fi
...
PROMPT='Use run_terminal_command with background=true to run exactly `bin/sq-sentry-arm.sh`. Never use a shell ampersand. Once it reports started, respond briefly.'
...
initial_sentry=
while [ "$i" -lt 240 ]; do
  initial_sentry=$(cat "$HOME_DIR/state/.sentry.lock/pid" 2>/dev/null || true)
  [ -n "$initial_sentry" ] && kill -0 "$initial_sentry" 2>/dev/null && break
  sleep 0.5
  i=$((i + 1))
done
if [ -z "$initial_sentry" ] || ! kill -0 "$initial_sentry" 2>/dev/null; then
  fail "Grok did not start the tracked background sentry"
fi

printf 'done: grok live e2e sentry fire\n' > "$HOME_DIR/state/grok-e2e.status"
i=0
while [ "$i" -lt 240 ]; do
