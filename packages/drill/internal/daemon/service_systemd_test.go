...
func TestStartInstallsSystemdUnitAndStartsManagedDaemon(t *testing.T) {
	...
	for _, want := range []string{
		"Description=drill background daemon",
		"ExecStart=/usr/local/bin/drill daemon run --root " + p.Root(),
...
func TestInstallSystemdUserServiceDoesNotRemoveLegacyUnitForDifferentRoot(t *testing.T) {
	...
	legacyUnit := renderSystemdUnit("/usr/local/bin/drill", paths.WithRoot(otherRoot), home)
	...
	if err := installSystemdUserService(p, "/usr/local/bin/drill"); err != nil {
		t.Fatal(err)
	}
...
func TestInstallSystemdUserServiceKeepsLegacyUnitOnEnableFailure(t *testing.T) {
	...
	err := installSystemdUserService(p, "/usr/local/bin/drill")
	if err == nil {
...
func TestRenderSystemdUnitForwardsProxyEnv(t *testing.T) {
	...
	unit := renderSystemdUnit("/usr/local/bin/drill", paths.WithRoot(t.TempDir()), "/home/u")
	for _, want := range []string{
...
func TestRenderSystemdUnitForwardsEveryProxyEnvKey(t *testing.T) {
	...
	unit := renderSystemdUnitWithProxyEnv("/usr/local/bin/drill", paths.WithRoot(t.TempDir()), "/home/u", proxyEnv)
	for _, key := range proxyEnvKeys {
