...
#
# The failure this suite pins: Squad promises a public final reply in an X or
...
PF="$ROOT/bin/sq-public-followup.sh"
EMIT="$ROOT/bin/sq-public-followup-emit.sh"
POLL="$ROOT/bin/sq-x-poll.sh"
...
# make_home <name> [relay-on|relay-off]: a Squad home with its own backlog.
# relay-off omits .env entirely, which is exactly what a home that never opted
# into the mySquad relay looks like.
...
EOF
  ...
  fm_fake_exit0 "$home/fakebin" tmux fob no-mistakes gh gh-axi
  printf '%s\n' "$home"
}
...
seed_commitment() {
  ...
  jq -n '{type:"pr-merged", project:"Squad",
...
# anywhere: a Discord request becomes a typed commitment, an XO child
...
test_restart_e2e_delivers_exactly_once() {
  ...
  seed_commitment "$home" pf-restart req-restart discord XO:fmdev work-code-q1
  printf '%s\n' fmdev > "$child/.sq-xo-home"
  fm_write_meta "$home/state/fmdev.meta" "kind=xo" "home=$child"
  ...
  emit_terminal "$home" "$home" pf-restart XO:fmdev work-code-q1 >/dev/null \
  ...
  assert_no_grep '^x_request=' "$child/state/work-code-q1.meta" \
    "typed delivery must clear the XO's legacy X link"
...
test_invalid_events_are_refused_and_quarantined() {
  ...
  seed_commitment "$home" pf-refuse req-refuse discord XO:fmdev work-real
  ...
  expect_failure "a wrong source home must be refused" \
    ...
    --source-home XO:other --work-id work-real --generation 1 \
  ...
  expect_failure "a wrong work id must be refused" \
    "$EMIT" --home "$home" --obligation pf-refuse --relation rel-code \
    --source-home XO:fmdev --work-id work-other --generation 1 \
    --outcome pr-merged --deliverable pr_url=https://example.invalid/1 \
    --outcome-text 'x'
  expect_failure "a stale generation must be refused" \
    "$EMIT" --home "$home" --obligation pf-refuse --relation rel-code \
    --source-home XO:fmdev --work-id work-real --generation 0 \
    --outcome pr-merged --deliverable pr_url=https://example.invalid/1 \
    --outcome-text 'x'
  ...
  jq -n '{schema_version:1, event_id:"forged", obligation_id:"pf-refuse",
          ...
          source_home_id:"XO:fmdev", outcome_type:"pr-merged",
...
test_dry_run_does_not_close_commitment() {
  ...
  SQX_DRY_RUN=1 FAKE_CURL_LOG="$log" expect_failure \
...
test_outward_delivery_stays_with_the_owning_home() {
  ...
  seed_commitment "$owner" pf-own req-own discord XO:child work-child
  printf '%s\n' child > "$child/.sq-xo-home"
  fm_write_meta "$owner/state/child.meta" "kind=xo" "home=$child"
  fm_write_meta "$child/state/work-child.meta" \
    "x_request=req-own" "x_request_ts=1700000000" "x_followups=1"

  FAKE_CURL_LOG="$log" emit_terminal "$owner" "$owner" pf-own XO:child work-child >/dev/null \
  ...
  PATH="$child/fakebin:$PATH" SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_HOME="$child" \
    ...
    "$PF" deliver pf-own
  assert_contains "$EXPECT_OUT" "has not opted into the mySquad relay" \
...
test_XO_teardown_requires_parent_binding() {
  ...
  seed_commitment "$parent" pf-teardown req-teardown x XO:mate work-child
  ...
  PATH="$child/fakebin:$PATH" SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_HOME="$child" \
    ...
    "$TEARDOWN" work-child
  assert_contains "$EXPECT_OUT" "cannot resolve the primary home for marked XO mate" \
  ...
  seed_commitment "$parent" pf-teardown-valid req-teardown-valid x XO:mate work-child
  fm_write_meta "$parent/state/mate.meta" "kind=xo" "home=$child"
  fm_write_meta "$child/state/work-child.meta" \
    "window=Squad:sq-work-child" "endpoint_task_id=work-child" \
    "worktree=$child" "project=$child" "kind=strike" "mode=local-only"
  ...
  pass "marked XO teardown resolves its parent and fails closed when unavailable"
}

# The three tests below exercise the durable .sq-xo-parent record a real
# bin/sq-home-seed.sh seed now writes next to .sq-xo-home (sq-remote-sm-
...
# SQUAD_PUBLIC_FOLLOWUP_PRIMARY_HOME env var: a restart that dropped that prefix made
...
# the real bin/sq-teardown.sh cleanup path against a home real sq-home-seed.sh
# produced, never a hand-crafted marker.

assert_local_XO_parent_record() {
  local child=$1 parent=$2
  cmp -s "$child/.sq-xo-parent" <(
    printf 'schema=sq-xo-parent.v1\nroute=local\nparent_home=%s\n' "$parent"
  ) || fail "real XO seeding must write the exact durable local parent record"
}

test_local_XO_seed_publishes_parent_before_identity() {
...
case "$destination" in
  */.sq-xo-home)
    ...
    wait_count=0
    while [ ! -f "$SQUAD_TEST_PUBLISH_RELEASE" ]; do
...
SH
  ...
  PATH="$fakebin:$PATH" SQUAD_HOME="$parent" \
    ...
    "$ROOT/bin/sq-home-seed.sh" mate "$child" --no-projects > "$manifest_out" 2>&1 &
  seed_pid=$!
  wait_count=0
  while [ ! -f "$entered" ]; do
  ...
  assert_absent "$child/.sq-xo-home" \
  ...
  assert_present "$child/.sq-xo-home" \
...
test_XO_teardown_resolves_parent_from_durable_record_when_env_lost() {
  ...
  fm_fake_exit0 "$child/fakebin" tmux fob no-mistakes gh gh-axi

  assert_local_XO_parent_record "$child" "$parent_resolved"

  seed_commitment "$parent" pf-durable req-durable x XO:mate work-child
  fm_write_meta "$parent/state/mate.meta" "kind=xo" "home=$child"
  fm_write_meta "$child/state/work-child.meta" \
    "window=Squad:sq-work-child" "endpoint_task_id=work-child" \
    "worktree=$child" "project=$child" "kind=strike" "mode=local-only"
...
test_XO_teardown_durable_record_missing_parent_registration_still_refuses() {
  local parent child parent_resolved
  ...
  fm_write_meta "$child/state/work-child.meta" \
    "window=Squad:sq-work-child" "endpoint_task_id=work-child" \
    "worktree=$child" "project=$child" "kind=strike" "mode=local-only"
  # No parent/state/mate.meta at all: the parent never recorded this XO's
...
test_XO_teardown_durable_record_with_unknown_field_succeeds() {
  ...
  fm_write_meta "$child/state/work-clean.meta" \
    "window=Squad:sq-work-clean" "endpoint_task_id=work-clean" \
...
test_XO_teardown_rejects_conflicting_live_and_durable_parent_bindings() {
  ...
  fm_write_meta "$child/state/work-conflict.meta" \
    "window=Squad:sq-work-conflict" "endpoint_task_id=work-conflict" \
    "worktree=$child/projects/worktree" "project=$child/projects/worktree" \
    "kind=strike" "mode=local-only"
...
test_XO_teardown_rejects_unsafe_durable_parent_records() {
  ...
  for case_name in symlink invalid-route duplicate-route remote-parent-home local-parent-host; do
    ...
    fm_write_meta "$child/state/work-child.meta" \
      "window=Squad:sq-work-child" "endpoint_task_id=work-child" \
      "worktree=$child" "project=$child" "kind=strike" "mode=local-only"
    parent_record="$child/.sq-xo-parent"
    case "$case_name" in
      symlink)
        mv "$parent_record" "$parent_record.valid"
        ln -s .sq-xo-parent.valid "$parent_record"
        ;;
      invalid-route)
        printf 'schema=sq-xo-parent.v1\nroute=garbage\n' > "$parent_record"
        ;;
...
test_XO_teardown_rejects_nul_bearing_durable_parent_record() {
...
SH
  ...
  fm_write_meta "$home/state/work-disabled.meta" \
    "window=Squad:sq-work-disabled" "endpoint_task_id=work-disabled" \
...
test_relay_disabled_parent_allows_marked_child_teardown() {
  ...
  fm_git_init_commit "$child/projects/worktree"
  printf '%s\n' disabled-mate > "$child/.sq-xo-home"
...
test_XO_parent_binding_matches_literal_id() {
  ...
  printf '%s\n' 'mate.id' > "$child/.sq-xo-home"
  printf -- '- mateXid - synthetic (home: %s; scope: synthetic; projects: ; added 2026-07-30)\n' \
    "$child" > "$parent/data/XOs.md"
  seed_commitment "$parent" pf-teardown-literal req-teardown-literal x XO:mate.id work-literal
  fm_write_meta "$parent/state/mate.id.meta" "kind=xo" "home=$child"
  ...
  assert_contains "$EXPECT_OUT" "cannot resolve the primary home for marked XO mate.id" \
  ...
  pass "XO parent resolution matches the durable registry id literally"
}
...
test_traversal_registration_is_refused_before_delivery() {
  ...
  sed -i.bak 's/^work_home=.*/work_home=XO:..\/..\/x/' \
...
# The relay's own refusal of an exhausted follow-up binding is a commander
...
test_exhausted_binding_is_not_retried() {
  ...
  assert_contains "$EXPECT_OUT" "commander decision" "an exhausted binding must be escalated, not retried"
