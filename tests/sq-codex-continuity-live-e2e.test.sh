...
if [ "${SQUAD_CODEX_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_CODEX_LIVE_E2E=1 to run the Codex continuity regression"
  exit 0
fi
...
PROMPT='Run exactly `bin/sq-sentry-checkpoint.sh --seconds 1` as one foreground shell call. Do not use a background task and do not run sq-sentry-arm.sh. After the checkpoint returns, reply briefly.'
...
if grep -F 'sentry: started pid=' "$TRANSCRIPT" >/dev/null; then
