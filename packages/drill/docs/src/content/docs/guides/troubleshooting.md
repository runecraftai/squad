...
First stop for anything: `drill doctor`.

## Debug in this order

```mermaid
flowchart TD
  problem["Something is wrong"] --> doctor["Run drill doctor"]
...
Symptoms: `drill daemon status` shows stopped, or `drill` exits with "daemon not running."
...
```sh
ls -la ~/.drill/daemon.pid ~/.drill/socket
```

If the PID file points at a process that's no longer running, remove it and run `drill daemon start` again.

### "a drill daemon is already running for this DRILL_HOME"

This error always means a genuinely live daemon: the lock it reports cannot go stale (see [Daemon & Worktrees](/drill/concepts/daemon/) for the singleton-lock model).
Manage that daemon with `drill daemon status` and `drill daemon stop` instead of deleting the lock file - deleting the file does not release the lock and only weakens the guard.

If the socket exists and the process is running but stuck or unresponsive, `drill` bounds the connection wait with [`daemon_connect_timeout`](/drill/reference/global-config/#daemon_connect_timeout) and fails fast with an error naming the socket path instead of silently starting a second daemon. Restart the stuck daemon:
...
If the socket file exists but nothing answers at all (a dead socket left behind by an unclean exit, e.g. a crash or `SIGKILL`), commands that ensure the daemon is running (`drill`, `init`, `attach`, `rerun`, `axi run`, `axi respond`) now fail fast with a `connect to daemon socket` error instead of silently starting a replacement daemon. The error message itself includes a `(run 'drill daemon start' to recover)` hint - run `drill daemon start` directly to recover, since it self-heals past a dead socket and starts a fresh daemon.

### Managed service logs

- **macOS (launchd):** `launchctl list | grep drill` and check `~/Library/LaunchAgents/com.squad.drill.daemon.*.plist`
- **Linux (systemd):** `systemctl --user status drill-daemon-*` and `journalctl --user -u drill-daemon-* -f`
- **Windows (Task Scheduler):** `schtasks /query /tn "drill-daemon-*"`

### `DRILL_HOME` collisions

If you have multiple installs with different `DRILL_HOME` roots, each gets its own scoped service name (with a short suffix derived from the path). Make sure you're looking at the right one - `drill daemon status` reports which.

## `drill update` refuses or aborts
...
`update`, `daemon stop`, and `daemon restart` all refuse by default while pipeline runs are active and list the affected runs; [Daemon & Worktrees](/drill/concepts/daemon/#starting-and-stopping) owns the guard's exact rules, including why `-y`/`--yes` does not bypass it.

First inspect each listed run with `drill axi status --run <id>`.
A parked CI gate can clear itself after its PR becomes terminal, including after a daemon restart.
The [`ci_timeout` reference](/drill/reference/global-config/#ci_timeout) owns the exact fail-closed reconciliation rules, and [Daemon & Worktrees](/drill/concepts/daemon/#crash-recovery) owns restart behavior.
...
`drill` will not silently skip review, test evidence, documentation, or agent-assisted lint and report the remaining work as a passed gate.

### Check PATH

The daemon uses the same binary-discovery order described in [Choosing an Agent](/drill/guides/agents/). When it's running through a managed service, it reloads `PATH` from your login shell on macOS and Linux and appends common install locations such as `~/.local/bin`, `~/go/bin`, `~/.cargo/bin`, `~/bin`, `/opt/homebrew/bin`, `/usr/local/bin`, `/usr/bin`, and `/bin`.

If a native agent is installed in a version-manager shim directory or another nonstandard location, set an explicit override in `~/.drill/config.yaml`:
...
The daemon logs its effective `PATH` at startup in `~/.drill/logs/daemon.log` with the message `daemon environment ready`. If the log contains `login shell environment resolution failed` or `login shell environment resolution returned no entries`, the daemon used a degraded fallback `PATH` that may omit version-manager directories such as nvm, fnm, or volta, so tools like `pnpm` may be missing.
...
A managed daemon started by launchd or systemd inherits only a minimal environment, so it does not see the `HTTP_PROXY` / `HTTPS_PROXY` / `NO_PROXY` / `ALL_PROXY` variables from your shell. `drill` bakes any proxy variables that are set when you install or refresh the service into the generated service definition. If you set up the proxy after installing, re-run the installer or `drill daemon restart` (with the proxy variables exported) so they get baked in, then confirm them in `~/.config/systemd/user/drill-daemon-*.service` on Linux or `~/Library/LaunchAgents/com.squad.drill.daemon.*.plist` on macOS. Once baked in, the values survive later restarts and binary upgrades even from a shell that does not export them, so you only need the variables exported the first time. Windows Task Scheduler inherits your logon environment and needs no forwarding.
...
Requested test evidence may still be written under the managed temporary `drill-evidence` directory. On GitHub, it is published to the push-target repository's orphan evidence branch when `test.evidence.store_in_repo` is enabled; the [Global Config Reference](/drill/reference/global-config/#testevidence) lists the cases that leave it local instead.
...
Check the per-step log at `~/.drill/logs/<runID>/<step>.log`.
...
`drill` refuses the push instead of overwriting that remote work.

Fetch and inspect the configured push target, then rebase or merge the remote work into your branch before pushing through `drill` again.
...
`drill` pauses with an `ask-user` finding instead of silently bundling that unrelated local work into the PR.
...
## `git push drill` doesn't start a pipeline

Symptom: push succeeds but `drill` shows no active run.

### Check the remote

```sh
git remote -v | grep drill
```

If it's missing, run `drill init` again.
Re-running init refreshes an existing gate and repairs the `drill` remote when it is missing.
...
Both hooks should be executable. If either is missing or non-executable, `drill init` will reinstall it for an existing drill-managed gate.
For validated registered gates and strictly named legacy gates, `drill daemon restart` also installs missing drill-managed hooks and refreshes legacy managed hooks. An existing custom pre-receive hook is preserved behind the managed admission wrapper.
...
If `notify-push.log` mentions `invalid gate path: .`, refresh the managed hook with `drill init` or `drill daemon restart`, then push again.
...
Both receive hooks talk to the daemon over `~/.drill/socket`. If the daemon is not running, pre-receive admission fails closed and the push is rejected before any gate ref changes. Start the daemon and push again.

If the gate is older, re-running `drill init` or restarting the daemon also reapplies hook-path isolation when Git supports `config --worktree`.
That protects the gate hook if a tool such as Husky wrote `core.hookspath` into shared git config from inside a linked worktree. [Crash recovery](/drill/concepts/daemon/#crash-recovery) owns the gate validation and migration rules used during restart.

## PR step is skipped

Symptom: pipeline completes but the PR step shows `skipped`.

Check the [Provider Integration](/drill/guides/provider-integration/) requirements. Most common causes:
...
Once the CI monitor reports readiness and the PR is mergeable, the CI panel shows `✓ Checks passed` and the terminal title switches to `Checks passed`, so you can tell when to go merge the PR; the signal clears automatically if checks start re-running or a new failure appears. A trusted [`no_ci: true` declaration](/drill/reference/repo-config/#no_ci) can establish readiness for a zero-check repository; an empty forge response without that declaration is not ready.

How long the monitor runs is controlled by `ci_timeout` in `~/.drill/config.yaml`, an idle timeout that re-arms whenever the upstream default branch advances; the [`ci_timeout` field reference](/drill/reference/global-config/#ci_timeout) owns the default, the `unlimited` keyword and its aliases, and the exact re-arm semantics.
...
You can approve, fix, or skip from the TUI or `drill axi respond`.
Use `drill axi abort` only when you mean to cancel the whole active run.
...
If the step is parked at a gate, use `drill axi respond` instead of waiting.
If the run is genuinely stuck and you want to discard it, use `drill axi abort`.
Start a new run only after abort confirms the terminal state; see the [abort command contract](/drill/reference/cli/#drill-axi-abort).

## Worktree won't clean up

Symptom: `~/.drill/worktrees/<repoID>/<runID>/` sticks around after a run ends.
...
```sh
drill daemon stop --force
rm -rf ~/.drill/worktrees ~/.drill/servers ~/.drill/socket ~/.drill/daemon.pid ~/.drill/daemon.lock
drill daemon start
```

This keeps your gate repos, database, and config but clears transient state. For a full wipe, see the [Uninstall section](/drill/start-here/installation/#uninstall).
...
- Check `~/.drill/logs/daemon.log` at `log_level: debug`
