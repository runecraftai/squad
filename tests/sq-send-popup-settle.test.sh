#!/usr/bin/env bash
# sq-send pre-submit popup-settle selection (the codex `$<skill>` fix).
#
...
# for a leading `$<skill>` invocation (e.g. `$drill`). Submitting before the
# popup settles lets it swallow the Enter, so the line never submits. sq-send
...
# sq-send's own post-submit SQUAD_SEND_SETTLE pause. So tail-vs-head matters: this
# suite asserts on the HEAD sleep, distinct from sq-send-settle.test.sh which pins
...
TMP_ROOT=$(fm_test_tmproot sq-send-popup-settle)

# Same stub shape as sq-send-settle.test.sh: a fake tmux that drives the submit
# path to a clean "empty" verdict on the first Enter, and a fake sleep that records
# every requested duration (one per line) into SQUAD_SLEEP_LOG instead of sleeping.
...
# (the popup-settle) equals <expected>. SQUAD_SEND_SETTLE=0 strips the trailing
# post-submit pause so the log holds only the popup-settle plus the 0.4 Enter wait,
# keeping the head assertion crisp. SQUAD_ROOT_OVERRIDE points at a non-repo dir so
# sq-guard's tangle check stays silent; its sentry-liveness note goes to stderr
# (discarded).
first_settle() {  # <expected> <label> <harness|--explicit> <message> [selector-form]
  ...
  pass "sq-send popup-settle: $label -> ${expected}s"
}

# Codex `$<skill>` gets the long settle so its `$` popup clears (the fix).
first_settle 1.2 'codex $skill -> long settle' codex '$drill'

# The same Codex `$<skill>` path must work when the target is addressed by exact
# task id, not only by the legacy `sq-<id>` window label.
first_settle 1.2 'codex $skill exact task id -> long settle' codex '$drill' exact

# Same `$` message to claude keeps the fast path: `$` is ordinary text there.
first_settle 0.3 'claude $-message -> fast path' claude '$drill'
...
first_settle 0.3 'explicit target $message -> fast path (unknown harness)' --explicit '$drill'

# The `/` slash case stays universal and unchanged: long settle regardless of
# harness (here a non-codex claude target).
first_settle 1.2 'claude /command -> long settle (slash unchanged)' claude '/drill'
