...
`drill` is pipeline-agent-agnostic by design: the gate should mean the same thing regardless of which supported agent backend you prefer.
...
The coding agent that calls `drill axi` drives approval gates, but it does not automatically become the pipeline agent that performs review, evidence testing, documentation, combined documentation-and-lint housekeeping, or fixes.
...
If that step attempts pipeline control, drill returns `error.code: nested_gate_context`; the agent must return control to the outer executor, while read-only `drill axi status`, `drill axi logs`, help, and `drill doctor` remain available.
...
The only intentional out-of-worktree write it allows is test evidence under the managed temporary `drill-evidence` directory when a testing prompt asks for it.
...
- Use an ordered fallback list when you prefer one agent but want drill to try another if the first process is unavailable.
...
1. Install any supported native agent CLI and leave `agent: auto`, or select it explicitly in `~/.drill/config.yaml`.
...
Run `drill doctor` afterward and look for a successful `gate validation` line.
...
```yaml
# ~/.drill/config.yaml or .drill.yaml
agent: [codex, claude]
```
...
The [`agent` field reference](/drill/reference/global-config/#agent) owns the exact resolution order, fallback-list filtering and retry semantics, and the failure behavior when no entry is runnable.
...
## Driving drill as an agent

The primary way to put a change through the gate from inside a coding agent is the `/drill` skill.
A skill-aware tool like Claude Code supports two invocation modes.
Use bare `/drill` to validate existing committed work.
Use `/drill <task>` to have the agent first do the task, commit only that task's changes on a feature branch, then run the pipeline with the task text as `--intent`.
In both modes, it resolves low-risk findings on its own and stops to relay anything that needs your decision.

`drill init` installs that skill at user level: `~/.claude/skills/drill/SKILL.md` for Claude Code and `~/.agents/skills/drill/SKILL.md` for Codex, OpenCode, Rovo Dev, and Pi.
...
Re-run `drill init` after an upgrade to refresh that skill, including overwriting stale `SKILL.md` content from an older binary.
...
The skill drives `drill axi`, a non-interactive command surface that prints TOON to stdout and progress to stderr.
When CI is ready - either its registered checks are green or the trusted default-branch config declares [`no_ci: true`](/drill/reference/repo-config/#no_ci) with no registered checks - but the PR is still open, `axi run` and `axi respond` return `outcome: checks-passed` with a help line pointing at the PR instead of waiting for a human merge. An empty check result without that declaration is not ready; see the [CI step reference](/drill/reference/pipeline-steps/#ci) for the readiness rules.
...
The one exception is when that monitor is no longer running - the PR was closed, the run was aborted or superseded, it idle-timed-out, or its auto-fix attempts were exhausted - in which case the agent recovers with `drill rerun`, which cancels the stale monitor and re-runs the full pipeline including a deterministic rebase step.
The agent must not use `drill axi run` to refresh a still-active PR: after `checks-passed` it reattaches to the running monitor with HEAD unchanged and returns the monitor output without rebasing.
...
Agents can also call `drill axi` directly:

```sh
drill axi run --intent "the user's goal"
drill axi status
drill axi sync --check
drill axi sync
drill axi sync --recover
drill axi respond --action approve
drill axi logs --step review --full
drill axi abort
drill axi abort --run <id>
```

Before any post-pipeline local commit or fresh run, read `branch_sync`.
Only when its structured `next_action.code` is `sync`, run `drill axi sync` first.
When `next_action.code` is `recover_custody` - a terminal run left unpublished pipeline commits preserved in the local gate - run `drill axi sync --recover` to return custody, or `drill rerun` to resume validating the preserved head.
...
The [CLI reference](/drill/reference/cli/) documents each `axi` command and output field for humans.
...
If native agent discovery does not resolve the binary you expect, check `~/.drill/logs/daemon.log` and set an explicit override; [Environment the daemon sees](/drill/reference/environment/#environment-the-daemon-sees) owns the full resolution story.

Five global config fields tune resolution and invocation, and the [Global Config Reference](/drill/reference/global-config/) owns each one:

- [`agent_path_override`](/drill/reference/global-config/#agent_path_override) - custom binary paths per native agent, plus the default native binary-name table.
- [`agent_args_override`](/drill/reference/global-config/#agent_args_override) - extra CLI flags per native agent for model selection, service tier, reasoning depth, or permission mode, including the reserved-flag rules and smart defaults. Keep it global-only; it reflects your local agent setup rather than repo policy.
- [`acpx_path`](/drill/reference/global-config/#acpx_path) - the bridge binary path for explicit ACP targets and first-class ACP aliases.
- [`acp_registry_overrides`](/drill/reference/global-config/#acp_registry_overrides) - raw ACP target commands, including replacements for alias defaults such as `cursor-agent acp`, plus their availability-probing rules.
- [`agent`](/drill/reference/global-config/#agent) - the `auto` resolution order and ordered fallback-list semantics.
...
- **Session** - optional drill-owned native session identity for review-fixer reuse
...
After drill starts one, it terminates any remaining child processes when the invocation exits, fails, or is cancelled, so agent-spawned test workers, build watchers, and dev servers do not survive the step.
...
When an agent starts a run through `drill axi run --intent`, drill uses that supplied intent verbatim as authoritative acceptance criteria and skips transcript-based inference, even if `intent.enabled` is false.
...
Otherwise, when `intent.enabled` is true, drill reads recent local transcripts from Claude Code, Codex, OpenCode, Rovo Dev, Pi, and the GitHub Copilot CLI during the `intent` pipeline step.
...
When deterministic matching leaves multiple plausible sessions, drill may ask the configured pipeline agent to choose among them using the matching file paths and sanitized transcript packet files.
...
Before disambiguation or summarization, drill excludes tool output, redacts likely secrets, strips common prompt-control markers, and clamps long transcripts while preserving the beginning and end.
drill stores derived intent summaries and matching metadata in `~/.drill/state.sqlite`, including the source, session ID, and match score on each run plus cached summaries for matching transcript sessions.
...
Any `agent_args_override.copilot` flags are inserted before drill's managed flags, so user choices such as `--model` or `--effort` take effect.
...
The Copilot CLI has no output-schema flag, so when structured output is requested drill injects the JSON schema into the prompt and validates the final text response with the same JSON fence and bare-object fallback used by Pi and Rovo Dev.
...
The [Global Config Reference](/drill/reference/global-config/) owns ACP availability, bridge-path, command-override, and equivalent-spelling deduplication rules.
...
When the target matches a first-class alias such as `acp:cursor`, drill supplies that alias' default raw command.
Configure custom target commands in the [Global Config Reference](/drill/reference/global-config/#acp_registry_overrides).

drill invokes acpx with JSON output, approve-all permissions, denied non-interactive permission prompts, and the repo worktree as `--cwd`.
...
Run `drill doctor` to inspect individual native and ACP runner binaries and to check the effective global agent configuration:
...
See the [Global Config Reference](/drill/reference/global-config/) for ACP availability and probing behavior.
