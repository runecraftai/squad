...
`config/` under the effective base holds local gitignored operating choices, while the tracked code root also contains the shared repository policy file `config/skill-verification.json`; `projects/` holds the local project clones that Squad reads but changes only through the narrow guarded and concrete commander-approved exceptions in `AGENTS.md`.
