...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/logstore"
...
// `drill daemon run` entrypoint used by managed and detached services.
...
func prepareDaemonEnvironment() error {
	drillHome := os.Getenv("DRILL_HOME")
	...
	if drillHome != "" {
		if err := os.Setenv("DRILL_HOME", drillHome); err != nil {
			return fmt.Errorf("restore DRILL_HOME: %w", err)
		}
	}
...
func RunWithOptions(p *paths.Paths, d *db.DB, stepFactory StepFactory) error {
	startupStarted := time.Now()
	// Singleton guard: only one live daemon may own this DRILL_HOME at a time.
...
func registerHandlers(srv *ipc.Server, mgr *RunManager, d *db.DB, shutdown func()) {
	...
	srv.Handle(ipc.MethodRerun, func(ctx context.Context, params json.RawMessage) (interface{}, error) {
		...
		runID, err := mgr.HandleRerun(ctx, p.RepoID, p.Branch, p.PreviousRunID, p.SkipSteps, p.Intent, p.SpecializedReview)
		if err != nil {
			return nil, err
		}
