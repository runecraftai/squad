---
title: CLI Commands
description: Complete reference for all drill commands and flags.
---

## drill

Attach to the active pipeline run for the current branch when one exists. If none exists, bare `drill` can start the setup wizard to create a branch, commit changes, push through the gate, wait for the daemon to register the new run, and then attach. If the push succeeds but no run is registered, that wizard path now exits with an explicit error instead of silently falling through. By default this wizard path is interactive and only runs in a TTY session. In non-interactive contexts, bare `drill` falls back to showing the last 5 runs inline unless you pass `-y` or `--yes` to run the wizard and accept defaults automatically. When a TTY is available, `-y` keeps the wizard visible, shows a brief `waiting for run…` state after push, and auto-advances the default path; without a TTY it falls back to the headless path.

```sh
drill
drill --skip test,lint
```
...
Unlike `drill attach`, bare `drill` only auto-attaches to an active run on the current branch.
...
```sh
drill init
drill init --fork-url git@github.com:you/my-repo.git
```
...
Creates or refreshes a local bare repo, installs the managed pre-receive admission and post-receive notification hooks, best-effort isolates the gate repo's hook path from shared git config changes when Git supports `config --worktree`, adds or repairs the `drill` git remote, detects the default branch, records or updates the repo in SQLite, installs the `/drill` agent skill at user level into `~/.claude/skills/drill/SKILL.md` and `~/.agents/skills/drill/SKILL.md`, and ensures the daemon is running, installing the managed service when available and falling back to a detached daemon otherwise.
...
If the repo still contains a vendored skill copy written by an older drill version, `init` leaves it untouched and prints a notice that it is no longer needed and can be removed.
The gate advertises Git push-option support, so you can skip steps for one push with `git push -o drill.skip=test,lint drill <branch>`.
...
Most agent workflows use the installed `/drill` skill, which drives this command surface underneath.
...
With no subcommand, shows the executable path, description, repo, current branch, daemon state, recent runs, and next-step help, including a pointer to `drill axi run --help` and the installed `/drill` skill for full driving guidance.
...
Use `drill rerun` only after that monitor is no longer running, such as a closed PR, aborted or superseded run, idle timeout, or exhausted CI auto-fix attempts.
...
drill axi respond --action fix --findings F1,F2 --instructions "optional guidance"
drill axi respond --action fix --add-finding '{"description":"...","action":"auto-fix"}'
drill axi respond --action skip
```
...
Gate summaries and finding descriptions are bounded in this default status view; truncated values disclose their original length, and the gate help points to `drill axi logs --step <step> --full` for the complete step log.
...
drill axi sync --recover --keep-local
```
...
While a run is still active, it reports `state: pipeline_owned`, the exact submitted/current heads and their relation, and `next_action.code: continue_active_run` with `drill axi status`, even when its head has not moved yet.
`--recover` verifies the run is terminal, anchors the preserved head under `refs/drill/recover/<run>` in the invoking repository, and stamps custody returned so a fresh run can start.
...
A rebase-only cancelled run can still refuse recovery because its detached worktree advances the recorded run head without advancing that gate branch; use `drill rerun` in that case.
That adoption anchors the pre-recovery local head under `refs/drill/recover-local/<run>`, then moves the branch with Git operations that refuse on their own rather than after a preceding check: an atomic compare-and-swap on the branch ref, and a working-tree update that aborts instead of overwriting a modified or untracked file.
...
`drill rerun` is the alternative exit that resumes validating the preserved head instead of taking the branch back.
...
```sh
drill axi logs --step review
drill axi logs --step review --full
drill axi logs --step review --run <id>
```
...
Pipeline-created commits remain preserved in the gate and a recoverable cancellation points directly to `drill axi sync --recover`; when the submitted head never moved, cancellation instead reports `state: user_owned` with no sync action.
While a run is active, do not use `axi abort` or `drill rerun` to go fix a finding yourself.
...
Removes the `drill` remote, deletes the bare repo directory, cleans up worktrees, and deletes the database record (cascades to runs and steps).
...
Opens the TUI for the active run anywhere in the current repo. If `--run` is specified, attaches to that specific run regardless of branch. Unlike bare `drill`, this does not stay branch-scoped before falling back.
...
```sh
drill rerun
drill rerun --intent "the revised user goal"
```
...
The command uses the same service and safety contract as `drill axi sync`, including the guarded equivalent advance and custody recovery documented there; it never stashes, rebases, creates a merge commit, switches branches, deletes a branch, or updates an external remote.
...
The field definitions and their local/remote split are owned by [the environment reference](/drill/reference/environment/#what-stays-local-and-what-leaves-the-machine).
...
The [Global Config Reference](/drill/reference/global-config/) owns ACP gate-validation availability and probing semantics.
...
`doctor` checks `gh` and `az` availability. For GitLab PR and CI steps, install and authenticate `glab`. For Bitbucket Cloud PR and CI steps, set `DRILL_BITBUCKET_EMAIL` and `DRILL_BITBUCKET_API_TOKEN`. For Azure DevOps PR and CI steps, install the `azure-devops` extension and provide a PAT.
...
[Daemon & Worktrees](/drill/concepts/daemon/#starting-and-stopping)
owns the active-run guard, the scope of `--force` and `--yes`, and recursive
validation-step containment.

Because `update` installs the latest official release binary, the replacement binary includes the default self-hosted telemetry host and website ID. Disable telemetry with `DRILL_TELEMETRY=0`, or override the host and website ID with `DRILL_UMAMI_HOST` and `DRILL_UMAMI_WEBSITE_ID`.

Background update checks run automatically on each CLI invocation (except `update` itself and version queries `--version` / `-v`, which stay side-effect-free). If a newer version is available, a notification is printed to stderr. Suppressed for dev builds or when `DRILL_NO_UPDATE_CHECK=1` is set.
...
This does not remove the managed service. A later `drill`, `drill daemon start`, `init`, `attach`, `rerun`, or `update` can start the daemon again through the same service manager when available, or as a detached daemon otherwise.

## drill daemon restart

Restart the daemon.

```sh
drill daemon restart
drill daemon restart --force
```
