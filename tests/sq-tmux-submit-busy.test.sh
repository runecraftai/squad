#!/usr/bin/env bash
# tests/sq-tmux-submit-busy.test.sh - regression: busy pane + pending composer
...
TMP_ROOT=$(mktemp -d "${TMPDIR:-/tmp}/sq-tmux-submit-busy.XXXXXX")
trap 'rm -rf "$TMP_ROOT"' EXIT

# Override fm_pane_is_busy for testing: SQUAD_FAKE_PANE_BUSY=1 means busy.
fm_pane_is_busy() {
  [ "${SQUAD_FAKE_PANE_BUSY:-0}" = 1 ]
}
...
case "${1:-}" in
  ...
  send-keys)
    ...
    if [ "$is_enter" = 1 ]; then
      [ -z "${SQUAD_FAKE_SENT:-}" ] || printf 'Enter\n' >> "$SQUAD_FAKE_SENT"
...
test_busy_pane_pending_returns_empty() {
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" fm_tmux_composer_state "win" > "$vfile" 2>/dev/null
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" SQUAD_FAKE_SENT="$sent" \
    SQUAD_FAKE_SWALLOW="$dir/.swallow" SQUAD_FAKE_PERSIST_SWALLOW=1 SQUAD_FAKE_PANE_BUSY=1 \
...
test_idle_pane_pending_returns_pending() {
  ...
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" SQUAD_FAKE_SENT="$sent" \
    SQUAD_FAKE_SWALLOW="$dir/.swallow" SQUAD_FAKE_PERSIST_SWALLOW=1 SQUAD_FAKE_PANE_BUSY=0 \
    fm_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
...
test_busy_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" SQUAD_FAKE_SENT="$sent" SQUAD_FAKE_PANE_BUSY=1 \
    fm_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
...
test_idle_pane_composer_clears_first_try() {
  ...
  printf '╭────────────╮\n│ > fix      │\n╰────────────╯\n' > "$composer"
  : > "$sent"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" SQUAD_FAKE_SENT="$sent" SQUAD_FAKE_PANE_BUSY=0 \
    fm_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
...
test_busy_pane_unknown_stays_unknown() {
  ...
  printf '│ > unbounded\n' > "$composer"
  touch "$dir/.swallow"
  PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" SQUAD_FAKE_PANE_BUSY=1 \
...
test_unrecognized_state_skips_busy_conversion() {
  ...
  (
    ...
    PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" \
      fm_tmux_submit_enter_core "win" 3 0.05 > "$vfile" 2>/dev/null
...
test_claude_busy_signature_uses_real_capture_shapes() {
  ...
  pane_busy() {
    PATH="$fakebin:$PATH" SQUAD_FAKE_COMPOSER="$composer" \
      bash -c '. "$1/bin/sq-tmux-lib.sh"; fm_pane_is_busy "$2" "$3"' \
      _ "$ROOT" "$1" "${2:-}"
  }
