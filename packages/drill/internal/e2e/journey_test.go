...
//
//   - `drill init` (gate setup, daemon bootstrap, post-receive
//     hook installation)
//   - `git push drill <branch>` (real git transport, hook fires,
...
func TestAXIControlByteFailureGateRemainsReadable(t *testing.T) {
	...
	h.CommitChange("control-byte-test-gate", ".drill.yaml", config, "configure control-byte test failure")
...
func runHappyPath(t *testing.T, agentName string) {
	...
	// `drill init` sets up the gate and starts the daemon.
...
func assertRootVersion(t *testing.T, h *Harness) {
	...
	if !strings.HasPrefix(out, "drill version ") {
...
func assertInitOutput(t *testing.T, h *Harness, out string) {
	...
	for _, want := range []string{resolved, "git push drill", "|__| |_/", "Gate initialized"} {
		if !strings.Contains(out, want) {
...
func assertRunsEmpty(t *testing.T, h *Harness) {
	...
	for _, want := range []string{"no runs", "git push drill <branch>"} {
		if !strings.Contains(out, want) {
...
func assertRootNoActiveRun(t *testing.T, h *Harness) {
	...
	for _, want := range []string{"No active run", "git push drill"} {
		if !strings.Contains(out, want) {
...
func assertRootNoActiveRunOnOtherBranch(t *testing.T, h *Harness, activeRun *ipc.RunInfo) {
	...
	for _, want := range []string{"No active run", "Recent runs", activeRun.Branch, string(activeRun.Status), "git push drill"} {
		if !strings.Contains(out, want) {
...
func assertRootRecentRuns(t *testing.T, h *Harness, run *ipc.RunInfo) {
	...
	for _, want := range []string{"No active run", "Recent runs", run.Branch, string(run.Status), sha, "git push drill"} {
		if !strings.Contains(out, want) {
...
func assertAgentEditCommitRun(t *testing.T, h *Harness) {
	...
	originalHead := h.CommitChange("agent-edits", ".drill.yaml", config, "configure formatter")
	...
	if strings.TrimSpace(string(message)) != "drill: apply agent fixes" {
...
func assertFormatFailureWarningRun(t *testing.T, h *Harness) {
	...
	config := "ignore_patterns:\n  - '*.generated.go'\n  - 'vendor/**'\ncommands:\n  format: \"exit 1\"\n"
	h.CommitChange("format-fails", ".drill.yaml", config, "configure failing formatter")
...
func assertGateRefDeletionDoesNotCreateRun(t *testing.T, h *Harness, branch string) {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()
	out, err := h.runGit(ctx, h.WorkDir, "push", "drill", ":"+branch)
	if err != nil {
...
func assertConfiguredCommandRun(t *testing.T, h *Harness) {
	...
	config := "ignore_patterns:\n  - '*.generated.go'\n  - 'vendor/**'\ncommands:\n  test: nm-test-e2e\n  lint: nm-lint-e2e\n"
	head := h.CommitChange("configured-commands", ".drill.yaml", config, "enable configured checks")
...
func assertExplicitAttachUsesRepoWideActiveRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("explicit-attach-active", ".drill.yaml", config, "configure explicit attach slow test")
...
func assertSupersededRunCancellation(t *testing.T, h *Harness) {
	...
	h.CommitChange("superseded-run", ".drill.yaml", config, "configure superseded slow test")
...
func assertDifferentBranchDoesNotCancelActiveRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("different-branch-slow", ".drill.yaml", slowConfig, "configure different-branch slow test")
	...
	h.CommitChange("different-branch-fast", ".drill.yaml", fastConfig, "configure different-branch fast checks")
...
func assertCancelRunStopsActivePipeline(t *testing.T, h *Harness) {
	...
	h.CommitChange("cancel-run", ".drill.yaml", config, "configure cancel slow test")
...
func assertAbortByRunIDReapsRunFromOutsideWorktree(t *testing.T, h *Harness) {
	...
	h.CommitChange("abort-by-id", ".drill.yaml", config, "configure abort-by-id slow test")
...
func assertRespondNoWaitingStepRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("respond-no-waiting", ".drill.yaml", config, "configure slow test command")
...
func assertFailingTestCommandRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("failing-test-command", ".drill.yaml", config, "configure failing test command")
...
func assertFailingLintCommandRun(t *testing.T, h *Harness) {
	...
	h.CommitChange("failing-lint-command", ".drill.yaml", config, "configure failing lint command")
...
func assertRootDefaultsToHistory(t *testing.T, h *Harness) {
	...
	if !strings.Contains(out, "more - run 'drill runs' to see all") {
...
func assertGateRemotePresent(t *testing.T, h *Harness) {
	t.Helper()
	out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "drill")
	if err != nil {
...
func assertGateRemoteAbsent(t *testing.T, h *Harness) {
	...
	out, err := h.runGit(context.Background(), h.WorkDir, "remote", "get-url", "drill")
	if err == nil {
		t.Fatalf("drill remote should have been removed after eject, got %s", out)
	}
}
