...
This page is an enforceable index of the `bin/sq-*.sh` commands that own the operations agents most often reach for by hand, so that raw `tmux`, ad hoc status-file reads, and manual process control never substitute for Squad's own tools.
...
- [`AGENTS.md`](../AGENTS.md) - the operating contract these tools implement; section 8 owns the supervision protocol, section 7 owns the task lifecycle these commands drive.
