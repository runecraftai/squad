#!/usr/bin/env bash
# tests/sq-backend-herdr-respawn-idem-e2e.test.sh - isolated real-herdr
# regression test for Squad-restart idempotency against herdr's
...
# restored sq-<id> task tab came back a husk - a dead pane, or a plain
...
# because a same-labeled tab already existed. Every unit respawn after a
...
# through the full bin/sq-spawn.sh + fob pipeline - mirrors
# tests/sq-backend-herdr-prune-safety-e2e.test.sh's own style, and avoids any
# question of whether fob itself supports re-acquiring a worktree for
...
SESSION="sq-lab-respawn-idem-e2e-$$"
export HERDR_SESSION="$SESSION"
SCRATCH=$(mktemp -d "${TMPDIR:-/tmp}/sq-herdr-respawn-idem.XXXXXX")
cleanup_all() {
...
# --- 1. spawn two real task tabs (operator-shaped and XO-shaped) ----
...
CREW_LABEL="sq-respawn-crew1"
CREW_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD" "$SEEDED_TAB_ID") \
  || fail "initial operator-shaped task creation failed"
read -r CREW_TAB_ID CREW_PANE_ID <<EOF
$CREW_IDS
EOF
if [ -z "$CREW_TAB_ID" ] || [ -z "$CREW_PANE_ID" ]; then
  fail "initial operator-shaped task did not return tab/pane ids"
fi

SM_LABEL="sq-respawn-sm1"
SM_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
  || fail "initial XO-shaped task creation failed"
read -r SM_TAB_ID SM_PANE_ID <<EOF
$SM_IDS
EOF
if [ -z "$SM_TAB_ID" ] || [ -z "$SM_PANE_ID" ]; then
  fail "initial XO-shaped task did not return tab/pane ids"
fi

pass "repro setup: two real sq-<id> task tabs exist (operator-shaped and XO-shaped), neither with a registered agent"
...
if ! herdr pane get "$CREW_PANE_ID" --session "$SESSION" >/dev/null 2>&1; then
  fail "repro setup is wrong: the operator-shaped pane should survive a session restart alive (docs/herdr-backend.md 'ID stability'), but it is gone"
fi
...
RESPAWN_CREW_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "$CREW_LABEL" "$PROJ_CWD") \
  || fail "REGRESSION: create_task refused to respawn into the operator-shaped husk instead of closing-and-replacing it - this is the exact 2026-07-03 incident (manual pane close required)"
read -r NEW_CREW_TAB_ID NEW_CREW_PANE_ID <<EOF
$RESPAWN_CREW_IDS
EOF
if [ -z "$NEW_CREW_TAB_ID" ] || [ -z "$NEW_CREW_PANE_ID" ]; then
  fail "husk respawn (operator-shaped) did not return new tab/pane ids"
fi
[ "$NEW_CREW_PANE_ID" != "$CREW_PANE_ID" ] || fail "husk respawn (operator-shaped) returned the SAME pane id - nothing was actually replaced"
if herdr pane get "$CREW_PANE_ID" --session "$SESSION" >/dev/null 2>&1; then
  fail "REGRESSION: the old operator-shaped husk pane should have been closed by close-and-replace, but it still exists"
fi
pass "fixed: create_task closes and replaces the operator-shaped restored husk instead of refusing - no manual pane close needed"

RESPAWN_SM_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "$SM_LABEL" "$PROJ_CWD") \
  || fail "REGRESSION: create_task refused to respawn into the XO-shaped husk instead of closing-and-replacing it"
read -r NEW_SM_TAB_ID NEW_SM_PANE_ID <<EOF
$RESPAWN_SM_IDS
EOF
if [ -z "$NEW_SM_TAB_ID" ] || [ -z "$NEW_SM_PANE_ID" ]; then
  fail "husk respawn (XO-shaped) did not return new tab/pane ids"
fi
[ "$NEW_SM_PANE_ID" != "$SM_PANE_ID" ] || fail "husk respawn (XO-shaped) returned the SAME pane id - nothing was actually replaced"
if herdr pane get "$SM_PANE_ID" --session "$SESSION" >/dev/null 2>&1; then
  fail "REGRESSION: the old XO-shaped husk pane should have been closed by close-and-replace, but it still exists"
fi
pass "fixed: create_task closes and replaces the XO-shaped restored husk instead of refusing - same fix, same function, both spawn shapes"
...
herdr pane report-agent "$NEW_CREW_PANE_ID" --source sq-respawn-e2e --agent sq-respawn-live-agent --state idle --session "$SESSION" >/dev/null 2>&1 \
  || fail "could not register a live agent on the respawned operator-shaped pane"
