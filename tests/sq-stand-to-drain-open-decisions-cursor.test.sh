#!/usr/bin/env bash
# tests/sq-stand-to-drain-open-decisions-cursor.test.sh - end-to-end behavior tests
# for the incremental, cursor-backed OPEN DECISIONS scan
# (sq-classify-lib.sh's status_open_decisions_incremental /
# scan_open_decisions_incremental, wired into bin/sq-stand-to-drain.sh). These drive
...
# property, not the fold's own source text. tests/sq-stand-to-drain-open-decisions.test.sh
...
TMP_ROOT=$(fm_test_tmproot sq-stand-to-drain-open-decisions-cursor-tests)
...
test_buried_decision_survives_many_growing_drains_and_resolution_clears_it() {
  ...
  SQUAD_STATE_OVERRIDE="$state" SQUAD_OPEN_DECISIONS_READ_PROBE="$probe" "$DRAIN" > "$out" \
...
test_read_failure_never_silently_returns_empty() {
  ...
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$out" \
...
SH
  ...
  SQUAD_STATE_OVERRIDE="$state" PATH="$fakebin:$PATH" "$DRAIN" > "$out" \
...
SH
  ...
  SQUAD_STATE_OVERRIDE="$state" SQUAD_OPEN_DECISIONS_READ_PROBE="$probe" PATH="$fakebin:$PATH" "$DRAIN" > "$out" \
