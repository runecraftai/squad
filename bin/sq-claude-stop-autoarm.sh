#!/usr/bin/env bash
# Claude Stop-owned sentry auto-arm (asyncRewake hook).
#
...
#     the hook delegates guarded recovery to bin/sq-lock.sh and then re-verifies
...
#   - AFK: while state/.afk exists the away daemon owns the sentry and triage;
...
#   - Foreground arm: the owner runs bin/sq-sentry-arm.sh in the FOREGROUND of
#     this hook-owned process tree (never shell &); Claude owns the process
#     group, so its timeout/session teardown kills arm and sentry together.
...
#     benign when a live identity-matched sentry still has a fresh beacon.
#   - Failure handling: a typed failure is rechecked against the same live,
#     fresh sentry predicate and retried a bounded number of times in this
#     hook. Only an exhausted failure with no verified sentry emits one
...
# outcome so the synchronous Stop guard (bin/sq-turnend-guard.sh --claude) can
...
AUTOARM_ATTEMPTS=${SQUAD_CLAUDE_AUTOARM_ATTEMPTS:-2}
...
# --- AFK: the away daemon owns the sentry and triage; never rewake ----------
[ -e "$STATE/.afk" ] && exit 0
...
# Delegate the claim to sq-lock.sh so its live-owner refusal and write semantics
# remain the single acquisition owner, then re-verify current-session identity
# before touching any auto-arm state.
if [ "$RECOVER_SESSION_LOCK" -eq 1 ]; then
  "$SCRIPT_DIR/sq-lock.sh" >/dev/null 2>&1 || exit 0
...
# 0 so one sentry cycle maps to at most one exit-2 rewake.
...
# 30s cadence (sq-bootstrap.sh x_mode_setup contract).
# shellcheck source=/dev/null
[ -f "$CONFIG/x-mode.env" ] && . "$CONFIG/x-mode.env"

# --- foreground the real arm wrapper ------------------------------------------
# NO shell &: this hook process tree is the harness-owned lifecycle. The arm
# forks the sentry as its own tracked child exactly as it does for the
...
# sentry and fresh-beacon predicate used by the turn-end guard before it is
...
while [ "$attempt" -lt "$AUTOARM_ATTEMPTS" ]; do
  ...
  if [ -n "$OUT" ]; then
    "$SCRIPT_DIR/sq-sentry-arm.sh" >"$OUT" 2>&1 || true
  else
    "$SCRIPT_DIR/sq-sentry-arm.sh" >/dev/null 2>&1 || true
  fi
...
# The need may have vanished mid-cycle (unit torn down, X opted out): nothing
...
if [ "$ACTIONABLE" -eq 1 ]; then
  write_epoch rewake
  {
    printf 'Squad sentry wake - one supervision event needs a handling turn now.\n'
    [ -n "$OUT" ] && grep -E '^(signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Run bin/sq-stand-to-drain.sh first and handle the wake. This Stop hook owns sentry continuity: when the handling turn ends, the next needed cycle arms automatically - do NOT run bin/sq-sentry-arm.sh after an ordinary wake.\n'
  } >&2
  [ -z "$OUT" ] || rm -f "$OUT" 2>/dev/null || true
  exit 2
fi
...
if [ ! -e "$FAILURE_NOTICE" ]; then
  write_epoch failed
  {
    printf 'Squad sentry auto-arm FAILED - the Stop-owned automatic supervision mechanism is broken after %s bounded attempts, and no live sentry with a fresh beacon was verified.\n' "$attempt"
    [ -n "$OUT" ] && grep -E '^(sentry:|signal:|stale:|check:|heartbeat)' "$OUT" 2>/dev/null | head -8
    printf 'Do not launch a manual background arm from this notice; investigate the automatic Stop hook and sentry startup before ending blind.\n'
  } >&2
