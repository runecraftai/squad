...
Six PreToolUse seatbelts mechanically enforce the sharpest-edged of these rules regardless of what the agent remembers: `bin/sq-cd-pretool-check.sh` blocks a persistent top-level `cd`/`pushd`/`popd` in the primary checkout ([`docs/cd-guard.md`](cd-guard.md)), `bin/sq-arm-pretool-check.sh` blocks a malformed or bypassed sentry-arm/checkpoint command ([`docs/arm-pretool-check.md`](arm-pretool-check.md)), `bin/sq-subagent-pretool-check.sh` blocks primary-session delegation outside the unit ([`docs/subagent-guard.md`](subagent-guard.md)), `bin/sq-backend-pretool-check.sh` blocks raw session-provider CLI control, `bin/sq-poll-pretool-check.sh` blocks narrow `sleep`/`while` polling of `state/`, and `bin/sq-push-pretool-check.sh` blocks manual branch pushes from drill task worktrees while allowing the validation pipeline's push.
...
- [docs/cd-guard.md](cd-guard.md), [docs/arm-pretool-check.md](arm-pretool-check.md), [docs/subagent-guard.md](subagent-guard.md), `bin/sq-backend-pretool-check.sh`, `bin/sq-poll-pretool-check.sh`, and `bin/sq-push-pretool-check.sh` - the six PreToolUse seatbelts that mechanically enforce a subset of these rules.
