...
import (
	...
	"github.com/runecraftai/squad/packages/drill/internal/lifecycle"
...
func parseSkipPushOptions(options []string) ([]types.StepName, error) {
	...
	for _, option := range options {
		value, ok := strings.CutPrefix(option, "drill.skip=")
		if !ok {
			continue
		}
...
func formatSkipPushOptions(steps []types.StepName) []string {
	...
	return []string{"drill.skip=" + strings.Join(parts, ",")}
}
...
func newDaemonRunCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			if root != "" {
				if err := os.Setenv("DRILL_HOME", root); err != nil {
					return fmt.Errorf("set DRILL_HOME: %w", err)
				}
			}
			return daemonRun()
		},
	...
	cmd.Flags().StringVar(&root, "root", "", "override drill data directory")
	return cmd
}
