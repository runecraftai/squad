...
TMP_ROOT=$(fm_test_tmproot sq-on)
...
cp "$ROOT/bin/sq-remote-entrypoint.sh" "$ROOT/bin/sq-remote-job-lib.sh" \
  "$ROOT/bin/sq-remote-job-worker.sh" "$REMOTE_ROOT/bin/"

cat > "$REMOTE_ROOT/bin/sq-probe-one.sh" <<'SH'
#!/usr/bin/env bash
set -u
out=$1
rc=$2
...
cat > "$REMOTE_ROOT/bin/sq-mutate.sh" <<'SH'
#!/usr/bin/env bash
printf 'mutation\n' >> "$1"
SH
...
write_registry() {
  cat > "$LOCAL_HOME/data/XOs.md" <<EOF
...
printf '%s\0' 'plain' 'two words' '$(touch /tmp/sq-on-injected)' '' $'line one\nline two' > "$ARGV_EXPECTED"
printf 'payload one\npayload two\n' > "$TMP_ROOT/stdin"
set +e
# shellcheck disable=SC2016 # Literal shell-looking argv is the injection probe.
fm_on ios sq-probe-one.sh "$ARGV_ACTUAL" 23 \
  'plain' 'two words' '$(touch /tmp/sq-on-injected)' '' $'line one\nline two' \
...
assert_absent /tmp/sq-on-injected "shell-looking argv was interpreted"
pass "sq-on preserves argv, stdin, stdout, stderr, and exit status without shell interpretation"
...
# this on the real ssh argv the SQUAD_SSH_BIN process seam captured, never on
# sq-on.sh source text.
...
pass "sq-on arms a bounded SSH dead-peer detection window by default (${DEFAULT_INTERVAL}s x $DEFAULT_COUNT = ${DEFAULT_WINDOW}s)"

: > "$SSH_LOG"
SQUAD_SSH_ALIVE_INTERVAL=7 SQUAD_SSH_ALIVE_COUNT_MAX=2 fm_on ios sq-probe-two.sh >/dev/null
OVERRIDE_ARGV=$(tail -n 1 "$SSH_LOG")
assert_contains "$OVERRIDE_ARGV" 'ServerAliveInterval=7' "SQUAD_SSH_ALIVE_INTERVAL override was not honored on the ssh transport"
assert_contains "$OVERRIDE_ARGV" 'ServerAliveCountMax=2' "SQUAD_SSH_ALIVE_COUNT_MAX override was not honored on the ssh transport"
pass "sq-on's dead-peer detection window is env-overridable"

SSH_CALLS_BEFORE_INVALID=$(cat "$SSH_COUNT")
set +e
INVALID_INTERVAL_OUT=$(SQUAD_SSH_ALIVE_INTERVAL=0 fm_on ios sq-probe-two.sh 2>&1)
INVALID_INTERVAL_RC=$?
INVALID_COUNT_OUT=$(SQUAD_SSH_ALIVE_COUNT_MAX=not-a-number fm_on ios sq-probe-two.sh 2>&1)
INVALID_COUNT_RC=$?
set -e
[ "$INVALID_INTERVAL_RC" -eq 1 ] || fail "a zero SQUAD_SSH_ALIVE_INTERVAL was accepted (got exit $INVALID_INTERVAL_RC)"
[ "$INVALID_COUNT_RC" -eq 1 ] || fail "a non-integer SQUAD_SSH_ALIVE_COUNT_MAX was accepted (got exit $INVALID_COUNT_RC)"
assert_contains "$INVALID_INTERVAL_OUT" 'SQUAD_SSH_ALIVE_INTERVAL must be a positive integer' "invalid interval did not explain its constraint"
assert_contains "$INVALID_COUNT_OUT" 'SQUAD_SSH_ALIVE_COUNT_MAX must be a positive integer' "invalid count did not explain its constraint"
...
CHILD_PATH=$(fm_on ios sq-probe-path.sh)
NVM_CHILD_DIRS=()
...
set +e
out=$(fm_on ios sq-remote-doctor.sh 2>&1)
set -e
...
# tests/sq-remote-doctor.test.sh owns the verdict against controlled fixtures.
set +e
out=$(fm_on ios sq-remote-doctor.sh 2>/dev/null)
set -e
...
fm_on ios sq-probe-two.sh >/dev/null
: > "$TOOL_PROBE_LOG"
set +e
out=$(fm_on ios sq-remote-doctor.sh 2>&1)
set -e
...
printf '#!/usr/bin/env bash\nexit 0\n' > "$DOCTOR_BIN/fob"
printf '#!/usr/bin/env bash\nexit 0\n' > "$DOCTOR_BIN/claude"
chmod +x "$DOCTOR_BIN/jq" "$DOCTOR_BIN/herdr" "$DOCTOR_BIN/tasks-axi" "$DOCTOR_BIN/fob" "$DOCTOR_BIN/claude"
set +e
...
assert_contains "$out" "required fob=$DOCTOR_BIN/fob" "the remote doctor did not require fob"
...
fm_on ios sq-probe-one.sh "$ARGV_TWO" 0 'second command' >/dev/null 2>/dev/null
cmp -s "$TMP_ROOT/argv-two-expected.bin" "$ARGV_TWO" || fail "second dynamic command did not execute"
pass "multiple sq-*.sh executables work without a command table"

for bad in '../sq-probe-one.sh' 'sq-probe-one.sh/extra' 'sh' 'sq-../../bin/sh'; do
  if fm_on ios "$bad" >/dev/null 2>&1; then
    fail "unsafe command name was accepted: $bad"
  fi
done
ln -s sq-probe-one.sh "$REMOTE_ROOT/bin/sq-symlink.sh"
if fm_on ios sq-symlink.sh >/dev/null 2>&1; then
  fail "a symlinked command was accepted"
fi
cat > "$REMOTE_ROOT/bin/sq-untracked.sh" <<'SH'
#!/usr/bin/env bash
printf 'untracked command ran\n'
SH
chmod +x "$REMOTE_ROOT/bin/sq-untracked.sh"
GIT_SHADOW_LOG="$TMP_ROOT/git-shadow.log"
cat > "$REMOTE_ROOT/bin/git" <<'SH'
#!/usr/bin/env bash
printf 'consulted\n' >> "$SQUAD_GIT_SHADOW_LOG"
exit 0
SH
chmod +x "$REMOTE_ROOT/bin/git"
SQUAD_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" "$REMOTE_ROOT/bin/git" -C "$REMOTE_ROOT" ls-files --error-unmatch bin/sq-untracked.sh \
...
untracked_argv_b64=$(printf '%s\0' sq-untracked.sh | base64 | tr -d '\n')
set +e
out=$(SQUAD_GIT_SHADOW_LOG="$GIT_SHADOW_LOG" SQUAD_REMOTE_JOB_PLATFORM_OVERRIDE=Linux \
  SQUAD_REMOTE_JOB_STATE_ROOT="$TMP_ROOT/remote-jobs" "$REMOTE_ROOT/bin/sq-remote-entrypoint.sh" \
  1 "$untracked_root_b64" "$untracked_home_b64" "$untracked_argv_b64" 2>&1)
rc=$?
set -e
if [ "$rc" -eq 0 ]; then
  fail "an untracked sq-*.sh executable was accepted"
fi
...
cp "$ROOT/bin/sq-remote-doctor.sh" "$REMOTE_ROOT/bin/sq-remote-doctor.sh"
...
EOF
if fm_on ios sq-probe-two.sh >/dev/null 2>&1; then
...
argv_b64=$(printf '%s\0' sq-probe-two.sh | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/sq-remote-entrypoint.sh" 2 "$root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
  fail "an incompatible transport protocol was accepted"
fi
traversal_root_b64=$(printf '%s' "$REMOTE_ROOT/../remote-root" | base64 | tr -d '\n')
if "$REMOTE_ROOT/bin/sq-remote-entrypoint.sh" 1 "$traversal_root_b64" "$home_b64" "$argv_b64" >/dev/null 2>&1; then
...
EOF
if fm_on remote-mac sq-probe-two.sh >/dev/null 2>&1; then
  fail "an ambiguous SSH alias was accepted"
fi
out=$(fm_on ios sq-probe-two.sh)
assert_contains "$out" "home=$REMOTE_HOME" "XO-id routing broke after alias ambiguity"
write_registry
pass "ambiguous aliases refuse while exact XO ids remain routable"

: > "$SSH_COUNT"
set +e
SQUAD_FAKE_SSH_MODE=unreachable fm_on ios sq-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
...
: > "$SSH_COUNT"
set +e
SQUAD_FAKE_SSH_MODE=ambiguous fm_on ios sq-mutate.sh "$REMOTE_HOME/mutations" >/dev/null 2>&1
rc=$?
set -e
