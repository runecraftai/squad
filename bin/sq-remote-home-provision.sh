...
EOF
  ...
  if [ -e "$DEST" ] || [ -L "$DEST" ]; then
    ...
    if [ "$MODE" = drill ]; then
      command -v drill >/dev/null 2>&1 || die "drill is unavailable for project $NAME"
      (cd "$DEST" && drill init >/dev/null && drill doctor >/dev/null) \
        || die "drill initialization failed for project $NAME"
    fi
  fi
