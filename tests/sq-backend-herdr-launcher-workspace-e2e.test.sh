#!/usr/bin/env bash
# tests/sq-backend-herdr-launcher-workspace-e2e.test.sh - mandatory ISOLATED
# end-to-end real-Herdr test for worker PLACEMENT with presentation spaces
# disabled.
#
# The guarantee under test: an operator or recon is created in the exact Herdr
# workspace of the Squad or XO process that launched it, identified
...
# "Squad", and the previous label-first-match resolution put the worker in
# whichever one sorted first - visibly the wrong space whenever the launcher was
# not in it.
#
# This drives the REAL bin/sq-spawn.sh and bin/sq-teardown.sh, because the
# guarantee spans the whole spawn handoff (sq-spawn.sh's herdr arm ->
...
# The headline duplicate-label case additionally runs sq-spawn.sh INSIDE a real
...
# operation goes through bin/sq-herdr-lab.sh, which appends the named session
# flag and verifies the default unit session is unchanged after teardown.
set -u
...
TMP_ROOT=$(mktemp -d "$(cd "${TMPDIR:-/tmp}" && pwd -P)/sq-herdr-launcher-e2e.XXXXXX")
HERDR_LAB_HELPER="$ROOT/bin/sq-herdr-lab.sh"
HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name sq-herdr-launcher-ws) || {
  rm -rf "$TMP_ROOT"
...
# so a second teardown would otherwise report the already-consumed unit-state
...
cleanup_all() {
  ...
  for wt in ${WORKTREES[@]+"${WORKTREES[@]}"}; do
    [ -n "$wt" ] && fob return --force "$wt" >/dev/null 2>&1
  done
  WORKTREES=()
...
# spawn_from_launcher <launcher-pane|""> <home> <task-id> <project> [extra sq-spawn args...]
# Composes exactly the Herdr identity Herdr itself injects into a pane's
# processes. An empty launcher pane means "this Squad is not running inside
# Herdr at all".
...
printf '%s\n' "$SM_ID" > "$SM_HOME/.sq-xo-home"
printf 'trivial e2e XO charter: nothing to do.\necho done >> %s.status\n' "$SM_HOME/data" > "$SM_HOME/data/charter.md"

SM2_ID="lwsm2"
SM2_HOME="$TMP_ROOT/XO-home-2"
...
printf '# scratch XO home AGENTS.md placeholder\n' > "$SM2_HOME/AGENTS.md"
printf '%s\n' "$SM2_ID" > "$SM2_HOME/.sq-xo-home"
printf 'trivial e2e XO charter: nothing to do.\necho done >> %s.status\n' "$SM2_HOME/data" > "$SM2_HOME/data/charter.md"
...
for id in uniqA uniqB dupC dupD staleF smE presU presD; do
  ...
  printf 'trivial launcher-placement brief: nothing to do.\necho done >> %s.status\n' "$PRIMARY_HOME/data/$id" > "$PRIMARY_HOME/data/$id/brief.md"
  printf 'trivial launcher-placement brief: nothing to do.\necho done >> %s.status\n' "$SM_HOME/data/$id" > "$SM_HOME/data/$id/brief.md"
  printf 'trivial launcher-placement brief: nothing to do.\necho done >> %s.status\n' "$PRES_HOME/data/$id" > "$PRES_HOME/data/$id/brief.md"
done
mkdir -p "$PRIMARY_HOME/data/$SM2_ID"
printf 'trivial XO charter brief: nothing to do.\necho done >> %s.status\n' "$PRIMARY_HOME/data/$SM2_ID" > "$PRIMARY_HOME/data/$SM2_ID/brief.md"
...
$(make_workspace commander-other)
EOF
[ -n "$WS_OTHER" ] || fail "could not create the unrelated commander workspace"
lab tab focus "$WS_OTHER_TAB" >/dev/null 2>&1 || fail "could not focus the unrelated commander workspace"

focused_workspace() {
  lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.focused == true) | .workspace_id][0] // empty' 2>/dev/null
}
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the unrelated commander workspace did not take focus"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = Squad ] || fail "uniqA did not land in a 'Squad' workspace"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the spawn stole focus from the commander's workspace"
pass "real herdr E2E: with one 'Squad' workspace and no herdr parent, an operator still lands in this home's own workspace without stealing focus"

# --- 2. unique label, WITH a launcher pane: same workspace, now by identity --

read -r _ _ LAUNCH_PRIMARY_PANE <<EOF
$(lab tab create --workspace "$WS_PRIMARY" --cwd "$TMP_ROOT" --label commander-shell --no-focus 2>/dev/null \
...
[ "$(workspace_of_pane "$UNIQB_PANE")" = "$WS_PRIMARY" ] \
  || fail "an operator launched from the 'Squad' workspace must stay in it"
...
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "a projected spawn stole focus from the commander's workspace"
...
$(make_workspace Squad)
EOF
[ -n "$WS_PRIMARY_DUP" ] || fail "could not create the second 'Squad' workspace"
[ "$WS_PRIMARY_DUP" != "$WS_PRIMARY" ] || fail "the two 'Squad' workspaces must be distinct"
DUP_COUNT=$(lab workspace list 2>/dev/null | jq -r '[.result.workspaces[]? | select(.label == "Squad")] | length')
[ "$DUP_COUNT" = 2 ] || fail "expected exactly two 'Squad' workspaces, got $DUP_COUNT"
...
lab pane run "$LAUNCH_DUP_PANE" "$TMP_ROOT/spawn-in-pane.sh" >/dev/null 2>&1 \
  || fail "could not run sq-spawn.sh inside the launcher's herdr pane"
i=0
while [ ! -f "$TMP_ROOT/dupC.rc" ] && [ "$i" -lt 120 ]; do sleep 2; i=$((i + 1)); done
[ -f "$TMP_ROOT/dupC.rc" ] || fail "sq-spawn.sh never finished inside the launcher's herdr pane"
...
[ "$DUPC_WS" = "$WS_PRIMARY_DUP" ] \
  || fail "a worker launched from the second 'Squad' workspace ($WS_PRIMARY_DUP) landed in '$DUPC_WS' instead"
...
pass "real herdr E2E: with two 'Squad' workspaces, a worker spawned from inside the second one lands in that exact workspace"
...
[ "$(label_of_workspace "$WS_PRIMARY")" = Squad ] \
  || fail "the other same-labeled workspace was renamed"
[ "$(focused_workspace)" = "$WS_OTHER" ] || fail "the in-pane spawn stole focus from the commander's workspace"
...
DUP_TABS=$(lab tab list --workspace "$WS_PRIMARY" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "sq-dupD")] | length')
DUP_TABS2=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "sq-dupD")] | length')
...
# A Squad whose own pane was closed under it has an identity that no longer
...
STALE_TABS=$(lab tab list --workspace "$WS_PRIMARY_DUP" 2>/dev/null | jq -r '[.result.tabs[]? | select(.label == "sq-staleF")] | length')
...
[ "$SME_WS" = "$WS_SM_LAUNCH" ] \
  || fail "an XO's own worker must land in the XO's exact workspace ($WS_SM_LAUNCH), got '$SME_WS'"
[ "$(tab_labels_of_workspace "$WS_SM_DECOY")" = "$WS_SM_DECOY_TABS_BEFORE" ] \
  || fail "the duplicate XO-labeled workspace was mutated"
pass "real herdr E2E: an XO launching its own worker gets the same exact-workspace guarantee, and its same-labeled sibling is untouched"

# --- 7. a --xo launch is NOT collapsed into the launcher's workspace -

spawn_from_launcher "$LAUNCH_DUP_PANE" "$PRIMARY_HOME" "$SM2_ID" "$SM2_HOME" --xo
[ "$SPAWN_RC" -eq 0 ] || fail "the primary's --xo launch failed"$'\n'"$(cat "$SPAWN_ERR")"
...
SQUAD_ROOT_OVERRIDE="$ROOT" SQUAD_STATE_OVERRIDE="$PRIMARY_HOME/state" SQUAD_DATA_OVERRIDE="$PRIMARY_HOME/data" \
  ...
  "$ROOT/bin/sq-teardown.sh" dupC >"$TMP_ROOT/teardown.out" 2>&1
status=$?
[ "$status" -eq 0 ] || fail "sq-teardown.sh failed for dupC"$'\n'"$(cat "$TMP_ROOT/teardown.out")"
[ ! -f "$DUPC_META" ] || fail "sq-teardown.sh did not remove dupC's meta"
if lab pane get "$DUPC_PANE" >/dev/null 2>&1; then
  fail "sq-teardown.sh did not close dupC's own pane"
fi
...
[ "$(label_of_workspace "$WS_PRIMARY_DUP")" = Squad ] || fail "teardown removed or renamed the launcher's workspace"
...
if ! cleanup_all; then
  trap - EXIT
  printf 'not ok - isolated Herdr lab teardown failed or the default unit session changed\n' >&2
  exit 1
fi
trap - EXIT
pass "real herdr E2E: isolated lab session removed and default unit session unchanged"
