...
// TestAxiAgentJourney proves an autonomous agent can drive a full drill
// pipeline headlessly through the `drill axi` surface in an isolated
...
func TestAxiBranchSyncJourney(t *testing.T) {
	...
	for _, want := range []string{"outcome: passed", "branch_sync:", "state: behind", "command: drill axi sync"} {
...
func TestAxiCustodyRecoveryJourney(t *testing.T) {
	...
	for _, want := range []string{"recovered: true", "state: custody_returned", "changed: true", "drill axi run --intent"} {
	...
	anchorRef := "refs/drill/recover/" + run.ID
...
func TestAxiCustodyRecoveryAfterRebaseJourney(t *testing.T) {
	...
	localAnchor := "refs/drill/recover-local/" + run.ID
...
// assertSkillInstalled verifies init wrote the drill skill into both
...
func assertSkillInstalled(t *testing.T, h *Harness) {
	...
	for _, rel := range []string{
		filepath.Join(".claude", "skills", "drill", "SKILL.md"),
		filepath.Join(".agents", "skills", "drill", "SKILL.md"),
	} {
