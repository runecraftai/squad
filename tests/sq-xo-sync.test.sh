#!/usr/bin/env bash
# Tests for the local-HEAD XO sync: every XO home tracks the
# PRIMARY Squad checkout's current default-branch commit by a purely LOCAL
# fast-forward (no origin fetch). Two hook points drive it - bin/sq-spawn.sh
# (before launching an XO) and bin/sq-bootstrap.sh (a startup sweep of
# every live XO home) - and both share the ff machinery in
# bin/sq-ff-lib.sh.
#
...
#   - The bootstrap sweep fast-forwards every live XO home and sends a
#     reread nudge ONLY for a running XO whose instruction surface
#     actually changed; a successful send is reported as BOOTSTRAP_INFO:, a
#     failed send is reported as NUDGE_XOS:, an already-current or
#     readme-only home is never nudged, a skipped home is reported as
#     XO_SYNC:, and a home with no live metadata is never swept.
#   - Spawning an XO fast-forwards its worktree to the primary's HEAD
...
TMP_ROOT=$(fm_test_tmproot sq-xo-sync)
export SQUAD_BACKEND=tmux

# --- world builders --------------------------------------------------------

# new_world <name>: a PRIMARY Squad repo on `main` with one commit (the
...
new_world() {
  ...
  # Fresh sentry beacon keeps sq-guard quiet for the spawn path.
  ...
  # so an XO home's data/state/projects can never block its fast-forward.
  printf 'projects/\nstate/\ndata/\n.drill/\nconfig/crew-harness\n' > "$w/main/.gitignore"
  printf 'v1\n' > "$w/main/AGENTS.md"
  printf 'r1\n' > "$w/main/README.md"
...
# add_sm_worktree <w> <id> <commit>: an XO home as a DETACHED worktree of
# the primary at <commit>, plus its seed marker and a LIVE kind=xo meta
...
# the tracked .gitignore and commit it on main. The marker (.sq-xo-home)
# is Squad-generic, written by bin/sq-home-seed.sh into every seeded home; once
...
# seed_marked_home <w> <id> <commit>: an XO home matching what
# bin/sq-home-seed.sh actually lays down - a detached worktree at <commit>, the
# seed marker, a live kind=xo meta, and the gitignored operational dirs
...
seed_marked_home() {
  ...
  printf 'charter\necho done >> %s.status\n' "$w/$id/data" > "$w/$id/data/charter.md"
}
...
# status line. Uses allow_detached=yes, ignore_seed_marker=yes (the XO
# home contract).
FF_OUT=""
run_ff() {
  local dir=$1 base=$2 outfile="$TMP_ROOT/ff.out"
  ff_target "$dir" "XO sm" "$base" yes yes >"$outfile" 2>&1
  FF_OUT=$(cat "$outfile")
}
...
test_ff_updated() {
  ...
  assert_contains "$FF_OUT" "XO sm: updated " "updated home prints an advance line"
...
test_ff_current() {
  ...
  assert_contains "$FF_OUT" "XO sm: already current" "current home reports already current"
...
test_ff_dirty() {
  ...
  assert_contains "$FF_OUT" "XO sm: skipped: dirty working tree" "dirty home is skipped"
...
test_ff_diverged() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "XO sm: skipped: diverged from $base" "diverged home is skipped"
...
# An XO home carrying its own in-flight work sits on a named feature
...
test_ff_inflight_feature_branch() {
  ...
  [ "$FF_STATUS" = skipped ] || fail "FF_STATUS: expected skipped, got '$FF_STATUS'"
  assert_contains "$FF_OUT" "XO sm: skipped: on feature/wip, expected main" \
...
test_bootstrap_sweep_nudges_only_instruction_change() {
  ...
  info_line=$(printf '%s\n' "$out" | grep '^BOOTSTRAP_INFO: nudged sq-sm-instr ' || true)
  [ -n "$info_line" ] || fail "no BOOTSTRAP_INFO nudge line emitted (got: $out)"
  assert_contains "$info_line" "Squad was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
    "successful nudge report should include the exact message sent"
  assert_not_contains "$out" "NUDGE_XOS:" "successful nudge must not leave a Squad action item"
  assert_not_contains "$out" "sm-readme" "readme-only advance is not nudged"
  assert_not_contains "$out" "sm-current" "already-current XO is not nudged"
  assert_contains "$(cat "$log")" "[sq-from-squad]" "nudge send should use the marked sq-send XO path"
  assert_contains "$(cat "$log")" "Squad was updated to the latest - please re-read your AGENTS.md" \
    "nudge send should type the exact re-read message"
  marker_dir="$w/home/state/.XO-nudge-pending"
...
test_bootstrap_nudge_send_uses_state_override() {
  ...
  touch "$override_state/.last-sentry-beat"
  fakebin=$(make_fake_toolchain "$w")
  log="$w/tmux.log"
  ...
  pass "T8a bootstrap nudge send respects SQUAD_STATE_OVERRIDE"
}
...
test_bootstrap_nudge_retry_rejects_malformed_marker_id() {
  ...
  printf '../escape\n' > "$evil/.sq-xo-home"
  mkdir -p "$w/home/state/.XO-nudge-pending"
  marker="$w/home/state/.XO-nudge-pending/bad.pending"
  {
    printf 'id=../escape\n'
    printf 'selector=sq-../escape\n'
    printf 'home=%s\n' "$evil"
    printf 'commit=%s\n' "$c1"
    printf 'instructions=AGENTS.md\n'
    printf 'message=Squad was updated to the latest - please re-read your AGENTS.md to pick up the new instructions.\n'
  } > "$marker"
  {
    printf 'window=Squad:sq-evil\n'
    printf 'kind=xo\n'
    printf 'home=%s\n' "$evil"
  } > "$w/home/escape.meta"
  fakebin=$(make_fake_toolchain "$w")
  ...
  assert_contains "$out" "NUDGE_XOS: XO ../escape: send failed: retry marker has unsafe id" \
    "malformed retry marker id should be rejected before target resolution"
  assert_not_contains "$out" "BOOTSTRAP_INFO: nudged sq-../escape" \
  ...
  assert_absent "$log" "malformed retry marker should not invoke sq-send"
...
test_bootstrap_nudge_failure_records_retry_marker() {
  ...
  assert_grep "selector=sq-sm-instr" "$marker" "retry marker should pin the stable selector"
  assert_grep "message=Squad was updated to the latest - please re-read your AGENTS.md to pick up the new instructions." \
...
# --- T8b: stale herdr nudge failures retry through current sq-<id> metadata ---
# Reproduces the 2026-07-07 session-start bug: XO_sync used to print raw
# backend targets (default:w9:pY) that liveness respawn immediately replaced
# (default:wA:p2), so sq-send with the printed target fell back to tmux and failed
# while sq-<id> resolved through current meta.
...
SH
  chmod +x "$spawn_stub"
  cp "$spawn_stub" "$w/main/bin/sq-spawn.sh"
  ...
  resolved=$(bash -c '. "$0/bin/sq-backend.sh"; fm_backend_resolve_selector sq-sm-instr "$1"' "$ROOT" "$w/home/state")
  [ "$resolved" = "$fresh" ] || fail "sq-<id> should resolve through post-respawn meta, got '$resolved'"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  stale_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/sq-backend.sh"; fm_backend_source herdr; fm_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$stale" 2>/dev/null; printf '%s' "$?")
  [ "$stale_send" != 0 ] || fail "explicit stale herdr endpoint send should fail"

  # shellcheck disable=SC2016  # $0/$1 belong to the inner bash -c process.
  fresh_send=$(PATH="$herdrfb:$toolchain:$BASE_PATH" bash -c \
    '. "$0/bin/sq-backend.sh"; fm_backend_source herdr; fm_backend_herdr_send_literal "$1" "nudge"' "$ROOT" "$fresh" 2>/dev/null; printf '%s' "$?")
  [ "$fresh_send" = 0 ] || fail "send through sq-<id>-resolved fresh endpoint should succeed"

  pass "T8b stale herdr nudge failures leave a retry marker after respawn rotates sq-<id> metadata"
}

# --- T9: bootstrap surfaces a skipped dirty live XO home --------------
test_bootstrap_sweep_surfaces_skipped_home() {
  ...
  [ -n "$skip_line" ] || fail "no XO_SYNC skip line emitted (got: $out)"
  ...
  pass "T9 bootstrap surfaces a skipped dirty live XO home"
}

# --- T10: spawning an XO fast-forwards its worktree before launch ------
test_spawn_fast_forwards_before_launch() {
  ...
  printf 'sm\n' > "$w/sm/.sq-xo-home"
  mkdir -p "$w/sm/data"
  printf 'charter\necho done >> %s.status\n' "$w/sm/data" > "$w/sm/data/charter.md"
  bump_primary "$w" instr
  c2=$(head_of "$w/main")
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    "$ROOT/bin/sq-spawn.sh" sm "$w/sm" codex --xo >/dev/null 2>&1 || true

  [ "$(head_of "$w/sm")" = "$c2" ] \
    || fail "spawn did not fast-forward the XO worktree to the primary's HEAD"
  pass "T10 spawn fast-forwards an XO worktree to the primary's local HEAD before launch"
}
...
SH
  ...
  PATH="$fakebin:$BASE_PATH" TMUX='' \
    ...
    SQUAD_PROJECTS_OVERRIDE="$w/home/projects" SQUAD_CONFIG_OVERRIDE="$w/home/config" \
    SQUAD_SPAWN_NO_GUARD=1 \
    "$ROOT/bin/sq-spawn.sh" sm "$w/sm" codex --xo >/dev/null 2>"$err" || true

  assert_contains "$(cat "$err")" \
    "warning: XO sm sync skipped before launch: dirty working tree" \
...
# The seed marker used to leave every home permanently dirty: bin/sq-unit-sync.sh
...
test_seed_marker_clean_when_gitignored() {
  ...
  # The exact dirtiness signal bin/sq-unit-sync.sh reads (its line: dirty=yes when
  ...
  # And the XO ff sweep sees no dirt: an at-HEAD home is a clean no-op.
  run_ff "$w/sm" "$base"
  [ "$FF_STATUS" = current ] || fail "fresh home not read as clean/current, got '$FF_STATUS': $FF_OUT"
  pass "T12 gitignored marker: a freshly seeded home reads clean to unit-sync and the ff sweep"
}
...
# Standalone-clone homes converge through /updatesquad's origin fetch instead.
...
test_seed_marker_does_not_mask_real_dirt() {
  ...
  assert_contains "$FF_OUT" "XO sm: skipped: dirty working tree" \
...
echo "# all sq-xo-sync tests passed"
