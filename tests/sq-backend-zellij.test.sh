#!/usr/bin/env bash
# tests/sq-backend-zellij.test.sh - fake-zellij-CLI unit tests for the zellij
# session-provider adapter (bin/backends/zellij.sh), P3 of
# data/sq-backend-design-d7 (report.md "Zellij Backend"). Mirrors
# tests/sq-backend-herdr.test.sh's fakebin/command-log convention: a small,
...
# tests/sq-backend-zellij-smoke.test.sh, gated on the zellij binary actually
# being installed.
set -u
...
TMP_ROOT=$(fm_test_tmproot sq-backend-zellij-tests)

# make_zellij_fakebin: a `zellij` stub that logs every invocation (one line,
# unit-separated args, to $SQUAD_ZELLIJ_LOG) and returns the canned response for
# that call read from $SQUAD_ZELLIJ_RESPONSES/<n>.out, consumed IN ORDER (call 1
# reads 1.out, call 2 reads 2.out, ...), mirroring
# tests/sq-backend-herdr.test.sh's make_herdr_fakebin. A missing response file
...
# SQUAD_ZELLIJ_SESSION_LIST instead.
...
#!/usr/bin/env bash
set -u
LOG="${SQUAD_ZELLIJ_LOG:?}"
RESP="${SQUAD_ZELLIJ_RESPONSES:?}"
...
if [ "${1:-}" = --version ]; then
  printf 'zellij %s\n' "${SQUAD_ZELLIJ_FAKE_VERSION:-0.44.0}"
  exit 0
fi
if [ "${1:-}" = list-sessions ]; then
  printf '%s\n' "${SQUAD_ZELLIJ_SESSION_LIST:-}"
  exit 0
fi
if [ "${1:-}" = attach ]; then
  exit "${SQUAD_ZELLIJ_ATTACH_EXIT:-0}"
fi
...
zellij_tab_response() {
  local dir=$1 n=$2 tab=${3:-3} name=${4:-sq-task}
...
# tests/sq-backend-cmux.test.sh's identical cmux_expected_* helpers), used to
...
zellij_expected_scoped_title() {  # <sq-task-label> [home] [root]
  local label=$1 home=${2:-$ROOT} root=${3:-$ROOT} rest
  case "$label" in
    sq-*) rest=${label#sq-} ;;
    *) rest=$label ;;
  esac
  printf 'sq-%s-%s' "$(zellij_expected_home_label "$home" "$root")" "$rest"
}
...
test_version_check_accepts_current_version() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_FAKE_VERSION=0.44.0 \
...
test_version_check_accepts_newer_version() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_FAKE_VERSION=0.45.2 \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_version_check' "$ROOT"
  status=$?
...
test_version_check_refuses_old_version() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_FAKE_VERSION=0.38.1 \
...
test_session_defaults_to_Squad() {
  local out
  out=$( bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_session' "$ROOT" )
  [ "$out" = Squad ] || fail "default session should be 'Squad', got '$out'"
  pass "fm_backend_zellij_session: defaults to 'Squad' when SQUAD_ZELLIJ_SESSION is unset"
}

test_session_honors_override() {
  local out
  out=$( SQUAD_ZELLIJ_SESSION=sq-test-iso bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_session' "$ROOT" )
  [ "$out" = sq-test-iso ] || fail "SQUAD_ZELLIJ_SESSION override was not honored, got '$out'"
  pass "fm_backend_zellij_session: honors the SQUAD_ZELLIJ_SESSION test-isolation override"
}
...
test_parse_target() {
  ( . "$ROOT/bin/backends/zellij.sh"
    fm_backend_zellij_parse_target "Squad:5" || exit 1
    [ "$SQUAD_BACKEND_ZELLIJ_SESSION" = Squad ] || { echo "session mismatch: $SQUAD_BACKEND_ZELLIJ_SESSION" >&2; exit 1; }
    [ "$SQUAD_BACKEND_ZELLIJ_PANE" = "5" ] || { echo "pane mismatch: $SQUAD_BACKEND_ZELLIJ_PANE" >&2; exit 1; }
...
test_normalize_key() {
  ...
  ) || fail "fm_backend_zellij_normalize_key did not map Squad's key vocabulary to zellij's verified names"
...
test_scoped_title_uses_XO_home_label() {
  local dir out expected
...
test_scoped_title_changes_with_root_path() {
  ...
  pass "fm_backend_zellij_scoped_title: includes the resolved SQUAD_ROOT hash in the home label"
}
...
test_expected_label_accepts_unambiguous_untagged_legacy_tab() {
  ...
  zellij_tab_response "$dir" 2 3 sq-legacy
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_key Squad:7 Escape sq-legacy' "$ROOT"
  ...
  pass "fm_backend_zellij_tab_matches_label: accepts an untagged legacy sq-<id> title when it is the only live tab carrying it"
}
...
test_expected_label_refuses_ambiguous_untagged_tab() {
  ...
  # untagged legacy title "sq-shared" (our pane's owning tab_id=3 is one of
  ...
  zellij_multi_tab_response "$dir" 2 3 sq-shared 9 sq-shared
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_key Squad:7 Escape sq-shared' "$ROOT"
  status=$?
...
test_list_live_scopes_to_own_home_tag() {
  ...
  own_title=$(zellij_expected_scoped_title sq-task1)
  foreign_title=$(zellij_expected_scoped_title sq-task2 "$ROOT" "$other_root")
  # 1: list-tabs --json -> our own home-scoped tab, a DIFFERENT installation's
  # home-scoped tab (same prefix shape, different SQUAD_ROOT hash), and an
  # unrelated non-Squad tab.
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_list_live Squad' "$ROOT" )
  [ "$out" = $'Squad:7\tsq-task1' ] \
    || fail "list_live should list only this home's own tagged tab with its plain sq-<id> label, got '$out'"
...
test_resolve_bare_selector_prefers_scoped_title() {
  ...
  title=$(zellij_expected_scoped_title sq-resolve1)
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_resolve_bare_selector sq-resolve1' "$ROOT" )
  [ "$out" = "Squad:12" ] || fail "resolve_bare_selector should resolve the home-scoped tagged tab, got '$out'"
...
test_resolve_bare_selector_refuses_ambiguous_untagged() {
  ...
  zellij_multi_tab_response "$dir" 1 3 sq-resolve2 9 sq-resolve2
  zellij_multi_tab_response "$dir" 2 3 sq-resolve2 9 sq-resolve2
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_resolve_bare_selector sq-resolve2' "$ROOT" 2>&1 )
  status=$?
...
test_resolve_bare_selector_prefers_later_session_scoped_title_over_legacy() {
  ...
  title=$(zellij_expected_scoped_title sq-resolve3)
  zellij_tab_response "$dir" 1 3 sq-resolve3
  zellij_tab_response "$dir" 2 6 "$title"
  zellij_pane_response "$dir" 3 12 6
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST=$'legacy-session\nscoped-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_resolve_bare_selector sq-resolve3' "$ROOT" )
...
test_resolve_bare_selector_refuses_cross_session_ambiguous_untagged() {
  ...
  zellij_tab_response "$dir" 1 3 sq-resolve4
  zellij_tab_response "$dir" 2 9 sq-resolve4
  zellij_tab_response "$dir" 3 3 sq-resolve4
  zellij_tab_response "$dir" 4 9 sq-resolve4
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST=$'one-session\ntwo-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_resolve_bare_selector sq-resolve4' "$ROOT" 2>&1 )
  status=$?
...
test_session_exists_true_when_listed() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST=$'Squad\nother-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_session_exists Squad' "$ROOT"
...
test_session_exists_false_when_absent() {
  ...
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST=$'other-session' \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_session_exists Squad' "$ROOT" 2>&1 )
  status=$?
...
test_server_ensure_skips_attach_when_already_exists() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_server_ensure Squad' "$ROOT"
...
# --- dispatch wiring (sq-backend.sh) ------------------------------------------

test_dispatch_routes_zellij_backend() {
  fm_backend_validate zellij 2>/dev/null || fail "fm_backend_validate should accept zellij (P3 adds it to SQUAD_BACKEND_KNOWN)"
...
test_dispatch_busy_state_unknown_for_zellij() {
  ...
  [ "$(fm_backend_busy_state zellij 'Squad:5')" = unknown ] \
    || fail "fm_backend_busy_state should report unknown for zellij (no native agent-state primitive; D5: sentry falls back to regex, same as tmux)"
...
test_create_task_refuses_duplicate_label() {
  ...
  title=$(zellij_expected_scoped_title sq-dup1)
  # 1: list-tabs --json -> existing tab already carrying the home-scoped title
  printf '[{"tab_id":2,"name":"%s","active":false}]\n' "$title" > "$dir/responses/1.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_create_task Squad sq-dup1 /tmp/proj' "$ROOT" 2>&1 )
  status=$?
...
test_create_task_creates_and_parses_ids() {
  ...
  title=$(zellij_expected_scoped_title sq-newtask)
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_create_task Squad sq-newtask /tmp/proj' "$ROOT" )
...
test_create_task_restores_previously_active_tab() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_create_task Squad sq-focustest /tmp/proj' "$ROOT" )
...
test_create_task_no_restore_when_new_tab_was_already_active() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_create_task Squad sq-noclient /tmp/proj' "$ROOT" )
...
test_capture_small_reads_use_viewport_and_trim() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_capture Squad:7 2' "$ROOT" )
...
test_capture_large_reads_use_full_scrollback_and_trim() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_capture Squad:7 80' "$ROOT" )
  ...
  pass "fm_backend_zellij_capture: reads above the sentry-size threshold request --full scrollback"
}
...
test_capture_fails_when_pane_absent() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_capture Squad:7 5' "$ROOT" 2>&1 )
  status=$?
...
test_send_key_normalizes_and_targets_pane() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_key Squad:7 Escape' "$ROOT"
...
test_send_literal_uses_paste_separator_for_option_shaped_text() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_literal Squad:7 "--help"' "$ROOT"
...
test_send_text_line_clears_partial_input_when_enter_fails() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" bash -c \
    '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_text_line "Squad:7" "export TRACEPARENT=carrier"' "$ROOT"
  status=$?
...
test_expected_label_allows_matching_task_tab() {
  ...
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 sq-label
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_key Squad:7 Escape sq-label' "$ROOT"
  expect_code 0 $? "send_key should succeed when the pane belongs to the expected sq-id tab"
  ...
  pass "fm_backend_zellij_target_ready: expected labels allow matching sq-<id> tabs"
}
...
test_expected_label_rejects_reused_pane_id() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    ...
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_key Squad:7 Escape sq-label' "$ROOT"
  status=$?
  [ "$status" -ne 0 ] || fail "send_key should reject a pane whose tab name does not match the expected sq-id label"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''list-tabs'$'\x1f''--json' \
...
test_current_path_probes_with_marker_and_ignores_prompt_paths() {
  ...
  # once a subshell (e.g. fob get) takes over, so current_path actively
  ...
  printf '%s\n' 'scratch-e2e-project HEAD' \
    ...
    '__SQUAD_ZELLIJ_CWD_BEGIN__' \
    '/home/fixture/.fob/fake-' \
    'worktree' \
    '__SQUAD_ZELLIJ_CWD_END__' \
    '/home/fixture/.fob/fake-worktree ❯' \
    > "$dir/responses/7.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_current_path Squad:7' "$ROOT" )
  [ "$out" = "/home/fixture/.fob/fake-worktree" ] || fail "current_path should read only the marked cwd line, got '$out'"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''paste' \
    "current_path did not verify the pane before the cwd probe paste"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''dump-screen' \
    "current_path did not verify the pane before capture"
  assert_contains "$(cat "$dir/log")" "__SQUAD_ZELLIJ_CWD_BEGIN__" "current_path did not send the cwd begin marker via paste"
...
test_current_path_ignores_tilde_prefixed_banner_lines() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_current_path Squad:7' "$ROOT" )
  [ "$out" = "/home/fixture/.fob/real-worktree" ] || fail "current_path should skip the ~-prefixed banner line and read the marked cwd output, got '$out'"
...
test_kill_resolves_tab_and_closes_by_id() {
  ...
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_kill Squad:7' "$ROOT"
...
test_kill_closes_recorded_tab_when_pane_already_gone() {
  ...
  printf '[]\n' > "$dir/responses/1.out"
  printf '[{"tab_id":3,"name":"sq-zghost"}]\n' > "$dir/responses/2.out"
  fb=$(make_zellij_fakebin "$dir")
  PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_kill Squad:7 3 sq-zghost' "$ROOT"
...
test_teardown_passes_recorded_tab_id_to_zellij_kill() {
  ...
  printf '[{"tab_id":3,"name":"sq-zghost"}]\n' > "$dir/responses/2.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_STATE_OVERRIDE="$state" SQUAD_DATA_OVERRIDE="$data" SQUAD_CONFIG_OVERRIDE="$config" \
    SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    "$ROOT/bin/sq-teardown.sh" zghost 2>&1 )
  status=$?
  expect_code 0 "$status" "sq-teardown should succeed for a zellij recon whose worktree is already gone: $out"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''list-tabs'$'\x1f''--json' \
    "sq-teardown did not verify the recorded zellij_tab_id against the task label"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-tab-by-id'$'\x1f''3' \
    "sq-teardown did not pass a verified recorded zellij_tab_id through to kill"
  assert_not_contains "$(cat "$dir/log")" $'\x1f''close-pane' \
    "sq-teardown should close the recorded tab id instead of falling back to close-pane"
  pass "sq-teardown.sh: passes recorded zellij_tab_id with the expected task label"
}

test_forced_XO_teardown_kills_zellij_children_with_child_home_tag() {
  local dir state data config home project fb out status child_title
  dir="$TMP_ROOT/teardown-zellij-XO-child"; state="$dir/state"; data="$dir/data"; config="$dir/config"; home="$dir/XO-home"; project="$dir/project"
  ...
  fm_write_meta "$home/state/childz.meta" \
    ...
    "project=$project" \
    "kind=recon"
  child_title=$(zellij_expected_scoped_title sq-childz "$home" "$home")
  ...
  out=$( PATH="$fb:$PATH" SQUAD_STATE_OVERRIDE="$state" SQUAD_DATA_OVERRIDE="$data" SQUAD_CONFIG_OVERRIDE="$config" \
    ...
    SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    "$ROOT/bin/sq-teardown.sh" smz --force 2>&1 )
  status=$?
  expect_code 0 "$status" "sq-teardown should force-retire an XO with a zellij child: $out"
  assert_contains "$(cat "$dir/log")" $'\x1f''close-tab-by-id'$'\x1f''4' \
    "forced XO teardown did not close a child zellij tab scoped to the child home"
  pass "sq-teardown.sh: force cleanup kills zellij children using the child home tag"
}
...
test_send_text_submit_detects_landed_send() {
  ...
  printf '%s' $'❯ hello commander' > "$dir/responses/4.out"
  printf '%s' $'hello commander\n❯' > "$dir/responses/8.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_text_submit Squad:7 "hello commander" 3 0.01 0.01' "$ROOT" )
  ...
  assert_contains "$(cat "$dir/log")" $'\x1f''paste'$'\x1f''--pane-id'$'\x1f''7'$'\x1f''--'$'\x1f''hello commander' "send_text_submit did not type the literal text first"
...
test_send_text_submit_detects_swallowed_enter() {
  ...
  printf '%s' $'❯ hello commander' > "$dir/responses/4.out"
  printf '%s' $'❯ hello commander' > "$dir/responses/8.out"
  printf '%s' $'❯ hello commander' > "$dir/responses/12.out"
  fb=$(make_zellij_fakebin "$dir")
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_text_submit Squad:7 "hello commander" 2 0.01 0.01' "$ROOT" )
...
test_send_text_submit_send_failed_when_session_absent() {
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" \
    SQUAD_ZELLIJ_SESSION_LIST="" \
    bash -c '. "$0/bin/backends/zellij.sh"; fm_backend_zellij_send_text_submit Squad:7 "x" 2 0.01 0.01' "$ROOT" )
...
# --- sq-*.sh script routing via explicit backend-tagged meta ------------------
...
test_scripts_route_explicit_target_through_meta_backend() {
  ...
  fm_write_meta "$state/zellij-stale.meta" "window=Squad:7" "backend=zellij"
...
SH
  ...
  out=$( PATH="$fb:$PATH" SQUAD_ROOT_OVERRIDE="$neutral" SQUAD_STATE_OVERRIDE="$state" \
    SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    "$ROOT/bin/sq-peek.sh" Squad:7 5 2>/dev/null )
  [ "$out" = "captured zellij pane" ] || fail "sq-peek did not capture through zellij for an explicit metadata-matched target, got '$out'"
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''dump-screen' \
    "sq-peek did not verify the pane before capture"
  assert_contains "$(cat "$dir/log")" $'\x1f''dump-screen'$'\x1f''--pane-id'$'\x1f''7' \
  ...
  zellij_assert_call_order "$dir/log" $'\x1f''list-panes'$'\x1f''--json' $'\x1f''send-keys' \
    "sq-send did not verify the pane before send-key"
  assert_contains "$(cat "$dir/log")" $'\x1f''send-keys'$'\x1f''--pane-id'$'\x1f''7'$'\x1f''Esc' \
    "sq-send did not route the explicit metadata-matched target through zellij send-key"

  pass "sq-peek/sq-send: explicit metadata-matched targets use the recorded zellij backend"
}

test_scripts_verify_label_for_fm_targets() {
  local dir state fb neutral out
  dir="$TMP_ROOT/script-sq-target-label"; state="$dir/state"; mkdir -p "$state" "$dir/responses"
  neutral="$dir/neutral-root"; mkdir -p "$neutral"
  fm_write_meta "$state/zlabel.meta" "window=Squad:7" "backend=zellij"
  touch "$state/.last-sentry-beat"
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 sq-zlabel
  printf 'captured through sq-id\n' > "$dir/responses/3.out"
  fb=$(make_zellij_fakebin "$dir")

  out=$( PATH="$fb:$PATH" SQUAD_ROOT_OVERRIDE="$neutral" SQUAD_STATE_OVERRIDE="$state" \
    SQUAD_ZELLIJ_LOG="$dir/log" SQUAD_ZELLIJ_RESPONSES="$dir/responses" SQUAD_ZELLIJ_SESSION_LIST="Squad" \
    "$ROOT/bin/sq-peek.sh" sq-zlabel 5 2>/dev/null )
  [ "$out" = "captured through sq-id" ] || fail "sq-peek did not capture through zellij for an sq-id target with a matching tab label, got '$out'"
  zellij_assert_call_order "$dir/log" $'\x1f''list-tabs'$'\x1f''--json' $'\x1f''dump-screen' \
    "sq-peek did not verify the sq-id tab label before capture"

  pass "sq-peek: sq-id zellij targets verify the owning tab label before capture"
}

test_scripts_reject_fm_target_label_mismatch() {
  local dir state fb neutral status
  dir="$TMP_ROOT/script-sq-target-label-mismatch"; state="$dir/state"; mkdir -p "$state" "$dir/responses"
  neutral="$dir/neutral-root"; mkdir -p "$neutral"
  fm_write_meta "$state/zreuse.meta" "window=Squad:7" "backend=zellij"
  touch "$state/.last-sentry-beat"
  zellij_pane_response "$dir" 1 7 3
  zellij_tab_response "$dir" 2 3 not-the-task
  fb=$(make_zellij_fakebin "$dir")
  ...
  status=$?
  [ "$status" -ne 0 ] || fail "sq-send --key should reject an sq-id zellij target whose pane belongs to a differently named tab"
  assert_not_contains "$(cat "$dir/log")" $'\x1f''send-keys' \
    "sq-send should not send a key after sq-id label verification fails"
  pass "sq-send: sq-id zellij targets reject pane ids whose tab label no longer matches"
}
