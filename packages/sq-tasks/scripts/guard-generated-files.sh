  ...
  git diff --name-status --find-renames --find-copies --find-copies-harder "${base_sha}...${head_sha}" |
    awk '
      function generated(path) {
        return path == "packages/sq-tasks/CHANGELOG.md" || path == "packages/sq-tasks/.release-please-manifest.json"
      }
      $1 == "A" && $2 == "packages/sq-tasks/.release-please-manifest.json" { next }
      $1 == "A" && generated($2) {
        printf " %s", $2
      }
      $1 ~ /^[MDT]/ && generated($2) {
...
if [ -n "$violated" ]; then
  {
    ...
    echo
    echo "packages/sq-tasks/CHANGELOG.md and packages/sq-tasks/.release-please-manifest.json are auto-generated by"
