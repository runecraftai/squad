...
state/               volatile runtime signals; gitignored
  ...
  <id>.exec          per-attempt execution state sidecar: exec_state=, exec_attempt=, exec_retry_count=, exec_started_at=, exec_last_activity=, exec_previous_state=, exec_error=, exec_next_retry_at=, exec_max_retries=, exec_workflow_version=, exec_workspace=, exec_backend=, exec_harness=; missing for legacy tasks (unclaimed); managed by bin/sq-exec-state.sh; state machine: unclaimed -> claimed -> running -> retry_queued -> released
