...
Squad can propagate a W3C [`traceparent`](https://www.w3.org/TR/trace-context/) to every agent it spawns so an external observer can identify each task as exactly one trace and correlate everything that task runs under that one identity.
The trace boundary is the task: a persistent XO is routing infrastructure with its own agent identity, never a shared trace root for the unrelated tasks routed through it.
The capability is default-off, source-owned, vendor-neutral, and deliberately narrow.
This document is the rationale and current-behavior guide; `docs/configuration.md` owns the configuration schema, `bin/sq-trace-context-lib.sh`'s header owns the exact mechanics, and [`verification/trace-context.md`](verification/trace-context.md) records the repeatable test evidence.

## Why this is a source change at all

Squad's durable operational artifacts already let a downstream observer derive logical task identity and lifecycle.
...
When enabled, for each spawn Squad resolves one W3C `traceparent` carrier for the task - minted as a fresh root on the task's first spawn and reused verbatim from the meta on relaunch - and:
...
`TRACEPARENT` as an environment variable is a Squad convention carrying a W3C-formatted value: W3C Trace Context standardizes the `traceparent` HTTP header, not an env var, and OpenTelemetry SDKs do not read it from the environment automatically, so a downstream observer must explicitly read this env value or the `traceparent=` meta field.
...
- **Root** - a spawn whose task meta holds no valid recorded carrier mints a fresh trace id, a fresh span id, and sampled flags (`01`).
  ...
  The spawning process's own ambient `TRACEPARENT` is never adopted: that value is the agent identity the process itself received at its launch, and a persistent XO keeps it for its whole life while unrelated requests are routed through it.
  Adopting it would chain every routed task into one ever-growing trace per XO; instead each routed task roots its own trace.
...
- **Cost and privacy consequence.**
  `01` records a sampling *decision*, and a conforming downstream parent-based sampler will honor it - but it does not by itself guarantee that any collector stores a span, and Squad emits no spans of its own; it only sets the flag on the carrier.
  ...
  An operator who wants unsampled roots or head-sampling owns that downstream or via a later, explicitly-scoped option; Squad does not embed a sampler.

## Safety

- **Default-off.**
  With no `config/trace-context` and no `SQUAD_TRACE_CONTEXT`, a fresh spawn or actual relaunch injects nothing and writes no `traceparent=` line, so the generated meta and the launch environment are unchanged.
...
- **What is and is not exposed.**
  A Squad-*minted* root uses a random id and reads no prompt, path, task prose, credential, or arbitrary environment key, so Squad never *originates* sensitive data in the carrier.
  Every carrier Squad injects is either such a mint or the same task's previously recorded carrier reused verbatim; ambient `TRACEPARENT` is never read, so no caller-controlled bytes enter a new carrier.
...
- **Fail-independent.**
  ...
  If the pre-launch carrier export fails, Squad omits the `traceparent=` metadata claim and still launches the task.
  If the backend reports that failed trace input could not be cleared, Squad refuses to append the launch command rather than risk launching with an unknown partial carrier.
  If recording the carrier fails after export, Squad unsets `TRACEPARENT` in the launch command and still launches the task, so the child never receives an identity absent from its metadata.
- **Metadata-only.**
