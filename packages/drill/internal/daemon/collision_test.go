...
func TestSplitCommandLineTokens(t *testing.T) {
	tests := []struct {
		...
		{name: "plain", in: `/x/drill daemon run --root /a/b`, want: []string{"/x/drill", "daemon", "run", "--root", "/a/b"}},
		{name: "equals form", in: `/x/drill daemon run --root=/a/b`, want: []string{"/x/drill", "daemon", "run", "--root=/a/b"}},
		{name: "quoted spaced root", in: `"/x drill" daemon run --root "/a b/c"`, want: []string{"/x drill", "daemon", "run", "--root", "/a b/c"}},
...
func TestLooksLikeDaemonRunCommand(t *testing.T) {
	tests := []struct {
		...
		{`/x/drill daemon run --root /a`, true},
		{`/x/drill run daemon --root /a`, true}, // order-insensitive
		{`/x/drill`, false},                     // detached: bare exe
		{`postgres --root /a daemon`, false},    // missing "run"
		{`drill daemon --root /a`, false},       // missing "run"
		{``, false},
	}
	for _, tc := range tests {
...
func TestParseDaemonProcessOutput(t *testing.T) {
	output := strings.Join([]string{
		"1234 /home/u/drill daemon run --root /home/u/.drill",
		"  5678 \t/usr/bin/drill daemon run --root=/tmp/link",
		`9 "/x drill" daemon run --root "/a b/c"`,
		`10 /x/drill`,                   // detached daemon, must be skipped
...
func TestListDaemonProcesses_FindsSpawnedDaemonRunHelper(t *testing.T) {
	...
	root := t.TempDir()
	// TestMain in helpers_test.go parks processes with DRILL_DAEMON_HELPER_PROCESS
	...
	helper.Env = append(os.Environ(), "DRILL_DAEMON_HELPER_PROCESS=block")
