#!/usr/bin/env bash
# tests/sq-trace-context-spawn.test.sh - spawn-path integration regressions for
...
. "$ROOT/bin/sq-trace-context-lib.sh"

SPAWN="$ROOT/bin/sq-spawn.sh"
TMP_ROOT=$(fm_test_tmproot sq-trace-context-spawn)
...
case "${1:-}" in
  ...
  list-windows)
    [ -z "${SQUAD_FAKE_DUPLICATE_WINDOW:-}" ] || printf '%s\n' "$SQUAD_FAKE_DUPLICATE_WINDOW"
    exit 0
    ;;
  has-session|new-session|new-window|kill-window) exit 0 ;;
  send-keys)
    if [ "${SQUAD_FAKE_TRACEPARENT_SEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${SQUAD_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
    ...
    if [ "${SQUAD_FAKE_TRACE_METADATA_APPEND_FAIL:-0}" = 1 ]; then
      for a in "$@"; do
        case "$a" in
          "export TRACEPARENT="*)
            chmod a-w "$SQUAD_FAKE_META_PATH"
            ;;
        esac
      done
    fi
...
# Hermetic against an ambient SQUAD_TRACE_CONTEXT: `env -u` unsets it so enablement
...
run_spawn() {
  ...
  env -u SQUAD_TRACE_CONTEXT \
    ...
    SQUAD_FAKE_TRACEPARENT_SEND_FAIL="${SQUAD_FAKE_TRACEPARENT_SEND_FAIL:-0}" \
    SQUAD_FAKE_TRACEPARENT_SEND_UNSAFE="${SQUAD_FAKE_TRACEPARENT_SEND_UNSAFE:-0}" \
...
start_trace_session() {
  ...
  printf '%s\n' "$$" > "$home/state/.lock"
  if [ -n "$tc" ]; then
    SQUAD_TRACE_CONTEXT="$tc" fm_trace_context_session_start \
      "$home/config" "$home/state/.trace-context-effective"
  else
    (
      unset SQUAD_TRACE_CONTEXT
...
# Two-level primary -> XO -> worker regression for the SQUAD_TRACE_CONTEXT
# effective override. Drives bin/sq-spawn.sh TWICE against real homes and a real
# worktree: first the primary launches an XO (capturing the exact env the
# primary injects into it), then that XO launches its OWN worker with
# exactly that inherited env, reading the XO home's own inherited config.
...
run_two_level() {
  ...
  # Seed the XO home so validate_Squad_home_for_spawn accepts it.
  ...
  env SQUAD_TRACE_CONTEXT="$penv" \
    ...
    SQUAD_FAKE_LAUNCH_LOG="$smlog" PATH="$smfake:$PATH" \
    "$SPAWN" "$sm_id" "$sm" --xo >/dev/null 2>&1 || true

  # Extract the EXACT env the primary put on the XO: the normalized
  # SQUAD_TRACE_CONTEXT in the launch prefix, and the TRACEPARENT carrier (if any).
  TL_ENV_TC=$(grep -o 'SQUAD_TRACE_CONTEXT=[a-z]*' "$smlog" | head -1 | cut -d= -f2)
  TL_CARRIER=$(injected_traceparent "$smlog" | head -1)

  # Spawn 2: the XO launches its own worker with exactly that inherited
  # env, reading the XO home's own (inherited) config.
  worker_id="w-$name"
  wproj="$base/wproj"
  wwt="$base/wwt"
...
test_disabled_writes_and_injects_neither() {
  ...
  read_case_record "$rec"
  # No config/trace-context and no SQUAD_TRACE_CONTEXT: default-off.
...
test_failed_delivery_omits_metadata_and_still_launches() {
  ...
  out=$(SQUAD_FAKE_TRACEPARENT_SEND_FAIL=1 \
...
test_unsafe_delivery_refuses_to_append_launch() {
  ...
  out=$(SQUAD_FAKE_TRACEPARENT_SEND_UNSAFE=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_failed_metadata_append_unsets_carrier_and_still_launches() {
  ...
  out=$(SQUAD_FAKE_TRACE_METADATA_APPEND_FAIL=1 \
    run_spawn "$HOME_DIR" "$WT_DIR" "$FAKEBIN_DIR" "$LAUNCH_LOG" "$CASE_ID" "$PROJ_DIR")
  status=$?
...
test_duplicate_XO_spawn_does_not_converge_trace_context() {
  local base prim sm id log fake out status
  base="$TMP_ROOT/duplicate-XO"
  prim="$base/primary"
  sm="$base/sm"
  ...
  out=$(env -u SQUAD_TRACE_CONTEXT \
    ...
    SQUAD_PROJECTS_OVERRIDE="$prim/projects" SQUAD_CONFIG_OVERRIDE="$prim/config" \
    SQUAD_SPAWN_NO_GUARD=1 CLAUDECODE=1 TMUX="fake,1,0" \
    SQUAD_FAKE_DUPLICATE_WINDOW="sq-$id" SQUAD_FAKE_LAUNCH_LOG="$log" \
    PATH="$fake:$PATH" "$SPAWN" "$id" "$sm" --xo 2>&1)
  status=$?

  [ "$status" -ne 0 ] || fail "duplicate XO spawn should be refused"
  assert_contains "$out" "already exists" "duplicate XO spawn should report the existing endpoint"
  [ ! -e "$sm/config/trace-context" ] \
    || fail "duplicate preflight must not converge trace-context into the XO home"
  pass "duplicate XO preflight leaves trace-context unchanged"
}
...
test_session_start_freezes_env_override_and_ignores_later_edits() {
  ...
  meta="$HOME_DIR/state/$CASE_ID.meta"
  ! grep -q '^traceparent=' "$meta" || fail "session-frozen off must ignore a later SQUAD_TRACE_CONTEXT=on"
  ...
  fm_trace_context_valid "$(meta_traceparent "$meta")" \
    || fail "session-frozen on must ignore a later SQUAD_TRACE_CONTEXT=off"
...
# XO's own worker. Before the effective-override fix, the XO saw
...
# fresh trace rather than adopt the XO's carrier from the environment.
test_XO_env_on_file_absent_keeps_nested_worker_enabled() {
  run_two_level enable absent on
  [ "$TL_ENV_TC" = on ] || fail "the primary must deliver SQUAD_TRACE_CONTEXT=on to the XO (got '$TL_ENV_TC')"
  fm_trace_context_valid "$TL_CARRIER" || fail "an enabled primary must mint a carrier for the XO (got '$TL_CARRIER')"
  ...
  [ "${TL_CARRIER:3:32}" != "${TL_WORKER_TP:3:32}" ] \
    || fail "the nested worker must root its own trace, not adopt the XO's trace id (XO='${TL_CARRIER:3:32}' worker='${TL_WORKER_TP:3:32}')"
...
# override while the config file is PRESENT (so it is copied into the XO
# home). The override must still disable the XO's own worker, or
# SQUAD_TRACE_CONTEXT=off is not a real kill switch. Before the fix the copied file
# re-enabled the nested worker.
test_XO_env_off_file_present_keeps_nested_worker_disabled() {
  run_two_level disable present off
  [ "$TL_ENV_TC" = off ] || fail "the primary must deliver SQUAD_TRACE_CONTEXT=off to the XO (got '$TL_ENV_TC')"
  [ -z "$TL_CARRIER" ] || fail "a disabled primary must inject no carrier into the XO (got '$TL_CARRIER')"
  [ "$TL_SM_FILE" = present ] \
    || fail "the disable case must exercise a copied config/trace-context in the XO home (got '$TL_SM_FILE')"
  [ -z "$TL_WORKER_TP" ] \
    || fail "env-off must keep the nested worker disabled even with the file present (got '$TL_WORKER_TP')"
  pass "two-level: env-off/file-present keeps the nested worker disabled even though the config file was copied into the XO home"
}

# The trace boundary is each routed task, not the routing agent: a persistent
# XO exports one TRACEPARENT into its process environment at its own
...
# distinct traces, and neither may adopt the XO's own trace id, while
# each task still keeps one stable identity across its own relaunch.
test_two_routed_tasks_through_one_XO_root_distinct_traces() {
  ...
  # The XO's own launch-time carrier: exported once into its pane shell
  ...
  [ "${tp_a:3:32}" != "${sm_tp:3:32}" ] \
    || fail "routed task A must not adopt the persistent XO's trace id (got '$tp_a')"
  [ "${tp_b:3:32}" != "${sm_tp:3:32}" ] \
    || fail "routed task B must not adopt the persistent XO's trace id (got '$tp_b')"
  [ "${tp_a:3:32}" != "${tp_b:3:32}" ] \
  ...
  pass "two unrelated routed tasks through one persistent XO root distinct traces, adopt nothing from its environment, and keep per-task identity across relaunch"
}

# Single-frozen-decision guarantee: for an XO spawn the recorded/injected
# carrier and the delivered SQUAD_TRACE_CONTEXT snapshot are always derived from ONE
...
# (SQUAD_TRACE_CONTEXT unset), which is exactly where the two-read correction matters
# because the environment override is empty and only the config file decides.
test_XO_carrier_and_snapshot_share_one_decision() {
  run_two_level fileon present ""
  [ "$TL_ENV_TC" = on ] || fail "a file-enabled XO must snapshot SQUAD_TRACE_CONTEXT=on (got '$TL_ENV_TC')"
  fm_trace_context_valid "$TL_CARRIER" \
    || fail "a file-enabled XO's carrier must be present and valid, consistent with the on snapshot (got '$TL_CARRIER')"

  run_two_level fileoff absent ""
  [ "$TL_ENV_TC" = off ] || fail "a file-disabled XO must snapshot SQUAD_TRACE_CONTEXT=off (got '$TL_ENV_TC')"
  [ -z "$TL_CARRIER" ] \
    || fail "a file-disabled XO must inject no carrier, consistent with the off snapshot (got '$TL_CARRIER')"
  pass "XO carrier and SQUAD_TRACE_CONTEXT snapshot always agree, both derived from one frozen decision (file-decided path)"
}
...
echo "# all sq-trace-context-spawn tests passed"
