#!/usr/bin/env bash
# tests/sq-stand-to-daemon-lifecycle-e2e.test.sh - the sentry + supervise-daemon
# lifecycle, end to end, over one shared state root and a shimmed tmux:
#
#   routine status -> self-handled, queued
#   terminal status written while the sentry is DOWN -> caught on restart (catch-up)
#   drain queued records -> exactly ONE commander-relevant digest is buffered
...
# sq-sentry.sh runs plus the daemon's own functions. The commander-relevant
# status-phrase matrix and the lock-primitive races stay as focused units
# (sq-daemon.test.sh, sq-sentry-lock.test.sh) - an e2e cannot deterministically
...
TMP_ROOT=$(fm_test_tmproot sq-stand-to-daemon-e2e)

# Run the daemon-managed sentry once: under the supervise-daemon (away mode) the
# sentry is one-shot - it exits with a single reason line on EVERY wake and the
# daemon does the triage. This e2e exercises exactly that path, so it runs with
# state/.afk present (which the daemon owns) to keep the sentry one-shot; the
# always-on standalone triage is covered by sq-sentry-triage.test.sh. fakebin
...
test_routine_then_terminal_after_restart() {
  ...
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  [ ! -s "$state/.subsuper-escalations" ] || fail "routine status was escalated by the daemon"

  # The sentry is now DOWN (one-shot exit). A terminal status lands while it is
  # down; the next sentry run must catch it up (losslessness across restart).
  printf 'done: PR https://example.test/pr/900\n' >> "$status_file"
  : > "$out"
  run_sentry_once "$state" "$fakebin" "$out" || fail "restarted sentry did not exit for the terminal signal"
  grep -F "signal: $status_file" "$out" >/dev/null || fail "terminal signal written while sentry down was not caught on restart"

  # Drain and route the terminal: exactly ONE digest is buffered.
  : > "$drain_out"
  SQUAD_STATE_OVERRIDE="$state" "$DRAIN" > "$drain_out" || fail "drain after terminal signal failed"
  SQUAD_STATE_OVERRIDE="$state" handle_wake "signal: $status_file" "$state"
  [ -s "$state/.subsuper-escalations" ] || fail "commander-relevant terminal status was not buffered"
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_PANE_ALIVE=1 SQUAD_FAKE_TMUX_SENT="$sent" \
    SQUAD_FAKE_TMUX_CAPTURE="$dir/pane.txt" SQUAD_ESCALATE_BATCH_SECS=0 escalate_flush "$state" \
  ...
  pass "lifecycle: routine self-handles, terminal survives a sentry restart, buffers once, no dup, injects once"
}
...
test_stale_pane_transient_persistent_resume() {
  ...
  # live window (the legacy tmux list fallback was removed in this PR).
  ...
  case "$(SQUAD_STATE_OVERRIDE="$state" classify_stale "$win" "$state")" in
    self\|*) : ;;
  ...
  PATH="$fakebin:$PATH" SQUAD_FAKE_TMUX_WINDOW="$win" SQUAD_FAKE_TMUX_CAPTURE="$dir/pane.txt" \
  ...
  # Resumed: a fresh transient marker but the operator is provably working again ->
  # housekeeping clears the marker without escalating. The proof is the operator's
  # own semantic busy-state record (bin/sq-busy-lib.sh), not rendered pane text.
  stale_marker_record "$win" "$state"
  echo $(( $(date +%s) - 500 )) > "$state/.subsuper-stale-$key"
  printf 'Working...\n' > "$dir/pane.txt"
  fm_write_meta "$state/stale-w2.meta" "window=$win" "worktree=$dir/wt" "kind=strike" "harness=pi"
  resumed_gen=$("$ROOT/bin/sq-busy-event.sh" arm "$state" stale-w2)
  "$ROOT/bin/sq-busy-event.sh" apply "$state" stale-w2 busy --gen "$resumed_gen" \
