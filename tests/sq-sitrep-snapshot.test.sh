#!/usr/bin/env bash
# Behavior tests for the sitrep projection wrapper over sq-unit-snapshot.sh.
...
TMP_ROOT=$(fm_test_tmproot sq-sitrep)
...
case "${1:-}" in
  ...
  capture-pane)
    case "$*" in
      *sq-domain-alpha*) printf 'stale terminal summary: Phase 7 started\n> \n' ;;
      *) printf 'all quiet\n> \n' ;;
    esac
    ;;
esac
...
cat <<'JSON'
[{"number":9,"title":"Ship the thing","url":"https://github.com/kunchenguid/Squad/pull/9","headRefName":"fm/ship-task","reviewDecision":"APPROVED","mergeable":"MERGEABLE","statusCheckRollup":[{"conclusion":"SUCCESS","status":"COMPLETED"}]}]
JSON
SH
  cat > "$fb/gh-axi" <<'SH'
#!/usr/bin/env bash
...
record_claude_state() {  # <state-dir> <id> <busy|idle>
  ...
  "$ROOT/bin/sq-busy-event.sh" apply "$state" "$id" "$semantic_state" --gen "$gen" \
...
# Standard fixture: a strike task with a recorded PR, a recon task with a report, a
# XO with a MASKED open decision (needs-decision then a later unrelated
# done), and a backlog with a superseded queued item.
write_fixture() {  # <home>
  local home=$1 mate
  mate=$(fixture_mate_home "$home")
  mkdir -p "$home/projects/ship-wt" "$home/data/recon-x" "$mate/data" "$mate/state" "$mate/config" "$mate/projects" "$mate/bin"
  printf '# Squad fixture\n' > "$mate/AGENTS.md"
  printf 'mate\n' > "$mate/.sq-xo-home"
  printf -- '- mate - fixture domain (home: %s; scope: fixture work; projects: Squad; added 2026-07-11)\n' \
    "$mate" > "$home/data/XOs.md"
  cat > "$home/data/backlog.md" <<EOF
## In flight
- [ ] ship-task - Ship the thing (repo: Squad) (kind: ship) (since 2026-07-11)
- [ ] recon-x - Investigate the thing data/recon-x/report.md (repo: Squad) (kind: recon) (since 2026-07-11)

## Queued
- [ ] live-gate - Real queued work blocked-by: ship-task (repo: Squad) (kind: ship)
- [ ] dead-gate - Old conditional work (repo: Squad) (kind: recon)
  NOT REQUIRED - superseded 2026-07-11; kept as reference only.

## Done
- [x] done-a - Landed thing https://github.com/kunchenguid/Squad/pull/7 (repo: Squad) (kind: ship) (merged 2026-07-10)
EOF
  ...
  # XO home, never the main backlog, so landed-work views only see it via the
  # bounded cross-home Done roll-up.
  cat > "$mate/data/backlog.md" <<'EOF'
## In flight
- [ ] mate - Decide subscription order (repo: Squad) (kind: ship) (since 2026-07-11)

## Queued
- [ ] mate-decision-race - Choose subscription order (repo: Squad) (kind: commander) (hold: commander choice pending) (hold-kind: commander)

## Done
- [x] mate-landed - XO-managed fix https://github.com/kunchenguid/Squad/pull/50 (repo: Squad) (kind: ship) (merged 2026-07-11)
EOF
...
write_domain_alpha_fixture() {  # <parent-home> <XO-home>
  ...
  printf -- '- domain-alpha - sample rollout (home: %s; scope: sample rollout and legal release; projects: sample; added 2026-07-13)\n' \
    "$mate" > "$home/data/XOs.md"
  fm_write_XO_meta "$home/state/domain-alpha.meta" "$mate" "Squad:sq-domain-alpha" sample
...
test_domain_alpha_stale_parent_event_does_not_become_current_work() {
  ...
  printf '%s' "$canonical" | jq -e '
    .XO_current.records[] | select(.id == "domain-alpha")
...
SH
  ...
  ' >/dev/null || fail "GNU stat fixture corrupted the authoritative XO summary: $canonical"
...
test_parent_activity_evidence_is_bounded_and_disclosed() {
  ...
  json=$(SQUAD_SNAPSHOT_PARENT_ACTIVITY_LINES=4 SQUAD_SNAPSHOT_PARENT_ACTIVITY_BYTES=4096 \
    SQUAD_SNAPSHOT_PARENT_ACTIVITIES=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    .omitted | any(.surface == "XO parent activity evidence truncated for 1 record(s)")
  ' >/dev/null || fail "sitrep did not disclose bounded parent activity evidence: $json"
...
EOF
  ...
  printf '%s' "$json" | jq -e '
    (.XOs | any(.[]; .id == "domain-alpha" and .state != "commander_decision"
  ...
  pass "Sitrep excludes a status-only child decision"
}

test_structured_child_decision_reaches_commanders_call() {
...
## Queued
- [ ] phase8-decision-release - Choose sample release (repo: sample) (kind: commander) (hold: commander release choice pending) (hold-kind: commander)
  ...
  printf '%s' "$json" | jq -e '
    (.XOs | any(.[]; .id == "domain-alpha" and .state == "commander_decision"))
      and (.decisions_open | any(.[]; .id == "domain-alpha/phase8-decision-release"
        and .key == "phase8-decision-release" and .verb == "commander-hold"))
      and (.in_flight | any(.[]; .id == "domain-alpha") | not)
  ' >/dev/null || fail "structured child commander hold did not reach Commander Call: $json"
  pass "a structured child commander hold reaches Commander's Call"
}

make_valid_XO_home() {  # <id> <home>
  local id=$1 home=$2
...
append_XO_registry() {  # <parent> <id> <home>
  printf -- '- %s - fixture domain (home: %s; scope: fixture; projects: sample; added 2026-07-13)\n' \
    "$2" "$3" >> "$1/data/XOs.md"
}

append_landed_row() {  # <XO-home> <id> <title> <date>
  printf -- '- [x] %s - %s (repo: Squad) (kind: ship) (merged %s)\n' \
    "$2" "$3" "$4" >> "$1/data/backlog.md"
}

make_landed_XO() {  # <parent> <id>
  local parent=$1 id=$2 mate
  mate="$TMP_ROOT/$(basename "$parent")-$id-home"
  make_valid_XO_home "$id" "$mate"
  append_XO_registry "$parent" "$id" "$mate"
  printf '%s\n' "$mate"
}

write_parent_XO_event() {  # <parent> <id> <home> <note>
  fm_write_XO_meta "$1/state/$2.meta" "$3" "Squad:sq-$2" sample
  printf 'working [key=%s]: %s\n' "$2" "$4" > "$1/state/$2.status"
}

test_bad_XO_homes_never_revive_parent_work() {
  ...
  append_XO_registry "$home" missing "$missing"

  make_valid_XO_home invalid "$invalid"
  printf 'someone-else\n' > "$invalid/.sq-xo-home"
  append_XO_registry "$home" invalid "$invalid"
  write_parent_XO_event "$home" invalid "$invalid" "old invalid work"

  make_valid_XO_home unreadable "$unreadable"
  chmod 000 "$unreadable/data"
  append_XO_registry "$home" unreadable "$unreadable"
  write_parent_XO_event "$home" unreadable "$unreadable" "old unreadable work"

  make_valid_XO_home malformed "$malformed"
  printf '## In flight\nthis current row is not structured\n' > "$malformed/data/backlog.md"
  append_XO_registry "$home" malformed "$malformed"
  write_parent_XO_event "$home" malformed "$malformed" "old malformed work"
  ...
  printf '%s' "$json" | jq -e '
    (.XOs | length) == 5
      and all(.XOs[]; .state == "unknown")
      and (.in_flight | map(.id) | all(. != "invalid" and . != "unreadable" and . != "malformed" and . != "timedout"))
      and (.XOs | any(.[]; .id == "missing" and .provenance == "unknown"
        and .freshness == "unknown" and (.reason | contains("invalid home"))))
      and ([.XOs[] | select(.id != "missing")]
        | all(.provenance == "parent-event-fallback" and .freshness == "historical-event"))
      and (.XOs | any(.[]; .id == "invalid" and (.reason | contains("marked for"))))
      and (.XOs | any(.[]; .id == "unreadable" and (.reason | test("invalid home|unreadable"))))
      and (.XOs | any(.[]; .id == "malformed" and (.reason | contains("unstructured current backlog row"))))
      and (.XOs | any(.[]; .id == "timedout" and (.reason | contains("timed out"))))
...
test_oversized_XO_summary_stays_strict_unknown() {
  local home mate fakebin json i
  home=$(make_home oversized-home)
  mate="$TMP_ROOT/oversized-XO-home"
  make_valid_XO_home oversized "$mate"
  append_XO_registry "$home" oversized "$mate"
  fm_write_XO_meta "$home/state/oversized.meta" "$mate" "Squad:sq-oversized" sample
...
EOF
  ...
  fakebin=$(make_fakebin "$home")
  json=$(SQUAD_SNAPSHOT_XO_MAX_BYTES=512 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.XOs | any(.id == "oversized" and .state == "unknown"
  ...
  pass "an oversized XO summary retains the strict empty unknown fallback"
}

test_XO_and_child_bounds_are_disclosed() {
  local home fakebin id mate child json expanded canonical i
  home=$(make_home XO-bounds)
  : > "$home/data/XOs.md"
  for id in a b c; do
  ...
  printf '%s' "$canonical" | jq -e '
    .XO_current.total_registered == 3
      and .XO_current.shown == 2
      and .XO_current.truncated == 1
      and (.XO_current.records[] | select(.id == "a")
        | .counts.active_children == 3 and (.active_children | length) == 2
          and (.omitted | any(.surface == "active_children" and .count == 1)))
      and (.XO_current.records | any(.id == "b" and .current.state == "no_active_work"))
  ' >/dev/null || fail "canonical XO or child bounds were not enforced: $canonical"
  json=$(SQUAD_SNAPSHOT_XOS=2 SQUAD_SNAPSHOT_XO_CHILDREN=2 SQUAD_BEARINGS_XOS=1 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.XOs | length) == 1
      and ([.omitted[].surface] | any(test("XOs showing 1 of 2")))
      and ([.omitted[].surface] | any(test("registered XOs omitted by snapshot bound: 1")))
  ' >/dev/null || fail "sitrep XO bound was not disclosed: $json"
  expanded=$(SQUAD_SNAPSHOT_XO_CHILDREN=2 SQUAD_BEARINGS_XOS=1 \
    run "$home" "$fakebin" --json --all-XOs)
  printf '%s' "$expanded" | jq -e '
    (.XOs | length) == 3
      and ([.omitted[].surface] | any(test("XOs showing|registered XOs omitted")) | not)
  ' >/dev/null || fail "--all-XOs did not expand the canonical and sitrep bounds: $expanded"
  pass "XO and per-home child counts are bounded, disclosed, and explicitly expandable"
}
...
test_parent_decision_is_untrusted_contradiction_only() {
  ...
  make_valid_XO_home authority "$mate"
  append_XO_registry "$home" authority "$mate"
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.XOs | any(.[]; .id == "authority" and .state == "no_active_work" and .contradiction == true))
      and (.decisions_open | any(.[]; .id == "authority") | not)
  ' >/dev/null || fail "sitrep promoted a stale parent decision: $json"
...
test_parent_evidence_reconciles_by_verb_and_key() {
  ...
  make_valid_XO_home hold "$hold"
  make_valid_XO_home blocked "$blocked"
  make_valid_XO_home decision "$decision"
  append_XO_registry "$home" hold "$hold"
  append_XO_registry "$home" blocked "$blocked"
  append_XO_registry "$home" decision "$decision"
  fm_write_XO_meta "$home/state/hold.meta" "$hold" "Squad:sq-hold" sample
  fm_write_XO_meta "$home/state/blocked.meta" "$blocked" "Squad:sq-blocked" sample
  fm_write_XO_meta "$home/state/decision.meta" "$decision" "Squad:sq-decision" sample
...
EOF
  ...
  printf '%s' "$canonical" | jq -e '
    (.XO_current.records[] | select(.id == "hold")
      ...
      and (.XO_current.records[] | select(.id == "blocked")
        | .current.state == "externally_held"
      ...
      and (.XO_current.records[] | select(.id == "decision")
        | .current.state == "commander_decision"
          and .contradiction == true
          and .terminal_evidence.captured == false
          and (.parent_event.reconciliation.activities
...
test_nonprogressing_child_states_are_explicit() {
  ...
  make_valid_XO_home states "$mate"
  append_XO_registry "$home" states "$mate"
...
test_registry_unavailability_and_bounds_are_explicit() {
  ...
  make_valid_XO_home hidden "$mate"
  ...
  printf '%s' "$canonical" | jq -e '
    .XO_current.registry.complete == false
      and (.XO_current.records[] | select(.id == "hidden")
        | .registered == null
  ...
  printf '%s' "$json" | jq -e '
    (.XOs | any(.[]; .id == "(registry)" and .state == "unknown"
      and .provenance == "registered-table" and .freshness == "unavailable"))
      and (.omitted | any(.surface | contains("XO registry unavailable")))
  ' >/dev/null || fail "unreadable registry disappeared from sitrep: $json"
  ...
  json=$(SQUAD_SNAPSHOT_REGISTRY_RECORDS=2 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    .omitted | any(.surface == "XO registry records omitted by bounded read")
  ' >/dev/null || fail "sitrep omitted registry truncation disclosure: $json"
  ...
  printf '%s' "$canonical" | jq -e '
    .XO_current.registry.complete == false
      and (.XO_current.records[] | select(.id == "z-hidden")
        | .registered == null
          and (.current.reason | contains("registration is unknown")))
...
test_default_is_bounded_and_local_only() {
  ...
  printf '%s' "$json" | jq -e '.schema == "sq-sitrep.v1"' >/dev/null || fail "json schema wrong"
...
test_open_decision_surfaces_end_to_end() {
  ...
  printf '%s' "$json" | jq -e '
    .decisions_open | any(.[]; .id == "mate/mate-decision-race"
      and .key == "mate-decision-race" and .verb == "commander-hold")
  ' >/dev/null || fail "an authoritative commander hold must surface in decisions_open: $json"
  pass "an authoritative commander hold surfaces end-to-end"
}
...
write_large_fixture() {  # <home> <count>
  ...
  while [ "$i" -le "$count" ]; do
    ...
    printf -- '- [ ] decision-%s - Decision %s (repo: repo-%s) (kind: commander) (hold: commander choice pending) (hold-kind: commander)\n' "$i" "$i" "$i" >> "$home/data/backlog.md"
...
test_section_caps_and_expansion_flags() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(SQUAD_BEARINGS_IN_FLIGHT=2 SQUAD_BEARINGS_DECISIONS=2 SQUAD_BEARINGS_GATES=2 \
    SQUAD_BEARINGS_REPORTS=2 SQUAD_BEARINGS_RECORDED_PRS=2 SQUAD_BEARINGS_UNHEALTHY=2 \
    run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
  ...
  expanded=$(SQUAD_BEARINGS_IN_FLIGHT=2 SQUAD_BEARINGS_DECISIONS=2 SQUAD_BEARINGS_GATES=2 \
    SQUAD_BEARINGS_REPORTS=2 SQUAD_BEARINGS_RECORDED_PRS=2 SQUAD_BEARINGS_UNHEALTHY=2 \
  ...
  pass "all unit-sized sections are capped with counted opt-in expansion"
}
...
test_pr_repository_cap_and_expansion() {
  ...
  fakebin=$(make_fakebin "$home"); : > "$home/net.log"
  json=$(SQUAD_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --json)
  ...
  : > "$home/net.log"
  expanded=$(SQUAD_BEARINGS_PR_REPOS=2 run "$home" "$fakebin" --include-prs --all-pr-repos --json)
...
test_per_repository_pr_cap_is_disclosed() {
  ...
  fakebin=$(make_fakebin "$home")
  json=$(SQUAD_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs --json)
  toon=$(SQUAD_BEARINGS_PR_LIMIT=2 FAKE_GH_MANY=1 run "$home" "$fakebin" --include-prs)
  printf '%s' "$json" | jq -e '
    (.candidate_prs | length) == 2
    and (.prs | test("2 shown, at least 3 open; capped in 1 repo"))
    and ([.omitted[] | select(.surface == "candidate_prs showing 2 of at least 3; capped in 1 repo(s)" and .reveal == "raise SQUAD_BEARINGS_PR_LIMIT")] | length) == 1
...
test_completed_scout_report_not_pending() {
  ...
  printf '# Lavish 103\nThe open question is whether to adopt approach A or B; this needs a commander decision.\n' > "$home/data/lavish-103/report.md"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
# Recently Landed must include merges an XO managed. Those completion records
# live in the XO home's OWN backlog, not the main one, so the projection must
# roll them up. Local, deterministic, no GitHub call.
test_landed_includes_XO_home_merges() {
  local home fakebin json
...
test_landed_default_balances_dominant_and_sparse_homes() {
  ...
  : > "$home/data/XOs.md"
  printf '## Done\n' > "$home/data/backlog.md"
  dominant=$(make_landed_XO "$home" dominant)
  sparse_a=$(make_landed_XO "$home" sparse-a)
  sparse_b=$(make_landed_XO "$home" sparse-b)
  sparse_c=$(make_landed_XO "$home" sparse-c)
  i=1
  while [ "$i" -le 12 ]; do
...
test_landed_default_refills_capacity_after_sparse_homes_exhaust() {
  ...
  printf '## Done\n' > "$home/data/backlog.md"
  dominant=$(make_landed_XO "$home" dominant)
  sparse=$(make_landed_XO "$home" sparse)
  i=1
  while [ "$i" -le 5 ]; do
    append_landed_row "$dominant" "$(printf 'dominant-landed-%02d' "$i")" \
...
test_landed_default_preserves_internal_order_for_ties() {
  ...
  : > "$home/data/XOs.md"
  printf '## Done\n' > "$home/data/backlog.md"
  tie_a=$(make_landed_XO "$home" tie-a)
  tie_b=$(make_landed_XO "$home" tie-b)
...
test_all_landed_keeps_complete_global_order() {
  ...
  : > "$home/data/XOs.md"
  printf '## Done\n' > "$home/data/backlog.md"
  alpha=$(make_landed_XO "$home" alpha)
  beta=$(make_landed_XO "$home" beta)
  ...
  fakebin=$(make_fakebin "$home")
  json=$(SQUAD_BEARINGS_LANDED=1 run "$home" "$fakebin" --json --all-landed)
  actual=$(printf '%s' "$json" | jq -r '.landed[] | "\(.owner)/\(.id)"')
...
test_landed_bounded_and_disclosed() {
  ...
  i=1
  while [ "$i" -le 12 ]; do
    printf -- '- [x] mate-landed-%02d - XO fix %02d (repo: Squad) (kind: ship) (merged 2026-06-%02d)\n' \
      "$i" "$i" "$((13 - i))" >> "$mate/data/backlog.md"
    i=$((i + 1))
  done
  fakebin=$(make_fakebin "$home")
  json=$(SQUAD_BEARINGS_LANDED=20 run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
...
# Sitrep projects authoritative structured state rather than inventing return
...
# during an away return, while this test pins Sitrep' own projection boundary.
test_live_blocker_is_not_charted_queue_work() {
  local home fakebin json
  home=$(make_home live-blocker); write_fixture "$home"
  printf 'blocked [key=synthetic-dependency]: Squad can refresh the synthetic token\n' > "$home/state/ship-task.status"
  ...
  pass "Sitrep keeps a live blocker in structured live state and never converts it to Charted Next queue work"
}

# Commander's Call is populated only from the durable keyed open-decision set. The
...
# cannot leak into Commander's Call. The standard fixture has exactly one genuine open
# decision (the XO's structured commander hold).
test_commanders_call_anti_leak() {
  ...
  jq -n -e --argjson sitrep "$json" --argjson canonical "$canonical" '
    ([$sitrep.decisions_open[].id] == ["mate/mate-decision-race"])
      ...
      and ([$sitrep.decisions_open[].id] | index("ship-task") | not)
      and ([$sitrep.decisions_open[].id] | index("recon-x") | not)
      and ([$sitrep.decisions_open[].id] | index("external-wait") | not)
      and ([$sitrep.decisions_open[].id] | index("done-a") | not)
      and ([$sitrep.decisions_open[].id] | index("mate-landed") | not)
      and ([$sitrep.decisions_open[].id] | index("live-gate") | not)
      and ([$sitrep.decisions_open[].id] | index("dead-gate") | not)
  ' >/dev/null || fail "only genuine open decisions may feed Commander's Call: $json"
  pass "action-free items (working/done/queued/landed) do not leak into Commander's Call"
}
...
test_main_orphan_in_flight_is_disclosed_not_invented() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] only-orphan - Structured in flight without meta (repo: Squad) (kind: ship) (since 2026-07-11)
...
## In flight
this current row is not structured
- [ ] structured-ship - Visible structured sibling (repo: Squad) (kind: ship) (since 2026-07-11)

## Queued
another free-form note without checkbox
- [ ] structured-queued - Structured queued (repo: Squad) (kind: ship)

## Done
EOF
  fm_write_meta "$home/state/structured-ship.meta" \
    "window=Squad:sq-structured-ship" \
...
test_main_orphan_counterfactual_meta_clears_inventory_warning() {
  ...
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] orphan-ship - Gains meta in counterfactual (repo: Squad) (kind: ship) (since 2026-07-11)
- [ ] visible-ship - Already live (repo: Squad) (kind: ship) (since 2026-07-11)

## Queued
- [ ] queued-ship - Ordinary queue (repo: Squad) (kind: ship)
...
test_mixed_XO_roles_partial_state_and_commander_readiness() {
  ...
  make_valid_XO_home hibit "$hibit"
  make_valid_XO_home wheel "$wheel"
  make_valid_XO_home sshhip "$sshhip"
  make_valid_XO_home home-assistant "$ha"
  append_XO_registry "$home" hibit "$hibit"
  append_XO_registry "$home" wheel "$wheel"
  append_XO_registry "$home" sshhip "$sshhip"
  append_XO_registry "$home" home-assistant "$ha"
...
## Queued
- [ ] reviewer-decision - Choose reviewer remediation (repo: sshhip) (kind: commander) (hold: choose reviewer remediation A or B) (hold-kind: commander)
...
- [ ] commander-run - Run commander canary blocked-by: prep blocked-by: security (repo: home-assistant) (kind: commander) (hold: commander runs canary) (hold-kind: commander)
...
EOF
  ...
  printf '%s' "$canonical" | jq -e '
    (.XO_current.records[] | select(.id == "hibit")
      ...
      and (.XO_landed.partial | length) == 1
      and (.XO_landed.partial[0] | endswith("/mixed-sshhip-home"))
      and (.XO_landed.unreadable | length) == 0
      and (.XO_current.records[] | select(.id == "home-assistant")
        | .current.state == "active_child_work"
          and .decisions_open == []
          and [.active_children[].id] == ["prep"]
          and (.queued[] | select(.id == "commander-run")
  ...
  printf '%s' "$json" | jq -e '
    ([.in_flight[].id] | sort) == ["hibit", "home-assistant", "wheel"]
      ...
      and (.decisions_open | any(.id == "home-assistant/commander-run") | not)
      and (.gates | any(.id == "production-observation" and .owner == "wheel"
        and .reason == "documented no live worker"))
      and (.gates | any(.id == "commander-run" and .owner == "home-assistant"
        and .blocked_by == "prep,security"))
      and (.XOs | any(.id == "sshhip" and .state == "unknown"
...
- [ ] unreadable-child - Submit App Store build (repo: sshhip) (kind: ship)' \
  ...
  fm_write_meta "$wheel/state/production-observation.meta" \
    "window=Squad:sq-production-observation" "worktree=$wheel/projects/worker" "project=wheelhouse" \
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.XOs | any(.id == "sshhip" and .state == "commander_decision" and .reason == "-"))
...
EOF
  ...
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "home-assistant/commander-run") | not)
      and (.gates | any(.id == "commander-run" and .owner == "home-assistant" and .blocked_by == "security"))
...
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.decisions_open[] | select(.id == "home-assistant/commander-run")] | length) == 1
      and (.gates | any(.id == "commander-run" and .owner == "home-assistant") | not)
  ' >/dev/null || fail "zero Home Assistant blockers did not yield exactly one commander action: $json"
  ...
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "home-assistant/commander-run") | not)
      and (.gates | any(.id == "commander-run" and .owner == "home-assistant" and .blocked_by == "missing"))
  ...
  pass "mixed XO roles, partial state, and commander readiness project independently"
}

test_main_commander_readiness_matches_XO_projection() {
  local home fakebin json
  home=$(make_home main-commander-readiness)
  : > "$home/data/XOs.md"
  mkdir -p "$home/projects/prep" "$home/projects/observation"
  cat > "$home/data/backlog.md" <<'EOF'
## In flight
- [ ] observation - Held observation (repo: Squad) (kind: recon) (hold: watch production) (hold-kind: external)
- [ ] prep - Prepare canary (repo: Squad) (kind: ship)

## Queued
- [ ] review - Security review (repo: Squad) (kind: ship)
- [ ] commander-run - Run commander canary blocked-by: prep blocked-by: review (repo: Squad) (kind: commander) (hold: commander runs canary) (hold-kind: commander)
...
EOF
  ...
  printf '%s' "$json" | jq -e '
    (.in_flight | any(.id == "prep"))
      ...
      and (.decisions_open | any(.id == "commander-run") | not)
      and (.gates | any(.id == "commander-run" and .blocked_by == "prep,review"))
  ' >/dev/null || fail "main blocked commander action or held-child projection was wrong: $json"
...
## Done
- [x] prep - Prepare canary (repo: Squad) (kind: ship) (done 2026-07-22)
EOF
  rm "$home/state/prep.meta" "$home/state/prep.status" \
    "$home/state/observation.meta" "$home/state/observation.status"
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    (.decisions_open | any(.id == "commander-run") | not)
      and (.gates | any(.id == "commander-run" and .blocked_by == "review"))
  ' >/dev/null || fail "main one-blocker commander action became premature: $json"
...
## Done
- [x] prep - Prepare canary (repo: Squad) (kind: ship) (done 2026-07-22)
- [x] review - Security review (repo: Squad) (kind: ship) (done 2026-07-22)
EOF
  json=$(run "$home" "$fakebin" --json)
  printf '%s' "$json" | jq -e '
    ([.decisions_open[] | select(.id == "commander-run")] | length) == 1
      and (.gates | any(.id == "commander-run") | not)
  ' >/dev/null || fail "main zero-blocker commander action was not projected exactly once: $json"
  pass "main and XO commander actionability use the same blocker readiness"
}
