#!/usr/bin/env bash
# tests/sq-trace-context-lib.test.sh - unit tests for the native, default-off
# W3C trace-context library (bin/sq-trace-context-lib.sh) plus structural checks
# that bin/sq-spawn.sh wires it in at the pre-launch injection seam and that the
# capability is inherited into XO homes. Pure functions, no backend and
...
# --- minted-root shape --------------------------------------------------------
# A Squad-MINTED root is exactly the fixed 55-char W3C form with random ids
# and no free-form field where Squad could originate a prompt, path, or
...
pass "a minted root is the fixed 55-char W3C form (hex and hyphens only), so Squad originates no free-form content in the carrier"

# --- enablement precedence ---------------------------------------------------

WORK=$(fm_test_tmproot sq-trace-context)
...
SQUAD_TRACE_CONTEXT=off fm_trace_context_enabled "$CFG_ON" && fail "SQUAD_TRACE_CONTEXT=off must override a present file"
SQUAD_TRACE_CONTEXT=on fm_trace_context_enabled "$CFG_OFF" || fail "SQUAD_TRACE_CONTEXT=on must override an absent file"
SQUAD_TRACE_CONTEXT=1 fm_trace_context_enabled "$CFG_OFF" || fail "SQUAD_TRACE_CONTEXT=1 must enable"
SQUAD_TRACE_CONTEXT=maybe fm_trace_context_enabled "$CFG_ON" && fail "a non-truthy SQUAD_TRACE_CONTEXT must disable"
SQUAD_TRACE_CONTEXT='' fm_trace_context_enabled "$CFG_ON" || fail "empty SQUAD_TRACE_CONTEXT must defer to a present file (enabled)"
SQUAD_TRACE_CONTEXT='' fm_trace_context_enabled "$CFG_OFF" && fail "empty SQUAD_TRACE_CONTEXT must defer to an absent file (disabled)"
pass "enablement is default-off; SQUAD_TRACE_CONTEXT overrides with truthy/other precedence, and unset or empty defers to config/trace-context"

SESSION_DIR="$WORK/session-state"
SESSION_STATE="$SESSION_DIR/.trace-context-effective"
mkdir -p "$SESSION_DIR"
printf '101\n' > "$SESSION_DIR/.lock"
SQUAD_TRACE_CONTEXT=off fm_trace_context_session_start "$CFG_ON" "$SESSION_STATE"
...
SQUAD_TRACE_CONTEXT=on fm_trace_context_session_start "$CFG_OFF" "$SESSION_STATE"
...
out=$(SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_OFF" "$NOMETA")
...
# --- XO home-session boundary ---------------------------------------
# sq-spawn launches every XO with the primary session's non-empty frozen
# SQUAD_TRACE_CONTEXT decision. The XO resolves it at its own session start.
# Its own launch-time TRACEPARENT stays in its process environment for its whole
# life, but that is the XO's agent identity, never a parent: every task
# it spawns must root a fresh trace, or unrelated routed tasks would accumulate
# into one ever-growing trace per XO.
PRIMARY_TP='00-abcabcabcabcabcabcabcabcabcabcab-1212121212121212-01'
saved_tp=${TRACEPARENT-__unset__}
unset TRACEPARENT
frozen_off=$(SQUAD_TRACE_CONTEXT=off fm_trace_context_resolve "$CFG_ON" "$WORK/sm-frozen-off.meta")
[ -z "$frozen_off" ] || fail "an XO launched off must stay disabled even after the config file appears: $frozen_off"
frozen_on=$(SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_OFF" "$WORK/sm-frozen-on.meta")
fm_trace_context_valid "$frozen_on" || fail "an XO launched on must stay enabled even while the config file is absent: $frozen_on"
[ "${frozen_on:3:32}" != "${PRIMARY_TP:3:32}" ] || fail "an enabled XO without an ambient carrier must start a new root"
routed_a=$(TRACEPARENT="$PRIMARY_TP" SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$WORK/sm-routed-a.meta")
routed_b=$(TRACEPARENT="$PRIMARY_TP" SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$WORK/sm-routed-b.meta")
...
pass "XO home-session state stays off or on despite later file state; ambient TRACEPARENT is never adopted, so each routed task roots its own trace"
...
out=$(TRACEPARENT='00-ffffffffffffffffffffffffffffffff-1111111111111111-01' \
  SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$REC_META")
...
out=$(SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$CORRUPT_META")
...
out=$(SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$NOMETA")
...
ef_res=$(SQUAD_TRACE_CONTEXT=on fm_trace_context_resolve "$CFG_ON" "$NOMETA"); ef_res_rc=$?
...
assert_no_grep 'sleep' "$ROOT/bin/sq-trace-context-lib.sh" "trace-context lib must not sleep on the spawn path"
assert_no_grep 'timeout' "$ROOT/bin/sq-trace-context-lib.sh" "trace-context lib must not depend on an external timeout"
assert_no_grep 'command:' "$ROOT/bin/sq-trace-context-lib.sh" "trace-context lib must not run an arbitrary command provider"
...
LIB_CODE=$(sed 's/#.*$//' "$ROOT/bin/sq-trace-context-lib.sh")
for tok in harness backend tmux herdr zellij orca cmux claude codex opencode grok kind ship recon XO ; do
  case "$LIB_CODE" in
...
# --- structural wiring in bin/sq-spawn.sh ------------------------------------
...
assert_grep 'sq-trace-context-lib.sh' "$SPAWN" "sq-spawn.sh must source the trace-context lib"
assert_grep 'SPAWN_TRACEPARENT=' "$SPAWN" "sq-spawn.sh must assign the resolved carrier"
assert_grep 'fm_trace_context_resolve' "$SPAWN" "sq-spawn.sh must resolve the carrier through the lib entry point"
...
assert_grep 'if spawn_send_text_line "$T" "export TRACEPARENT=$SPAWN_TRACEPARENT"; then' "$SPAWN" \
  "sq-spawn.sh must condition metadata publication on successful carrier delivery"
# shellcheck disable=SC2016 # Dollar signs are literal source text in this fixed-string assertion.
assert_grep 'echo "traceparent=$SPAWN_TRACEPARENT" >> "$STATE/$ID.meta"' "$SPAWN" \
  "sq-spawn.sh must record the delivered carrier in metadata"
assert_grep 'export TRACEPARENT=' "$SPAWN" "sq-spawn.sh must inject the W3C TRACEPARENT env var"
pass "sq-spawn.sh sources the lib and records one shared SPAWN_TRACEPARENT only after successful injection"
...
# --- XO inheritance wires the nested chain ---------------------------

# shellcheck source=/dev/null
. "$ROOT/bin/sq-config-inherit-lib.sh"
case " $SQUAD_INHERITABLE_CONFIG " in
  *" trace-context "*) : ;;
  *) fail "config/trace-context must be in SQUAD_INHERITABLE_CONFIG so XO homes stay traced" ;;
esac
pass "config/trace-context is inherited into XO homes, keeping the nested chain enabled end to end"

echo "# sq-trace-context-lib.test.sh: all assertions passed"
