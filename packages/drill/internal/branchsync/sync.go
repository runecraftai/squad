...
// drill ref. It never updates an ordinary remote-tracking ref.
func (s *Service) Refresh(ctx context.Context) State {
	...
	if err != nil {
		...
		state.NextAction = &NextAction{Code: "retry", Command: "drill sync --check"}
		return state
	}
	...
	privateRef := "refs/drill/sync/" + run.ID
...
//     private anchor ref refs/drill/recover/<runID> locally without gate
...
// run_pipeline as the next step. `drill rerun` remains the alternative
...
func (s *Service) Recover(ctx context.Context, keepLocal bool) State {
	...
	default:
		...
		state.Relation = RelationDiverged
		blocked := blockedPlan(state, StatePipelineOwned, "blocked_recover_diverged", fmt.Sprintf("the local branch and the preserved pipeline head have diverged; the preserved commits are anchored at %s - reconcile manually and re-run the recovery, run `drill rerun` to resume validating the preserved head, or use --keep-local to keep the current head; no files or refs were changed", anchorRef))
...
func (s *Service) recoverKeepLocal(ctx context.Context, run *db.Run, state State, gateHead string) State {
	...
	if gateHead != state.Local.Head {
		...
		stagingRef := "refs/drill/custody-return/" + run.ID
...
func (s *Service) recoverAdoptPreserved(ctx context.Context, run *db.Run, state State, preserved string) State {
	...
	// refs/drill/recover-local/<run>, custody is never stamped, and the
...
func recoverAnchorRef(runID string) string {
	return "refs/drill/recover/" + runID
}
...
func recoverLocalAnchorRef(runID string) string {
	return "refs/drill/recover-local/" + runID
}
...
func (s *Service) inspect(ctx context.Context) (State, *db.Run, bool) {
	...
	if run.PushActive || pushStepRunning(s.DB, run.ID) {
		...
		state.NextAction = &NextAction{Code: "continue_active_run", Command: "drill axi status"}
...
func (s *Service) classifyRelation(ctx context.Context, state *State, pushed, base string, live bool) {
	...
	if objectExists(ctx, s.workDir(), pushed) {
		...
		case isAncestor(ctx, s.workDir(), pushed, state.Local.Head):
			...
			state.NextAction = &NextAction{Code: "run_pipeline", Command: `drill axi run --intent "<what the user set out to accomplish>"`}
			return
		default:
			if equivalentDivergence(ctx, s.workDir(), state.Local.Head, pushed, base) {
				...
				state.NextAction = &NextAction{Code: "sync", Command: "drill axi sync"}
				state.Error = ""
				return
			}
			state.State = StateDiverged
		...
		state.NextAction = &NextAction{Code: "check_sync", Command: "drill sync --check"}
		return
	}
	if live {
...
func syncAnchorRef(runID string) string {
	return "refs/drill/sync-anchor/" + runID
}
...
func (s *Service) classifyPipelineOwned(ctx context.Context, state *State, run *db.Run, activeMessage string) {
	...
	if terminalRunStatus(run.Status) {
		...
		state.NextAction = &NextAction{Code: "recover_custody", Command: "drill axi sync --recover"}
		return
	}
