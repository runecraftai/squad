...
# the one shared ANSI-aware owner (fm_composer_strip_ghost, bin/sq-composer-lib.sh,
...
#   4. The human/LLM-facing capture path (sq-peek.sh) stays PLAIN - no escape codes
#      ever reach Squad's context.
set -u

# shellcheck source=tests/lib.sh
. "$(dirname "${BASH_SOURCE[0]}")/lib.sh"

LIB="$ROOT/bin/sq-tmux-lib.sh"
PEEK="$ROOT/bin/sq-peek.sh"

# shellcheck source=/dev/null
. "$LIB"

TMP_ROOT=$(fm_test_tmproot sq-ghost-tests)
...
# from SQUAD_FAKE_CY. The fake deliberately returns the complete fixture for every
...
case "${1:-}" in
  ...
  capture-pane)
    ...
    f="${SQUAD_FAKE_STYLED:-/dev/null}"
    if [ -n "${SQUAD_FAKE_ROW:-}" ] \
       && [ "$start" = "${SQUAD_FAKE_CY:-0}" ] \
       && [ "$end" = "${SQUAD_FAKE_CY:-0}" ]; then
      f=$SQUAD_FAKE_ROW
    fi
    if [ "$has_e" = 1 ]; then
      cat "$f" 2>/dev/null
...
# narrowest margin in the unit - roughly a fifth of grok's real-input margin.
...
test_strip_ghost_keeps_muse_composer_colors() {
  ...
  out=$(printf '\033[38;2;90;160;255m\033[48;2;38;56;84m\xe2\x9f\xa9 \033[38;2;204;211;219mhello from Squad\033[39m\n' | fm_tmux_strip_ghost)
  [ "$out" = "$(printf '%s hello from Squad' "$glyph")" ] \
...
test_dim_ghost_only_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=0 \
...
test_dim_ghost_inside_bordered_composer_is_not_pending() {
  ...
  if PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=1 \
     fm_pane_input_pending "fakepane"; then
...
test_normal_text_still_pending() {
  ...
  PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=0 \
...
test_dark_truecolor_bare_shell_prompt_is_unknown() {
  ...
  for prompt in '$' 'user@host $'; do
    ...
    out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=0 \
...
╰────────────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=2 \
    fm_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
...
╰────────────────────────────────────────────╯
EOF
  out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=4 \
    fm_tmux_composer_state "fakepane")
  [ "$out" = pending ] \
...
test_bordered_busy_signatures_are_pending() {
  ...
  for signature in 'Working...' 'Ctrl+c:cancel'; do
    printf '╭────────────────────╮\n│ %-18s │\n╰────────────────────╯\n' "$signature" > "$capture"
    out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=1 \
      fm_tmux_composer_state "fakepane")
    [ "$out" = pending ] \
...
test_asymmetric_composer_edges_are_unknown() {
  ...
  for row in '│ > text' 'text │' '│ > text ┃' '──────' '+----' 'text +'; do
    printf '%s\n' "$row" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=0 \
      fm_tmux_composer_state "fakepane")
    [ "$out" = unknown ] \
  ...
  for fixture in ghost idle malformed-top; do
    case "$fixture" in
      ...
      idle)
        printf '╭────────────╮\n│ idle hint │\n╰────────────╯\n' > "$capture"
        out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=1 \
          SQUAD_COMPOSER_IDLE_RE='^idle hint$' fm_tmux_composer_state "fakepane")
        ;;
      malformed-top)
        printf '╭────x───────╮\n│            │\n╰────────────╯\n' > "$capture"
        out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=1 \
...
test_wide_composer_text_is_pending() {
  ...
  for text in '修复登录问题' 'fix the bug 🔧'; do
    printf '╭────────────────────╮\n│ > %s │\n╰────────────────────╯\n' "$text" > "$capture"
    out=$(PATH="$fb:$PATH" LC_ALL=C SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY=1 \
      fm_tmux_composer_state "fakepane")
    [ "$out" = pending-unproven ] \
...
test_fallback_capture_race_with_edge_is_unknown() {
  ...
  printf '│ > │\n' > "$row_capture"
  out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_ROW="$row_capture" SQUAD_FAKE_CY=0 \
    fm_tmux_composer_state "fakepane")
  [ "$out" = unknown ] \
...
test_legitimate_empty_routes_remain_empty() {
  ...
  for fixture in bordered double-bordered agent-prompt blank; do
    ...
    out=$(PATH="$fb:$PATH" SQUAD_FAKE_STYLED="$capture" SQUAD_FAKE_CY="$cursor" \
      fm_tmux_composer_state "fakepane")
    [ "$out" = empty ] \
...
# --- sq-peek.sh stays escape-free (LLM-facing path) -------------------------
...
test_peek_output_is_escape_free() {
  ...
  case "$out" in
    ...
    *) fail "sq-peek dropped pane content (expected the ghost text body as plain text)" ;;
  esac
  pass "sq-peek output is escape-free (no raw -e bytes reach Squad context)"
}
