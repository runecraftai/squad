...
const nativeAgentEscapedPipeHelperEnv = "DRILL_AGENT_NATIVE_PIPE_HELPER"
...
func TestNativeAgentCommand_WaitDelayClosesEscapedPipeHolder(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		...
		"DRILL_AGENT_NATIVE_PIPE_READY="+readyFile,
		"DRILL_AGENT_NATIVE_PIPE_PID="+pidFile,
	)
...
func TestNativeAgentEscapedPipeHelper(t *testing.T) {
	...
	case "leader":
		...
		child.Env = append(os.Environ(), nativeAgentEscapedPipeHelperEnv+"=escaped",
			"DRILL_AGENT_NATIVE_PIPE_READY="+os.Getenv("DRILL_AGENT_NATIVE_PIPE_READY"))
		child.Stdout = os.Stdout
		child.Stderr = os.Stderr
		if err := child.Start(); err != nil {
			os.Exit(2)
		}
		_ = os.WriteFile(os.Getenv("DRILL_AGENT_NATIVE_PIPE_PID"), []byte(strconv.Itoa(child.Process.Pid)), 0o644)
		if !waitForNativeAgentPipeHelperReady(os.Getenv("DRILL_AGENT_NATIVE_PIPE_READY"), 5*time.Second) {
			os.Exit(3)
		}
	...
	case "escaped":
		_, _ = syscall.Setsid()
		_ = os.WriteFile(os.Getenv("DRILL_AGENT_NATIVE_PIPE_READY"), []byte("ready"), 0o644)
...
func TestClaudeAgent_LargeStdinReapsGrandchildHoldingPipesOnLeaderExit(t *testing.T) {
	...
	readyFile := filepath.Join(dir, "ready")
	pidFile := filepath.Join(dir, "grandchild.pid")
	t.Setenv("DRILL_CLAUDE_STDIN_HELPER", "spawn-grandchild")
	t.Setenv("DRILL_CLAUDE_STDIN_READY", readyFile)
	t.Setenv("DRILL_CLAUDE_STDIN_PID", pidFile)
