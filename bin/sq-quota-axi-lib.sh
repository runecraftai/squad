...
#
# This file is the single owner of that version number. bin/sq-bootstrap.sh
...
fm_quota_axi_compatible() {
  ...
  # The floor is compared from SQUAD_QUOTA_AXI_MIN so bumping it needs one edit.
  IFS='.' read -r min_major min_minor min_patch min_extra <<< "$SQUAD_QUOTA_AXI_MIN"
