...
Verified on 2026-07-31 on macOS (Darwin 25.5.0) with `lavish-axi` 0.1.45 installed (the pre-rebrand name of the vendored `sq-report` CLI).

## The published sq-report poll interface the adapter wraps
...
## Why an ended sq-report review is terminal
...
| one `Send & End`, one result | an armed sq-report source driven against a stand-in for the published poll, which delivers the final `session_ended` feedback once and empty ended sessions afterward, polls exactly once, captures exactly one result, publishes one distinct event, and retires itself |
...
| trusted classification boundary | sq-report lifecycle classification reads the leading response envelope, so prompt payload text that resembles a missing-session error cannot override a valid session status |
...
| canonical physical identity | a final-component symlink and its target produce the same sq-report source id |
...
sq-report is the first adapter; adding another requires only a new `bin/sq-procevent-<adapter>.sh`, whose `terminal` command is optional and defaults to keeping the source armed.
