...
1. Run `bin/sq-startup-memory-budget.sh report` before considering a write.
   ...
   The budget is per home: this home's three files against this home's own allowance, never a unit total.
...
2. Read every current memory file completely: `data/commander.md`, `data/commander-shared.md`, and `data/learnings.md`.
   ...
   In an XO home, `data/commander-shared.md` is a read-only primary-owned input: count it, never edit it, and curate only the editable local files.
3. Build one whole-file retention plan before editing.
   Retain, in order: current commander preferences, authority and safety boundaries, and recurring working style; stable home-local operating facts that repeatedly affect future work and are expensive to rediscover; then concise pointers to an existing authoritative report, project document, configuration, or backlog item.
...
3. **Write within the existing boundaries.**
   ...
   - In a primary home, curate shared commander preferences only under the existing primary-authoritative shared-preference contract.
...
- every unresolved exception, including a primary-owned shared-file constraint in an XO home;
...
In a primary home, every `/debrief` cascades to every registered XO after this home's own required pass and knowledge sweep are complete.
In an XO home, `/debrief` curates that home only and never cascades further.
...
It enumerates each registered XO exactly once, reports that home's own budget accounting, and resolves how the sweep reaches it; its header owns the stanza fields, the bound, and the exit codes.
...
- `direct` - curate that local home's editable memory files yourself under the same retention plan, then re-run the cascade to confirm the after totals.
  `data/commander-shared.md` stays a read-only counted input there, exactly as it is in any XO home.
...
- `unavailable` - that home's own accounting did not complete. Report the concrete exception and continue; a slow or unreachable home never blocks this home's `/debrief`.

A newly discovered shared commander preference still routes to the primary's `data/commander-shared.md` under the existing primary-authoritative contract, whichever home found it.

Extend the completion receipt with one entry per XO alongside the primary's own, carrying that home's budget before and after, its per-file actions, its exceptions, and whether that home swept itself or was curated from here.
