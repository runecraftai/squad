...
func TestNonInteractiveEnv_PreservesAmbientEnv(t *testing.T) {
	t.Setenv("DRILL_ENV_PROBE_XYZ", "kept")

	got := resolveEnv(NonInteractiveEnv(""))

	if got["DRILL_ENV_PROBE_XYZ"] != "kept" {
		t.Errorf("ambient env not preserved: DRILL_ENV_PROBE_XYZ = %q, want \"kept\"", got["DRILL_ENV_PROBE_XYZ"])
	}
}
...
func TestNonInteractiveEnvFrom_UsesBaseAndOverridesGitKeys(t *testing.T) {
	...
	if got["DRILL_ENV_PROBE_XYZ"] != "kept" {
		t.Errorf("DRILL_ENV_PROBE_XYZ = %q, want kept", got["DRILL_ENV_PROBE_XYZ"])
	}
	if got["GIT_TERMINAL_PROMPT"] != "0" {
