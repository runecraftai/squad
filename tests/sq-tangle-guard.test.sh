...
#
# Squad is a fob-pooled git repo of itself: linked worktrees and
# XO homes all sit at a detached HEAD on the default branch, while the
# PRIMARY checkout (SQUAD_ROOT) is a normal checkout on a real branch. The "tangle"
# is an operator branching/committing in the primary instead of its own worktree,
...
#            sq-spawn refuses to launch unless the resolved worktree is isolated.
#   GUARD 2 (detection)  - sq-guard and sq-bootstrap alarm when the primary is on
#            a feature branch, and stay silent on the default branch or detached.
# These cases pin: the shared lib's branch classification, the sq-guard banner,
# the sq-bootstrap problem line, the brief assertion ordering, and the sq-spawn
...
. "$ROOT/bin/sq-tangle-lib.sh"

TMP_ROOT=$(fm_test_tmproot sq-tangle-guard)
...
on a feature branch is the tangle|feature|sq/readme-restructure-d3|sq/readme-restructure-d3
detached HEAD on default is healthy (worktrees, XO homes)|detached||
ROWS
...
test_guard_banner() {
  ...
  pass "sq-guard: bordered tangle banner fires only for a feature branch and suppresses repair commands in read-only mode"
}

# --- GUARD 2b: sq-bootstrap problem line ------------------------------------
...
test_bootstrap_line() {
  ...
  git -C "$repo" checkout -q -B sq/tangle-bb2
  out=$(run_bootstrap "$repo" | grep '^TANGLE:' || true)
  assert_contains "$out" "sq/tangle-bb2" "bootstrap did not report the tangled branch"
  ...
  assert_contains "$out" "sq/tangle-bb2" "detect-only bootstrap did not report the tangled branch"
  ...
  pass "sq-bootstrap: TANGLE problem line fires only for a feature branch and suppresses repair commands in detect-only mode"
}

# --- GUARD 1a: brief isolation assertion ------------------------------------

# The generated ship brief must carry the isolation assertion AHEAD of the
# `git checkout -b` step, so the operator verifies its worktree before branching.
test_brief_assertion_precedes_branch() {
  ...
  br=$(grep -n 'git checkout -b sq/' "$brief" | head -1 | cut -d: -f1)
  if [ -z "$iso" ] || [ -z "$br" ]; then
  ...
  pass "sq-brief: ship brief asserts worktree isolation before the branch step"
}

# --- GUARD 1b: sq-spawn isolation abort -------------------------------------

# A fake tmux that reports SQUAD_FAKE_PANE_PATH as the post-`fob get` pane cwd
...
test_spawn_isolation_abort() {
  ...
  pass "sq-spawn: aborts unless the resolved worktree is a genuine, isolated worktree"
}

# --- GUARD 1c: sq-spawn tmux window construction ----------------------------

# The prevention guard also depends on sq-spawn building robust tmux commands
...
#     are disabled so the sq-<id> name survives fob cd'ing into the worktree;
#   - the fob-get send-keys and the worktree wait loop target that stable
#     window id, never the (possibly-renamed) name - a lost name would let
#     display-message fall back to the active client's window and misread Squad's
...
#!/usr/bin/env bash
set -u
[ -n "${SQUAD_TMUX_REC:-}" ] && printf 'tmux %s\n' "$*" >> "$SQUAD_TMUX_REC"
case "$*" in
  *"#{pane_current_path}"*) printf '%s\n' "${SQUAD_FAKE_PANE_PATH:-}"; exit 0 ;;
esac
case "${1:-}" in
...
test_spawn_tmux_window_construction() {
  ...
  assert_grep "new-window -dP -F #{window_id} -t Squad: -n sq-rec-win-gg7" "$rec" \
    "new-window must append at the session (trailing colon) and capture the window id"
  assert_no_grep "new-window -dP -F #{window_id} -t Squad -n" "$rec" \
  ...
  # Bug 2 fix (b): fob-get and the worktree wait loop target the stable id.
  assert_grep "send-keys -t @spawnwid fob get Enter" "$rec" \
    "fob get must be sent to the stable window id"
  assert_grep "display-message -p -t @spawnwid #{pane_current_path}" "$rec" \
    "the worktree wait loop must query the stable window id, not the name"

  pass "sq-spawn: appends windows by session-colon, pins the name, and targets the window id"
}
