...
func newEvidencePublishContext(t *testing.T, branch string) (sctx *pipeline.StepContext, remote string) {
	...
	sctx.Config.Test.Evidence = config.Evidence{StoreInRepo: true, Dir: ".drill/evidence", Branch: "drill/evidence"}
	return sctx, remote
}
...
func TestPublishRunEvidence_LandsOnOrphanBranchAndLinksFromThePRBody(t *testing.T) {
	...
	tip := gitCmd(t, remote, "rev-parse", "refs/heads/drill/evidence")
	tree := gitCmd(t, remote, "ls-tree", "-r", "--name-only", tip)
	for _, want := range []string{
		".drill/evidence/feature/add-login/checkout.png",
		".drill/evidence/feature/add-login/cli-run.txt",
	} {
	...
	wantLink := "https://github.com/example/widgets/blob/" + tip + "/.drill/evidence/feature/add-login/checkout.png"
	...
	if !strings.Contains(md, "https://github.com/example/widgets/blob/"+tip+"/.drill/evidence/feature/add-login/cli-run.txt") {
...
func TestPublishRunEvidence_UsesTheConfiguredBranchName(t *testing.T) {
	...
	if !strings.Contains(tree, ".drill/evidence/feature/add-login/cli-run.txt") {
		t.Errorf("configured evidence branch missing the artifact, has:\n%s", tree)
	}
	if refs := gitCmd(t, remote, "for-each-ref", "--format=%(refname)", "refs/heads/drill"); refs != "" {
