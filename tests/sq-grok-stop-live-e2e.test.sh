...
if [ "${SQUAD_GROK_STOP_LIVE_E2E:-0}" != 1 ]; then
  echo "skip: set SQUAD_GROK_STOP_LIVE_E2E=1 with SQUAD_GROK_NATIVE_BIN and SQUAD_GROK_LEGACY_BIN"
  exit 0
fi
...
NATIVE_BIN=${SQUAD_GROK_NATIVE_BIN:-}
LEGACY_BIN=${SQUAD_GROK_LEGACY_BIN:-}
AUTH=${SQUAD_GROK_AUTH_FILE:-$HOME/.grok/auth.json}
REAL_TMUX=$(command -v tmux || true)
ACTIVE_LAB=

[ -x "$NATIVE_BIN" ] || fail "SQUAD_GROK_NATIVE_BIN must be an exact executable path"
[ -x "$LEGACY_BIN" ] || fail "SQUAD_GROK_LEGACY_BIN must be an exact executable path"
[ -f "$AUTH" ] || fail "SQUAD_GROK_AUTH_FILE must name the already-managed auth artifact"
...
cleanup_exact_cell() {
  ...
  SQUAD_E2E_TMUX_SOCKET_ID="$lab/dedicated.sock" PATH="$lab/bin:$PATH" \
...
run_cell() {  # <native|legacy> <exact-binary>
  ...
  tmp_base=${TMPDIR:-/tmp}
  tmp_base=${tmp_base%/}
  lab=$(mktemp -d "$tmp_base/sq-grok-stop-$kind.XXXXXX") || return 1
  session="$kind-e2e"
  target="sq-$kind"
  ACTIVE_LAB=$lab
  umask 077
...
[ "\${SQUAD_E2E_TMUX_SOCKET_ID:-}" = "\$expected" ] || exit 92
...
#!/usr/bin/env bash
printf 'active=%s' "${GROK_TURNEND_GUARD_ACTIVE:-}" >> "${SQUAD_GROK_E2E_ROOT:?}/resume-invocations.log"
printf ' <%s>' "$@" >> "${SQUAD_GROK_E2E_ROOT:?}/resume-invocations.log"
printf '\n' >> "${SQUAD_GROK_E2E_ROOT:?}/resume-invocations.log"
exec "${SQUAD_GROK_E2E_BIN:?}" "$@"
EOF
  ...
  if [ "$kind" = native ]; then
    cat > "$lab/project/.grok/hooks/00-sq-stop-live-e2e.json" <<EOF
...
EOF
    ...
    cp "$ROOT/.grok/hooks/sq-primary-turnend-guard.json" "$lab/grok-home/hooks/sq-primary-turnend-guard.json"
    rm -f "$lab/project/.grok/hooks/sq-primary-turnend-guard.json"
    cat > "$lab/grok-home/hooks/00-sq-stop-live-e2e.json" <<EOF
{"hooks":{"Stop":[{"hooks":[{"type":"command","command":"bash -lc 'cat >> \"$lab/payloads.jsonl\"'","timeout":30}]}]}}
EOF
...
EOF
  ...
  i=0
  while [ "$i" -lt "${SQUAD_GROK_STOP_LIVE_TIMEOUT:-600}" ] && [ ! -f "$lab/done" ]; do
    sleep 1
    i=$((i + 1))
  done
