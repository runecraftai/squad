...
description: All environment variables recognized by drill.
---
...
- Global config: `$DRILL_HOME/config.yaml`
- Gate repos: `$DRILL_HOME/repos/<id>.git`
- Worktrees: `$DRILL_HOME/worktrees/<repoID>/<runID>/`
- Logs: `$DRILL_HOME/logs/`
- Database: `$DRILL_HOME/state.sqlite`
- Socket / PID / singleton lock: `$DRILL_HOME/socket`, `$DRILL_HOME/daemon.pid`, and `$DRILL_HOME/daemon.lock`
- Managed agent server PID records: `$DRILL_HOME/servers/`
- Managed service names get a short stable suffix derived from `$DRILL_HOME` so multiple installs don't collide.

## `DRILL_DAEMON_CONNECT_TIMEOUT`
...
Used alongside `DRILL_BITBUCKET_API_TOKEN`. See [Provider Integration](/drill/guides/provider-integration/#bitbucket-cloud).

## `DRILL_BITBUCKET_API_TOKEN`
...
See [Provider Integration](/drill/guides/provider-integration/#azure-devops).
...
Fallback GitHub token used by `drill update` when `GITHUB_TOKEN` is unset or empty.
...
When set, drill looks for OpenCode's intent transcript database at `$XDG_DATA_HOME/opencode/opencode.db`.
...
When the upstream hostname carries no `gitlab` marker, drill reads glab's configured hosts from `$GLAB_CONFIG_DIR/config.yml` to decide whether the host is a GitLab instance. It takes precedence over `XDG_CONFIG_HOME`. See [Provider Integration](/drill/guides/provider-integration/#self-hosted-githubgitlab).
...
When the upstream hostname is not `github.com`, drill reads gh's configured hosts from `$GH_CONFIG_DIR/hosts.yml` to decide whether the host is a GitHub Enterprise instance. It takes precedence over `XDG_CONFIG_HOME`. See [Provider Integration](/drill/guides/provider-integration/#self-hosted-githubgitlab).
...
When `GLAB_CONFIG_DIR` is unset, drill looks for glab's configured hosts at `$XDG_CONFIG_HOME/glab-cli/config.yml`, falling back to `~/.config/glab-cli/config.yml` when `XDG_CONFIG_HOME` is unset.
When `GH_CONFIG_DIR` is unset, drill looks for gh's configured hosts at `$XDG_CONFIG_HOME/gh/hosts.yml`, falling back to `~/.config/gh/hosts.yml` when `XDG_CONFIG_HOME` is unset.
...
| ------- | --------------------------- |
| Type    | `URL`                       |
| Default | `https://a.squad.example` |

When set, telemetry sends events to this host's `/api/send` endpoint. If it is unset in a dev build, `drill` also checks a repo-local `.env` file for `DRILL_UMAMI_HOST`. If no runtime value is found, it falls back to any host embedded at build time and then the default self-hosted Umami instance.

## `DRILL_UMAMI_WEBSITE_ID`
...
When set, telemetry uses this website ID at runtime. If it is unset in a dev build, `drill` also checks a repo-local `.env` file for `DRILL_UMAMI_WEBSITE_ID`. If no runtime value is found, it falls back to any website ID embedded at build time.
...
Inspect the evidence with `drill stats --agents` (per-purpose aggregates, including a `METRICS` coverage count so a real zero is distinguishable from missing instrumentation) or `drill stats --run <id>` (one run's invocations, the per-round-vs-cumulative token split, and parked time).
