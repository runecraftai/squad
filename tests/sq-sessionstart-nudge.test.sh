...
#
# The run-wrapper cases drive the REAL bin/sq-session-start.sh against a
...
if [ "${SQUAD_SESSIONSTART_TEST_HARNESS:-0}" != 1 ]; then
  HARNESS_FIXTURE=$(mktemp -d "${TMPDIR:-/tmp}/sq-sessionstart-harness.XXXXXX") || exit 1
  ln -s /bin/bash "$HARNESS_FIXTURE/codex" || exit 1
  # shellcheck disable=SC2016 # Expand in the fixture shell, not this parent.
  SQUAD_SESSIONSTART_TEST_HARNESS=1 "$HARNESS_FIXTURE/codex" \
    -c '"$@"; rc=$?; :; exit "$rc"' _ "$0" "$@"
  HARNESS_STATUS=$?
...
RUN="$ROOT/bin/sq-sessionstart-run.sh"
# shellcheck source=/dev/null
. "$ROOT/bin/sq-operational-input.sh"
NUDGE_TEXT="Run \`bin/sq-session-start.sh\` now, exactly once, before executing any other instructions."
...
test_genuine_primary_nudges() {
  ...
  pass "sq-sessionstart-nudge: a genuine primary gets one explicitly marked instruction line"
}
...
test_unmarked_linked_worktree_is_silent() {
  ...
  pass "sq-sessionstart-nudge: an unmarked linked task worktree is silent"
}
...
test_linked_XO_primary_nudges() {
  ...
  out=$(run_nudge "$root") || status=$?
  expect_code 0 "$status" "linked XO nudge"
  [ "$out" = "$NUDGE_LINE" ] || fail "linked XO printed unexpected output: $out"
  pass "sq-sessionstart-nudge: a marked linked XO home is a primary"
}
...
test_missing_state_is_silent() {
  ...
  pass "sq-sessionstart-nudge: a checkout without state is silent"
}
...
test_owned_lock_is_silent() {
  ...
  pass "sq-sessionstart-nudge: a lock holder in process ancestry is already run"
}

test_opencode_plugin_delivers_exact_nudge_once() {
  local root="$TMP_ROOT/opencode-primary" out status=0
  make_primary "$root"
  cp "$ROOT/bin/sq-sessionstart-nudge.sh" "$ROOT/bin/sq-primary-scope-lib.sh" \
    "$ROOT/bin/sq-gate-refuse-lib.sh" "$ROOT/bin/sq-operational-input.sh" "$root/bin/"
  chmod +x "$root/bin/sq-sessionstart-nudge.sh"
  out=$(PLUGIN="$ROOT/.opencode/plugins/sq-primary-sessionstart-nudge.js" \
...
test_run_startup_runs_the_full_digest() {
  ...
  assert_not_contains "$out" "SQUAD_OP" "a run-tier open also emitted the nudge instruction"
...
test_run_clear_and_compact_reemit() {
  ...
  for source in clear compact; do
    ...
    assert_contains "$out" "Queued wakes ARE still drained" "$source did not preserve the stand-to queue drain"
    assert_not_contains "$out" "SQUAD_OP" "a $source open also emitted the nudge instruction"
  done
...
test_pi_large_sessionstart_digest_is_delivered_loudly() {
  ...
  : > "$fixture/AGENTS.md"
  cp "$ROOT/.pi/extensions/sq-primary-turnend-guard.ts" "$fixture/.pi/extensions/"
  cp "$ROOT/.pi/extensions/lib/sq-operational-input.ts" "$fixture/.pi/extensions/lib/"
  cp "$ROOT/bin/sq-sessionstart-run.sh" "$ROOT/bin/sq-sessionstart-nudge.sh" \
    "$ROOT/bin/sq-primary-scope-lib.sh" "$ROOT/bin/sq-gate-refuse-lib.sh" \
    "$ROOT/bin/sq-operational-input.sh" "$fixture/bin/"
  cat > "$fixture/bin/sq-session-start.sh" <<'SH'
#!/usr/bin/env bash
...
if (!content.includes("SQUAD_OP: v1 session-start:")) throw new Error("operational provenance was lost");
JS
  ) || status=$?
...
test_run_resume_delegates_to_the_nudge() {
  ...
  assert_absent "$root/state/.lock" "resume acquired the unit lock instead of delegating"
...
test_run_reads_source_from_the_hook_payload() {
  ...
  assert_contains "$out" "SQUAD_OP" "a resume hook payload did not delegate to the nudge"
...
test_run_gate_and_scope_are_silent() {
  ...
  assert_absent "$linked/state/.lock" "an unmarked task worktree still took the unit lock"
