...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    ...
    def malformed_optional_fields($items):
      ...
      or ($items | any(has("account") and (((.account | type) != "string") or (.account | length) == 0)));
    def bad_efforts:
      configured_profiles
    ...
    elif malformed_optional_fields([(.rules // [])[]? | profiles(.use?)[]?]) then "use profile model, effort, and account must be non-empty strings when present"
    ...
    elif has("default") and malformed_optional_fields([profiles(.default)[]?]) then "default profile model, effort, and account must be non-empty strings when present"
    else
      (configured_profiles
        | map(.harness)
        | map(select(. != null))
...
if [ "${SQUAD_BOOTSTRAP_DETECT_ONLY:-0}" != 1 ] && local_phase; then
  if ! "$SCRIPT_DIR/sq-register-package-skills.sh"; then
    echo "package skill registration failed" >&2
    exit 1
  fi
fi
exit 0
