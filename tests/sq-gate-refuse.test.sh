...
test_helper_normal_is_noop() {
  ...
  pass "sq-gate-refuse-lib: no-op for a normal session (neither signal, set -eu clean)"
}

# --- sq-spawn ---------------------------------------------------------------

# A fake tmux/fob so sq-spawn resolves the operator worktree from a controlled
# pane path and completes without a live terminal (mirrors tests/sq-tangle-guard).
make_spawn_fakebin() {
  local dir=$1 fakebin
...
test_spawn_refuses_and_admits() {
  ...
  pass "sq-spawn: refuses on marker and gate-worktree backstop; a normal crew spawn is unaffected"
}

# --- sq-send ----------------------------------------------------------------

# A fake tmux that logs send-keys to SQUAD_TMUX_LOG and reports live endpoints
# (mirrors tests/sq-send-strict), so a successful send is observable and a
...
test_send_refuses_and_admits() {
  ...
  : > "$log"
  out=$(run_send "$GATE_WT" "$home" "$fakebin" "$log" sq-lane-ok "hello commander"); rc=$?
  ...
  # no-regression.
  : > "$log"
  out=$(run_send "$NORMAL_CWD" "$home" "$fakebin" "$log" sq-lane-ok "hello commander"); rc=$?
  ...
  assert_contains "$(cat "$log")" "target=sess:sq-lane-ok literal=1 arg=hello commander" "send: normal send should type the text"
  pass "sq-send: refuses on marker and gate-worktree backstop; a normal steer is unaffected"
}
...
test_teardown_refuses_and_admits() {
  ...
  pass "sq-teardown: refuses on marker and gate-worktree backstop; a normal teardown is unaffected"
}
