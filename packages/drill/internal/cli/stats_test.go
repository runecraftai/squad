...
func TestStatsCommandRendersAllRepoDashboard(t *testing.T) {
	...
	for _, notWant := range []string{"Saved", "Rescue runs", "Mistakes fixed", "auto-fix", "caught in review", "╭─ drill"} {
...
func TestStatsDashboardTopBorderShowsGitPushDrill(t *testing.T) {
	out := renderStatsDashboard(&db.Stats{})
	firstLine := strings.Split(out, "\n")[0]
	if !strings.Contains(firstLine, "git push drill") {
