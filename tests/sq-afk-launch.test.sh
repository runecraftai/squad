#!/usr/bin/env bash
# tests/sq-afk-launch.test.sh - the script-owned, backend-aware away-daemon
# launch (bin/sq-afk-launch.sh) and the away-mode stale-artifact lifecycle fixes
# (bin/sq-afk-start.sh). Two layers:
#
...
#   the commander's active tab topology UNCHANGED, because the daemon lands in a
#   NON-VISIBLE separate terminal (a herdr dedicated workspace, a detached tmux
#   session), never a split of the commander's pane. The herdr path runs on a
#   throwaway, NEVER-default HERDR_SESSION and asserts the default session is
#   byte-identical via the sq-herdr-lab.sh unit-state tripwire; the tmux path
#   uses uniquely-named throwaway sessions killed by exact name. A harmless
#   sleeper replaces the real daemon (SQUAD_AFK_LAUNCH_ENTRY) so the test observes
#   only the terminal lifecycle.
set -u

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
LAUNCH="$ROOT/bin/sq-afk-launch.sh"
START="$ROOT/bin/sq-afk-start.sh"

FAILED=0
fail() { printf 'not ok - %s\n' "$1" >&2; FAILED=1; }
pass() { printf 'ok - %s\n' "$1"; }

SLEEPER=$(mktemp "${TMPDIR:-/tmp}/sq-afk-sleeper.XXXXXX")
...
unit_clear_stale() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-clear.XXXXXX")
  mkdir -p "$st/state"
  ...
  if [ -e "$st/state/.stand-to-queue" ]; then
    pass "clear-stale: leaves the durable stand-to queue intact (no pending work dropped)"
  else
    fail "clear-stale: removed the durable stand-to queue"
  fi
  rm -rf "$st"
}
...
unit_relative_paths_are_absolute_before_daemon_launch() {
  ...
  root=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-relative-home.XXXXXX")
  ...
  if [ "$status" -ne 0 ] && printf '%s\n' "$out" | grep -F "SQUAD_STATE_OVERRIDE directory cannot be resolved: missing-state" >/dev/null; then
    pass "launcher paths: unresolved relative SQUAD_STATE_OVERRIDE fails loudly"
  else
    fail "launcher paths: unresolved relative SQUAD_STATE_OVERRIDE did not name the bad input ($out)"
  fi
  rm -rf "$root"
}
...
unit_fresh_vs_refresh() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-refresh.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.subsuper-escalations"
  : > "$st/state/.subsuper-inject-wedged"
...
unit_stop_ordering() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-stop.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
...
unit_stop_rejects_reused_pid() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-pid-reuse.XXXXXX")
  mkdir -p "$st/state"
  date '+%s' > "$st/state/.afk"
  sleep 600 &
  sleeper_pid=$!
...
unit_failed_start_rolls_back_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-failed-start.XXXXXX")
  mkdir -p "$st/state"
  printf 'pending\n' > "$st/state/.subsuper-escalations"
  printf 'wedged\n' > "$st/state/.subsuper-inject-wedged"
...
unit_concurrent_start_serialized() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-concurrent.XXXXXX")
  cap_session="sq-afk-concurrent-cap-$$"
...
unit_lock_initialization_grace() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-lock-init.XXXXXX")
  ...
  (
    ...
    if [ -d "$st/state/.afk-launch.lock" ]; then
      ...
      ( . "$ROOT/bin/sq-stand-to-lib.sh"; fm_pid_identity "$$" > "$st/state/.afk-launch.lock/pid-identity" 2>/dev/null ) || true
      : > "$marker"
      sleep 0.15
...
unit_herdr_partial_create_recovery() {
  ...
  SQUAD_HOME="$st" SQUAD_STATE_OVERRIDE="$st/state" SQUAD_AFK_LAUNCH_ENTRY=/bin/true \
    ...
    fm_afk_launch_create_herdr lab:commander herdr
  ' _ "$LAUNCH"
...
unit_native_entry_preserves_prepared_state() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-native-entry.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  : > "$st/state/.subsuper-escalations"
...
unit_malformed_record_fails_closed() {
  local st acted
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-record-malformed.XXXXXX")
  mkdir -p "$st/state"
...
unit_stop_malformed_record_fails_closed() {
  local st
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-stop-malformed.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  if SQUAD_HOME="$st" SQUAD_STATE_OVERRIDE="$st/state" bash -c '
...
unit_stop_validates_before_signal() {
  ...
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-stop-validate.XXXXXX")
  mkdir -p "$st/state"
  : > "$st/state/.afk"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
...
unit_stop_confirms_daemon_exit() {
  ...
  if SQUAD_HOME="$st" SQUAD_STATE_OVERRIDE="$st/state" bash -c '
    ...
    kill() {
      command kill "$@"
      if [ "$1" = -TERM ]; then
        rm -rf "$SQUAD_AFK_LAUNCH_STATE/.supervise-daemon.lock"
      fi
    }
    ! fm_afk_launch_stop
...
unit_refresh_validates_record() {
  local st daemon_pid
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-refresh-record.XXXXXX")
  mkdir -p "$st/state/.supervise-daemon.lock"
  printf 'tmux\tonly-two-fields\n' > "$st/state/.afk-daemon-terminal"
  sleep 30 & daemon_pid=$!
  printf '%s' "$daemon_pid" > "$st/state/.supervise-daemon.lock/pid"
...
unit_incomplete_restore_retains_backup() {
  local st backup
  st=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-restore-fail.XXXXXX")
  mkdir -p "$st/state"
...
e2e_herdr() {
  ...
  out=$(fm_backend_herdr_cli "$SESSION" workspace create --cwd "$ROOT" --label commander --no-focus 2>/dev/null)
  ...
  if [ -z "$cap_ws" ] || [ -z "$cap_pane" ]; then E2E_HERDR_CLEANUP; fail "herdr e2e: could not create commander workspace"; return 0; fi
  ...
  if [ "$before" = "$during" ]; then pass "herdr e2e: commander tab pane count unchanged after start (no split)"; else fail "herdr e2e: commander tab pane count changed ($before -> $during)"; fi
  ...
  if [ "$after" = "$before" ]; then pass "herdr e2e: commander tab pane count restored after stop"; else fail "herdr e2e: commander tab pane count not restored ($before -> $after)"; fi
...
e2e_tmux() {
  ...
  cap_session="sq-afk-launch-cap-$$"
  home_tmp=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-tmux-home.XXXXXX")
  tmux new-session -d -s "$cap_session" 2>/dev/null || { fail "tmux e2e: could not create commander session"; rm -rf "$home_tmp"; return 0; }
  TRACK_TMUX_SESSIONS="$TRACK_TMUX_SESSIONS $cap_session"
  cap_pane=$(tmux display-message -p -t "$cap_session" '#{pane_id}')
  ...
  if [ "$after" = "$before" ]; then pass "tmux e2e: commander window pane count unchanged after stop"; else fail "tmux e2e: commander window changed ($before -> $after)"; fi
