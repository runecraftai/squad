#!/usr/bin/env bash
# tests/sq-afk-inject-herdr-e2e.test.sh - real-herdr end-to-end test for the
# away-mode daemon's herdr transport (bin/sq-supervise-daemon.sh), the herdr
# counterpart of tests/sq-afk-inject-e2e.test.sh's private-socket tmux e2e.
# Mirrors tests/sq-backend-herdr-smoke.test.sh and tests/herdr-test-safety.sh's
...
# at SQUAD_SUPERVISOR_BACKEND=herdr, SQUAD_SUPERVISOR_TARGET="<session>:<pane-id>",
...
# tests/sq-afk-inject-e2e.test.sh uses for its tmux supervisor pane, so this
...
# --- source the daemon (for afk_enter/afk_exit/SQUAD_INJECT_MARK) + the backend -
...
STATE_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sq-afk-herdr-e2e.XXXXXX")
mkdir -p "$STATE_DIR"
...
TASK_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "sq-afk-e2e-supervisor" /tmp "$SEEDED_TAB_ID") \
...
# e2e's fake sq-fake-c1 operator window - not required by scan_signals (which
...
FAKE_CREW_IDS=$(fm_backend_herdr_create_task "$CONTAINER" "sq-fake-c1" /tmp) \
  || fail "could not create the fake operator scratch tab"
read -r _FAKE_TAB_ID FAKE_CREW_PANE_ID <<EOF
$FAKE_CREW_IDS
EOF

# --- deterministic bordered-composer loop, drawn in the scratch pane ---------
# Mirrors tests/sq-afk-inject-e2e.test.sh's supervisor-loop.sh, but draws a
...
MARK=$'\xE2\x81\xA3'
LOG="$1"
AGENT_SOURCE=sq-test-supervisor
AGENT_LABEL=sq-test-supervisor
...
submit_line() {
  ...
  # window comfortably covers the daemon's SQUAD_INJECT_CONFIRM_SLEEP=0.5
...
HERDR_SHIM_DIR=$(mktemp -d "${TMPDIR:-/tmp}/sq-herdr-shim.XXXXXX")
...
start_daemon() {
  ...
  SQUAD_STATE_OVERRIDE="$STATE_DIR" \
  SQUAD_SUPERVISOR_BACKEND=herdr \
  SQUAD_SUPERVISOR_TARGET="$SUPERVISOR_TARGET" \
  SQUAD_ESCALATE_BATCH_SECS=0 \
  SQUAD_HOUSEKEEPING_TICK=1 \
  SQUAD_POLL=1 \
  SQUAD_SIGNAL_GRACE=1 \
  SQUAD_HEARTBEAT=999999 \
  SQUAD_CHECK_INTERVAL=999999 \
  SQUAD_INJECT_CONFIRM_SLEEP=0.5 \
  SQUAD_INJECT_CONFIRM_RETRIES=6 \
  SQUAD_STALE_ESCALATE_SECS=999999 \
...
reset_state() {
  rm -f "$STATE_DIR"/*.status \
         "$STATE_DIR"/.subsuper-* \
         "$STATE_DIR"/.stand-to-queue* \
         "$STATE_DIR"/.sentry.lock* \
         "$STATE_DIR"/.last-* \
         "$STATE_DIR"/.hash-* \
         "$STATE_DIR"/.count-* \
         "$STATE_DIR"/.stale-* \
...
test_scenario_c() {
  ...
  stop_daemon
  pass "real herdr Scenario C: a normal commander status injects exactly one clean single-line sentinel digest"
}
...
test_scenario_d_max_defer() {
  ...
  SQUAD_SUPERVISOR_BACKEND=herdr \
  SQUAD_SUPERVISOR_TARGET="$SUPERVISOR_TARGET" \
  SQUAD_ESCALATE_BATCH_SECS=99999 \
  SQUAD_HOUSEKEEPING_TICK=1 \
  SQUAD_POLL=1 \
  SQUAD_SIGNAL_GRACE=1 \
  SQUAD_HEARTBEAT=999999 \
  SQUAD_CHECK_INTERVAL=999999 \
  SQUAD_MAX_DEFER_SECS=3 \
  SQUAD_INJECT_CONFIRM_SLEEP=0.3 \
  SQUAD_INJECT_CONFIRM_RETRIES=2 \
  SQUAD_STALE_ESCALATE_SECS=999999 \
  nohup "$DAEMON" >"$STATE_DIR/daemon.out" 2>"$STATE_DIR/daemon.err" &
  DAEMON_PID=$!
