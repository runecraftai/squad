...
func buildTestNMBin(t *testing.T) string {
	...
	outDir := t.TempDir()
	bin := filepath.Join(outDir, "drill")
	if runtime.GOOS == "windows" {
		bin += ".exe"
	}
	cmd := exec.Command("go", "build", "-o", bin, "./cmd/drill")
	cmd.Dir = repoRoot
	out, err := cmd.CombinedOutput()
	if err != nil {
		t.Fatalf("build drill: %v\n%s", err, out)
	}
	return bin
}
...
func startDetachedTestDaemon(t *testing.T, bin, drillHome string) int {
	t.Helper()
	if err := os.MkdirAll(filepath.Join(drillHome, "logs"), 0o755); err != nil {
		t.Fatal(err)
	}
	cfg := "agent: claude\nlog_level: error\n"
	if err := os.WriteFile(filepath.Join(drillHome, "config.yaml"), []byte(cfg), 0o644); err != nil {
		t.Fatal(err)
	}
	logPath := filepath.Join(drillHome, "logs", "daemon.log")
	logFile, err := os.OpenFile(logPath, os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
		t.Fatal(err)
	}
	cmd := exec.Command(bin, "daemon", "run", "--root", drillHome)
	cmd.Env = append(os.Environ(),
		"DRILL_HOME="+drillHome,
	...
	for time.Now().Before(deadline) {
		if MatchesDaemonRoot(pid, drillHome) {
		...
		if found, err := FindDaemonsForRoot(drillHome); err == nil {
			for _, p := range found {
				if MatchesDaemonRoot(p, drillHome) {
					pid = p
				}
			}
		}
...
func assertNoDaemonForRoot(t *testing.T, drillHome string) {
	t.Helper()
	found, err := FindDaemonsForRoot(drillHome)
	if err != nil {
		t.Fatalf("FindDaemonsForRoot: %v", err)
	}
	if len(found) != 0 {
		t.Fatalf("want zero daemons for %s, found pids %v", drillHome, found)
	}
}
...
func TestReapAll_NormalCompletion(t *testing.T) {
	...
	if err := inv.Register("normal", drillHome, bin, pid, os.Getpid()); err != nil {
		t.Fatal(err)
	}
	...
	assertNoDaemonForRoot(t, drillHome)
	list, _ := inv.List()
	if len(list) != 0 {
...
func TestReapAll_CallerInterrupt(t *testing.T) {
	...
	if err := inv.Register("interrupt", drillHome, bin, pid, os.Getpid()); err != nil {
		t.Fatal(err)
	}
	if !MatchesDaemonRoot(pid, drillHome) {
...
func TestReapAll_Timeout(t *testing.T) {
	...
	if err := inv.Register("timeout", drillHome, bin, pid, os.Getpid()); err != nil {
		t.Fatal(err)
	}
	result := inv.ReapAll()
	if result.Removed < 1 {
...
func TestReapAll_ChildSIGKILL_WrapperReaps(t *testing.T) {
	...
	cmd.Env = append(os.Environ(),
		...
		"E2EDAEMON_HELPER_HOME="+drillHome,
	)
	...
	for time.Now().Before(deadline) {
		...
		if err == nil {
			if list, err := inv.List(); err == nil && len(list) > 0 {
				...
				if found, _ := FindDaemonsForRoot(drillHome); len(found) > 0 {
					ready = true
					break
				}
			}
...
func runSigkillChildHelper() {
	...
	drillHome := os.Getenv("E2EDAEMON_HELPER_HOME")
	invDir := os.Getenv(EnvInventory)
	if bin == "" || drillHome == "" || invDir == "" {
		fmt.Fprintln(os.Stderr, "missing helper env")
		os.Exit(1)
	}
	// Inline start (no testing.T).
	_ = os.MkdirAll(filepath.Join(drillHome, "logs"), 0o755)
	_ = os.WriteFile(filepath.Join(drillHome, "config.yaml"), []byte("agent: claude\nlog_level: error\n"), 0o644)
	logFile, err := os.OpenFile(filepath.Join(drillHome, "logs", "daemon.log"), os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0o644)
	if err != nil {
	...
	if err := inv.Register("child", drillHome, bin, pid, os.Getpid()); err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(1)
	}
	fmt.Println("ready")
...
func TestReapAll_StaleInventoryRecovery(t *testing.T) {
	...
	if err := inv.Register("stale", drillHome, bin, 999999, os.Getpid()); err != nil {
		t.Fatal(err)
	}
	result := inv.ReapAll()
	if result.Removed < 1 {
...
func TestReapAll_DoesNotTouchSharedDaemon(t *testing.T) {
	...
	sharedHome := filepath.Join(mustUserHome(t), ".drill")
