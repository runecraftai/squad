...
func TestLoadRepo_Defaults(t *testing.T) {
	// Non-existent directory or no .drill.yaml
...
`
	if err := os.WriteFile(filepath.Join(dir, ".drill.yaml"), []byte(data), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestMerge_CIRerunTransientFromRepoConfig(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			dir := t.TempDir()
			if err := os.WriteFile(filepath.Join(dir, ".drill.yaml"), []byte(tc.yaml), 0o644); err != nil {
				t.Fatal(err)
			}
