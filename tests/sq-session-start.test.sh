#!/usr/bin/env bash
# tests/sq-session-start.test.sh - behavior tests for bin/sq-session-start.sh,
...
#   - output section ordering: the safety preamble leads unchanged, live unit
...
#     sentry ownership
#   - status-tail bounding, default and SQUAD_SESSION_START_STATUS_TAIL override
...
#     tmux and herdr both
#   - composition: the script invokes the real sq-lock.sh/sq-bootstrap.sh/
#     sq-stand-to-drain.sh (their real, distinctive output appears verbatim), it
...
SESSION_START="$ROOT/bin/sq-session-start.sh"
BASE_PATH=${SQUAD_TEST_BASE_PATH:-/usr/bin:/bin:/usr/sbin:/sbin}
TMP_ROOT=$(fm_test_tmproot sq-session-start-tests)
SESSION_START_SECOND_MATE_ID="fmtest-sm-${TMP_ROOT##*.}"
SESSION_START_SECOND_MATE_TMP="/tmp/sq-$SESSION_START_SECOND_MATE_ID"
SESSION_START_HERDR_SECOND_MATE_ID="fmtest-herdr-${TMP_ROOT##*.}"
SESSION_START_HERDR_SECOND_MATE_TMP="/tmp/sq-$SESSION_START_HERDR_SECOND_MATE_ID"
SQUAD_TEST_CLEANUP_DIRS+=("$TMP_ROOT" "$SESSION_START_SECOND_MATE_TMP" "$SESSION_START_HERDR_SECOND_MATE_TMP")
...
# make_fake_toolchain <fakebin>: every tool sq-bootstrap.sh detects, present
# and compatible, so its own detect-only section stays quiet except where a
# test deliberately breaks one. Mirrors sq-bootstrap.test.sh's fixture.
make_fake_toolchain() {
  local fakebin=$1
...
# or done rows. SQUAD_FAKE_TASKS_AXI_READY sizes the ready set so the queued bound
...
#!/usr/bin/env bash
set -u
log=${SQUAD_FAKE_TASKS_AXI_LOG:-}
[ -n "$log" ] && printf '%s\n' "$*" >> "$log"
ready_count=${SQUAD_FAKE_TASKS_AXI_READY:-2}
require_file() {
...
case "${1:-}" in
  ...
  ready)
    ...
    while [ "$i" -le "$ready_count" ]; do
      printf '  ready-%s,queued,ship,Squad,Ready item %s\n' "$i" "$i"
      i=$((i + 1))
    done
  ...
  list)
    ...
    case "$*" in
      ...
      *'--state in_flight'*)
        task_header 1
        printf '%s\n' '  compact-startup,in_flight,ship,Squad,Compact startup digest,none,commander,commander choice pending'
        ;;
      *'--state held'*)
        task_header 1
        printf '%s\n' '  held-queued,queued,ship,Squad,Held queued work,none,commander,commander choice pending'
        ;;
      *'--state queued'*'--blocked'*)
        task_header 1
        printf '%s\n' '  blocked-followup,queued,recon,Squad,Follow compact startup,compact-startup,"-","-"'
        ;;
      *)
...
# make_fake_ps_claude <fakebin>: harness_pid()/holder_alive() (sq-lock.sh) walk
...
#!/usr/bin/env bash
set -u
harness=${SQUAD_FAKE_HARNESS:-claude}
pid=
previous=
for argument in "$@"; do
...
case "$*" in
  *"comm="*)
    if [ -z "${SQUAD_FAKE_HARNESS_PID:-}" ] || [ "$pid" = "$SQUAD_FAKE_HARNESS_PID" ]; then
  ...
  *"args="*)
    ...
    ;;
  *"ppid="*)
    [ -n "${SQUAD_FAKE_HARNESS_PID:-}" ] || exit 1
    /bin/ps -o ppid= -p "$pid"
    ;;
esac
exit 1
...
# make_fake_tmux_XO_recovery <fakebin>: a stateful tmux boundary
# fixture for the real session-start -> bootstrap -> spawn path.
# SQUAD_FAKE_TMUX_MODE selects missing, ambiguous, unreadable, or shell; missing
# reproduces real tmux's active-window fallback while inventory omits the mate.
make_fake_tmux_XO_recovery() {
  local fakebin=$1
  cat > "$fakebin/tmux" <<'SH'
#!/usr/bin/env bash
set -u
mode=${SQUAD_FAKE_TMUX_MODE:?}
log=${SQUAD_FAKE_TMUX_LOG:?}
spawned=${SQUAD_FAKE_TMUX_SPAWNED:?}
killed=${spawned}.killed
mate_home=${SQUAD_FAKE_SECOND_MATE_HOME:?}
mate_id=${SQUAD_FAKE_SECOND_MATE_ID:?}
mate_window="sq-$mate_id"
case "${1:-}" in
  display-message)
    target=
    format=
...
case "${1:-} ${2:-}" in
  ...
  "session list")
    ...
    ;;
  "workspace list")
    printf '{"result":{"workspaces":[{"workspace_id":"ws1","label":"xo-%s","focused":true,"active_tab_id":"t-focus"}]}}\n' "$mate_id"
    ;;
  "tab list")
    if [ -e "$spawned" ]; then
      printf '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"commander","focused":true},{"tab_id":"t-new","workspace_id":"ws1","label":"sq-%s","focused":false}]}}\n' "$mate_id"
    elif [ -e "$killed" ]; then
      printf '%s\n' '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"commander","focused":true}]}}'
    else
      printf '{"result":{"tabs":[{"tab_id":"t-focus","workspace_id":"ws1","label":"commander","focused":true},{"tab_id":"t-old","workspace_id":"ws1","label":"sq-%s","focused":false}]}}\n' "$mate_id"
    fi
    ;;
  "tab create")
    : > "$spawned"
...
# liveness check must never auto-start a server (sq-backend.sh's contract).
make_fake_herdr() {
  local fakebin=$1 live=$2
  cat > "$fakebin/herdr" <<SH
#!/usr/bin/env bash
...
# Drop every harness env marker from bin/sq-harness.sh detect_own so the
# surrounding interactive shell cannot leak past the suite's fake ps harness.
# Markers today: CLAUDECODE (claude), PI_CODING_AGENT plus SQUAD_PI_HARNESS
...
# prepare_session_start_XO <name>: a throwaway main home and Pi
# XO home wired to the real spawn implementation through the fixture
# root. Echoes root|home|fakebin|mate|log|spawned.
prepare_session_start_XO() {
...
EOF
  ...
  make_fake_tmux_XO_recovery "$fakebin"
  : > "$log"
  printf '%s|%s|%s|%s|%s|%s\n' "$root" "$home" "$fakebin" "$mate" "$log" "$spawned"
}

run_session_start_XO() {
  local root=$1 home=$2 fakebin=$3 mate=$4 log=$5 spawned=$6 mode=$7
  TMUX='' SQUAD_BACKEND=tmux SQUAD_FAKE_TMUX_MODE="$mode" SQUAD_FAKE_TMUX_LOG="$log" \
    SQUAD_FAKE_TMUX_SPAWNED="$spawned" SQUAD_FAKE_SECOND_MATE_HOME="$mate" \
    SQUAD_FAKE_SECOND_MATE_ID="$SESSION_START_SECOND_MATE_ID" \
    SQUAD_FAKE_HARNESS_PID=$$ \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH"
}

prepare_session_start_herdr_XO() {
...
EOF
  ...
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_herdr_XO_recovery "$fakebin"
  : > "$log"
  printf '%s|%s|%s|%s|%s|%s\n' "$root" "$home" "$fakebin" "$mate" "$log" "$state"
}

run_session_start_herdr_XO() {
  local root=$1 home=$2 fakebin=$3 mate=$4 log=$5 state=$6
  SQUAD_BACKEND=herdr SQUAD_FAKE_HERDR_LOG="$log" SQUAD_FAKE_HERDR_STATE="$state" \
    SQUAD_FAKE_SECOND_MATE_ID="$SESSION_START_HERDR_SECOND_MATE_ID" \
    SQUAD_FAKE_HARNESS_PID=$$ \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH"
}
...
wait_for_network_wake() {
  ...
  while ! grep -Fq $'check\tstartup-network' "$home/state/.stand-to-queue" 2>/dev/null \
    && [ "$waited" -lt "$limit" ]; do
    sleep 1
    waited=$((waited + 1))
  done
  grep -Fq $'check\tstartup-network' "$home/state/.stand-to-queue" 2>/dev/null
}
...
install_pi_turnend_extension_fixture() {
  ...
  cp "$ROOT/.pi/extensions/sq-primary-turnend-guard.ts" "$root/.pi/extensions/sq-primary-turnend-guard.ts"
}

install_pi_watch_extension_fixture() {
  local root=$1
  mkdir -p "$root/.pi/extensions"
  cp "$ROOT/.pi/extensions/sq-primary-pi-watch.ts" "$root/.pi/extensions/sq-primary-pi-watch.ts"
}

write_pi_watch_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/sq-primary-pi-watch.ts")
  printf '%s\n%s\n' "$version" "$pid" > "$home/state/.pi-watch-extension-loaded"
}

write_pi_turnend_loaded_marker() {
  local home=$1 root=$2 pid=$3 version
  version=$(hash_file_for_test "$root/.pi/extensions/sq-primary-turnend-guard.ts")
...
EOF
  ...
  assert_contains "$out" "data/commander-shared.md (shared, main-authoritative, read-only in XO homes)" \
    "digest did not label the shared commander section"

  assert_contains "$out" "data/XOs.md" "digest did not label the XOs.md section"
  assert_contains "$out" "data/learnings.md" "digest did not label the learnings.md section"

  # Exactly four context ABSENT markers (XOs.md, commander-shared.md,
  # learnings.md; backlog.md is covered by its own test) - and the
  # present-but-empty commander.md must NOT print ABSENT.
  absent_count=$(printf '%s\n' "$out" | grep -c '^ABSENT$')
  [ "$absent_count" -eq 4 ] || fail "expected 4 ABSENT markers (XOs.md, commander-shared.md, learnings.md, backlog.md), got $absent_count: $out"

  cap_section=$(printf '%s\n' "$out" | awk '/^data\/commander\.md$/{flag=1;next}/^data\//{flag=0}flag')
  assert_contains "$cap_section" "(present, empty)" "empty-but-present commander.md was not distinguished from ABSENT"
...
EOF
  ...
  # A live XO meta with a window pointed at nothing real - if the
  # bootstrap sweep's XO_sync ran (a MUTATING step), it would try to
  # fast-forward this "home" and/or report a XO_SYNC/NUDGE_XOS
  # line. Absence of any such line is this test's proof that
  # SQUAD_BOOTSTRAP_DETECT_ONLY=1 actually suppressed the mutating sweep.
  ...
  expect_code 0 "$status" "sq-session-start.sh must exit 0 even on a lock refusal"
  assert_contains "$out" "READ-ONLY SESSION" "read-only banner missing on lock refusal"
  assert_contains "$out" "another live Squad session holds the lock" "read-only banner did not surface sq-lock.sh's own error text"
  ...
  assert_not_contains "$out" "run bin/sq-sentry-arm.sh" "read-only guard printed a mutating sentry-arm instruction"
  ...
  # The mutating XO sweep must NOT have run: no XO_SYNC/
  # NUDGE_XOS line, and the sowed XO meta's target dir is
  # untouched (sq-ff-lib would have tried to fast-forward it otherwise).
  assert_not_contains "$out" "XO_SYNC" "mutating XO sweep ran during a lock refusal"
  assert_not_contains "$out" "NUDGE_XOS" "mutating XO sweep ran during a lock refusal"

  # The rest of the digest (read-only-safe) still completed.
  assert_contains "$out" "UNIT STATE" "unit-state digest section missing on the read-only path"
...
EOF
  ...
  expect_code 0 "$status" "sq-session-start.sh must exit 0 when lock publication fails"
  ...
  assert_contains "$out" "UNIT LOCK OWNERSHIP WAS NOT VERIFIED" "lock publication failure was misreported as a live holder"
  assert_contains "$out" "lacks verified unit-lock ownership" "lock publication failure did not explain why queued wakes remain untouched"
  assert_not_contains "$out" "ANOTHER LIVE SQUAD SESSION HOLDS THE UNIT LOCK" "lock publication failure falsely claimed a live lock holder"
  [ -s "$home/state/.stand-to-queue" ] || fail "lock publication failure allowed the stand-to queue to mutate"
...
EOF
  ...
  SQUAD_TRACE_CONTEXT=off run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  ...
  SQUAD_TRACE_CONTEXT=on run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  ...
  holder_pid=$!
  printf '%s\n' "$holder_pid" > "$home/state/.lock"
  out=$(SQUAD_TRACE_CONTEXT=off run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  kill "$holder_pid" 2>/dev/null || true
  wait "$holder_pid" 2>/dev/null || true
...
case "$*" in
  *"comm="*)
    if [ -f "$SQUAD_FAKE_LOCK_STATE/harness-$pid" ]; then
...
# still leads, live unit identity now outranks curated memory, and the
...
EOF
  ...
  printf 'Commander memory that may be truncated away safely.\n' > "$home/data/commander.md"
  ...
  fleet_line=$(printf '%s\n' "$out" | grep -n '^UNIT STATE$' | head -1 | cut -d: -f1)
  ...
  [ "$read_once_line" -lt "$fleet_line" ] || fail "the read-once contract did not precede UNIT STATE"
  [ "$fleet_line" -lt "$context_line" ] || fail "UNIT STATE did not precede CONTEXT"
...
SH
    ...
    assert_not_contains "$out" "MISSING: fob" "Herdr session start missed its available worktree provider"
  done
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "sq-sess:live"

  printf 'window=sq-sess:live\nkind=strike\n' > "$home/state/task-a.meta"
  ...
  out=$(SQUAD_SESSION_START_STATUS_TAIL=2 run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "working: step 7" "SQUAD_SESSION_START_STATUS_TAIL=2 tail missing the most recent line"
  assert_not_contains "$out" "working: step 5" "SQUAD_SESSION_START_STATUS_TAIL=2 did not bound the tail to 2 lines"

  pass "status tail is bounded to the configured line count, with the full log path always printed"
}

# An operator writes its own status lines, so nothing upstream bounds their
...
# left recoverable rather than allowed to scale the digest with unit load.
...
# --- session-start XO recovery boundary -----------------------------

test_session_start_relaunches_missing_pi_XO() {
  local rec root home fakebin mate log spawned out first_calls second_calls
  rec=$(prepare_session_start_XO XO-missing-pi)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF

  out=$(run_session_start_XO "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" missing)
  ...
  assert_contains "$out" "dead-XO relaunch" \
    "the digest never accounted for the dead-XO relaunch"

  wait_for_network_stage "$home" "$root" \
    || fail "the deferred network stage never published: $(network_stage_report "$home" "$root")"

  assert_not_contains "$(network_stage_report "$home" "$root")" "XO_LIVENESS:" \
    "successful missing-window recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "new-window" "the deferred stage did not relaunch the missing Pi XO"
  ...
  run_session_start_XO "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" missing >/dev/null
  wait_for_network_stage "$home" "$root" \
  ...
  [ "$first_calls" -eq 1 ] && [ "$second_calls" -eq 1 ] \
    || fail "a second session-start pass duplicated the relaunched Pi XO: $(cat "$log")"
  pass "session start: an absent recorded tmux window relaunches its Pi XO exactly once, off the blocking path"
}
...
test_deferred_relaunch_is_always_reported() {
  ...
  rec=$(prepare_session_start_XO XO-relaunch-reported)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF
  ...
  assert_contains "$report" "XO $SESSION_START_SECOND_MATE_ID relaunched" \
...
test_session_start_preserves_ambiguous_pi_process() {
  ...
  rec=$(prepare_session_start_XO XO-ambiguous-pi)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF

  out=$(run_session_start_XO "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" ambiguous)
  wait_for_network_stage "$home" "$root" || fail "the deferred network stage never published"

  assert_contains "$(network_stage_report "$home" "$root")" \
    "XO_LIVENESS: XO $SESSION_START_SECOND_MATE_ID: skipped: existing endpoint has ambiguous agent process (backend=tmux)" \
  ...
  assert_contains "$out" "endpoint: alive (backend=tmux window=Squad:sq-$SESSION_START_SECOND_MATE_ID)" \
    "the later unit read should still see the ambiguous endpoint"
...
test_session_start_preserves_transiently_unreadable_tmux() {
  local rec root home fakebin mate log spawned out
  rec=$(prepare_session_start_XO XO-unreadable-pi)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF

  out=$(run_session_start_XO "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" unreadable)
  wait_for_network_stage "$home" "$root" || fail "the deferred network stage never published"

  assert_contains "$(network_stage_report "$home" "$root")" \
    "XO_LIVENESS: XO $SESSION_START_SECOND_MATE_ID: skipped: endpoint probe unreadable (backend=tmux)" \
  ...
  assert_contains "$out" "endpoint: dead (backend=tmux window=Squad:sq-$SESSION_START_SECOND_MATE_ID)" \
...
test_session_start_preserves_proven_bare_shell_recovery() {
  local rec root home fakebin mate log spawned out
  rec=$(prepare_session_start_XO XO-bare-shell)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF

  run_session_start_XO "$root" "$home" "$fakebin" "$mate" "$log" "$spawned" shell >/dev/null
  wait_for_network_stage "$home" "$root" || fail "the deferred network stage never published"

  out=$(network_stage_report "$home" "$root")
  assert_not_contains "$out" "XO_LIVENESS:" "successful bare-shell recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "kill-window -t =Squad:=sq-$SESSION_START_SECOND_MATE_ID" \
...
test_session_start_relaunches_herdr_husk_XO() {
  local rec root home fakebin mate log state out
  rec=$(prepare_session_start_herdr_XO XO-herdr-husk)
  IFS='|' read -r root home fakebin mate log state <<EOF
$rec
EOF

  run_session_start_herdr_XO "$root" "$home" "$fakebin" "$mate" "$log" "$state" >/dev/null
  wait_for_network_stage "$home" "$root" || fail "the deferred network stage never published"

  out=$(network_stage_report "$home" "$root")
  assert_not_contains "$out" "XO_LIVENESS:" "successful Herdr husk recovery should stay non-actionable"
  assert_contains "$(cat "$log")" "pane close p-old" "session start did not close the confirmed Herdr husk"
  assert_contains "$(cat "$log")" "tab create" "session start did not relaunch the Herdr XO"
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  make_fake_tmux "$fakebin" "sq-sess:live-window"

  printf 'window=sq-sess:live-window\nkind=strike\n' > "$home/state/task-live.meta"
  printf 'window=sq-sess:dead-window\nkind=strike\n' > "$home/state/task-dead.meta"

  out=$(run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "endpoint: alive (backend=tmux window=sq-sess:live-window)" "live tmux endpoint not reported alive"
  assert_contains "$out" "endpoint: dead (backend=tmux window=sq-sess:dead-window)" "dead tmux endpoint not reported dead"
...
EOF
  ...
  # sq-lock.sh's own exact success text.
  assert_contains "$out" "lock acquired: harness pid" "sq-lock.sh's real output did not appear (composition, not reimplementation)"
  # sq-bootstrap.sh's own exact MISSING-tool line format.
  assert_contains "$out" "MISSING: node (install:" "sq-bootstrap.sh's real detect line did not appear verbatim"
  # sq-stand-to-drain.sh's real drained record (raw tab-separated queue line).
  assert_contains "$out" "$(printf 'signal\ttask-z.status\tneeds-decision: pick a library')" "sq-stand-to-drain.sh's real drained record did not appear"
  assert_contains "$out" "wake annotation: latest wake-EVENT observed at drain, not current state: task-z.status: needs-decision: pick a library" "sq-session-start.sh did not preserve the drain's separate annotation line"
...
test_unreachable_network_never_blocks_the_digest() {
  ...
  rec=$(prepare_session_start_XO XO-slow-network)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF
  ...
  assert_contains "$out" "NOT yet confirmed: GitHub authentication, dead-XO relaunch" \
  ...
  assert_contains "$(cat "$log")" "new-window" \
    "the deferred stage lost the dead-XO relaunch"
...
# is asserted deterministically in tests/sq-startup-network.test.sh, where the
...
test_deferred_result_reaches_the_agent_when_the_digest_cannot_print_it() {
  ...
  rec=$(prepare_session_start_XO XO-wake-once)
  IFS='|' read -r root home fakebin mate log spawned <<EOF
$rec
EOF
  install_slow_gh "$fakebin" 8
  queue="$home/state/.stand-to-queue"
...
## In flight
- [ ] compact-startup - Compact startup digest (repo: Squad) (kind: ship) (since 2026-07-15) (hold: commander choice pending) (hold-kind: commander)
...
## Queued
- [ ] blocked-followup - Follow compact startup blocked-by: compact-startup - waits for implementation (repo: Squad) (kind: recon) (since 2026-07-15)
  QUEUED-BODY-LINE this is another long multiline note.
- [ ] held-queued - Held queued work (repo: Squad) (kind: ship) (hold: commander choice pending) (hold-kind: commander)
EOF
  while [ "$i" -le 25 ]; do
    printf -- '- [ ] plain-%s - Plain queued item %s (repo: Squad) (kind: ship)\n' "$i" "$i" >> "$path"
    i=$((i + 1))
  done
  cat >> "$path" <<'EOF'

## Done
- [x] landed-earlier - DONE-ROW-LINE already landed and torn down (repo: Squad) (kind: ship)
EOF
}
...
EOF
  ...
  out=$(SQUAD_FAKE_TASKS_AXI_READY=7 SQUAD_SESSION_START_QUEUED_LIMIT=3 \
    run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
...
EOF
  ...
  out=$(SQUAD_SESSION_START_QUEUED_LIMIT=4 run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "(hold: commander choice pending) (hold-kind: commander)" \
  ...
  assert_contains "$out" "(21 more queued - raise SQUAD_SESSION_START_QUEUED_LIMIT or read data/backlog.md for the rest)" \
...
EOF
  ...
  mechanism=$(SQUAD_TIMEOUT_MECHANISM_OVERRIDE=bash bash -c '. "$1"; fm_timeout_mechanism' \
    _ "$ROOT/bin/sq-timeout-lib.sh")
  [ "$mechanism" = bash ] || fail "the forced pure-Bash timeout fixture selected '$mechanism'"

  out=$(SQUAD_TIMEOUT_MECHANISM_OVERRIDE=bash SQUAD_SESSION_START_TIMEOUT=3 SQUAD_STARTUP_NETWORK_TIMEOUT=2 \
  ...
  assert_contains "$out" "stand-to queue supervision-instructions read-once unit-state network-checks context next-step" \
  ...
  status=0
  SQUAD_TIMEOUT_MECHANISM_OVERRIDE=bash bash -c \
    '. "$1"; fm_run_timed 2 bash -c "exit 137"' _ "$ROOT/bin/sq-timeout-lib.sh" || status=$?
...
SH
  ...
  perl -e '
    ...
    waitpid $pid, 0;
    exit($? >> 8);
  ' env PATH="$fakebin:$BASE_PATH" "$driver" "$ROOT/bin/sq-timeout-lib.sh" \
...
EOF
  ...
  assert_absent "${TMPDIR:-/tmp}/sq-session-start-stage" "the stage breadcrumb leaked a fixed-name file"
...
EOF
  ...
  # sq-session-lock-lib.sh walks a BOUNDED sixteen parents to find it, and the
...
case "$*" in
  *"comm="*)
    if [ "$pid" = "${SQUAD_FAKE_HARNESS_PID:-}" ]; then printf '%s\n' /usr/local/bin/claude
    else printf '%s\n' /bin/bash; fi
    ;;
  *"args="*)
    if [ "$pid" = "${SQUAD_FAKE_HARNESS_PID:-}" ]; then printf '%s\n' claude
...
EOF
  ...
  make_fake_ps_claude "$fakebin"
  mkdir -p "$home/other-XO/state"
  fm_write_XO_meta "$home/state/sm-r.meta" "$home/other-XO" "Squad:sq-sm-r" alpha
  append_wake "$home/state" signal task-r "done: queued after startup" || fail "seed wake failed"

  # A full startup reconciles the XO sweep and reports it.
  SQUAD_FAKE_HARNESS_PID=$$ run_session_start "$home" "$root" "$fakebin:$BASE_PATH" >/dev/null
  wait_for_network_stage "$home" "$root" \
    || fail "the full startup fixture's deferred network stage never published"
  network_report=$(network_stage_report "$home" "$root")
  assert_contains "$network_report" "XO_LIVENESS" \
  ...
  assert_not_contains "$reemit" "XO_LIVENESS" "--reemit repeated a mutating sweep startup already ran"
  assert_contains "$reemit" "done: queued after the re-emit too" "--reemit did not drain the stand-to queue"
  [ ! -s "$home/state/.stand-to-queue" ] || fail "--reemit left queued wakes behind: $(cat "$home/state/.stand-to-queue")"
  assert_contains "$reemit" "CONTEXT" "--reemit dropped the context digest"
  assert_contains "$reemit" "UNIT STATE" "--reemit dropped the unit-state digest"
...
EOF
  ...
  assert_not_contains "$reemit" "must leave restore work to the session holding the unit lock" \
  ...
  assert_contains "$readonly_out" "must leave restore work to the session holding the unit lock" \
...
# --- unit-state digest: no in-flight tasks ----------------------------------

test_fleet_digest_empty_fleet() {
  local rec root home fakebin out
  rec=$(new_world empty-unit)
  IFS='|' read -r root home fakebin <<EOF
$rec
EOF
  make_fake_toolchain "$fakebin"
  ...
  out=$(run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  assert_contains "$out" "(none)" "empty unit did not report (none) for in-flight tasks"
  assert_contains "$out" "absent" "empty unit's AFK section did not report absent"

  pass "an empty unit reports (none) for in-flight tasks and an absent AFK flag"
}
...
EOF
  ...
  printf 'SQX_PAIRING_TOKEN=tok-next-step\n' > "$home/.env"

  out=$(run_session_start "$home" "$root" "$fakebin:$BASE_PATH")

  assert_contains "$out" "SQX: X mode on" "bootstrap did not activate X mode"
...
EOF
  ...
  assert_contains "$out" "daemon owns the sentry" "next step did not delegate sentry ownership to the daemon"
  assert_contains "$out" "- Away mode: active" "supervision block did not include active AFK state"
  assert_not_contains "$out" "  bin/sq-sentry-arm.sh" "AFK next step still told the agent to arm the sentry directly"

  pass "next step delegates sentry ownership to the AFK daemon"
}
...
EOF
  ...
  out=$(SQUAD_FAKE_HARNESS=pi run_session_start "$home" "$root" "$fakebin:$BASE_PATH")
  ...
  assert_contains "$out" "restart plain pi so $root/.pi/extensions/sq-primary-turnend-guard.ts and $root/.pi/extensions/sq-primary-pi-watch.ts auto-load" "pi extension load diagnostic omits the turn-end guard extension"

  wake_line=$(printf '%s\n' "$out" | grep -n '^WAKE QUEUE$' | head -1 | cut -d: -f1)
  sup_line=$(printf '%s\n' "$out" | grep -n '^SUPERVISION OPERATING INSTRUCTIONS' | head -1 | cut -d: -f1)
  context_line=$(printf '%s\n' "$out" | grep -n '^CONTEXT$' | head -1 | cut -d: -f1)
  [ "$wake_line" -lt "$sup_line" ] || fail "supervision block did not follow stand-to queue"
...
EOF
  ...
  out=$(SQUAD_FAKE_HARNESS=pi-signed run_session_start "$home" "$root" "$fakebin:$BASE_PATH" pi-signed)
  ...
  assert_contains "$out" "restart pi-signed so $root/.pi/extensions/sq-primary-turnend-guard.ts and $root/.pi/extensions/sq-primary-pi-watch.ts auto-load" \
...
echo "# sq-session-start.test.sh: all assertions passed"
