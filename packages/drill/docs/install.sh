...
INSTALL_DIR="${DRILL_INSTALL_DIR:-$HOME/.drill/bin}"
LINK_DIR="${DRILL_LINK_DIR:-}"
...
BIN_PATH="$INSTALL_DIR/drill"
LINK_PATH="$LINK_DIR/drill"
...
FILENAME="drill-${VERSION}-${OS}-${ARCH}.tar.gz"
URL="https://github.com/${REPO}/releases/download/${VERSION}/${FILENAME}"

TMPDIR="$(mktemp -d)"
trap 'rm -rf "$TMPDIR"' EXIT

echo "Downloading drill ${VERSION} for ${OS}/${ARCH}..."
...
echo "drill ${VERSION} installed to ${BIN_PATH}"
