...
func TestRunInsertAndUpdatePreserveBuildIdentity(t *testing.T) {
	...
	if got.DrillVersion == nil || *got.DrillVersion != buildinfo.CurrentVersion() {
		t.Fatalf("drill version = %v, want %q", got.DrillVersion, buildinfo.CurrentVersion())
	}
	if got.DrillBuildSHA == nil || *got.DrillBuildSHA != buildinfo.Commit {
		t.Fatalf("drill build SHA = %v, want %q", got.DrillBuildSHA, buildinfo.Commit)
	}
}
