#!/usr/bin/env bash
# tests/sq-harness-liveness-drift-live-e2e.test.sh - opt-in drift guard proving
...
# tests/sq-tmux-agent-liveness.test.sh pins the classifier logic in CI. Run this
# guard after any harness upgrade and before trusting refreshed evidence.
set -u

if [ "${SQUAD_HARNESS_LIVENESS_DRIFT:-0}" != 1 ]; then
  echo "skip: set SQUAD_HARNESS_LIVENESS_DRIFT=1 to run the installed-harness liveness drift guard"
  exit 0
fi
...
SOCKET="sq-liveness-drift-$$"
LAB=$(mktemp -d "${TMPDIR:-/tmp}/sq-liveness-drift.XXXXXX")
SESSION=drift
...
# Kimi is not required to be on PATH; mirror bin/sq-spawn.sh's own resolution
# order so this guard covers the same binary Squad would actually launch.
